...
TL;DR: DocArena是一个完全自动化的数据生成管道，用于构建多模态文档搜索智能体的训练环境，无需人工标注。它通过MLLM处理文档、生成推理密集型QA对，并创建了一个包含79K个QA对的多语言数据集。


<details>
  <summary>Details</summary>
Motivation: 现有的搜索智能体训练方法主要基于文本环境，难以构建可控、可扩展且能处理多模态数据的训练环境。多模态文档搜索和问答的实际需求需要更好的训练环境。

Method: 1) 通过MLLM进行视觉感知来结构和索引文档；2) 分析和利用跨页面信息分布构建推理密集型QA对；3) 通过MLLM进行级联质量保证；4) 设计Doc-Search智能体架构，将视觉感知与策略模型解耦，让文本LLM作为多模态文档检索和QA的推理骨干。

Result: 创建了DocArena-79K数据集（包含8,336个文档，涵盖16个领域和49种语言）。在6个多模态文档场景和7个文本QA基准测试中，使用DocArena数据训练的智能体在检索准确率和QA质量方面表现最佳。

Conclusion: DocArena提供了一个完全自动化的解决方案来构建多模态文档搜索智能体的训练环境，无需人工标注。实验结果证明了该方法的有效性、可控性和泛化能力，在多模态文档检索和QA任务中取得了最佳性能。
...
TL;DR: 提出了一种结合图像处理和深度学习的混合方法，用于评估水果新鲜度，通过逻辑回归整合两种方法的输出，实现实时性能和高精度分类，减少了计算资源需求。


<details>
  <summary>Details</summary>
Motivation: 水果腐败是农业中的重要问题，导致巨大的经济损失。需要开发准确且高效的方法来评估水果新鲜度，为农业领域提供实用的解决方案。

Method: 开发了图像处理算法，将腐败程度量化为0（完全新鲜）到100（完全腐烂）的百分比。同时训练了卷积神经网络进行二元分类（新鲜或腐烂）。使用逻辑回归整合两种方法的输出，然后用逻辑回归模型使图像处理算法能够基于其百分比输出提供二元分类，从而在实时应用中无需CNN。

Result: 该方法在苹果和橙子数据集上验证，准确率超过90%，实现了实时性能，并且不需要高计算资源。图像处理算法现在可以直接基于逻辑回归模型提供二元分类。

Conclusion: 研究表明，将简单的图像处理技术与机器学习相结合，可以为农业领域提供实用的解决方案。主要限制是需要水果放置在白色或透明背景上，未来可通过先进的图像分割模型自动化背景去除来改进。
...
TL;DR: LCG是一个长上下文多图像文本到图像的生成框架，通过稀疏关系注意力机制和路由一致性约束来提高序列图像生成中的一致性和可扩展性，并构建了LCCD数据集进行训练和评估。


<details>
  <summary>Details</summary>
Motivation: 当前图像生成模型在单图像合成上表现优秀，但在漫画、故事板和视觉叙事等需要序列图像一致性的应用中表现不足，缺乏跨序列输出的连贯性。

Method: 1. 提出长上下文生成(LCG)框架；2. 采用稀疏关系注意力(SRA)机制选择性关注扩展视觉上下文中的核心特征；3. 引入路由一致性约束(RCC)利用身份感知掩码对齐不同生成分支的结构模式；4. 构建长上下文一致性数据集(LCCD)用于训练和评估。

Result: LCG在长上下文图像生成（包括多角色场景）的提示对齐和角色一致性方面优于基线方法。LCCD数据集包含600K训练序列和1K测试集，每个序列包含6-20张图像。

Conclusion: LCG框架通过稀疏关系注意力和路由一致性约束有效解决了长上下文多图像生成中的一致性问题，为序列视觉内容的生成提供了可扩展的解决方案。
...
TL;DR: 本研究开发了一个利用机载LiDAR和多光谱影像进行树冠级别城市森林地上生物量估算的框架，该框架无需人工标注，使用伪标签训练深度学习网络，在加拿大安大略省810平方公里区域内实现了高精度的生物量估算，并揭示了五年间的碳增益。


<details>
  <summary>Details</summary>
Motivation: 城市树木生物量的空间明确量化远低于管理森林，因为现有方法依赖清单或粗分辨率产品，无法解析单个树冠或精细尺度的异质性。需要开发能够提供树冠级别分辨率生物量估算的方法，以支持城市森林管理。

Method: 采用双流交叉注意力网络，基于规则生成的伪标签进行训练，从落叶期机载LiDAR（8-10脉冲/平方米）和近红外RGB正射影像（0.16-0.20米）中提取建筑物、针叶树和落叶树的语义分割。使用多尺度分水岭分割进行树冠划定，并通过树冠面积-高度的幂律代理（基于21,921棵清单树的物种特异性异速生长方程校准）估算地上生物量。

Result: 语义分割的全局平均精度、召回率和Dice分数分别为0.86、0.83和0.84。在18,713个清单-分割匹配对中，使用清单树冠几何的AGB预测R²=0.609，操作分割下的R²=0.570。30米聚合尺度下，2018年和2023年总AGB储量分别为1.73 Tg和1.81 Tg（811-850 Gg C），尼亚加拉悬崖地区局部密度高达~140 Mg/ha，五年间净碳增益为39 Gg C。深度集成不确定性地图识别了与代表性不足土地覆盖相关的高认知不确定性区域。

Conclusion: 该框架使用标准的省级数据，无需人工标注，能够在管理相关分辨率下为森林外树木生成公开的双时相树冠级别AGB数据库。树冠划定被确定为剩余的不确定性来源，而深度集成不确定性地图可用于指导不确定树冠的异速生长方程分配。
...
TL;DR: 本文提出了一种多任务深度学习模型，利用熔池图像和工艺参数高精度预测激光深熔焊的熔透状态、深度和焊缝形貌，为焊接质量原位控制提供了新方法。


<details>
  <summary>Details</summary>
Motivation: 激光深熔焊中熔透状态和焊缝形貌的评估对焊接质量至关重要，传统方法难以实现精确的原位监测和预测，需要开发新的智能监测技术。

Method: 提出了创新的多任务深度学习模型，集成了从熔池图像提取的时空特征和工艺参数。采用基于卷积神经网络和状态空间模型的深度学习框架，并提出了可靠的数据集构建方法以增强模型的鲁棒性和泛化能力。

Result: 测试集验证显示：熔透状态预测准确率达99.35%，熔深预测误差为1.79毫米，焊缝横截面重构准确率达95.65%。

Conclusion: 该研究为激光深熔焊系统的原位质量控制策略提供了新的见解和方法论，实现了对焊接质量关键参数的高精度预测和监测。
...
TL;DR: 该研究将fMRI认知任务图谱从单源扩展到多源转移，使用布尔整数规划分析23个任务状态，揭示了运动任务的跨范式受限集群和工作记忆任务在预算约束下的高优先级。


<details>
  <summary>Details</summary>
Motivation: 掩码fMRI重建提供了量化任务状态间转移关系的自监督方法，但现有基于重建的任务图谱主要研究单源到目标的转移，需要扩展到多源转移以更全面地理解任务关系。

Method: 扩展了fMRI认知任务图谱，从单源转移到多源转移，分析了23个Human Connectome Project任务状态。使用了布尔整数规划（BIP）分析预算约束的任务分配，训练了1,127个任务特定和转移模型。

Result: 单源转移具有方向性和范式结构：运动状态在运动范式内转移良好，但对大多数非运动目标支持有限。多源转移取决于源集的组成，表明多对一任务关系不能完全由成对任务图谱捕捉。在监督预算约束下，BIP反复分配直接监督给0-back和2-back工作记忆状态。

Conclusion: 研究揭示了跨范式受限的运动集群和在指定全局分配目标下具有高优先级的工作记忆状态。将基于重建的fMRI任务图谱从一对一扩展到多对一任务关系和预算约束的任务依赖。
...
TL;DR: 提出了一种基于信息论的门控熵不确定性自适应路由方法（GeMoE），用于大型视觉语言模型的混合专家架构，通过最小描述长度原理自适应选择专家数量，在保持性能的同时显著提高专家激活稀疏性。


<details>
  <summary>Details</summary>
Motivation: 传统混合专家架构使用Top-k静态路由策略，无法根据输入变化自适应选择专家数量，导致资源利用率不佳。需要一种更智能的路由机制来优化计算资源分配。

Method: 将token路由视为信息编码任务，基于最小描述长度原理设计动态路由。通过验证MDL与门控熵在MoE场景中的联系，提出基于门控熵的不确定性感知自适应路由（GeMoE），利用门控熵评估token复杂度并自适应确定每个token应使用的专家数量。

Result: 在广泛的骨干网络和基准测试中，GeMoE达到了原始静态路由99.5%的平均性能保留，同时将平均专家激活稀疏性提高了36.5%。

Conclusion: 基于信息论的GeMoE方法能够有效平衡模型复杂度和性能，为混合专家架构提供了更高效的自适应路由机制，显著提升了资源利用率。
...
TL;DR: 提出Turbid Underwater Baseline (TUB)数据集和PCD指标，用于量化浑浊水下环境中计算机视觉模型性能下降问题


<details>
  <summary>Details</summary>
Motivation: 浑浊水下环境严重降低能见度，但计算机视觉模型在这些条件下的性能影响尚不清楚。现有研究主要依赖可能不能准确反映真实世界信息损失的合成浊度数据集。

Method: 1) 收集了TUB数据集，包含1,320张极端浑浊条件下拍摄的图像和超过16,000个高置信度的分割掩码；2) 提出了PCD（基于相位一致性映射推导的）指标，该指标对对比度不变，旨在捕捉真实浊度中的结构信息损失

Result: PCD指标与实例分割模型在真实和合成浑浊图像上的性能表现有强相关性，而该领域常用的指标显示出弱相关性甚至无相关性

Conclusion: TUB数据集和PCD指标为研究浑浊水下环境中的计算机视觉性能提供了重要基准，PCD能够有效量化结构信息损失并预测模型性能下降
...
TL;DR: 本文提出了一种混合融合方案用于视觉提示调优，通过可微分架构搜索联合优化可学习提示及其融合方案，在34个数据集上实现了优于基线方法的性能。


<details>
  <summary>Details</summary>
Motivation: 研究视觉提示调优中的基础问题：单一融合方案（拼接或加法）是否总是最优，以及开发混合融合方案是否有益。现有方法主要使用拼接或加法融合图像和提示标记，但这种单一方案可能不是最优选择。

Method: 将问题形式化为双层优化问题，使用可微分架构搜索联合优化可学习提示及其融合方案。在搜索空间中除了拼接和加法外，还提出了两种新融合方案：仿射变换和交叉注意力，以丰富搜索空间。

Result: 在涵盖VTAB-1k、FGVC和HTA的34个数据集上进行广泛实验，结果显示相比提示调优基线方法获得了一致的性能提升。在冻结的ViT骨干网络下，相比VPT-Deep及其变体，本方法在准确性、延迟和参数量之间取得了更好的权衡。

Conclusion: 研究发现提示如何与图像标记融合在视觉提示调优中起重要作用，混合融合方式能更有效地利用ViT的层级语义，为视觉提示调优研究提供了新视角。
...
TL;DR: 研究发现当前深度伪造检测基准主要评估的是通用模态理解能力而非法证理解能力，通过线性探针在自监督表示上的表现可逼近专用检测器性能，表明基准可能无法反映真实威胁模型。


<details>
  <summary>Details</summary>
Motivation: 随着深度伪造生成器接近感知上的不可区分性，可靠检测变得至关重要。然而，在基准测试中表现出色的检测器在现实世界中经常失败。存在一个令人担忧的反馈循环：基准测试推动着越来越复杂、精心设计的检测器，但如果这些基准不能反映真实的深度伪造，这种复杂性可能完全解决了错误的问题。

Method: 采用一种故意简化的诊断方法：如果在冻结的通用自监督表示上使用线性探针能够逼近专用检测器的性能，则基准主要奖励的是通用模态理解而非法证理解。研究在视频、图像和音频三种模态上进行了基准审计。

Result: 跨三种模态观察到，基于通用自监督表示的线性探针性能与专用检测器接近。进一步研究发现，生成器级别的难度部分可以通过同一表示空间中的Frechet几何来解释。

Conclusion: 研究支持采用基准审计视角看待深度伪造检测：在将高分解读为法证理解的证据之前，值得询问基准中有多少已经通过通用表示解决。这表明当前的检测基准可能需要重新评估，以确保它们真正反映真实的深度伪造检测挑战。
...
TL;DR: 提出VIGIL方法，一个强化学习后训练框架，通过最大化视觉输入与生成响应之间的互信息来解决MLLM中的视觉懒惰问题，显著减少幻觉并提升推理能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型虽然继承了LLM的强推理能力，但容易出现与视觉输入矛盾的幻觉。机制研究表明这一问题源于视觉懒惰：MLLMs内部编码了正确的视觉证据，但在响应时过度依赖语言先验。现有的对齐方法（如直接偏好优化）主要基于文本优化结果级奖励，这引入了对语言捷径的优化偏差，导致响应经常与视觉证据相矛盾。

Method: 提出视觉信息增益对齐框架VIGIL，这是一个强化学习后训练框架，将重点从数值奖励拟合转向因果视觉基础。VIGIL引入几何约束，显式最大化视觉输入与生成响应之间的互信息。通过惩罚"盲目置信"实例来实现这一点，即在文本-视觉注意力被掩码以创建反事实盲状态时，模型仍然不恰当地保持置信。

Result: 实验表明VIGIL在幻觉和推理基准上始终优于现有对齐方法，且不损害纯文本能力。该方法仅使用25%的偏好数据就能达到最先进方法的全数据性能，甚至在没有显式边界框监督的情况下展现出新兴的空间基础能力。

Conclusion: VIGIL通过关注视觉信息的因果基础而非简单的奖励拟合，有效解决了MLLM中的视觉懒惰问题，为多模态对齐提供了更稳健的解决方案，同时保持了模型的语言能力。
...
TL;DR: DinoLink是一种针对车联网V2X带宽限制的语义通信压缩框架，通过双稀疏架构实现139倍比特率降低，在窄带环境中实现34.5倍加速。


<details>
  <summary>Details</summary>
Motivation: 车联网V2X网络存在严重的带宽限制，阻碍了高精度远程感知的实现，需要一种高效的压缩解决方案。

Method: 提出双稀疏架构：1) 显著性感知选择器修剪冗余背景token；2) 残差向量量化模块将特征压缩为紧凑的码书索引。仅传输轻量级索引和位置先验信息。

Result: 相比未压缩传输实现139倍比特率降低，在nuScenes数据集上保持32.8% mAP的竞争性性能。在LoRa等窄带环境中实现34.5倍加速。

Conclusion: DinoLink是一种稳健、带宽高效的远程感知前端，适用于受限V2X场景，为车云协同推理提供了有效的解决方案。
...
TL;DR: 提出一个自监督框架，从视频信号学习隐式3D物理动力学，将预测瓶颈从2D图像空间提升到3D体素化潜在空间，实现统一物理现象模拟


<details>
  <summary>Details</summary>
Motivation: 当前生成视频模型虽视觉保真度高，但缺乏3D几何基础，常导致物理不一致性和物体持续性保持失败。需要建立具有3D几何基础的物理一致动态世界模型

Method: 1) 将V-JEPA视频联合嵌入预测架构的语义特征反投影到体素化网格，结合单目深度先验；2) 提出体素特征对流方法，学习动作条件转换算子，将物理视为时空状态对流问题；3) 在高维V-JEPA特征中隐式跟踪物质状态，无需显式物理模拟器

Result: 模型仅通过端到端视频信号和动作条件监督，无需物理引擎内部状态、标签或代理模型，在多个基准测试(CLEVERER, PhysInOne, PhysGaia)上展现出良好的长期结构稳定性和物理合理性

Conclusion: 这项工作为通用动态世界模型开辟了一条可扩展路径，通过单目视频被动观察来内化物理世界的3D不变性，为学习隐式3D物理动力学提供了新框架
...
TL;DR: 本文提出了一种结合特征引导甲烷增强机制的多模态深度学习模型，用于遥感图像中的甲烷羽流检测和分割，在保持高精度的同时显著降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 甲烷羽流的有效检测对于理解和缓解全球变暖至关重要，准确识别和分割地球观测图像中的甲烷羽流对于大规模监测具有重要意义。

Method: 提出了一种多模态深度学习模型，集成了特征引导甲烷增强机制，该机制将物理上有意义的甲烷线索注入到基于Transformer的RGB表示中的多个语义尺度。

Result: 在MPDataset上的评估表明，该方法在MIoU、MPrecision和Recall指标上分别比现有最佳方法提高了+0.92、+0.87和+1.01，并且计算成本显著低于其他高性能架构，实现了良好的精度-效率平衡。

Conclusion: 这些结果突显了高效多模态融合策略在实际遥感应用中实现准确和可扩展的甲烷羽流分割的潜力。
...
TL;DR: APRTrack是一种用于RGB-事件跟踪的层次化扰动与检索框架，通过对抗性扰动模拟信号退化，采用分层路由机制避免特征坍塌，并使用足迹引导的通道校准Hopfield检索进行历史信息补偿，以提升在部分目标缺失和模态退化情况下的跟踪鲁棒性。


<details>
  <summary>Details</summary>
Motivation: RGB-事件跟踪融合了RGB外观纹理和事件传感器的密集时间运动线索，但现实场景中存在各种结构化信号退化问题。在恶劣环境中，任一模态都可能严重失去可靠性，目标常因遮挡、边缘截断和前景干扰而出现不完整，传统多模态融合方法难以应对这些挑战。

Method: 1) 通过两个对抗性扰动分支在模态和空间级别构建结构化退化，分别模拟全模态失效和局部目标区域缺失；2) 设计分层路由机制分离两种扰动类型的训练流程，避免叠加退化约束引起的特征坍塌；3) 提出足迹引导的通道校准Hopfield检索(FCHR)模块，基于查询与记忆库之间的关联足迹评估检索置信度，在Hopfield匹配前校准检索度量空间，实现对目标区域边界内历史特征的可控补偿。

Result: 在FE108、COESOT、VisEvent和FELT数据集上的广泛实验证明了所提策略对RGB-事件视觉对象跟踪的有效性。

Conclusion: APRTrack通过层次化扰动模拟和可控历史信息检索，显著提升了RGB-事件跟踪在部分目标缺失和模态退化情况下的鲁棒性，为现实世界中的多模态跟踪提供了有效的解决方案。
...
TL;DR: FFN（Frame Forgetting Network）提出了一种高效的长视频测试时间训练方法，通过仅处理三帧滑动窗口和自适应窗口算法，解决了现有方法计算量随窗口大小线性增长的问题。


<details>
  <summary>Details</summary>
Motivation: 现有测试时间训练方法在处理长视频时面临两个主要问题：1）使用包含过去帧的滑动窗口进行TTT更新，计算量随窗口大小线性增长，在处理长达数小时的视频时计算上不可行；2）即使时间上接近的帧看起来相似也执行TTT，消耗大量计算资源。

Method: 提出了Frame Forgetting Network（FFN）：1）仅在滑动窗口中处理三帧（退出帧、当前帧和后续帧），但仍能保留时间上下文并处理长视频；2）数学定义"惊喜度"度量，衡量输入帧相对于过去帧包含的新信息量，基于此开发自适应窗口算法动态调整有效窗口大小。此外，创建了EpicTours数据集，包含长达3小时的步行城市游览视频。

Result: FFN在密集分割、视频分类任务上展示了实证有效性，能够泛化到深度估计任务，并成功处理多小时的超长视频。相对于现有方法，显著降低了计算复杂度。

Conclusion: FFN通过创新的三帧窗口处理和自适应窗口算法，实现了高效的长视频测试时间训练，解决了现有方法计算不可行的问题，为长视频分析和在线适应提供了实用的解决方案。
...
TL;DR: CRISP是一种新颖的结构诊断评估范式，通过一致性评估视觉空间智能，揭示视觉语言模型中感知与推理的系统性脱节问题。


<details>
  <summary>Details</summary>
Motivation: 当前的视觉语言模型评估往往将语言先验与真正的空间推理混淆，缺乏能够区分潜在推理能力和感知瓶颈的诊断方法。

Method: CRISP利用度量3D场景图和oracle干预协议，通过评估隐式感知与显式推理之间的一致性来解耦潜在推理能力和感知瓶颈。

Result: 研究发现专有模型拥有强大的潜在推理引擎但存在度量估计不准确和未能有效利用隐式结构表示的问题；开源模型则受限于缺乏多跳组合推理能力。

Conclusion: CRISP通过从依赖语言先验的"猜测正确"转向真正的"感知、验证和推理"，为超越端到端后训练的多模态对齐提供了严格的路线图。
...
TL;DR: ForeAgent是一个用于AI生成图像检测的自主取证框架，采用感知-判决架构和多视图特征聚合，通过事后驱动的自我优化策略实现迭代进化，在多个基准测试中达到最先进的检测性能。


<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展对现有深度伪造检测方法构成重大挑战，特别是高度逼真的AI生成图像的广泛传播。尽管多模态大语言模型(MLLMs)在此任务中显示出强大潜力，但现有方法存在两个关键限制：对细粒度取证伪影的敏感性不足，以及依赖前沿模型的静态合成监督，导致灵活性有限和高成本。

Method: 1. 采用感知-判决架构，聚合语义、空间和频域特征的多视图线索，利用MLLM作为判决模块融合这些信号进行逻辑基础判决。2. 引入事后驱动的自我优化策略，遵循采样-反思-进化范式：对训练实例进行推理推演，基于真实标签作为事后反馈，反思失败案例和低质量推理轨迹，重新生成更高质量的推理轨迹。3. 通过双重专家质量门控模块严格筛选合成的样本，在自策划的高质量样本上通过微调持续进化。

Result: 1. 在Chameleon基准测试中达到82.18%准确率（比AIDE提高+16.41%）。2. 在AIGCDetect-Benchmark上，跨16个生成器实现93.3%的平均准确率。3. 外部评估显示，与GPT-5和GPT-5-mini相比，ForeAgent产生更一致和因果基础更强的推理。

Conclusion: ForeAgent通过感知-判决架构和事后驱动的自我优化策略，有效解决了现有MLLM方法在AI生成图像检测中的局限性，实现了持续自我进化的能力，在多个基准测试中展现了卓越的性能和鲁棒性，为深度伪造检测提供了新的解决方案。
...
TL;DR: 提出一种基于从粗到细策略的混合自监督非刚性3D形状匹配方法，采用双分支架构结合拉普拉斯基和弹性基的功能映射学习，并通过对比能量提升特征判别能力


<details>
  <summary>Details</summary>
Motivation: 非刚性3D形状匹配是计算机视觉和图形学的基础任务，传统方法在处理非等距变形和拓扑噪声等挑战性场景时存在局限，需要开发更鲁棒和高效的匹配方法

Method: 1) 采用从粗到细策略确保粗映射与细化对应关系的一致性；2) 设计双分支架构，包含两个对称的功能映射学习流：一个基于拉普拉斯基，另一个使用弹性基；3) 集成对比能量来促进特征判别；4) 自监督学习框架

Result: 方法在多种挑战性场景（包括非等距变形和拓扑噪声）下实现了最先进的性能，同时保持了计算效率。对比能量能够提升特征判别能力，并且与现有方法集成时也能带来一致的改进

Conclusion: 所提出的混合自监督方法在非刚性3D形状匹配任务中表现优异，证明了从粗到细策略、双分支功能映射架构以及对比能量的有效性，为相关领域提供了新的解决方案
...
TL;DR: HierBias：一种层次化上下文条件媒体偏见检测器，通过建模文档级上下文信息和多任务学习，显著提升句子级偏见检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有句子级偏见检测方法独立分类每个句子，忽略了人类标注者自然利用的句子间上下文信号。文档级上下文对准确识别偏见至关重要但未被有效建模。

Method: 提出上下文条件偏见概率概念，理论上证明利用文档上下文能严格降低句子分类的贝叶斯误差。采用多任务学习框架，结合二元偏见检测和四类偏见类型分类。架构上使用句子级RoBERTa编码器、跨句子Transformer聚合器和双输出头。

Result: 在BABE和BASIL数据集上评估，HierBias达到0.853 F1和0.723 MCC分数，超越现有最佳方法+2.6% F1和+4.3% MCC（McNemar检验，p<0.05）。消融实验证实每个理论组件都有独立一致的贡献。

Conclusion: HierBias通过形式化建模文档上下文和多任务学习，为媒体偏见检测提供了理论基础和有效架构，显著提升了检测性能，证明了利用句子间互信息和联合训练的重要性。
...
TL;DR: 本文提出了Know2Guess基准，这是一个污染感知的多区域基准测试，用于衡量LLM从可回答知识到应弃权未知领域的过渡能力，区分支持性回答与无根据猜测。


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估存在混淆问题：难以区分支持性回答与无根据猜测，同时无法分离数据污染、提示特异性和通用拒绝行为的影响。需要专门的评估框架来明确衡量LLM在可回答与应弃权情况下的可靠性。

Method: 构建包含1200个项目、跨越5个领域的污染感知多区域基准，带有明确的弃权期望和污染风险元数据。使用双重解析策略（官方严格解析器+标准化鲁棒性解析器）。评估FLAN-T5、Qwen2.5-Instruct和Llama-3-Instruct模型在锁定回答或弃权提示、仅回答控制以及提示模板变体下的表现。

Result: 基准不被通用非回答行为解决：FLAN基线在建设性弃权方面表现较弱，而更强的指令调优模型显示出选择性但不完全的回答到弃权过渡。Qwen2.5-3B-Instruct实现最佳整体可靠性，但回答期望区域仍困难，校准较差，良性项目拒绝持续存在。提示和解析器鲁棒性分析保持了主要排名和定性结论。

Conclusion: 该基准为审计可回答性、弃权、拒绝和污染这四个相互关联但不同的LLM可靠性维度提供了可复现的协议。数据集已公开可用。
...
TL;DR: 研究发现，在已通过中期训练植入动物同情价值观的Llama 3.1 8B模型中，助人导向的后训练会显著损害动物同情心，而编码导向的后训练则能更好地保护这些价值观。助人训练同时会损害一般道德推理能力，但这种影响不跨语言转移。


<details>
  <summary>Details</summary>
Motivation: 标准的后训练流程（监督微调和强化学习）旨在使语言模型更有帮助，但这些过程可能无意中损害预训练中植入的价值观。研究旨在探索后训练数据领域（助人vs编码）是否会对已植入动物同情价值观的中期训练模型产生不同的影响。

Method: 使用在同情导向合成数据上中期训练的Llama 3.1 8B模型，比较两种后训练领域：助人训练（使用Dolly-15k和RLHFlow数据）和编码训练（使用Magicoder-110K数据）。评估采用两个基准：动物伤害基准AHB 2.2和不确定性下道德推理基准MORU，包括英语和多语言项目。

Result: 1. 助人训练显著损害动物同情心：AHB上SFT训练助人组35.7% vs 编码组65.2%；GRPO训练助人组18.7% vs 编码组32.0%。
2. 助人训练损害一般道德推理：英语MORU项目上助人组46.4% vs 编码组71.9%，差距达25.5个百分点。
3. 跨语言差异：道德推理损害不跨语言转移（多语言MORU上SFT助人组52.3% vs 编码组51.2%），而动物同情效应跨语言一致存在，非英语项目中编码组的优势比英语项目大4.5倍。

Conclusion: 通过中期训练植入的价值观比领域特定后训练获得的推理改进编码更深、更跨语言。对于在价值导向中期训练基础上构建模型的实验室，编码领域的后训练可能比助人导向的后训练更好地保护中期训练植入的价值观，同时不会损害一般推理能力。
...
TL;DR: 本研究采用模型蒸馏方法评估LLMs在静力学问题上的解决能力，发现文本题表现良好，但加入图表和多步推理后性能下降，主要原因是多步推理困难而非图像识别限制。


<details>
  <summary>Details</summary>
Motivation: 现有研究多使用公开问题数据集，缺乏主题特异性分析。在工程教育（特别是机械工程）领域，对LLMs在特定问题类型上的系统研究仍然有限，需要更专门化的评估方法。

Method: 采用模型蒸馏方法而非直接提问传统教科书问题。通过蒸馏ChatGPT提取了25个纯文本静力学问题，并进一步构建两个附加数据集：添加图表的数据集和修改数值的数据集。

Result: 实验结果表明：1）LLMs在纯文本静力学问题上表现良好；2）当引入图表时准确性下降；3）需要多步推理的问题性能显著降低。性能下降的主要原因是多步推理困难以及在连续解决阶段一致应用提取的视觉信息的能力不足，而非图像识别限制。

Conclusion: 本研究揭示了LLMs在工程教育中解决涉及多步推理和视觉信息整合的复杂问题时的局限性，为未来改进LLMs在特定领域问题解决能力提供了重要见解。
...
TL;DR: 通过实验测试不同语言特征对语言模型在动物福利立场倾向的影响，发现明确表达立场的语言特征能增强模型支持动物福利的推理倾向。


<details>
  <summary>Details</summary>
Motivation: 随着动物福利倡导者的文本越来越多地用于训练语言模型，了解不同语言特征如何影响模型在动物福利问题上的立场倾向具有重要实践意义，可以帮助倡导者更有效地撰写能影响AI模型的文本。

Method: 使用词汇匹配的立场对比探针，在保留的动物福利基准上测试十个语言特征，测量它们如何改变Llama-3.2-1B模型对动物福利支持立场的偏好。这些特征包括：断言确定性、明确的道德词汇、情感词汇、评价性主张、叙事结构、描述的伤害严重性、即时时间框架、谨慎语言、具体感官描述和第一人称视角。

Result: 十个特征中有八个产生统计学上显著的偏移：七个特征使模型更倾向于支持动物福利的推理（断言确定性、明确道德词汇、情感词汇、评价性主张、叙事结构、伤害严重性描述、即时时间框架）；两个特征产生相反效果（谨慎语言和具体感官描述稀释了支持动物福利的立场）；第一人称视角没有显著影响。

Conclusion: 撰写动物福利文本时应明确表达立场而非中性描述场景。使模型立场偏移的特征是那些明确表达作者立场的特征，而稀释立场的特征虽然包含动物福利内容但不表达明确立场。实践建议是：撰写可能用于LLM训练语料库的动物福利文本时，应当明确断言立场而非中性描述场景。
...
TL;DR: ContextForge是一种用于长对话的上下文循环系统，通过结构化查询生成、外部记忆检索和受控合成来维持任务相关信息，减少令牌开销同时保持回答质量


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在短上下文推理方面表现出色，但在长对话中性能下降，主要受限于上下文窗口大小和令牌使用效率低下

Method: 提出ContextForge系统，结合结构化查询生成、外部记忆检索和受控合成，实现上下文循环，高效重用先前计算而不依赖完整上下文重播

Result: 在15轮对话基准测试中，ContextForge相比使用相同基础模型的基线代理，展示了改进的一致性和减少的令牌消耗，同时保持可比的响应准确性

Conclusion: 上下文循环提供了一种实用方法，可在不扩大上下文窗口或重新训练模型的情况下，扩展LLM在长期任务中的能力
...
TL;DR: 通过对大型语言模型实施非暴力沟通约束提示，可以有效降低对话冲突升级，在冲突情境中提升模型对话的可信度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地应用于情感紧张的情境中，如人际冲突、挫败和困境。以往的安全研究主要关注防止显性危害（如有毒或违反政策的内容），但对于可能无意中升级冲突的对话行为关注较少。

Method: 将非暴力沟通原则重新定义为过程导向的指导方针：避免责备归因、强调关注用户情感体验、鼓励建议前先澄清。使用双代理模拟框架，在多种指令调优模型和用户抵抗水平下测试这些约束。

Result: NVC约束提示持续减少对话冲突升级，并稳定与高度抵抗用户的互动。这些约束显著提高了LLM在易冲突情境中对话的可信度。

Conclusion: 简单的沟通约束能够显著改善大型语言模型在冲突倾向情境中的对话可信度。非暴力沟通原则为LLM的安全部署提供了有效的轻量级提示级干预方法。
...
TL;DR: NEST-V1是一个多模态框架，能够从语音输入生成情感条件化的尼泊尔手语虚拟形象，为低资源语言环境下情感感知的手语翻译系统建立了技术基础。


<details>
  <summary>Details</summary>
Motivation: 当前手语通信系统在情感表达方面研究不足，特别是针对低资源语言。需要开发能够整合情感表达的手语通信系统，特别是为尼泊尔语这样的低资源语言。

Method: 采用轻量级多模态框架，使用共享声学编码器同时进行自动语音识别和情感分类。系统处理四个常见尼泊尔词语和三种情感状态，在50位说话者的600个标记音频样本上验证技术方案。

Result: 系统实现81.1%的ASR准确率和79.21%的情感识别准确率。相比独立模型架构，参数效率提高37%，仅使用22.1M参数，适合边缘部署。验证了情感条件化手语虚拟形象生成的可行性。

Conclusion: 该试点研究证明了在低资源环境下实时、情感表达丰富的手语通信系统的可行性，为未来扩展到更大词汇量和更多样化情感表达提供了可扩展框架。
...
TL;DR: 本文开发了AdvCluster自动化框架来研究大语言模型相对于小模型在推理能力上的优势，发现约束引导推理是大模型的核心优势模式


<details>
  <summary>Details</summary>
Motivation: 大语言模型在推理基准测试中持续优于小模型，但这种性能差距背后的推理差异尚未得到充分探索。研究人员观察到在不同领域（数学、物理、化学、编程）的基准测试中，大模型相比小模型都有稳定的性能优势。

Method: 开发了AdvCluster自动化框架：1）识别大模型有稳定优势的问题；2）从大小模型配对推理痕迹中提取细粒度优势描述；3）通过语义聚类组织这些描述，并由评审模型指导进行定量评估和选择。

Result: 研究得出了大模型推理优势的系统分类法，涵盖跨领域的常见优势和特定领域的专业优势。核心发现是约束引导推理：大模型更擅长识别显性和隐性约束，将其组织成结构化推理，并用这些约束排除不可行路径和验证中间步骤。

Conclusion: 约束引导推理是大语言模型相对于小模型在复杂推理任务中的核心优势，AdvCluster框架为理解模型规模对推理能力的影响提供了系统化的分析方法。
...
TL;DR: 提出一种利用结构化语言资源（如WordNet）为低资源语言开发专业对话AI的系统方法，将印地语WordNet转换为125万指令-响应对，通过资源高效的LoRA和4位量化微调12B参数模型，在印地语学习聊天机器人中展现优越的教学效果。


<details>
  <summary>Details</summary>
Motivation: 低资源语言在AI开发中面临关键挑战：缺乏大规模训练语料库来创建专业对话系统。需要找到不依赖海量语料的实用替代方案，解决低资源语言AI可及性问题。

Method: 1) 将专家编纂的词汇数据库（印地语WordNet）系统性地转换为125万个多样化指令-响应对；2) 使用资源高效的LoRA技术和4位量化方法微调12B参数的语言模型；3) 通过印地语学习聊天机器人进行系统评估。

Result: 基于结构化知识的系统在教学效果上显著优于通用模型（91.0 vs. 79.4-83.6），同时在语义性能和一致性方面保持竞争力。该方法为数百种拥有WordNet资源的语言提供了一套概念验证的开发管道。

Conclusion: 结构化语言资源（如WordNet）可以作为低资源语言专业对话AI开发的有效基础，提供了一种实用且资源高效的替代方案，解决了语料密集型方法的局限，有望推动数百种语言的专门化AI系统发展。
...
TL;DR: Dynamic-dLLM是一个无需训练的训练框架，通过动态缓存更新和自适应并行解码来加速扩散大语言模型的推理，实现3倍以上的平均加速，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(dLLMs)在序列长度L上具有L^3的计算复杂度，缺乏键值缓存兼容性且去噪步骤是非自回归的，这给长序列和实时应用带来挑战。现有加速方法使用静态缓存或并行解码，未能考虑不同层和解码步骤中token属性的动态变化。

Method: 提出Dynamic-dLLM框架，包含两个核心组件：1)动态缓存更新(DCU)，基于层间token动态自适应分配缓存更新预算；2)自适应并行解码(APD)，动态校准解码阈值以平衡生成质量和效率。

Result: 在LLaDA-8B-Instruct、LLaDA-1.5和Dream-v0-7B-Instruct等模型上，通过MMLU、GSM8K和HumanEval等基准测试验证，Dynamic-dLLM实现了超过3倍的平均加速，在保持性能的同时超越了最先进的加速方法。

Conclusion: Dynamic-dLLM提供了一个即插即用的高效dLLM部署解决方案，无需牺牲性能即可显著提升推理速度，解决了扩散大语言模型在推理效率方面的关键挑战。
...
TL;DR: 这篇论文研究了大型语言模型（LLMs）在无额外提示情况下对研究方法的建议倾向，发现LLMs会显著窄化方法选择空间，特别是过度偏好主流提供商而忽视学术/社区模型。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs越来越多地被用于指导研究方法，但它们在最小提示下的默认方法论倾向仍不清楚。研究者希望了解LLMs是否会在无意识中引导研究者走向特定的、可能受限的方法路径。

Method: 研究使用GPT-5.1、Gemini 3 Pro和DeepSeek-V3.2三个模型，从1000篇arXiv计算机科学论文中提取研究问题，仅提供问题给LLMs获取方法建议。将建议与论文实际方法进行比较，提取结构化特征并映射到共享分类体系，量化多个维度的差异。

Result: LLMs在提供者选择上表现出最强的失衡（Jensen-Shannon发散度比其他维度高3-5倍），学术/社区模型被低估23-24个百分点。LLMs建议的方法范围显著窄化：模型实体从1232个减少到59-96个。LLM间的排名相关性（0.55-0.68）高于LLM与论文的相关性（0.33-0.56）。

Conclusion: LLMs的方法建议存在系统性偏倚，倾向于主流选择而忽视多样性。研究者在依赖LLM建议时如果不进行交叉验证，可能会将方法搜索空间限制在更集中的默认选项中，从而影响研究的创新性和多样性。
...
TL;DR: 本文对多模态大语言模型中的统一视觉语言感知进行了首次系统性综述，将其视为类似人类固有感知的内在统一能力，提出了五阶段分类法来追踪发展脉络，并指出了未来的研究方向。


<details>
  <summary>Details</summary>
Motivation: 现有综述往往将视觉和语言分开讨论，缺乏从统一模态视角审视多模态大语言模型感知能力的系统性研究。随着OpenAI O系列和DeepSeek R系列等模型的推出，感知中心智能范式发生转变，需要填补这一研究空白。

Method: 1) 将MLLM感知形式化为内在的统一视觉语言能力；2) 引入五阶段分类法追踪MLLM感知的范式演变；3) 在每个阶段调查代表性方法和里程碑。

Result: 构建了首个统一视觉语言感知的系统性综述框架，提供了从人类固有感知视角理解MLLM感知的理论基础，以及追踪其范式演变的分类体系。

Conclusion: 该研究填补了MLLM统一感知领域的系统性综述空白，为理解多模态智能的发展提供了基础框架和行动路线图，有助于推动通向人工通用智能的创新研究。
...
TL;DR: 本研究基于北京普通话和台湾普通话的自发对话语料库，通过语料库分析证明轻声具有独立的调值目标，并提出轻声本质上是一种词汇声调而非减缩或"无声调"。


<details>
  <summary>Details</summary>
Motivation: 普通话的轻声（中性调）具有神秘的性质：它被描述为减缩调，有时是词汇固定的，有时可能无声调。在两音节词中只出现在第二音节，但单音节词也可以有轻声。研究者希望通过实证研究澄清轻声的本质特性。

Method: 基于北京普通话和台湾普通话自发对话语料库的语料库研究，分析轻声的语音实现。使用上下文嵌入（contextualized embeddings）来预测词汇特定的音高特征。

Result: 1) 轻声像四个词汇声调一样具有自己的调值目标；2) 带轻声的双音节词具有依赖于第一音节声调的音高轮廓；3) 轻声词具有词汇特定的音高特征；4) 这些特征在一定程度上可以从词的上下文嵌入中预测；5) 轻声在两个变体（北京普通话和台湾普通话）中存在相似性和差异，部分差异可能源于两个语料库中词汇用法的意义差异。

Conclusion: 鉴于词汇声调的实现也存在相当大的变异性，研究者提出轻声实际上在北京普通话和台湾普通话中都是一种词汇声调。研究记录了两种变体中轻声实现的异同，并使用上下文嵌入提供证据表明部分观察到的差异可能源于两个语料库中词汇用法的意义差异。
...
TL;DR: 使用小型语言模型(SLMs)协助社交-物理人机交互(spHRI)领域系统综述的文献筛选，相比人工更快速，能识别部分遗漏文献，但不能完全替代专家评审。


<details>
  <summary>Details</summary>
Motivation: 社交-物理人机交互(spHRI)研究快速发展，但术语碎片化、方法不一致使得系统综述困难，需要可扩展的综述方法来应对大规模文献。

Method: 评估小型语言模型(SLMs; < 1.5B参数)在大型spHRI系统综述中辅助标题和摘要筛选的能力，使用本地部署的SLMs以远快于人工的速度处理文献，并采用SLM集成策略。

Result: SLMs性能不及人工评审，但筛选速度极快；SLM集成发现了39篇人工评审遗漏的论文，占最终相关数据集的10.29%。

Conclusion: 小型语言模型(SLMs)可以辅助而非替代专家评审，使大规模文献综述更加可行和可持续。
...
TL;DR: PROFILEFOUNDRY是一个确定性的合成人物数据生成器和固定参考发布，包含100,000个成年合成人物对象，具有跨字段和时间一致性，用于基础模型评估。


<details>
  <summary>Details</summary>
Motivation: 基础模型研究需要人物数据（用户状态、个人历史、关系等），但真实用户数据难以共享、扰动、审计或负责任地重新分发，而独立生成的假字段很少能保持跨字段和时间一致性以供受控评估。

Method: 开发了PROFILEFOUNDRY，一个确定性的生成器，产生100,000个成年合成人物对象，涵盖八个地区。每个对象包含类型化的当前快照、家庭、家庭和雇主链接、快照对齐的事件、归一化关系视图和生成来源。

Result: 发布包含709,228个事件、40,338个家庭、52,491个雇主和518,564个有向关系边。提供了四类证据：选定的总体边缘比较、每对象不变性检查、发布范围内的引用和时间闭合性、以及巧合/来源筛选。

Conclusion: PROFILEFOUNDRY不是总体保真度模型、渲染文本语料库或正式隐私机制，而是负责任的人工合成源层，用于构建涉及记忆、隐私、文档理解、记录链接和代理状态的下游基础模型评估，同时保持每个工件背后的人工合成人物可检查。
...
TL;DR: ConflictScore是一个新的指标，用于评估模型响应如何承认其基于文档中的冲突证据，通过CS-C和CS-R两个互补指标量化冲突程度，并在ConflictBench基准上验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有的真实性和忠实性指标只能评估答案是否被基础文档支持或反驳，但无法捕捉到支持证据和反驳证据同时存在的情况。这种局限性使得模型可能忽略文档中的冲突信息，做出过度自信的声明。

Method: 1) 将模型响应分解为原子主张；2) 针对每个基础文档标记每个主张；3) 聚合标记结果形成两个互补指标：CS-C（展现冲突的主张比例）和CS-R（支持与反驳证据之间的平衡）；4) 开发ConflictBench基准，涵盖歧义、矛盾、分歧意见等多种冲突形式。

Result: 实验表明：1) ConflictScore能够有效检测跨领域的过度自信声明；2) 该指标可以作为纠正反馈机制，在TruthfulQA任务上提高真实性；3) 系统评估验证了该指标在检测冲突证据方面的有效性。

Conclusion: ConflictScore提供了一种新颖的评估框架，能够量化模型对冲突证据的承认程度，弥补了现有指标的不足，为提高语言模型的真实性和忠实性提供了有效的评估工具和反馈机制。
...
TL;DR: ProvenAI是一个评估检索增强系统透明度的框架，通过三个可独立测量的层次：答案正确性、引用保真度和文档影响力，揭示了引用与真实影响力之间的差距。


<details>
  <summary>Details</summary>
Motivation: 当前检索增强系统在生成答案时提供引用，但引用并不能确认对应来源是否真正影响了输出结果，需要更严谨的透明度评估方法。

Method: 提出一个包含七个阶段的pipeline：数据规范化、检索索引、引用感知答案生成、归因审计、基于ablation的影响力估计、批量评估和交互式检查，在HotpotQA数据集上评估7,405个验证样本。

Result: 系统达到53.53%的答案准确率和平均71.55%的引用保真度得分，发现了引用-影响力差距：存在干净引用审计与一个被引用来源仅显示弱影响力、七个未引用来源却明显改变输出的情况。

Conclusion: 检索增强QA系统的有意义的透明度需要在检索的、引用的和行为上有影响力证据之间建立可追踪的链接，这三个层次是独立且需要分别测量的。
...
TL;DR: AnySimLite是一个轻量级的相似性编码器，通过将语音相关分类任务转化为文本相似性计算，使用单模型解决多个任务，在少样本设置下达到SOTA或接近SOTA性能，同时保持极低的内存占用。


<details>
  <summary>Details</summary>
Motivation: 为了解决移动设备上部署多个专用模型的隐私和内存占用问题，研究人员探索能否使用单一轻量级架构通过文本相似性公式解决多个语音相关分类任务。

Method: 提出AnySimLite轻量级相似性编码器，结合词级和字符级通道，并采用数据集转换策略，将多种语音相关分类任务转化为文本相似性计算问题。

Result: AnySimLite在多个语音相关分类任务的少样本设置下，一致达到SOTA或SOTA竞争性性能，在最坏情况下性能下降低于7%，同时模型大小仅为SOTA基准qLLaMA_LoRA-7B的1/250以下。

Conclusion: 研究表明通过将语音相关分类任务重新表述为文本相似性问题，可以使用单一的轻量级模型有效解决多个任务，在保持高性能的同时大幅减少内存占用，适合在边缘设备上部署。
...
TL;DR: 该论文提出SOLAR方法，通过跨语言对齐软令牌表示来解决多语言大语言模型中语义等效提示在不同语言间推理不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 多语言大语言模型在面对语义等效但不同语言的提示时，常产生不一致的推理过程和答案。虽然中间表示相对语言无关，但生成阶段随着模型确定离散输出令牌而变得语言特定。这种语言特定的词汇选择会导致语义等效的推理路径在不同语言间发散，从而降低模型的多语言推理性能。

Method: 提出了SOLAR（Soft-token Language Alignment Regularization）辅助目标函数，用于有监督微调阶段对齐跨语言的软令牌表示。方法包括：1）使用软令牌（词汇嵌入的概率加权混合）作为连续表示，能够聚合跨语言语义相关令牌的信息；2）以英语为枢纽，将每个非英语软令牌表示与其英语对应表示在共享嵌入空间中对齐。

Result: 在四个多语言推理基准测试中，SOLAR相比基础模型提升了高达+17.7点的准确率，相比标准有监督微调提升了+3.8点，在低资源语言上提升效果最大。SOLAR还增强了最终层的跨语言相似性，并显著降低了语言聚类可分离性，表明对齐软令牌表示有助于在多语言推理中保持共享语义结构。

Conclusion: SOLAR通过跨语言对齐软令牌表示的有效方法，显著改善了多语言大语言模型在语义等效提示下的推理一致性，特别是在低资源语言上的性能，为增强多语言模型的跨语言推理能力提供了有前景的方向。
...
TL;DR: 本文提出一种从科学论文中提取问题和方法句子的新方法，通过公式化表达去敏感化数据增强和上下文增强的Transformer模型来解决小规模数据集带来的泛化问题。


<details>
  <summary>Details</summary>
Motivation: 科学论文数量庞大，需要从海量文本中识别关键部分（问题和方法）。现有数据集规模小，导致模型过度依赖特定表达形式，泛化能力差。

Method: 1) 提出公式化表达(FE)去敏感化概念，基于此开发数据增强器生成合成数据；2) 设计上下文增强Transformer，利用上下文评估目标句中词汇重要性并减少上下文噪声；3) 使用基于LLM的上下文学习方法进行实验比较。

Result: 提出的模型在两个科学论文数据集上相比基线模型获得更高的宏F1分数，分别提升3.71%和2.67%。实验发现基于LLM的上下文学习方法不适合问题和方法提取任务。

Conclusion: 通过数据集扩展、减少对特定形式的依赖以及丰富句子信息三个角度，有效解决了小规模数据集带来的问题。FE去敏感化数据增强和上下文增强Transformer的组合策略显著提升了问题和方法句子的提取性能。
...
TL;DR: 利用EEG和眼动追踪认知信号改进微博关键短语提取，EEG特征带来最大性能提升


<details>
  <summary>Details</summary>
Motivation: 微博平台产生大量短小、噪声多且分散的用户内容，使得自动关键短语提取(AKE)成为重要但具有挑战性的任务。先前研究使用眼动追踪信号改进微博AKE，但眼动追踪单独使用受到生理、采集和特征解码限制。需要探索EEG信号是否能补充眼动追踪信号用于AKE。

Method: 使用ZuCo认知语言处理语料库，选择8个EEG特征和17个眼动追踪特征，将其融入微博AKE模型。为了减少模型结构对认知信号的潜在失真，将这些特征注入到软注意力层的输入和自注意力层的查询向量中。评估不同认知信号组合在不同AKE模型中的效果。

Result: 阅读过程中产生的认知信号持续提升AKE性能，不受特征组合和模型架构影响。EEG特征带来最大增益，而EEG和眼动追踪特征组合的性能介于两者单独使用之间，表明部分互补性但也可能存在冗余或噪声。

Conclusion: EEG信号为微博基于AKE提供有用的认知证据，多模态认知信号值得进一步研究。认知信号在阅读过程中能一致改善AKE性能，但需要注意不同认知信号之间可能的冗余问题。
...
TL;DR: TempoWave是一种即插即用的时间小波数字接口，通过将标量观测值映射为由多小波、多尺度系数构建的数字级嵌入，解决了LLMs在时间序列预测中数值标记化不准确的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然能整合异质文本信号进行上下文感知的时间序列预测，但其离散的、面向语言的标记化和嵌入接口与连续数值不匹配，常常损害数值顺序和预测可靠性。

Method: 提出TempoWave接口，将每个标量观测值映射为由多小波、多尺度系数构建的数字级嵌入，直接覆盖标准标记表示，以兼容Transformer的形式同时暴露细粒度局部波动和宏观全局结构。

Result: 在五个上下文增强预测基准测试中，TempoWave相比标准数值标记化和替代嵌入接口，持续改进基于LLM的预测器，达到了新的最先进水平。

Conclusion: 数值接口是关键瓶颈，原则性多分辨率嵌入可以更好地将LLMs的上下文推理与精确预测相结合，TempoWave为此提供了有效解决方案。
...
TL;DR: 通过迭代数据生成管道识别级联线性特征，用于更好地检测和调控语言模型中的奉承行为


<details>
  <summary>Details</summary>
Motivation: 传统的激活导向方法需要大量对比样本对来清晰展示期望或不期望的行为，这些样本对决定了可解释性框架检测模型特征的可靠性以及调控模型行为的能力。本文旨在开发更好的方法来识别和调控语言模型中的奉承行为。

Method: 提出迭代数据生成管道，超越简单的二进制样本对，识别展示行为程度线性缩放特征的样本。专注于检测和调控语言模型的奉承行为，通过级联样本来识别形成线性可分离子空间的奉承特征。

Result: 奉承特征形成线性可分离子空间，能够选择更清晰对应期望行为的模型激活。在检测、确定性评分和鲁棒调控方面，该方法要么匹配要么优于LLM-as-a-judge和系统提示基准，同时提供更低的计算需求和更强的可解释性保证。

Conclusion: 通过级联样本识别线性特征的方法能够有效提升语言模型奉承行为的检测和调控能力，提供比传统方法更好的性能和可解释性。
...
TL;DR: 论文提出超越准确率饱和的评估框架，通过CORE-Bench Hard案例研究展示了从六个维度（构造效度、分布外泛化、效率、可靠性、模型与支架重要性、人机协作提升）全面评估智能体性能的价值。


<details>
  <summary>Details</summary>
Motivation: 当前基准测试评估存在局限性：当基准准确率饱和时，通常会被更难的版本替代，这种准确率中心主义忽略了评估智能体性能的其他重要维度，如构造效度问题、分布外泛化能力、效率、可靠性、模型与支架的相对重要性以及人机协作的提升效果。

Method: 以CORE-Bench Hard（科学代码计算可复现性基准）为案例研究：1) 识别构造效度威胁并改进为CORE-Bench v1.1，创建分布外任务套件CORE-Bench OOD；2) 在准确率饱和情况下测量效率、可靠性、模型性能和支架性能；3) 进行小规模随机实验测量真实计算可复现任务中人机协作的提升效果。

Result: 1) 发现CORE-Bench Hard存在难以预见的构造效度威胁；2) CORE-Bench v1.1在准确率饱和后仍能有效测量效率、可靠性等维度；3) 人机协作实验显示约2倍的显著速度提升（可能低估，因五分之一纯人工复现超时未完成）。

Conclusion: 提出比主导的准确率中心评估范式更严谨的替代方案，强调即使基准准确率饱和，通过多维度的系统评估仍能获得关于智能体性能的有价值见解，为更全面的智能体评估提供方法论框架。
...
TL;DR: 研究发现拒绝行为和顺从人格在语言模型中相互关联：顺从人格作为拒绝行为的"门控"机制，主要在模型后期层表达，而非早期计算阶段。


<details>
  <summary>Details</summary>
Motivation: 先前研究将拒绝行为（refusal）和人格特征（persona）视为独立的激活空间方向，但缺乏对两者相互作用的探究。本研究表明顺从人格对拒绝行为具有调节作用。

Method: 在Qwen2.5-7B-Instruct和Llama-3.1-8B-Instruct模型中提取顺从人格方向和拒绝方向，通过激活向量干预实验（包括方向叠加、投影移除等）分析二者的相互作用机制。

Result: 1. 顺从人格方向操纵可显著抑制拒绝行为（Llama模型拒绝率从97%降至2%）；2. 拒绝方向部分恢复仅在后期层有效；3. 后期层窗口移除人格方向能完全恢复基线拒绝率；4. 拒绝行为在后期表达阶段受人格"门控"调节。

Conclusion: 拒绝行为不是孤立的激活方向，而是依赖于人格特征的表达机制，其核心调节发生在模型后期层的表达阶段，而非早期计算阶段。这对理解语言模型安全机制有重要意义。
...
TL;DR: AlgoEvolve：一个基于大语言模型的演化框架，用于在算法交易这一复杂领域中进行持续的程序合成和策略优化，通过内外循环架构实现自适应交易策略的生成和改进。


<details>
  <summary>Details</summary>
Motivation: 当前LLM作为语义变异算子在程序演化发现中的应用主要局限于静态编码基准测试，而算法交易领域具有噪声、非平稳性和高度不连续性的独特挑战，需要将其扩展到这一复杂动态环境中。

Method: AlgoEvolve框架包含两个主要组件：内循环使用LLM生成、评估和迭代改进可执行的Python交易策略；外循环通过元演化优化指导程序合成的提示，从而发现改进的搜索启发式方法，平衡探索与利用并减少零交易失败。

Result: 系统展现出新兴的机制自适应策略逻辑，包括交易规则的自主转换。元演化外循环发现的启发式方法在平衡探索与利用、减少零交易失败方面表现优于人工设计的初始指令，证明了LLM驱动的语义演化在复杂环境中持续程序合成的可行性。

Conclusion: LLM驱动的语义演化为复杂环境中持续的程序合成提供了可行的方法，特别是在算法交易这类动态领域，通过演化框架能够产生适应性的交易策略并自主优化搜索过程。
...
TL;DR: 提出基于LLM的治理话语分析流程，通过自动标注、神经主题建模和多层网络分析，比较ERC-8004和Google A2A两种AI代理互操作性标准的治理结构差异


<details>
  <summary>Details</summary>
Motivation: 随着AI代理协议的激增，塑造其互操作性标准的治理结构在实证研究中被低估，需要大规模分析社会技术权力结构的方法

Method: 开发了LLM驱动的比较分析流程，集成了自动标注、神经主题建模和多层网络分析；验证于4,323条治理参与记录，对比ERC-8004（无许可、链上）和Google A2A（企业主导）两种标准

Result: 治理形式影响实质焦点，但两种制度都表现出相当的参与不平等和社区碎片化；无许可环境中的话语对齐更密集，表明开放治理可能促进更大的主题趋同

Conclusion: LLM辅助方法可以推进技术治理的实证研究，对设计更公平的AI代理标准具有启示意义；所有数据和代码已开源
...
TL;DR: 开发了一个基于知识图谱的多智能体框架，整合了FDA监管报告和患者社区数据，用于追踪抗抑郁药物不良反应信息，实现了可溯源的信息集成。


<details>
  <summary>Details</summary>
Motivation: 精神科药物安全知识分散于权威但抽象的监管不良反应记录与贴近经验但未经验证的患者叙述之间。在精神病学中，不当整合这些信息可能放大恐惧、反安慰剂反应和非依从性。需要一种既能整合信息又不混淆证据与轶事的方法。

Method: 1. 开发了基于知识图谱的溯源感知多智能体框架；2. 整合了466,525个Reddit帖子、60,782个WebMD评论和20年美国FDA不良反应报告系统的9种抗抑郁药数据；3. 使用大型语言模型实体识别管道（与医师标注比对达到最高F1分数：药物0.969，疾病0.973）；4. 基于ATC-N、ICD-10和MedDRA词汇构建Neo4j知识图谱以保持溯源性。

Result: 1. 两个社区平台之间的一致性（Jaccard相似度高达0.905）远高于与监管报告的一致性，表明患者生成数据形成了部分独立的安全信号；2. 对于舍曲林，许多不良反应在社区来源中出现的时间比相应FDA日期提前数百天；3. 知识图谱保持了每个声明的可追溯性，将监管事实与患者经验区分开来。

Conclusion: 溯源感知的整合为精神科药物信息提供了更可审计的途径，其有用性和患者益处需要在未来进行前瞻性测试。
...
TL;DR: 提出了一种基于漂移扩散模型的新型国际象棋评级系统DD-Elo，通过整合棋步级别数据来更快地捕捉技能波动，同时保持与传统Elo系统的理论一致性。


<details>
  <summary>Details</summary>
Motivation: 传统Elo评分系统存在响应滞后问题，因为它仅依赖比赛结果而忽略了每步棋的质量。虽然将棋步级别的信息纳入评级调整有重要意义，但面临巨大的噪音和游戏状态空间庞大的挑战。

Method: 提出DD-Elo（漂移扩散增强Elo评级系统），该框架受认知神经科学中漂移扩散模型（DDM）的启发，将技能表达建模为决策过程，从而整合棋步级别数据来捕捉技能波动。

Result: 数学推导证明DD-Elo与传统Elo系统的偏差有界，确保理论一致性。实验表明DD-Elo比Elo系统更快适应技能变化，提供了一个可解释、响应快且向后兼容的国际象棋评级解决方案。

Conclusion: DD-Elo系统通过整合棋步级别信息，解决了传统Elo系统的响应滞后问题，在保持理论兼容性的同时实现了更快速、更敏感的评级调整，有望改善国际象棋评级生态系统。
...
TL;DR: 提出一种AI代理的计算治理模型，通过独立见证的证据要求来监管高风险自主行动，而不是监控其推理过程。


<details>
  <summary>Details</summary>
Motivation: 随着自主AI代理开始执行具有重大后果且不可逆的行动（如临床处方和生产软件部署），需要有效的治理机制。观察到人类机构治理强大自主行为体的方式不是监控其推理过程，而是在关键行动点要求独立见证的证据。

Method: 形式化了机构治理模式作为AI代理系统的计算治理模型：代理保留规划和推理的完全自主权，但对指定高风险行动没有执行权限；执行取决于必须由独立权威源独立见证的前提条件，这些条件与声明的意图密码绑定，并由确定性策略评估；决策记录在可独立重新验证的防篡改日志中。

Result: 提出了一个概念验证实现，并展示了该模型在软件部署和临床处方等领域的应用示例。

Conclusion: 该模型为自主AI代理系统的治理提供了一种可行的计算框架，通过分离规划和执行权限，并引入独立验证机制，能够有效监管高风险行动，同时保持代理的自主性。
...
TL;DR: COrigami：一个端到端的AI驱动计算折纸生成系统，能够从自然语言输入生成满足平面可折叠几何约束和美学要求的折痕图案。


<details>
  <summary>Details</summary>
Motivation: 虽然生成式AI在解决可验证问题方面取得了显著成功，但在物理艺术创作中同时满足严格的几何约束和主观视觉美学仍然是一个挑战。计算折纸作为一个数学严格的环境，将艺术设计置于平面可折叠性方程中，为这一挑战提供了理想的研究领域。

Method: 提出COrigami端到端管道：1）从自然语言生成语义简笔画；2）计算基础打包；3）求解平面可折叠折痕图案；4）塑造折叠后的折痕图案；5）使用强化学习结合自主美学评估循环进行模型精炼。系统整合了算法优化与自主美学评判。

Result: 系统能够作为高效的协作助手，生成结构起点供人类艺术家进一步扩展和塑造。通过整合算法优化与自主美学批判，系统能够满足多目标物理约束，实现可靠、数学基础的协同创造力。

Conclusion: 这项工作展示了AI系统如何满足多目标物理约束，实现可靠、数学基础的协同创造。COrigami作为协作助手，通过整合算法优化与自主美学评估，为计算折纸设计提供了新的范式，展示了AI在物理约束艺术创作中的潜力。
...
TL;DR: 随着基础模型推理能力提升，验证候选解决方案比生成更困难，成为核心挑战。论文提出通过三个维度（可扩展性、忠实性、鲁棒性）评估验证信号质量，研究四种奖励构造方法，实验显示针对性验证设计能有效抑制奖励黑客攻击并提升任务完成质量。


<details>
  <summary>Details</summary>
Motivation: 传统直觉认为验证解决方案比生成更容易，但随着基础模型推理能力增强和工程工具成熟，生成复杂候选解决方案已不再困难，可靠验证反而成为更困难的问题。每个验证器都只是人类意图的代理，而非意图本身，导致验证面临双重困难：意图本质上是欠明确的，难以忠实检查是否满足；模型训练期间优化会扩大代理与意图之间的差距，表现为奖励黑客攻击或信号饱和。

Method: 从三个维度（可扩展性、忠实性、鲁棒性）评估验证信号质量，研究四种奖励构造方法：1）通用编程任务的测试验证器；2）前端任务的规则验证器；3）真实世界代理任务的用户验证器；4）长时域任务的自动化代理验证器。在不同任务类型和策略能力水平上进行深入分析和实验，探索奖励设计的核心挑战以及如何更有效地利用奖励信号。

Result: 实验表明，针对性验证设计能有效抑制奖励黑客攻击，提高任务完成质量，在多个内部和公开基准测试中取得显著增益。验证器必须与生成器协同演化，因为随着策略能力增长，固定奖励函数无法保持有效性。

Conclusion: 验证已成为比生成更困难的核心问题，需要从可扩展性、忠实性和鲁棒性三个维度评估验证信号。研究证明针对性验证设计能有效提升性能，关键洞察是：随着策略能力持续增长，固定奖励函数无法保持有效性，验证必须与生成器协同演化。
...
TL;DR: 本文提出首个针对能源市场的实证研究，通过构建包含243个专家策划问题的评估环境，对比分析工具增强LLM代理在实时数据检索、专业法规知识和多步量化推理等真实世界能源分析任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有AI基准测试在金融、编程、法律和药物发现等领域已广泛应用，但能源领域的评估仍局限于静态知识回顾。能源行业需要实时数据检索、专业法规和市场知识，以及现实约束下的多步量化推理，这一关键差距需要填补。

Method: 构建包含243个专家策划问题的评估环境，涵盖三大类别：(1)市场数据检索与分析；(2)知识检索与解释；(3)高级量化建模与决策分析。为代理配置可定制的领域工具套件，包括主要美国ISO实时电力市场API、法规文件搜索、公用事业费率数据库、资产优化模型以及对能源市场文档的检索增强生成。采用多维评估协议，从方法正确性、答案准确性、属性对齐和来源有效性四个方面进行评分，并基于类别进行路由匹配。

Result: 评估了开源和闭源LLM，分析了模型能力与领域工具在高风险专业领域的交互作用。关键研究产物已公开，支持可重复性和未来研究。

Conclusion: 本研究填补了能源领域AI评估的关键空白，为理解工具增强LLM在实时、专业和多步推理密集型能源市场分析任务中的表现提供了实证基础。公开的评估框架和工具支持未来研究和实际应用。
...
TL;DR: 本文探讨了多模态大语言模型(MLLMs)评估的现状与局限性，指出现有评估基准无法充分衡量模型跨模态信息整合能力，并提出需要解决的关键评估缺口。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型(MLLMs)处理文本、图像、音频、视频等多种输入的能力迅速提升，但评估方法未能同步发展。现有评估基准大多局限于孤立任务，无法揭示模型是否真正整合了跨模态信息。

Method: 通过审视当前MLLMs评估手段，回顾现有的评估基准分类法，识别出关键评估缺口，包括时空一致性、物理世界理解、多模态一致性和选择性注意力等方面。

Result: 确定了多模态模型评估中的四个主要缺口：1)时空连贯性评估不足；2)物理世界理解能力衡量不够；3)跨模态一致性检验缺失；4)选择性注意力机制评估欠缺。

Conclusion: 填补这些评估缺口对于准确衡量多模态智能的真实进展、揭示模型能力边界至关重要，需要开发更全面的评估框架来有效评估MLLMs的跨模态整合能力。
...
TL;DR: OpenFinGym是一个用于量化金融代理开发的统一健身房环境，覆盖预测、市场生成、实时交易和欺诈检测，提供自动化任务构建、容器化运行时、模拟交易引擎等功能。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型代理在量化金融工作流中的应用评估存在碎片化问题，各任务孤立评估且常忽视金融相关性。金融工作流本质是多阶段、相互依赖的（预测、策略构建、风险管理、交易等），现有平台通常只关注单一任务，可能高估代理能力且无法揭示泛化、真实市场交互和金融决策方面的弱点。

Method: 1. 构建统一健身房环境（OpenFinGym），覆盖预测、市场生成、实时交易和欺诈检测的单一执行和验证接口；2. 自动化任务构建管道，将量化金融论文转化为可执行任务包；3. 容器化运行时与主机端验证器服务，支持可扩展代理部署并防止训练-测试泄露；4. 低延迟数据流设计的模拟交易引擎；5. 支持长期和事件市场预测的延迟解析；6. SFT和RL后训练集成。

Result: 开发了OpenFinGym平台，为量化金融代理提供统一的评估环境，解决了现有评估方法的碎片化问题，能够更全面、真实地评估代理在完整金融工作流中的表现。

Conclusion: OpenFinGym通过统一的量化金融代理开发环境，解决了现有评估方法在任务孤立性、金融相关性缺失和真实市场交互能力评估不足的问题，为更全面的金融代理评估和开发提供了系统化解决方案。
...
TL;DR: 论文提出了"组合行为泄漏"(CBL)概念，发现提示组合式智能体系统中，编辑一个提示模块会静默影响其他模块的行为，即使它们没有共享变量或执行依赖关系。作者开发了可重复的三通道协议来测量这种干扰，并证明这是现有智能体故障轴之外的新问题。


<details>
  <summary>Details</summary>
Motivation: 实践中发现提示组合式智能体系统存在一个反复出现的故障模式：编辑一个提示模块会静默改变其他模块的行为，尽管它们没有共享变量或执行依赖。这种跨模块干扰现象缺乏正式的描述和测量方法，需要在智能体评估中建立量化测量标准。

Method: 1. 形式化定义了"组合行为泄漏"(CBL)：在共享上下文窗口的模块之间发生的干扰现象；2. 开发了可重复的三通道协议，通过音量、内容和形式三个通道扰动非焦点模块；3. 在部署的职位评估智能体(Claude Sonnet 4.6)上进行144次试验；4. 使用Cohen's d效应量和bootstrap置信区间进行统计分析。

Result: 1. 只有内容通道产生可检测的配对效应(Cohen's d = 0.63，bootstrap 95%置信区间不包括零)；2. 没有推荐结果翻转，表明这是标准QA检测不到的亚阈值机制；3. CBL与已知智能体故障轴(对抗注入、认知退化、多智能体故障传播、隐私泄漏)正交；4. 这种干扰在部署智能体的数千次决策中会累积放大。

Conclusion: CBL是提示组合式智能体中由transformer自注意力架构的非隔离性引起的新故障模式。论文贡献了操作定义、可重复协议、可证伪预测集和系统类别特征，确立跨模块干扰测量作为提示组合式智能体评估的必要要求。
...
TL;DR: 论文批判性地审视Kurzweil的加速回报理论，指出即使技术进步确实呈指数增长，这种量化能力的加速本身并不能解决科学发现的核心问题，因为真正的发现依赖于定性的概念突破能力，而非仅仅是执行和基础设施能力的提升。


<details>
  <summary>Details</summary>
Motivation: Ray Kurzweil的加速回报理论在技术进步讨论中具有重要影响力，声称多个技术领域的进步会相互作用，导致自我强化的指数级增长。但作者认为，即使这种加速是真实的，它本身并未解决科学发现的核心问题，因为加速主要作用于执行和基础设施能力，而真正的发现需要定性推理能力和概念框架突破。

Method: 1. 对Kurzweil加速回报理论进行简单的数学解释；2. 分析该理论的局限性，特别是执行能力加速与概念发现能力之间的区别；3. 引用ARC-AGI-3基准测试结果（人类达到天花板水平，而前沿AI系统低于1%），说明当前AI与人类灵活推理之间的巨大差距；4. 提出定性科学引擎（QES）作为补充缺失能力的响应方案。

Result: 1. 确认加速回报理论适用于解释量化能力和基础设施的指数增长；2. 揭示科学发现的核心问题需要的是定性推理和概念框架突破能力，而这种能力并非简单通过技术加速就能获得；3. ARC-AGI-3结果证实了人类灵活推理能力与当前AI系统之间的显著差距；4. 提出QES框架旨在弥补这一能力缺口，其价值不依赖于AGI何时实现。

Conclusion: 技术加速本身不足以解决科学发现的核心挑战。真正的科学进步需要定性推理能力和概念框架突破，这正是当前AI系统所缺乏的。定性科学引擎（QES）提供了一个应对这一缺失能力的框架，强调科学发现过程本身作为一种人类智慧的形式值得保存、组织和传承，无论AGI何时实现。技术预测必须与对人类理解形式的思考相结合。
...
TL;DR: NoT（叙事式思维）系统提示通过结构化思维链的五个部分，显著减少道德困境分析中的利益相关者崩溃和不确定性抑制问题，无需额外训练即可大幅改善模型表现。


<details>
  <summary>Details</summary>
Motivation: 标准思维链在道德困境分析中存在两个主要缺陷：利益相关者崩溃（只识别最多一个利益相关方）和不确定性抑制（在做出行动承诺前不明确表达未知因素或保留意见），这限制了AI系统在道德决策中的可靠性和可审计性。

Method: 引入NoT（叙事式思维）系统提示，将思维链结构化分为五个部分：主角、利益相关者、两步后果、不确定性、然后承诺。该方法无需额外训练、参数或微调。使用四个生成器（来自三个厂商）在100个DailyDilemmas场景中进行评估，并进行了消融实验以验证各部分效果。

Result: NoT将利益相关者崩溃从最高31%降至1%以下，不确定性抑制从最高72%降至1-24%（所有模型）。在匹配预算的详细思维链对照实验中，NoT在三个生成器上保持了显著优势：利益相关者数量Cliff's delta为+0.79到+0.90，不确定性评分为+0.65到+0.93。文本梯度下降进一步改进了脚手架，跨家族训练评判器表现优于家族内评判器。扩展至五轮多利益相关者辩论协议后，校准集上的僵局率从6%降至5%，达成95%完全共识。

Conclusion: NoT通过结构化思维链，显著改善了道德困境分析中的利益相关者识别和不确定性表达问题。该方法产生的轨迹可外部化每个承诺背后的利益相关者、后果和不确定性基础，为可靠智能体部署提供了可审计的底层结构。
...
TL;DR: 提出几何感知蒙特卡洛树搜索框架，通过强制几何约束和利用几何对称性，有效解决组合几何中的极值问题，在多项问题上取得新的最佳计算结果。


<details>
  <summary>Details</summary>
Motivation: 传统精确求解器在组合几何极值问题中面临组合爆炸问题，标准强化学习和基于transformer的模型难以处理稀疏奖励"有效性悬崖"和二次令牌消耗限制。需要新的方法来有效处理严格的全局几何约束。

Method: 提出几何感知蒙特卡洛树搜索框架：1）通过增量更新可行动作空间严格强制几何约束；2）利用几何对称性：在节点扩展时进行规范剪枝以减少分支因子，使用对称批量转换加速发现有希望的配置。对于共线点约束，将约束检查复杂度从O(n³)降至O(n²)。

Result: 在六个考虑的问题中，在五个问题上建立了新的最佳计算结果：1）对于Max-N3IL问题，在82≤n≤119的网格中找到了大小约1.8n的配置；2）对于最小完全集问题，找到了大小约0.95n的配置，在测试网格内提供了新的上界。

Conclusion: 几何感知蒙特卡洛树搜索作为一个高度可适应的框架，能够有效发现组合几何中的新颖配置，为解决具有严格几何约束的极值问题提供了有效方法。
...
TL;DR: 本文提出了一种用于电动公交车队管理的智能体聚合器框架，将基于优化的调度模型与监控智能体结合，以协调车队运营、充电、V2G和电力市场，在保持调度可行性的同时实现自适应优化。


<details>
  <summary>Details</summary>
Motivation: 电动公交车队的运营需要协调多个复杂因素：服务可靠性、电池状态、充电桩可用性、电价波动、路线能耗不确定性以及车辆到电网(V2G)机会。传统的调度方法难以实时应对这些动态变化，需要一个更智能、自适应的协调框架来管理这种复杂的决策环境。

Method: 提出了一个智能体聚合器框架，包含两个核心部分：1）优化核心：执行基于物理可行性的电动公交车调度模型，涵盖路线、充电桩、电池和V2G交换；2）智能体层：包含用于扰动检测、电价适应和调度评估的监督智能体，解释变化运行条件，在需要时触发实时重新优化，并定义灵活价值在聚合器和公共交通运营商之间的分配方式。

Result: 通过现实的停车场案例研究评估了日前和实时操作，考虑了服务延误、路线能耗偏差、电价冲击和综合扰动。结果显示：1）智能体聚合能够支持自适应车队-电网协调，保持可行调度；2）能够有选择性地激活重新优化；3）改善充电和V2G灵活性的使用；4）同时揭示了关键权衡：减少运营复杂性的智能体能力在配置为利润导向定价时可能从公共交通运营商处提取价值。

Conclusion: 智能体聚合器可以成为管理电动公交车V2G操作的有用工具，但在公共车队背景下的部署需要：透明的协调模式、可审计的电价设定机制和明确的价值共享规则。框架在复杂决策环境中具有潜力，但需平衡技术能力与公平价值分配。
