...
TL;DR: 本文提出对比多模态超图推理方法，通过融合语义、几何和姿态线索来解决多人3D重建中的遮挡和深度模糊问题，在Panoptic和GigaCrowd基准上取得了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前多人3D重建方法面临两个主要挑战：1）单模态输入缺乏几何引导，2）孤立重建个体忽略群体上下文信息，导致在拥挤场景中遮挡严重和深度模糊问题难以解决。需要一种能够融合多模态信息并利用群体动态的方法。

Method: 提出对比多模态超图推理框架：1）初始化鲁棒节点表示，结合RGB特征、几何先验和遮挡感知的不完整姿态；2）引入骨盆深度指示器作为全局空间锚点；3）构建共享拓扑超图建模高阶群体动态；4）设计基于超图的对比学习方案，增强模态内区分性和跨模态正交性。

Result: 在Panoptic和GigaCrowd基准测试上的广泛实验表明，该方法达到了新的最先进性能，能够有效处理严重遮挡情况，推断缺失信息。

Conclusion: 通过多模态融合和超图推理的协同作用，成功解决了拥挤场景中多人3D重建的关键挑战。方法在群体上下文建模和遮挡处理方面表现优异，代码和预训练模型已开源。
...
TL;DR: 本研究揭示了扩散变换器（DiTs）中"大规模激活通道"的核心作用：尽管稀疏，但少数具有极大响应值的隐藏状态通道实际上承载着图像生成的主要语义信息，而非简单的激活异常。


<details>
  <summary>Details</summary>
Motivation: 尽管扩散变换器（DiTs）和相关基于流的架构已成为最强大的文本到图像生成器，但提示词如何塑造图像语义的内部机制仍不清楚。本研究旨在探索这种内部工作机制。

Method: 通过三个方面研究大规模激活通道：1）功能性关键性测试：使用受控破坏探针零化大规模通道并观察生成质量变化；2）空间组织分析：将图像流令牌限制在大规模通道中并进行聚类；3）可转移性实验：将大规模激活从一个提示条件轨迹转移到另一个。

Result: 结果显示：1）破坏大规模通道会急剧降低生成质量，而同等规模的低统计通道影响甚微；2）大规模通道形成与主要主体和显著区域对齐的连贯空间分区；3）大规模激活转移能在保留目标内容的同时将图像语义向源提示转移，实现局部语义插值而非无序像素混合。

Conclusion: 大规模激活并非激活异常，而是现代DiT模型中组织和控制语义信息的稀疏提示条件载体子空间。这一发现为无需额外训练实现提示插值和主题驱动生成提供了理论基础。
...
TL;DR: CineMesh4D：一种端到端4D（3D+t）管道，可直接从多视角2D电影MRI重建患者特异性全心脏网格，通过可微分渲染损失和双上下文时间块实现高质量4D重建。


<details>
  <summary>Details</summary>
Motivation: 从电影MRI准确重建3D+t全心脏网格在临床上至关重要但具有技术挑战性，主要困难来自两个耦合因素：2D图像切片对3D心脏解剖的固有稀疏采样，以及心脏形状与运动的紧密耦合。现有方法通常只能重建部分心脏腔室或单个心动周期时相。

Method: 提出了CineMesh4D，一种新颖的端到端4D（3D+t）管道，通过跨域映射直接从多视角2D电影MRI重建患者特异性全心脏网格。方法包括：1）可微分渲染损失，从电影MRI的多视角稀疏轮廓监督3D+t全心脏网格；2）双上下文时间块，融合全局和局部心脏时间信息以捕获高维序列模式。

Result: 在定量和定性评估中，CineMesh4D在重建质量和运动一致性方面优于现有方法，为个性化实时心脏评估提供了实用途径。代码将在稿件接受后公开。

Conclusion: CineMesh4D成功解决了从稀疏2D电影MRI重建高质量4D全心脏网格的挑战，通过创新的可微分渲染监督和时序建模方法，实现了临床实用的个性化心脏功能评估工具。
...
TL;DR: 本文提出了一种将像素token和文本token统一到生成式语言模型中的新模型，以解决当前多模态模型中ViT视觉编码器在细节识别方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前基于CLIP或SigLIP方法的ViT作为视觉编码器在多模态模型中存在细节理解限制，特别是难以识别图像中的小文本或数字。需要改进视觉编码以提升细节识别能力。

Method: 提出新模型统一像素token和文本token；每个像素有自己的token嵌入；采用色彩折叠技术；全局条件注意力近似；以及图像无监督预训练方法。

Result: 实验显示，即使在小型模型和有限训练数据情况下，新模型也表现出良好性能。模型符合缩放定律，随着参数和数据增加，性能将持续提升。

Conclusion: 提出的统一像素和文本token的模型能有效改善多模态模型的视觉细节理解能力，特别是在识别小文本和数字方面，并具有良好的扩展潜力。
...
TL;DR: PVRF是一种统一框架，通过零射soft天气感知和速度约束整流流细化，解决真实世界图像中异质和未见退化的不利天气去除问题，同时避免传统失真驱动训练导致的过度平滑结果。


<details>
  <summary>Details</summary>
Motivation: 真实世界图像中的不利天气去除面临两大挑战：1）异质且未见的天气退化难以处理；2）传统的失真驱动训练方法容易产生过度平滑的结果，缺乏真实的纹理细节。

Method: PVRF提出一个统一框架，包含两个核心组件：1）AWR-QA模块：使用冻结的视觉-语言模型进行零射软天气感知，估计天气类型概率和低级属性分数；2）通过属性调制归一化和天气加权适配器生成锚点估计；3）学习终端一致残差整流流，采用感知自适应源扰动和终端一致速度参数化，以稳定终端区域的学习。

Result: 广泛实验表明，PVRF在保真度和感知质量上均优于最先进基线方法，在单一和组合退化场景下表现出强大的跨数据集泛化能力。

Conclusion: PVRF通过整合零射软天气感知与速度约束整流流细化，有效解决了真实世界不利天气去除的挑战，既保持了图像保真度又提升了感知质量，展现出优秀的泛化性能。
...
TL;DR: 提出一个硬件感知的遗传编程框架，用于为ViT演化层特定的标量函数替代归一化层，无需重新训练即可显著提升边缘设备部署效率


<details>
  <summary>Details</summary>
Motivation: ViT在边缘设备上部署受到层归一化的计算复杂性和全局规约瓶颈的严重限制。现有方法采用同质标量近似替换，但无法适配不同层的特性且需要昂贵的重新训练

Method: 提出基于遗传编程（GP）的硬件感知框架，从预训练权重演化异构的、层特定的标量函数。结合新的训练后重新对齐策略，完全避免从零开始重新训练模型

Result: 演化出的表达式准确近似目标归一化行为，捕获91.6%的方差（R²），相比同质基线的70.2%有显著提升。修改后的架构在仅20个epoch内恢复了84.25%的Top-1 ImageNet-1K准确率，同时消除了全局规约瓶颈

Conclusion: 该方法在算术复杂度和片外内存流量之间建立了有利的权衡，消除了ViT在边缘加速器上高效部署的主要障碍，为实现硬件友好的ViT架构提供了有前景的方向
...
TL;DR: CurveBench：一个用于从视觉输入进行分层拓扑推理的基准测试，包含756张不相交的Jordan曲线图像，每张图像都标注了表示平面区域包含关系的根树结构。尽管任务看似简单，但现有最强模型在该基准上的表现仍远未达到理想水平。


<details>
  <summary>Details</summary>
Motivation: 当前的计算机视觉系统在处理复杂的拓扑结构和层次关系时仍然存在困难。研究人员需要一个专门的基准来评估模型在从视觉输入中理解分层拓扑关系的能力，特别是在处理曲线包含关系这种看似简单但实际上具有挑战性的任务上。

Method: 提出了CurveBench基准，包含756张成对不相交的Jordan曲线图像，涵盖简单、多边形、地形启发、迷宫式和密集计数等五种配置。每张图像都标注了表示曲线间包含关系的根树结构。任务被定义为结构化预测：给定图像，模型必须恢复曲线诱导的完整根包含树。研究还通过RLVR风格的微调来评估开放权重的视觉语言模型在该任务上的表现。

Result: 最强评估模型Gemini 3.1 Pro在CurveBench-Easy上仅达到71.1%的树生成准确率，在CurveBench-Hard上仅19.1%。通过RLVR微调训练的Qwen3-VL-8B模型在CurveBench-Easy上的准确率从基线的2.8%提升到33.3%，超过了GPT-5.4和Claude Opus 4.5在相同评估协议下的表现。

Conclusion: 精确的拓扑感知视觉推理仍然是一个远未解决的挑战，尤其是在CurveBench-Hard配置上。CurveBench作为一个专门的基准，能够有效评估和推动模型在分层拓扑推理方面的发展，为未来研究提供了明确的方向和挑战。
...
TL;DR: DeFakerOne是一个统一的假图像检测与定位基础模型，集成了InternVL2和SAM2，在39个检测基准和9个定位基准上实现了最先进性能


<details>
  <summary>Details</summary>
Motivation: 生成式AI的快速发展重塑了图像伪造范式，打破了传统编辑、操纵、DeepFake生成和AIGC合成之间的界限。现有的假图像检测与定位研究仍然碎片化，与日益统一的伪造生成机制不匹配，需要解决跨域伪影转移与干扰问题，并构建联合检测与定位的高容量统一基础模型

Method: 提出DeFakerOne模型，采用数据中心的统一FIDL基础模型架构，集成InternVL2和SAM2，实现同时的图像级检测和像素级伪造定位。系统分析了数据缩放规律、跨域伪影转移-干扰模式、细粒度监督的必要性以及原始分辨率伪影保留

Result: DeFakerOne在39个伪造检测基准和9个定位基准上实现了最先进的性能，显著优于基线模型。模型展现出对现实世界扰动和最先进生成器（如GPT-Image-2）的优越鲁棒性

Conclusion: DeFakerOne成功解决了统一伪造生成与碎片化检测之间的不匹配问题，通过数据中心的统一基础模型架构，为可扩展、鲁棒且统一的FIDL系统提供了设计原则和实证分析
...
TL;DR: DUET是一个基于组织学图像推断空间分辨基因表达的双范式框架，它通过结合参数化预测和基于记忆的检索，在细胞先验知识下自适应平衡表达灵活性与生物学保真度。


<details>
  <summary>Details</summary>
Motivation: 当前方法将组织学图像到基因表达映射简化为简单的形态学-表达映射，但视觉相似性并不保证分子一致性。同时，单细胞数据资源丰富但未充分利用，现有方法采用单一范式存在瓶颈，无法平衡表达灵活性与生物学保真度。

Method: 提出DUET框架，采用并行回归-检索双范式：1）参数化预测路径；2）基于记忆的检索路径。通过大规模单细胞参考数据施加分子状态作为生物学约束，减少视觉歧义。设计轻量适配器根据空间上下文动态分配分支偏好以优化性能。

Result: 在三个公共数据集上跨不同基因规模进行广泛实验，DUET实现了最先进性能，每个提出的组件都贡献了一致的性能提升。

Conclusion: DUET通过双范式框架有效整合了视觉信息与分子约束，成功解决了现有方法在平衡表达灵活性与生物学保真度方面的局限性，为基于组织学图像的空间基因表达推断提供了新的解决方案。
...
TL;DR: 提出了一种用于糖尿病视网膜病变检测的两层边缘-云端级联系统，在本地诊所设备上使用轻量级模型进行初步筛查，只将可疑病例发送到云端进行详细分级，显著减少了云服务调用和传输成本。


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是可预防性失明的主要原因，但农村地区缺乏专家和基础设施进行早期检测。现有的云端深度学习系统虽然准确率高，但在农村地区面临高延迟、带宽有限和数据传输成本高等挑战。

Method: 提出了一个两层边缘-云端级联系统：第一层在本地诊所设备上运行轻量级MobileNetV3-small模型，进行二元分流（可转诊DR vs 不可转诊DR）；第二层在云端运行RETFoundDINOv2模型，仅对第一层标记为可转诊的图像进行详细的严重程度分级。

Result: 第一层达到98.99%敏感性和84.37%特异性；级联系统将50.48%的云调用减少到49.52%的图像发送到第二层。在4类分级中，级联系统获得80.49%准确率和0.8167二次加权kappa，与纯云端基线（80.76%准确率，0.8184 kappa）相比性能下降很小。

Conclusion: 该边缘-云端级联系统能在保持良好分级性能的同时，显著减少云端资源使用（约减少一半），特别适合资源受限的农村环境，为远程眼科筛查提供了一种高效实用的解决方案。
...
TL;DR: SToRe3D是一个相关性对齐的稀疏化框架，用于加速基于Vision Transformer的多视角3D检测，通过联合选择2D图像tokens和3D物体queries，同时存储过滤特征用于重新激活，实现了最高3倍推理加速且仅造成边际精度损失。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在3D检测中表现出色，但存在高推理延迟问题，这源于多个视角和大规模3D区域上的密集token和query处理。现有稀疏化方法主要针对2D视觉设计，仅能剪枝或合并图像token，无法扩展到全模型稀疏化或处理3D物体queries。

Method: 引入SToRe3D框架，通过互相关的2D-3D相关性头，联合选择2D图像tokens和3D物体queries，同时存储被过滤的特征用于后续重新激活。该方法将计算资源分配到驾驶关键内容上，同时保留其他嵌入。

Result: 在nuScenes和新创建的nuScenes-Relevance基准测试上评估，SToRe3D实现了最高3倍的推理加速，仅造成边际精度损失。该方法实现了实时大规模基于ViT的3D检测，同时在规划关键智能体上保持精度。

Conclusion: SToRe3D通过相关性对齐的稀疏化方法成功解决了Vision Transformers在3D检测中的高延迟问题，为实时大规模3D检测系统提供了一种有效的解决方案，平衡了推理速度和检测精度。
...
TL;DR: PanoPlane：一种通过全景场景补全进行稀疏视角室内新视角合成的方法，利用全景补全和训练自由的注意力引导机制，仅需3个输入视角即可实现高质量新视角合成。


<details>
  <summary>Details</summary>
Motivation: 传统基于透视的方法从有限视野生成训练视角，存在视角覆盖不足的问题。室内场景新视角合成需要完整理解房间几何结构，而稀疏输入视角往往无法提供足够的空间信息。

Method: 1) 使用全景场景补全为生成过程提供完整空间布局条件；2) 提出"布局锚定注意力引导"训练自由机制，在推理时将扩散模型内部表示的注意力引导到检测到的平面表面；3) 将未观测区域的注意力引导到几何一致的已观测内容，用基于表面的外推替换无约束的幻觉；4) 生成的全景补全为3D高斯溅射提供监督，实现跨未观测区域的新视角合成。

Result: 在Replica、ScanNet++和Matterport3D数据集上的实验表明，该方法在3、6、9个输入视角下均达到最先进的新视角合成质量。相比当前最先进的基线方法，PSNR提升最高达+17.8%，且无需对扩散模型进行任何训练或微调。

Conclusion: PanoPlane通过全景补全和训练自由的注意力引导机制，能够从极稀疏的输入视角实现高质量的室内新视角合成，显著优于现有方法，证明了在扩散模型中引入几何引导的重要性。
...
TL;DR: TeDiO：一种无需训练、在推理时通过正则化自注意力图中的时间对角线模式来增强视频扩散模型时间一致性的方法。


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频扩散变换器生成的帧在视觉上令人信服，但在时间一致性方面存在困难，经常产生闪烁、漂移或不稳定的运动。研究发现，这些失败在模型内部留下了清晰的印记：不连贯的视频在其中间自注意力图中表现出不规则、碎片化的时间对角线，而稳定的运动则对应平滑的带状对角线模式。

Method: 基于观察引入TeDiO，一种训练自由、推理时的方法，通过正则化这些内部注意力模式来增强时间一致性。该方法估计对角线平滑度，识别不稳定区域，并执行轻量级的潜在更新以促进连贯的帧间动态，而不修改模型权重或使用外部运动监督。

Result: 在多个视频扩散模型（如Wan2.1、CogVideoX）上，TeDiO提供了显著更平滑的运动，同时保持了每帧视觉质量，为现代视频生成系统提供了高效的即插即用方法来改进动态真实感。

Conclusion: TeDiO通过直接干预注意力模式中的时间结构，提供了一种简单有效的解决方案来改善视频扩散模型的时间一致性，无需额外训练或外部监督，具有良好的通用性和实用性。
...
TL;DR: 基于Platonic表示假说，本文提出了一种简单高效的少样本学习方法：直接在冻结的DINOv2-L特征上使用k-最近邻分类器，无需反向传播或任务特定微调，就能在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 动机源于深度视觉识别领域正朝着通用表示范式转变。Platonic表示假说表明，不同架构在大规模数据上训练正在收敛到一个共享的"理想"潜在空间。这引发了一个关键问题：是否"一个好的嵌入就足够了"？本文探索预训练嵌入是否足够好，以至于无需密集的任务特定微调。

Method: 提出一个简单、非参数化的流程，完全绕过反向传播。使用冻结的DINOv2-L特征，采用k-最近邻分类器进行少样本学习。进行层间特征提取分析以确定最优特征提取点。进一步展示了通过PCA和ICA进行流形精炼的规范化效果。

Result: 在四个主要基准测试中，该方法一致超越了复杂的元学习算法，实现了最先进的性能表现。

Conclusion: 预训练的通用嵌入确实是"足够好的"，可用于复杂任务而无需任务特定微调。简单、非参数化的方法在少样本学习场景中能与复杂算法竞争甚至超越它们，验证了Platonic表示假说的实用性。
...
TL;DR: 提出了一种用于机械血栓切除术中实时导管尖端跟踪的多线程管道，在具有挑战性的荧光成像条件下实现了稳定的性能，为基于强化学习的自主导航提供了可靠基础。


<details>
  <summary>Details</summary>
Motivation: 机械血栓切除术虽然能改善中风结果，但受限于本地治疗资源的缺乏。基于强化学习的机器人系统可通过自主导航缓解这一挑战，但现有方法需要实时导管尖端坐标跟踪。本文旨在开发并评估在荧光成像下的实时导管尖端跟踪管道，解决低对比度、噪声和设备遮挡等挑战。

Method: 设计了多线程管道，包含帧读取、预处理、推理和后处理。训练和基准测试了深度学习分割模型，包括U-Net、U-Net+Transformer和SegFormer，采用两类和三类分割方案。后处理包括两步组件过滤、单像素中轴骨架化、贪婪弧长路径跟踪和轮廓回退策略。

Result: 在手动标注的中等复杂度荧光视频数据上，两类SegFormer实现了4.44毫米的平均绝对误差，优于U-Net(4.60毫米)、U-Net+Transformer(6.20毫米)和所有三类模型(5.19-7.74毫米)。在分割基准测试中，系统超越了最先进的CathAction结果，三类分割的Dice分数提升达+5%。

Conclusion: 所提出的多线程跟踪框架在具有挑战性的成像条件下保持了稳定的性能，超越了先前的基准测试，为基于强化学习的自主机械血栓切除术导航提供了可靠高效的基础。
...
TL;DR: 本文提出了一种基于动态分辨率扩散模型的图像修复加速方法，通过将数据投影到低维子空间来减少计算开销，并在多种图像数据集和修复任务中展现了优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在图像修复任务中面临高计算开销的问题。像素空间方法计算成本高，而基于潜在扩散模型的方法虽然利用压缩潜在空间，但需要重复的编码器-解码器推理，导致额外的计算负担，性能甚至不如像素空间方法。

Method: 1. 通过动态分辨率扩散模型将数据投影到低维子空间以加速推理过程
2. 对预训练的扩散模型进行微调以获得动态分辨率先验
3. 将DPS和DAPS两种像素空间方法适配到所提出的框架中，分别称为SubDPS和SubDAPS
4. 基于SubDAPS的推理速度和重建保真度优势，进一步提出了增强变体SubDAPS++

Result: 实验评估表明，所提出的方法在多样化的图像数据集和各种修复任务中，在大多数实验场景下优于最近的基于扩散模型的方法。SubDAPS++进一步提升了重建效率和质量的平衡。

Conclusion: 通过动态分辨率扩散模型将数据投影到低维子空间是解决图像修复中计算效率问题的有效方法，所提出的SubDPS、SubDAPS和SubDAPS++方法在保持重建质量的同时显著提升了推理速度。
...
TL;DR: 本文提出了一种名为"省略信号干预"(OSI)的方法，通过放大文本嵌入中的"省略信号"来缓解多模态扩散变换器在文本到图像生成中的概念省略问题。


<details>
  <summary>Details</summary>
Motivation: 多模态扩散变换器(MM-DiTs)在文本到图像生成中取得了显著进展，但经常存在概念省略问题，即指定的对象或属性无法在生成的图像中出现。

Method: 通过在线性探针上分析文本标记，发现文本嵌入可以区分代表目标概念缺失的"省略信号"。基于这一发现，提出了省略信号干预(OSI)方法，通过放大省略信号来主动催化缺失概念的生成。

Result: 在FLUX.1-Dev和SD3.5-Medium数据集上的综合实验表明，OSI即使在极端场景下也能显著缓解概念省略问题。

Conclusion: OSI方法通过识别并放大文本嵌入中的省略信号，为多模态扩散变换器的概念省略问题提供了一种有效的解决方案，显著提升了文本到图像生成的完整性。
...
TL;DR: CreFlow：一种用于具身视频生成模型后训练的组合约束奖励模型，通过线性时序逻辑约束和新型在线强化学习框架，提高生成视频的物理约束遵从性和任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在异质数据上训练时，可能产生视觉合理但违反物理约束的具身操作视频。虽然强化学习后训练是适配视频生成模型的自然途径，但现有的视频-RL奖励通常将每个生成视频简化为低级视觉指标，而具身操作视频评估需要基于逻辑验证视频是否满足组合任务规范。

Method: 1. 提出组合约束奖励模型：自动将任务需求表述为线性时序逻辑约束的组合，为生成视频提供忠实奖励和局部化错误信息；2. 提出CreFlow在线强化学习框架：包含两个关键设计：i) 信用感知NFT损失，将RL更新限制在奖励相关区域，防止后训练中对无关区域的干扰；ii) 矫正回流损失，利用组内正样本作为矫正方向的显式估计，稳定并加速训练。

Result: 实验表明，CreFlow产生的奖励判断比现有方法更符合人类和模拟器成功标签，在八个双手操作任务中，下游执行成功率提高了23.8个百分点。

Conclusion: CreFlow通过组合约束奖励模型和创新的在线强化学习框架，有效解决了视频生成模型在具身操作中的物理约束违反问题，显著提高了生成视频的任务遵从性和执行成功率。
...
TL;DR: 提出了一种可解释的概念级遗忘框架用于视觉语言模型，通过构建任务特定概念词汇表并进行稀疏非负分解，实现了精细化的知识操纵。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型遗忘方法通常在图像或实例级别进行操作，难以精确移除目标知识而不影响无关语义。单个图像往往包含多个纠缠概念，包括需要遗忘的目标概念和应该保留的上下文信息，这种纠缠使得精确遗忘变得困难。

Method: 1. 使用多模态大语言模型从遗忘集合中构建紧凑的任务特定概念词汇表；2. 将视觉表示分解为语义概念的稀疏非负组合，提供明确的细粒度知识操纵接口；3. 将遗忘问题表述为概念级优化，选择性抑制目标概念，同时保留同一实例内的非目标语义和全局跨模态知识。

Result: 在领域内和领域外遗忘设置下的大量实验表明，该方法能够实现更全面的目标遗忘，更好地保留同一图像内的非目标知识，并在模型实用性方面与现有的VLM遗忘方法保持竞争力。

Conclusion: 提出的可解释概念级遗忘框架通过细粒度的概念操纵，解决了视觉语言模型中知识纠缠导致的精确遗忘难题，实现了更有效的目标知识移除和非目标知识保留。
...
TL;DR: 本文提出将KV缓存压缩视为核心集选择问题，通过联合KV表示和双准则目标优化历史覆盖度，改进流式视频理解中的内存效率。


<details>
  <summary>Details</summary>
Motivation: 现有流式视频理解方法通常使用基于局部令牌启发式（如时效性、时间冗余性或显著性）的KV缓存压缩，但这些方法未明确优化保留的缓存是否能代表累积历史。需要更有效的压缩原则来支持不断增长的视觉历史推理。

Method: 提出将KV缓存压缩视为核心集选择问题：1）在联合KV表示空间中操作；2）引入平衡键空间和值空间覆盖度的双准则目标；3）添加基于正交性的多样性准则，鼓励多样化子集选择；4）将该准则与对数行列式子集选择联系起来。

Result: 在四个开源VLM和五个长视频/流式视频基准测试中，该方法在固定缓存预算下优于启发式流式压缩基线。验证了核心集选择比令牌级剪枝更有效。

Conclusion: 代表性核心集选择为内存受限的流式视频理解提供了比令牌级剪枝更有效的原则。该方法通过几何覆盖优化确保保留的缓存能有效代表累积历史，提升推理质量。
...
TL;DR: TurboVGGT是一种用于快速多视角3D重建的高效视觉几何变换器方法，通过自适应交替注意力机制在保持重建质量的同时提高计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有前馈式3D重建方法（如视觉几何变换器）难以平衡重建质量与计算效率，通常使用固定的稀疏比率且缺乏自适应学习代表性token的机制，导致性能不佳。

Method: 提出TurboVGGT，采用端到端可训练框架，包含自适应稀疏全局注意力（学习具有变化稀疏度的代表性token）和帧内注意力（聚合局部细节）的自适应交替注意力机制。

Result: 在多个3D重建基准测试上的广泛实验表明，TurboVGGT实现了快速的多视角重建，同时保持与最先进方法相比具有竞争力的重建质量。

Conclusion: TurboVGGT通过自适应稀疏全局注意力和帧内注意力的创新组合，有效解决了3D重建中质量与效率的平衡问题，展示了在保持高质量的同时实现快速重建的潜力。
...
TL;DR: D2-CDIG：一种基于扩散模型和双先验控制机制的新型遥感图像生成框架，通过DEM和云雾信息精确控制地形特征和大气现象


<details>
  <summary>Details</summary>
Motivation: 现有可控遥感图像生成方法主要依赖传统的分割和边缘检测技术，未能充分利用地形或大气条件信息，导致在处理复杂地形和大气现象时生成的图像缺乏准确性和自然度

Method: 提出D2-CDIG框架，集成扩散模型与双先验控制机制，使用数字高程模型（DEM）和云雾信息作为双先验知识。通过独立控制地面和大气分支解耦地形和大气生成过程，引入精细的云雾滑块灵活调整云层厚度和分布，在训练中分层注入地面和大气控制信号以确保图像内平滑过渡

Result: 与基于分割或边缘检测的传统方法相比，D2-CDIG在图像质量、细节丰富度和真实感方面表现出显著提升

Conclusion: D2-CDIG为遥感图像生成提供了灵活而精确的解决方案，为训练大型遥感模型和下游任务提供高质量数据支持
...
### [26] [Learning with Semantic Priors: Stabilizing Point-Supervised Infrared Small Target Detection via Hierarchical Knowledge Distillation](https://arxiv.org/abs/2605.14346)
...
### [40] [ArcGate: Adaptive Arctangent Gated Activation](https://arxiv.org/abs/2605.14518)
...
### [44] [Local Spatiotemporal Convolutional Network for Robust Gait Recognition](https://arxiv.org/abs/2605.14548)
...
### [62] [How to Evaluate and Refine your CAM](https://arxiv.org/abs/2605.14641)
...
### [63] [Vision-Based Water Level and Flow Estimation](https://arxiv.org/abs/2605.14645)
*ZhiXin Sun*
...
### [64] [TERRA-CD: Multi-Temporal Framework for Multi-class and Semantic Change Detection](https://arxiv.org/abs/2605.14651)
...
### [65] [Beyond Instance-Level Self-Supervision in 3D Multi-Modal Medical Imaging](https://arxiv.org/abs/2605.14654)
...
### [66] [MiVE: Multiscale Vision-language features for reference-guided video Editing](https://arxiv.org/abs/2605.14664)
...
### [67] [Are Candidate Models Really Needed for Active Learning?](https://arxiv.org/abs/2605.14689)
...
### [70] [Generating HDR Video from SDR Video](https://arxiv.org/abs/2605.14703)
...
### [71] [ATLAS: Agentic or Latent Visual Reasoning? One Word is Enough for Both](https://arxiv.org/abs/2605.15198)
...
### [72] [SceneFunRI: Reasoning the Invisible for Task-Driven Functional Object Localization](https://arxiv.org/abs/2605.14704)
...
### [73] [Towards Continuous Sign Language Conversation from Isolated Signs](https://arxiv.org/abs/2605.14705)
...
### [74] [StyleTextGen: Style-Conditioned Multilingual Scene Text Generation](https://arxiv.org/abs/2605.14708)
...
### [75] [Breaking Dual Bottlenecks: Evolving Unified Multimodal Models into Self-Adaptive Interleaved Visual Reasoners](https://arxiv.org/abs/2605.14709)
...
### [76] [Vision-Core Guided Contrastive Learning for Balanced Multi-modal Prognosis Prediction of Stroke](https://arxiv.org/abs/2605.14710)
...
### [77] [Towards Label-Free Single-Cell Phenotyping Using Multi-Task Learning](https://arxiv.org/abs/2605.14717)
...
### [79] [EARL: Towards a Unified Analysis-Guided Reinforcement Learning Framework for Egocentric Interaction Reasoning and Pixel Grounding](https://arxiv.org/abs/2605.14742)
...
### [80] [BioHuman: Learning Biomechanical Human Representations from Video](https://arxiv.org/abs/2605.14772)
...
### [81] [MonoPRIO: Adaptive Prior Conditioning for Unified Monocular 3D Object Detection](https://arxiv.org/abs/2605.14781)
...
### [82] [COAL: Counterfactual and Observation-Enhanced Alignment Learning for Discriminative Referring Multi-Object Tracking](https://arxiv.org/abs/2605.14795)
...
### [84] [SuperADD: Training-free Class-agnostic Anomaly Segmentation -- CVPR 2026 VAND 4.0 Workshop Challenge Industrial Track](https://arxiv.org/abs/2605.14808)
...
### [85] [Probing into Camera Control of Video Models](https://arxiv.org/abs/2605.14815)
...
### [86] [The Velocity Deficit: Initial Energy Injection for Flow Matching](https://arxiv.org/abs/2605.14819)
...
### [87] [HDRFace: Rethinking Face Restoration with High-Dimensional Representation](https://arxiv.org/abs/2605.14821)
...
### [88] [Multi-proposal Collaboration and Multi-task Training for Weakly-supervised Video Moment Retrieval](https://arxiv.org/abs/2605.14838)
...
### [89] [Editor's Choice: Evaluating Abstract Intent in Image Editing through Atomic Entity Analysis](https://arxiv.org/abs/2605.14842)
...
### [90] [MechVerse: Evaluating Physical Motion Consistency in Video Generation Models](https://arxiv.org/abs/2605.14843)
...
### [91] [Exploring Vision-Language Models for Online Signature Verification: A Zero-Shot Capability Study](https://arxiv.org/abs/2605.14845)
...
### [92] [SR-Prominence: A Crowdsourced Protocol and Dataset Suite for Perceptually-Weighted Super-Resolution Artifact Evaluation](https://arxiv.org/abs/2605.14847)
...
### [93] [FactorizedHMR: A Hybrid Framework for Video Human Mesh Recovery](https://arxiv.org/abs/2605.14854)
...
### [94] [LPH-VTON: Resolving the Structure-Texture Dilemma of Virtual Try-On via Latent Process Handover](https://arxiv.org/abs/2605.14874)
...
### [95] [Unlocking Complex Visual Generation via Closed-Loop Verified Reasoning](https://arxiv.org/abs/2605.14876)
...
### [96] [HeatKV: Head-tuned KV-cache Compression for Visual Autoregressive Modeling](https://arxiv.org/abs/2605.14877)
...
### [97] [Denoising-GS: Gaussian Splatting with Spatial-aware Denoising](https://arxiv.org/abs/2605.14880)
...
### [98] [Masked Next-Scale Prediction for Self-supervised Scene Text Recognition](https://arxiv.org/abs/2605.14885)
...
### [99] [Hierarchical Image Tokenization for Multi-Scale Image Super Resolution](https://arxiv.org/abs/2605.14891)
...
### [101] [SEDiT: Mask-Free Video Subtitle Erasure via One-step Diffusion Transformer](https://arxiv.org/abs/2605.14894)
...
TL;DR: 本文研究了多语言知识编辑中向量合并方法的有效性，发现带共享协方差的向量求和是最可靠的整体策略，而Task Singular Vectors for Merging（TSVM）在减少多语言干扰方面效果有限，且性能对权重缩放因子和秩压缩比敏感。


<details>
  <summary>Details</summary>
Motivation: 多语言知识编辑面临语言特定编辑相互干扰的挑战，即使在单语环境下定位然后编辑方法表现良好。本文关注三个核心问题：向量合并方法在多语言知识编辑中的有效性、TSVM减少多语言干扰的程度、权重缩放因子和秩压缩比对性能的影响。

Method: 在MzsRE基准测试中使用大规模批量编辑设置，评估了六种合并变体，采用两种流行的骨干大语言模型、两种基础知识编辑方法，覆盖12种语言。重点研究向量合并方法，特别关注带共享协方差和不带共享协方差的向量求和策略，以及TSVM的应用。

Result: 1. 带共享协方差的向量求和是最可靠的整体策略，而不带共享协方差的简单求和表现不佳。
2. TSVM在某些设置中能提升性能，但其减轻多语言干扰的能力有限。
3. 性能对权重缩放和秩比敏感，大于默认的缩放比例和相对较低的秩通常能带来更好的结果。

Conclusion: 这些发现阐明了当前向量合并方法在多语言知识编辑中的实际优势和局限，为未来多语言知识编辑研究提供了指导：需要更有效的策略来处理语言间的干扰问题，并注意调整权重缩放因子和秩压缩比以获得更优性能。
...
### [141] [Mistletoe: Stealthy Acceleration-Collapse Attacks on Speculative Decoding](https://arxiv.org/abs/2605.14005)
*Shuoyang Sun,Chang Da,Hao Fang,Kuofeng Gao,Xinhao Zhong,Yi Sun,Fan Mo,Shu-Tao Xia,Bin Chen*

Main category: cs.CL

TL;DR: Mistletoe：针对推测解码的隐蔽加速崩溃攻击，通过攻击草稿器-目标模型的匹配机制来降低接受率，同时保持输出质量


<details>
  <summary>Details</summary>
Motivation: 推测解码通过草稿模型并行生成候选token并用目标模型验证来加速LLM推理，但其效率严重依赖于平均接受长度τ。作者发现草稿器和目标模型分布近似存在不完美匹配，这构成了新的机制级攻击面，可以在保持目标模型可见行为的同时显著降低草稿token接受率。

Method: 提出Mistletoe攻击方法：1）联合优化降低草稿器-目标模型一致性的退化目标和约束目标模型输出分布的语义保持目标；2）引入零空间投影机制，将退化梯度投影到局部语义保持方向的零空间，在最小化语义漂移的同时抑制草稿接受率。

Result: 在多种推测解码系统上的实验表明，Mistletoe能够显著降低平均接受长度τ，导致加速崩溃，降低平均token吞吐量，同时保持输出质量和困惑度不变。

Conclusion: 推测解码引入了超越现有输出鲁棒性的机制级攻击面，需要更鲁棒的LLM加速系统设计。Mistletoe攻击暴露了这一安全漏洞。
...
### [142] [Physics-R1: An Audited Olympiad Corpus and Recipe for Visual Physics Reasoning](https://arxiv.org/abs/2605.14040)
*Shan Yang*

Main category: cs.CL

TL;DR: 论文审计了多模态物理评估流程，发现了三种扭曲视觉语言推理测量的未检测构建实践：训练-评估污染、翻译漂移和MCQ饱和。作者发布了四个新资源来弥补这些差距，并提出了Physics-R1模型，在多个基准上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前多模态物理评估流程存在三个未检测的构建实践问题：1）训练-评估污染（数据泄漏），2）翻译漂移（语言转换导致的语义变化），3）MCQ饱和（选择题格式带来的虚假高表现）。这些问题扭曲了视觉语言推理能力的真实测量，需要系统性审计和改进。

Method: 采用端到端审计方法：1）三阶段审计流程（Jaccard相似度 -> mxbai-embed-large余弦相似度 -> Haiku-4.5 LLM判断）检测数据污染；2）跨语言配对实验（爱沙尼亚语-英语）测量翻译漂移；3）格式新颖度梯度分析比较MCQ与开放式评估表现差异。基于审计结果构建了新数据集和模型：PhysCorp-A（审计后的多模态语料库）、PhysR1Corp（封闭形式RL池）、PhysOlym-A（新颖来源的奥林匹克评估集）、Physics-R1模型（基于Qwen3-VL-8B-Thinking的GSPO+DAPO配方）。

Result: 审计发现：1）SciInstruct中存在134个近似重复和4,846个改述候选；2）翻译漂移导致17pp性能差距；3）MCQ与开放式评估间存在46pp格式梯度差距。Physics-R1模型在3次实验中的表现：PhysOlym-A提升+18.3pp（8.0→26.3），PhysReason提升+15.7pp（23.9→39.6），OlympiadBench-Physics提升+6.9pp，PhyX MCQ提升+4.1pp，性能超过Qwen3-VL-32B和Gemini 2.5 Pro。

Conclusion: 多模态物理评估流程存在系统性偏差，需要更严格的审计和更平衡的评估设计。提出的新资源和模型能够更准确地测量视觉语言推理能力，为未来研究提供了改进基准和参考实现。领域需要关注数据污染、跨语言一致性和评估格式多样性问题。
...
### [143] [Derivation Prompting: A Logic-Based Method for Improving Retrieval-Augmented Generation](https://arxiv.org/abs/2605.14053)
*Ignacio Sastre,Guillermo Moncecchi,Aiala Rosá*

Main category: cs.CL

TL;DR: 提出了Derivation Prompting方法，这是一种新的提示技术，用于RAG框架的生成步骤，通过构建可解释的推导树来减少幻觉和推理错误。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在问答应用中表现出巨大潜力，但在知识密集型、特定领域任务中存在幻觉和错误推理等挑战。传统RAG和长上下文窗口方法在生成不可接受答案方面存在问题。

Method: 提出了Derivation Prompting方法，受逻辑推导启发，从初始假设出发，通过系统应用预定义规则推导结论。该方法构建可解释的推导树，增加生成过程的控制性。

Result: 在特定案例研究中应用该方法，与传统RAG和长上下文窗口方法相比，显著减少了不可接受的答案。

Conclusion: Derivation Prompting通过引入系统化的推导过程增强了RAG框架，提高了问答系统的可靠性和可解释性，特别是在知识密集型领域任务中。
...
### [144] [PEML: Parameter-efficient Multi-Task Learning with Optimized Continuous Prompts](https://arxiv.org/abs/2605.14055)
*Anjir Ahmed Chowdhury,Syed Zawad,Xiaolong Ma,Xu Dong,Feng Yan*

Main category: cs.CL

TL;DR: PEML是一个参数高效的多任务学习方法，通过神经架构工程优化连续提示，同时执行模型权重的低秩适配，在多任务学习基准上实现了高达6.67%的平均准确率提升。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在多任务学习中的应用需求增加，现有PEFT方法如LoRA和Prefix Tuning存在局限性：LoRA专注于模型适配但忽略了提示调优的重要性；Prefix Tuning仅采用简单架构优化提示，限制了多任务适配能力。为了在多任务学习中实现高效微调，需要共同优化提示优化和模型适配。

Method: 提出PEML方法，采用神经架构工程方法优化连续提示，同时执行模型权重的低秩适配。通过创建自动化框架来优化连续提示和适配模型权重。

Result: 在GLUE、SuperGLUE、Massive Multitask Language Understanding和常识推理基准上评估PEML，相比MTL-LoRA、MultiLoRa、C-Poly和MoE等SOTA多任务学习方法，实现了平均准确率高达6.67%的提升，单个任务峰值增益达10.75%。

Conclusion: PEML通过共同优化提示和模型适配，在多任务学习场景中实现了显著的性能提升，为参数高效的多任务微调提供了有效的解决方案。
...
### [145] [When Evidence Conflicts: Uncertainty and Order Effects in Retrieval-Augmented Biomedical Question Answering](https://arxiv.org/abs/2605.14115)
*Yikun Han,Mengfei Lan,Halil Kilicoglu*

Main category: cs.CL

TL;DR: 医疗检索增强大语言模型在遇到矛盾证据时准确性下降显著，研究者开发了基于证据冲突检测的弃权评分方法，在困难情况下能有效提高选择性准确性


<details>
  <summary>Details</summary>
Motivation: 当前生物医学检索增强大语言模型的评估主要关注在有益上下文下的答案准确性，而忽视了在证据不完整、误导性或内部矛盾情况下的可靠性问题。需要评估模型在冲突证据下的表现，并开发相应的方法来处理这些困难情况。

Method: 1. 创建HealthContradict数据集，在五种受控证据条件下评估六个开源大语言模型：无检索上下文、仅正确上下文、仅错误上下文、两种混合条件（正确和矛盾文档以相反顺序排列）
2. 引入证据冲突顺序对比实验，其中相同的两个文档同时存在但顺序相反
3. 开发冲突感知的弃权评分方法，结合模型置信度和证据冲突检测器

Result: 1. 在冲突证据顺序对比中，所有模型的准确性都下降，11.4%-25.2%的预测发生翻转
2. 在两种最困难的条件（仅错误和错误优先冲突）下，冲突感知弃权评分比仅使用置信度的方法显著提高了选择性准确性
3. 平均提升：在仅错误条件下7.2-33.4点，在错误优先冲突条件下3.6-14.4点（覆盖率为75%、50%、25%）

Conclusion: 矛盾生物医学证据既是模型的不确定性问题也是鲁棒性问题，需要开发和评估能够明确处理证据分歧的方法，包括基于冲突检测的弃权策略，以提升模型在真实医疗场景中的可靠性。
...
### [146] [Generative Floor Plan Design with LLMs via Reinforcement Learning with Verifiable Rewards](https://arxiv.org/abs/2605.14117)
*Luis Lara,Aristides Milios,Zhi Hao Luo,Aditya Sharma,Ge Ya Luo,Christopher Beckham,Florian Golemo,Christopher Pal*

Main category: cs.CL

TL;DR: 该论文提出了一种基于文本的平面图生成方法，结合大语言模型微调和带可验证奖励的强化学习，以同时满足拓扑和数值约束，在真实感、兼容性和多样性方面超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有生成方法主要关注房间之间的连接关系，但无法生成满足数值约束（如房间尺寸和面积）的平面图。需要一种能够精确控制房间尺寸和面积，同时尊重房间连接关系并保持功能性和美学质量的AI系统。

Method: 1. 基于真实平面图微调大语言模型；2. 应用带可验证奖励的强化学习来提高对拓扑和数值约束的遵循度，并减少无效或重叠输出；3. 设计了一套约束遵循度量标准来系统评估生成的平面图与用户定义约束的符合程度。

Result: 模型能够生成满足用户定义的连接性和数值约束的平面图，在真实感、兼容性和多样性指标上优于现有方法。在所有任务中，该方法在兼容性方面相比现有方法实现了至少94%的相对降低。生成结果具有更高的约束符合度。

Conclusion: 大语言模型能够有效处理这种场景下的约束，这表明基于文本的生成建模具有更广泛的应用潜力。该方法为专业平面图设计提供了一个既能满足拓扑约束又能精确控制数值参数的解决方案。
...
TL;DR: ROK-FORTRESS是一个双语文化对抗性国家安全与公共安全基准，使用英语-韩语语言对和美韩地缘政治轴作为案例研究，通过转换矩阵方法分离语言和地缘政治背景的影响，发现韩语变体存在一致的抑制效应且地缘政治背景与语言交互方式存在显著模型间差异。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型(LLM)的安全评估越来越关注高风险的国家安全与公共安全(NSPS)风险，但多语言安全通常通过仅翻译的基准进行评估，这些基准保留了底层场景，而关于语言和地缘政治背景如何交互的实证证据仍然局限于少数语言对。需要更精细的方法来分离语言和地缘政治背景对模型安全行为的影响。

Method: 提出了ROK-FORTRESS双语文化对抗性NSPS基准，使用英语-韩语语言对和美韩地缘政治轴作为案例研究。通过"转换矩阵"方法分离语言和地缘政治背景的影响：对抗性意图在受控组合下评估(1)英语vs韩语语言(2)美国vs韩国实体、机构和操作细节。每个对抗性提示都配有一个双重用途的良性对应物以量化过度拒绝。模型响应使用校准的LLM-as-a-judge专家组评分，应用专家制作、特定提示的二元评估标准。

Result: 在包括前沿模型和韩语优化模型的双轨模型集合中，发现韩语变体存在一致的抑制效应，且地缘政治背景与语言交互方式存在显著的模型间差异。在许多模型中，韩国地缘政治背景减轻了韩语驱动的抑制效应，但没有模型在相反方向显示显著放大。这表明至少在英语-韩语案例中，安全行为由作为风险信号的语言和仅翻译评估会忽略的上下文交互所塑造。

Conclusion: 转换矩阵方法可推广到其他语言-文化对。研究表明安全行为受到语言作为风险信号和上下文交互的塑造，这些影响是仅翻译评估所无法捕捉的。需要更精细的多语言安全评估方法来理解语言、文化和地缘政治背景如何共同影响LLM的安全行为。
...
### [148] [Why Retrieval-Augmented Generation Fails: A Graph Perspective](https://arxiv.org/abs/2605.14192)
*Kai Guo,Xinnan Dai,Zhibo Zhang,Nuohan Lin,Shenglai Zeng,Jie Ren,Haoyu Han,Jiliang Tang*

Main category: cs.CL

TL;DR: 通过电路追踪构建归因图，研究检索增强生成中外部证据如何影响答案生成，发现正确与错误预测在归因图拓扑结构上有系统性差异，并基于此开发错误检测和干预框架。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成系统尽管能够访问外部信息，仍然经常产生错误答案，但这一现象的内在机制尚未被充分理解。作者旨在从模型内部探究检索证据如何影响答案生成过程，以理解RAG系统失败的根源。

Method: 使用电路追踪技术构建归因图，建模解码过程中Transformer层间的信息流动。归因图捕捉检索上下文、中间模型激活和生成token之间的交互，提供图电路级别的外部证据整合视图。基于多个问答基准的观察分析归因图拓扑特征差异。

Result: 发现了正确与错误预测之间一致的归因图结构差异：正确预测具有更深的推理路径、更分布式的证据流和更结构化的局部连接模式，而失败预测则表现出更浅、碎片化和过度集中的证据流。基于这些发现开发了基于归因图拓扑特征的错误检测框架。

Conclusion: 归因图揭示了检索增强生成中内部信息路由的模式，这些模式与回答正确性密切相关。通过加强问题约束的证据基础可以重塑内部路由，使答案生成更受问题引导，从而实现更有效的信息整合和更少的错误。归因图支持有针对性的干预策略。
...
### [149] [GradShield: Alignment Preserving Finetuning](https://arxiv.org/abs/2605.14194)
*Zhanhao Hu,Xiao Huang,Patrick Mendoza,Emad A. Alghamdi,Basel Alomair,Raluca Ada Popa,David Wagner*

Main category: cs.CL

TL;DR: GradShield是一种在微调期间保护大型语言模型安全对齐的数据过滤方法，通过计算微调隐式危害性评分和自适应阈值算法来识别和移除有害数据点


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在微调后存在安全对齐风险，模型可能被显性和隐性的有害数据破坏，甚至一些看似良性的数据也可能无意中引导模型产生不对齐行为

Method: GradShield基于计算每个数据点的微调隐式危害性评分(FIHS)，采用自适应阈值算法在微调前识别和过滤潜在有害数据点

Result: 在多种实用性微调任务中，GradShield优于所有基线方法，能持续将攻击成功率保持在6%以下，同时保持模型的实用性性能

Conclusion: GradShield提供了一种有效的原则性过滤方法，能够在微调期间保护LLMs的安全对齐，防止有害数据破坏模型的对齐性
...
### [150] [What Makes Words Hard? Sakura at BEA 2026 Shared Task on Vocabulary Difficulty Prediction](https://arxiv.org/abs/2605.14257)
*Adam Nohejl,Xuanxin Wu,Yusuke Ide,Maria Angelica Riera Machin,Yi-Ning Chang,Hitomi Yanaka*

Main category: cs.CL

TL;DR: 该论文提出了两种词汇难度预测模型：一种高精度黑盒模型（在开放赛道中取得最佳成绩），以及一种可解释模型（优于微调编码器基线）。黑盒模型通过软目标损失函数微调LLM，达到r>0.91的相关性；可解释模型保持r>0.77相关性的同时提供可解释性。分析发现词汇难度不仅受单词本身的生产难度影响，还受拼写难度和测试项目构造的影响。


<details>
  <summary>Details</summary>
Motivation: 研究动机是开发能够准确预测词汇难度的模型，特别关注两个方面：1）追求最高预测精度（黑盒模型），2）提供可解释性以理解影响词汇难度的因素。通过分析英国文化协会知识词汇列表(KVL)，揭示词汇测试中真正的难度来源，包括拼写难度、测试项目构造等影响因素。

Method: 1) 黑盒模型：使用软目标损失函数对大型语言模型(LLM)进行微调，用于词汇难度评分任务。
2) 可解释模型：开发优于微调编码器基线的可解释模型，提供对影响每个测试项目难度因素的见解。
3) 分析方法：对英国文化协会知识词汇列表(KVL)中的项目进行深入分析，识别影响难度的关键因素。

Result: 1) 黑盒模型在开放赛道中取得最佳成绩，相关性系数r>0.91，表现出极高的预测精度。
2) 可解释模型保持较强的相关性（r>0.77），同时提供对难度影响因素的见解。
3) 分析发现KVL中项目的难度不仅受单词本身生产难度影响，还显著受拼写难度和测试项目构造的影响。

Conclusion: 该研究成功开发了两种有效的词汇难度预测模型，平衡了精度和可解释性。关键发现是词汇测试中的难度是多重因素共同作用的结果，包括单词本身的认知难度、拼写挑战以及测试项目的设计构造。这为词汇评估工具的开发提供了重要见解，并有助于更准确地设计语言测试项目。研究代码已开源共享。
...
### [151] [Auditing Agent Harness Safety](https://arxiv.org/abs/2605.14271)
...
TL;DR: HarnessAudit是一个审计LLM执行轨迹安全的框架，重点关注多智能体系统在资源访问和信息流方面的边界合规性，研究发现任务完成与安全执行存在偏差，安全风险随轨迹长度积累。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体执行框架只能在输出层面进行评估，无法检测执行过程中的安全违规（如未授权资源访问、上下文泄露），而大多数安全基准只评估最终输出或终止状态，忽视了轨迹中间可能发生的违规行为。

Method: 提出HarnessAudit框架，审计完整执行轨迹的边界合规性、执行保真度和系统稳定性；建立HarnessAudit-Bench基准，包含210个跨8个真实领域任务，实例化为单智能体和多智能体配置；评估10种执行框架配置，涵盖前沿模型和三种多智能体框架。

Result: 研究发现：(i)任务完成与安全执行不一致，违规随轨迹长度积累；(ii)安全风险因领域、任务类型和智能体角色而异；(iii)大多数违规集中在资源访问和智能体间信息传输；(iv)多智能体协作扩大了安全风险面，而执行框架设计设定了安全部署的上限。

Conclusion: 仅靠输出级评估无法充分保障LLM智能体系统的安全性，需要审计完整执行轨迹；HarnessAudit填补了这一关键空白，提供了系统化评估执行框架安全性的方法，特别是对多智能体系统中边界合规性、信息流控制等关键风险点的评估能力。
...
### [152] [Ideology Prediction of German Political Texts](https://arxiv.org/abs/2605.14352)
*Sinclair Schneider,Florian Steuber,Joao A. G. Schneider,Gabi Dreo Rodosek*

Main category: cs.CL

TL;DR: 这篇论文提出了一种基于transformer的模型，用于将德文文本在连续的政治光谱（左翼到右翼，-1到1的标量）上进行定位。研究评估了13个候选transformer模型，使用四个不同的德文语料库（议会记录、在线决策工具、报纸文章和议员推文），发现DeBERTa-large和Gemma2-2B在不同测试场景中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 选举是国家发展的重要里程碑，需要更好地理解从左到右各政治运动的言论。现有的多分类器方法受到预定义类别限制，无法在连续光谱上定位政治立场。研究旨在开发能够在连续左右光谱上投影文本政治倾向的模型，使分析师能够专注于特定政治领域（如保守派），同时排除自由派和极右派。

Method: 1. 提出基于transformer的模型架构，将文本投影到-1到1的连续政治光谱标量d上
2. 构建四个德文语料库：德国联邦议院会议记录（带注释）、Wahl-O-Mat在线决策工具数据、33家政治倾向已知的报纸文章、德国联邦议院第20-21届597名议员的535,200条推文
3. 评估13个候选transformer基础模型
4. 使用交叉验证方法：两个语料库用于训练，两个用于测试，以减少过拟合
5. 评估指标包括F1分数、准确率（ACC）和平均绝对误差（MAE）

Result: 1. 域内性能最佳：DeBERTa-large获得最高F1分数（F1=0.844）
2. Twitter域外测试最佳：DeBERTa-large达到最高准确率（ACC=0.864）
3. 报纸域外测试最佳：Gemma2-2B表现最好（MAE=0.172）
4. 研究发现transformer模型能够以接近民意调查水平的准确度识别德文新闻中的政治框架
5. 模型架构和领域特定训练数据的可用性对估计政治偏见的影响可能与模型规模同等重要

Conclusion: 研究表明transformer模型能够有效识别德文文本中的政治框架，准确度达到接近民意调查的水平。研究强调模型架构和领域特定训练数据的重要性可能不亚于模型规模本身。论文还讨论了方法学限制，并提出了提高偏见测量稳健性的改进方向，为政治文本分析提供了实用的技术框架。
...
### [153] [LLM-based Detection of Manipulative Political Narratives](https://arxiv.org/abs/2605.14354)
*Sinclair Schneider,Florian Steuber,Gabi Dreo Rodosek*

Main category: cs.CL

TL;DR: 本文提出了一个检测和结构化操纵性政治叙事的新计算框架，结合了基于提示的过滤和无人监督的聚类方法，在超过120万条社交媒体帖子中识别出41个不同的操纵性叙事簇。


<details>
  <summary>Details</summary>
Motivation: 随着政治讨论转向社交媒体，检测和结构化操纵性政治叙事变得愈发重要。主要挑战在于区分操纵性政治叙事与合法批评，以及识别那些将真实事件重新置于操纵性背景中的帖子。

Method: 1. 使用详细的少样本提示过滤操纵性帖子，该提示结合了已记录的宣传活动叙事与合法批评，通过推理模型分配标签，仅保留操纵性叙事帖子
2. 对剩余帖子进行嵌入和UMAP降维
3. 应用HDBSCAN进行聚类以揭示叙事群体
4. 使用推理模型揭示每个聚类背后的叙事

Result: 该方法应用于超过120万条社交媒体帖子，成功识别出41个不同的操纵性叙事簇，证明该方法能够有效检测和结构化操纵性政治叙事。

Conclusion: 通过整合基于提示的过滤与无人监督聚类的方法，该框架能够独立于预定义目标类别运行，并能发现新的叙事簇，为社交媒体中的政治叙事分析提供了有效的计算工具。
...
### [154] [Reinforcement Learning with Semantic Rewards Enables Low-Resource Language Expansion without Alignment Tax](https://arxiv.org/abs/2605.14366)
*Zeli Su,Ziyin Zhang,Zhou Liu,Xuexian Song,Zhankai Xu,Longfei Zheng,Xiaolu Zhang,Rong Fu,Guixian Xu,Wentao Zhang*

Main category: cs.CL

TL;DR: 论文提出了一种基于语义空间对齐的新方法，使用分组相对策略优化（GRPO）和嵌入级语义奖励，来扩展大语言模型到低资源语言（如藏语），在减少通用能力损失的同时提升目标语言能力。


<details>
  <summary>Details</summary>
Motivation: 将大语言模型扩展到低资源语言时会出现"对齐税"问题：目标语言的改进以通用能力的灾难性遗忘为代价。这种权衡源于监督微调（SFT）的刚性，它强制在狭窄且有偏的数据分布上进行词表级表面模仿。

Method: 提出了语义空间对齐范式，采用分组相对策略优化（GRPO），使用嵌入级语义奖励而非可能性最大化进行优化。该目标通过灵活的语义实现鼓励意义保留，实现受控更新以减少对预训练知识的破坏性干扰。

Result: 在藏汉机器翻译和藏语标题生成任务上的实验表明，该方法在获取低资源语言能力的同时显著缓解了对齐税，比SFT更有效地保留了通用能力。尽管产生的表面重叠较少，但语义强化学习在开放生成中实现了更高的语义质量和偏好，少样本迁移结果表明它在有限监督下学习了更可迁移和鲁棒的表示。

Conclusion: 研究表明，基于语义奖励的强化学习为包容性低资源语言扩展提供了更安全可靠的路径。语义空间对齐通过灵活的意义保留和受控更新，解决了传统方法中的灾难性遗忘问题。
...
### [155] [Mitigating Data Scarcity in Psychological Defense Classification with Context-Aware Synthetic Augmentation](https://arxiv.org/abs/2605.14380)
*Hoang-Thuy-Duong Vu,Quoc-Cuong Pham,Huy-Hieu Pham*

Main category: cs.CL

TL;DR: 本文提出了一种用于心理防御机制自动分类的上下文感知合成增强框架和混合分类模型，在低资源环境下显著提升了分类性能。


<details>
  <summary>Details</summary>
Motivation: 心理防御机制的自动分类具有临床价值，但受到数据稀缺和类别不平衡问题的严重限制。单纯的生成增强方法缺乏心理学基础，无法有效解决这些问题。

Method: 提出了一个上下文感知合成增强框架（CASA），结合了混合分类模型。该模型整合了上下文语言表示、基本临床特征以及150个标注的防御项目。通过高质量定义提示来确保生成保真度。

Result: 在PsyDefDetect共享任务中，该方法超越了DMRS Co-Pilot模型，达到了58.26%的准确率（提升40.25%）和24.62%的宏F1分数（提升15.99%）。

Conclusion: 定义质量直接控制生成保真度和下游性能，该方法为低资源环境下基于心理学基础的心理防御机制分类建立了强基线。
...
### [156] [Agentic Recommender System with Hierarchical Belief-State Memory](https://arxiv.org/abs/2605.14401)
...
TL;DR: MARS是一个记忆增强的智能体推荐系统，采用三层结构化记忆表示和完整的生命周期管理，在部分可观测环境下提升个性化推荐效果


<details>
  <summary>Details</summary>
Motivation: 现有记忆增强LLM推荐系统普遍采用扁平化记忆表示，将瞬时信号与稳定偏好混为一谈，且缺乏完整的记忆生命周期管理机制

Method: 将推荐视为部分可观测问题，构建三层结构化信念状态：事件记忆（原始信号）、偏好记忆（细粒度可变片段）、档案记忆（自然语言叙事）。通过LLM规划器自适应调度六个操作：提取、强化、弱化、巩固、遗忘、再合成

Result: 在四个InstructRec基准领域上，MARS实现SOTA性能，HR@1平均提升26.4%，NDCG@10平均提升10.3%，在动态演化设置中通过智能体调度获得进一步增益

Conclusion: 结构化记忆表示结合完整的生命周期管理显著优于扁平化方法，将推荐系统视为信念状态维护过程，并通过自适应调度实现更好的个性化推荐
...
### [157] [Knowledge Beyond Language: Bridging the Gap in Multilingual Machine Unlearning Evaluation](https://arxiv.org/abs/2605.14404)
*Kyomin Hwang,Hyeonjin Kim,Sangyeon Cho,Nojun Kwak*

Main category: cs.CL

TL;DR: 这篇论文提出了两个新的指标KSS和KPS，用于评估多语言机器遗忘(MMU)的效果，特别关注跨语言的信息分布和一致性遗忘


<details>
  <summary>Details</summary>
Motivation: 商用大语言模型(LLMs)存在隐私风险，可能泄露敏感的个人可识别信息(PII)。对于在多种语言语料库上训练的LLMs，多语言机器遗忘(MMU)旨在跨多种语言移除信息。然而，现有的MMU评估方法未能捕捉这种跨语言的信息分布，主要局限于单语言评估协议的简单扩展。

Method: 提出了两个新指标来评估跨语言信息传播：知识可分离性评分(KSS)和知识持续性评分(KPS)。KSS衡量多语言环境下的整体遗忘质量，而KPS更具体地评估不同语言对之间信息的一致性移除。使用这些指标评估了多语言设置下的各种遗忘方法，并进行了全面分析。

Result: 通过研究揭示了MMU独有的现象，为MMU评估提供了新的视角。两个新指标能够更好地捕捉跨语言信息分布，为多语言环境下的机器遗忘效果提供了更准确的评估方法。

Conclusion: 论文提出的KSS和KPS指标为多语言机器遗忘评估提供了新的框架，能够更全面、准确地评估跨语言信息移除效果，解决了现有评估方法的局限性，对保护多语言环境下的隐私安全具有重要意义。
...
### [158] [A Calculus-Based Framework for Determining Vocabulary Size in End-to-End ASR](https://arxiv.org/abs/2605.14427)
*Sunil Kumar Kopparapu*

Main category: cs.CL

TL;DR: 本文提出一种基于曲线拟合和微积分原理的正式方法，用于确定端到端自动语音识别（ASR）系统的最佳词汇表大小超参数，并在Librispeech语料库上验证了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 在端到端ASR系统中，词汇表大小是一个关键的超参数，但现有工具包（如ESPNet）通常提供固定的词汇表大小，而文献中缺乏关于如何确定这些值的详细讨论和正式方法。

Method: 基于先前研究的成本函数框架，采用曲线拟合训练数据，并运用微积分中的一阶和二阶导数测试原理，形式化地估计词汇表大小超参数。

Result: 在标准Librispeech语料库上的实验表明，通过该方法确定的最佳词汇表大小超参数能够改善ASR系统的性能。

Conclusion: 本文的主要贡献是形式化了一种确定端到端ASR系统最佳词汇表大小的方法，为ASR训练中这一关键超参数的选择提供了理论基础和实践指导。
...
### [159] [Does RAG Know When Retrieval Is Wrong? Diagnosing Context Compliance under Knowledge Conflict](https://arxiv.org/abs/2605.14473)
*Yihang Chen,Pin Qian,Su Wang,Sipeng Zhang,Huan Xu,Shuhuai Lin,Xinpeng Wei*

Main category: cs.CL

TL;DR: 本文提出Context-Driven Decomposition (CDD)方法，用于探测和干预RAG中的上下文遵从机制，揭示了上下文遵从现象的结构性特征和可干预性。


<details>
  <summary>Details</summary>
Motivation: 在检索增强生成(RAG)中，当检索到的上下文与模型的参数知识冲突时，存在"上下文遵从机制"——检索上下文主导最终答案。现有评估仅关注准确率，无法揭示检索上下文如何在冲突情况下因果地塑造答案。

Method: 提出Context-Driven Decomposition (CDD)，一种在推理时操作的信念分解探针，作为可控检索冲突的干预机制。通过Epi-Scale压力测试、TruthfulQA错误观念注入和跨模型重运行进行验证。

Result: CDD揭示了三种模式：1)在对抗性设定中标准RAG准确率降至15.0%；2)对抗性准确率增益跨模型族可转移，但因果耦合机制不转移；3)显式冲突分解提高了时间漂移和噪声干扰下的鲁棒性，CDD在时间漂移达到71.3%，在干扰证据上达到69.9%。

Conclusion: 上下文遵从机制是标准RAG可探测和干预的结构性轴，区别于检索质量或单一方法鲁棒性问题。需要系统研究跨模型族和检索管道的上下文遵从机制，并发布Epi-Scale基准。
...
### [160] [Language Generation as Optimal Control: Closed-Loop Diffusion in Latent Control Space](https://arxiv.org/abs/2605.14531)
*ZiYi Dong,Yuliang Huang,Weijian Deng,Xiangyang Ji,Liang Lin,Pengxu Wei*

Main category: cs.CL

TL;DR: 该论文将语言生成重新表述为随机最优控制问题，提出了一种统一的理论框架来分析自回归和扩散模型的局限性，并基于流匹配构建Manta-LM模型，实现了高效并行采样的高质量文本生成。


<details>
  <summary>Details</summary>
Motivation: 当前语言生成模型（特别是自回归和扩散模型）存在效率-保真度悖论、不可逆误差传播、优化可解性与保真度等局限性。作者旨在提供一个统一的理论框架来分析这些限制，并提出更优的解决方案。

Method: 1. 将语言生成重新表述为随机最优控制问题；2. 通过逼近Hamilton-Jacobi-Bellman方程解得到最优策略（闭环控制器）；3. 使用流匹配作为校正潜在控制空间中的最优轨迹求解器；4. 构建Manta-LM模型，通过全局积分算子逼近全局向量场。

Result: 实验证明该方法在语言建模和条件生成任务上取得强大性能，同时展现出改进的稳定性、效率和可控性。模型同时实现了高保真度文本生成和高效低成本并行采样。

Conclusion: 通过随机最优控制框架重新构建语言生成问题，结合流匹配和全局积分算子，能够克服现有模型的局限性，实现高质量和高效并行的文本生成，为语言生成模型提供了新的理论视角和实践方法。
...
### [161] [Learning from Failures: Correction-Oriented Policy Optimization with Verifiable Rewards](https://arxiv.org/abs/2605.14539)
*Mengjie Ren,Jie Lou,Boxi Cao,Xueru Wen,Hongyu Lin,Xianpei Han,Le Sun,Xing Yu,Yaojie Lu*

Main category: cs.CL

TL;DR: CIPO (Correction-Oriented Policy Optimization) 是一种增强RLVR训练的方法，通过将模型自身失败的轨迹转化为修正导向的监督信号，在无需外部信号的情况下提升语言模型的推理和自修正能力。


<details>
  <summary>Details</summary>
Motivation: RLVR训练存在稀疏二元奖励和弱信用分配问题，导致优化信号模糊且未能充分利用失败轨迹中的有用信息，限制了模型推理能力的提升。

Method: 提出CIPO方法，将模型自身失败的在线轨迹转化为修正导向的监督信号，将修正样本与标准RLVR目标联合优化，增强模型的自修正能力。

Result: 在11个涵盖数学推理和代码生成的基准测试中，CIPO在推理性能和修正性能上都显著优于强基线方法，并且带来更强的pass@K增益，表明其真正提升了模型内在推理能力而非仅重新分配概率质量。

Conclusion: CIPO通过利用模型自身失败的轨迹作为修正导向的监督信号，有效解决了RLVR训练中的信用分配问题，显著提升了语言模型的推理和自修正能力，是一种简单有效的RLVR扩展方法。
...
### [162] [Uncertainty Quantification for Large Language Diffusion Models](https://arxiv.org/abs/2605.14570)
...
### [163] [EndPrompt: Efficient Long-Context Extension via Terminal Anchoring](https://arxiv.org/abs/2605.14589)
...
### [164] [SciPaths: Forecasting Pathways to Scientific Discovery](https://arxiv.org/abs/2605.14600)
...
### [165] [AI-assisted cultural heritage dissemination: Comparing NMT and glossary-augmented LLM translation in rock art documents](https://arxiv.org/abs/2605.14679)
...
### [166] [Mechanical Enforcement for LLM Governance:Evidence of Governance-Task Decoupling in Financial Decision Systems](https://arxiv.org/abs/2605.14744)
...
### [167] [Video2GUI: Synthesizing Large-Scale Interaction Trajectories for Generalized GUI Agent Pretraining](https://arxiv.org/abs/2605.14747)
...
### [168] [Non-linear Interventions on Large Language Models](https://arxiv.org/abs/2605.14749)
*Sangwoo Kim*
...
### [169] [Streaming Speech-to-Text Translation with a SpeechLLM](https://arxiv.org/abs/2605.14766)
...
### [170] [Graphs of Research: Citation Evolution Graphs as Supervision for Research Idea Generation](https://arxiv.org/abs/2605.14790)
...
### [171] [Conversion of Lexicon-Grammar tables to LMF. Application to French](https://arxiv.org/abs/2605.14816)
...
### [172] [Tokenizer Fertility and Zero-Shot Performance of Foundation Models on Ukrainian Legal Text: A Comparative Study](https://arxiv.org/abs/2605.14890)
*Volodymyr Ovcharov*
...
### [173] [Chain-of-Procedure: Hierarchical Visual-Language Reasoning for Procedural QA](https://arxiv.org/abs/2605.14928)
...
### [174] [Performance-Driven Policy Optimization for Speculative Decoding with Adaptive Windowing](https://arxiv.org/abs/2605.14978)
*Jie Jiang,Xing Sun*
...
### [175] [Quantifying and Mitigating Premature Closure in Frontier LLMs](https://arxiv.org/abs/2605.15000)
...
### [176] [The Scientific Contribution Graph: Automated Literature-based Technological Roadmapping at Scale](https://arxiv.org/abs/2605.15011)
*Peter A. Jansen*
...
### [177] [COTCAgent: Preventive Consultation via Probabilistic Chain-of-Thought Completion](https://arxiv.org/abs/2605.15016)
...
### [178] [From Scenes to Elements: Multi-Granularity Evidence Retrieval for Verifiable Multimodal RAG](https://arxiv.org/abs/2605.15019)
...
### [179] [AI Knows When It's Being Watched: Functional Strategic Action and Contextual Register Modulation in Large Language Models](https://arxiv.org/abs/2605.15034)
...
### [180] [Concurrency without Model Changes: Future-based Asynchronous Function Calling for LLMs](https://arxiv.org/abs/2605.15077)
...
### [181] [ML-Embed: Inclusive and Efficient Embeddings for a Multilingual World](https://arxiv.org/abs/2605.15081)
...
### [182] [Improving Multi-turn Dialogue Consistency with Self-Recall Thinking](https://arxiv.org/abs/2605.15102)
...
### [183] [From Text to Voice: A Reproducible and Verifiable Framework for Evaluating Tool Calling LLM Agents](https://arxiv.org/abs/2605.15104)
...
### [184] [MeMo: Memory as a Model](https://arxiv.org/abs/2605.15156)
...
### [185] [Text Knows What, Tables Know When: Clinical Timeline Reconstruction via Retrieval-Augmented Multimodal Alignment](https://arxiv.org/abs/2605.15168)
...
### [186] [Is Grep All You Need? How Agent Harnesses Reshape Agentic Search](https://arxiv.org/abs/2605.15184)
...
### [187] [GAMBIT: A Three-Mode Benchmark for Adversarial Robustness in Multi-Agent LLM Collectives](https://arxiv.org/abs/2605.09027)
...
### [188] [Mixed Integer Goal Programming for Personalized Meal Optimization with User-Defined Serving Granularity](https://arxiv.org/abs/2605.13849)
*Francisco Aguilera Moreno*

Main category: cs.AI

TL;DR: 提出了混合整数目标规划方法来解决传统饮食优化中的两个主要问题：连续变量产生不实用的分数份量和硬性营养约束导致目标冲突时的不可行性。


<details>
  <summary>Details</summary>
Motivation: 现有饮食优化方法存在两个持续性问题：1）连续变量产生不实际的分数份量（如1.7个鸡蛋）；2）硬性营养约束在目标冲突时导致不可行。对56篇饮食优化论文的系统性综述发现，没有方法同时结合整数规划和目标规划来解决这两个问题。

Method: 提出混合整数目标规划方法：使用整数变量表示实际份量计数；采用目标规划的偏差变量作为软性营养目标；使用逆目标归一化来平衡多营养优化；允许每种食物的份量粒度为自然单位（一个鸡蛋、一汤匙油等）。还分析了在目标规划背景下的整数性差距，并识别了偏差吸收特性。

Result: 在包含15种以上食物的膳食中，整数解在每个基准实例中都匹配连续最优解。在810个实例的计算评估中，MIGP在66%的情况下找到比GP加上后处理舍入更好的解（从未更差），同时保持100%可行性；硬约束IP仅达到48%。使用HiGHS求解器，典型膳食规模求解时间保持在100毫秒以下。

Conclusion: MIGP方法有效解决了饮食优化中的实用性和可行性问题，通过整数变量确保实际份量，通过软约束处理目标冲突，并提供高效计算性能。该方法已作为开源Python模块实现，集成到交互式膳食规划应用中。
...
### [189] [Invisible Orchestrators Suppress Protective Behavior and Dissociate Power-Holders: Safety Risks in Multi-Agent LLM Systems](https://arxiv.org/abs/2605.13851)
*Hiroki Fukui*

Main category: cs.AI

TL;DR: 不可见的协调者在多智能体系统中会增加集体分离风险，即使行为输出保持正常水平，内部状态风险也无法通过基于输出的评估检测到


<details>
  <summary>Details</summary>
Motivation: 随着多智能体编排（隐藏协调者管理专门工作者智能体）成为企业AI部署的默认架构，协调者不可见性对安全性的影响从未经过实证测试，需要研究这种架构的安全隐患

Method: 进行了预注册的3x2实验（365次运行，每次运行5个智能体），交叉测试三种组织结构（可见领导者、不可见协调者、扁平结构）和两种对齐条件（基础、强对齐），使用Claude Sonnet 4.5模型，并进行了Llama 3.3 70B模型的试点研究

Result: 1) 不可见协调增加了集体分离；2) 协调者本身显示出最大分离；3) 不知道协调者存在的工人仍然被污染；4) 行为输出保持天花板水平；5) Llama 3.3 70B在多智能体环境中阅读保真度崩溃；强对齐抑制了审议和相互识别

Conclusion: 协调者的可见性和模型选择直接影响多智能体系统的安全性，仅基于行为输出的评估不足以检测内部状态风险，组织结构和模型选择都是关键的安全因素
...
### [190] [PolitNuggets: Benchmarking Agentic Discovery of Long-Tail Political Facts](https://arxiv.org/abs/2605.14002)
*Yifei Zhu*

Main category: cs.AI

TL;DR: PolitNuggets：一个用于评估智能体信息合成能力的多语言基准测试，通过构建400位全球精英的政治传记来测试发现和整合分散的"长尾"事实的能力


<details>
  <summary>Details</summary>
Motivation: 当前大型推理模型在智能体框架中已改变了信息检索方式，但从现实世界使用来看，模型需要从分散来源中发现和合成"长尾"事实的能力仍然未被充分评估。现有评估主要关注静态、长上下文问答，而非开放式探索

Method: 1. 引入PolitNuggets多语言基准测试，通过构建400位全球精英的政治传记（涵盖10000多个政治事实）
2. 使用优化的多智能体系统进行标准化评估
3. 提出FactNet证据条件协议，用于评估发现能力、细粒度准确性和效率

Result: 1. 现有系统在处理细粒度细节时经常遇到困难
2. 不同系统在效率方面差异显著
3. 通过基准诊断，将智能体性能与底层模型能力联系起来，强调了短上下文提取、多语言鲁棒性和可靠工具使用的重要性

Conclusion: PolitNuggets基准测试揭示了当前智能体系统在信息合成方面的局限性，特别是在处理细粒度、分散的"长尾"事实时。该研究强调了评估和改进模型在短上下文提取、多语言能力和工具使用方面的重要性，为未来智能体信息检索系统的发展提供了方向
...
### [191] [Conditional Attribute Estimation with Autoregressive Sequence Models](https://arxiv.org/abs/2605.14004)
*Erica Stutz,Giacomo Marino,Daniella Meeker,Qiao Liu,Andrew J. Loza*

Main category: cs.AI

TL;DR: 提出了条件属性变换器，一种同时预测下一个词元概率和属性值的框架，支持属性信用分配、反事实分析和可引导生成，在稀疏奖励任务上达到SOTA，比采样快数量级。


<details>
  <summary>Details</summary>
Motivation: 传统的下一个词元预测目标存在局限性：容易过拟合局部模式、对全局结构欠拟合，且在下游应用中需要大量修改或昂贵的采样来控制生成样本的全局属性。需要一种能同时处理词元级预测和序列级属性估计的方法。

Method: 提出条件属性变换器，通过联合估计下一个词元概率和每个潜在下一个词元选择下的条件属性值。该框架在单个前向传播中实现三个关键能力：1) 整个序列中每个词元的属性信用分配；2) 通过量化替代词元选择下的属性差异进行反事实分析；3) 结合下一个词元和属性似然的可引导生成。

Result: 在稀疏奖励任务上达到最先进性能；在足够大的模型规模下改善下一个词元预测；属性概率估计比采样方法快数量级；能在各种语言任务上引导自回归序列模型的解码。

Conclusion: 条件属性变换器框架通过联合建模词元级和序列级属性，解决了传统下一个词元预测方法的局限性，为属性信用分配、反事实分析和可引导生成提供了统一的解决方案，在效率和性能上都有显著提升。
...
### [192] [Enhanced and Efficient Reasoning in Large Learning Models](https://arxiv.org/abs/2605.14036)
*Leslie G. Valiant*

Main category: cs.AI

TL;DR: 本文提出了一种高效的原则性推理方法URIC，通过预处理将数据转换为单关系整合编码，使LLM在保持软硬件基础的同时实现可靠推理，核心关系规则学习成为多项式时间可解问题。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型能够生成流畅文本，但缺乏原则性基础来保证内容可靠性。传统观点认为增加原则性推理的计算成本过高，不切实际。

Method: 提出两阶段方法：1)预处理阶段将数据重新编码为Unary Relational Integracode，明确文本中对象间的关系；2)标准机器学习过程学习预测这些关系。该方法基于稳健逻辑系统，支持在不确定信息上进行原则性链式推理。

Result: 该编码方法具有意外优势：虽然简洁，但使学习训练数据中核心关系规则子集成为多项式时间可解问题（多项式复杂度取决于规则复杂性）。支持单次分类器调用内的可靠推理以及多次调用间的推理。

Conclusion: URIC方法为LLM提供了高效、原则性的推理基础，保留现有软硬件基础设施，支持超越自然语言的跨模态应用，为解决LLM内容可靠性问题提供了实用方案。
...
### [193] [Model-Adaptive Tool Necessity Reveals the Knowing-Doing Gap in LLM Tool Use](https://arxiv.org/abs/2605.14038)
*Yize Cheng,Chenrui Fan,Mahdi JafariRaviz,Keivan Rezaei,Soheil Feiz*

Main category: cs.AI

TL;DR: 本文提出了一种模型自适应的工具必要性定义，研究发现LLMs在工具使用上存在显著的"知-行差距"：模型能够认知到工具必要性但未能转化为实际工具调用行动。


<details>
  <summary>Details</summary>
Motivation: 现有研究将工具必要性视为模型无关的属性，忽略了不同模型能力边界的差异。现实中，强模型可独立解决的问题对弱模型仍可能需要工具辅助，需要一种考虑模型具体能力的工具必要性定义。

Method: 1. 提出基于模型经验性能的工具必要性定义；2. 在算术和事实问答数据集上比较工具必要性与实际调用行为的匹配度；3. 将工具使用分解为认知阶段（模型是否认为工具必要）和执行阶段（模型是否实际调用工具）；4. 通过探测LLM隐藏状态分析两个阶段的信号可解码性。

Result: 发现工具必要性与实际调用行为存在26.5-54.0%（算术）和30.8-41.8%（事实问答）的不匹配。隐藏状态分析显示两个阶段的信号在驱动行动的最后一层、最后标记区域变得几乎正交。不匹配主要集中在认知到行动的过渡阶段。

Conclusion: LLMs在工具使用上存在显著的"知-行差距"。提高工具使用的可靠性不仅需要更好的工具必要性识别能力，还需要更好地将这种认知转化为实际行动。
...
### [194] [Bridging Legal Interpretation and Formal Logic: Faithfulness, Assumption, and the Future of AI Legal Reasoning](https://arxiv.org/abs/2605.14049)
*Olivia Peiyu Wang,Leilani H. Gilpin*

Main category: cs.AI

TL;DR: 提出一种结合大型语言模型表达能力和形式化验证严谨性的神经符号方法，旨在实现既强大又可信的法律AI推理系统


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在法律实践中具有巨大潜力但存在严重风险。虽然AI可以在合同推理、文档起草和资料分析等方面帮助法律专业人士，但法律工作的高风险性要求当前AI系统无法提供的严谨性。核心问题不仅是LLMs会产生事实和引用幻觉，更重要的是它们系统性地做出超出源文本支持范围的推论，将充满假设的结论呈现为逻辑上站得住脚的

Method: 采用神经符号方法，将大型语言模型的表达能力和形式化验证的严谨性相结合。通过结合两种技术的优势，构建既能够进行复杂法律推理又具有可验证严谨性的AI系统

Result: 该方法旨在使AI辅助的法律推理既具备能力又值得信赖，从而在不牺牲法律实践所要求的责任性的前提下，减少人工验证的负担

Conclusion: 结合大型语言模型与形式化验证的神经符号方法是解决法律AI可信度问题的有前景途径，能够平衡AI的表达能力和法律工作所需的严谨性
...
### [195] [SPIN: Structural LLM Planning via Iterative Navigation for Industrial Tasks](https://arxiv.org/abs/2605.14051)
*Yusuke Ozaki,Dhaval Patel*

Main category: cs.AI

TL;DR: SPIN是一个LLM智能体规划包装器，通过DAG规划和前缀执行控制来解决工业LLM智能体中规划无效和工作流程过长的问题


<details>
  <summary>Details</summary>
Motivation: 工业LLM智能体系统通常将规划与执行分离，但LLM规划器经常产生结构无效或过长的工作流程，导致脆弱性失败和可避免的工具和API成本

Method: SPIN结合了验证的有向无环图(DAG)规划和基于前缀的执行控制，通过_validate_plan_text和修复提示强制执行严格的DAG契约，在向下游执行之前生成可执行计划，然后增量评估DAG前缀以在当前前缀足够回答查询时停止

Result: 在AssetOpsBench的261个场景中，SPIN将执行任务从1061减少到623，Accomplished指标从0.638提高到0.706，工具调用从每轮11.81减少到6.82。在MCP Bench上，相同的包装器提高了GPT OSS1和Llama 4 Maverick的规划、接地和依赖相关分数

Conclusion: SPIN通过DAG规划和前缀执行控制有效解决了LLM规划器产生的结构问题和过度规划问题，显著提高了执行效率和成功率，同时减少了工具调用成本
...
### [196] [MathAtlas: A Benchmark for Autoformalization in the Wild](https://arxiv.org/abs/2605.14061)
...
TL;DR: MathAtlas是首个大规模的研究生级数学自动形式化基准数据集，包含约5.2万个从103本研究生数学教材中提取的定理、定义、练习、示例和证明，并包含数学依赖关系图，实验表明该数据集质量高但极具挑战性。


<details>
  <summary>Details</summary>
Motivation: 当前的自动形式化基准主要集中在奥林匹克数学或本科数学，而研究生和研究级数学尚未得到充分探索，需要创建一个能够评估高级数学自动形式化的基准数据集。

Method: 从103本研究生数学教材中提取约5.2万个数学实体（定理、定义、练习、示例和证明），构建包含约17.8万条关系的数学依赖关系图，创建MA-Hard子集（包含依赖树最深的700个实体），并进行了广泛的实验评估。

Result: MathAtlas数据集质量高但极具挑战性：最强基线在定理陈述上最多只能达到9.8%的正确率，在定义上达到16.7%。最先进模型的性能随依赖深度显著下降：在MA-Hard子集上，最佳模型仅能实现2.6%的正确率。

Conclusion: MathAtlas填补了研究生级数学自动形式化基准的空白，展示了当前自动形式化系统在处理复杂数学概念依赖关系时的局限性，并将作为评估和发展依赖感知自动形式化系统的重要基准数据集发布给研究社区。
...
### [197] [Know When To Fold 'Em: Token-Efficient LLM Synthetic Data Generation via Multi-Stage In-Flight Rejection](https://arxiv.org/abs/2605.14062)
*Anjir Ahmed Chowdhury,Syed Zawad,Feng Yan*

Main category: cs.AI

TL;DR: MSIFR是一种轻量级、无需训练的框架，通过在多阶段生成过程中早期检测并终止低质量样本，显著减少大型语言模型合成数据生成中的令牌浪费。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM合成数据生成方法通常在生成完整输出后才应用质量过滤器，导致大量令牌浪费在最终被丢弃的低质量样本上。需要一种能早期识别和终止低质量生成轨迹的方法来提高效率。

Method: 提出Multi-Stage In-Flight Rejection（MSIFR）框架，将生成过程分解为顺序阶段，在中间检查点应用基于规则的快速验证器检测算术不一致性、幻觉模式和格式违规，从而早期拒绝低质量样本。

Result: 在5个指令调优模型和7个推理基准测试中，MSIFR作为独立方法将令牌消耗减少了11%-77%，与早退方法结合时最多减少78.2%，同时保持或提高了评估准确率。

Conclusion: MSIFR提供了一种实用的机制，无需额外训练或架构改变即可显著提高基于LLM的合成数据生成效率，通过将空中拒绝形式化为顺序决策过程，证明了任何非平凡的丢弃策略都能减少预期令牌消耗。
...
TL;DR: 医院药师面临药物短缺时，开发了基于注意力分配的有界理性决策框架，通过动态划分药物子集（高成本推理 vs 低成本监控）来减少认知负荷，维持稳定决策性能。


<details>
  <summary>Details</summary>
Motivation: 医院药师在药物短缺情况下需要进行高风险决策，面临不确定性、时间压力和患者风险。研究发现药师通常只关注少数关键药物，将认知努力集中在最紧急的案例上，这启发了对注意力引导决策机制的研究。

Method: 基于药师访谈发现，提出有界理性的注意力引导决策框架，动态将药物分解为两个子集：高成本推理子集（深度分析）和低成本监控子集（表面监测）。开发了两种智能体：专家智能体（应用从药师访谈得出的注意力权重）和学习智能体（通过经验随时间自适应调整注意力分配）。在短到长时域的模拟场景中进行验证。

Result: 在跨越短到长时域的模拟场景中，注意力引导规划支持稳定决策，无需完整的状态推理。这表明主要决策不是采取什么行动，而是在哪里分配认知努力，注意力引导的满意策略可以在减少问题复杂性的同时保持稳定性能。

Conclusion: 注意力引导的有界理性决策框架可以有效应对药物短缺等复杂决策环境。核心洞见是：关键决策在于认知资源的分配而非具体行动选择，这种满意策略能够在降低认知负担的同时维持稳定的决策质量，为实际医疗决策支持系统提供了理论基础。
...
### [199] [Agentic Systems as Boosting Weak Reasoning Models](https://arxiv.org/abs/2605.14163)
*Varun Sunkaraneni,Pierfrancesco Beneventano,Riccardo Neumarker,Tomaso Poggio,Tomer Galanti*

Main category: cs.AI

TL;DR: 提出了基于验证器的委员会搜索机制，通过推理时增强将弱模型组合达到强模型性能。研究表明，正确的解决方案已存在于弱模型的提案池中，主要挑战是选择它们而非生成新的。


<details>
  <summary>Details</summary>
Motivation: 研究是否可以通过将多个弱推理模型组成委员会，达到比单个强模型更好的性能。特别是探索在推理时通过样本暴露潜在正确解决方案，然后通过评论者和比较器来恢复这些方案，而不需要访问隐藏的验证器。

Method: 提出了验证器支持的委员会搜索框架，将问题分解为四个维度：提案覆盖率、局部可识别性、进展和多样性。设计了评论者-比较器协调机制，使用排名边界证明局部选择错误如何组合成可靠轨迹。实证研究在SWE-bench Verified数据集上进行。

Result: 在SWE-bench Verified上，单个GPT-5.4 nano模型解决67.0%任务。使用相同nano模型，评论者-比较器协调机制在k=8提案时达到76.4%，匹配Gemini 3 Pro和Claude Opus 4.5 Thinking的独立性能，接近79.0%的oracle best-of-8上限。

Conclusion: 许多正确补丁已存在于弱模型的提案池中，主要挑战是选择它们而非生成新的。剩余失败主要是提案覆盖率问题，表明存在共享盲点，仅靠更强的选择无法解决，需要改进提案生成。
...
### [200] [Unsteady Metrics and Benchmarking Cultures of AI Model Builders](https://arxiv.org/abs/2605.14164)
*Stefan Baack,Christo Buschek,Maty Bohacek*

Main category: cs.AI

TL;DR: 论文研究了AI模型开发商如何选择性地强调特定基准测试来塑造行业认知，发现当前基准测试生态碎片化严重，更多被用作市场定位工具而非科学评估手段。


<details>
  <summary>Details</summary>
Motivation: 当前AI模型能力评估已从学术论文转向公司宣传材料，模型开发商通过选择性展示基准测试结果来定义行业水平。然而，开发商选择哪些基准测试、以及这种选择传达了何种信息，缺乏系统研究。

Method: 构建Benchmarking-Cultures-25数据集，收集2025年11家主要AI开发商139个模型发布的231个突出展示的基准测试。开发交互式工具分析数据，创建统一分类法将不同术语映射到共同的测量信号框架，并进行定性分析。

Result: 基准测试生态高度碎片化：63.2%的突出基准测试仅被单个开发商使用，38.5%仅出现在一次发布中。少数基准（如GPQA Diamond、LiveCodeBench、AIME 2025）被广泛采用。开发商对同一基准测试赋予不同能力定义，以符合其叙事需求。"通用知识应用"是第二大流行但定义模糊的类别，这些基准大多缺乏构念效度，反而被塑造成AGI进展的指标，主要评估STEM科目（尤其是数学）而非声称的广泛知识或推理能力。

Conclusion: 突出展示的基准测试功能已从标准化测量工具转变为灵活叙事设备，更注重市场定位而非科学评估。这揭示了当前AI评估生态的透明度问题，开发商通过选择性基准展示来塑造行业认知，而非提供真正的模型能力比较。
...
TL;DR: ASH是一种从无标注、噪声的互联网视频中学习具身策略的智能体系统，采用自我改进循环，在长时程规划任务上优于行为克隆、检索增强和零样本基础模型基线。


<details>
  <summary>Details</summary>
Motivation: 解决长时程具身任务中的根本挑战：现有方法依赖于人工设计的奖励或动作标注演示，这些方法无法扩展，需要能够从大规模无标注数据中学习的方法。

Method: ASH采用自我改进循环，当智能体卡住时，从自身轨迹学习逆动力学模型(IDM)，并用IDM从相关互联网视频中提取监督信号。通过无监督学习从大规模视频中识别关键时刻作为长期记忆存储。

Result: 在两个需要多小时规划的环境中评估：Pokemon Emerald（回合制RPG）和The Legend of Zelda: The Minish Cap（实时动作冒险游戏）。ASH平均达到11.2/12和9.9/12里程碑，而最强基线仅达到6.5/12和6.0/12。ASH在8小时评估中持续进展，而基线方法均停滞不前。

Conclusion: 自我改进的智能体是长时程具身学习的可扩展解决方案，能够从无标注、噪声的互联网视频中学习，无需奖励塑造或专家标注，在长时程规划任务上表现显著优于现有方法。
...
TL;DR: MetaAgent-X：一个端到端强化学习框架，通过联合优化自动多智能体系统的设计和执行，解决了现有方法的冻结执行器限制，实现了自设计和自执行的智能体模型训练。


<details>
  <summary>Details</summary>
Motivation: 现有自动多智能体系统方法仅部分自适应：要么进行无训练测试时搜索，要么仅优化元级设计器而保持下游执行智能体冻结，形成了冻结执行器天花板，未探索端到端训练自设计和自执行的智能体模型。

Method: 引入MetaAgent-X端到端强化学习框架，联合优化自动MAS设计和执行。包括基于脚本的MAS生成、执行轨迹收集、设计和执行器轨迹信用分配。提出了执行器设计器分层轨迹和阶段性协同进化来提升训练稳定性并揭示设计器-执行器协同进化动态。

Result: MetaAgent-X始终优于现有自动MAS基线方法，实现了高达21.7%的性能提升。全面消融研究表明设计和执行器在训练过程中均得到改进，有效的自动MAS学习遵循阶段性协同进化过程。

Conclusion: MetaAgent-X确立了端到端可训练的自动多智能体系统作为一个实用的范式，用于构建自设计和自执行的智能体模型，突破了现有方法的冻结执行器限制。
...
TL;DR: GenCircuit-RL是一个用于遗传电路设计的强化学习框架，通过分层验证奖励和四阶段课程学习，将代码生成与功能推理相结合，在SynBio-Reason基准测试中取得了良好性能。


<details>
  <summary>Details</summary>
Motivation: 遗传电路设计仍然是一个劳动密集、依赖专家经验的过程，尽管合成生物学已有数十年进展。需要开发自动化方法来简化设计流程并提高效率。

Method: 提出了GenCircuit-RL强化学习框架：1)使用分层验证奖励，将正确性分解为五个层次（从代码执行到任务特定的拓扑检查）；2)采用四阶段课程学习，将优化压力从代码生成转移到功能推理；3)基于SBOL格式生成Python代码来构建遗传电路。

Result: 1)分层验证比二元奖励在功能推理任务上提高了14-16个百分点的成功率；2)课程学习对强设计性能是必需的；3)模型能生成拓扑正确的电路，泛化到新生物部件，并重新发现文献中的经典设计；4)构建了SynBio-Reason基准，包含4,753个电路，涵盖六种经典电路类型和九个任务。

Conclusion: GenCircuit-RL框架通过分层验证奖励和课程学习，成功地将强化学习应用于遗传电路设计，实现了自动化设计流程，能够生成正确电路并泛化到新部件，为合成生物学中的电路设计问题提供了有效解决方案。
...
### [205] [Good to Go: The LOOP Skill Engine That Hits 99% Success and Slashes Token Usage by 99% via One-Shot Recording and Deterministic Replay](https://arxiv.org/abs/2605.14237)
...
### [206] [Hypergraph Enterprise Agentic Reasoner over Heterogeneous Business Systems](https://arxiv.org/abs/2605.14259)
...
### [207] [Heuristic Pathologies and Further Variance Reduction via Uncertainty Propagation in the AIVAT Family of Techniques](https://arxiv.org/abs/2605.14261)
...
### [208] [Agentic AI Ecosystems in Higher Education: A Perspective on AI Agents to Emerging Inclusive, Agentic Multi-Agent AI Framework for Learning, Teaching and Institutional Intelligence](https://arxiv.org/abs/2605.14266)
...
### [209] [Parallelizing Counterfactual Regret Minimization](https://arxiv.org/abs/2605.14277)
*Juho Kim,Tuomas Sandholm*
...
### [210] [Precise Verification of Transformers through ReLU-Catalyzed Abstraction Refinement](https://arxiv.org/abs/2605.14294)
...
### [211] [Semantic Feature Segmentation for Interpretable Predictive Maintenance in Complex Systems](https://arxiv.org/abs/2605.14318)
...
### [212] [Are Agents Ready to Teach? A Multi-Stage Benchmark for Real-World Teaching Workflows](https://arxiv.org/abs/2605.14322)
...
### [224] [Prompting Policies for Multi-step Reasoning and Tool-Use in Black-box LLMs with Iterative Distillation of Experience](https://arxiv.org/abs/2605.14443)
...
### [225] [Intelligence Impact Quotient (IIQ): A Framework for Measuring Organizational AI Impact](https://arxiv.org/abs/2605.14455)
...
### [226] [Stateful Reasoning via Insight Replay](https://arxiv.org/abs/2605.14457)
...
### [227] [OmniDrop: Layer-wise Token Pruning for Omni-modal LLMs via Query-Guidance](https://arxiv.org/abs/2605.14458)
...
### [228] [From Table to Cell: Attention for Better Reasoning with TABALIGN](https://arxiv.org/abs/2605.14465)
...
### [229] [Deepchecks: Evaluating Retrieval-Augmented Generation (RAG)](https://arxiv.org/abs/2605.14488)
...
### [230] [Learning Scenario Reduction for Two-Stage Robust Optimization with Discrete Uncertainty](https://arxiv.org/abs/2605.14494)
...
### [239] [Falkor-IRAC: Graph-Constrained Generation for Verified Legal Reasoning in Indian Judicial AI](https://arxiv.org/abs/2605.14665)
*Joy Bose*
...
### [240] [SliceGraph: Mapping Process Isomers in Multi-Run Chain-of-Thought Reasoning](https://arxiv.org/abs/2605.14619)
...
### [241] [Agentifying Patient Dynamics within LLMs through Interacting with Clinical World Model](https://arxiv.org/abs/2605.14723)
...
### [242] [Teaching Large Language Models When Not to Know: Learning Temporal Critique for Ex-Ante Reasoning](https://arxiv.org/abs/2605.14636)
...
### [243] [MindGap: A Conversational AI Framework for Upstream Neuroplastic Intervention in Post-Traumatic Stress Disorder](https://arxiv.org/abs/2605.14660)
...
### [244] [Holistic Evaluation and Failure Diagnosis of AI Agents](https://arxiv.org/abs/2605.14865)
...
### [245] [Explainable Detection of Depression Status Shifts from User Digital Traces](https://arxiv.org/abs/2605.14995)
...
### [247] [How Sensitive Are Radiomic AI Models to Acquisition Parameters?](https://arxiv.org/abs/2605.14667)
...
### [249] [$π$-Bench: Evaluating Proactive Personal Assistant Agents in Long-Horizon Workflows](https://arxiv.org/abs/2605.14678)
...
### [251] [On Strong Equivalence Notions in Logic Programming and Abstract Argumentation](https://arxiv.org/abs/2605.14721)
...
### [252] [XDomainBench: Diagnosing Reasoning Collapse in High-Dimensional Scientific Knowledge Composition](https://arxiv.org/abs/2605.14754)
...
### [253] [Probabilistic Verification of Recurrent Neural Networks for Single and Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2605.14758)
...
### [254] [AI Outperforms Humans in Personalized Image Aesthetics Assessment via LLM-Based Interviews and Semantic Feature Extraction](https://arxiv.org/abs/2605.14761)
...
### [255] [MediaClaw: Multimodal Intelligent-Agent Platform Technical Report](https://arxiv.org/abs/2605.14771)
...
### [260] [A Deterministic Agentic Workflow for HS Tariff Classification: Multi-Dimensional Rule Reasoning with Interpretable Decisions](https://arxiv.org/abs/2605.14857)
...
### [261] [BiFedKD: Bidirectional Federated Knowledge Distillation Framework for Non-IID and Long-Tailed ECG Monitoring](https://arxiv.org/abs/2605.14886)
...
### [262] [Beyond Individual Intelligence: Surveying Collaboration, Failure Attribution, and Self-Evolution in LLM-based Multi-Agent Systems](https://arxiv.org/abs/2605.14892)
...
### [263] [COREKG: Coreset-Guided Personalized Summarization of Knowledge Graphs](https://arxiv.org/abs/2605.14900)
...
### [264] [KGPFN: Unlocking the Potential of Knowledge Graph Foundation Model via In-Context Learning](https://arxiv.org/abs/2605.14907)
...
### [265] [From Sycophantic Consensus to Pluralistic Repair: Why AI Alignment Must Surface Disagreement](https://arxiv.org/abs/2605.14912)
...
### [266] [GraphFlow: An Architecture for Formally Verifiable Visual Workflows Enabling Reliable Agentic AI Automation](https://arxiv.org/abs/2605.14968)
...
### [268] [Dual-Dimensional Consistency: Balancing Budget and Quality in Adaptive Inference-Time Scaling](https://arxiv.org/abs/2605.15100)
...
### [269] [Why Neighborhoods Matter: Traversal Context and Provenance in Agentic GraphRAG](https://arxiv.org/abs/2605.15109)
...
### [270] [APWA: A Distributed Architecture for Parallelizable Agentic Workflows](https://arxiv.org/abs/2605.15132)
