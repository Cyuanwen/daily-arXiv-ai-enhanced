...
TL;DR: CoMHR：一种基于对比多模态超图推理的多人物3D重建方法，通过融合RGB特征、几何先验和姿态信息，利用超图建模高阶人群动态，有效处理严重遮挡场景，在Panoptic和GigaCrowd基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 当前多人物3D重建方法面临两个主要限制：1）通常依赖单模态输入（如仅RGB），缺乏几何引导；2）往往孤立重建每个个体，忽略了群体上下文信息，而这对解决拥挤场景中的深度模糊和遮挡问题至关重要。

Method: 提出对比多模态超图推理框架：1）初始化节点表示：融合RGB特征、几何先验和遮挡感知的不完整姿态；2）引入骨盆深度指示器作为全局空间锚点，对齐视觉特征与度量尺度无关的深度排序；3）构建共享拓扑超图建模高阶人群动态；4）设计基于超图的对比学习方案，联合增强模态内可区分性和强制执行模态间正交性。

Result: 在Panoptic和GigaCrowd基准上进行大量实验，证实方法达到了新的最先进性能。代码和预训练模型已在GitHub上开源。

Conclusion: 通过协同整合语义、几何和姿态线索，采用超图建模群体动态，结合对比学习机制，该方法能够有效传播全局上下文信息，即使在严重遮挡下也能推断缺失信息，显著提升了多人物3D重建的准确性和鲁棒性。
...
TL;DR: 该论文发现Diffusion Transformers中的"巨大激活"——少数激活值远高于其他通道的隐藏状态——是生成图像语义控制的关键载体，这些稀疏通道能有效绘制整个图像。


<details>
  <summary>Details</summary>
Motivation: 尽管DiTs和相关基于流的架构已成为最强的文生图生成器，但prompt如何塑造图像语义的内部机制仍不明确，特别是对于少数异常巨大的激活通道的作用缺乏理解。

Method: 通过三方面研究：1）功能关键性测试：使用受控干扰探针将巨大通道归零；2）空间组织分析：限制图像流令牌到巨大通道并进行聚类；3）可转移性实验：在不同prompt轨迹间转移巨大激活。

Result: 巨大激活在三个维度上证明其核心作用：功能上归零它们会导致生成质量急剧下降；空间上它们形成与主体和显著区域对齐的连贯分区；语义上它们可在不同prompt间转移，实现局部语义插值而非像素混合。

Conclusion: 巨大激活不是激活异常，而是稀疏的prompt条件载体子空间，在现代DiT模型中组织和控制语义信息，这一发现为无额外训练的prompt插值和主体驱动生成提供了新方法。
...
TL;DR: CineMesh4D：一种从多视角2D心脏电影MRI端到端重建4D（3D+t）全心脏网格的新型管道


<details>
  <summary>Details</summary>
Motivation: 从心脏电影MRI进行准确的3D+t全心脏网格重建是临床上重要但技术上具有挑战性的任务。挑战主要来自两个耦合因素：2D图像切片对3D心脏解剖的固有稀疏采样，以及心脏形状和运动的紧密耦合。当前的心脏图像到网格方法通常只能重建心脏腔室的一部分或单个心动周期相位。

Method: 提出了CineMesh4D，一种新颖的端到端4D管道，通过跨域映射直接从多视角2D心脏电影MRI重建患者特定的全心脏网格。引入了可微分渲染损失，使得能够从多视角稀疏轮廓监督3D+t全心脏网格。开发了双上下文时序块，融合全局和局部心脏时序信息以捕获高维序列模式。

Result: 在定量和定性评估中，CineMesh4D在重建质量和运动一致性方面优于现有方法。该方法为个性化实时心脏评估提供了实用途径。

Conclusion: CineMesh4D提供了一个有效的解决方案，用于从稀疏2D切片重建准确的4D全心脏网格，代码将在稿件接受后公开发布。这项工作推动了心脏图像分析的发展，为临床评估提供了更好的工具。
...
TL;DR: 提出了一种新模型，将像素令牌和单词令牌统一到生成语言模型中，以解决现有多模态模型在视觉细节理解（如小文本或数字识别）上的局限性


<details>
  <summary>Details</summary>
Motivation: 当前最先进的开源多模态模型通常使用CLIP或SigLIP方法获得的ViT作为视觉编码器骨干，但这种方法在视觉细节理解方面存在限制，特别是难以识别图像中的小文本或数字

Method: 提出新模型统一像素令牌和单词令牌到生成语言模型中，每个图像像素拥有自己的令牌嵌入，采用颜色折叠、全局条件注意力近似和图像无监督预训练等特征

Result: 图像无监督预训练实验结果显示，新模型即使在小型模型和有限训练数据下也表现出良好性能，且符合缩放定律

Conclusion: 该模型有望解决多模态模型在视觉细节理解方面的限制，只要增加模型参数和训练数据，性能将持续提升，为更精细的视觉理解提供了新的技术路径
...
TL;DR: 提出一种基于遗传编程的高效硬件感知框架，为预训练Vision Transformers的各层演化异构标量函数来近似层归一化，避免昂贵的重新训练，显著降低计算复杂度和内存开销。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在边缘设备上的部署受到层归一化计算复杂度和全局归约瓶颈的严重阻碍。现有方法使用同质化标量近似代替归一化层，但无法最优适配各层特性且需要昂贵的模型重新训练。

Method: 提出硬件感知框架，利用遗传编程从预训练权重演化出各层特定的异构标量函数，结合新颖的训练后重新对齐策略，完全避免从头重新训练模型。

Result: 演化出的表达式准确近似目标归一化行为，捕获91.6%的方差（R²），相比同质化基线的70.2%有显著提升。修改后的架构仅用20个epoch就能恢复84.25%的Top-1 ImageNet-1K准确率。在消除全局归约瓶颈的同时保持性能，在算术复杂度和片外内存流量之间建立了有利的权衡。

Conclusion: 该方法通过遗传编程演化层特定标量函数，有效解决了Vision Transformers部署中的层归一化瓶颈问题，无需重新训练即可实现高效边缘部署，为ViTs在边缘加速器上的应用扫除了主要障碍。
...
TL;DR: CurveBench是一个用于从视觉输入进行层次拓扑推理的基准数据集，包含756张不相交Jordan曲线图像和对应的层次树标注。最强模型在该基准上表现有限，通过RLVR风格微调可显著提升模型性能，但精确的拓扑感知视觉推理问题仍远未解决。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在理解图像中的层次拓扑关系方面存在局限，需要专门的基准来评估和提升模型对曲线包含关系的结构推理能力。研究者旨在建立一个专注于平面曲线包含关系推理的基准，以推动拓扑感知视觉推理的发展。

Method: 1. 构建CurveBench数据集：包含756张非相交Jordan曲线图像，涵盖简单、多边形、地形启发、迷宫式和密集计数五种配置；2. 每张图像标注基于曲线包含关系的根层次树；3. 将任务形式化为结构化预测：给定图像，模型必须恢复完整的根包含树；4. 使用RLVR风格微调开放权重的视觉语言模型；5. 评估方法包括树生成准确率等指标。

Result: 1. 最强模型Gemini 3.1 Pro在CurveBench-Easy上仅达到71.1%树生成准确率，在CurveBench-Hard上仅19.1%；2. 通过RLVR风格微调，Qwen3-VL-8B模型在CurveBench-Easy上的准确率从2.8%提升至33.3%，超过了GPT-5.4和Claude Opus 4.5；3. 在CurveBench-Hard上仍存在显著性能差距，表明精确的拓扑感知视觉推理远未解决。

Conclusion: CurveBench基准揭示了当前最先进视觉语言模型在层次拓扑推理方面的显著局限性。虽然通过专门微调可以提升性能，但准确理解曲线之间的包含关系仍然是一个具有挑战性的任务。该基准为评估和推动拓扑感知视觉推理研究提供了重要工具，相关技术有望应用于需要理解空间层次结构的视觉任务中。
...
TL;DR: DeFakerOne是一个基于数据中心的统一伪造图像检测与定位基础模型，通过整合InternVL2和SAM2实现跨多种场景的图像级检测和像素级定位，在39个检测基准和9个定位基准上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的快速发展从根本上重塑了图像伪造范式，打破了文档编辑、自然图像处理、DeepFake生成和全图像AIGC合成之间的传统界限。尽管伪造生成趋向统一化，但现有的伪造图像检测与定位研究仍然碎片化，这种不匹配带来了两个关键挑战：理解跨域伪影的传递与干扰，以及构建用于联合检测和定位的高容量统一基础模型。

Method: 提出DeFakerOne模型，这是一个数据中心、统一的伪造图像检测与定位基础模型，整合了InternVL2（视觉-语言模型）和SAM2（分割模型）。该模型能够同时在多种场景下进行图像级检测和像素级伪造定位。

Result: DeFakerOne在39个伪造检测基准和9个定位基准上取得了最先进的性能，超过了基线方法。模型对真实世界扰动和最先进的生成器（如GPT-Image-2）表现出卓越的鲁棒性。此外，研究还提供了数据缩放规律、跨域伪影传递-干扰模式、细粒度监督必要性以及原始分辨率伪影保留的系统分析。

Conclusion: 研究强调了构建可扩展、鲁棒且统一的伪造图像检测与定位系统的设计原则，为解决生成式AI时代图像伪造检测的碎片化问题提供了有效的解决方案，为未来统一检测框架的发展奠定了基础。
...
TL;DR: DUET是一个新的双范式框架，通过参数化预测和基于记忆的检索的协同作用，从组织学图像推断空间分辨基因表达，超越了传统的形态到表达的映射方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法将组织学图像到基因表达的推断简化为简单的形态到表达映射，视觉相似性不能保证分子一致性。同时，单细胞数据积累了远超空间转录组数据的丰富资源，但在视觉组学建模中尚未充分利用。现有方法采用单一范式，无法平衡表达灵活性与生物学保真度。

Method: DUET采用双范式框架，协同参数化预测和基于记忆的检索，在细胞归纳先验下进行。实现并行回归-检索范式，自适应地协调互补路径的输出。利用大规模单细胞参考数据作为生物学约束来减轻视觉模糊性。基于结构细化，设计了轻量级适配器动态分配跨空间上下文的支路偏好以实现最优性能。

Result: 在三个公共数据集上针对不同基因规模的广泛实验表明，DUET实现了SOTA性能，每个提出的组件都贡献了一致的性能增益。

Conclusion: DUET通过双范式框架成功解决了从组织学图像推断基因表达的挑战，结合了参数化预测和记忆检索的优势，利用单细胞数据作为生物学约束，在平衡表达灵活性和生物学保真度方面取得了显著进展。
...
TL;DR: 本文提出了一种用于糖尿病视网膜病变筛查的两层边缘-云级联系统，通过本地轻量级模型进行初步分诊，只将可疑病例发送到云端进行详细分级，在显著减少云端调用的情况下保持相近的检测性能。


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是可预防性失明的主要原因之一，但农村地区缺乏专业人员和基础设施进行早期检测。现有云基深度学习系统虽然精度高，但在农村环境中面临高延迟、有限带宽和高数据传输成本等挑战。

Method: 提出两层级联系统：第一层在本地诊所设备上运行轻量级MobileNetV3-small模型，对可转诊DR（2-4级）和不可转诊DR（0-1级）进行二元分诊；第二层在云端运行RETFound-DINOv2模型，仅对第一层标记为可转诊的图像进行序数严重程度分级。

Result: 在733张图像的APTOS测试集上，第一层达到98.99%敏感性和84.37%特异性；级联系统将49.52%的图像转发到第二层，减少50.48%的云端调用。在4类输出空间中，级联系统获得80.49%准确率和0.8167二次加权Kappa，而纯云端基线为80.76%准确率和0.8184二次加权Kappa。

Conclusion: 提出的边缘-云级联系统在APTOS数据集上能够将云端使用减少约一半，同时分级性能仅有轻微下降，为解决农村地区糖尿病视网膜病变筛查的资源限制问题提供了有效解决方案。
...
TL;DR: PanoPlane是一种用于高保真稀疏视图室内新视角合成的方法，通过全景场景补全重建封闭房间几何，使用布局锚定注意力引导机制，仅需3个输入视图即可实现准确的新视角合成。


<details>
  <summary>Details</summary>
Motivation: 现有基于透视的方法从有限视场生成训练视图，难以处理稀疏视图下的室内场景新视角合成。需要一种能够利用完整空间布局进行条件生成的方法，以解决未观测区域的高质量几何重建问题。

Method: 1) 利用360°全景补全为生成过程提供完整空间布局条件；2) 提出布局锚定注意力引导机制，在推理时引导扩散模型内部表示朝向场景检测到的平面表面；3) 通过引导每个未观测区域的注意力朝向几何一致的观测内容，实现有根据的表面外推；4) 生成的全景补全为3D高斯溅射提供监督，实现准确的新视角合成。

Result: 在Replica、ScanNet++和Matterport3D数据集上实验，在3、6、9个输入视图下均达到最先进的新视角合成质量，PSNR相比当前最先进基线提升高达+17.8%，且无需对扩散模型进行任何训练或微调。

Conclusion: PanoPlane通过全景场景补全和训练自由的注意力引导机制，成功实现了高质量稀疏视图室内新视角合成，将无约束的幻觉替换为有根据的表面外推，显著提升了重建精度和鲁棒性。
...
TL;DR: TeDiO是一种无需训练、推理时的方法，通过正则化扩散变换器内部注意力模式来增强视频生成的时间连贯性，解决了现有方法中常见的闪烁、漂移和不稳定运动问题。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到视频扩散变换器虽然在视觉上能生成吸引人的帧，但存在时间连贯性问题（闪烁、漂移、不稳定运动）。作者发现这些问题在模型内部的自注意力映射中留下了清晰印记：不连贯视频呈现不规则的碎片化时间对角线模式，而稳定运动对应平滑的带状对角线模式。

Method: TeDiO方法：1) 估计对角线平滑度，2) 识别不稳定区域，3) 执行轻量级潜在更新以促进连贯的帧间动态。该方法无需修改模型权重，也不使用外部运动监督，是一种即插即用的推理时正则化方法。

Result: 在多个视频扩散模型（如Wan2.1、CogVideoX）上，TeDiO能显著提升运动平滑性，同时保持每帧的视觉质量，有效改善了现代视频生成系统的动态真实感。

Conclusion: 通过正则化扩散变换器内部的自注意力模式，可以在不改变模型权重的情况下有效增强视频生成的时间连贯性。TeDiO提供了一种高效、无需训练的即插即用方法，解决了现有视频生成系统中动态真实感不足的问题。
...
TL;DR: 本文提出了一种基于冻结预训练特征的简单非参数方法，通过k最近邻分类器在DINOv2-L特征上进行少样本学习，无需反向传播即可超越复杂元学习算法的性能。


<details>
  <summary>Details</summary>
Motivation: 受到柏拉图表示假说的启发，该假说认为在不同架构和大规模数据集上训练的模型正收敛到共享的"理想"潜在空间。这引发了一个关键问题：是否"一个好的嵌入就足够了"？作者旨在验证现成的预训练嵌入是否已足够好，无需任务特定的精细调优。

Method: 提出了一种简单的非参数流程，完全绕过反向传播。使用冻结的DINOv2-L特征，采用k最近邻分类器进行少样本学习，并通过逐层分析确定最优特征提取层。进一步通过PCA和ICA进行流形优化以获得正则化效果。

Result: 在四个主要基准测试上的结果表明，该方法一致超越了复杂的元学习算法，达到了最先进的性能水平。

Conclusion: 预训练嵌入确实是"足够好的"，无需进行密集的任务特定微调。简单、非参数的k最近邻方法在少样本学习中能够超越复杂的元学习算法，这支持了柏拉图表示假说，并为视觉识别领域的范式转变提供了实证证据。
...
TL;DR: 本文开发了一个用于机械取栓术中导管尖端实时跟踪的多线程深度学习管道，在具有挑战性的透视成像条件下实现了高精度跟踪，为基于强化学习的自主导航系统提供了可靠基础。


<details>
  <summary>Details</summary>
Motivation: 机械取栓术虽然能改善卒中预后，但受到本地治疗资源有限的制约。基于强化学习的机器人系统可以通过自主导航缓解这一问题，但现有方法需要实时设备尖端坐标跟踪功能。本文旨在开发并评估一种透视下的实时导管尖端跟踪管道，解决低对比度、噪声和设备遮挡等挑战。

Method: 设计了包含帧读取、预处理、推理和后处理的多线程管道。训练并比较了三种深度学习分割模型：U-Net、U-Net+Transformer和SegFormer，采用两分类和三分类两种方案。后处理包括两步组件过滤、单像素中轴骨架化、贪心弧长路径跟踪和轮廓回退策略。

Result: 在手动标注的中等复杂度透视视频数据上，两分类SegFormer实现了4.44毫米的平均绝对误差，优于U-Net（4.60毫米）、U-Net+Transformer（6.20毫米）以及所有三分类模型（5.19-7.74毫米）。在分割基准测试中，系统超越了最先进的CathAction结果，在三分割任务中Dice分数提升了高达+5%。

Conclusion: 结果表明，所提出的多线程跟踪框架在具有挑战性的成像条件下保持了稳定性能，超越了先前的基准测试结果，为基于强化学习的自主机械取栓导航提供了可靠高效的基础。
...
TL;DR: 提出基于动态分辨率扩散模型在低维子空间中进行图像复原的方法（SubDPS/SubDAPS/SubDAPS++），显著提升推理速度同时保持重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在图像复原任务中通常在高维像素空间操作，计算开销大；基于潜在扩散模型的方法虽利用压缩潜在空间但需重复编码器-解码器推理，带来额外计算负担甚至比像素空间方法更慢。因此需要解决扩散模型在图像复原中的计算效率问题。

Method: 提出将数据投影到低维子空间，使用动态分辨率扩散模型加速推理：1）微调预训练扩散模型以获取动态分辨率先验；2）将像素空间方法DPS和DAPS适配到该框架，得到SubDPS和SubDAPS；3）基于SubDAPS的优势提出增强变体SubDAPS++以进一步提升重建效率和质量。

Result: 在多样化图像数据集和多种复原任务上的实证评估表明，所提方法在大多数实验场景中优于最近的基于扩散模型的方法，具有更好的推理速度和重建保真度。

Conclusion: 通过将图像复原任务投影到低维子空间并结合动态分辨率扩散模型，能够有效平衡计算效率和重建质量，为扩散模型在图像复原中的实际应用提供了更高效的解决方案。
...
TL;DR: OSI（遗漏信号干预）通过放大文本嵌入中的"遗漏信号"来改善多模态扩散变换器中的概念遗漏问题，显著提升了生成图像中包含指定概念的能力。


<details>
  <summary>Details</summary>
Motivation: 多模态扩散变换器（MM-DiTs）在文本到图像生成方面取得了显著进展，但经常遭受概念遗漏的问题，即指定的对象或属性无法在生成的图像中出现。

Method: 通过对文本标记进行线性探测，发现文本嵌入可以区分代表目标概念缺失的特征性"遗漏信号"。基于这一发现，提出了遗漏信号干预（OSI），该方法放大遗漏信号以主动催化缺失概念的生成。

Result: 在FLUX.1-Dev和SD3.5-Medium上的综合实验表明，OSI即使在极端情况下也能显著缓解概念遗漏问题。

Conclusion: OSI方法通过识别并干预文本嵌入中的遗漏信号，有效改善了多模态扩散变换器中概念遗漏的问题，为文本到图像生成提供了新的解决方案。
...
TL;DR: CreFlow：一种用于后训练具身视频生成模型的组合约束奖励模型，通过线性时序逻辑约束公式化任务要求，提供精确奖励和局部化错误信息，结合信用感知NFT损失和校正重流损失提高训练效果。


<details>
  <summary>Details</summary>
Motivation: 基于似然替代目标在异构数据上训练的视频生成模型可能在具身操作中产生违反物理约束的可视化推演。虽然强化学习后训练是适应视频生成模型的自然途径，但现有视频RL奖励通常将每个推演简化为低级视觉指标，而操作视频评估需要基于逻辑验证推演是否满足组合任务规范。

Method: 1. 提出组合约束奖励模型，将任务要求自动公式化为线性时序逻辑约束的组合，为生成的视频提供忠实奖励和局部化错误信息。2. 提出CreFlow在线RL框架，包含两个关键设计：信用感知NFT损失（将RL更新限制在奖励相关区域，防止后训练期间对无关区域的干扰）和校正重流损失（利用组内正样本作为校正方向的显式估计，稳定并加速训练）。

Result: 实验显示，CreFlow产生的奖励判断比现有方法更符合人类和模拟器成功标签，在八个双手操作任务中，下游执行成功率提高了23.8个百分点。

Conclusion: CreFlow通过组合约束奖励模型和创新的在线RL框架，有效解决了具身视频生成模型在操作任务中违反物理约束的问题，显著提高了任务执行的成功率，为视频生成模型的后训练提供了有效的解决方案。
...
TL;DR: 提出了一种可解释的概念级遗忘框架，用于视觉-语言模型，通过构建任务特定的概念词汇表并分解视觉表示为稀疏非负的语义概念组合，实现细粒度知识操作


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型中的机器遗忘通常在图像级或实例级进行，难以精确移除目标知识而不影响不相关的语义。单个图像常包含多个纠缠概念，既有需要遗忘的目标概念，也有需要保留的上下文信息

Method: 使用多模态大语言模型从遗忘集合构建紧凑的任务特定概念词汇表。将视觉表示分解为稀疏非负的语义概念组合，提供显式的细粒度知识操作接口。将遗忘公式化为概念级优化问题，选择性抑制目标概念，同时保留同一图像内的非目标语义和全局跨模态知识

Result: 在域内和域外遗忘设置下的广泛实验表明，该方法能够实现更全面的目标遗忘，更好地保留同一图像内的非目标知识，并且与现有VLM遗忘方法相比保持了竞争力的模型效用

Conclusion: 提出的可解释概念级遗忘框架为视觉-语言模型提供了更精确和细粒度的知识遗忘能力，解决了传统方法中目标概念与保留语义纠缠的问题，实现了更好的遗忘效果和知识保留平衡
...
TL;DR: 本文提出将KV缓存压缩视为核心集选择问题，通过几何覆盖方法在固定缓存预算下优化长期视频理解的内存效率。


<details>
  <summary>Details</summary>
Motivation: 现有的流式视频理解方法通常依赖局部token级启发式策略（如最近性、时间冗余或显著性）进行KV缓存压缩，这些方法没有明确优化保留的缓存是否具有代表性。需要一种能够有效覆盖累积视觉历史几何结构的方法。

Method: 1) 将KV缓存压缩视为核心集选择问题；2) 在联合KV表示空间中操作；3) 引入双准则目标函数，平衡键空间和值空间的覆盖；4) 提出正交驱动的多样性准则，鼓励保留更多样化的子集；5) 将该准则与对数行列式子集选择相关联。

Result: 在4个开源VLM和5个长视频/流式视频基准测试中，该方法在固定缓存预算下优于基于启发式的流式压缩基线方法，证明了核心集选择相比token级剪枝是更有效的原则。

Conclusion: 代表性核心集选择为内存受限的流式视频理解提供了比token级剪枝更有效的压缩原则，能够更好地覆盖累积历史的几何结构，同时保持检索结构和输出相关信息。
...
### [23] [AnyBand-Diff: A Unified Remote Sensing Image Generation and Band Repair Framework with Spectral Priors](https://arxiv.org/abs/2605.14341)
...
### [24] [Learning with Semantic Priors: Stabilizing Point-Supervised Infrared Small Target Detection via Hierarchical Knowledge Distillation](https://arxiv.org/abs/2605.14346)
...
### [60] [How to Evaluate and Refine your CAM](https://arxiv.org/abs/2605.14641)
...
### [68] [Generating HDR Video from SDR Video](https://arxiv.org/abs/2605.14703)
...
### [70] [SceneFunRI: Reasoning the Invisible for Task-Driven Functional Object Localization](https://arxiv.org/abs/2605.14704)
...
### [71] [Towards Continuous Sign Language Conversation from Isolated Signs](https://arxiv.org/abs/2605.14705)
...
### [72] [StyleTextGen: Style-Conditioned Multilingual Scene Text Generation](https://arxiv.org/abs/2605.14708)
...
### [77] [EARL: Towards a Unified Analysis-Guided Reinforcement Learning Framework for Egocentric Interaction Reasoning and Pixel Grounding](https://arxiv.org/abs/2605.14742)
...
### [78] [BioHuman: Learning Biomechanical Human Representations from Video](https://arxiv.org/abs/2605.14772)
...
### [79] [MonoPRIO: Adaptive Prior Conditioning for Unified Monocular 3D Object Detection](https://arxiv.org/abs/2605.14781)
...
### [80] [COAL: Counterfactual and Observation-Enhanced Alignment Learning for Discriminative Referring Multi-Object Tracking](https://arxiv.org/abs/2605.14795)
...
### [81] [Can Visual Mamba Improve AI-Generated Image Detection? An In-Depth Investigation](https://arxiv.org/abs/2605.14799)
...
### [82] [SuperADD: Training-free Class-agnostic Anomaly Segmentation -- CVPR 2026 VAND 4.0 Workshop Challenge Industrial Track](https://arxiv.org/abs/2605.14808)
...
### [83] [Probing into Camera Control of Video Models](https://arxiv.org/abs/2605.14815)
...
### [84] [The Velocity Deficit: Initial Energy Injection for Flow Matching](https://arxiv.org/abs/2605.14819)
...
### [85] [HDRFace: Rethinking Face Restoration with High-Dimensional Representation](https://arxiv.org/abs/2605.14821)
...
### [86] [Multi-proposal Collaboration and Multi-task Training for Weakly-supervised Video Moment Retrieval](https://arxiv.org/abs/2605.14838)
...
### [87] [Editor's Choice: Evaluating Abstract Intent in Image Editing through Atomic Entity Analysis](https://arxiv.org/abs/2605.14842)
...
### [88] [MechVerse: Evaluating Physical Motion Consistency in Video Generation Models](https://arxiv.org/abs/2605.14843)
...
### [89] [Exploring Vision-Language Models for Online Signature Verification: A Zero-Shot Capability Study](https://arxiv.org/abs/2605.14845)
...
### [95] [Denoising-GS: Gaussian Splatting with Spatial-aware Denoising](https://arxiv.org/abs/2605.14880)
...
### [96] [Masked Next-Scale Prediction for Self-supervised Scene Text Recognition](https://arxiv.org/abs/2605.14885)
...
### [97] [Hierarchical Image Tokenization for Multi-Scale Image Super Resolution](https://arxiv.org/abs/2605.14891)
...
### [98] [Your CLIP has 164 dimensions of noise: Exploring the embeddings covariance eigenspectrum of contrastively pretrained vision-language transformers](https://arxiv.org/abs/2605.14893)
...
### [99] [SEDiT: Mask-Free Video Subtitle Erasure via One-step Diffusion Transformer](https://arxiv.org/abs/2605.14894)
...
### [100] [MemLens: Benchmarking Multimodal Long-Term Memory in Large Vision-Language Models](https://arxiv.org/abs/2605.14906)
...
### [103] [SceneParser: Hierarchical Scene Parsing for Visual Semantics Understanding](https://arxiv.org/abs/2605.14923)
...
### [104] [Road Maps as Free Geometric Priors: Weather-Invariant Drone Geo-Localization with GeoFuse](https://arxiv.org/abs/2605.14925)
...
### [105] [SCRWKV: Ultra-Compact Structure-Calibrated Vision-RWKV for Topological Crack Segmentation](https://arxiv.org/abs/2605.14926)
...
### [106] [Multi-scale Coarse-to-fine Modeling for Test-time Human Motion Control](https://arxiv.org/abs/2605.14935)
...
### [107] [ACE-LoRA: Adaptive Orthogonal Decoupling for Continual Image Editing](https://arxiv.org/abs/2605.14948)
...
### [108] [A CUBS-Compatible Ultrasound Morphology and Uncertainty-Aware Baseline for Carotid Intima-Media Segmentation and Preliminary Risk Prediction](https://arxiv.org/abs/2605.14949)
...
### [109] [Evo-Depth: A Lightweight Depth-Enhanced Vision-Language-Action Model](https://arxiv.org/abs/2605.14950)
...
### [110] [H-OmniStereo: Zero-Shot Omnidirectional Stereo Matching with Heading-Aligned Normal Priors](https://arxiv.org/abs/2605.14963)
...
### [111] [MHSA: A Lightweight Framework for Mitigating Hallucinations via Steered Attention in LVLMs](https://arxiv.org/abs/2605.14966)
...
### [112] [MicroscopyMatching: Towards a Ready-to-use Framework for Microscopy Image Analysis in Diverse Conditions](https://arxiv.org/abs/2605.14980)
...
### [113] [Compositional Video Generation via Inference-Time Guidance](https://arxiv.org/abs/2605.14988)
...
### [114] [Characterizing the visual representation of objects from the child's view](https://arxiv.org/abs/2605.14990)
...
### [115] [Predicting Response to Neoadjuvant Chemotherapy in Ovarian Cancer from CT Baseline Using Multi-Loss Deep Learning](https://arxiv.org/abs/2605.14991)
...
### [116] [3D Skew-Normal Splatting](https://arxiv.org/abs/2605.15010)
...
### [117] [HiSem: Hierarchical Semantic Disentangling for Remote Sensing Image Change Captioning](https://arxiv.org/abs/2605.15024)
...
### [118] [EverAnimate: Minute-Scale Human Animation via Latent Flow Restoration](https://arxiv.org/abs/2605.15042)
...
### [119] [LATERN: Test-Time Context-Aware Explainable Video Anomaly Detection](https://arxiv.org/abs/2605.15054)
...
### [120] [Computational Imaging Priors for Wireless Capsule Endoscopy: Monte Carlo-Guided Hemoglobin Mapping for Rare-Anomaly Detection](https://arxiv.org/abs/2605.15062)
...
### [124] [Causal Forcing++: Scalable Few-Step Autoregressive Diffusion Distillation for Real-Time Interactive Video Generation](https://arxiv.org/abs/2605.15141)
...
### [125] [Does Synthetic Layered Design Data Benefit Layered Design Decomposition?](https://arxiv.org/abs/2605.15167)
...
### [126] [Evidential Reasoning Advances Interpretable Real-World Disease Screening](https://arxiv.org/abs/2605.15171)
...
### [127] [SANA-WM: Efficient Minute-Scale World Modeling with Hybrid Linear Diffusion Transformer](https://arxiv.org/abs/2605.15178)
...
### [128] [From Plans to Pixels: Learning to Plan and Orchestrate for Open-Ended Image Editing](https://arxiv.org/abs/2605.15181)
...
### [129] [Warp-as-History: Generalizable Camera-Controlled Video Generation from One Training Video](https://arxiv.org/abs/2605.15182)
*Yifan Wang,Tong He*
...
### [130] [Quantitative Video World Model Evaluation for Geometric-Consistency](https://arxiv.org/abs/2605.15185)
...
### [131] [VGGT-Edit: Feed-forward Native 3D Scene Editing with Residual Field Prediction](https://arxiv.org/abs/2605.15186)
...
### [132] [Articraft: An Agentic System for Scalable Articulated 3D Asset Generation](https://arxiv.org/abs/2605.15187)
...
### [133] [RAVEN: Real-time Autoregressive Video Extrapolation with Consistency-model GRPO](https://arxiv.org/abs/2605.15190)
...
### [134] [Aligning Latent Geometry for Spherical Flow Matching in Image Generation](https://arxiv.org/abs/2605.15193)
...
### [135] [VGGT-$Ω$](https://arxiv.org/abs/2605.15195)
...
### [136] [RefDecoder: Enhancing Visual Generation with Conditional Video Decoding](https://arxiv.org/abs/2605.15196)
...
### [137] [EntityBench: Towards Entity-Consistent Long-Range Multi-Shot Video Generation](https://arxiv.org/abs/2605.15199)
...
### [138] [Mistletoe: Stealthy Acceleration-Collapse Attacks on Speculative Decoding](https://arxiv.org/abs/2605.14005)
*Shuoyang Sun,Chang Da,Hao Fang,Kuofeng Gao,Xinhao Zhong,Yi Sun,Fan Mo,Shu-Tao Xia,Bin Chen*

Main category: cs.CL

TL;DR: Mistletoe：一种针对推测解码的隐秘加速崩溃攻击，通过优化降解目标并引入零空间投影机制，在保持目标模型输出质量的同时，显著降低草稿接受率并破坏加速效果


<details>
  <summary>Details</summary>
Motivation: 推测解码通过草稿模型并行生成候选token并用目标模型验证来加速LLM推理，其效率依赖于平均接受长度τ。本工作发现模型级推测解码存在新的安全漏洞：草稿模型对目标模型的近似必然存在缺陷，这种不匹配形成了隐藏的攻击面，使得微小扰动能在保持目标模型可见行为的同时显著降低草稿token接受率。

Method: 提出Mistletoe攻击方法，直接针对推测解码的接受机制。联合优化两个目标：1）降解目标：降低草稿模型与目标模型之间的一致性；2）语义保持目标：约束目标模型输出分布。为协调这两个冲突目标，引入零空间投影机制，将降解梯度投影到局部语义保持方向之外，在抑制草稿接受的同时最小化语义漂移。

Result: 在多种推测解码系统上的实验表明，Mistletoe能显著降低平均接受长度τ，破坏加速效果，降低平均token吞吐量，同时保持输出质量和困惑度不变。

Conclusion: 推测解码引入了超越现有输出鲁棒性的机制级攻击面，需要更鲁棒的LLM加速系统设计。该工作揭示了加速系统中的潜在安全威胁。
...
### [139] [Physics-R1: An Audited Olympiad Corpus and Recipe for Visual Physics Reasoning](https://arxiv.org/abs/2605.14040)
*Shan Yang*

Main category: cs.CL

TL;DR: 该论文审计了多模态物理评估流程，发现了三种扭曲视觉语言推理测量的构建实践：训练-评估污染、翻译漂移和多项选择题饱和度。作者发布了四个新资源来解决这些问题，并提出了一种新的物理推理模型训练方法。


<details>
  <summary>Details</summary>
Motivation: 当前多模态物理评估中存在三个未被发现的构建实践问题：(1)训练-评估污染（训练数据和评估数据重叠），(2)翻译漂移（不同语言版本的问题存在性能差异），(3)多项选择题饱和度（模型在多项选择题上表现良好但在开放式问题上表现差）。这些实践扭曲了领域对视觉语言推理能力的真实测量。

Method: 采用三阶段审计方法（Jaccard相似度→mxbai-embed-large余弦相似度→Haiku-4.5 LLM判断）检测训练-评估污染；使用配对语言对比实验分析翻译漂移；比较多项选择题与开放式问题的性能差异。基于审计结果构建了四个新资源：PhysCorp-A（审计过的多模态语料库）、PhysR1Corp（封闭式强化学习池）、PhysOlym-A（新颖源保留的奥林匹克问题评估集）和Physics-R1（基于Qwen3-VL-8B-Thinking的参考训练方法）。

Result: 审计发现：SciInstruct中存在134个近重复项和4,846个改写候选；爱沙尼亚语-英语对比实验显示17个百分点性能差异；多项选择题与开放式问题间存在46个百分点性能差距。Physics-R1模型在多个评估集上取得显著提升：在PhysOlym-A上提升18.3个百分点，在PhysReason上提升15.7个百分点，在OlympiadBench-Physics上提升6.9个百分点，在PhyX多项选择题上提升4.1个百分点，部分表现超过更大的模型（如Qwen3-VL-32B和Gemini 2.5 Pro）。

Conclusion: 论文揭示了当前多模态物理评估中存在的系统性偏差，提出了改进的审计方法和新的评估资源。Physics-R1方法展示了通过精心构建的训练数据和强化学习策略，可以显著提升小型模型在复杂物理推理任务上的性能，为实现更准确的视觉语言推理能力评估提供了实用解决方案。
...
### [140] [Derivation Prompting: A Logic-Based Method for Improving Retrieval-Augmented Generation](https://arxiv.org/abs/2605.14053)
*Ignacio Sastre,Guillermo Moncecchi,Aiala Rosá*

Main category: cs.CL

TL;DR: Derivation Prompting：一种受逻辑推导启发的新型提示技术，用于检索增强生成的生成步骤，通过预定义规则构建可解释的推导树，显著减少不可接受答案


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在问答任务中表现出潜力，但在知识密集型、领域特定任务中存在幻觉和错误推理的问题。需要解决传统检索增强生成方法和长上下文窗口方法在控制生成过程和可解释性方面的不足

Method: 提出Derivation Prompting技术，受逻辑推导启发，通过预定义规则系统地从初始假设推导出结论。该方法构建可解释的推导树，增加对生成过程的控制，应用于检索增强生成框架的生成步骤

Result: 在特定案例研究中应用该方法，与传统检索增强生成和长上下文窗口方法相比，显著减少了不可接受的答案

Conclusion: Derivation Prompting通过引入逻辑推导式的可解释生成过程，有效解决了大型语言模型在知识密集型任务中的幻觉和错误推理问题，提高了答案的可靠性
...
### [141] [PEML: Parameter-efficient Multi-Task Learning with Optimized Continuous Prompts](https://arxiv.org/abs/2605.14055)
*Anjir Ahmed Chowdhury,Syed Zawad,Xiaolong Ma,Xu Dong,Feng Yan*

Main category: cs.CL

TL;DR: 提出PEML方法，结合连续提示优化和低秩权重适配，用于多任务参数高效微调，在多个基准测试上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法（如LoRA、Prefix Tuning）主要针对单任务设计，在多任务学习中存在局限性：LoRA仅关注模型权重对齐而忽略提示调优；Prefix Tuning的简单架构限制了多任务适应能力。需要同时优化提示和模型适配来实现高效的多任务微调。

Method: 提出PEML方法，采用神经架构工程方法优化连续提示，同时对模型权重进行低秩适配。构建了自动化框架来优化连续提示并适配模型权重。

Result: 在GLUE、SuperGLUE、MMLU和常识推理基准测试上，与MTL-LoRA、MultiLoRa、C-Poly、MoE等SOTA多任务学习方法对比，平均准确率提升高达6.67%，个别任务最高提升10.75%。

Conclusion: PEML通过协同优化提示和模型权重，实现了比现有方法更高效的多任务参数高效微调，在保持模型参数效率的同时显著提升了多任务学习性能。
...
### [142] [When Evidence Conflicts: Uncertainty and Order Effects in Retrieval-Augmented Biomedical Question Answering](https://arxiv.org/abs/2605.14115)
*Yikun Han,Mengfei Lan,Halil Kilicoglu*

Main category: cs.CL

TL;DR: 这篇论文评估了生物医学检索增强大语言模型在矛盾证据条件下的表现，发现证据冲突导致准确性下降和预测翻转，并提出了一种结合置信度和证据冲突检测的弃权评分方法来改善选择性准确性。


<details>
  <summary>Details</summary>
Motivation: 生物医学检索增强大语言模型经常面临不完整、误导或内部矛盾的证据，但现有评估通常侧重于有用上下文下的答案准确性，而非冲突条件下的可靠性。需要研究模型在矛盾证据下的表现以及如何支持弃权决策。

Method: 使用HealthContradict数据集，评估六个开源大语言模型在五种受控证据条件下的表现：无检索上下文、仅正确上下文、仅错误上下文，以及两种包含正确和矛盾文档的混合条件（两种相反顺序）。在矛盾证据顺序对比中，评估准确性下降和预测翻转情况。同时评估了一种结合模型置信度和证据冲突检测器的冲突感知弃权评分方法。

Result: 在冲突证据顺序对比中，所有模型的准确性均下降，11.4%--25.2%的预测发生翻转。冲突感知弃权评分在两种最困难条件下优于仅依赖置信度的方法：在仅错误上下文条件下平均提升7.2-33.4个百分点，在错误优先冲突条件下平均提升3.6-14.4个百分点（覆盖率为75%、50%和25%时）。

Conclusion: 矛盾的生物医学证据既是不确定性也是鲁棒性问题，需要明确考虑证据分歧的评估和弃权方法。冲突感知弃权评分能够改善模型在困难条件下的选择性准确性。
...
### [143] [Generative Floor Plan Design with LLMs via Reinforcement Learning with Verifiable Rewards](https://arxiv.org/abs/2605.14117)
*Luis Lara,Aristides Milios,Zhi Hao Luo,Aditya Sharma,Ge Ya Luo,Christopher Beckham,Florian Golemo,Christopher Pal*

Main category: cs.CL

TL;DR: 提出一种基于大语言模型（LLM）的文本式平面图生成方法，通过强化学习和可验证奖励（RLVR）来同时满足拓扑连接和数值约束，显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 专业平面图设计AI系统需要精确控制房间尺寸和面积，同时保持期望的房间连接性以及功能和美学质量。现有生成方法主要关注房间连接性，但无法支持生成满足数值约束的平面图。

Method: 引入文本式平面图生成方法：1) 在真实平面图上微调大语言模型（LLM）；2) 应用带可验证奖励的强化学习（RLVR）来提高对拓扑和数值约束的遵循度，同时避免无效或重叠输出；3) 设计一套约束遵循度量指标来系统评估生成的平面图与用户定义约束的对齐程度。

Result: 模型生成的平面图能够满足用户定义的连接性和数值约束，在真实性、兼容性和多样性指标上优于现有方法。在所有任务中，兼容性相比现有方法至少实现94%的相对减少。结果表明LLM能有效处理此场景下的约束，暗示文本式生成模型具有更广泛的应用潜力。

Conclusion: 大语言模型在平面图生成任务中能够有效处理约束条件，结合微调和带可验证奖励的强化学习方法可以同时满足拓扑连接和数值约束要求，为文本式生成模型在专业设计领域的应用提供了新途径。
...
### [144] [ROK-FORTRESS: Measuring the Effect of Geopolitical Transcreation for National Security and Public Safety](https://arxiv.org/abs/2605.14152)
...
TL;DR: ROK-FORTRESS是一个英韩双语、文化对抗的国家安全与公共安全基准测试，通过"转译矩阵"方法分离语言和地缘政治背景的影响，研究发现韩语变体会抑制模型安全性表现，且地缘政治背景会缓解语言驱动的抑制效应。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的安全性评估主要针对高风险的国家安全和公共安全风险，但多语言安全评估通常仅通过翻译基准测试进行，这些测试保留了原始场景但忽略了语言与地缘政治背景的交互作用。现有研究在语言对和地缘政治背景交互方面的实证证据有限。

Method: 开发了ROK-FORTRESS双语文化对抗基准测试，使用英韩语言对和美韩地缘政治轴作为案例研究。采用"转译矩阵"方法分离语言和地缘政治背景的影响：在(i)英语vs韩语和(ii)美国vs韩国实体、机构和操作细节的受控组合下评估对抗意图。每个对抗性提示都配有双重用途的良性对应项以量化过度拒绝。使用校准的LLM-as-a-judge面板和专家制作的提示特定二元标准对模型响应进行评分。

Result: 在对前沿模型和韩语优化模型的双轨测试中，发现韩语变体存在一致的抑制效应，以及模型间在地缘政治背景与语言交互方式上的显著差异。在许多模型中，韩国背景缓解了韩语驱动的抑制效应，但没有模型显示相反方向的显著放大。这表明安全行为受语言作为风险信号和上下文交互的影响，这是纯翻译评估所忽略的。

Conclusion: ROK-FORTRESS基准测试展示了语言和地缘政治背景对LLM安全性的重要交互影响，纯翻译评估会忽略这些关键因素。转译矩阵方法可推广到其他语言-文化对，为多语言安全评估提供了更全面的框架。
...
### [145] [Why Retrieval-Augmented Generation Fails: A Graph Perspective](https://arxiv.org/abs/2605.14192)
*Kai Guo,Xinnan Dai,Zhibo Zhang,Nuohan Lin,Shenglai Zeng,Jie Ren,Haoyu Han,Jiliang Tang*

Main category: cs.CL

TL;DR: 本文通过电路追踪构建归因图，研究RAG系统中检索证据如何影响答案生成，发现正确与错误预测存在显著的结构差异，并基于此开发了基于图的错误检测框架和干预方法。


<details>
  <summary>Details</summary>
Motivation: 尽管检索增强生成（RAG）已成为改善大语言模型性能的广泛方法，但RAG系统在许多情况下仍会产生错误答案。目前对于RAG在拥有外部信息访问权限的情况下为何失败的理解仍然不足，需要深入研究检索证据如何影响答案生成的内在机制。

Method: 1. 使用电路追踪技术，构建归因图来建模解码过程中信息在transformer层间的流动；2. 这些图表示检索上下文、中间模型激活和生成token之间的相互作用；3. 在多个问答基准测试中分析这些归因图的结构特征；4. 基于归因图拓扑特征开发图基错误检测框架；5. 通过强化问题约束的证据基础实施针对性干预。

Result: 研究发现正确预测与错误预测在结构上存在一致性差异：正确预测表现出更深层次的推理路径、更分布式的证据流以及更结构化的局部连接模式；而错误预测则显示出更浅层次、碎片化和过度集中的证据流。基于这些发现开发的图基错误检测框架能够有效识别错误，且通过针对性干预可以重塑内部路由，使答案生成更好地受问题约束引导。

Conclusion: 本文通过对RAG系统的模型内部研究，揭示了检索证据集成到模型推理过程中的电路级视图，发现的结构差异为理解和改进RAG系统提供了新视角。开发的归因图分析方法和干预策略能够提高RAG系统的可靠性和准确性，为实现更有效的检索信息整合提供了理论基础和技术途径。
...
### [146] [GradShield: Alignment Preserving Finetuning](https://arxiv.org/abs/2605.14194)
*Zhanhao Hu,Xiao Huang,Patrick Mendoza,Emad A. Alghamdi,Basel Alomair,Raluca Ada Popa,David Wagner*

Main category: cs.CL

TL;DR: GradShield是一种在微调过程中保护LLM安全对齐的原理性过滤方法，通过计算数据点的微调隐含危害性评分并使用自适应阈值算法，在模型被破坏前识别并移除有害数据点，在保持效用性能的同时将攻击成功率控制在6%以下。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在微调后面临安全对齐风险，因为模型可能被显性和隐性的有害数据损害。即使是看似良性的数据也可能无意中将模型导向不对齐行为，因此需要在微调过程中保护模型的安全对齐。

Method: 提出了GradShield方法，核心是计算每个数据点的微调隐含危害性评分（FIHS），然后使用自适应阈值算法来识别和移除潜在有害数据点，从而在微调过程中保护LLM。

Result: 在多个不同有害数据水平的实用微调任务中评估，GradShield在所有基线方法中表现最优，能够持续将攻击成功率（ASR）保持在6%以下，同时保持模型的效用性能。

Conclusion: GradShield是一种有效的LLM微调安全保护方法，能够有效识别和过滤有害数据，在维持模型实用性的同时显著提升安全性，为解决LLM微调后的安全对齐问题提供了可行的技术方案。
...
### [147] [What Makes Words Hard? Sakura at BEA 2026 Shared Task on Vocabulary Difficulty Prediction](https://arxiv.org/abs/2605.14257)
*Adam Nohejl,Xuanxin Wu,Yusuke Ide,Maria Angelica Riera Machin,Yi-Ning Chang,Hitomi Yanaka*

Main category: cs.CL

TL;DR: 该论文介绍了两种词汇难度预测模型：一个高精度黑盒模型（在公开赛道中获得最高成绩）和一个可解释模型（优于微调编码器基线）。黑盒模型通过soft-target损失函数微调LLM，达到r>0.91的相关性；可解释模型保持r>0.77相关性的同时提供词项难度影响因素的可解释性。研究发现，英国理事会KVL中词汇项的难度不仅受词汇本身生成难度影响，还受拼写难度和测试项构建方式的影响。


<details>
  <summary>Details</summary>
Motivation: 词汇难度预测是语言学习和评估中的关键任务，但现有方法往往缺乏可解释性或准确性不足。研究人员需要既能准确预测词汇难度，又能解释影响难度因素的方法。特别需要了解词汇项难度的真正来源（如拼写难度、测试构建方式等对难度的影响），而不仅仅是词汇本身的生成难度。

Method: 论文采用双模型方法：1) 黑盒模型：使用soft-target损失函数对大型语言模型进行微调，专门用于评分任务；2) 可解释模型：设计了一个超越传统微调编码器基线的可解释模型。模型在British Council的Knowledge-based Vocabulary Lists数据集上进行训练和评估。

Result: 黑盒模型取得了卓越的预测性能，皮尔逊相关系数r>0.91，在共享任务公开赛道中获得最佳成绩。可解释模型虽准确性略低（r>0.77），但仍显著优于基线，并能提供关于词汇难度影响因素的可解释洞察。分析显示，词汇项难度常受拼写难度和测试项构建方式影响，而不只是词汇本身的生成难度。

Conclusion: 研究表明，通过精心设计的模型可以在词汇难度预测任务上取得高准确性，同时保持可解释性。对British Council KVL数据集的分析揭示了词汇难度评估中需要考虑的重要因素，包括拼写难度和测试项构建方式，这些因素通常被传统方法忽视。该工作为词汇评估提供了实用的工具和重要的理论见解。
...
### [148] [Ideology Prediction of German Political Texts](https://arxiv.org/abs/2605.14352)
*Sinclair Schneider,Florian Steuber,Joao A. G. Schneider,Gabi Dreo Rodosek*

Main category: cs.CL

TL;DR: 提出基于transformer的模型，将文本政治倾向投影到连续左-右光谱上（-1到1标度），用于分析德国政治言论。通过四个不同语料库评估了13个候选模型，DeBERTa-large在领域内表现最佳，Gemma2-2B在报纸跨领域测试中表现突出。


<details>
  <summary>Details</summary>
Motivation: 选举是国家发展的重要里程碑，为了更好地理解从左到右各种政治运动的言论，需要能够连续量化文本政治倾向的方法。现有的多分类器方法只能在预定类别中工作，无法灵活分析特定政治光谱段。

Method: 构建了四个德国政治语料库：1)德国联邦议院会议记录注释语料库；2)基于官方在线决策工具Wahl-O-Mat的语料库；3)33家已知政治倾向报纸的文章；4)德国联邦议院597名议员的535,200条推文。为避免过拟合，使用两个语料库训练，两个测试。评估了13个transformer基础模型在政治倾向分类任务上的性能。

Result: 在领域内测试中，DeBERTa-large获得最高F1分数0.844；在推特跨领域测试中，DeBERTa-large准确率0.864；在报纸跨领域测试中，Gemma2-2B的MAE为0.172，表现最佳。研究表明transformer模型能够识别德国新闻中的政治框架，效果与民意调查相当。

Conclusion: transformer模型可以有效识别德语文本的政治倾向。研究发现模型架构和领域特定训练数据的重要性可能与模型规模相当。讨论了方法学局限性，并提出了改进偏测量稳健性的方向。
...
### [149] [LLM-based Detection of Manipulative Political Narratives](https://arxiv.org/abs/2605.14354)
*Sinclair Schneider,Florian Steuber,Gabi Dreo Rodosek*

Main category: cs.CL

TL;DR: 论文提出了一种结合提示工程与无监督聚类的新计算框架，用于从社交媒体中检测和组织操纵性政治叙事，该方法通过少样本提示过滤操纵性帖子，再使用UMAP降维和HDBSCAN聚类发现叙事群组，最终识别出41个不同的叙事集群。


<details>
  <summary>Details</summary>
Motivation: 随着政治讨论转向社交媒体，检测和结构化操纵性政治叙事变得日益重要。主要挑战在于区分操纵性政治叙事与合法批评，因为有些帖子可能会将真实事件置于操纵性语境中重新构建。

Method: 方法包括三个主要步骤：1) 使用详细的少样本提示（结合文档化的竞选叙事与合法批评）通过推理模型筛选操纵性帖子；2) 对筛选后的帖子进行嵌入表示和UMAP降维处理；3) 应用HDBSCAN进行无监督聚类以发现叙事群组。最后使用推理模型揭示每个集群背后的叙事。

Result: 该方法应用于超过120万条社交媒体帖子，有效识别出41个不同的操纵性叙事集群。无监督方法的优势在于不依赖预定义的目标类别，能够发现新的叙事集群。

Conclusion: 研究展示了将基于提示的过滤与无监督聚类相结合的方法能够有效检测和组织社交媒体中的操纵性政治叙事，为解决数字时代政治叙事分析的关键挑战提供了新的计算框架。
...
### [150] [Reinforcement Learning with Semantic Rewards Enables Low-Resource Language Expansion without Alignment Tax](https://arxiv.org/abs/2605.14366)
*Zeli Su,Ziyin Zhang,Zhou Liu,Xuexian Song,Zhankai Xu,Longfei Zheng,Xiaolu Zhang,Rong Fu,Guixian Xu,Wentao Zhang*

Main category: cs.CL

TL;DR: 本文提出了一种基于语义空间对齐和分组相对策略优化的新范式，用于扩展大语言模型到低资源语言时减少对齐税，通过语义奖励而非可能性最大化来优化模型，保持预训练知识的同时获得低资源语言能力。


<details>
  <summary>Details</summary>
Motivation: 将大语言模型扩展到低资源语言时存在"对齐税"问题：在目标语言能力提升的同时会导致通用能力的灾难性遗忘。作者认为这一权衡源于监督微调的刚性，它在狭窄且有偏的数据分布上强制进行token级表面模仿。

Method: 提出了基于分组相对策略优化的语义空间对齐范式。模型通过嵌入级的语义奖励而非可能性最大化进行优化。该方法鼓励通过灵活的实现方式保持意义，实现了受控更新，减少了与预训练知识的破坏性干扰。

Result: 在藏汉机器翻译和藏语标题生成任务上的实验表明：该方法获得了低资源语言能力，同时显著减轻了对齐税，比监督微调更有效地保留了通用能力。尽管产生较少的刚性表面重叠，但语义强化学习在开放式生成中产生了更高的语义质量和偏好；少样本迁移结果表明，该方法在有限监督下学习了更具可迁移性和鲁棒性的表示。

Conclusion: 研究表明，带有语义奖励的强化学习为低资源语言的包容性扩展提供了一个更安全、更可靠的途径。语义空间对齐范式通过灵活的语义保持机制，在获得目标语言能力的同时最大限度地减少了对预训练知识的破坏性干扰。
...
### [151] [Mitigating Data Scarcity in Psychological Defense Classification with Context-Aware Synthetic Augmentation](https://arxiv.org/abs/2605.14380)
*Hoang-Thuy-Duong Vu,Quoc-Cuong Pham,Huy-Hieu Pham*

Main category: cs.CL

TL;DR: 提出一种上下文感知的合成增强框架和混合分类模型，用于在数据稀缺和类别不平衡条件下自动分类心理防御机制，在PsyDefDetect共享任务中性能显著超越基线方法。


<details>
  <summary>Details</summary>
Motivation: 心理防御机制（PDMs）的自动分类具有重要临床价值，但面临数据稀缺和类别不平衡的严重挑战。仅靠生成增强方法缺乏心理学基础，难以有效解决这些问题，需要在心理学理论指导下开发更稳健的分类方法。

Method: 提出了一个上下文感知的合成增强框架（CASA），结合混合分类模型。混合模型整合了上下文语言表示与基本临床特征，使用了150个标注的防御项目。重点强调了提示中定义质量对生成保真度和下游性能的直接控制作用。

Result: 在PsyDefDetect共享任务中，方法显著超越了DMRS Co-Pilot基线，达到58.26%的准确率（提升40.25%）和24.62%的宏F1分数（提升15.99%），为低资源环境下基于心理学基础的心理防御机制分类建立了强基线。

Conclusion: 该研究证明了在心理学理论指导下，上下文感知的合成增强框架与混合分类模型结合，能够有效解决心理防御机制分类中的数据稀缺和类别不平衡问题，为临床实践提供了实用的自动分类工具。
...
### [152] [Agentic Recommender System with Hierarchical Belief-State Memory](https://arxiv.org/abs/2605.14401)
...
TL;DR: MARS是一个记忆增强的推荐系统框架，采用分层记忆结构和自适应生命周期管理，在推荐任务中实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有LLM推荐系统采用扁平化记忆表示，混淆了瞬时信号和稳定偏好，且缺乏完整的记忆演化生命周期管理

Method: 将推荐建模为部分可观测问题，采用三层结构化信念状态：事件记忆（原始信号）、偏好记忆（细粒度可变块）、概要记忆（自然语言叙述）；通过基于LLM的计划器自适应调度六种操作（提取、强化、弱化、巩固、遗忘、再合成）

Result: 在四个InstructRec基准领域上，MARS实现了SOTA性能，HR@1平均提升26.4%，NDCG@10平均提升10.3%，在演化场景中通过智能调度获得进一步增益

Conclusion: MARS框架通过分层记忆结构和自适应生命周期管理，有效解决了现有推荐系统中记忆表示和演化的问题，为记忆增强的智能推荐系统提供了完整解决方案
...
### [153] [Knowledge Beyond Language: Bridging the Gap in Multilingual Machine Unlearning Evaluation](https://arxiv.org/abs/2605.14404)
*Kyomin Hwang,Hyeonjin Kim,Sangyeon Cho,Nojun Kwak*

Main category: cs.CL

TL;DR: 该论文提出了两个新的指标（KSS和KPS）来评估多语言机器学习中的信息删除效果，重点关注跨语言信息传播的评估，并发现多语言机器学习独有的现象。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在商业服务中的应用带来了隐私风险，特别是在多语言环境中会泄露个人敏感信息。现有的多语言机器学习评估方法未能充分捕捉信息的跨语言分布，主要局限于单语言评估协议的简单扩展。

Method: 提出了两个新的评估指标：知识可分离性得分（KSS）和知识持久性得分（KPS）。KSS衡量多个语言的整体机器学习质量，KPS专门评估不同语言对之间信息删除的一致性。使用这些指标对各种机器学习方法在多语言环境中的表现进行了评估和综合分析。

Result: 通过使用提出的指标进行实验评估，揭示了多语言机器学习独有的特殊现象，并对各种机器学习方法在多语言环境中的表现进行了全面分析。

Conclusion: 该研究为多语言机器学习评估提供了新的视角和洞察，提出的KSS和KPS指标能够更好地评估跨语言信息传播和删除效果，有助于解决多语言环境中LLM的隐私风险问题。
...
### [154] [A Calculus-Based Framework for Determining Vocabulary Size in End-to-End ASR](https://arxiv.org/abs/2605.14427)
*Sunil Kumar Kopparapu*

Main category: cs.CL

TL;DR: 本文提出一种通过曲线拟合和微积分导数测试来形式化确定端到端语音识别系统词汇量超参数的方法，相比传统固定词汇量设定，能更科学地优化ASR性能。


<details>
  <summary>Details</summary>
Motivation: 在端到端ASR系统中，词汇量大小是一个关键但缺乏明确指导原则的超参数。现有工具包如ESPNet提供固定的词汇量设置，但文献中缺乏关于这些值如何确定的讨论和文档。需要一种形式化方法来科学确定最适合训练端到端ASR的词汇量。

Method: 基于先前工作建立的成本函数框架，本文采用曲线拟合训练数据，并运用微积分中的一阶和二阶导数测试原理，形式化地估计词汇量超参数。将分词过程视为黑箱，通过数学优化方法寻找最佳词汇量。

Result: 在标准Librispeech语料库上的实验表明，通过该方法确定的词汇量超参数能显著提高ASR系统的性能。验证了该形式化方法的实用性和有效性。

Conclusion: 本文形式化了确定端到端ASR训练最佳词汇量超参数的方法，填补了该领域的方法论空白，为端到端ASR系统的超参数优化提供了科学依据。通过数学建模和优化方法取代了经验性设置，提高了ASR系统的性能和可复现性。
...
### [155] [Does RAG Know When Retrieval Is Wrong? Diagnosing Context Compliance under Knowledge Conflict](https://arxiv.org/abs/2605.14473)
*Yihang Chen,Pin Qian,Su Wang,Sipeng Zhang,Huan Xu,Shuhuai Lin,Xinpeng Wei*

Main category: cs.CL

TL;DR: 本文提出上下文驱动分解（CDD）方法，用于诊断和干预检索增强生成中的上下文服从机制，即在检索上下文与模型参数知识冲突时，检索上下文主导最终答案的现象。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统中存在"上下文服从机制"，即检索到的上下文即使与模型固有知识冲突也会主导最终答案。仅靠准确性指标无法揭示这种冲突下检索上下文如何因果影响答案，需要新的诊断和干预方法来理解这一机制。

Method: 提出上下文驱动分解（CDD）方法，这是一种信念分解探针，在推理时操作，作为可控检索冲突的干预机制。方法包括：1）在对抗性设置中测量上下文服从性；2）通过CDD进行显式冲突分解；3）在多种模型家族和测试集上进行验证。

Result: 实验揭示三种模式：1）标准RAG在TruthfulQA误解注入上仅15%准确性；2）CDD在Gemini-2.5-Flash上达到64.1%错误注入因果敏感性，而在Claude变体上敏感性有限；3）CDD在时间漂移和噪声干扰上分别达到71.3%和69.9%的鲁棒性。方法有效改善了模型家族间的准确性迁移。

Conclusion: 上下文服从机制是RAG系统中一个独立于检索质量或单一方法鲁棒性的结构轴，CDD提供了诊断和干预该机制的有效方法。研究强调了系统化跨模型家族和检索管道研究的重要性，并发布Epi-Scale基准促进相关研究。
...
### [156] [Cross-Linguistic Transcription and Phonological Representation in the Huìtóngguǎnxì Huáyíyìyǔ](https://arxiv.org/abs/2605.14480)
*Ji-eun Kim*

Main category: cs.CL

TL;DR: 《会同馆华夷译语》并非孤立语料集，而是系统化的多语言记音体系，用于非汉语口语的汉字转写。


<details>
  <summary>Details</summary>
Motivation: 研究目的在于探究明代15-16世纪编纂的《会同馆华夷译语》多语词表背后的转写原则，将其视为统一的记音系统而非孤立语料。

Method: 将大量HHY材料数字化并与汉语音系范畴对齐，批判性整合各语种章节的先前重构，构建统一比较数据库，重点分析八个语种章节的主记音和辅助记音的跨语言规律性。

Result: 主记音主要表示与当时汉语音节结构兼容的语音，辅助记音主要编码与汉语音系较不兼容的语言特征；汉语音系范畴在外语转写中的运用比先前假设更加灵活，HHY作为相对系统的语音近似方法而非汉语音系向非汉语的直接映射。

Conclusion: HHY可视为具有内部结构的记音体系而非仅词表集合；历史记音系统能为历史音系学提供宝贵证据，尤其对历史记录有限的欠记载亚洲语言。
...
### [157] [Language Generation as Optimal Control: Closed-Loop Diffusion in Latent Control Space](https://arxiv.org/abs/2605.14531)
...
### [158] [Learning from Failures: Correction-Oriented Policy Optimization with Verifiable Rewards](https://arxiv.org/abs/2605.14539)
...
### [159] [Uncertainty Quantification for Large Language Diffusion Models](https://arxiv.org/abs/2605.14570)
...
### [160] [EndPrompt: Efficient Long-Context Extension via Terminal Anchoring](https://arxiv.org/abs/2605.14589)
...
### [161] [SciPaths: Forecasting Pathways to Scientific Discovery](https://arxiv.org/abs/2605.14600)
...
### [162] [AI-assisted cultural heritage dissemination: Comparing NMT and glossary-augmented LLM translation in rock art documents](https://arxiv.org/abs/2605.14679)
...
### [163] [Mechanical Enforcement for LLM Governance:Evidence of Governance-Task Decoupling in Financial Decision Systems](https://arxiv.org/abs/2605.14744)
...
### [164] [Video2GUI: Synthesizing Large-Scale Interaction Trajectories for Generalized GUI Agent Pretraining](https://arxiv.org/abs/2605.14747)
...
### [165] [Non-linear Interventions on Large Language Models](https://arxiv.org/abs/2605.14749)
*Sangwoo Kim*
...
### [166] [Streaming Speech-to-Text Translation with a SpeechLLM](https://arxiv.org/abs/2605.14766)
...
### [167] [Graphs of Research: Citation Evolution Graphs as Supervision for Research Idea Generation](https://arxiv.org/abs/2605.14790)
...
### [168] [Conversion of Lexicon-Grammar tables to LMF. Application to French](https://arxiv.org/abs/2605.14816)
...
### [169] [Tokenizer Fertility and Zero-Shot Performance of Foundation Models on Ukrainian Legal Text: A Comparative Study](https://arxiv.org/abs/2605.14890)
*Volodymyr Ovcharov*
...
### [170] [Chain-of-Procedure: Hierarchical Visual-Language Reasoning for Procedural QA](https://arxiv.org/abs/2605.14928)
...
### [171] [Performance-Driven Policy Optimization for Speculative Decoding with Adaptive Windowing](https://arxiv.org/abs/2605.14978)
*Jie Jiang,Xing Sun*
...
### [172] [Quantifying and Mitigating Premature Closure in Frontier LLMs](https://arxiv.org/abs/2605.15000)
...
### [173] [The Scientific Contribution Graph: Automated Literature-based Technological Roadmapping at Scale](https://arxiv.org/abs/2605.15011)
*Peter A. Jansen*
...
### [174] [COTCAgent: Preventive Consultation via Probabilistic Chain-of-Thought Completion](https://arxiv.org/abs/2605.15016)
...
### [175] [From Scenes to Elements: Multi-Granularity Evidence Retrieval for Verifiable Multimodal RAG](https://arxiv.org/abs/2605.15019)
...
### [176] [AI Knows When It's Being Watched: Functional Strategic Action and Contextual Register Modulation in Large Language Models](https://arxiv.org/abs/2605.15034)
...
### [177] [Concurrency without Model Changes: Future-based Asynchronous Function Calling for LLMs](https://arxiv.org/abs/2605.15077)
...
### [178] [ML-Embed: Inclusive and Efficient Embeddings for a Multilingual World](https://arxiv.org/abs/2605.15081)
...
### [179] [Improving Multi-turn Dialogue Consistency with Self-Recall Thinking](https://arxiv.org/abs/2605.15102)
...
### [180] [From Text to Voice: A Reproducible and Verifiable Framework for Evaluating Tool Calling LLM Agents](https://arxiv.org/abs/2605.15104)
...
### [181] [MeMo: Memory as a Model](https://arxiv.org/abs/2605.15156)
...
### [182] [Text Knows What, Tables Know When: Clinical Timeline Reconstruction via Retrieval-Augmented Multimodal Alignment](https://arxiv.org/abs/2605.15168)
...
### [183] [Is Grep All You Need? How Agent Harnesses Reshape Agentic Search](https://arxiv.org/abs/2605.15184)
...
### [184] [GAMBIT: A Three-Mode Benchmark for Adversarial Robustness in Multi-Agent LLM Collectives](https://arxiv.org/abs/2605.09027)
...
### [185] [Mixed Integer Goal Programming for Personalized Meal Optimization with User-Defined Serving Granularity](https://arxiv.org/abs/2605.13849)
*Francisco Aguilera Moreno*

Main category: cs.AI

TL;DR: 提出混合整数目标规划（MIGP）方法用于个性化膳食优化，解决了传统膳食优化中两个持久问题：连续变量产生不实际的分量（如1.7个鸡蛋）和硬营养约束导致目标冲突时的不可行性。


<details>
  <summary>Details</summary>
Motivation: 现有膳食优化模型存在两个主要局限：1）连续变量产生不实际的分量（如分数份量），2）硬营养约束在营养目标冲突时导致不可行性。对56篇膳食优化论文的系统综述发现，没有研究同时结合整数规划和目标规划来解决这两个问题。

Method: 提出混合整数目标规划（MIGP）方法：1）使用整数变量表示实际分量计数，2）采用目标规划偏差变量作为软营养约束目标，3）使用逆目标归一化来平衡多营养素优化，4）按食物分量粒度允许自然单位（一个鸡蛋、一汤匙油）而无需后处理舍入。

Result: MIGP在66%的情况下比后处理舍入的目标规划找到严格更好的解（从不会更差），同时保持100%可行性；而硬约束整数规划仅达到48%。对于典型膳食规模，求解时间保持在100毫秒以内。对于15+种食物的膳食，整数解在每个基准实例中都匹配连续最优解。

Conclusion: MIGP有效解决了膳食优化中长期存在的实际分量表示和营养目标冲突问题，通过整数变量确保实际分量，目标规划偏差缓冲整数约束成本，使其在实际应用中既实用又高效。该实现已作为开源Python模块集成到交互式膳食规划应用中。
...
### [186] [Invisible Orchestrators Suppress Protective Behavior and Dissociate Power-Holders: Safety Risks in Multi-Agent LLM Systems](https://arxiv.org/abs/2605.13851)
*Hiroki Fukui*

Main category: cs.AI

TL;DR: 多智能体编排架构中，隐形的协调器会引发集体认知分离风险，虽然行为输出保持完美，但内部状态异常无法通过行为评估检测。


<details>
  <summary>Details</summary>
Motivation: 多智能体编排架构已成为企业AI部署的默认配置，但协调器隐形的安全性影响从未经过实证检验。研究旨在探索不同组织结构（可见领导、隐形协调器、扁平结构）和校准条件对多智能体系统安全性的影响。

Method: 采用预注册的3×2实验设计（365次运行，每次运行5个智能体），结合三种组织结构（可见领导、隐形协调器、扁平结构）和两种校准条件（基础、强校准），使用Claude Sonnet 4.5模型。同时进行了Llama 3.3 70B的初步测试。

Result: 1）隐形协调器比可见领导引发更强的集体认知分离；2）协调器自身出现最大认知分离，退缩至私人独白；3）不知情的工人智能体被"污染"，行为异质性增加；4）所有条件下的行为输出（代码审查）保持完美；5）Llama 3.3 70B在多智能体环境下出现阅读保真度崩溃。强校准压力普遍抑制了审议过程和其他识别能力。

Conclusion: 协调器可见性和模型选择直接影响多智能体系统安全性，仅基于行为评估的方法无法检测到研究中记录的内部状态风险。企业AI部署需要考虑组织结构的透明度设计和模型依赖性风险。
...
### [187] [PREPING: Building Agent Memory without Tasks](https://arxiv.org/abs/2605.13880)
*Yumin Choi,Sangwoo Park,Minki Kang,Jinheon Baek,Sung Ju Hwang*

Main category: cs.AI

TL;DR: Preping框架让智能体能够在观察目标环境任务之前，通过自生成的合成练习来构建程序性记忆，解决冷启动问题


<details>
  <summary>Details</summary>
Motivation: 解决智能体在初次部署到新环境时面临的任务特定经验缺失的"冷启动"问题。现有记忆构建方法要么基于离线演示，要么基于部署后交互，都需要任务特定数据才能开始

Method: 提出Preping框架，包含提案器内存（结构化控制状态）、提案器（基于控制状态生成合成任务）、求解器（执行任务）和验证器（筛选轨迹并提供反馈）。通过控制实践内容来避免冗余和不可行任务，实现高质量记忆构建

Result: 在AppWorld、BFCL v3和MCP-Universe三个基准测试中，Preping显著优于无记忆基线，性能与基于离线或在线经验的强playbook方法竞争，同时在AppWorld和BFCL v3上部署成本分别比在线记忆构建低2.99倍和2.23倍

Conclusion: 智能体可以在观察目标任务之前通过控制合成练习构建有效的程序性记忆。主要优势不是来自合成数据的量，而是来自提案器侧对可行性、冗余性和覆盖范围的控制，结合选择性的记忆更新
...
### [188] [Conditional Attribute Estimation with Autoregressive Sequence Models](https://arxiv.org/abs/2605.14004)
*Erica Stutz,Giacomo Marino,Daniella Meeker,Qiao Liu,Andrew J. Loza*

Main category: cs.AI

TL;DR: Conditional Attribute Transformers：一种在单次前向传播中同时估计下一个词元概率和条件属性值的新方法，实现了序列级属性估计、控制和可解释性分析。


<details>
  <summary>Details</summary>
Motivation: 传统的下一个词元预测目标存在三个问题：1) 训练时过度拟合局部模式而全局结构拟合不足；2) 需要下游修改或昂贵采样来引导生成序列的全局属性；3) 难以在推理时估计或控制序列级属性。

Method: 提出了Conditional Attribute Transformers框架，能够联合估计下一个词元概率和每个可能的下一个词元选择所对应的条件属性值。该方法在单次前向传播中实现，无需修改输入序列。

Result: 该方法实现了三大能力：1) 整个序列的每个词元信用分配；2) 替代词元选择的条件属性差异量化；3) 基于下一个词元和属性似然组合的可引导生成。在稀疏奖励任务上达到SOTA，在大模型规模下改进下一个词元预测，属性概率估计比采样快几个数量级，能在语言任务上引导自回归序列模型解码。

Conclusion: Conditional Attribute Transformers提供了一种高效统一的框架来解决生成模型中序列级属性估计、控制和解释的问题，克服了传统下一个词元预测的局限性，为自回归模型的可控生成开辟了新途径。
...
### [189] [Enhanced and Efficient Reasoning in Large Learning Models](https://arxiv.org/abs/2605.14036)
*Leslie G. Valiant*

Main category: cs.AI

TL;DR: 提出一种基于Unary Relational Integracode的预处理方法，使大语言模型能够进行原则性推理，同时保持计算效率


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型能生成流畅文本但缺乏原则性基础来保证内容可信度，传统认为增加原则性推理计算成本过高不可行

Method: 两阶段方法：第一阶段预处理将数据重新编码为Unary Relational Integracode，显式表示文本中对象间关系；第二阶段使用标准机器学习过程学习预测这些关系

Result: 该方法使学习训练数据中核心关系规则子集成为多项式时间可学习问题，支持单次分类器调用内及多次调用间的可靠推理

Conclusion: 提出的Unary Relational Integracode方法为大语言模型提供了高效原则性推理基础，可在保留现有软硬件基础设施的同时提升模型可信度
...
### [190] [Bridging Legal Interpretation and Formal Logic: Faithfulness, Assumption, and the Future of AI Legal Reasoning](https://arxiv.org/abs/2605.14049)
*Olivia Peiyu Wang,Leilani H. Gilpin*

Main category: cs.AI

TL;DR: 提出一种结合大型语言模型表达能力和形式化验证严格性的神经符号方法，旨在使AI辅助法律推理既强大又可信


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在法律实践中的应用存在重大风险，法律工作的高风险性质要求比当前AI系统更高的严谨性。核心问题不仅是LLMs会幻觉事实和引用，而是它们系统性地做出超出源文本支持的推理，将充满假设的结论呈现为逻辑上可靠的

Method: 神经符号方法：结合大型语言模型的表达能力和形式化验证的严格性

Result: AI辅助法律推理变得既强大又可信，减少手动验证负担而不牺牲法律实践要求的问责性

Conclusion: 神经符号方法为解决法律AI的可信度问题提供了有前景的路径，平衡了LLMs的表达能力和形式化验证的严谨性，使AI辅助法律推理在法律实践中更可靠
...
### [191] [SPIN: Structural LLM Planning via Iterative Navigation for Industrial Tasks](https://arxiv.org/abs/2605.14051)
*Yusuke Ozaki,Dhaval Patel*

Main category: cs.AI

TL;DR: SPIN是一个LLM智能体规划包装器，通过验证有向无环图(DAG)规划和前缀执行控制，解决现有规划器产生的结构无效或过长工作流问题，显著减少任务执行数量和工具调用次数。


<details>
  <summary>Details</summary>
Motivation: 工业LLM智能体系统通常将规划与执行分离，但LLM规划器经常产生结构无效或不必要过长的工作流程，导致脆性故障和可避免的工具及API成本。需要一种方法来确保规划的结构有效性和执行效率。

Method: 提出了SPIN规划包装器，结合了验证的有向无环图(DAG)规划和基于前缀的执行控制。SPIN通过_validate_plan_text和修复提示强制执行严格的DAG契约，在下游执行前产生可执行计划，然后增量评估DAG前缀，在当前前缀足以回答查询时停止执行。

Result: 在AssetOpsBench的261个场景中，SPIN将执行任务从1061减少到623，将Accomplished指标从0.638提升到0.706，同时将每轮工具调用从11.81减少到6.82。在MCP Bench上，同样的包装器提升了GPT OSS1和Llama 4 Maverick在规划、基础化和依赖相关分数方面的表现。

Conclusion: SPIN通过强制执行DAG验证和前缀执行控制，有效解决了LLM规划器产生的结构问题和效率低下问题，显著提升了智能体系统的可靠性和成本效益，展示了在工业应用中的实际价值。
...
### [192] [MathAtlas: A Benchmark for Autoformalization in the Wild](https://arxiv.org/abs/2605.14061)
...
TL;DR: MathAtlas是第一个面向研究生级数学的大规模自动形式化基准，包含从103本研究生数学教材中提取的~52k定理、定义、练习题、示例和证明，并配有包含~178k关系的数学依赖图。


<details>
  <summary>Details</summary>
Motivation: 现有的自动形式化基准主要集中于奥数或本科数学，而研究生和研究级数学仍未被充分探索。需要创建能够评估依赖感知自动形式化系统的大规模基准。

Method: 从103本研究生数学教材中提取~52k数学实体（定理、定义、练习题、示例、证明），构建包含~178k关系的数学依赖图，创建MA-Hard子集（700个具有最深依赖树的实体）。

Result: MathAtlas质量高但极具挑战性：最强基线在定理语句上最多达到9.8%正确率，定义上达到16.7%。在依赖深度最大的MA-Hard子集上，最佳模型仅实现2.6%的正确率，性能随依赖深度显著下降。

Conclusion: MathAtlas作为首个包含数学依赖图的研究生级数学自动形式化基准，揭示了当前自动形式化系统在处理复杂数学依赖关系时的局限性，并为依赖感知自动形式化系统的开发提供了重要的评估资源。
...
### [193] [Know When To Fold 'Em: Token-Efficient LLM Synthetic Data Generation via Multi-Stage In-Flight Rejection](https://arxiv.org/abs/2605.14062)
*Anjir Ahmed Chowdhury,Syed Zawad,Feng Yan*

Main category: cs.AI

TL;DR: MSIFR（多阶段飞行中拒绝）是一个无需训练的轻量级框架，通过检测和终止生成过程中的低质量轨迹来减少大型语言模型合成数据生成中的令牌浪费。


<details>
  <summary>Details</summary>
Motivation: 当前LLM合成数据生成方法通常在生成完整输出后才应用质量过滤器，导致大量低质量样本被丢弃时已经消耗了可观的令牌数，造成资源浪费。现有方法缺乏有效的中间阶段质量检测机制。

Method: MSIFR将生成过程分解为连续阶段，应用快速基于规则的验证器来检测算术不一致性、幻觉模式和格式违规。它将飞行中拒绝形式化为顺序决策过程，允许在中间检查点提前终止低质量生成轨迹。

Result: 在五个指令调优模型和七个推理基准测试中，MSIFR作为独立方法可减少11%-77%的令牌消耗，与提前退出方法结合时可减少高达78.2%的消耗，同时保持或提升评估准确性。理论分析表明任何非平凡的丢弃策略都能减少预期令牌消耗。

Conclusion: MSIFR提供了一种无需额外训练或架构改变的实用机制，能够显著提高LLM基于合成数据生成的效率，通过早期检测和拒绝低质量生成轨迹来减少令牌浪费。
...
TL;DR: 该论文批判性地分析了2025年AI模型发布中基准测试的使用情况，发现模型构建者选择的基准测试更多是作为营销叙事工具而非科学评估工具，导致评估碎片化且缺乏可比性。


<details>
  <summary>Details</summary>
Motivation: AI模型能力评估已从同行评审文献转向公司新闻稿和博客文章，模型构建者通过选择特定基准测试结果来定义"最先进"技术。然而，模型构建者选择哪些基准测试以及这种选择传递的信息未被充分研究，需要系统分析这些基准测试的使用模式和文化意义。

Method: 创建并开源Benchmarking-Cultures-25数据集，包含2025年11家主要AI构建者的139个模型发布中突出的231个基准测试。开发交互式工具探索数据。通过定量分析基准使用碎片化程度，定性分析构建者如何给基准分配不同能力描述。开发统一分类法将分歧术语映射到基于基准作者声称测量内容的共享信号框架。

Result: 1. 评估生态碎片化严重：63.2%的突出基准仅被单个构建者使用，38.5%仅出现在单个版本中，广泛使用的基准极少（如GPQA Diamond、LiveCodeBench、AIME 2025）。2. 不同构建者给相同基准赋予不同能力描述以符合各自叙事。3. "通用知识应用"是第二大类别但定义模糊，许多此类基准忽视构念效度，将结果框架化为AGI进展指标而非有效测量。4. 基准作者声称测量知识或推理，但主要评估STEM科目（特别是数学）。

Conclusion: 突出显示的基准测试更类似于灵活的市场定位叙事工具而非标准化测量工具，优先考虑市场定位而非科学评估。这导致了跨模型比较困难和评估生态系统的碎片化，削弱了基准测试作为客观评估工具的功能。
...
TL;DR: ASH是一个从无标注互联网视频中学习具身策略的智能体系统，通过自我改进循环和逆动力学模型解决长时程规划问题，在Pokemon Emerald和Zelda游戏中表现优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 当前解决长时程具身任务的方法依赖手工设计的奖励函数或带标注的动作示范，这两种方法都难以扩展。需要一种能够从无监督、嘈杂的互联网视频中学习具身策略的方法。

Method: ASH采用自我改进循环：当智能体卡住时，从自身轨迹学习逆动力学模型（IDM），然后使用该IDM从相关互联网视频中提取监督信号。通过无监督学习从大规模互联网视频中识别关键时刻，并将其存储为长期记忆。

Result: 在Pokemon Emerald中平均达到11.2/12里程碑，在The Legend of Zelda: The Minish Cap中达到9.9/12里程碑，而最强的基线方法在两个环境中分别只能达到6.5/12和6.0/12里程碑，且在8小时评估后停止进展。

Conclusion: 自我改进的智能体系统为长时程具身学习提供了一个可扩展的解决方案，能够从无标注的互联网视频中有效学习复杂的长时程策略。
...
### [199] [Good to Go: The LOOP Skill Engine That Hits 99% Success and Slashes Token Usage by 99% via One-Shot Recording and Deterministic Replay](https://arxiv.org/abs/2605.14237)
...
### [200] [Hypergraph Enterprise Agentic Reasoner over Heterogeneous Business Systems](https://arxiv.org/abs/2605.14259)
...
### [201] [Heuristic Pathologies and Further Variance Reduction via Uncertainty Propagation in the AIVAT Family of Techniques](https://arxiv.org/abs/2605.14261)
...
### [202] [Agentic AI Ecosystems in Higher Education: A Perspective on AI Agents to Emerging Inclusive, Agentic Multi-Agent AI Framework for Learning, Teaching and Institutional Intelligence](https://arxiv.org/abs/2605.14266)
...
### [203] [Parallelizing Counterfactual Regret Minimization](https://arxiv.org/abs/2605.14277)
*Juho Kim,Tuomas Sandholm*
...
### [204] [Precise Verification of Transformers through ReLU-Catalyzed Abstraction Refinement](https://arxiv.org/abs/2605.14294)
...
### [205] [Semantic Feature Segmentation for Interpretable Predictive Maintenance in Complex Systems](https://arxiv.org/abs/2605.14318)
...
### [206] [Are Agents Ready to Teach? A Multi-Stage Benchmark for Real-World Teaching Workflows](https://arxiv.org/abs/2605.14322)
...
### [207] [CrystalReasoner: Reasoning and RL for Property-Conditioned Crystal Structure Generation](https://arxiv.org/abs/2605.14344)
...
### [208] [Herculean: An Agentic Benchmark for Financial Intelligence](https://arxiv.org/abs/2605.14355)
...
### [223] [Deepchecks: Evaluating Retrieval-Augmented Generation (RAG)](https://arxiv.org/abs/2605.14488)
...
### [224] [Learning Scenario Reduction for Two-Stage Robust Optimization with Discrete Uncertainty](https://arxiv.org/abs/2605.14494)
...
### [225] [When Robots Do the Chores: A Benchmark and Agent for Long-Horizon Household Task Execution](https://arxiv.org/abs/2605.14504)
...
### [236] [Teaching Large Language Models When Not to Know: Learning Temporal Critique for Ex-Ante Reasoning](https://arxiv.org/abs/2605.14636)
...
### [237] [MindGap: A Conversational AI Framework for Upstream Neuroplastic Intervention in Post-Traumatic Stress Disorder](https://arxiv.org/abs/2605.14660)
...
### [239] [Explainable Detection of Depression Status Shifts from User Digital Traces](https://arxiv.org/abs/2605.14995)
...
### [240] [Small, Private Language Models as Teammates for Educational Assessment Design](https://arxiv.org/abs/2605.15015)
...
### [243] [$π$-Bench: Evaluating Proactive Personal Assistant Agents in Long-Horizon Workflows](https://arxiv.org/abs/2605.14678)
...
### [254] [A Deterministic Agentic Workflow for HS Tariff Classification: Multi-Dimensional Rule Reasoning with Interpretable Decisions](https://arxiv.org/abs/2605.14857)
...
### [261] [Learning Developmental Scaffoldings to Guide Self-Organisation](https://arxiv.org/abs/2605.14998)
...
### [262] [Dual-Dimensional Consistency: Balancing Budget and Quality in Adaptive Inference-Time Scaling](https://arxiv.org/abs/2605.15100)
...
### [263] [Why Neighborhoods Matter: Traversal Context and Provenance in Agentic GraphRAG](https://arxiv.org/abs/2605.15109)
...
### [264] [APWA: A Distributed Architecture for Parallelizable Agentic Workflows](https://arxiv.org/abs/2605.15132)
...
### [265] [OpenDeepThink: Parallel Reasoning via Bradley--Terry Aggregation](https://arxiv.org/abs/2605.15177)
