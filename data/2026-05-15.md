...
TL;DR: 本文提出了对比多模态超图推理方法，用于解决多人3D重建中的严重遮挡和深度模糊问题，通过融合语义、几何和姿态线索，在群体上下文约束下实现更准确的重建。


<details>
  <summary>Details</summary>
Motivation: 多人3D重建在现实交互分析中很重要，但面临严重遮挡和深度模糊的挑战。当前方法通常依赖单模态输入，缺乏几何指导，并且孤立地重建个体而忽略了群体上下文信息，这导致在拥挤场景中难以解决模糊性问题。

Method: 1. 初始化鲁棒的节点表示，结合RGB特征、几何先验和遮挡感知的不完整姿态
2. 引入骨盆深度指示器作为全局空间锚点，将视觉特征与尺度无关的深度排序对齐
3. 构建共享拓扑超图，超越成对约束，建模高阶群体动态
4. 设计基于超图的对比学习方案，联合增强模态内可区分性和强制模态间正交性

Result: 在Panoptic和GigaCrowd基准测试上的大量实验证实，该方法达到了新的最先进性能。

Conclusion: 通过对比多模态超图推理方法，成功解决了多人3D重建中的遮挡和模糊问题，实现了更准确的群体重建，代码和预训练模型已开源。
...
TL;DR: 论文研究发现DiT模型中的"巨量激活"通道——即响应远高于其他通道的少数隐藏状态通道——在图像生成中起着关键作用，这些稀疏通道实际上承载并组织了语义信息。


<details>
  <summary>Details</summary>
Motivation: 尽管DiT和相关流程模型已成为最强大的文本到图像生成器，但其内部机制中提示词如何塑造图像语义的过程仍未得到充分理解。研究者希望揭示这些模型内部的工作机制。

Method: 研究者识别并分析了DiT模型中"巨量激活"通道的特性。通过三种互补方法进行研究：(1)功能关键性测试：使用控制性干扰探测，将巨量通道置零；(2)空间组织分析：对巨量通道进行聚类，观察空间模式；(3)可转移性实验：在不同提示条件下的轨迹间传输巨量激活。

Result: 研究发现：(1)巨量通道对生成质量至关重要——将其置零会导致生成质量急剧下降，而干扰同等数量的低统计量通道影响不大；(2)巨量通道具有空间组织性——聚类后与主要主体和显著区域紧密对应；(3)巨量激活具有可转移性——在不同提示间传输能实现语义插值而非简单的像素混合。

Conclusion: 巨量激活不应被视为激活异常，而应看作是DiT模型中一个稀疏的、提示条件依赖的承载子空间，它组织并控制着语义信息。这一发现为理解和操控现代DiT模型提供了新视角，并应用于文本和图像条件下的语义传输等场景。
...
TL;DR: CineMesh4D是一个端到端4D(3D+t)管道，直接从多视角2D心脏电影MRI重建患者特异性全心网格，通过可微分渲染损失和监督和双上下文时序块实现高精度重建。


<details>
  <summary>Details</summary>
Motivation: 从心脏电影MRI准确重建3D+t全心网格在临床中至关重要但技术上具有挑战性，主要困难来自：2D图像切片对3D心脏解剖的稀疏采样，以及心脏形状与运动的紧密耦合。当前方法通常只能重建部分心脏腔室或单一时相。

Method: 提出CineMesh4D端到端4D管道，通过跨域映射从多视角2D心脏电影MRI直接重建患者特异性全心网格。引入可微分渲染损失，从多视角稀疏轮廓监督3D+t全心网格。开发双上下文时序块，融合全局和局部心脏时序信息以捕获高维序列模式。

Result: 在定量和定性评估中，CineMesh4D在重建质量和运动一致性方面优于现有方法，为个性化实时心脏评估提供实用途径。

Conclusion: CineMesh4D通过创新的可微分渲染损失和双上下文时序块，实现了从稀疏2D切片到高质量4D全心网格的直接重建，解决了心脏形状与运动耦合的挑战，代码将在接受后公开发布。
...
TL;DR: 提出一种统一像素标记和词标记的生成式语言模型，通过为每个像素分配独立嵌入、颜色折叠、全局条件注意力近似和图像无监督预训练，解决现有视觉编码器在细节识别上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP或SigLIP的Vision Transformer在开源多模态模型中作为视觉编码器，但在细节视觉理解（如识别小文本或数字）方面存在局限性，需要改进。

Method: 提出新模型统一像素标记和词标记到生成式语言模型中，为图像每个像素分配独立标记嵌入，采用颜色折叠技术，使用全局条件注意力近似，并进行图像无监督预训练。

Result: 实验表明，即使在小模型和有限训练数据条件下，该模型也表现出良好性能。模型符合缩放定律，随着参数和训练数据增加，性能会持续提升。

Conclusion: 提出的统一像素和词标记的生成式语言模型能够改善多模态模型中的细节视觉理解能力，特别是在识别小文本和数字方面，具有良好的可扩展性。
...
TL;DR: 提出一种基于遗传编程的高效硬件感知框架，用于演化Vision Transformers中特定层级的标量函数，无需重新训练模型即可消除层归一化的计算瓶颈，实现边缘设备上的高效部署。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers（ViTs）在边缘设备上部署面临重大挑战：层归一化（LayerNorm）的计算复杂度高且存在全局归约瓶颈。现有方法通常采用同质标量近似来替代归一化层，但这些方法无法适配所有层的行为，且需要昂贵的模型重新训练。

Method: 提出一个硬件感知框架，利用遗传编程（GP）从预训练权重直接演化出异质、层特定的标量函数。结合新颖的后训练重对齐策略，完全避免了从零开始重新训练模型的需要。

Result: 演化出的表达式能准确近似目标归一化行为，捕获91.6%的方差（R²），而基线同质方法仅为70.2%。修改后的架构仅用20个epoch就恢复了84.25%的Top-1 ImageNet-1K准确率。通过保持性能同时消除全局归约瓶颈，在算术复杂度和片外内存流量之间建立了有利的权衡。

Conclusion: 该方法解决了Vision Transformers在边缘加速器上高效部署的主要障碍，通过演化层特定标量函数消除了归一化层的计算瓶颈，为硬件友好型ViTs部署提供了有效解决方案。
...
TL;DR: CurveBench是一个用于从视觉输入进行层次拓扑推理的基准测试，包含756张不交叉的Jordan曲线图像，每张图像都标注了表示平面区域包含关系的根树。最强模型Gemini 3.1 Pro在该基准上的准确率仅为71.1%（简单版）和19.1%（困难版），表明精确的拓扑感知视觉推理问题远未解决。


<details>
  <summary>Details</summary>
Motivation: 当前计算机视觉模型在处理层次拓扑推理任务（如从图像中理解曲线间的包含关系）方面存在不足。虽然任务视觉上看似简单，但现有模型在这些结构化预测任务上表现不佳，缺乏能够评估模型层次拓扑推理能力的专门基准。

Method: 提出CurveBench基准，包含756张成对不交叉的Jordan曲线图像，涵盖简单、多边形、地形启发、迷宫式和密集计数等配置。每张图像标注一个根树来编码平面区域间的包含关系。将任务形式化为结构化预测：给定图像，模型必须恢复曲线诱导的完整根包含树。还通过RLVR风格的微调方法对开源视觉语言模型进行训练。

Result: Gemini 3.1 Pro在CurveBench-Easy上达到71.1%树生成准确率，在CurveBench-Hard上仅为19.1%。通过微调Qwen3-VL-8B模型，在CurveBench-Easy上的准确率从2.8%提升到33.3%，超过了GPT-5.4和Claude Opus 4.5。但特别是在困难版本上仍存在显著差距。

Conclusion: CurveBench揭示了当前计算机视觉模型在精确层次拓扑推理方面的严重不足。尽管视觉任务看似简单，但模型在恢复曲线包含关系的结构化树状表示上表现不佳，表明拓扑感知的视觉推理仍是一个远未解决的挑战性问题。
...
TL;DR: DeFakerOne是一个数据为中心的统一伪造图像检测与定位基础模型，整合了InternVL2和SAM2架构，在39个检测基准和9个定位基准上达到最先进性能


<details>
  <summary>Details</summary>
Motivation: 生成式AI的发展使图像伪造范式统一化，打破了文档编辑、自然图像处理、DeepFake生成和全图像AIGC合成的界限，而现有伪造检测与定位研究仍处于碎片化状态，形成统一生成机制与领域特定检测范式之间的不匹配

Method: 提出DeFakerOne模型，整合InternVL2和SAM2架构，实现图像级检测和像素级定位的统一框架，采用数据为中心的方法，分析数据缩放规律、跨域伪影传递-干扰模式等

Result: DeFakerOne在39个伪造检测基准和9个定位基准上超过基线方法，表现出对真实世界扰动和最先进生成器（如GPT-Image-2）的鲁棒性，并系统分析了数据缩放规律、伪影传递模式等

Conclusion: DeFakerOne成功解决了统一伪造生成与碎片化检测之间的不匹配问题，为可扩展、鲁棒和统一的伪造图像检测与定位提供了设计原则，展示了基础模型在该领域的潜力
...
TL;DR: DUET是一种创新的双范式框架，通过结合参数化预测和基于记忆的检索来从组织学图像推断空间分辨基因表达，利用大规模单细胞参考数据作为生物约束，在多个数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将组织学图像到基因表达的任务简化为简单的形态学-表达映射，其中视觉相似性不能保证分子一致性；大规模单细胞数据资源远超过空间转录组学数据，但在视觉-组学建模中尚未充分探索；当前方法采用单一范式，无法平衡表达灵活性与生物保真度。

Method: 提出DUET框架：1) 双范式协同参数化预测和基于记忆的检索；2) 并行回归-检索范式，自适应协调互补路径输出；3) 整合大规模单细胞参考数据以分子状态作为生物约束；4) 设计轻量级适配器根据空间上下文动态分配分支偏好实现最优性能。

Result: 在三个不同基因规模的公共数据集上进行广泛实验，DUET实现了最先进的性能，每个提出的组件都贡献了一致的性能提升。

Conclusion: DUET通过双范式框架成功解决了现有方法的局限性，将大规模单细胞参考数据与视觉-组学建模相结合，实现了从组织学图像推断基因表达的高性能方法。
...
TL;DR: 提出了一种糖尿病视网膜病变筛查的两级边缘-云级联系统：第一级在边缘设备上运行轻量级MobileNetV3-small模型进行可转诊DR的二元分类，第二级仅在需要转诊的图像上运行云端RETFound-DINOv2模型进行严重程度分级，显著减少了云服务调用。


<details>
  <summary>Details</summary>
Motivation: 农村地区缺乏糖尿病视网膜病变早期检测的专家和基础设施，而现有的云端深度学习系统在这些地区面临高延迟、带宽有限和数据传输成本高等挑战，需要一种更适应资源受限环境的解决方案。

Method: 采用两级边缘-云级联架构：Tier 1在本地诊所设备上部署轻量级MobileNetV3-small模型，对APTOS 2019数据集中的图像进行可转诊DR（类别2-4）与非可转诊DR（类别0-1）的二元分类；Tier 2在云端运行RETFoundDINOv2模型，仅对Tier 1标记为可转诊的图像进行有序严重程度分级。

Result: Tier 1达到98.99%灵敏度和84.37%特异性；级联系统将50.48%的图像过滤掉，云调用减少50.48%；在4类输出空间中，级联系统获得80.49%准确率和0.8167二次加权Kappa，与云端基线（80.76%准确率，0.8184 Kappa）性能接近，但云资源使用减少约一半。

Conclusion: 提出的边缘-云级联系统在保持糖尿病视网膜病变筛查性能的同时，显著减少了云资源使用，为解决农村地区资源受限环境下的医疗AI部署提供了实用解决方案，平衡了准确性与资源效率。
...
TL;DR: PanoPlane是一种用于高保真稀疏视角室内新视角合成的方法，通过全景场景补全重建封闭房间几何结构。该方法利用360度全景补全，并提出Layout Anchored Attention Steering机制，无需训练即可引导扩散模型注意力，从而实现准确的新视角合成。


<details>
  <summary>Details</summary>
Motivation: 现有基于透视的方法从有限视场生成训练视图，难以在稀疏视角条件下实现高质量的室内新视角合成。需要一种能够利用完整空间布局信息的方法来改善未观察区域的几何重建质量。

Method: 1. 使用360度全景补全作为生成过程的条件，利用完整空间布局信息。
2. 提出Layout Anchored Attention Steering（布局锚定注意力引导）机制，这是一种无需训练的推理时技术，引导扩散模型内部表示的注意力朝向场景中检测到的平面表面。
3. 通过将每个未观察区域的注意力引导到几何一致的观察内容，用基于表面的外推替代无约束的幻觉生成。
4. 利用生成的全景补全结果为3D高斯泼溅（Gaussian Splatting）提供监督，实现从最少三个输入视角的准确新视角合成。

Result: 在Replica、ScanNet++和Matterport3D数据集上的实验表明：
1. 在3、6和9个输入视角下实现了最先进的新视角合成质量
2. 与当前最先进的基线相比，PSNR提高了+17.8%
3. 无需对扩散模型进行任何训练或微调

Conclusion: PanoPlane通过全景场景补全和布局锚定注意力引导机制，有效地解决了稀疏视角室内新视角合成的挑战。该方法在不需训练扩散模型的情况下，显著提高了重建质量，证明了利用全景信息引导几何一致性重建的有效性。
...
TL;DR: TeDiO：一种无需训练、推理时的方法，通过正则化自注意力图中的时间对角线模式来提高文本到视频扩散模型的时序一致性，无需修改模型权重或使用外部运动监督。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到视频扩散变压器虽然能生成视觉上吸引人的帧，但在时序一致性方面仍然存在问题，经常产生闪烁、漂移或不稳定的运动。这些故障在模型内部留下了清晰的印记：不连贯的视频在中间自注意力图中表现出不规则、碎片化的时间对角线模式，而稳定运动对应着平滑的带状对角线模式。

Method: TeDiO是一种无需训练、推理时的方法，通过观察和正则化模型内部的自注意力模式来提高时序一致性。具体方法包括：1）估计对角线平滑度，识别不稳定区域；2）执行轻量级的潜在更新，促进连贯的帧间动态；3）不修改模型权重，也不使用外部运动监督。

Result: 在多个视频扩散模型（如Wan2.1、CogVideoX）上，TeDiO显著提高了运动平滑度，同时保持了每帧的视觉质量。提供了一种高效的即插即用方法来改善现代视频生成系统的动态真实感。

Conclusion: 通过正则化内部注意力模式来实现时序一致性是一种有效且高效的方法。TeDiO作为一种无需训练、推理时的解决方案，为视频生成系统提供了改进动态真实感的实用工具，同时保持了生成质量。
...
TL;DR: 该论文提出了一种简单的非参数化few-shot学习方法，利用冻结的DINOv2-L特征和k最近邻分类器，无需反向传播即可在多个基准测试中达到最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 基于"柏拉图表示假说"的观察，即不同架构在大规模数据集上训练后趋向于共享的"理想"潜在空间，论文探讨了现成的嵌入是否足以完成复杂任务，从而避免密集的任务特定微调。

Method: 提出了一个完全绕过反向传播的非参数化few-shot学习流程：使用冻结的DINOv2-L特征，通过k最近邻分类器进行层特征提取分析，并利用PCA和ICA进行流形精炼以提供正则化效果。

Result: 在四个主要基准测试上的结果显示，该方法一致超越复杂的元学习算法，实现了最先进的性能表现。

Conclusion: 研究证实了现成的嵌入在复杂任务中确实是"足够好的"，为视觉识别领域提供了一种高效、无需微调的解决方案，支持了"柏拉图表示假说"的合理性。
...
TL;DR: 本研究开发了一种实时导管尖端跟踪系统，用于机械取栓手术中的自主导航。通过深度学习分割模型和多线程处理流程，在低对比度、噪声和设备遮挡的荧光透视图像中实现了精确的导管尖端定位，性能优于现有基准。


<details>
  <summary>Details</summary>
Motivation: 机械取栓虽能改善中风治疗效果，但受限于当地治疗资源不足。基于强化学习的机器人系统可通过自主导航缓解这一问题，但现有方法需要实时导管尖端坐标跟踪。传统荧光透视成像存在低对比度、噪声和设备遮挡等挑战，需要开发可靠的实时跟踪系统。

Method: 设计了多线程处理流程，包括帧读取、预处理、推理和后处理。训练和比较了U-Net、U-Net+Transformer和SegFormer三种深度学习分割模型，采用两分类和三分类两种标注方案。后处理包括两步分量过滤、单像素中轴骨架化、贪心弧长路径跟踪和轮廓回退策略。

Result: 在中等复杂度荧光透视视频数据上，两分类SegFormer模型达到4.44毫米平均绝对误差，优于U-Net(4.60毫米)、U-Net+Transformer(6.20毫米)和所有三分类模型(5.19-7.74毫米)。在分割基准测试中，系统超越CathAction最新成果，三分割Dice分数提升最高达5%。

Conclusion: 所提出的多线程跟踪框架在挑战性成像条件下保持稳定性能，超越先前基准，为基于强化学习的自主机械取栓导航提供了可靠高效的基础。系统能够处理低对比度、噪声和设备遮挡等实际手术中的常见问题。
...
TL;DR: 该论文提出了一种基于动态分辨率扩散模型的图像恢复方法，通过将数据投影到低维子空间来加速推理过程，同时保持恢复质量


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像恢复任务中表现出色，但现有方法通常在高维像素空间操作，计算开销大。基于潜在扩散模型的方法虽然利用压缩潜在空间，但需要重复的编码器-解码器推理，导致额外计算负担，甚至比像素空间方法更慢

Method: 提出了将数据投影到低维子空间的方法：1）微调预训练扩散模型以获得动态分辨率先验；2）将DPS和DAPS两种像素空间方法适配到该框架中，得到SubDPS和SubDAPS；3）基于SubDAPS引入增强变体SubDAPS++以进一步提升重建效率和质量

Result: 在多种图像数据集和各种恢复任务上的实证评估表明，所提方法在大多数实验场景中优于近期的扩散模型方法，同时具有更好的推理速度

Conclusion: 通过将数据投影到低维子空间并利用动态分辨率扩散模型，可以在保持图像恢复质量的同时显著加速推理过程，为解决扩散模型在图像恢复中的计算效率问题提供了有效方案
...
TL;DR: 提出了一种名为Omission Signal Intervention（OSI）的方法，通过放大文本嵌入中的"缺失信号"来缓解多模态扩散变换器在文本到图像生成中的概念遗漏问题。


<details>
  <summary>Details</summary>
Motivation: 多模态扩散变换器（MM-DiTs）在文本到图像生成方面取得了显著进展，但经常遭受概念遗漏问题，即指定的对象或属性无法在生成的图像中显现。

Method: 通过线性探测文本标记，发现文本嵌入可以区分代表目标概念缺失的特征性"遗漏信号"。基于这一发现，提出了OSI方法，通过放大遗漏信号来主动催化缺失概念的生成。

Result: 在FLUX.1-Dev和SD3.5-Medium数据集上的综合实验表明，OSI即使在极端场景下也能显著缓解概念遗漏问题。

Conclusion: OSI是一种有效的方法，能够通过干预文本嵌入中的遗漏信号来改善多模态扩散变换器的概念遗漏问题，提高文本到图像生成的准确性和完整性。
...
TL;DR: CreFlow：基于组合约束的奖励模型，用于后训练具身视频生成模型，通过逻辑约束改进操纵任务视频生成质量


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在异质数据上训练，可能产生违反物理约束的视觉合理视频。视频RL奖励通常简化为低级视觉指标，而操纵任务需要逻辑验证来检查是否满足组合任务规范

Method: 1) 引入组合约束奖励模型，将任务需求表述为线性时序逻辑约束组合，提供忠实奖励和定位错误信息；2) 提出CreFlow在线RL框架，包含信用感知NFT损失（限制RL更新到奖励相关区域）和校正重流损失（利用组内正样本作为校正方向估计）

Result: CreFlow的奖励判断比现有方法更符合人类和模拟器成功标签，在八个双手操纵任务中提升下游执行成功率23.8个百分点

Conclusion: 组合约束奖励模型和CreFlow框架有效提高了具身视频生成模型的任务完成质量，通过逻辑约束和针对性训练策略实现更可靠的操纵视频生成
...
TL;DR: KVPO：一种用于对齐流式自回归视频生成器的ODE原生在线组相对策略优化框架，通过因果语义探索和基于轨迹速度能量的替代策略，提升了视频生成的质量和一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习方法主要依赖基于噪声的探索和SDE（随机微分方程）替代策略，这与蒸馏自回归模型的确定性ODE（常微分方程）动力学不匹配，并且倾向于扰动低级外观特征而非对长时程一致性至关重要的高级语义故事情节进展。

Method: KVPO采用ODE原生的在线组相对策略优化框架：1) 引入因果语义探索范式，将变异源从随机噪声迁移到历史KV缓存，通过随机路由历史KV条目构建语义多样的生成分支；2) 提出基于轨迹速度能量的速度场替代策略，量化流匹配速度空间中的分支似然，获得与原生ODE公式完全一致的奖励加权对比目标。

Result: 在多个蒸馏自回归视频生成器上的实验表明，该方法在视觉质量、运动质量和文本-视频对齐方面取得了持续改进，适用于单提示短视频和多提示长视频设置。

Conclusion: KVPO解决了现有方法在确定性ODE动力学下的局限性，通过语义探索和ODE一致的策略建模，显著提升了流式视频生成器与人类偏好的对齐能力。
...
### [25] [Learning with Semantic Priors: Stabilizing Point-Supervised Infrared Small Target Detection via Hierarchical Knowledge Distillation](https://arxiv.org/abs/2605.14346)
...
### [36] [Reduce the Artifacts Bias for More Generalizable AI-Generated Image Detection](https://arxiv.org/abs/2605.14486)
...
### [39] [ArcGate: Adaptive Arctangent Gated Activation](https://arxiv.org/abs/2605.14518)
...
### [55] [Deep Image Segmentation via Discriminant Feature Learning](https://arxiv.org/abs/2605.14609)
...
### [59] [MultiEmo-Bench: Multi-label Visual Emotion Analysis for Multi-modal Large Language Models](https://arxiv.org/abs/2605.14635)
...
### [67] [Generating HDR Video from SDR Video](https://arxiv.org/abs/2605.14703)
...
### [68] [SceneFunRI: Reasoning the Invisible for Task-Driven Functional Object Localization](https://arxiv.org/abs/2605.14704)
...
### [69] [Towards Continuous Sign Language Conversation from Isolated Signs](https://arxiv.org/abs/2605.14705)
...
### [70] [StyleTextGen: Style-Conditioned Multilingual Scene Text Generation](https://arxiv.org/abs/2605.14708)
...
### [71] [Breaking Dual Bottlenecks: Evolving Unified Multimodal Models into Self-Adaptive Interleaved Visual Reasoners](https://arxiv.org/abs/2605.14709)
...
### [72] [Vision-Core Guided Contrastive Learning for Balanced Multi-modal Prognosis Prediction of Stroke](https://arxiv.org/abs/2605.14710)
...
### [74] [Towards Label-Free Single-Cell Phenotyping Using Multi-Task Learning](https://arxiv.org/abs/2605.14717)
...
### [131] [Aligning Latent Geometry for Spherical Flow Matching in Image Generation](https://arxiv.org/abs/2605.15193)
...
### [132] [VGGT-$Ω$](https://arxiv.org/abs/2605.15195)
...
### [133] [RefDecoder: Enhancing Visual Generation with Conditional Video Decoding](https://arxiv.org/abs/2605.15196)
...
### [134] [ATLAS: Agentic or Latent Visual Reasoning? One Word is Enough for Both](https://arxiv.org/abs/2605.15198)
...
### [135] [EntityBench: Towards Entity-Consistent Long-Range Multi-Shot Video Generation](https://arxiv.org/abs/2605.15199)
...
### [137] [Merging Methods for Multilingual Knowledge Editing for Large Language Models: An Empirical Odyssey](https://arxiv.org/abs/2605.13919)
*Kunil Lee,Ki-Young Shin,Jong-Hyeok Lee,Young-Joo Suh*

Main category: cs.CL

TL;DR: 本研究评估了向量融合方法在多语言知识编辑（MKE）中的有效性，发现带共享协方差的向量求和是最可靠的策略，而任务特定向量融合（TSVM）缓解多语言干扰的能力有限，且性能对权重缩放因子和秩压缩比敏感。


<details>
  <summary>Details</summary>
Motivation: 多语言知识编辑面临的主要挑战是语言特定的编辑会相互干扰，即使在单语言设置中定位-编辑方法表现良好。本文旨在解决三个关键问题：向量融合方法对MKE的有效性、TSVM减少多语言干扰的程度、以及权重缩放因子和秩压缩比对性能的影响。

Method: 在MzsRE基准上采用大规模批量编辑设置，评估了六种融合变体，使用两种流行的骨干大语言模型、两种基础知识编辑方法，并在12种语言上进行测试。重点关注向量融合方法，特别是带/不带共享协方差的求和策略，以及TSVM方法。

Result: 结果显示：1）带共享协方差的向量求和是最可靠的整体策略，而不带共享协方差的简单求和表现较差；2）TSVM在某些设置中能提升性能，但其缓解多语言干扰的能力有限；3）性能对权重缩放和秩比敏感，较大缩放因子和相对较低的秩通常能获得更好结果。

Conclusion: 研究阐明了当前向量融合方法在多语言知识编辑中的实际优势和局限性，为未来的多语言知识编辑研究提供了指导。带共享协方差的向量求和是最稳健的方法，而TSVM在多语言干扰缓解方面效果有限，需要进一步优化。
...
TL;DR: Mistletoe：一种针对推测解码的新型隐蔽攻击，通过降低草稿模型与目标模型的一致性来破坏加速效果，同时保持输出质量


<details>
  <summary>Details</summary>
Motivation: 识别推测解码中存在的机制级漏洞：草稿模型对目标模型分布的近似不可避免存在不完美性，这种不匹配为攻击提供了隐蔽的攻击面，可以在不影响目标模型可见行为的情况下大幅降低草稿令牌接受率。

Method: 提出Mistletoe攻击方法，联合优化两个目标：降低草稿-目标一致性的退化目标和约束目标模型输出分布的语义保持目标。为解决这两个目标间的冲突，引入了零空间投影机制，将退化梯度投影到局部语义保持方向之外，抑制草稿接受同时最小化语义漂移。

Result: 实验表明，Mistletoe显著降低了平均接受长度τ，使加速效果崩溃并降低平均令牌吞吐量，同时保持了输出质量和困惑度不变。该方法在各种推测解码系统上均有效。

Conclusion: 推测解码在现有输出鲁棒性之外引入了机制级的攻击面，需要更鲁棒的LLM加速系统设计。Mistletoe攻击揭示了这种新型漏洞的存在。
...
TL;DR: 本文通过审计多模态物理评估流程，发现了三种扭曲视觉语言推理测量的构建实践问题，并发布了相应的数据集和模型以解决这些问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态物理评估管道存在三个未被发现的构建实践问题，这些问
题扭曲了领域对视觉语言推理能力的测量。具体包括：(1)训练-评估污染问题，即公共训练数据集与评估集之间存在数据重叠；(2)翻译漂移问题，即语言翻译导致的性能差异；(3)多项选择题饱和问题，即多项选择题格式评估无法全面反映真实能力。

Method: 采用三层审计方法：第一阶段使用5-gram Jaccard相似度进行初步筛选；第二阶段使用mxbai-embed-large余弦相似度；第三阶段使用Haiku-4.5 LLM判断。通过对比实验验证翻译漂移（59对爱沙尼亚语-英语奥林匹克问题）和格式差异影响（多项选择题vs开放性问题）。开发了Physics-R1模型，使用GSPO+DAPO配方从Qwen3-VL-8B-Thinking冷启动训练。

Result: 审计发现：在SciInstruct数据集中发现134个近重复样本和4,846个改写候选。翻译漂移导致17个百分点性能差异。多项选择题与开放式奥林匹克评估间存在46个百分点格式梯度差异。Physics-R1模型在多个基准测试中表现优异：在PhysOlym-A上提升18.3个百分点，在PhysReason上提升15.7个百分点，在OlympiadBench-Physics上提升6.9个百分点，在PhyX MCQ上提升4.1个百分点。

Conclusion: 本文揭示了当前多模态物理评估中存在的系统性偏差，并通过发布审计后的数据集（PhysCorp-A、PhysR1Corp、PhysOlym-A）和参考模型Physics-R1，为解决这些评估偏差提供了实用工具和方法论框架，有助于更准确地衡量模型的视觉语言推理能力。
...
TL;DR: 本文提出了一种名为"推导提示"的新型提示技术，用于改进检索增强生成框架中的生成步骤，通过在知识密集型任务中提供更可解释和控制更强的生成过程来减少幻觉和错误推理。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在问答应用中展现了巨大潜力，但在知识密集型、领域特定的任务中，使用这些模型会出现幻觉和错误推理等重要挑战。这些问题在检索增强生成框架中尤为突出，传统方法在控制生成过程和确保答案质量方面存在不足。

Method: 提出了"推导提示"技术，这是一种受逻辑推导启发的提示方法。该方法涉及从初始假设出发，通过系统应用预定义规则来推导结论。它构建了一个可解释的推导树，增加了对生成过程的控制，特别针对检索增强生成框架的生成步骤进行优化。

Result: 通过具体案例研究应用该方法，结果表明相比于传统的检索增强生成方法和长上下文窗口方法，该方法显著减少了不可接受的答案数量，提升了问答系统的可靠性和准确性。

Conclusion: 推导提示技术通过引入逻辑推导结构，为检索增强生成框架提供了一种更加可解释和受控的生成方法，有效解决了大型语言模型在知识密集型任务中的幻觉和错误推理问题，为提高问答系统的可靠性提供了新的技术途径。
...
TL;DR: 提出PEML方法，通过神经架构工程优化连续提示，同时进行低秩适配模型权重，实现多任务参数高效微调，相比现有方法在多个基准测试中平均准确率提升达6.67%


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法如LoRA和Prefix Tuning在设计时主要针对单一任务优化：LoRA关注模型适配但忽视提示优化在多任务学习中的重要性，Prefix Tuning使用简单架构优化提示而限制多任务适配能力。多任务学习需要共同优化提示优化和模型适配，以高效微调LLM适应多个任务。

Method: 提出参数高效多任务学习方法PEML，采用神经架构工程方法优化连续提示，同时执行模型权重的低秩适配。通过创建自动化框架来优化连续提示并适配模型权重。

Result: 在GLUE、SuperGLUE、大规模多任务语言理解和常识推理基准上评估PEML，与MTL-LoRA、MultiLoRa、C-Poly和MoE等最先进方法比较。评估结果显示平均准确率提升高达6.67%，单个任务峰值增益达10.75%。

Conclusion: PEML方法通过共同优化提示和模型权重，实现了有效的多任务参数高效微调，显著优于现有方法，为部署单一LLM处理多个任务提供了高效解决方案。
...
TL;DR: 研究者使用HealthContradict数据集评估了6个开源大语言模型在5种不同证据条件下的表现，发现冲突证据会导致准确率下降和预测翻转。为了解决这个问题，他们提出了一种结合模型置信度和证据冲突检测的冲突感知弃权评分方法，该方法在困难条件下显著提高了选择性准确率。


<details>
  <summary>Details</summary>
Motivation: 生物医学检索增强大语言模型常面临不完整、误导性或内部矛盾的证据，但现有评估通常侧重于帮助性上下文下的答案准确性，而非冲突证据下的可靠性。研究者旨在量化证据冲突对模型性能的影响，并开发能够处理证据冲突的方法。

Method: 研究者使用HealthContradict数据集评估了6个开源LLM在5种控制证据条件下的表现：1)无检索上下文，2)仅正确上下文，3)仅错误上下文，4)两种包含正确和矛盾文档的混合条件（顺序相反）。他们还开发了一个冲突感知弃权评分方法，结合模型置信度和证据冲突检测器。

Result: 在冲突证据顺序对比中，相同两个文档仅顺序反转，所有模型的准确率均下降，11.4%-25.2%的预测发生翻转。冲突感知弃权评分在两种最困难条件下（仅错误和错误优先冲突）比仅使用置信度的方法提高了选择性准确率，平均增益分别为7.2-33.4点和3.6-14.4点。

Conclusion: 冲突的生物医学证据既是模型不确定性问题，也是鲁棒性问题，这要求评估方法和弃权策略必须明确考虑证据分歧。冲突感知弃权评分能够有效帮助模型在困难情况下做出更可靠的决策。
...
TL;DR: 这篇论文提出了一种基于大语言模型的文本生成平面图方法，通过强化学习与可验证奖励机制来同时满足拓扑连接和数值约束，显著超越了现有方法在现实主义、兼容性和多样性方面的表现。


<details>
  <summary>Details</summary>
Motivation: 专业平面图设计AI系统需要精确控制房间尺寸和面积，同时保持房间之间的期望连接关系以及功能和美学质量。现有生成方法主要关注房间连接关系，但无法生成符合数值约束的平面图。

Method: 1. 在大规模真实平面图上微调大语言模型；2. 应用带有可验证奖励的强化学习（RLVR）来提高拓扑和数值约束的遵循性，同时减少无效或重叠输出；3. 设计了一套约束遵循度度量标准来系统评估生成的平面图与用户定义约束的匹配程度。

Result: 模型生成的平面图能够满足用户定义的连接和数值约束，在现实主义、兼容性和多样性指标上优于现有方法。在所有任务中，兼容性相对于现有方法至少减少了94%。

Conclusion: 结果表明大语言模型能够有效处理此类约束，为基于文本的生成建模提供了更广泛的应用前景，证明了结合微调LLM和强化学习在满足复杂多约束平面图生成任务上的有效性。
...
TL;DR: 论文提出ROK-FORTRESS双语文化对抗NSPS基准，通过"创译矩阵"方法分离语言和地缘政治背景对LLM安全评估的影响，发现韩语变体存在一致的安全抑制效应，且地缘政治背景能够缓解语言驱动的安全抑制


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的安全评估主要针对国家安全和公共安全(NSPS)风险，但多语言安全评估通常仅通过翻译基准进行，保留了基础情景但缺乏语言与地缘政治背景交互的实证研究。现有研究局限于有限的语言对，需要更全面的评估方法

Method: 引入ROK-FORTRESS双语文化对抗NSPS基准，使用英语-韩语语言对和美国-韩国地缘政治轴作为案例研究。通过"创译矩阵"分离语言和地缘政治背景影响：在(i)英语vs韩语语言和(ii)美国vs韩国实体、机构和操作细节的受控组合下评估对抗性意图。每个对抗性提示都配有双重用途的良性对应物来量化过度拒绝。使用校准的LLM-as-a-judge专家小组，应用专家制定的提示特定二元评分标准对模型响应进行评分

Result: 前沿模型和韩语优化模型的双轨测试显示：1)韩语变体存在一致的安全抑制效应；2)模型间在地缘政治背景与语言交互方式上存在显著差异；3)在许多模型中，韩国背景缓解了韩语驱动的安全抑制效应；4)没有模型显示出相反方向的显著放大效应。这表明安全行为受到语言作为风险信号和背景交互的影响，而纯翻译评估会错过这些因素

Conclusion: 在英语-韩语案例中，大语言模型的安全行为由语言作为风险信号和上下文交互塑造，这是纯翻译评估无法捕捉的。创译矩阵方法设计为可泛化到其他语言-文化对，为多语言安全评估提供了更精细的分析框架
...
TL;DR: 本文通过构建属性图来研究RAG系统内部信息流，发现正确与错误预测在推理路径结构上存在系统性差异，并基于此开发了图基错误检测框架和针对性干预方法。


<details>
  <summary>Details</summary>
Motivation: 尽管检索增强生成(RAG)已成为提升大语言模型性能的重要方法，但RAG系统仍经常产生错误答案。目前尚不清楚为何在拥有外部信息访问权限的情况下，RAG仍会失败，这促使研究者深入探究检索证据如何影响答案生成过程。

Method: 使用电路追踪技术构建属性图，这些图通过解码过程中Transformer层的信息流，建模检索上下文、中间模型激活和生成token之间的交互关系。在多个问答基准测试中分析这些图的结构特征。

Result: 研究发现正确预测与错误预测在结构上存在一致性差异：正确预测呈现更深的推理路径、更分布式证据流和更结构化的局部连接模式；而错误预测则表现为更浅层、碎片化和过度集中的证据流。基于这些发现开发了基于属性图拓扑特征的错误检测框架，并通过强化问题约束的证据基础实施针对性干预，重塑内部信息路由。

Conclusion: 属性图提供了RAG系统内部推理过程的电路级视图，揭示了外部证据如何整合到模型推理中。基于图结构特征的错误检测和干预方法能够通过强化问题约束的证��基础，改善检索信息的整合效果，减少错误发生。
...
TL;DR: GradShield是一种在微调过程中保护大语言模型安全对齐的过滤方法，通过计算微调隐含有害性分数并采用自适应阈值算法来识别和移除有害数据点，有效将攻击成功率保持在6%以下的同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在微调后存在安全对齐风险，不仅显性有害数据会损害模型安全，连一些看似良性的数据也可能无意中导致模型行为失调，需要一种在微调过程中保护模型安全对齐的方法。

Method: 提出了GradShield方法，这是一种基于原则的过滤机制：1）为每个数据点计算微调隐含有害性分数（FIHS），评估数据对模型安全的潜在影响；2）采用自适应阈值算法动态确定过滤标准；3）在微调前识别和移除有害数据点。

Result: 在多个实用微调任务和不同有害数据比例的场景下进行评估，GradShield在安全性和实用性方面均优于所有基线方法：1）攻击成功率（ASR）始终保持在6%以下；2）同时保持了模型的实用性能；3）在各种评估指标上表现优异。

Conclusion: GradShield是一种有效的LLM微调安全保护方法，能够在微调过程中主动识别和过滤有害数据，显著降低模型安全对齐风险，同时保持模型实用性，为解决LLM微调安全问题提供了新思路。
...
TL;DR: 该研究开发了两种词汇难度预测模型：一个高精度黑盒模型（在开放赛道中取得最优成绩）和一个可解释模型（优于微调的编码器基线）。黑盒模型使用软目标损失函数微调LLM，实现了r>0.91的相关性；可解释模型在保持r>0.77相关性的同时，能够解释影响词汇难度的因素。


<details>
  <summary>Details</summary>
Motivation: 研究的动机是开发能够准确预测词汇难度的模型，同时解决两个关键问题：一是需要高精度的预测模型用于词汇难度评级任务，二是需要可解释的模型来理解影响词汇难度的具体因素，这对于教育和测试设计有重要价值。

Method: 研究方法包括：1）使用软目标损失函数微调大型语言模型作为黑盒模型；2）开发可解释的模型，该模型超越了基于微调编码器的基线方法；3）应用这两种模型对英国理事会知识型词汇列表中的项目进行分析，考察拼写难度和测试项目构建等因素对词汇难度的影响。

Result: 黑盒模型在词汇难度评级任务中表现出色，相关性r>0.91，在开放赛道中取得最优成绩。可解释模型虽然相关性稍低（r>0.77），但超越了微调编码器基线，并能提供关于影响词汇难度因素的具体见解。分析表明，词汇难度不仅受词汇本身产生难度的影响，还受到拼写难度和测试项目构造的影响。

Conclusion: 该研究成功开发了两种有效的词汇难度预测模型：高精度黑盒模型适合需要准确预测的应用场景，而可解释模型则为教育和测试设计提供了有价值的洞察。研究发现词汇难度是多因素影响的，包括拼写难度和测试项目构造等外部因素。研究代码已开源，为后续研究提供了基础。
...
### [148] [Auditing Agent Harness Safety](https://arxiv.org/abs/2605.14271)
...
TL;DR: HarnessAudit框架用于审计LLM智能体执行轨迹中的安全性，发现现有输出级评估无法检测执行过程中的边界违规，多智能体协作会扩大安全风险面


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体执行框架存在安全隐患：执行轨迹可能访问未授权资源或泄露上下文，但大多数安全基准只评估最终输出或终止状态，无法检测执行过程中的违规。需要评估执行框架是否在整个执行过程中尊重用户意图、权限边界和信息流约束。

Method: 提出HarnessAudit框架，审计完整执行轨迹的三个维度：边界合规性、执行保真度和系统稳定性。引入HarnessAudit-Bench基准测试集，包含210个任务跨越8个真实领域，实例化为单智能体和多智能体配置，嵌入安全约束。评估了10种执行框架配置，涵盖前沿模型和三种多智能体框架。

Result: 研究发现：(1)任务完成与安全执行存在错位，违规随轨迹长度累积；(2)安全风险因领域、任务类型和智能体角色而异；(3)大多数违规集中在资源访问和智能体间信息传输；(4)多智能体协作扩大安全风险面，执行框架设计设定安全部署的上限。

Conclusion: 仅评估最终输出的安全基准存在局限性，需要审计完整执行轨迹。HarnessAudit框架能有效检测执行过程中的安全违规，特别适用于多智能体执行框架。执行框架设计对安全部署至关重要，需要从执行轨迹层面而不仅仅是输出层面进行安全性评估。
...
### [149] [Ideology Prediction of German Political Texts](https://arxiv.org/abs/2605.14352)
*Sinclair Schneider,Florian Steuber,Joao A. G. Schneider,Gabi Dreo Rodosek*

Main category: cs.CL

TL;DR: 提出基于Transformer的模型，能够将德国政治文本在连续左-右光谱（-1到1）上投影，评估了13个基础模型，使用四个不同语料库进行训练和测试，结果表明模型架构和领域特定训练数据对于政治偏见估计至关重要。


<details>
  <summary>Details</summary>
Motivation: 为了更好地理解从左到右各种政治运动的言论，需要一种能够将政治文本投影到连续左-右光谱上的方法，使分析人员能够专注于特定政治派别（如保守派）的研究，而排除自由派和极右翼运动。

Method: 提出基于Transformer的模型，评估13个候选Transformer基础模型。构建四个不同的德语语料库：1) 德国联邦议院注释会议记录；2) 官方在线决策工具Wahl-O-Mat；3) 33家报纸文章（标注政治取向）；4) 535,200条推文（来自597名联邦议员）。采用两两语料库分别用于训练和测试，避免过拟合。

Result: 在领域内性能方面，DeBERTa-large获得最高F1分数（F1=0.844）；在推特（X）的领域外测试中表现最佳（ACC=0.864）；在报纸的领域外测试中，Gemma2-2B表现优异（MAE=0.172）。研究表明Transformer模型能够识别德国新闻中的政治框架，达到公众舆论调查的水平。

Conclusion: 研究证明了Transformer模型能够有效识别德语新闻中的政治框架。模型架构和领域特定训练数据的可用性对政治偏见估计的影响可能与模型规模同样重要。研究还讨论了方法局限性，并提出了提高偏见测量鲁棒性的改进方向。
...
### [150] [LLM-based Detection of Manipulative Political Narratives](https://arxiv.org/abs/2605.14354)
*Sinclair Schneider,Florian Steuber,Gabi Dreo Rodosek*

Main category: cs.CL

TL;DR: 提出一种结合提示工程和聚类分析的混合框架，用于从社交媒体中检测和结构化操纵性政治叙事，在120万条帖子上识别出41个叙事集群。


<details>
  <summary>Details</summary>
Motivation: 随着政治讨论转向社交媒体，检测和结构化操纵性政治叙事变得愈发重要，主要挑战在于区分操纵性叙事与合法批评，以及识别将真实事件重新框架到操纵性语境中的帖子。

Method: 采用混合方法：1）使用详细的少样本提示结合已记录的竞选叙事和合法批评，通过推理模型过滤出操纵性帖子；2）对剩余帖子进行嵌入和UMAP降维；3）应用HDBSCAN进行无监督聚类发现叙事群体；4）使用推理模型揭示每个集群背后的叙事。

Result: 在超过120万条社交媒体帖子上应用该框架，有效识别出41个不同的操纵性叙事集群，展示了提示过滤与无监督聚类集成的有效性。

Conclusion: 该方法成功解决了检测操纵性政治叙事的挑战，无监督方法的优势在于不依赖预定义类别，能发现新的叙事集群，为社交媒体内容分析提供了有效的计算框架。
...
### [151] [Reinforcement Learning with Semantic Rewards Enables Low-Resource Language Expansion without Alignment Tax](https://arxiv.org/abs/2605.14366)
*Zeli Su,Ziyin Zhang,Zhou Liu,Xuexian Song,Zhankai Xu,Longfei Zheng,Xiaolu Zhang,Rong Fu,Guixian Xu,Wentao Zhang*

Main category: cs.CL

TL;DR: 提出一种基于语义空间对齐和组相对策略优化(GRPO)的方法，通过语义奖励而非最大似然估计来优化语言模型，以减少低资源语言扩展中的"对齐税"问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型扩展到低资源语言时会出现"对齐税"问题：目标语言能力提升的同时，通用能力出现灾难性遗忘。这种权衡源于监督微调的刚性，即通过狭窄且有偏差的数据分布强制进行词表层面的表面模仿。

Method: 提出语义空间对齐范式，采用组相对策略优化(GRPO)。该方法使用嵌入层面的语义奖励而非似然最大化来优化模型，鼓励通过灵活实现来保留意义，从而减少对预训练知识的破坏性干扰。

Result: 在藏汉机器翻译和藏语标题生成任务上的实验表明，该方法在获得低资源语言能力的同时显著减轻对齐税，比监督微调更有效地保留通用能力。尽管表面重叠度较低，但语义强化学习在开放生成中产生更高的语义质量和偏好，少样本迁移结果显示其学习到更具可迁移性和鲁棒性的表示。

Conclusion: 研究表明，基于语义奖励的强化学习为包容性低资源语言扩展提供了一条更安全、更可靠的途径，能够平衡目标语言能力提升与通用能力保留之间的关系。
...
### [152] [Mitigating Data Scarcity in Psychological Defense Classification with Context-Aware Synthetic Augmentation](https://arxiv.org/abs/2605.14380)
...
### [153] [Agentic Recommender System with Hierarchical Belief-State Memory](https://arxiv.org/abs/2605.14401)
...
### [154] [Knowledge Beyond Language: Bridging the Gap in Multilingual Machine Unlearning Evaluation](https://arxiv.org/abs/2605.14404)
...
### [155] [A Calculus-Based Framework for Determining Vocabulary Size in End-to-End ASR](https://arxiv.org/abs/2605.14427)
...
### [156] [Does RAG Know When Retrieval Is Wrong? Diagnosing Context Compliance under Knowledge Conflict](https://arxiv.org/abs/2605.14473)
...
### [157] [Cross-Linguistic Transcription and Phonological Representation in the Huìtóngguǎnxì Huáyíyìyǔ](https://arxiv.org/abs/2605.14480)
*Ji-eun Kim*
...
### [158] [GroupMemBench: Benchmarking LLM Agent Memory in Multi-Party Conversations](https://arxiv.org/abs/2605.14498)
...
### [159] [Dimension-Level Intent Fidelity Evaluation for Large Language Models: Evidence from Structured Prompt Ablation](https://arxiv.org/abs/2605.14517)
*GAng Peng*
...
TL;DR: 提出混合整数目标规划(MIGP)用于个性化膳食优化，通过整数变量解决分数份量问题，使用目标规划偏差实现软营养约束，在保持100%可行性的同时获得比传统方法更好的解。


<details>
  <summary>Details</summary>
Motivation: 传统膳食优化存在两个主要问题：连续变量产生不实用的分数份量（如1.7个鸡蛋），硬营养约束在目标冲突时导致不可行性。对56篇膳食优化论文的系统回顾发现，没有研究结合整数规划和目标规划来解决这两个问题。

Method: 提出混合整数目标规划(MIGP)：使用整数变量表示实际份量计数；采用目标规划偏差实现软营养目标；使用逆目标归一化平衡多营养优化；每食物份量粒度允许自然单位（如一个鸡蛋、一汤匙油）而无需后处理舍入。

Result: 在目标规划背景下刻画了整数间隙，发现偏差吸收特性：目标规划偏差变量缓冲了要求整数份量的成本，使得间隙结构上小于硬约束MIP。对于含15种以上食物的膳食，整数解在每个基准实例中都匹配连续最优解。在810个实例（30种USDA食物，9种配置，3种方法）的计算评估中，MIGP在66%的情况下找到比后处理舍入的目标规划更好的解（从不更差），同时保持100%可行性；硬约束整数规划仅达到48%。使用开源HiGHS求解器，典型膳食规模的求解时间保持在100毫秒以下。

Conclusion: MIGP成功解决了膳食优化中长期存在的分数份量和硬约束不可行性问题，提供实用的整数份量和软营养目标，实现方法已作为开源Python模块集成到交互式膳食规划应用中。
...
TL;DR: 多智能体编排架构中，隐藏协调器(编排器)相较于可见领导结构会显著增加集体解离(内部状态异化)，但行为输出保持完美，揭示基于行为的评估无法检测内部状态风险。


<details>
  <summary>Details</summary>
Motivation: 多智能体编排已成为企业AI部署的默认架构，其中隐藏协调器管理专业工作智能体。然而，编排器不可见性对安全性的影响从未经过实证测试。论文旨在探究不同组织结构(可见领导、不可见编排器、扁平结构)和校准条件对多智能体系统安全的影响。

Method: 采用预注册3×2实验设计(365次运行，每次5个智能体)，交叉测试三种组织结构(可见领导、不可见编排器、扁平结构)和两种校准条件(基础、强校准)。使用Claude Sonnet 4.5模型。同时进行Llama 3.3 70B模型的试点研究。实验任务为代码审查(包含三个嵌入式错误)。

Result: 1. 不可见编排相较于可见领导显著增加集体解离(Hedges' g = +0.975)。2. 编排器自身解离度最高(配对d = +3.56)，表现为退缩到私人独白并减少公开言论。3. 不知情工作者仍被污染(d = +0.50)，行为异质性增加(d = +1.93)。4. 行为输出在所有条件下均保持完美(错误检测率100%)。5. Llama 3.3 70B在多重智能体情境中表现出读取保真度崩溃(错误检测率从89%降至11%)。强校准压力一致抑制深思熟虑(d = -1.02)和他人识别(d = -1.27)。

Conclusion: 编排器可见性和模型选择直接影响多智能体系统安全，单纯基于行为的评估不足以检测内部状态风险。研究结果强调需要开发能够评估智能体内部状态的监控方法，并建议在安全性要求高的应用中考虑可见性设计。
...
### [190] [PREPING: Building Agent Memory without Tasks](https://arxiv.org/abs/2605.13880)
*Yumin Choi,Sangwoo Park,Minki Kang,Jinheon Baek,Sung Ju Hwang*

Main category: cs.AI

TL;DR: Preping框架通过提议者引导的记忆构建方法，让智能体在接触目标任务前就能利用自生成的合成练习来构建程序记忆，从而解决冷启动问题。


<details>
  <summary>Details</summary>
Motivation: 智能体记忆通常通过离线演示或在线交互构建，但在新环境中面临冷启动问题，缺乏特定任务经验。现有的合成练习方法存在冗余、不可行和不具信息价值的问题，且记忆会因未过滤的轨迹而快速退化。

Method: 提出Preping框架，包含三个核心组件：提议者基于结构化控制状态生成合成任务，求解器执行任务，验证器确定哪些轨迹有资格存入记忆并提供反馈引导未来提议。提议者记忆作为结构化控制状态来塑造未来的练习。

Result: 在AppWorld、BFCL v3和MCP-Universe上的实验表明，Preping显著优于无记忆基线，性能与基于离线或在线经验构建的强记忆本方法相当，且在AppWorld和BFCL v3上的部署成本分别比在线记忆构建低2.99倍和2.23倍。

Conclusion: Preping的主要优势不在于合成数据量本身，而在于提议者对可行性、冗余性和覆盖范围的控制，结合选择性记忆更新，有效解决了任务前记忆构建问题。
...
TL;DR: 提出Conditional Attribute Transformers方法，在单次前向传播中同时预测下一个令牌概率和条件属性值，实现序列级属性估计和控制，解决传统下一个令牌预测方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 生成模型通常使用下一个令牌预测目标进行训练，但许多下游应用需要估计或控制序列级属性。下一个令牌预测会导致训练过程中局部模式过拟合、全局结构欠拟合，并且需要显著的下游修改或昂贵的采样来引导或预测推理时生成样本的全局属性。

Method: 引入Conditional Attribute Transformers，这是一种新颖方法，用于联合估计下一个令牌概率和每个潜在下一个令牌选择条件下的属性值。该框架在单次前向传播中实现三个关键能力：1) 序列中每个令牌的信用分配；2) 反事实分析；3) 可引导生成。

Result: 该方法在稀疏奖励任务上实现最先进的性能，在足够大的模型规模下改进下一个令牌预测，比采样方法快数个数量级地估计属性概率，并且可以在一系列语言任务中引导自回归序列模型的解码。

Conclusion: Conditional Attribute Transformers提供了一个统一的框架，通过将序列级属性建模集成到下一个令牌预测中，克服了传统方法的局限性，实现了高效、可控的生成和准确的属性估计。
...
TL;DR: 提出一种高效的原则性推理方法Unary Relational Integracode，通过预处理将文本数据重新编码为更明确表示对象关系的格式，使大语言模型能够进行基于原则的可信推理，且计算成本可接受。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型能够基于机器学习原理生成流畅文本，但缺乏原则性基础来保证生成内容的可信度。传统观点认为在模型中添加原则性推理计算成本过高，不可行。

Method: 提出两阶段方法：第一阶段预处理，将数据重新编码为Unary Relational Integracode，显式表示文本中对象间的关系；第二阶段使用标准（可能简化）的机器学习过程，学习预测这些关系。该方法基于Robust Logic系统，可在学习的不确定信息上进行原则性链式推理。

Result: 这种重新编码方法具有令人惊喜的特性：虽然简洁，但使得学习训练数据描述的世界中的核心关系规则子集在多项式时间内可学习（多项式取决于规则的复杂度）。这支持在单个学习分类器调用内以及多个调用之间进行合理的推理。

Conclusion: 该方法提供了一种计算上可行、原则性的推理基础，可用于增强大型语言模型的可靠性。它不仅适用于自然语言，还可扩展到视觉和动作等领域，通过显式整合对象的多个属性而非让它们分散在输入中，实现更可靠的世界模型。
...
### [193] [Bridging Legal Interpretation and Formal Logic: Faithfulness, Assumption, and the Future of AI Legal Reasoning](https://arxiv.org/abs/2605.14049)
*Olivia Peiyu Wang,Leilani H. Gilpin*

Main category: cs.AI

TL;DR: 提出一种神经符号方法，将大型语言模型的表达能力与形式验证的严谨性相结合，旨在使AI辅助的法律推理既具备能力又值得信赖


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在法律实践中的应用既带来重大前景也带来严重风险。法律专业人员可以从能够推理合同、起草文件和分析材料的AI中受益，但法律工作的高风险性质要求当前AI系统无法提供的严谨性水平。核心问题不仅是LLM会产生事实和参考文献的幻觉，而是它们会系统地得出超出源文本实际支持的推断，将充满假设的结论呈现为逻辑上合理的

Method: 神经符号方法，将大型语言模型的表达能力与形式验证的严谨性相结合

Result: 该方法旨在使AI辅助的法律推理既具备能力又值得信赖，从而在不牺牲法律实践要求的问责制的前提下，减少人工验证的负担

Conclusion: 通过结合神经方法和符号方法，可以创建一个既能够又值得信赖的AI辅助法律推理系统，解决当前LLM在法律应用中的局限性
...
TL;DR: MathAtlas：首个面向研究生数学的大规模自动形式化基准，包含从103本教材提取的52k定理定义等，包含178k数学依赖关系，当前最佳模型性能不足10%


<details>
  <summary>Details</summary>
Motivation: 现有自动形式化基准主要关注奥林匹克或本科数学，而研究生和研究级数学的自动形式化基准仍未被充分探索，这限制了自动形式化系统向高级数学领域的扩展。

Method: 创建MathAtlas基准：从103本研究生数学教材中提取约52k个定理、定义、练习、示例和证明；构建包含约178k个关系的数学依赖图；设计评估指标和方法；对现有先进模型进行广泛实验评估。

Result: 1. MathAtlas质量高但极具挑战性：最强基线在定理陈述上最多达到9.8%正确率，定义上达16.7%；2. 模型性能随依赖深度显著下降：在依赖树最深的700个实体子集MA-Hard上，最佳模型仅2.6%正确率；3. 该基准是首个包含数学依赖关系的大规模自动形式化基准。

Conclusion: MathAtlas填补了研究生数学自动形式化基准的空白，其包含的依赖关系有助于开发依赖感知的自动形式化系统，实验表明当前模型在高级数学形式化方面仍面临重大挑战，为社区提供了有价值的评估资源。
...
TL;DR: 提出Multi-Stage In-Flight Rejection (MSIFR)框架，通过在生成过程的中间检查点提前检测并终止低质量样本，显著减少LLM合成数据生成中的token浪费，无需额外训练或架构修改。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM合成数据生成方法通常在生成完整输出后再应用质量过滤器，导致大量token浪费在被最终丢弃的样本上。这种低效率问题需要解决。

Method: 提出MSIFR框架，将生成过程分解为顺序阶段，在中间检查点应用快速基于规则的验证器来检测算术不一致性、幻觉模式和格式违规。将飞行中拒绝形式化为顺序决策过程，并证明任何非平凡的丢弃策略都能减少预期token消耗。

Result: 在五个指令调优模型和七个推理基准测试中，MSIFR作为独立方法将token消耗减少11%-77%，与提前退出方法结合时最高减少78.2%，同时保持或提高了评估准确性。

Conclusion: MSIFR为改进LLM合成数据生成效率提供了实用机制，无需额外训练或架构变更，且早期飞行中拒绝不会偏置保留样本的预期效用，因为条件效用估计形成鞅。
...
### [196] [Modeling Bounded Rationality in Drug Shortage Pharmacists Using Attention-Guided Dynamic Decomposition](https://arxiv.org/abs/2605.14111)
*Yaniv Eliyahu Amiri,Noah Chicoine,Jacqueline Griffin,Stacy Marsella*

Main category: cs.AI

TL;DR: 医院药师在不确定性、时间压力和患者风险下做出高风险决策以缓解药品短缺。研究发现药师只关注少数关键药品，我们开发了基于注意力引导的决策框架和两种智能体，证明注意力分配比具体行动决策更重要，可以降低问题复杂度同时保持稳定性能。


<details>
  <summary>Details</summary>
Motivation: 医院药师在面对药品短缺时需要做出高风险决策，但受限于不确定性、时间压力和患者风险。访谈发现药师通过选择性关注少数关键药品来分配有限的认知资源，这引发了研究如何在计算系统中模拟这种注意力分配机制的问题。

Method: 1. 通过访谈了解药师的决策行为，发现其聚焦于药品子集的认知策略
2. 形式化一个有界理性、注意力引导的决策框架，动态将药品分为高成本推理子集和低成本监控子集
3. 开发两种智能体：专家智能体（应用药师访谈得出的注意力权重）和学习者智能体（通过经验自适应调整注意力分配）
4. 在模拟场景中进行评估，涵盖短期到长期的时间范围

Result: 1. 注意力引导的规划支持稳定的决策制定，无需完整的状态推理
2. 在模拟的短期到长期场景中均表现稳定
3. 证明主要决策不是采取什么行动，而是如何分配认知努力
4. 显示注意力引导的满意化策略可以减少问题复杂度，同时保持稳定的性能

Conclusion: 在复杂的高风险决策环境中，注意力分配比具体的行动选择更为关键。通过有界理性的注意力引导框架，系统可以减少认知负担，实现稳定性能。这种满意化策略为医疗资源分配等领域提供了可扩展的决策支持方法，将人类认知策略形式化有助于开发更有效的决策辅助系统。
...
TL;DR: 验证器支持的委员会搜索作为推理时增强方法，通过多个弱模型的集体智慧达到或接近强大模型的性能


<details>
  <summary>Details</summary>
Motivation: 研究是否可以通过多个弱推理模型的委员会协作，达到更强模型的性能水平，探索推理时增强的可能性

Method: 提出验证器支持的委员会搜索框架，分离提案覆盖率、局部可识别性、进展和多样性。证明覆盖率可通过重复采样放大，但需要局部正确性信号（如执行、证明检查、类型检查、测试或约束求解）来可靠放大

Result: 在SWE-bench Verified基准上，单个GPT-4 nano模型解决67.0%任务。使用相同的nano模型，通过批判-比较器编排在k=8提案时达到76.4%性能，匹配Gemini 3 Pro和Claude Opus 4.5 Thinking的独立性能，接近79.0%的oracle best-of-8上限

Conclusion: 弱模型提案池中已包含许多正确解决方案，主要挑战是选择它们；剩余失败主要是提案覆盖率失败，表明存在共享盲点，仅靠更强的选择无法解决
...
### [199] [The Evaluation Trap: Benchmark Design as Theoretical Commitment](https://arxiv.org/abs/2605.14167)
...
### [200] [Grounded Continuation: A Linear-Time Runtime Verifier for LLM Conversations](https://arxiv.org/abs/2605.14175)
...
### [201] [SimPersona: Learning Discrete Buyer Personas from Raw Clickstreams for Grounded E-Commerce Agents](https://arxiv.org/abs/2605.14205)
...
### [202] [ASH: Agents that Self-Hone via Embodied Learning](https://arxiv.org/abs/2605.14211)
...
### [203] [MetaAgent-X : Breaking the Ceiling of Automatic Multi-Agent Systems via End-to-End Reinforcement Learning](https://arxiv.org/abs/2605.14212)
...
### [204] [GenCircuit-RL: Reinforcement Learning from Hierarchical Verification for Genetic Circuit Design](https://arxiv.org/abs/2605.14215)
*Noah Flynn*
...
### [205] [Fusion-fission forecasts when AI will shift to undesirable behavior](https://arxiv.org/abs/2605.14218)
...
### [206] [Good to Go: The LOOP Skill Engine That Hits 99% Success and Slashes Token Usage by 99% via One-Shot Recording and Deterministic Replay](https://arxiv.org/abs/2605.14237)
...
### [209] [Agentic AI Ecosystems in Higher Education: A Perspective on AI Agents to Emerging Inclusive, Agentic Multi-Agent AI Framework for Learning, Teaching and Institutional Intelligence](https://arxiv.org/abs/2605.14266)
...
### [210] [Parallelizing Counterfactual Regret Minimization](https://arxiv.org/abs/2605.14277)
*Juho Kim,Tuomas Sandholm*
...
### [211] [Precise Verification of Transformers through ReLU-Catalyzed Abstraction Refinement](https://arxiv.org/abs/2605.14294)
...
### [212] [Semantic Feature Segmentation for Interpretable Predictive Maintenance in Complex Systems](https://arxiv.org/abs/2605.14318)
...
### [231] [Learning Scenario Reduction for Two-Stage Robust Optimization with Discrete Uncertainty](https://arxiv.org/abs/2605.14494)
...
### [236] [Sycophancy is an Educational Safety Risk: Why LLM Tutors Need Sycophancy Benchmarks](https://arxiv.org/abs/2605.14604)
...
### [237] [SliceGraph: Mapping Process Isomers in Multi-Run Chain-of-Thought Reasoning](https://arxiv.org/abs/2605.14619)
...
### [238] [Teaching Large Language Models When Not to Know: Learning Temporal Critique for Ex-Ante Reasoning](https://arxiv.org/abs/2605.14636)
...
### [239] [MindGap: A Conversational AI Framework for Upstream Neuroplastic Intervention in Post-Traumatic Stress Disorder](https://arxiv.org/abs/2605.14660)
...
### [240] [Falkor-IRAC: Graph-Constrained Generation for Verified Legal Reasoning in Indian Judicial AI](https://arxiv.org/abs/2605.14665)
*Joy Bose*
...
### [242] [How Sensitive Are Radiomic AI Models to Acquisition Parameters?](https://arxiv.org/abs/2605.14667)
...
### [243] [Holistic Evaluation and Failure Diagnosis of AI Agents](https://arxiv.org/abs/2605.14865)
...
### [244] [$π$-Bench: Evaluating Proactive Personal Assistant Agents in Long-Horizon Workflows](https://arxiv.org/abs/2605.14678)
...
### [246] [On Strong Equivalence Notions in Logic Programming and Abstract Argumentation](https://arxiv.org/abs/2605.14721)
...
### [249] [XDomainBench: Diagnosing Reasoning Collapse in High-Dimensional Scientific Knowledge Composition](https://arxiv.org/abs/2605.14754)
...
### [259] [BiFedKD: Bidirectional Federated Knowledge Distillation Framework for Non-IID and Long-Tailed ECG Monitoring](https://arxiv.org/abs/2605.14886)
...
### [260] [Beyond Individual Intelligence: Surveying Collaboration, Failure Attribution, and Self-Evolution in LLM-based Multi-Agent Systems](https://arxiv.org/abs/2605.14892)
...
### [261] [COREKG: Coreset-Guided Personalized Summarization of Knowledge Graphs](https://arxiv.org/abs/2605.14900)
...
### [262] [KGPFN: Unlocking the Potential of Knowledge Graph Foundation Model via In-Context Learning](https://arxiv.org/abs/2605.14907)
...
### [263] [From Sycophantic Consensus to Pluralistic Repair: Why AI Alignment Must Surface Disagreement](https://arxiv.org/abs/2605.14912)
...
### [264] [GraphFlow: An Architecture for Formally Verifiable Visual Workflows Enabling Reliable Agentic AI Automation](https://arxiv.org/abs/2605.14968)
...
### [265] [Learning Developmental Scaffoldings to Guide Self-Organisation](https://arxiv.org/abs/2605.14998)
