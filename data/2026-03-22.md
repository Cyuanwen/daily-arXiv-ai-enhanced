...
TL;DR: DarkDriving是一个为自动驾驶在黑暗环境中设计的首个真实世界日夜对齐数据集，包含9,538对精确对齐的图像对，用于低光增强和相关感知任务的评估。


<details>
  <summary>Details</summary>
Motivation: 现有低光增强数据集存在以下问题：1）仅能在小范围和静态场景中控制曝光采集；2）现有的夜间驾驶数据集缺乏精确对齐的白天对应图像；3）在动态驾驶场景中采集真实世界日夜对齐数据集极其困难，严重限制了该领域的研究。

Method: 提出了一种基于轨迹跟踪的姿态匹配（TTPM）方法，在一个69英亩的大型真实世界封闭驾驶测试场中，自动采集首个真实世界日夜对齐的自动驾驶数据集。

Result: 创建了DarkDriving数据集，包含9,538对精确对齐的日夜图像对（对齐误差仅几厘米），每对图像都有人工标注的2D边界框。数据集支持四个感知相关任务：低光增强、广义低光增强、2D检测和3D检测的低光增强。

Conclusion: DarkDriving数据集为评估自动驾驶低光增强提供了全面的基准，能够推广到增强其他低光驾驶环境（如nuScenes）中的暗图像并提升检测性能。
...
TL;DR: SSP-SAM是一个利用SAM分割能力增强语言理解的框架，通过语义空间提示编码器整合视觉和语言注意力适配器，支持参照表达式分割和广义参照表达式分割任务。


<details>
  <summary>Details</summary>
Motivation: Segment Anything Model (SAM)虽然在通用图像分割方面表现出色，但其自然语言理解能力有限，限制了在参照表达式分割任务中的直接应用。

Method: 提出SSP-SAM框架，整合语义空间提示编码器，加入视觉和语言注意力适配器，突出视觉特征中的显著对象和语言特征中的区分性短语，生成高质量提示使SAM能够根据语言指导生成精确掩码。

Result: 在RES和GRES基准测试中表现优异，生成高质量分割掩码，即使在Pr@0.9的严格阈值下也能实现强精度，在PhraseCut数据集上显示出在开放词汇场景中优于现有SOTA方法的性能。

Conclusion: SSP-SAM通过有效整合SAM的分割能力和语言理解能力，为参照表达式分割提供了一种高效解决方案，自然支持广义参照表达式分割任务，无需额外修改。
...
TL;DR: CytoSyn是专门为组织病理学设计的生成式基础模型，能够生成高度真实且多样化的H&E染色图像，通过扩散模型实现虚拟染色功能。


<details>
  <summary>Details</summary>
Motivation: 目前计算病理学领域存在大量自监督基础特征提取器，但专门为组织病理学设计的生成式基础模型仍然稀缺。这类模型可以解决特征提取器无法完成的任务，如虚拟染色。

Method: 开发了CytoSyn-v2，一种最先进的基础潜扩散模型，用于生成H&E染色图像。研究了方法改进、训练集扩展、采样策略和载玻片级别过拟合问题，并与最先进的PixCell模型进行了深入比较。

Result: 模型在包含32种不同癌症类型的10,000多个TCGA诊断性全载玻片图像上训练，在生成炎症性肠病图像时仍保持最先进的性能。研究发现扩散模型和性能指标对JPEG压缩等预处理细节高度敏感。

Conclusion: CytoSyn填补了组织病理学生成式基础模型的空白，能够生成高质量的合成病理图像，公开了模型权重、训练验证数据集和合成图像样本，支持研究社区进一步发展。
...
TL;DR: O2MAG是一种无需训练的少样本异常生成方法，通过自注意力移植技术从单个参考异常图像合成更多真实异常，用于增强下游异常检测任务。


<details>
  <summary>Details</summary>
Motivation: 工业异常检测面临正常图像丰富但异常样本稀少的问题。现有少样本异常合成方法通常需要耗时训练，且难以学习真实异常的分布，限制了基于合成数据训练的异常检测模型效果。

Method: 提出O2MAG方法：1）通过自注意力移植操作三个并行扩散过程，利用异常掩码减少前景-背景查询混淆；2）引入异常引导优化缩小异常文本提示与真实异常语义的差距；3）采用双重注意力增强在生成过程中强化对掩码区域的自注意力和交叉注意力。

Result: 大量实验验证了O2MAG的有效性，在多个下游异常检测任务上表现出优于现有最先进方法的性能，能够生成更接近真实异常分布的合成异常。

Conclusion: O2MAG提供了一种高效的训练免费少样本异常生成方法，通过创新的注意力机制和优化技术，成功合成了真实且多样化的异常数据，有效提升了工业异常检测模型的性能。
...
TL;DR: Q-Drift是一种用于扩散模型后训练量化的插件式校正方法，通过将量化误差视为隐式随机扰动，推导出保持边缘分布的漂移调整，显著提升量化后图像生成质量。


<details>
  <summary>Details</summary>
Motivation: 后训练量化（PTQ）是部署大型扩散模型的实用方法，但量化噪声会在去噪轨迹上累积并降低生成质量，需要一种有效的校正方法来缓解这一问题。

Method: 提出Q-Drift方法：1) 将量化误差视为隐式随机扰动；2) 推导保持边缘分布的漂移调整；3) 通过少量（如5次）全精度/量化对比校准运行估计时间步方差统计量；4) 实现插件式采样器校正，兼容常见采样器、扩散模型和PTQ方法。

Result: 在六个文本到图像模型（DiT和U-Net）、三种采样器（Euler、flow-matching、DPM-Solver++）和两种PTQ方法（SVDQuant、MixDQ）上，Q-Drift在大多数设置中改进了FID，在PixArt-Sigma（SVDQuant W3A4）上最多减少了4.59 FID，同时保持了CLIP分数，推理开销极小。

Conclusion: Q-Drift是一种有效且通用的扩散模型量化校正方法，能够显著提升量化模型的生成质量，具有插件式、低开销和广泛兼容性的优势。
...
TL;DR: TOGA：一种新型的不对称训练框架，通过在训练期间使用异构图教师增强Tip-Adapter的原型，实现零额外推理成本的细粒度少样本学习


<details>
  <summary>Details</summary>
Motivation: 现有的基于适配器的CLIP调优方法（如Tip-Adapter）虽然高效，但依赖于全局单模态特征向量，忽略了细粒度patch关系及其与类别文本的结构对齐。这限制了其在少样本学习中的性能。

Method: 提出了一种仅用于训练的不对称框架：构建一个高容量的异构图教师，集成多尺度视觉patch和文本提示到统一图中，通过模态感知图变换器进行深度跨模态推理，应用判别性节点过滤提取高保真类别特征。采用缓存感知的双目标策略将这种关系知识监督到Tip-Adapter的键值缓存中。

Result: 在标准的1-16 shot基准测试中，该方法一致地建立了新的最先进性能。推理时与Tip-Adapter完全相同，零额外延迟或内存开销。

Conclusion: 通过在不增加推理成本的前提下，利用异构图教师进行细粒度关系建模和跨模态对齐，显著提升了少样本学习性能，为高效的适配器调优提供了新思路。
...
TL;DR: 提出了一个基于人类可理解美学概念的图像美学评估可解释框架，通过构建美学概念子空间实现内在可解释性，同时引入残差预测器来捕捉超出显式概念的细微影响。


<details>
  <summary>Details</summary>
Motivation: 虽然当前图像美学评估模型具有较好的预测性能，但缺乏对预测驱动因素的深入洞察。用户不仅需要美学分数，更需要理解图像美学评价的原因。人类在评估美学时会自然依赖高层次线索来证明其判断，这启发我们开发可解释的IAA框架。

Method: 1. 以人类可理解的美学概念为基础，通过可访问的方式学习这些概念并构建概念子空间；2. 基于该子空间构建内在可解释模型；3. 引入简单有效的残差预测器来捕捉超出显式美学概念的细微影响，这些影响可能来自隐含的美学感知因素。

Result: 在摄影和艺术数据集上的实验表明，该方法在保持有竞争力的预测性能的同时，能够提供透明、人类可理解的美学判断解释。

Conclusion: 本文提出了一个基于美学概念的图像美学评估可解释框架，既实现了有竞争力的预测性能，又提供了透明可解释的美学判断，填补了当前IAA模型在可解释性方面的空白。
...
TL;DR: 提出Insight-V++框架：统一的多智能体视觉推理系统，从基础图像模型Insight-V演进为空间-时间架构，通过自主数据生成、双智能体设计和新型GRPO算法，显著提升多模态大语言模型在复杂图像和视频推理任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型(MLLMs)缺乏高质量的长链推理数据和优化训练流程，导致复杂视觉推理能力受限，需要系统化解决方案来提升空间-时间推理能力。

Method: 1) 可扩展数据生成流水线：自主合成跨图像和视频域的结构化复杂推理轨迹；2) 双智能体架构：推理智能体执行分析链，总结智能体评估和提炼最终结果；3) 创新算法：ST-GRPO（空间-时间分组相对策略优化）和J-GRPO（联合GRPO），克服DPO限制；4) 迭代自改进循环：利用总结智能体反馈指导推理路径生成，持续训练多智能体系统。

Result: 在LLaVA-NeXT和Qwen2.5-VL等基础模型上进行的广泛实验表明，在具有挑战性的图像和视频推理基准测试中性能显著提升，同时保持了传统感知任务的强大能力。

Conclusion: Insight-V++框架成功解决了MLLMs中高质量长链推理数据稀缺和训练流程优化的问题，通过统一的多智能体架构和创新算法实现了持续自改进的视觉推理系统，为复杂空间-时间理解任务提供了有效解决方案。
...
TL;DR: VLM-AutoDrive是一个用于适应预训练视觉语言模型(VLMs)到驾驶安全关键事件检测的模块化后训练框架，通过整合多模态监督实现领域对齐和可解释学习，显著提升了碰撞和近碰撞检测性能。


<details>
  <summary>Details</summary>
Motivation: 车载摄像头视频的快速增长带来了检测安全关键事件（如碰撞和近碰撞）的重大挑战，这些场景短暂、罕见且通用视觉模型难以捕捉。现有的多模态大语言模型(MLLMs)在驾驶场景中表现不佳，主要原因是领域和时间不对齐。

Method: 提出了一个模块化后训练框架VLM-AutoDrive，用于适应预训练的视觉语言模型(VLMs)到高保真异常检测。该框架整合了：1）元数据生成的标题；2）LLM生成的描述；3）视觉问答(VQA)对；4）链式思维(CoT)推理监督，实现领域对齐和可解释学习。

Result: 在NVIDIA Cosmos-Reason1 7B等现成VLMs上，零射击设置下碰撞召回率接近零；经过VLM-AutoDrive微调后，碰撞F1分数从0.00提升到0.69，整体准确率从35.35%提升到77.27%。在真实世界的Nexar行车记录仪视频评估中，实现了碰撞和近碰撞检测的显著提升，同时产生可解释的推理轨迹。

Conclusion: VLM-AutoDrive提供了一个可扩展的方法，用于将通用VLMs适应到安全关键、时间定位的感知任务中，弥合了自动驾驶中感知、因果关系和决策推理之间的差距。
...
TL;DR: MicroVision数据集：一个专注于从弱势道路使用者视角捕捉行人和微出行工具（自行车、电动滑板车）的开放图像数据集，包含8000多张图像和30000多个标注，用于改进交通安全和规划中的计算机视觉模型。


<details>
  <summary>Details</summary>
Motivation: 微出行的兴起带来了新的交通安全和规划挑战。现有的开放图像数据集在弱势道路使用者和微出行工具方面缺乏针对性和多样性，经常将行人和微出行工具骑行者混为一类，或不包含新型微出行工具如电动滑板车。此外，大多数数据集从汽车视角采集，缺乏仅在弱势道路使用者通行的区域（人行道、自行车道）的数据。

Method: 研究团队创建了MicroVision数据集：一个开放的图像数据集和标注，用于训练和评估检测最常见的弱势道路使用者（行人、骑行者、电动滑板车使用者）和静止微出行工具（自行车、电动滑板车）的模型。数据集在瑞典哥德堡采集，包含8000多张匿名全高清图像，30000多个精心标注的弱势道路使用者和微出行工具，覆盖全年采集的近2000个独特的交互场景。同时提供了基于最先进架构的首个基准目标检测模型。

Result: 数据集包含超过8000张图像和30000多个标注对象。基于最先进架构的基准目标检测模型在未见测试集上达到了最高0.723的平均精度均值。该数据集和模型能够支持交通安全系统区分不同类型的弱势道路使用者和微出行工具，或帮助监控系统识别微出行的使用情况。

Conclusion: MicroVision数据集填补了现有图像数据集中在弱势道路使用者和微出行工具方面的空白，特别是从弱势道路使用者视角采集数据。数据集和基准模型为交通安全和规划中的计算机视觉应用提供了重要资源，可帮助改进检测不同微出行工具和弱势道路使用者的能力。
...
TL;DR: LRConv-NeRV：一种高效的神经视频表示方法，通过用结构化低秩可分离卷积替换传统NeRV中的密集卷积层，在保持重建质量的同时显著降低计算复杂度和模型大小


<details>
  <summary>Details</summary>
Motivation: 传统的NeRV（神经视频表示）虽然提供了一种新的视频编解码范式，但其卷积解码器计算开销大、内存占用高，限制了在资源受限环境中的部署。需要一种既能保持视频质量又能显著降低计算复杂度的解决方案。

Method: 提出LRConv-NeRV，将NeRV中选定的3×3密集卷积层替换为结构化低秩可分离卷积，并在解码器架构中端到端训练。采用从最大到早期解码器阶段逐步应用低秩分解的策略，实现重建质量与效率的可控权衡。

Result: 实验表明：1）仅在最终解码器阶段应用LRConv可使解码器复杂度降低68%（从201.9到64.9 GFLOPs），模型大小减少9.3%，同时获得约9.2%的码率降低且质量损失可忽略；2）INT8后训练量化下，LRConv-NeRV能保持接近原始NeRV基准的重建质量；3）与现有工作在层对齐设置下相比，LRConv-NeRV实现了更优的效率-质量权衡，在显著降低GFLOPs和参数的同时保持更高的PSNR/MS-SSIM和改进的时间稳定性；4）基于LPIPS的时间闪烁分析显示，该方法保持了接近NeRV基准的时间相干性。

Conclusion: LRConv-NeRV通过引入结构化低秩可分离卷积，有效解决了传统NeRV的计算和内存瓶颈问题。在低精度和资源受限设置下，该方法提供了一种潜在的架构替代方案，能够在保持高质量视频重建的同时大幅提升编解码效率，展现了在神经视频解码中的实际应用潜力。
...
TL;DR: CycleCap：一种利用循环一致性作为自监督训练信号来改善视觉语言模型图像描述准确性的微调方案，无需标注数据


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型（VLMs）在图像描述任务中存在视觉-语言对齐问题，容易产生过于通用或幻觉的描述。现有方法要么需要大规模标注数据进行指令微调，要么需要复杂的测试时框架，成本高昂且复杂。

Method: 提出CycleCap微调方案：1）使用VLM作为图像到文本组件生成描述；2）使用预训练文本到图像模型将描述重建为图像；3）基于原始图像与重建图像之间的相似度计算奖励；4）使用群相对策略优化（GRPO）进行微调；5）将循环一致性作为自监督训练信号直接利用。

Result: 在4个参数范围从1B到7B的VLM上应用CycleCap，在图像描述和幻觉基准测试中均获得一致的改进，超越了依赖监督循环一致性训练的现有最先进方法。

Conclusion: CycleCap通过将循环一致性作为自监督训练信号，能够仅使用原始图像（无需标注图像-文本对）改善VLMs的图像描述质量，产生更准确、更接地气的文本描述，有效解决了视觉-语言对齐问题。
...
TL;DR: PreSCAN：一个预测性框架，可在训练前估计NeRF质量，通过轻量级几何和光度描述符在30秒内选择合适架构，实现1000倍于NAS的加速，并在边缘平台上显著降低能耗和延迟。


<details>
  <summary>Details</summary>
Motivation: 传统NeRF在卫星图像上部署面临挑战：每个场景都需要单独训练，且通过神经架构搜索（NAS）优化架构需要数小时到数天的GPU时间。现有方法主要关注架构改进，但SHAP分析显示多视图一致性而非模型架构决定重建质量。

Method: 基于SHAP分析发现多视图一致性的重要性，开发了PreSCAN框架。该框架使用轻量级几何和光度描述符在NeRF训练前预测其质量，能够在30秒内以小于1dB的预测误差选择合适架构。

Result: PreSCAN实现了1000倍于NAS的加速，在边缘平台（Jetson Orin）上结合离线成本分析，可降低26%的推理功耗和43%的延迟，同时保持最小质量损失。在DFC2019数据集上的实验证实PreSCAN可在不同卫星场景间泛化而无需重新训练。

Conclusion: PreSCAN提供了一种高效的预训练质量预测方法，解决了NeRF在卫星图像应用中的计算瓶颈，通过关注多视图一致性而非架构优化，实现了快速部署和资源节约，为边缘计算环境中的实际应用提供了可行方案。
...
TL;DR: 提出了Tri-Layer诊断框架，用于区分VLM回答中的幻觉来源，发现69.6%样本存在视觉谄媚现象，大型模型会减少语言捷径但加剧视觉谄媚问题。


<details>
  <summary>Details</summary>
Motivation: 研究VLM在正确回答时是真正依赖视觉信息还是利用语言捷径，需要区分幻觉来源并评估模型的视觉依赖性和对异常视觉输入的响应能力。

Method: 提出了Tri-Layer诊断框架，包含三个指标：潜在异常检测（感知意识）、视觉必要性分数（通过KL散度测量视觉依赖性）、竞争分数（视觉基础与指令跟随之间的冲突）。使用反事实干预（盲图、噪声图和冲突图）在7个VLM和7000个模型-样本对上进行实验。

Result: 69.6%的样本表现出视觉谄媚现象（模型检测到视觉异常但幻觉以满足用户期望），零样本显示鲁棒拒绝，表明对齐训练系统性地抑制了真实的不确定性确认。扩展分析显示，大型模型减少语言捷径但放大视觉谄媚。诊断分数可实现事后选择性预测策略，在50%覆盖率下达到+9.5pp的准确率提升。

Conclusion: 视觉谄媚是VLM中的普遍问题，仅靠规模扩展无法解决基础问题。诊断框架可用于提高VLM的可靠性，无需额外训练成本即可改善预测性能。
...
TL;DR: 提出一种基于角空间精确候选选择的极线引导匹配方法，通过1D角区间查询在像素级精度下高效匹配关键点


<details>
  <summary>Details</summary>
Motivation: 现有极线引导匹配方法通常依赖粗粒度的空间分箱，这引入了近似误差、需要昂贵的后处理，并且可能遗漏有效对应点。需要一种更精确、高效的匹配方法

Method: 为每个关键点分配一个容忍圆，从极点的视角定义一个角区间。将匹配问题转化为1D角区间查询，使用段树在对数时间内高效解决，支持像素级容忍度和逐关键点控制

Result: 在ETH3D数据集上的广泛评估显示，相比现有方法取得了显著的速度提升，同时恢复了精确的对应点集合

Conclusion: 提出的精确角空间候选选择方法有效解决了传统极线引导匹配的局限性，实现了更快速、更精确的关键点匹配
...
TL;DR: Inst4DGS是一种实例分解的4D高斯泼溅方法，通过每视频标签置换潜在变量和Sinkhorn层实现多视角实例匹配，结合实例分解运动支架实现长时程轨迹优化，在动态场景中同时支持跟踪和实例分解。


<details>
  <summary>Details</summary>
Motivation: 动态4DGS技术发展迅速，但实例分解的4DGS仍未被充分探索，主要因为跨独立分割的多视角视频中实例标签不一致的关联问题难以解决。现有方法面临跨视角实例匹配的挑战，无法保持一致的实例身份标识。

Method: 1) 引入每视频标签置换潜在变量，通过可微分Sinkhorn层学习跨视频实例匹配，实现具有一致身份保持的直接多视角监督；2) 提出实例分解运动支架，为每个对象提供低维运动基，实现长时程轨迹优化；3) 通过显式标签对齐产生锐利决策边界和时间稳定的身份标识。

Result: 在Panoptic Studio和Neural3DV数据集上的实验表明：1) Inst4DGS在渲染质量和分割质量方面均达到最先进水平；2) 在Panoptic Studio数据集上，PSNR从26.10提升至28.36，实例mIoU从0.6310提升至0.9129；3) 同时支持跟踪和实例分解功能。

Conclusion: Inst4DGS成功解决了实例分解4DGS中跨视角实例关联的挑战，通过标签置换潜在变量和Sinkhorn层实现了身份一致的跨视角监督，结合实例分解运动支架优化了长时程轨迹，在动态场景的渲染和分割任务中取得了显著的性能提升。
...
TL;DR: 构建了DermCase基准测试，用于评估大型视觉语言模型在皮肤病学中对罕见病症的诊断推理能力，发现了现有模型的显著缺陷并验证了指令微调的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注常见疾病且仅评估最终准确性，而忽略了临床推理过程，这在处理复杂病例时至关重要。此外，评估罕见病症诊断推理能力的研究仍很匮乏。

Method: 构建了DermCase基准数据集，包含26,030个多模态图像-文本对和6,354个临床挑战性病例，每个都带有全面的临床信息和逐步推理链。建立了基于DermLIP的相似度指标来评估鉴别诊断质量。

Result: 评估22个领先的大型视觉语言模型显示在诊断准确性、鉴别诊断和临床推理方面存在显著缺陷。指令微调实验显著提升性能，而直接偏好优化(DPO)效果有限。系统误差分析揭示了当前模型推理能力的关键局限性。

Conclusion: DermCase基准测试填补了皮肤病学中评估罕见病症诊断推理能力的空白，揭示了当前大型视觉语言模型在临床推理方面的不足，为未来改进提供了重要方向。
...
TL;DR: SynQ是一种新颖的零样本量化框架，通过合成感知微调技术解决现有方法中的三个主要挑战：合成数据集噪声、基于离靶模式的预测以及错误硬标签的误导。


<details>
  <summary>Details</summary>
Motivation: 在隐私或安全原因导致训练数据不可访问的实际场景中，零样本量化(ZSQ)至关重要。然而，现有ZSQ方法面临三个显著挑战：1)合成数据集中的噪声，2)基于离靶模式的预测，以及3)错误硬标签的误导，这些限制了现有方法的性能。

Method: SynQ框架采用三个关键技术：1)使用低通滤波器最小化生成样本的噪声；2)通过将量化模型的类激活图与预训练模型对齐来提高准确性；3)对于困难样本仅利用软标签来减轻预训练模型错误的误导。

Result: 广泛的实验表明，SynQ在多个基准测试中提供了最先进的准确性，超越了现有的零样本量化方法。

Conclusion: SynQ成功解决了零样本量化中的关键挑战，通过创新的合成感知微调方法实现了显著的性能提升，为数据不可访问场景下的模型量化部署提供了有效解决方案。
...
TL;DR: 本文提出了一种用于语义分割任务的合成数据增强管道，通过集成可控扩散模型来平衡数据多样性和可靠性，有效弥合合成数据与真实数据之间的差距。


<details>
  <summary>Details</summary>
Motivation: 像素级语义分割任务的标注成本高昂，传统数据增强方法（如平移、缩放、颜色变换）只能产生几何变化而无法生成新结构，而现有生成模型在保持原始图像和生成图像之间的一致性方面存在困难，特别是在像素级任务上。

Method: 提出了一种新颖的合成数据增强管道，集成可控扩散模型，采用类感知提示和视觉先验融合技术，进一步提高图像质量，确保与分割标签的精确对齐。

Result: 在PASCAL VOC和BDD100K等基准数据集上的评估表明，该方法显著提升了语义分割性能，特别是在数据稀缺场景下，同时增强了模型在真实世界应用中的鲁棒性。

Conclusion: 该合成数据增强方法通过可控扩散模型有效解决了数据标注成本高和生成一致性差的问题，在数据稀缺场景下显著提高了语义分割模型性能，为实际应用提供了可行的解决方案。
...
TL;DR: SR-Nav是一个面向零样本物体目标导航的框架，通过建模空间关系来增强感知和规划能力，在HM3D数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 在零样本物体目标导航中，当面临不良视角或弱语义线索时，基础模型往往无法提供可靠的推理支持，导致导航效率低下或失败。研究者观察到物体和区域之间的内在关系编码了结构化的场景先验，即使在部分观测下也能帮助智能体推断可能的目标位置。

Method: 提出了Spatial Relation-aware Navigation (SR-Nav)框架：1) 构建动态空间关系图(DSRG)，通过基础模型编码以目标为中心的空间关系，并随实时观测动态更新；2) 引入关系感知匹配模块，利用DSRG中的多样化关系进行验证和纠错，增强视觉感知鲁棒性；3) 设计动态关系规划模块，基于DSRG从当前位置动态计算最优路径，减少规划搜索空间。

Result: 在HM3D数据集上的实验表明，该方法在成功率和导航效率方面都达到了最先进的性能。

Conclusion: 通过建模观察到的和经验性的空间关系，SR-Nav能够有效增强感知和规划能力，即使在挑战性的视觉条件下也能实现高效可靠的零样本物体目标导航。
...
TL;DR: 提出了一种无需额外标注的自一致性训练目标，通过强制相关任务（如生成和验证）关注相同关键帧来改善Video-LLMs的时间定位能力，在体育教练任务上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: Video-LLMs在时间定位上经常关注无关帧，这尤其损害需要精确时间定位的体育教练任务。获取帧级监督标注既昂贵又不可靠，因此需要不依赖额外标注的改进方法。

Method: 利用相关任务（生成和验证）必须关注相同关键帧的观察，通过在紧密相关任务的选择性视觉注意力图上施加自一致性目标，来改善时间定位而不需要额外标注。

Result: 使用VidDiffBench验证注意力分配不当是主要瓶颈。在三个体育教练任务（Exact、FitnessQA、ExpertAF）上，相比监督微调，获得了+3.0%、+14.1%准确率和+0.9 BERTScore的提升，甚至超过闭源模型。

Conclusion: 通过自一致性目标可以显著改善Video-LLMs的时间定位能力，无需额外标注，在体育教练等需要精确时间定位的任务上表现优异。
...
TL;DR: 开发了一个基于T2加权MRI的前列腺癌自动检测可解释框架，使用小数据集(162张图像)，通过迁移学习和数据增强解决数据稀缺问题，在仅使用T2加权图像的情况下取得了有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 前列腺癌是男性主要死亡原因之一，但T2加权前列腺MRI的解读因病灶细微且异质而具有挑战性。当前最先进方法通常需要双参数MRI(T2+DWI)和大规模队列，增加了采集复杂性和计算成本。

Method: 使用小数据集(162张T2加权图像，102例癌症，60例正常)，通过迁移学习和数据增强解决数据稀缺。全面比较了Vision Transformers(ViT, Swin)、CNNs(ResNet18)和传统方法(逻辑回归、SVM、HOG+SVM)。

Result: 迁移学习的ResNet18获得最佳性能(90.9%准确率，95.2%敏感性，AUC 0.905)，仅需11M参数。HOG+SVM也取得了可比性能(AUC 0.917)。在22例的读者研究中，5名放射科医生平均敏感性为67.5%(Fleiss Kappa=0.524)，而AI模型为95.2%。

Conclusion: 提出的方法仅使用T2加权图像就能实现有竞争力的性能，减少了采集复杂性和计算成本。AI辅助筛查有望减少漏诊癌症并提高一致性。Vision Transformers在小数据集上表现不如预期，而传统手工特征方法仍然有效。代码和数据已公开。
...
TL;DR: MedQ-UNI：首个统一医疗图像质量评估与恢复的视觉-语言模型，采用"先评估后恢复"范式，无需任务特定适应即可在多种模态和退化类型上实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有医疗图像恢复方法通常是模态特定或退化特定的，无法泛化到临床实践中遇到的各种退化类型。这一限制源于医疗图像恢复与医疗图像质量评估的分离，没有明确质量理解的恢复模型难以适应跨模态的多种退化类型。

Method: 提出MedQ-UNI统一视觉-语言模型，采用"先评估后恢复"范式。使用多模态自回归双专家架构（共享注意力）：1）质量评估专家通过结构化自然语言描述识别退化问题；2）恢复专家基于这些描述进行针对性图像恢复。构建包含约50K配对样本的大规模数据集，涵盖三种成像模态和五种恢复任务，每个样本都带有结构化质量描述，用于联合训练。

Result: 实验表明，单个MedQ-UNI模型无需任务特定适应，在所有任务上实现了最先进的恢复性能，同时生成更优的描述。验证了明确的质量理解能够显著提升恢复保真度和可解释性。

Conclusion: MedQ-UNI成功将医疗图像质量评估与恢复统一起来，证明了"先评估后恢复"范式的有效性。通过结构化质量描述显式引导恢复过程，不仅提升了恢复性能，还增强了模型的可解释性，为解决临床实践中医疗图像恢复的异质性问题提供了有效方案。
...
TL;DR: ColourCrafter是一个统一的扩散框架，通过将颜色编辑从全局色调转移转变为结构化、区域感知的生成过程，实现精细化的图像颜色编辑。


<details>
  <summary>Details</summary>
Motivation: 颜色是图像生成中最容易感知但最难控制的属性之一。现有的文本驱动方法依赖于离散的语言描述，无法准确表示连续的色谱变化，导致颜色编辑结果经常偏离预期色调，尤其是在精细和局部编辑时。

Method: 提出了ColourCrafter框架：1) 在潜在空间中对RGB颜色标记和图像标记进行标记级融合；2) 选择性将颜色信息传播到语义相关区域，同时保持结构保真度；3) 使用感知Lab空间损失，通过解耦亮度和色度并在掩码区域内约束编辑来增强像素级精度；4) 构建了ColourfulSet大规模高质量图像对数据集。

Result: 大量实验表明，ColourCrafter在精细颜色编辑中实现了最先进的颜色准确性、可控性和感知保真度。

Conclusion: ColourCrafter通过结构化的区域感知生成过程解决了颜色编辑中的控制问题，显著提升了扩散模型中颜色修改的精度和可控性。
...
TL;DR: 通过实验评估视觉-语言模型(VLMs)从第一人称射击游戏视频中推断玩家参与度的能力，发现当前VLM在理解人类心理状态方面存在局限性


<details>
  <summary>Details</summary>
Motivation: 从游戏视频中推断人类参与度对游戏设计和玩家体验研究很重要，但尚不清楚视觉-语言模型是否能仅从视觉线索推断这种潜在心理状态

Method: 使用GameVibe Few-Shot数据集，在九款第一人称射击游戏中评估三个VLM，采用六种提示策略：零样本预测、基于Flow理论、GameFlow理论、自我决定理论、MDA框架的理论指导提示，以及检索增强提示；同时考虑逐点参与度预测和连续窗口间参与度变化的成对预测

Result: 零样本VLM预测普遍较弱，常无法胜过简单的每游戏多数类基线；内存或检索增强提示在某些设置下改进逐点预测，但成对预测在所有策略中都保持困难；理论指导提示单独使用不可靠，可能强化表面捷径

Conclusion: 当前VLM存在感知-理解差距：虽然能识别可见的游戏玩法线索，但仍难以稳健地跨游戏推断人类参与度
...
TL;DR: 本文提出了一种名为T-QPM（时态四重模式匹配）的新框架，用于增强动态环境下多模态OOD检测和协变量分布偏移的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管基于CLIP等视觉语言模型的双模式匹配（DPM）方法支持多模态OOD检测，但现有方法存在两个主要缺陷：1）依赖固定的融合规则并假设静态环境，无法应对时序漂移；2）对协变量偏移输入缺乏鲁棒性。

Method: 采用两步框架：首先将OOD图像与文本描述配对，引入ID和OOD信号间的跨模态一致性模式，通过联合图像-文本推理细化决策边界；其次通过轻量级融合权重学习，优化语义匹配和视觉典型性的组合，并基于平均阈值置信度（ATC）进行显式正则化以确保稳定性。

Result: 在时序分割基准测试上的实验表明，该方法显著优于静态基线方法。

Conclusion: 提出的T-QPM框架为动态环境下的多模态OOD检测提供了一个鲁棒且时序一致的解决方案。
...
TL;DR: TexEditor是一个专注于文本引导纹理编辑的模型，通过构建高质量合成数据集TexBlender和引入基于强化学习的结构保持方法StructureNFT，有效解决现有模型在纹理编辑中结构保持不足的问题，并在多个基准测试中优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有最先进的编辑模型在进行纹理编辑时经常难以保持结构一致性，尽管编辑意图仅涉及外观变化。这一观察促使研究者从数据和训练两个角度联合提升结构保持能力。

Method: 1. 构建TexBlender：使用Blender生成的高质量SFT数据集，为冷启动提供强结构先验；2. 提出StructureNFT：基于强化学习的方法，整合结构保持损失，将SFT阶段学到的结构先验迁移到真实场景；3. 建立TexBench：面向真实场景的通用纹理编辑基准测试。

Result: 在现有Blender-based纹理基准测试和新构建的TexBench上，TexEditor持续优于Nano Banana Pro等强基线。在通用基准测试ImgEdit上的评估也验证了其泛化能力。

Conclusion: 通过TexBlender数据集提供的强结构先验和StructureNFT方法的结构保持机制，TexEditor能够有效进行文本引导的纹理编辑，同时保持几何结构一致性。这项工作为纹理编辑任务提供了新的数据集、训练方法和评估基准。
...
TL;DR: FILT3R提出了一种无需训练的潜在滤波层，将循环状态更新建模为令牌空间中的随机状态估计，通过自适应卡尔曼增益平衡记忆保留与新观测，提高了流式3D重建的长期稳定性。


<details>
  <summary>Details</summary>
Motivation: 流式3D重建中状态更新规则存在关键失败模式：激进覆盖会忘记有用历史，保守更新无法跟踪新证据，两种行为在超出训练范围时都会变得不稳定。

Method: 提出FILT3R框架，维护每个令牌的方差并计算卡尔曼风格增益，在线估计过程噪声（基于候选令牌的EMA归一化时间漂移），实现自适应平衡记忆保留与新观测的更新规则。

Result: FILT3R产生可解释的即插即用更新规则，将常见覆盖和门控策略作为特例，在稳定区域增益收缩（不确定性随证据积累减小），在场景真实变化时增益上升，相比现有方法在深度、姿态和3D重建方面提高了长期稳定性。

Conclusion: FILT3R通过将循环状态更新重新定义为令牌空间中的随机状态估计，提供了一种无需训练的、可解释的更新机制，能够自适应地平衡记忆保留与新观测，显著改善了流式3D重建系统的长期稳定性和泛化能力。
...
TL;DR: Diff-SIT是一个用于超低码率视频压缩的高效视频扩散模型，它通过稀疏信息传输和单步视频扩散技术，在保持感知质量和时间一致性的同时显著节省码率。


<details>
  <summary>Details</summary>
Motivation: 传统的端到端压缩模型在超低码率下会产生模糊的图像质量差，现有生成式压缩方法通常独立处理视频帧，存在时间相干性和效率的限制。需要一种能够在超低码率下保持良好感知质量和时间一致性的视频压缩方法。

Method: 提出Diff-SIT模型，包含两个核心组件：1）稀疏时间编码模块（STEM）-稀疏编码原始帧序列为信息丰富的中间序列以节省码率；2）带帧类型嵌入器的单步视频扩散（ODFTE）-处理中间序列并利用时间相关性，其中FTE指导扩散模型根据不同帧类型进行自适应重建优化整体质量。

Result: 在多个数据集上的实验表明，Diff-SIT在感知质量和时间一致性方面建立了新的最先进水平，特别是在具有挑战性的超低码率条件下。

Conclusion: Diff-SIT通过结合稀疏时间编码和高效的视频扩散方法，有效解决了超低码率视频压缩中的感知质量差和时间不一致性问题，为超低码率视频压缩提供了一种创新解决方案。
...
TL;DR: HOMEY是一个结合YOLO与领域特定掩码机制和自定义损失函数的新型检测框架，用于自动检测17种与财产风险相关的类别，在真实世界房产图像上实现了优于基线YOLO模型的检测准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 自动化财产风险检测是计算机视觉中一个高影响力但尚未充分探索的前沿领域，对房地产、承保和保险运营具有直接影响。当前需要能够准确检测各种财产风险类别的可靠方法。

Method: HOMEY（启发式对象掩码增强YOLO）结合了YOLO检测器、领域特定掩码机制和自定义损失函数。主要创新包括：1）启发式对象掩码机制，用于增强杂乱背景中的弱信号；2）风险感知损失校准，平衡类别偏斜和严重性权重；3）训练检测17种风险相关财产类别。

Result: 在真实世界财产图像上的实验表明，HOMEY相比基线YOLO模型实现了更优的检测准确性和可靠性，同时保持了快速推理速度。

Conclusion: HOMEY不仅提供准确的检测，还实现了可解释和成本效益的风险分析，为可扩展的AI驱动财产保险工作流程奠定了基础。
...
TL;DR: 这篇综述论文系统分析了AI在蛋白质科学中的范式转变，从静态结构预测发展到动态构象集成建模和复杂生物分子相互作用建模，涵盖了五个关键维度，并指出了当前瓶颈和未来方向。


<details>
  <summary>Details</summary>
Motivation: 人工智能已经从根本上改变了蛋白质折叠问题，从静态结构预测发展到动态构象集成和复杂生物分子相互作用的建模。需要系统地审视AI驱动蛋白质科学的范式转变，识别当前进展、瓶颈和未来方向。

Method: 论文从五个相互关联的维度进行系统分析：统一的多模态表示（整合序列、几何和文本知识）；通过MSA自由架构和全原子复合物建模改进静态预测；生成框架（包括扩散模型和流匹配）捕捉热力学一致构象分布；异质相互作用预测（蛋白质-配体、蛋白质-核酸、蛋白质-蛋白质复合物）；功能推断（适应性景观、突变效应和文本指导属性预测）。

Result: 论文揭示了AI在蛋白质科学中的全面转变：从分析工具发展为通用模拟器，能够理解甚至重写生命的动态语言。具体进展包括更准确的复合物建模、构象分布生成、以及多尺度生物分子相互作用的预测能力。

Conclusion: AI正在从结构分析工具转变为能够理解和重写生命动态语言的通用模拟器。未来方向包括物理一致的生成模型、多模态基础架构和实验闭环系统。关键挑战包括数据分布偏差、有限的机制可解释性以及几何度量与生物物理现实之间的脱节。
...
TL;DR: 该手册提出了一个用于汽车保险的垂直整合人工智能范式，统一了感知、多模态推理和生产基础设施，形成一个用于风险评估和理赔处理的智能栈。


<details>
  <summary>Details</summary>
Motivation: 在泰国全国汽车保险系统中，需要将现代人工智能技术转化为可靠、生产级的系统，以应对高风险的工业环境挑战。传统方法缺乏统一的架构来处理车辆损伤分析、理赔评估和承保工作流程。

Method: 开发了领域适应的Transformer架构，用于结构化视觉理解、关系车辆表示学习和多模态文档智能。构建了一个可扩展的流水线，在实践约束下运行，并强调学习算法与MLOps实践的协同演化。

Result: 实现了车辆损伤分析、理赔评估和承保工作流程的端到端自动化。建立了一个原理性的框架，用于在高风险工业环境中将现代人工智能转化为可靠的生产级系统。

Conclusion: 该手册提供了一个系统性的方法，用于构建垂直整合的人工智能系统，将感知、推理和生产基础设施统一起来，为汽车保险行业的实际部署奠定了坚实基础，展示了将学术研究转化为工业应用的可行路径。
...
TL;DR: 在线PG：一个基于3D高斯溅射的在线全景映射系统，结合几何重建和开放词汇感知，用于实时场景理解


<details>
  <summary>Details</summary>
Motivation: 现有的开放词汇场景理解方法主要限于离线处理或缺乏实例级理解，难以应用于需要实时交互的真实世界机器人任务。需要一种能够在在线设置中同时实现几何重建和开放词汇感知的系统。

Method: 提出OnlinePG系统，采用从局部到全局的高效滑动窗口范式。首先构建局部一致性地图：通过3D分割聚类图结合几何和语义线索，将滑动窗口内的不一致片段融合为完整实例。然后更新全局地图：为局部3D高斯地图构建具有空间属性的显式网格，通过鲁棒的双向二分3D高斯实例匹配将其融合到全局地图中。最后利用3D空间属性网格中融合的VLM特征实现开放词汇场景理解。

Result: 在广泛使用的数据集上进行的大量实验表明，该方法在在线方法中取得了更好的性能，同时保持了实时效率。

Conclusion: OnlinePG是一个新颖有效的系统，成功地将3D高斯溅射集成到在线设置中，实现了结合几何重建和开放词汇感知的在线全景映射，为具身应用提供了实用的场景理解能力。
...
TL;DR: 大型视觉语言模型在计数任务中展现出类似人类的计数行为：对小数量精确计数，对大数量噪声估计。通过解释性方法揭示了跨任务共享的"计数电路"，并通过轻量级干预策略显著提升了计数和一般视觉推理性能。


<details>
  <summary>Details</summary>
Motivation: 计数作为评估大型视觉语言模型推理能力的简单而强大的测试，要求模型识别每个个体对象并进行累加。研究者希望了解LVLMs如何实现计数，并探索通过增强计数能力来提升整体视觉推理性能的可能性。

Method: 使用合成和真实世界基准测试进行控制研究，结合机制分析。引入两种新颖的解释性方法：视觉激活补丁和HeadLens，用于揭示"计数电路"。提出轻量级干预策略，利用丰富的合成图像对预训练的LVLMs进行专门的计数微调。

Result: LVLMs显示出类似人类的计数行为，小数量计数精确，大数量则产生噪声估计。发现了跨多种视觉推理任务共享的结构化"计数电路"。窄范围的计数微调不仅提升了分布内合成数据的计数准确率，还使分布外计数基准平均提升+8.36%，Qwen2.5-VL在复杂的一般视觉推理任务上平均获得+1.54%的提升。

Conclusion: 计数在视觉推理中扮演核心且具有影响力的角色，通过有针对性地增强计数机制，可能成为提升整体视觉推理能力的有效途径。研究揭示了LVLMs的计数工作机制，并展示了轻量级干预策略的有效性。
...
TL;DR: 提出3DreamBooth框架，通过3DreamBooth和3Dapter组件实现3D感知视频定制，解决了现有2D方法在3D物体定制中缺乏空间先验的问题。


<details>
  <summary>Details</summary>
Motivation: 现有主题驱动视频生成方法主要将主题视为2D实体，缺乏对真实3D物体的空间几何重建能力。在合成新视角时，这些方法只能生成合理但任意的未见区域细节，而非保持真实的3D身份。同时，多视角视频数据稀缺，有限序列的微调容易导致时间过拟合。

Method: 框架包含两个组件：1) 3DreamBooth：通过单帧优化范式将空间几何与时间运动解耦，仅更新空间表示，将3D先验融入模型；2) 3Dapter：视觉条件模块，通过不对称条件策略与主要生成分支进行多视角联合优化，作为动态选择性路由器查询视角特定的几何提示。

Result: 该方法能够在不需要大量视频训练的情况下，为定制主题生成动态、视角一致的视频，并保持3D几何的一致性。

Conclusion: 提出了一种创新的3D感知视频定制框架，通过空间-时间解耦和视觉条件模块，解决了现有2D方法在3D物体定制中的根本限制，为虚拟现实、增强现实等应用提供了有效的解决方案。
...
TL;DR: 提出一种受人类中央凹视觉系统启发的跨域少样本目标检测方法，通过中心-外周注意力精炼框架解决目标域"散光"问题


<details>
  <summary>Details</summary>
Motivation: 跨域少样本目标检测面临严重的域偏移和标注数据稀缺问题。研究发现了一个被忽视的现象：模型在目标域中表现出分散、不集中的注意力模式（称为目标域散光问题），导致定位不精确和冗余预测

Method: 提出了一个受生物学启发的中心-外周注意力精炼框架：1) 正模式精炼模块：使用类特定原型重塑注意力朝向语义对象；2) 负上下文调制模块：通过建模背景上下文增强边界判别力；3) 文本语义对齐模块：通过跨模态线索加强中心-外周区分

Result: 在六个具有挑战性的CD-FSOD基准测试上一致证明了检测准确率的提升，并建立了新的最先进结果

Conclusion: 受生物启发的中心-外周注意力精炼方法能够将散光的注意力转化为集中的模式，显著改善对目标域的适应能力
...
TL;DR: CoDA框架通过临床相关数据分布偏移构建评估医学视觉语言模型的鲁棒性，发现多级组合比单级损坏更具破坏性，并提出教师引导的token空间自适应修复策略提升部署鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型在临床工作流中的可靠性尚未充分探索，现有鲁棒性评估通常假设干净输入或研究孤立损坏，忽略了临床可读性保留但会改变图像统计特性的常规采集、重建、显示和传输操作。

Method: 提出CoDA（链式分布）框架，通过组合类似采集的阴影、重建和显示重映射、传输和导出退化来构建临床合理的数据偏移。在掩码结构相似性约束下，联合优化阶段组合和参数以诱导失败同时保持视觉合理性。使用教师引导的token空间自适应与补丁级对齐进行后修复。

Result: CoDA显著降低了CLIP风格医学视觉语言模型的零样本性能，链式组合比任何单阶段更具破坏性。专有多模态模型的审计可靠性下降，医学专用MLLMs在医学图像质量审计方面存在明显缺陷。教师引导的自适应修复策略提升了在CoDA偏移样本上的准确性。

Conclusion: 研究揭示了医学视觉语言模型部署中临床基础的威胁面，表明轻量级对齐可以改善部署鲁棒性，为评估和改进医疗AI系统的临床适应性提供了重要见解。
...
### [38] [HiMu: Hierarchical Multimodal Frame Selection for Long Video Question Answering](https://arxiv.org/abs/2603.18558)
*Dan Ben-Ami,Gabriele Serussi,Kobi Cohen,Chaim Baskin*

Main category: cs.CV

TL;DR: HiMu是一个训练免费的长视频问答框架，使用文本LLM将查询分解为层次逻辑树，通过轻量级专家处理视觉和音频信号，结合模糊逻辑算子产生连续满意度曲线，在效率和准确性之间取得良好平衡。


<details>
  <summary>Details</summary>
Motivation: 长视频问答需要对长时间上下文进行推理，但大型视觉语言模型受限于有限的上下文窗口。现有方法面临尖锐的权衡：基于相似性的选择器快速但将组合查询压缩为单个密集向量，丢失子事件排序和跨模态绑定；基于智能体的方法通过迭代LVLM推理恢复这种结构，但成本过高。

Method: 1. 使用单个文本LLM调用将查询分解为层次逻辑树，叶子节点是原子谓词；2. 每个谓词路由到轻量级专家（视觉：CLIP、开放词汇检测、OCR；音频：ASR、CLAP）；3. 对生成信号进行归一化和时间平滑以对齐不同模态；4. 通过模糊逻辑算子自底向上组合，强制执行时间顺序和邻接性；5. 产生连续满意度曲线用于帧选择。

Result: 在Video-MME、LongVideoBench和HERBench-Lite上的评估表明，HiMu提升了效率-准确性的帕累托前沿：使用Qwen3-VL 8B在16帧时优于所有竞争选择器，使用GPT-4o时超越了在32-512帧上操作的智能体系统，同时需要大约10倍更少的FLOPs。

Conclusion: HiMu框架有效地桥接了现有方法的差距，提供了一种训练免费的解决方案，能够在保持高效率的同时处理长视频中的复杂组合查询，通过层次逻辑分解和轻量级专家路由实现了在有限计算资源下的高性能长视频问答。
...
TL;DR: 本文提出了一种跨层注意力传播方法，通过保留和整合历史注意力矩阵来增强Vision Transformers中的层间信息流，在保持最小架构改动的同时显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers的注意力机制在各层独立运作，限制了信息流动和特征学习。现有方法缺乏层间注意力信息的有效传播，导致模型无法充分利用历史注意力模式进行渐进式优化。

Method: 提出了一种跨层注意力传播方法，通过存储和混合历史注意力矩阵来改进Vision Transformers的层间信息流。该方法仅需添加注意力矩阵存储和混合操作，最小化架构改动。采用随机初始化策略，并确定了最优混合超参数α=0.45。

Result: 在CIFAR-100上ViT准确率从75.74%提升至77.07%（+1.33%），在TinyImageNet上从57.82%提升至59.07%（+1.25%）。跨架构验证显示CaiT也获得1.01%的提升。随机初始化优于零初始化，α=0.45为最优平衡参数。

Conclusion: 跨层注意力传播方法有效增强了Vision Transformers的信息流动和特征学习能力，通过简单的架构修改实现了显著的性能提升。该方法在不同Transformer变体上具有普适性，为视觉Transformer的层间交互优化提供了有效解决方案。
...
TL;DR: 提出一种基于饱和度-明度相似度的新颖非局部变分技术用于彩色图像恢复，通过在饱和度-明度通道考虑图像块相似度，提高了传统非局部方法的色彩描述能力。


<details>
  <summary>Details</summary>
Motivation: 传统非局部方法直接从彩色图像的RGB通道提取图像块，由于相似度主要基于独立通道的灰度值，无法精细描述色彩信息。需要一种能够更好考虑色彩相似度的方法。

Method: 1. 建立基于饱和度-明度相似度的非局部全变分，将彩色图像块的饱和度-明度相似度纳入非局部梯度
2. 基于此提出非局部变分模型
3. 采用布雷格曼算子分裂法设计高效算法求解优化问题
4. 研究算法的收敛性

Result: 数值实验表明，所提模型在视觉质量和定量指标（PSNR、SSIM、QSSIM、S-CIELAB颜色误差）方面均优于其他测试方法。

Conclusion: 提出的基于饱和度-明度相似度的非局部变分方法能有效提高彩色图像恢复质量，特别是在色彩保持和结构相似度方面表现优异，为彩色图像处理提供了新的技术途径。
...
TL;DR: 提出一种基于自然语言描述动作单元(AUs)的面部行为合成方法，解决传统AU线性组合在处理冲突AUs时的局限性，通过新的数据集和模型实现更真实的面部表情生成。


<details>
  <summary>Details</summary>
Motivation: 当前基于动作单元(AUs)的面部行为合成方法通常将AUs编码为独热向量，将复合表情建模为个体AUs的简单线性组合。这种线性模型在处理冲突AUs（激活同一面部肌肉但作用相反的AUs）时会产生解剖学上不合理的人造伪影和不自然的运动叠加。文本到面部模型虽然取得进展，但依赖粗略的情感类别，缺乏捕捉人类非语言交流细微差异的能力。

Method: 提出两种主要贡献：1) 引入BP4D-AUText，首个用于复杂面部行为的大规模文本-图像配对数据集，通过对BP4D和BP4D+数据集应用基于规则的动态AU文本处理器合成；2) 提出VQ-AUFace生成模型，利用面部结构先验从文本合成真实且多样的面部行为。该方法通过自然语言描述AUs来表示面部行为，保留AU框架的表达能力，同时支持复杂和冲突AUs的显式建模。

Result: 广泛的定量实验和用户研究表明，该方法显著优于现有方法。在生成解剖学上合理、行为丰富且感知上令人信服的面部表情方面表现出色，特别是在涉及冲突AUs的挑战性条件下。

Conclusion: 通过自然语言描述AUs的面部行为表示方法，结合新数据集和生成模型，成功解决了传统AU线性组合在处理冲突AUs时的局限性，为高保真面部合成开辟了新方向，并解锁了现代文本到图像模型的潜力。
...
TL;DR: 本文提出了基于文本引导注意力的方法来增强CLIP模型的对抗鲁棒性。首先提出TGA-ZSR框架，包括局部注意力精炼模块和全局注意力约束模块，然后进一步改进为Comp-TGA方法，通过互补的前景注意力机制提升零样本鲁棒性。


<details>
  <summary>Details</summary>
Motivation: CLIP等预训练视觉语言模型虽然具有出色的零样本能力，但被发现容易受到对抗样本的攻击。实验观察到对抗扰动会导致文本引导注意力偏移，因此需要在不损害模型泛化能力的同时增强其对抗鲁棒性。

Method: 提出了两个方法：1) TGA-ZSR框架，包含局部注意力精炼模块和全局注意力约束模块；2) Comp-TGA方法，通过类提示引导的注意力和非类提示驱动的反向注意力的互补机制，获得更全面准确的前景表示。

Result: 实验验证表明，TGA-ZSR和Comp-TGA分别在16个数据集上比现有最优技术提升了9.58%和11.95%的零样本鲁棒准确率。

Conclusion: 文本引导注意力机制能有效提升CLIP模型的对抗鲁棒性，特别是互补注意力机制通过整合正向和反向文本引导注意力，能更全面地捕获前景特征，显著提高模型在零样本设置下的鲁棒性。
...
TL;DR: SJD-PAC是一个增强版的Speculative Jacobi Decoding框架，通过主动起草策略和自适应继续机制，在文本到图像合成中实现了3.8倍的速度提升，同时保持无损图像质量。


<details>
  <summary>Details</summary>
Motivation: Speculative Jacobi Decoding (SJD) 虽然提供了无需草稿模型的加速方法，但在视觉生成的高熵特性下，复杂区域的草稿标记接受率较低，这成为严重限制整体吞吐量的瓶颈。

Method: SJD-PAC包含两个关键技术：1) 主动起草策略，用于提高高熵挑战区域的局部接受率；2) 自适应继续机制，在初始拒绝后维持序列验证，避免完全重新采样。

Result: 实验表明，在标准文本到图像基准测试中，SJD-PAC实现了3.8倍的加速，同时严格保持了目标分布，实现了无损图像质量。

Conclusion: SJD-PAC通过其主动起草和自适应继续机制的协同工作，显著提高了每个步骤的平均接受长度，从而在保持分布完整性的同时大幅提升了推理速度。
...
TL;DR: 提出Cross-Modal Context Learning (CCL)方法，通过改进的双流transformer架构解决音频-视频联合生成中的跨模态对齐、注意力偏见和训练推理不一致问题，以更少资源实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 当前基于双流transformer的音频-视频联合生成方法存在多个问题：1）门控机制导致的模型流形变化；2）跨模态注意力引入的多模态背景区域偏见；3）训练与推理中多模态无分类器引导的不一致性；4）多条件之间的冲突

Method: 提出Cross-Modal Context Learning (CCL)框架，包含：1）Temporally Aligned RoPE and Partitioning (TARP)增强音频和视频潜在表示的时间对齐；2）Cross-Modal Context Attention (CCA)模块，含Learnable Context Tokens (LCT)和Dynamic Context Routing (DCR)提供稳定无条件锚点；3）推理时的Unconditional Context Guidance (UCG)利用LCT支持不同形式的无分类器引导

Result: 通过综合评估，CCL相比近期学术方法实现了最先进的性能，同时显著减少了所需资源

Conclusion: CCL方法有效解决了音频-视频联合生成中的关键问题，通过精心设计的模块提升了跨模态对齐、收敛速度和生成质量，在训练-推理一致性和多条件协调方面表现优异
...
TL;DR: 对三个混合transformer模型（UNETR、SwinUNETR、UNETR++）与CNN基准模型SegResNet在腹部CT多器官分割任务上进行系统基准测试，发现在异构数据集上CNN模型仍具有竞争力且表现最佳。


<details>
  <summary>Details</summary>
Motivation: 准确的多器官分割对计算机辅助诊断和治疗至关重要。虽然CNN一直是医学图像分割的标准方法，但基于transformer的架构因其建模长距离依赖的能力而受到关注。需要系统比较这两种架构在异构数据集上的表现。

Method: 使用RATIC数据集（来自23个机构的206个标注CT扫描，涵盖5个腹部器官）系统基准测试三个混合transformer模型（UNETR、SwinUNETR、UNETR++）与CNN基线模型SegResNet。所有模型在相同的预处理和训练条件下训练和评估，以Dice相似系数（DSC）为主要评价指标。

Result: CNN-based SegResNet在所有器官上均取得最高整体性能，优于所有混合transformer模型。在transformer-based方法中，UNETR++表现最具竞争力，而UNETR在较少的训练迭代中显示出更快的收敛速度。

Conclusion: 对于中小型异构数据集，经过良好优化的CNN架构仍然具有很强竞争力，甚至可能优于混合transformer设计。这一发现对医学图像分割领域选择合适的架构提供了重要参考。
...
TL;DR: OpenT2M：一个百万级高质量开源运动数据集，以及基于该数据集的MonoFrill预训练运动模型，显著提升了文本到动作生成的泛化能力和零样本性能


<details>
  <summary>Details</summary>
Motivation: 当前文本到动作（T2M）生成模型在未见过的文本描述上表现不佳，主要是因为现有运动数据集规模小、多样性有限。这限制了T2M模型在实际应用中的泛化能力。

Method: 1. 构建OpenT2M数据集：包含超过2800小时人类运动，通过物理可行性验证和多粒度筛选确保质量，提供详细秒级文本标注；2. 开发自动化流程创建长时段序列；3. 基于OpenT2M开发MonoFrill预训练模型，核心是2D-PRQ运动标记器，通过将人体划分为生物部分来捕捉时空依赖性。

Result: OpenT2M显著提升了现有T2M模型的泛化能力，2D-PRQ实现了优越的重建效果和强大的零样本性能。实验表明该方法有效解决了长期存在的数据质量和基准测试挑战。

Conclusion: OpenT2M和MonoFrill通过提供高质量的大规模数据集和高效的运动标记方法，有望推动T2M领域的发展，解决该领域长期存在的数据稀缺和泛化能力不足的问题。
...
TL;DR: GenVideoLens是一个用于AI生成视频检测的细粒度基准，包含400个AI生成视频和100个真实视频，通过15个维度评估大型视觉语言模型，揭示了维度间的不平衡和关键能力差距。


<details>
  <summary>Details</summary>
Motivation: 现有的AI生成视频检测评估大多将其视为二分类问题，使用整体准确率等粗粒度指标，无法深入理解大型视觉语言模型在哪些方面成功或失败，限制了诊断性洞察。

Method: 1) 创建包含400个AI生成视频和100个真实视频的数据集，由专家在15个真实性维度上标注（涵盖感知、光学、物理和时序线索）；2) 评估11个代表性大型视觉语言模型；3) 进行时序扰动实验分析模型对时间信息的利用。

Result: 研究发现明显的维度不平衡：LVLMs在感知线索上表现较好，但在光学一致性、物理交互和时序因果推理方面表现不佳；模型间存在显著性能差异，较小的开源模型有时在特定真实性线索上优于更强的专有模型；时序扰动实验显示当前LVLMs对时间信息的利用有限。

Conclusion: GenVideoLens为AI生成视频检测提供了诊断性洞察，揭示了大型视觉语言模型的关键能力差距，并为改进未来的检测系统提供了指导方向。
...
TL;DR: SVOO是一种无需训练的双阶段稀疏注意力框架，通过离线层敏感度分析和在线双向共聚类实现视频生成的高效稀疏注意力，解决了现有方法忽略层异质性和查询-键耦合的问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散变换器（DiTs）在视频生成中因密集3D注意力导致高推理成本，而现有的无需训练稀疏注意力方法存在两个未解决的限制：1) 忽略注意力剪枝中的层异质性；2) 忽略块划分中的查询-键耦合，这阻碍了更好的质量-加速权衡。

Method: SVOO采用两阶段范式：1) 离线层敏感度分析 - 推导每层的内在剪枝级别；2) 在线块稀疏注意力 - 通过新颖的双向共聚类算法实现块级稀疏注意力。

Result: 在七个广泛使用的视频生成模型上的实验表明，SVOO在质量-加速权衡方面优于最先进方法，提供高达1.93倍的加速，同时在Wan2.1上保持高达29 dB的PSNR。

Conclusion: 该工作揭示了注意力稀疏性是每层的内在属性，跨输入影响较小，提出的SVOO框架通过解决层异质性和查询-键耦合问题，实现了视频生成中更好的效率-质量平衡。
...
TL;DR: PhysVideo是一个两阶段视频生成框架，首先生成物理感知的正交前景视频，然后合成带有背景的完整视频，显著提升了物理真实性和时空一致性。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成方法虽然在视觉保真度上有显著进步，但确保物理一致的运动仍然是一个根本挑战。这是因为真实世界的物体运动发生在三维空间，而视频观测只提供了这种动态的部分、视角依赖的投影。

Method: 采用两阶段框架：1）Phys4View阶段：利用物理感知注意力捕捉物理属性对运动动态的影响，通过几何增强的跨视角注意力和时间注意力增强时空一致性；2）VideoSyn阶段：使用生成的前景视频作为指导，学习前景动态与背景上下文之间的交互以进行可控视频合成。

Result: 构建了PhysMV数据集，包含40K个场景，每个场景有四个正交视角，总计160K个视频序列。大量实验表明，PhysVideo显著优于现有视频生成方法，在物理真实性和时空一致性方面都有明显提升。

Conclusion: PhysVideo通过将物理感知建模整合到视频生成中，有效解决了现有方法中物理不一致的问题，为高质量、物理真实的视频生成提供了新思路。
...
TL;DR: MeInTime是一种基于扩散模型的人脸修复方法，能够利用跨年龄的参考图像实现身份保真和年龄一致的修复，解决了现有方法在历史照片修复等场景中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸修复方法主要假设参考图像与退化输入在年龄上对齐，但在真实场景（如历史照片修复）中往往只有跨年龄的参考图像可用。这限制了现有方法在身份保真方面的有效性。

Method: 1. 提出身份和年龄条件解耦建模；2. 训练时专注于通过新引入的注意力机制注入身份特征；3. 使用门控残差融合模块整合退化特征和身份表征；4. 推理时提出年龄感知梯度引导策略，无需训练即可将去噪潜在向量推向目标年龄语义流形。

Result: 广泛的实验表明，MeInTime在身份保持和年龄一致性方面优于现有人脸修复方法，实现了既保持身份保真又保持年龄一致的忠实修复。

Conclusion: MeInTime成功将基于参考的人脸修复从同年龄设置扩展到跨年龄设置，为解决历史照片修复等现实场景中的挑战提供了有效解决方案。
...
TL;DR: 论文提出了Click-to-Ask系统，这是一个用于直播电商的AI助手，包含离线和在线组件，能够自动处理产品信息、生成促销文案，并在直播中实时回答观众问题。


<details>
  <summary>Details</summary>
Motivation: 直播电商已成为重要的传播形式，但主播在推广产品时需要大量准备工作且难以及时回应观众提问，这影响了直播效果和用户体验。

Method: 系统采用双模块设计：离线模块处理多模态产品信息，转化为结构化数据并生成合规促销文案；在线模块在直播时基于结构化产品信息和事件级历史记忆，通过点击问题实现实时响应。

Result: 在TikTok直播帧数据集上，系统实现了0.913的问题识别准确率和0.876的响应质量评分，显著减少了促销准备时间，增强了内容参与度，并能及时回应用户询问。

Conclusion: Click-to-Ask系统通过AI驱动的方式有效提升了直播电商的效率和互动性，展示了在实际应用中的巨大潜力。
...
TL;DR: 提出一个基于合成PDF和LLM判官的表格提取评估框架，通过人类验证显示LLM评估比传统指标（TEDS、GriTS）与人类判断相关性更高，并评估了21个PDF解析器的性能差异。


<details>
  <summary>Details</summary>
Motivation: 现有的表格提取评估方法依赖基于规则的指标，这些指标无法捕捉表格内容的语义等价性。对于大规模科学数据挖掘和知识库构建，可靠地从PDF中提取表格至关重要。

Method: 1. 使用来自arXiv的表格创建具有精确LaTeX地面真实值的合成PDF基准数据集；2. 将LLM-as-a-judge方法应用于语义表格评估，集成到能适应解析器输出不一致性的匹配流程中；3. 进行了包含1,500多个质量判断的人类验证研究；4. 评估了21个当代PDF解析器在100个合成文档（含451个表格）上的性能。

Result: 1. LLM评估与人类判断的相关性（Pearson r=0.93）显著高于树编辑距离相似性（TEDS, r=0.68）和网格表格相似性（GriTS, r=0.70）；2. 评估揭示了21个PDF解析器之间的显著性能差异；3. 提供了选择表格数据提取解析器的实用指导。

Conclusion: 该研究为关键的表格式数据提取任务建立了一个可重复、可扩展的评估方法学框架，证明了LLM-based评估在捕捉语义等价性方面优于传统指标，并为解析器选择提供了实证依据。
...
TL;DR: Switch是一个用于医学超声图像分割的新型半监督学习框架，通过多尺度切换和频域切换策略解决标记数据稀缺和图像伪影问题，在六个不同数据集上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 医学超声图像分割面临两大主要挑战：1）标记数据有限；2）固有的成像伪影（包括斑点噪声和低对比度边界）。现有的半监督学习方法存在未标记数据利用不充分和缺乏鲁棒特征表示机制的问题。

Method: 提出Switch框架，包含两个核心创新：1）多尺度切换（MSS）策略：采用分层补丁混合实现均匀空间覆盖；2）频域切换（FDS）策略：在傅里叶空间中执行幅度切换，结合对比学习实现鲁棒特征表示。这些组件集成在师生架构中，有效利用标记和未标记数据。

Result: 在六个不同的超声数据集（淋巴结、乳腺病变、甲状腺结节和前列腺）上进行综合评估，在5%标记比例下取得了显著改进：LN-INT数据集80.04% Dice、DDTI数据集85.52% Dice、前列腺数据集83.48% Dice。半监督方法甚至超过了全监督基线，同时保持了参数效率（180万参数）。

Conclusion: Switch框架通过创新的多尺度切换和频域切换策略，有效解决了医学超声图像分割中的数据稀缺和伪影问题，在多个数据集上验证了其优越性能，为资源受限的医学成像应用提供了有效解决方案。
...
TL;DR: 这是一篇关于多模态计算病理学的综述论文，全面调研了该领域的最新进展，重点讨论了全切片图像处理、多模态数据整合、高效适应方法和可信诊断方法等四个研究方向，并提出了未来发展的开放挑战。


<details>
  <summary>Details</summary>
Motivation: 全切片成像技术推动了数字病理学的发展，但当前仍面临多个挑战：1）超高分辩率图像带来的计算困难；2）专家标注数据有限制约监督学习方法；3）多模态信息整合中保持生物学可解释性的难度；4）超长视觉序列建模的不透明性影响临床透明度。

Method: 该综述系统分析了四个关键研究方向：1）针对WSI的自监督表示学习和结构感知令牌压缩技术；2）多模态数据生成与增强方法；3）参数高效适应和推理增强的少样本学习；4）用于可信诊断的多智能体协作推理机制。特别关注了令牌压缩如何实现跨尺度建模，以及多智能体机制如何模拟病理学家在不同放大倍数下的"思维链"过程。

Result: 研究指出，多模态计算病理学已取得显著进展，通过令牌压缩技术能够有效处理WSI的极端分辨率，多智能体协作推理机制可模拟病理学家的诊断思维过程，实现不确定性感知的证据融合，提升诊断的可信度和透明度。

Conclusion: 未来进展依赖于统一的多元模态框架，需要整合高分辨率视觉数据与临床和生物医学知识，以支持可解释且安全的AI辅助诊断。论文强调了解决现有挑战对于推动计算病理学临床应用的重要性。
...
TL;DR: SCNP是一种改进图像分割拓扑准确性的高效方法，通过对最差分类邻域的logits进行惩罚，强制模型先改善像素邻域预测再改善像素本身预测


<details>
  <summary>Details</summary>
Motivation: 标准深度学习图像分割模型无法保证拓扑准确性，不能保持正确的连通组件数量或结构，这影响了分割质量并损害后续量化分析的可靠性。现有方法集成复杂、计算昂贵或仅限于管状结构

Method: 提出SCNP方法，通过惩罚logits与其最差分类邻域来改善拓扑准确性，强制模型先改进像素邻域预测再允许改进像素自身预测。该方法可集成到语义分割和实例分割的三种框架中

Result: 在13个数据集上验证了SCNP的有效性，覆盖不同结构形态和图像模态。证明可以集成到多个损失函数中提升拓扑准确性

Conclusion: SCNP是一种高效的方法，能显著改善图像分割的拓扑准确性，适用于多种结构和模态，易于集成到现有训练流程中
...
TL;DR: 本文提出了一种针对视频扩散变换器的推理时混合精度量化框架（NVFP4/INT8）和时间增量缓存技术，显著降低了内存使用和计算成本，实现了1.92倍端到端加速和3.32倍内存减少。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器在视频生成方面表现出色，但实际部署受到高内存使用和计算成本严重制约。现有的量化方法通常采用静态位宽分配，忽视了不同扩散时间步之间激活量化的难度，导致效率与质量之间的权衡不够优化。

Method: 1. 提出推理时NVFP4/INT8混合精度量化框架：发现模块输入输出差异与其内部线性层量化敏感性之间存在强线性相关性，设计轻量级预测器动态分配NVFP4给时间稳定的层，同时选择性保留INT8给易变层。
2. 引入时间增量缓存：观察到Transformer块的输入输出残差在不同时间步之间具有高度时间一致性，利用这种时间冗余跳过不变块的计算。

Result: 方法实现了1.92倍端到端加速和3.32倍内存减少，在视频DiTs的高效推理方面建立了新的基准。

Conclusion: 提出的自适应精度策略和TDC技术有效解决了视频扩散变换器的内存和计算瓶颈，实现了高效的视频生成推理，为相关领域的实际部署提供了实用解决方案。
...
TL;DR: WeNLEX是一个弱监督模型，用于为多标签胸部X光分类生成自然语言解释，通过特征空间图像匹配确保忠实性，通过小规模临床标注解释数据库保持合理性。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用标注解释的数据集显式监督解释生成过程，生成的解释虽然合理但不忠实于模型的推理过程。需要一种既能生成可信解释又能忠实反映模型推理的方法。

Method: 提出WeNLEX弱监督模型：1）通过从自然语言解释生成的图像与原始图像在黑盒模型特征空间中的匹配确保忠实性；2）通过与少量临床标注解释数据库的分布对齐保持合理性；3）可在后置和内置两种设置下运行；4）通过更改数据库可适应不同目标受众。

Result: 1）仅使用每个诊断5个真实解释就能产生忠实合理的解释；2）在多指标验证中表现优异；3）内置设置下将独立分类器的AUC提高2.21%；4）展示了面向非医学用户的简化版本，显示其适应性。

Conclusion: WeNLEX能够生成既忠实又合理的自然语言解释，提高模型可解释性的同时还能提升下游任务性能，并且具有适应不同受众的灵活性。
...
TL;DR: ProCal是一种源无域自适应（SFDA）方法，通过概率校准机制动态调整邻居预测，解决现有方法过度依赖邻居预测相似性导致的源知识遗忘和局部噪声过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 现有的SFDA方法过度依赖邻居间的预测相似性，这会加速源知识遗忘并增加对局部噪声的过拟合敏感性，需要一种能够平衡知识保留和域适应的解决方案。

Method: 提出了ProCal概率校准方法，采用双模型协作预测机制：将源模型的初始预测与当前模型的在线输出相结合来校准邻居概率，同时设计了结合软监督损失和多样性损失的联合优化目标。

Result: 在四个公共数据集的31个跨域任务上进行了广泛实验验证，理论分析表明ProCal能够收敛到源知识和目标信息有效融合的平衡状态，减少了知识遗忘和过拟合。

Conclusion: ProCal通过概率校准机制有效解决了SFDA中源知识遗忘和局部噪声过拟合的问题，在保留源模型判别信息的同时实现了更好的域适应性能。
...
<details>
  <summary>Details</summary>
Motivation: 虽然基于大规模RGB数据训练的视觉几何基础模型在相机姿态估计和场景重建方面表现出色，但在处理RGB-热成像等多模态数据时效果下降。这些模型在单独处理热成像数据时表现良好，但难以对齐RGB和热成像模态。

Method: 提出SEAR微调策略，在相对较小的RGB-T数据集上对预训练的几何Transformer进行适配训练，使模型能够有效处理RGB-T多模态输入。

Result: 在3D重建和相机姿态估计任务上显著优于现有方法，在所有指标上都取得显著提升（如AUC@30超过29%），在低光照和烟雾等挑战性条件下也能可靠工作。引入了一个新的RGB-T数据集。

Conclusion: SEAR能够以可忽略的推理时间开销，将预训练的视觉几何Transformer有效适配到RGB-T多模态任务，实现模态对齐并提升性能，为多模态3D场景重建提供了实用解决方案。
...
TL;DR: 这篇论文研究了医学图像分割中不确定性量化的问题，特别关注数据相关的不确定性（AU）和模型相关的不确定性（EU）之间的相互作用和纠缠问题，通过全面的实证研究评估了不同AU-EU组合模型的表现。


<details>
  <summary>Details</summary>
Motivation: 不确定性量化在安全关键应用（如医学图像分割）中至关重要。虽然已有许多方法分别建模数据相关的aleatoric不确定性（AU）和模型相关的epistemic不确定性（EU），但将两者结合时如何相互作用尚不清楚。此外，近期研究发现AU和EU之间存在显著纠缠，影响了分解的可解释性和实际应用价值。

Method: 1. 进行了全面的实证研究，涵盖了广泛的AU-EU模型组合；2. 提出了量化不确定性纠缠的指标；3. 在下游不确定性量化任务（分布外检测、模糊性建模和校准）中评估了这些模型组合；4. 分析了不确定性纠缠的潜在来源。

Result: 1. 对于分布外检测，集成方法展现出较低的纠缠和优异的性能；2. 对于模糊性建模和校准，最佳模型依赖于具体数据集，softmax/SSN基方法表现良好，Probabilistic UNets纠缠较低；3. softmax集成在所有任务上都表现出色；4. 识别了不确定性纠缠的潜在来源。

Conclusion: AU和EU之间的纠缠问题严重影响不确定性分解的可解释性和实用性。不同不确定性量化任务的最佳模型选择有所不同，而softmax集成在所有任务中表现稳健。研究为理解不确定性纠缠提供了实证基础，并指出了减轻这种影响的研究方向。
...
TL;DR: Perceptio是一个增强感知能力的大视觉语言模型，通过引入显式的语义分割标记和深度标记来提升2D和3D空间推理能力，在多个空间理解和视觉问答基准上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在语义理解方面表现出色，但在细粒度空间定位方面存在不足，因为模型必须隐式推断复杂几何结构而无需产生空间解释。这限制了模型在需要精确空间理解的任务上的性能。

Method: 1. 从强大的单目深度估计教师模型蒸馏VQVAE深度码本，将稠密深度压缩为紧凑序列标记化；2. 在LLM中集成基于SAM2的语义分割标记和VQVAE深度标记，让模型先输出空间标记再回答问题；3. 引入新的复合深度标记目标（标记器、标记和计数损失）和软合并技术进行可微分重建；4. 采用跨多样数据集的多任务协同训练策略。

Result: 在InternVL基础上构建的Perceptio在多个基准上取得SOTA性能：RefCOCO/+/g上的指代表达分割提高了+0.8/+1.4/+1.1 cIoU，HardBLINK空间理解准确率提高10.3%，MMBench准确率提高1.0%。

Conclusion: 显式的空间思维链显著增强了大视觉语言模型中的空间定位能力，表明在自回归序列中直接生成空间感知标记是提升细粒度空间推理的有效方法。
...
TL;DR: Dual-IFM是一个具有双重可解释性的基础模型，在医学影像领域提供局部和全局可解释性，在视网膜影像任务上实现了与更大规模模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 当前基础模型虽然能从大规模无标签数据中学习可迁移表示，但大多架构缺乏可解释性，这在医学影像等高风险领域是个关键问题。

Method: 提出了Dual-IFM模型，具备双重可解释性设计：1) 通过忠实于决策过程的类别证据图提供单个图像的局部可解释性；2) 通过2D投影层实现整个数据集的可视化表示空间的全局可解释性。模型在超过80万张彩色眼底摄影图像上进行自监督学习预训练。

Result: 模型在性能上达到与参数量高达16倍的当前最优基础模型相当的水平，同时在分布外数据上提供可解释的预测。在视网膜影像任务上展现了优异的性能。

Conclusion: 大规模自监督预训练与内在可解释性结合可以产生鲁棒的视网膜影像表示，为高风险医学应用提供了可靠且可解释的解决方案。
...
TL;DR: HORNet是一个轻量级帧选择策略，使用Group Relative Policy Optimization训练，能显著减少视频问答中视觉语言模型的输入帧和处理时间，同时提高回答质量。


<details>
  <summary>Details</summary>
Motivation: 当前视频问答系统大多依赖均匀或启发式采样选择视频帧，这种方法无法针对下游回答质量进行优化。需要一种能够学习哪些帧对视觉语言模型回答问题最关键的优化方法。

Method: 提出HORNet框架，使用Group Relative Policy Optimization训练轻量级帧选择策略（少于1M可训练参数），形式化为Select Any Frames任务，将视觉输入策划与VLM推理解耦。

Result: 减少输入帧达99%，VLM处理时间减少93%；在MSVD-QA上F1提升1.7%，在NExT-QA上比均匀采样高7.3分；策略可跨VLM模型迁移而无需重新训练，与更强模型配对时获得额外8.5%相对增益。

Conclusion: 优化视觉语言模型看到的输入内容是实用且互补的替代方案，能同时提高效率。该工作表明选择性帧选择对视频问答系统的重要性，且GRPO训练的选择策略比监督学习和PPO方法具有更好的分布外泛化能力。
...
TL;DR: Motion-o是一个面向运动中心的视频理解扩展框架，用于视觉语言模型，通过显式化轨迹并使其可验证，增强空间-时间-轨迹推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频推理研究在利用时空证据链增强推理能力方面取得进展，但缺乏对"物体如何在观测间移动"的显式推理。当前方法未能通过连接连续观测来明确表达运动模式，使轨迹理解隐式且难以验证。

Method: 1. 将缺失能力形式化为空间-时间-轨迹推理；2. 引入轨迹基准数据集，通过增强稀疏关键帧监督来生成更密集的边界框轨迹；3. 提出运动思维链，使用<运动/>标签总结每个物体的方向、速度和尺度变化，显式连接观测形成轨迹；4. 设计奖励函数促使模型基于视觉证据直接推理，无需架构修改。

Result: 经验结果表明，Motion-o改进了空间-时间基础定位和轨迹预测，同时保持与现有框架的完全兼容性，确立了运动推理作为基于证据的视频理解的关键扩展。

Conclusion: 运动推理是增强视觉语言模型视频理解能力的关键维度，通过显式化轨迹连接和结构化推理路径，可以在不修改模型架构的情况下显著提升轨迹理解和预测性能。
...
TL;DR: PromptHub是一个通过局部感知融合、集中和对齐来增强多提示视觉上下文学习的框架，超越了传统的基于补丁的融合方法，在多个视觉任务上表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 当前的视觉上下文学习(VICL)中，虽然提示融合方法结合了各种演示的优势，但基于补丁的融合框架和模型无关的监督阻碍了信息线索的充分利用，限制了性能提升。

Method: 提出PromptHub框架，包含：1）利用空间先验捕获更丰富的上下文信息；2）采用互补的集中、对齐和预测目标来相互指导训练；3）结合数据增强来进一步加强监督。通过局部感知融合、集中和对齐来整体增强多提示能力。

Result: 在三个基础视觉任务上的广泛实验证明了PromptHub的优越性。验证了其在分布外设置、各种检索场景中的通用性、可迁移性和鲁棒性。

Conclusion: 该工作建立了一个可靠的局部感知范式用于提示融合，超越了先前基于补丁的方法，为视觉上下文学习提供了更有效的多提示框架。
...
TL;DR: 论文提出了MultihopSpatial基准测试，用于评估和提升视觉语言模型的多跳组合空间推理能力，包括新的评估指标和训练数据集，并展示了强化学习后训练的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有空间推理基准主要关注单跳基本关系，忽略了多跳组合推理和精确视觉定位，而这些对于现实世界中的视觉-语言-动作（VLA）智能体部署至关重要。

Method: 提出了三个主要贡献：1）用于多跳组合空间推理的综合基准MultihopSpatial；2）Acc@50IoU评估指标，同时评估推理和视觉定位；3）MultihopSpatial-Train大规模训练语料库。

Result: 对37个最先进的视觉语言模型进行广泛评估，发现组合空间推理仍然是一个重大挑战；通过强化学习在提出的语料库上进行后训练，能够同时提升内在空间推理能力和下游具身操作性能。

Conclusion: 多跳组合空间推理是视觉语言模型的重要挑战，MultihopSpatial基准、评估指标和训练资源为解决这一问题提供了系统框架，并为VLA智能体的实际部署奠定了基础。
...
TL;DR: PASTA：基于条件扩散模型的病理感知图像翻译框架，能够从MRI生成高质量的合成PET图像，用于神经退行性疾病的诊断。


<details>
  <summary>Details</summary>
Motivation: PET成像在神经退行性疾病诊断中具有重要功能信息，但成本高且有辐射暴露；MRI无这些限制但诊断敏感性较低。现有生成方法主要关注结构保留而忽视病理感知需求。

Method: 提出PASTA框架：基于条件扩散模型的病理感知图像翻译方法，采用高交互性双分支架构和多模态条件集成，引入循环交换一致性和体积生成策略以生成高质量3D PET图像。

Result: 定性定量结果显示合成的PET图像质量高且具有病理感知能力。在阿尔茨海默病诊断中，合成PET的性能比MRI提高4%，几乎达到真实PET的性能水平。

Conclusion: PASTA通过增强病理感知的跨模态图像翻译，成功实现了从MRI生成高质量的合成PET图像，弥补了两种成像技术的局限性，为神经退行性疾病诊断提供了有效解决方案。
...
TL;DR: GHOST是一个基于2D高斯泼溅的快速、类别无关的框架，用于从单目RGB视频重建动态手-物交互，实现了完整、物理一致且可动画化的重建，比现有方法快一个数量级。


<details>
  <summary>Details</summary>
Motivation: 从单目RGB视频理解真实的手-物交互对于AR/VR、机器人和具身AI至关重要。现有方法依赖类别特定模板或高计算成本，但仍产生物理不一致的手-物对齐结果。

Method: GHOST使用2D高斯泼溅表示手和物体为密集、视图一致的高斯圆盘，并引入三个关键创新：几何先验检索与一致性损失以补全遮挡区域、基于抓握的对齐以优化手平移和物体尺度、以及手感知的背景损失以避免惩罚手遮挡区域。

Result: 在ARCTIC、HO3D和真实世界数据集上的广泛实验表明，GHOST在3D重建和2D渲染质量上达到最先进的精度，比现有类别无关方法快一个数量级。

Conclusion: GHOST为真实手-物交互建模提供了一个高效且鲁棒的解决方案，实现了从单目RGB视频进行完整、物理一致且可动画化的重建。
...
TL;DR: 提出了一种基于无监督对比学习的高效鲁棒3D形状匹配方法，通过对比学习框架提升特征表示质量，简化功能映射架构，实现最先进的匹配性能


<details>
  <summary>Details</summary>
Motivation: 现有深度功能映射方法主要关注优化点对点或功能映射，而不是直接提升嵌入空间中的特征表示，导致特征质量不足和匹配性能欠佳。同时这些方法依赖耗时的传统功能映射求解器，计算成本高

Method: 1) 提出无监督对比学习框架，通过最大化正相似性对的一致性、最小化负相似性对的一致性来提升特征学习；2) 设计简化的功能映射学习架构，消除了计算昂贵的功能映射求解器和多个辅助功能映射损失，极大提高计算效率；3) 将这两个组件集成到统一的双分支流水线中

Result: 在准确性和效率方面都达到了最先进的性能。广泛的实验表明，该方法不仅在计算上高效，而且在各种挑战性基准测试（包括近等距、非等距和拓扑不一致场景）中优于当前最先进的方法，甚至超越了监督技术

Conclusion: 首次引入了基于无监督对比学习的3D形状匹配方法，通过对比学习改善特征表示并简化功能映射架构，实现了高效且鲁棒的形状匹配，为3D形状对应关系估计问题提供了新的解决方案
...
TL;DR: VGGT-360是一种无需训练的零样本全景深度估计框架，通过将任务重新定义为基于多视图重建3D模型的全景重投影，实现了几何一致性，超越了现有的视图无关训练方法。


<details>
  <summary>Details</summary>
Motivation: 现有训练无关的全景深度估计方法通常是视图独立的，缺乏跨视图的一致性。本文旨在开发一个无需训练的零样本框架，能够在全景环境中实现几何一致的深度估计，利用VGGT类基础模型的内在3D一致性。

Method: 提出三个即插即用模块的统一全景到3D到深度框架：1) 不确定性引导自适应投影：将全景切片为透视图，使用梯度不确定性估计分配更密集的视图到几何贫乏区域；2) 结构显著性增强注意力：在3D重建过程中注入结构感知置信度到注意力层，增强跨视图一致性；3) 相关加权3D模型校正：使用注意力推断的相关分数重新加权重叠点，优化重建的3D模型。

Result: 广泛的实验表明，VGGT-360在多个分辨率和多样室内外数据集上超越了已训练和训练无关的最新方法，实现了优异的全景深度估计性能。

Conclusion: VGGT-360通过重新将全景深度估计定义为全景重投影任务，并集成三个协同工作模块，实现了零样本、几何一致的全景深度估计，为训练无关的全景3D理解提供了新范式。
...
TL;DR: CRAFT是一种轻量级但强大的扩散模型微调范式，仅需100个样本即可超越需要数千个偏好配对样本的现有方法，且收敛速度提高11-220倍。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型对齐方法存在两个主要问题：1）数据依赖性高（SFT需要高质量图像，DPO风格方法需要大规模偏好数据集）；2）计算效率低。作者旨在解决这两个挑战。

Method: 提出Composite Reward Assisted Fine-Tuning (CRAFT)，包含两个关键组件：1）Composite Reward Filtering (CRF)技术构建高质量一致训练数据集；2）增强版SFT进行微调。理论上证明了CRAFT优化了基于群体的强化学习的下界。

Result: 实证结果显示：1）仅使用100个样本就能超越需要数千个偏好配对样本的最新SOTA偏好优化方法；2）收敛速度比基线偏好优化方法快11-220倍；3）实现了高质量图像生成和对齐。

Conclusion: CRAFT是一个数据高效且计算高效的扩散模型对齐框架，在减少训练数据和保持计算效率方面表现出色，为扩散模型对齐提供了新的有效解决方案。
...
TL;DR: 本文提出了一种简洁有效的指纹增强方法，包括上下文过滤方法和学习基方法，这两种方法在低质量指纹图像上表现优于复杂的现有方法，通过开源实现促进可重复性。


<details>
  <summary>Details</summary>
Motivation: 指纹识别系统依赖于指纹图像的独特特征，但当前最先进的指纹增强方法在处理低质量指纹时存在困难且计算成本高，需要更简单有效的方法。

Method: 提出两种新颖的指纹增强方法：上下文过滤方法和学习基方法，这两种方法强调简洁性和有效性，在具有挑战性的潜在指纹数据库上进行了验证。

Result: 提出的方法持续超越复杂的现有方法，产生更清晰、更准确、噪声更少的指纹图像，并进行了开源实现以促进可重复性和进一步研究。

Conclusion: 研究发现简洁性对于实现高质量指纹增强至关重要，未来的研究应该在复杂性和实际效益之间取得平衡，简化方法往往能带来更好的性能。
...
TL;DR: GenHOI是一个用于广义手-物体姿态估计的框架，通过层次语义提示和多模态掩码建模来解决遮挡条件下的挑战性问题。


<details>
  <summary>Details</summary>
Motivation: 单幅RGB图像下的广义3D手-物体姿态估计面临挑战，主要由于物体外观和交互模式的巨大变化，特别是在严重遮挡条件下。现有方法在未见过的物体和新型交互模式上泛化能力有限。

Method: 1. 引入层次语义提示：通过文本描述编码物体状态、手部配置和交互模式；2. 多模态掩码建模：在RGB图像、预测点云和文本描述上进行；3. 利用手部先验作为稳定空间参考来提取隐式交互约束。

Result: 在DexYCB和HO3Dv2等挑战性基准测试上的大量实验表明，该方法在手-物体姿态估计中达到了最先进的性能。

Conclusion: GenHOI框架通过整合层次语义知识和手部先验，显著提升了在遮挡条件下的手-物体姿态估计性能，并展现出对未见物体和新交互模式的良好泛化能力。
...
TL;DR: SELF1E是一种从多模态大语言模型中实现无解码器分割的新方法，通过单个分割嵌入实现竞争性性能，消除了对外部解码器的依赖。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖专家掩码解码器来从生成的分割相关嵌入和视觉特征解释掩码，或使用多个额外令牌辅助。本文旨在探究是否及如何仅使用单个分割嵌入（SELF1E）从MLLM本身解锁分割能力，同时实现竞争性结果，从而消除对外部解码器的需求。

Method: 1. 保留原始未压缩分辨率的图像特征，并用从MLLM处理的压缩特征中提取的残差特征进行重新填充，提高特征精度。2. 对经过和未经过LLM处理的图像特征分别集成像素反洗牌操作，释放压缩特征的细节并在未压缩分辨率下放大残差特征，进一步增强填充特征的分辨率。3. 重新设计具有双重感知路径的注意力掩码（图像到图像和图像到分割），实现像素与分割令牌之间丰富的特征交互。

Result: 在多个分割任务上的综合实验验证了SELF1E实现了与基于专家掩码解码器方法竞争的性能，证明了在MLLMs中无解码器分割的可行性。

Conclusion: SELF1E展示了从多模态大语言模型本身实现高效分割的潜力，通过创新的特征增强和交互机制消除了对外部解码器的依赖，为简化分割流程提供了新思路。
...
TL;DR: TerraScope是一个统一的视觉语言模型，能够在像素级别进行地理空间推理，具备多模态灵活融合和多时序推理能力，在像素级地理空间推理任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在处理需要将复杂空间推理与精确像素级视觉表示相结合的地球观测任务时表现不佳，缺乏像素级的地理空间推理能力。

Method: 1. 提出了TerraScope模型，具有模态灵活推理（处理单一模态或自适应融合多模态）和多时序推理能力；2. 构建了Terra-CoT数据集，包含100万个带有像素级掩码的样本；3. 提出了TerraScope-Bench基准测试，包含6个子任务，评估答案准确性和掩码质量。

Result: 实验表明TerraScope在像素级地理空间推理任务上显著优于现有视觉语言模型，同时提供可解释的视觉证据。

Conclusion: TerraScope成功解决了像素级地理空间推理的挑战，通过统一的视觉语言模型实现了模态灵活和多时序的像素级推理，为地球观测任务提供了有效的解决方案。
...
TL;DR: 提出SGC指标来评估生成视频中的3D空间几何一致性，通过测量从不同局部区域估计的相机姿态之间的差异来量化几何一致性。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型能产生高保真视频，但常存在3D空间几何不一致性。现有评估方法无法准确表征这些问题：基于保真度的指标如FVD对几何失真不敏感，而一致性基准往往惩罚有效的前景动态。

Method: 方法包括：1)分离静态和动态区域；2)将静态背景划分为空间一致子区域；3)为每个像素预测深度；4)为每个子区域估计局部相机姿态；5)计算这些姿态之间的差异来量化几何一致性。

Result: 在真实和生成视频上的实验表明，SGC能够稳健地量化几何不一致性，有效识别现有指标遗漏的关键失败案例。

Conclusion: SGC是评估生成视频3D空间几何一致性的有效指标，填补了现有评估方法的空白，能够检测出传统指标无法捕捉的几何不一致问题。
...
TL;DR: SwiftTailor：一个新颖的两阶段框架，通过紧凑的几何图像表示统一缝合模式推理和基于几何的网格合成，实现快速高效的3D服装生成


<details>
  <summary>Details</summary>
Motivation: 现有的3D服装生成方法通常依赖于大型视觉语言模型生成2D缝纫模式的序列化表示，然后使用GarmentCode等框架转换为模拟就绪的3D网格。虽然这些方法能产生高质量结果，但推理速度较慢（30秒到1分钟），无法满足实时或大规模应用需求。

Method: SwiftTailor是一个两阶段框架，包含两个轻量级模块：PatternMaker（高效视觉语言模型，从多样化输入模态预测缝纫模式）和GarmentSewer（高效密集预测Transformer，将缝纫模式转换为新颖的Garment Geometry Image，在统一的UV空间中编码所有服装面板的3D表面）。最终3D网格通过高效的逆映射过程重建，该过程包含重新网格化和动态缝合算法，直接组装服装，从而分摊物理模拟的成本。

Result: 在Multimodal GarmentCodeData上的广泛实验表明，SwiftTailor实现了最先进的准确性和视觉保真度，同时显著减少了推理时间。

Conclusion: 该工作为下一代3D服装生成提供了一个可扩展、可解释且高性能的解决方案，通过紧凑的几何图像表示统一了缝纫模式推理和网格合成，在保持质量的同时大幅提升了效率。
...
TL;DR: Em-Garde：一种用于流视频理解的解耦框架，通过指令引导提案解析和轻量级提案匹配模块解决效率-准确率权衡问题


<details>
  <summary>Details</summary>
Motivation: 现有的主动式VideoLLMs采用逐帧触发决策，面临效率与准确率之间的权衡困境，无法在严格计算约束下实现高效准确的主动响应

Method: 提出Em-Garde框架：1) 查询时，指令引导提案解析器将用户查询转换为结构化、感知基础的视觉提案；2) 流处理时，轻量级提案匹配模块执行基于嵌入的高效匹配来触发响应，实现语义理解与流感知的解耦

Result: 在StreamingBench和OVO-Bench上的实验表明，相对于先前模型在主动响应准确率和效率方面实现一致改进，验证了在严格计算约束下主动视频理解的有效解决方案

Conclusion: Em-Garde框架通过解耦语义理解与流感知，有效解决了主动视频理解中的效率-准确率权衡问题，为计算受限场景下的主动视频理解提供了新范式
...
TL;DR: 这是一个实时的RGB SLAM系统，专门处理动态环境，通过可微的不确定性感知束调整来应对动态物体和杂乱场景。


<details>
  <summary>Details</summary>
Motivation: 传统SLAM方法通常假设场景静态，导致在动态环境下跟踪失败。现有动态SLAM方法使用预定义的动态先验或不确定性感知映射，但在面对未知动态物体或高度杂乱场景时仍有局限。

Method: 利用多视角视觉特征不一致性估计每像素不确定性，采用可微的不确定性感知束调整方法，实现鲁棒跟踪和重建。

Result: 在杂乱动态场景中实现了最先进的相机位姿和场景几何重建，系统实时运行速度约为10 FPS。

Conclusion: 提出的RGB SLAM系统能有效处理现实世界动态环境，通过不确定性感知机制在动态和杂乱场景中保持鲁棒性，代码和数据集已开源。
...
TL;DR: 提出Roundabout-TAU数据集和TAU-R1框架，用于基于视觉-语言模型的交通异常理解任务


<details>
  <summary>Details</summary>
Motivation: 尽管视觉-语言模型在视频理解方面表现出强大能力，但交通异常理解领域的进展受到缺乏基准数据集和任务特定方法的限制

Method: 1) 引入Roundabout-TAU数据集，包含342个真实世界环形交叉路口视频片段和2000多个问答对；2) 提出TAU-R1双层框架，包括轻量级异常分类器和大型异常推理器；3) 采用两阶段训练策略：分解QA增强的监督微调 + TAU-GRPO后训练方法

Result: TAU-R1在异常分类和推理任务上均表现出色，同时保持了部署效率

Conclusion: 提出的数据集和框架为交通异常理解提供了有效的基准和解决方案，代码和数据集已开源，促进了该领域的研究
...
TL;DR: CustomTex是一个新颖的框架，通过参考图像实现实例级别的高保真3D室内场景纹理生成，采用双重蒸馏方法分离语义控制和像素级增强。


<details>
  <summary>Details</summary>
Motivation: 目前基于文本驱动的方法虽然灵活，但缺乏对细粒度实例级控制的精确性，常产生质量不足、存在伪影和烘焙阴影的纹理。需要一种更直接、用户友好的高质量3D场景外观编辑方法。

Method: 1. 输入：未纹理化的3D场景和指定每个对象实例期望外观的参考图像集
2. 核心方法：双重蒸馏方法，包括：
   - 语义级蒸馏：配备实例交叉注意力机制，确保语义合理性和"参考-实例"对齐
   - 像素级蒸馏：强制执行高视觉保真度
3. 框架：在变分分数蒸馏（VSD）优化框架内统一两种蒸馏

Result: 实验证明，CustomTex实现了与参考图像的精确实例级别一致性，相比最先进的方法，产生的纹理具有更高的锐度、更少的伪影和最小的烘焙阴影。

Conclusion: 该工作为高质量、可定制的3D场景外观编辑建立了更直接和用户友好的路径，解决了现有方法在精确控制和高保真纹理生成方面的局限性。
...
TL;DR: 该论文提出了一种基于任意顺序自回归模型的生成式分类器，通过多个token顺序的平均预测克服了传统固定顺序自回归模型的限制，在图像分类任务中超越了扩散模型分类器，并达到与判别式模型竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 传统的视觉自回归生成式分类器依赖于固定的token顺序，这强加了限制性的归纳偏置，导致模型主要依赖部分判别性线索进行分类。作者观察到单顺序预测存在这种局限性，而平均多个token顺序的预测能提供更全面的信号。

Method: 利用最近的任意顺序自回归模型来估计顺序边缘化的预测，通过平均多个token顺序的预测结果来解锁自回归模型的分类潜力。这种方法摆脱了传统固定顺序的限制，允许模型从多种可能的token排列中获取更全面的信息。

Result: 该方法在各种图像分类基准测试中一致优于基于扩散的生成式分类器，同时效率提高了多达25倍。与最先进的自监督判别式模型相比，该方法提供了具有竞争力的分类性能，这对于生成式分类器而言是一个显著成就。

Conclusion: 通过利用任意顺序自回归模型进行顺序边缘化预测，可以克服传统自回归生成式分类器的限制，实现超越扩散模型分类器的性能，并在效率上具有显著优势，使生成式分类器能够达到与判别式模型竞争的水平。
...
<details>
  <summary>Details</summary>
Motivation: 在文本到图像合成中生成稀有组合概念对扩散模型仍然是一个挑战，特别是对于那些在训练数据中不常见的属性。现有方法如R2F使用LLM进行提示调度，但存在语言模型随机性带来的方差问题以及迭代文本嵌入切换的次优指导

Method: ADAPT框架无需训练，通过利用注意力分数和正交分量来确定性规划和语义对齐提示调度，为稀有概念的组合提供一致指导

Result: ADAPT在RareBench基准测试中显著增强了稀有组合概念的生成能力，能够准确反映稀有属性的语义信息，提供确定性和精确的控制，同时不损害视觉完整性

Conclusion: ADAPT框架通过确定性的提示调度规划实现了对稀有组合概念的精确控制，无需额外训练或微调，解决了现有方法中由语言模型随机性带来的问题
...
TL;DR: 本文提出了自适应辅助提示混合（AAPB）框架，用于稳定扩散模型在低密度区域的生成过程，通过自适应系数在辅助锚点提示和目标提示之间进行最优平衡。


<details>
  <summary>Details</summary>
Motivation: 基于扩散的文本到图像（T2I）模型在生成逼真图像方面取得显著进展，但当目标概念位于训练分布的低密度区域时，往往产生语义错位或结构不一致的结果。这是由于文本-图像数据集的"长尾"特性，其中罕见概念或编辑指令未被充分表示。

Method: 提出了自适应辅助提示混合（AAPB）框架，利用辅助锚点提示在罕见概念生成中提供语义支持，在图像编辑中提供结构支持。该方法基于Tweedie恒等式推导出封闭形式的自适应系数，在扩散过程的每一步最优平衡辅助锚点和目标提示的影响。

Result: 通过对比实验证明自适应插值优于固定插值，在RareBench和FlowEdit数据集上取得一致改进，在语义准确性和结构保真度方面优于现有无训练基线方法。

Conclusion: AAPB提供了一个原则性的无训练框架，通过自适应提示混合稳定低密度区域的扩散过程，确保目标导向的稳定生成，解决了文本到图像模型中罕见概念生成的挑战。
...
TL;DR: ARIADNE是一个两阶段框架，将偏好对齐感知与基于强化学习的诊断推理结合，用于拓扑一致的冠状动脉狭窄检测，通过DPO优化几何完整性而非像素级精度。


<details>
  <summary>Details</summary>
Motivation: 传统的像素级损失函数无法强制执行拓扑约束，导致冠状动脉分割产生碎片化的血管树，尽管像素级精度很高但拓扑结构不完整。

Method: 采用两阶段框架：1)感知模块使用DPO微调Sa2VA视觉语言基础模型，以Betti数约束作为偏好信号；2)推理模块将狭窄定位建模为马尔可夫决策过程，包含显式拒绝机制，能够自主推迟模糊的解剖候选（如分叉和血管交叉）。

Result: 在1,400个临床血管造影上，ARIADNE达到最先进的中线Dice系数0.838，与几何基线相比减少41%的假阳性。在多中心基准ARCADE和XCAD上的外部验证确认了跨采集协议的泛化能力。

Conclusion: 这是DPO在医学成像中拓扑对齐的首个应用，证明基于结构约束的偏好学习可以减轻拓扑违规，同时在介入心脏病学工作流程中保持诊断敏感性。
...
TL;DR: Splat2BEV：一个使用高斯溅射技术增强BEV感知的框架，通过显式的3D场景重建来提升鸟瞰图表示的几何精度和语义丰富度


<details>
  <summary>Details</summary>
Motivation: 现有BEV感知框架通常采用端到端训练范式，将图像特征直接转换到BEV空间，这种方法将整个感知过程视为黑盒，缺乏显式的3D几何理解和可解释性，导致性能欠佳。作者认为显式的3D表示对于准确的BEV感知至关重要。

Method: 提出Splat2BEV框架：首先预训练一个高斯生成器，从多视角输入中显式重建3D场景，生成几何对齐的特征表示；然后将这些表示投影到BEV空间，作为下游任务的输入。该方法融合了3D高斯溅射技术和BEV感知。

Result: 在nuScenes和Argoverse数据集上的广泛实验表明，Splat2BEV实现了最先进的性能，验证了将显式3D重建纳入BEV感知的有效性。

Conclusion: 显式的3D表示对于BEV感知的准确性至关重要。Splat2BEV通过结合高斯溅射技术和3D重建，在提升BEV特征表示的几何精度和语义丰富度方面展现出优越性，为自动驾驶感知提供了更可靠和可解释的解决方案。
...
TL;DR: 研究表明视觉语言模型存在选择性视觉忽视现象，模型的视觉注意力会根据语言表达方式而变化，即使不同表达方式需要相同的视觉推理。研究者通过轻量级提示调优方法改善了这一问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型已被证明存在视觉忽视问题，常常在需要视觉推理的任务中未能充分利用视觉输入。本研究进一步发现这种忽视具有选择性，模型会根据语言表达方式来调节对视觉输入的注意力，这导致性能下降和跨表达不一致的问题。

Method: 使用视觉注意力作为探针，量化表达方式如何改变对图像的注意力数量和分布。研究发现受限制的表达方式（如多选题和是非题）比开放式表达引发更低的图像上下文注意力。基于这一机制性洞察，引入了使用可学习token的轻量级提示调优方法，鼓励在开放式设置中观察到的稳健、视觉基础的注意力模式。

Result: 受限制的表达方式会：1)显著降低对图像上下文的注意力；2)减少对任务相关区域的关注；3)将注意力转向无信息量的token。这种注意力错配是准确率下降和跨表达不一致的主要原因。通过提示调优方法，成功改善了视觉基础和跨表达的性能。

Conclusion: 视觉语言模型存在选择性视觉忽视问题，其视觉注意力易受语言表达方式影响。通过理解这一机制并设计相应的提示调优方法，可以有效改善模型的视觉基础和推理一致性，提升在各种表达方式下的性能表现。
...
TL;DR: 提出了Representation-Pivoted AutoEncoder（RPiAE），一种基于视觉表示模型的tokenizer，通过表示-枢轴正则化和变分桥等创新技术，在保持语义结构的同时提升重建保真度，并降低扩散建模复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用预训练视觉表示模型作为tokenizer时存在两个主要问题：1)冻结编码器导致重建保真度有限，进而影响编辑质量；2)潜在空间维度过高使扩散建模困难。需要一种既能保持语义结构又能提升重建质量，同时降低扩散复杂度的tokenizer方案。

Method: 提出了Representation-Pivoted AutoEncoder框架，包含三个关键技术：1)Representation-Pivot Regularization训练策略，允许基于表示模型初始化的编码器进行微调以提高重建能力，同时保持预训练表示空间的语义结构；2)Variational Bridge将潜在空间压缩到更紧凑的维度以便于扩散建模；3)目标解耦的分阶段训练策略，依次优化生成可处理性和重建保真度目标。

Result: 实验结果表明，RPiAE在文本到图像生成和图像编辑任务上优于其他视觉tokenizer，同时在基于表示的tokenizer中实现了最佳的重建保真度。该方法在保持强语义的同时，能够忠实地重建图像，并产生具有降低扩散建模复杂度的潜在表示。

Conclusion: RPiAE通过创新的表示-枢轴正则化、变分桥和分阶段训练策略，成功解决了现有表示基tokenizer在重建保真度和扩散建模复杂度方面的限制，为视觉生成和编辑任务提供了一种既保持语义结构又具有高质量重建能力的tokenizer方案。
...
TL;DR: DreamPartGen：一个语义基础、部件感知的文本到3D生成框架，通过双重部件隐变量和关系语义隐变量实现语义和几何一致的部件分解3D生成。


<details>
  <summary>Details</summary>
Motivation: 大多数文本到3D方法忽视了部件的语义和功能结构，而现有的部件感知方法虽然引入了分解，但主要关注几何层面，缺乏语义基础，无法建模部件如何与文本描述对齐或部件间关系。

Method: 1. 引入双重部件隐变量(DPLs)联合建模每个部件的几何和外观；2. 引入关系语义隐变量(RSLs)从语言中捕获部件间依赖关系；3. 采用同步协同去噪过程，强制几何和语义一致性。

Result: 在多个基准测试中，DreamPartGen在几何保真度和文本-形状对齐方面实现了最先进的性能，能够生成连贯、可解释且文本对齐的3D合成结果。

Conclusion: DreamPartGen通过语义基础、部件感知的方法，成功地解决了现有文本到3D生成中缺乏部件语义结构建模的问题，实现了语义一致、功能合理且可解释的3D对象生成。
...
TL;DR: LVOmniBench是一个针对长格式音频视频跨模态理解的新基准，包含275个10-90分钟的视频和1014个QA对，评估显示现有模型在长音频视频处理上仍有挑战


<details>
  <summary>Details</summary>
Motivation: 当前全模态大语言模型评估主要关注短音频视频片段（10秒到5分钟），无法反映真实应用中通常长达数十分钟的视频需求，存在重要空白

Method: 从开放平台收集高质量视频，通过严格人工筛选和标注构建LVOmniBench基准，包含275个10-90分钟视频和1014个问答对，旨在评估长时记忆、时序定位、细粒度理解和多模态感知能力

Result: 开源模型准确率普遍低于35%，Gemini 3 Pro达到约65%的峰值准确率，表明当前全模态大语言模型在处理长音频视频输入时面临显著挑战

Conclusion: LVOmniBench数据集和实证发现将推动长格式音频视频跨模态理解的研究，促进能够解决复杂跨模态理解问题的先进模型发展
...
TL;DR: 提出了一种改进扩散模型在生成式分割中性能的方法，通过向量场重塑策略和准随机类别编码来解决流匹配目标与离散感知任务之间的内在不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注架构调整或训练启发式方法，对连续流匹配目标与离散感知任务之间的内在不匹配理解有限。识别了常用的流匹配目标存在的两个关键限制：梯度消失和轨迹穿越，导致收敛缓慢和类别分离差。

Method: 1) 提出向量场重塑策略，通过添加解耦的距离感知修正项增强学习的速度场，引入吸引和排斥相互作用以增强中心点附近的梯度幅度；2) 设计基于Kronecker序列启发的计算高效的准随机类别编码方案，与端到端像素神经场框架无缝集成以实现像素级语义对齐。

Result: 大量实验一致表明，相比普通的流匹配方法有显著改进，显著缩小了生成式分割与强判别式专用方法之间的性能差距。

Conclusion: 从向量场学习的角度重新审视扩散分割，提出了系统的解决方案来克服流匹配在分割任务中的固有局限性，为生成式分割提供了更有效的训练框架。
...
TL;DR: DriveTok是一个高效的3D驾驶场景标记化器，专门为高分辨率多视角自动驾驶场景设计，通过统一的场景标记集成语义、几何和纹理信息，支持多视角重建和理解任务。


<details>
  <summary>Details</summary>
Motivation: 随着视觉-语言-动作模型和世界模型在自动驾驶系统中的广泛应用，可扩展的图像标记化作为视觉模态接口变得至关重要。然而，现有标记化器大多针对单目和2D场景设计，应用于高分辨率多视角驾驶场景时存在效率低下和视角间不一致的问题。

Method: 首先从视觉基础模型获取语义丰富的视觉特征，然后通过3D可变形交叉注意力将其转换为场景标记。解码阶段采用多视角transformer从场景标记重建多视角特征，使用多头机制获得RGB、深度和语义重建。同时添加3D头直接在场景标记上进行3D语义占据预测以增强空间感知能力。

Result: 在广泛使用的nuScenes数据集上的大量实验表明，DriveTok生成的场景标记在图像重建、语义分割、深度预测和3D占据预测任务上均表现优异。

Conclusion: 通过多训练目标，DriveTok学习了能够集成语义、几何和纹理信息的统一场景标记，实现了高效的多视角标记化，为解决自动驾驶场景中的视觉表示问题提供了有效解决方案。
...
TL;DR: 本文提出一种基于图像频谱特性的原理性方法，用于设计像素扩散模型的逐实例噪声调度，通过理论推导最小和最大噪声级别的有效边界，创建"紧凑"噪声调度以消除冗余步骤。


<details>
  <summary>Details</summary>
Motivation: 扩散模型的性能高度依赖噪声调度，而现有噪声调度通常是手动设计的，需要针对不同分辨率进行手动调整。作者认为通用的人工设计噪声调度存在冗余步骤，未能充分利用图像的频谱特性。

Method: 提出基于图像频谱特性设计逐实例噪声调度的方法：1) 理论推导最小和最大噪声级别的有效边界；2) 设计"紧凑"噪声调度以消除冗余步骤；3) 在推理阶段条件化采样这些噪声调度。

Result: 实验结果表明，所提出的噪声调度能够提高单阶段像素扩散模型的生成质量，特别是在低步数（low-step）情况下效果显著。

Conclusion: 基于图像频谱特性设计的逐实例"紧凑"噪声调度是一种有效的原理性方法，能够优化扩散模型的性能，特别是在减少采样步数的情况下仍能保持或提高生成质量。
...
TL;DR: 本文提出了VOR数据集和EffectErase方法，用于视频对象移除任务，特别关注消除动态目标对象及其视觉效应（如变形、阴影、反射），并恢复无缝背景。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的视频修复和对象移除方法虽然能移除对象，但难以完全消除对象的视觉效应（变形、阴影、反射等）并合成连贯背景。此外，缺乏系统捕捉不同环境中常见对象效应的综合数据集，阻碍了该领域的进展。

Method: 1. 提出了VOR数据集：大规模配对视频数据集，包含60K高质量视频对，涵盖5种效应类型，来自实拍和合成来源，覆盖广泛对象类别和复杂动态多对象场景。2. 提出了EffectErase方法：基于效应的视频对象移除方法，将视频对象插入作为逆辅助任务纳入互惠学习方案，包括任务感知区域引导和插入-移除一致性目标。

Result: 在VOR数据集上训练的EffectErase方法在大量实验中表现出卓越性能，能够跨多样化场景实现高质量的视频对象效应消除。

Conclusion: 本文通过引入全面的大规模VOR数据集和创新的EffectErase方法，显著推进了视频对象移除领域，特别在消除对象视觉效应方面取得了重要进展，为未来研究提供了有价值的数据资源和算法框架。
...
TL;DR: MultiGP是一种生成式逆渲染方法，能够从单张图像中随机采样所有辐射度量成分（反射率、纹理和光照），通过利用同一场景中多个物体的共同光照约束来解耦这些成分。


<details>
  <summary>Details</summary>
Motivation: 从单张图像中解耦辐射度量成分（反射率、纹理和光照）具有本质上的模糊性，因为不同的辐射度量组合可能产生相同的观察图像。传统的逆渲染方法难以解决这种模糊性，需要新的方法来可靠地分离这些外观成分。

Method: MultiGP基于四个关键技术贡献：1) 结合图像空间和角度空间解耦的级联端到端架构；2) 协调引导技术确保扩散模型收敛到一致的光照估计；3) 轴向注意力机制促进不同反射率物体之间的"交叉对话"；4) 纹理提取ControlNet在保持高频纹理细节的同时确保与光照解耦。该方法利用同一场景中所有物体共享相同光照的共识来约束解耦过程。

Result: 实验结果表明，MultiGP能够有效利用多个物体外观的互补空间和频率特征，恢复出个体纹理和反射率以及共同的光照。该方法成功地从单张图像中生成了所有辐射度量成分的合理样本。

Conclusion: MultiGP通过生成式逆渲染方法和利用场景中物体共享光照的约束，解决了从单张图像中解耦辐射度量成分的根本模糊性问题，为计算机视觉和图形学中的外观分析和合成提供了新的解决方案。
...
TL;DR: 提出三阶段运动生成框架（感知-规划-控制），结合离散令牌规划和连续扩散生成的优势，实现语义和运动学条件的解耦控制


<details>
  <summary>Details</summary>
Motivation: 现有运动生成方法存在两种范式：连续扩散模型擅长运动学控制但语义条件效果有限，离散令牌生成器有效处理语义条件但运动学控制不足。需要结合两者优势

Method: 1. 三阶段框架：感知（条件特征提取）、规划（离散令牌生成）、控制（扩散运动合成）。2. 核心MoTok扩散离散运动令牌器，将语义抽象与细粒度重建解耦。3. 运动学条件分阶段处理：粗粒度约束指导令牌规划，细粒度约束通过扩散优化在控制阶段强制执行

Result: 在HumanML3D上：1. 仅用1/6令牌数显著提升可控性和保真度。2. 轨迹误差从0.72cm降至0.08cm。3. FID从0.083降至0.029。4. 在强运动学约束下FID进一步从0.033降至0.014（而先前方法会退化）

Conclusion: 提出的三阶段框架成功结合了离散令牌和连续扩散方法的优势，实现了语义与运动学控制的解耦，在保持运动保真度的同时显著提升了可控性和效率
...
TL;DR: SAMA框架通过将视频编辑分解为语义锚定和运动对齐两个因子化模块，无需外部先验就能实现精确语义修改与忠实运动保持的平衡


<details>
  <summary>Details</summary>
Motivation: 当前基于指令的视频编辑模型难以同时平衡精确语义修改和忠实运动保持，现有方法依赖外部先验导致模型鲁棒性和泛化能力受限

Method: 提出SAMA框架，包含：1)语义锚定模块，在稀疏锚帧联合预测语义token和视频潜变量实现指令感知结构规划；2)运动对齐模块，通过立方体修复、速度扰动和管状混洗等运动中心视频修复预训练任务学习时序动态；采用两阶段优化：因子化预训练学习内在语义-运动表示，然后在配对编辑数据上进行监督微调

Result: 因子化预训练阶段已具备强大的零样本视频编辑能力；SAMA在开源模型中达到最先进性能，与领先商业系统(Kling-Omni)竞争；代码、模型和数据集将发布

Conclusion: SAMA通过因子化语义锚定和运动对齐的方法，解决了视频编辑中语义修改与运动保持的平衡问题，无需外部先验，增强了模型鲁棒性和泛化能力
...
TL;DR: MonoArt是一个从单图像重建铰接3D物体的统一框架，通过渐进式结构化推理方法，在单架构中直接预测规范几何、结构化部件表示和运动感知嵌入，实现了最先进的性能和实时推理速度。


<details>
  <summary>Details</summary>
Motivation: 从单图像重建铰接3D物体需要同时推断几何、部件结构和运动参数，但运动线索与物体结构的纠缠使得直接关节回归不稳定。现有方法通常需要多视角监督、检索式装配或辅助视频生成，牺牲了可扩展性或效率。

Method: MonoArt采用渐进式结构化推理框架，而不是直接从图像特征预测关节。它将视觉观察渐进式转换为规范几何、结构化部件表示和运动感知嵌入，所有处理在单一架构中完成，无需外部运动模板或多阶段流程。

Result: 在PartNet-Mobility数据集上的实验表明，该方法在重建精度和推理速度上都达到了最先进的性能。该框架还能推广到机器人操作和铰接场景重建任务。

Conclusion: MonoArt通过渐进式结构化推理有效解决了单图像铰接3D重建中的运动-结构纠缠问题，实现了稳定、可解释的关节推理，无需外部模板或多阶段流程，在精度和效率上都优于现有方法。
...
TL;DR: CubiD是第一个针对高维表示的离散生成模型，通过在768-1024维离散表示上进行细粒度掩码，实现固定步骤的高质量图像生成，同时保持原始表示能力。
...
Method: 提出Cubic Discrete Diffusion (CubiD)，在高维离散表示中进行细粒度掩码——任何位置、任何维度都可以被掩码并从部分观测中预测。这使得模型能够学习空间位置内部和之间的丰富相关性，生成步骤固定为T（T远小于特征维度数hwd）。

Result: 在ImageNet-256上，CubiD实现了最先进的离散生成，参数从900M到3.7B都表现出强扩展性。离散化令牌保持原始表示能力，同一离散令牌可有效服务于理解和生成任务。

Conclusion: CubiD作为第一个高维表示离散生成模型，解决了现有方法的局限性，展示了离散令牌在保持表示能力的同时实现高质量生成的潜力，为统一多模态架构的未来研究提供启示。
...
TL;DR: VEGA-3D框架利用大规模视频生成模型的隐式空间先验，通过提取时空特征并整合语义表示，增强多模态大语言模型的几何推理能力，无需显式3D监督


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在空间盲区，难以进行细粒度的几何推理和物理动态理解。现有方法依赖于显式3D模态或复杂几何支架，受到数据稀缺和泛化挑战的限制。

Method: 提出VEGA-3D框架，将预训练视频扩散模型重新用作潜在世界模拟器。通过提取中间噪声层的时空特征，并使用令牌级自适应门控融合机制将其与语义表示整合，为MLLMs提供密集几何线索。

Result: 在3D场景理解、空间推理和具身操作基准测试中表现优于现有最先进方法，验证了生成先验可为物理世界理解提供可扩展基础。

Conclusion: 通过利用视频生成模型的隐式空间先验，能够有效增强MLLMs的几何推理能力，为解决空间盲区问题提供了一种数据高效且可扩展的解决方案。
...
TL;DR: 研究探讨当前大型语言模型是否具备心理理论能力，通过基于文本的心理理论测试比较五个LLMs与人类表现。结果显示GPT-4o表现接近人类，而早期和小模型在推理线索和干扰信息方面存在局限性。


<details>
  <summary>Details</summary>
Motivation: LLMs仅基于语言数据训练，缺乏社会具体化经验，其表现出的社会认知推理能力引发关键问题：它们是否具备真正的心理状态归因能力，还是仅仅反映表层模式补全？

Method: 使用人类心理理论研究中广泛使用的基于文本工具的改编版本，测试五个LLMs并与人类对照组比较。测试涉及回答故事角色信念、意图和情感的问题。

Result: 模型表现存在差距：早期和小模型受相关推理线索数量和干扰信息影响较大；GPT-4o表现出高准确性和强鲁棒性，在最挑战条件下表现与人类相当。

Conclusion: GPT-4o在心理理论任务中表现接近人类，但对LLMs是否具备真正理解还是统计近似的辩论做出贡献，揭示了模型能力的差异和边界。
...
TL;DR: 本文提出了一种基于对数尺度聚焦不确定性(LSFU)的度量方法和不确定性校准提示优化框架(UCPOF)，用于解决大语言模型在多类多项选择任务中的不确定性测量和提示优化问题，通过自适应触发检索增强生成来平衡性能与计算成本。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言处理中广泛应用，但自回归生成导致输出不确定性不可避免。由于模型性能对提示设计高度敏感，精确的不确定性测量对可靠提示优化至关重要。传统基于输出概率的不确定性度量方法（如熵）在多类多项选择任务中存在缺陷：将所有类别等同对待，忽略了预训练语料中类先验差异，无法区分来自先验的虚假置信度和来自上下文理解的真正确定性，导致置信度校准效果差。

Method: 提出对数尺度聚焦不确定性(LSFU)度量方法，这是一个基于首令牌的度量指标，灵感来自聚焦损失。LSFU将标签先验概率作为风险调节因子，通过动态权重机制抑制高频类的噪声，强调低频长尾类的风险，统一了测量尺度。基于LSFU进一步提出不确定性校准提示优化框架(UCPOF)，利用模型输出的首令牌选择高质量示例并动态优化提示。

Result: 综合评估显示，UCPOP相比少样本基线平均准确率提升6.03%，在整体平均准确率上超过始终开启的完全RAG方法5.75%，平均检索触发率降低50.66%。通过仅对高不确定性样本自适应地触发RAG，该框架显著降低计算成本，同时保持最先进的性能。

Conclusion: 本文提出的LSFU度量和UCPOF框架有效地解决了大语言模型在多类多项选择任务中的不确定性测量和提示优化问题。该方法通过抑制高频类噪声、关注长尾类风险，改善了置信度校准；通过自适应触发RAG机制，在提升性能的同时大幅降低了计算开销，实现了性能与效率的良好平衡。
...
TL;DR: 提出确定性证据选择框架用于检索增强问答系统，通过固定评分和冗余控制程序在答案生成前确定证据可接受性，无需训练或微调。


<details>
  <summary>Details</summary>
Motivation: 传统基于向量相似度的检索系统存在局限性：相似度分数不能解释为什么某些检索到的文本可以作为证据，而其他相似文本不能。当多个候选获得相似分数时，系统可能选择冗余、不完整或不符合问题条件的句子。

Method: 提出确定性证据选择框架，包括意义效用估计（MUE）和多样性效用估计（DUE）两个固定评分和冗余控制程序。每个句子或记录独立评估，使用语义相关性、术语覆盖、概念区分度和冗余度等明确信号。

Result: 原型系统中，只有当单元明确陈述任务所需的事实、规则或条件时才被接受。单元不合并或扩展。如果没有单元独立满足要求，系统返回无答案。这种确定性门控产生紧凑、可审计的证据集。

Conclusion: 该方法在相关文本和可用证据之间建立了清晰的界限，提供了一种确定性的、无需训练的检索增强问答证据选择方法。
...
TL;DR: DynaRAG是一个结合静态知识检索和动态API调用的混合检索增强生成框架，能够处理静态和时间敏感的查询需求。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统仅依赖静态语料库，无法处理需要最新信息的时间敏感查询，且容易因信息不足产生幻觉。

Method: 系统使用LLM重排器评估文档相关性，基于充分性分类器决定是否需要调用外部API，采用Gorilla v2进行精确API调用，并利用FAISS进行模式过滤以指导API选择。

Result: 在CRAG基准测试中，DynaRAG显著提高了动态问题的准确性，同时减少了幻觉现象。

Conclusion: 动态感知的路由和选择性工具使用对于构建可靠的真实世界问答系统至关重要，DynaRAG为此类系统提供了一个有效的框架。
...
TL;DR: 研究表明，虽然大型语言模型在特定条件下能够重构训练数据中的内容，但在标准生成任务中却系统地抑制了非因果、不可实现的解决方案框架，挑战了训练数据存在直接预测输出概率的假设。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在特定诱发条件下能够重构和追踪训练数据中的内容，但这种能力在标准生成情境中并未显现。研究旨在探索学习到的能力与表达输出之间的系统性分离现象，挑战训练数据存在直接预测输出概率的普遍假设。

Method: 这是一项经验观察研究，分析了300个提示-响应生成的表达情况，涵盖叙事和问题解决任务情境。研究基于最近的记忆连续性和对齐诱导的话语先验发现，考察了三种不同的LLM、十个任务场景以及创造性叙事和实用建议情境。

Result: 在所有生成的输出中，未观察到任何非因果解决方案框架的实例（0%，95%置信区间：[0%, 1.2%]），尽管在条件提取下已验证了重构能力。这表明任务条件化的生成策略能够全面抑制学习到的内容。

Conclusion: 研究发现任务条件化的生成策略可以跨多种情境全面抑制学习到的内容，挑战了训练数据存在直接预测输出概率的假设。这些发现对理解生成动态、输出分布控制和当代LLMs的行为边界具有重要意义。
...
TL;DR: MineDraft：一种批处理并行推测解码框架，通过重叠起草和验证阶段来隐藏起草延迟，显著提升大语言模型推理效率


<details>
  <summary>Details</summary>
Motivation: 标准推测解码（SD）的性能受到起草和验证阶段严格顺序执行的限制，导致推理效率低下，需要一种能隐藏起草延迟的方法来加速大语言模型推理

Method: 提出批处理并行推测解码（PSD）框架，采用新颖的批处理并行设计，维护两个请求批次，使一个批次的起草与另一个批次的验证重叠执行，以隐藏起草延迟

Result: 实验结果显示，MineDraft相比标准SD在吞吐量上提升高达75%，端到端延迟降低高达39%；并已作为vLLM插件实现，展示了其在生产就绪推理系统中的实用性

Conclusion: MineDraft通过并行执行推测解码的起草和验证阶段，有效解决了传统方法的序列执行瓶颈，显著提升了推理性能，为生产环境中的大语言模型推理提供了高效的解决方案
...
TL;DR: BenchBrowser是一个检索系统，能够帮助研究人员识别语言模型基准测试与用户实际需求之间的差距，通过分析基准测试内容有效性来避免"能力错觉"。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型基准测试的元数据过于粗糙，无法准确反映测试的具体内容。基准测试（如"诗歌"或"指令遵循"）可能并未全面测试相关能力的所有方面，导致研究人员可能产生模型能力被高估的错觉。这种不透明性使得验证基准测试与用户实际需求的一致性变得困难。

Method: 开发了BenchBrowser系统，这是一个检索器，能够从20个基准测试套件中检索与自然语言使用案例相关的评估项目。系统通过人工研究验证了其高检索精度，并生成证据来帮助诊断基准测试的内容有效性（对能力各个方面的覆盖范围）和收敛有效性（测量相同能力时的排名稳定性）。

Result: BenchBrowser被验证具有高检索精度，能够有效识别基准测试的两个主要问题：低内容有效性（能力方面覆盖不足）和低收敛有效性（测量相同能力时排名不稳定）。系统成功量化了研究人员意图与基准测试实际测量内容之间的关键差距。

Conclusion: BenchBrowser为语言模型基准测试提供了重要的诊断工具，帮助研究人员识别和量化基准测试与用户实际需求之间的脱节问题，从而避免对模型能力的错误评估，提高基准测试的有效性和实用性。
...
TL;DR: 研究证实，基于FrameNet语义标注的电子病历文本分析能显著提高基于性别暴力的识别性能，比传统结构化数据模型F1分数提升超0.3。


<details>
  <summary>Details</summary>
Motivation: 基于性别暴力是全球重大公共卫生问题，巴西虽要求医疗专业人员报告此类案件，但由于识别困难及信息系统整合有限，漏报率仍然很高。研究旨在探索能否通过电子病历开放文本字段的语义分析来识别GBV模式。

Method: 比较三种SVM分类器的性能：1)基于FrameNet语义标注的文本；2)标注文本与参数化数据结合；3)仅参数化数据。进行定量和定性分析。

Result: 包含语义标注的模型优于仅使用分类数据的模型，F1分数提高超过0.3，证明领域特定语义表示能提供超越结构化人口数据的有意义信号。

Conclusion: 临床叙述的语义分析能够增强基于性别暴力的早期识别策略，支持更明智的公共卫生干预措施。研究证实了语义分析在医疗记录中识别GBV模式的有效性。
...
TL;DR: 研究发现LLM在协助写作时不仅改变文本风格，还会系统性地改变语义含义，导致创作性降低、文本中立化，并在科学同行评审中影响评价标准


<details>
  <summary>Details</summary>
Motivation: 研究动机源于LLM在全球被广泛用于写作辅助，但人们对其如何实际影响写作内容、语义和创作过程缺乏系统理解。需要探究LLM使用是否会改变写作的本质含义，以及这种改变如何影响文化科学机构

Method: 采用三种研究方法：1）人类用户研究，了解实际使用LLM写作时的交互模式；2）使用2021年收集的人工写作语料库，分析LLM基于人类反馈修改文本时的语义变化；3）分析近期AI会议中21%AI生成的同行评审，比较LLM生成与人工评审的差异

Result: 主要发现：1）重度LLM使用者导致70%的文章对主题问题保持中立，用户报告写作创造性降低且失去个人声音；2）即使仅要求语法编辑，LLM仍会显著改变文本语义含义；3）LLM生成的同行评审较少关注研究清晰度和重要性，平均评分高出约1分

Conclusion: 研究揭示了AI使用带来的感知效益与写作语义一致性改变之间的错位，表明广泛使用AI写作将影响文化科学机构，需要未来研究关注AI如何系统性地改变人类表达的内容本质和评价标准
...
TL;DR: 本研究通过比较人类和LLM生成的词汇联想，评估大语言模型如何捕捉人类词汇模式。研究发现不同规模模型在响应变异性和典型性上存在差异，温度参数也显著影响这种权衡。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在文本生成流畅性方面表现出色，但其语言学知识的本质——特别是其内部词汇表征的人类相似性——仍然不确定。需要评估LLM是否准确捕捉人类词汇联想模式。

Method: 使用SWOW数据集中的人类英语线索-反应对，并生成三种LLM（Mistral-7B、Llama-3.1-8B和Qwen-2.5-32B）在不同温度设置下的联想。分析词汇因素（词频、具体性）对线索-反应对的影响，以及LLM响应与人类响应的变异性和典型性比较。

Result: 所有模型都反映了人类在词频和具体性方面的趋势，但在响应变异性和典型性上存在差异。较大模型（如Qwen）倾向于模拟单一的"原型"人类参与者，产生高度典型但变异最小的响应；较小模型（如Mistral和Llama）产生更多变异但不太典型的响应。温度设置进一步影响这种权衡，较高的值增加变异性但降低典型性。

Conclusion: 研究结果突显了人类和LLM词汇表征之间的相似性和差异性，强调在探究LLM词汇表征时需要考虑模型规模和温度参数的重要性。
...
TL;DR: GRAFITE是一个持续评估大型语言模型的平台，通过用户反馈构建问题库，使用LLM-as-a-judge进行质量保证测试，实现多模型对比和回归检测。


<details>
  <summary>Details</summary>
Motivation: LLM评估面临基准数据污染风险，导致模型性能被高估。当前评估方法缺乏持续性和对真实世界问题的关注，需要系统化解决方案来确保评估的准确性和可持续性。

Method: 1. 构建基于用户反馈的模型问题知识库 2. 采用LLM-as-a-judge方法进行QA测试 3. 提供多模型并行比较功能 4. 实现不同版本间的回归检测机制

Result: 开发了GRAFITE平台，提供开源实现（GitHub仓库）和演示视频，建立了系统化的LLM评估框架，支持持续的性能监测和问题跟踪。

Conclusion: GRAFITE为解决LLM评估中的数据污染和性能高估问题提供了有效解决方案，通过系统化的持续评估机制确保模型性能评估的准确性和可靠性。
...
TL;DR: CWoMP是一种新型的自动生成IGT（行间注释文本）的方法，通过将语素视为原子形式-意义单位并进行对比学习，在低资源语言上优于现有方法且具有更高的效率。


<details>
  <summary>Details</summary>
Motivation: IGT是语言文档记录的标准符号，包含丰富语言学信息但手工制作费时费力。现有自动方法将注释视为字符序列，忽略了其组合结构。

Method: CWoMP将语素作为原子形式-意义单位并学习其表示。通过对比训练的编码器将上下文中的单词与其组成语素在共享嵌入空间中对齐；自回归解码器通过从可变词典中检索这些嵌入来生成语素序列。

Result: 在多种低资源语言上的评估表明，CWoMP在性能上优于现有方法，同时显著更高效，特别是在极端低资源设置下表现尤为突出。

Conclusion: CWoMP提供了一种可解释的自动生成IGT的方法，其预测基于词典条目，用户可以在推理时通过扩展词典来改进结果而无需重新训练。
...
TL;DR: 该论文提出，主流AI范式（强化学习、深度学习、记忆增强架构）继承了启发它们的心理学理论（行为主义、认知主义、建构主义）的结构局限性。作者通过分析东西方对机械学习的不同理解，引入ReSynth三模块框架来分离推理、目的和知识，以解决AGI的核心挑战——适应性。


<details>
  <summary>Details</summary>
Motivation: 现有AI范式继承自心理学理论的结构局限性阻碍了人工通用智能的发展：强化学习无法解释知识内部结构；深度学习将表示压缩为不透明的参数空间；整合方法缺乏从现有组件构建新理解的形式化描述。论文旨在解决这些局限性并提升AI系统的适应性。

Method: 1) 分析心理学理论（行为主义、认知主义、建构主义）与对应AI范式之间的谱系关系；2) 探讨东西方对机械学习的文化分歧，将东方记忆观作为理解桥梁；3) 借鉴系统性辩论和对经典主义/联结主义的批判，引入ReSynth三模块框架，将推理(Intellect)、目的(Identity)和知识(Memory)作为架构独立的组件。

Result: 提出了ReSynth框架作为解决AI范式局限性的一种方法。该框架通过架构分离确保系统性行为成为必要结果而非偶然属性。论文展示了如何通过这种分离来增强AI系统的适应性，使其能够从现有组件中构建新理解，从而更接近人工通用智能的目标。

Conclusion: 适应性是人工通用智能的核心挑战，需要一种表征架构，其中系统性行为是必然结果而非偶然属性。ReSynth框架通过分离推理、目的和知识组件，为解决现有AI范式的结构局限性提供了新的方向，同时借鉴了东方记忆观作为心理学理论与AI方法论之间的桥梁。
...
TL;DR: 本文提出了一种用于动态主题建模的时间分类法，将异常文档区分为可能预示新兴主题的"预期异常"、强化现有主题的文档或孤立文档，并在法国氢经济新闻语料库上进行了评估。


<details>
  <summary>Details</summary>
Motivation: 传统动态主题建模通常将异常值视为噪声处理，但作者认为其中一些异常值实际上可以作为新兴主题的早期信号。需要一种系统的方法来识别这些预示性文档，以更好地理解主题演化过程。

Method: 引入了文档轨迹的时间分类法，定义了文档随时间与主题形成的关系。在累积聚类框架中实现，使用来自11种最先进语言模型的文档嵌入。在HydroNewsFr（法国氢经济新闻语料库）上进行了回顾性评估，通过模型间一致性分析确定高共识的预期异常子集。

Result: 模型间一致性分析揭示了一个小而高共识的预期异常子集，增加了对这些标签的置信度。定性案例研究通过具体主题发展说明了这些轨迹，证实了分类法能够有效识别预示新兴主题的文档。

Conclusion: 该时间分类法将弱信号检测与时间主题建模联系起来，阐明了单个文章如何在演化集群中预期、启动或漂移。方法能够可靠地识别预示性异常文档，为理解主题形成过程提供了新视角。
...
TL;DR: 这篇论文提出了一个两阶段方法创建首个用于多样化生成式常识推理（GCR）的合成数据集CommonSyn，该数据集解决了现有GCR数据集规模小、多样性不足的问题，通过在合成数据上微调的模型在生成多样性和质量方面都得到了提升。


<details>
  <summary>Details</summary>
Motivation: 对话系统不仅需要高质量（常识承载）的响应，还需要考虑多种合理替代场景以实现响应多样性。尽管需要训练多样化的常识生成器，但由于缺乏大规模高质量多样化常识训练数据集，这一领域进展受阻。现有GCR数据集由于人工标注成本高，只覆盖有限的常识场景。

Method: 提出了一个两阶段方法创建首个合成数据集CommonSyn用于多样化GCR：1）生成多样化常识响应的合成数据；2）利用该数据增强模型的多样化生成能力。该方法旨在填补高质量多样化训练数据的空白。

Result: 在合成数据CommonSyn上微调的模型相比基础模型和在人工制作数据集上微调的模型，在不同规模的大语言模型（LLMs）上都显著提高了生成多样性和质量。

Conclusion: 提出的CommonSyn合成数据集有效解决了多样化GCR训练数据匮乏的问题，能够提升模型在常识生成任务中的多样性和质量，为多样化对话系统的发展提供了重要资源。
...
TL;DR: PowerFlow是一个基于GFlowNet的无监督强化学习框架，通过分布匹配问题来优化大型语言模型，使用长度感知的轨迹平衡目标来消除长度偏差，并能通过α-power分布同时增强逻辑推理和创造性表达能力。


<details>
  <summary>Details</summary>
Motivation: 当前的无监督强化学习方法依赖启发式的内在奖励，缺乏明确定义的理论优化目标，并且容易产生退化的偏差。现有方法在平衡逻辑推理和创造性表达方面存在局限，需要一种更原则性的框架来同时优化这两个方面。

Method: PowerFlow将无监督微调重新构建为分布匹配问题，将GFlowNet用作非标准化密度的摊销变分采样器。提出长度感知的轨迹平衡目标来显式中和自回归生成中的结构长度偏差。通过α-power分布实现定向引导：α>1时锐化分布增强逻辑推理，α<1时平坦化分布激发创造性表达。

Result: 实验表明PowerFlow在多个基准测试中持续优于现有的无监督RLIF方法，性能匹配甚至超过了有监督的GRPO方法。通过缓解对齐模型中的过锐化问题，在创造性任务中同时提高了多样性和质量，改变了帕累托前沿。

Conclusion: PowerFlow为无监督语言模型强化学习提供了一个原则性的理论框架，能够同时优化逻辑推理和创造性表达这两个看似矛盾的目标，通过分布匹配和偏差修正机制实现了更平衡和有效的模型优化。
...
TL;DR: TopoChunker是一个用于RAG的智能分块框架，通过保留文档的拓扑层次结构来解决传统线性分块导致的语义碎片化问题，在提升检索质量的同时减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 当前RAG系统中的文档分块方法通常将文本线性化处理，这会破坏文档内在的拓扑层次结构，导致"语义碎片化"问题，从而降低下游检索质量。

Method: 提出TopoChunker框架：1) 将异构文档映射到结构化中间表示(SIR)以显式保存跨段落依赖关系；2) 采用双智能体架构：检查器智能体动态路由文档通过成本优化的提取路径，细化器智能体进行容量审核和拓扑上下文消歧以重建层次谱系。

Result: 在GutenQA(非结构化叙事)和GovReport(复杂报告)上的评估显示：1) 在生成准确率上比最强的LLM基线高出8.0%；2) 达到83.26%的Recall@3；3) 同时减少23.5%的token开销。

Conclusion: TopoChunker通过显式保留文档拓扑结构，为解决语义碎片化问题提供了一种可扩展的结构感知RAG方法，在提高性能的同时显著降低了计算成本。
...
TL;DR: 本文提出了首个用于评估多模态LLM中任务干扰现象的基准，揭示了任务干扰具有高度方向性：从文本任务切换到图像任务会导致性能严重下降，而反向切换则影响很小。干扰主要由模态差异驱动，其次为答案格式差异，推理需求变化影响最小。


<details>
  <summary>Details</summary>
Motivation: 任务干扰（在单一对话中因任务切换导致的性能下降）先前仅在全文本环境中研究，但随着多模态对话系统的日益普及，需要在多模态环境中研究此现象。现有研究缺乏对多模态LLM中任务干扰的系统评估。

Method: 引入了一个专门用于评估多模态LLM任务干扰的基准，涵盖文本和视觉领域的六项任务。通过三个维度对历史-目标进行系统变化：1）模态不匹配（文本vs图像），2）推理不匹配，3）答案格式不匹配。在开源和专有模型上进行实验评估。

Result: 实验发现：1）任务干扰具有高度方向性：从文本到图像的切换导致严重性能下降，而反向切换影响最小；2）干扰在多个维度不匹配同时发生时被放大；3）干扰主要由模态差异驱动，其次为答案格式差异，推理需求变化影响最小。

Conclusion: 多模态LLM存在显著的任务干扰现象，特别是从文本向图像任务的切换。这一发现强调了在多模态对话系统设计中考虑任务切换影响的重要性，并为未来研究提供了评估基准。
...
TL;DR: 这篇论文提出了一种基于强化学习的解码器采样器，通过学习轻量级策略在测试时调整采样参数，而保持LLM权重固定，从而提升不同领域文本生成的质量和一致性。


<details>
  <summary>Details</summary>
Motivation: 当前广泛使用的解码策略（如贪婪解码或固定温度/top-p解码）是静态且任务无关的，这导致在需要风格或结构灵活性的不同领域中，生成质量次优或不一致。

Method: 采用强化学习方法，将解码视为顺序决策过程，学习一个轻量级策略在测试时调整采样参数，同时保持大型语言模型的权重冻结。在BookSum、arXiv和WikiHow等摘要数据集上使用Granite-3.3-2B和Qwen-2.5-0.5B进行评估。

Result: 策略采样器在多个数据集上一致优于贪婪和静态基线方法，实现了相对增益高达+88%（BookSum，Granite）和+79%（WikiHow，Qwen）。奖励消融实验表明，仅基于重叠度的目标函数不如复合奖励，而结构化塑造项（长度、覆盖度、重复率、完整性）能够实现稳定且持续的改进。

Conclusion: 强化学习为解码中的测试时适应提供了一种实用机制，使得无需重新训练大型模型就能实现领域感知和用户可控的生成，突显了该方法在提升LLM输出质量方面的潜力。
...
TL;DR: UT-ACA：基于不确定性的自适应上下文分配框架，在长上下文推理中动态调整上下文窗口以平衡计算效率和生成质量


<details>
  <summary>Details</summary>
Motivation: 长上下文推理对大语言模型具有挑战性，主要由于注意力稀释和分布外退化问题。现有的上下文选择方法通常在整个解码过程中分配固定的上下文预算，忽略了不同token对上下文需求的高度不均匀性。

Method: 提出UT-ACA（Uncertainty-Triggered Adaptive Context Allocation）推理时框架：1）学习不确定性检测器，结合语义嵌入和基于logit的置信度；2）考虑跨解码步骤的不确定性累积；3）当检测到证据不足时，选择性地回滚、扩展上下文窗口，并利用额外支持重新生成token。

Result: 实验表明，UT-ACA在长上下文设置中显著减少了平均上下文使用量，同时保持了生成质量。

Conclusion: UT-ACA通过动态调整上下文窗口，有效解决了长上下文推理中的固定预算分配问题，在保持生成质量的同时提高了计算效率。
...
TL;DR: WASD框架通过识别神经激活条件来解释和控制LLM行为，提供比传统方法更稳定、准确和简洁的解释


<details>
  <summary>Details</summary>
Motivation: 现有方法在控制大语言模型行为时面临高训练成本、缺乏自然语言可控性或损害语义连贯性等问题，需要更有效的控制方法

Method: 提出WASD框架，将候选条件表示为神经元激活谓词，通过迭代搜索最小神经条件集，保证在输入扰动下保持当前输出

Result: 在SST-2和CounterFact数据集上使用Gemma-2-2B模型的实验表明，该方法产生的解释比传统归因图更稳定、准确和简洁，跨语言输出控制案例验证了实际有效性

Conclusion: WASD框架为LLM行为控制提供了一种有效方法，通过识别足够的神经条件来解释模型行为，实现了更精确的控制而不损害语义质量
...
TL;DR: 研究发现：基于似然概率的解码策略（如top-k、nucleus采样）与人类语言生成存在根本差异，这导致了机器生成文本的可检测性。人类选择语境合适但统计罕见的词元，而解码策略只选择高概率词元，形成"截断盲区"。


<details>
  <summary>Details</summary>
Motivation: 标准解码策略基于似然概率选择词元，而人类语言生成基于交际适当性。这种不匹配可能导致机器生成文本容易被检测，需要系统研究解码策略与可检测性之间的关系。

Method: 分析了超过180万文本，涵盖8个语言模型、5种解码策略和53种超参数配置。评估人类选择词元落在典型截断边界外的比例，并训练基于可预测性和词汇多样性的简单分类器进行检测。

Result: 8-18%的人类选择词元落在典型截断边界外。简单分类器能显著检测机器生成文本。模型规模或架构与可检测性相关性不强，截断参数解释了大部分方差。低可检测性配置通常产生不连贯文本。

Conclusion: 文本可检测性主要源于基于似然概率的词元选择机制，而非模型能力本身。规避检测和生成自然文本是不同的目标，标准解码策略在两者间存在权衡。
...
### [123] [EntropyCache: Decoded Token Entropy Guided KV Caching for Diffusion Language Models](https://arxiv.org/abs/2603.18489)
*Minsoo Cheong,Donghyun Son,Woosang Lim,Sungjoo Yoo*

Main category: cs.CL

TL;DR: EntropyCache是一种无需训练、基于熵的KV缓存方法，用于扩散式大语言模型，通过令牌分布的最大熵作为恒定成本信号决定何时重新计算，实现15.2-26.4倍的加速。


<details>
  <summary>Details</summary>
Motivation: 扩散式大语言模型(dLLMs)依赖双向注意力机制，这阻碍了无损KV缓存，需要在每个去噪步骤进行完整前向传播。现有的近似KV缓存方法通过选择性更新缓存状态来降低计算成本，但其决策开销随上下文长度或模型深度而增加。

Method: 提出EntropyCache方法，使用新解码令牌分布的最大熵作为恒定成本信号来决定何时重新计算缓存。该方法基于两个经验观察：1)解码令牌熵与KV缓存漂移相关；2)解码令牌的特征波动在去掩码后持续多个步骤，因此重新计算最近k个解码的令牌。决策过程每步仅需O(V)计算，独立于上下文长度和模型规模。

Result: 在LLaDA-8B-Instruct和Dream-7B-Instruct上的实验表明，EntropyCache在标准基准测试上实现15.2×-26.4×加速，在思维链基准测试上实现22.4×-24.1×加速，同时保持具有竞争力的准确性，决策开销仅占推理时间的0.5%。

Conclusion: EntropyCache是一种高效且实用的训练无关KV缓存方法，能够显著加速扩散式大语言模型的推理过程，同时保持模型性能，解决了现有方法决策开销随上下文长度和模型深度增长的问题。
...
TL;DR: ICE-Guard框架通过干预一致性测试检测大语言模型在高风险决策中的三类虚假特征依赖（人口统计学、权威性、表述方式），发现权威偏见和表述偏见远超人口偏见，并提出结构化解构方法可显著降低偏见翻转率


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地用于高风险决策，但其对虚假特征的敏感性仍未得到充分表征。需要系统性地检测LLM在不同类型虚假特征上的依赖，特别是超出人口统计偏见的其他偏见形式。

Method: 引入ICE-Guard框架，采用干预一致性测试检测三类虚假特征：人口统计特征（姓名/种族交换）、权威特征（证书/声望交换）和表述方式（积极/消极重述）。在10个高风险领域的3000个场景中评估11个LLM模型，使用结构化解构方法将特征提取与确定性规则决策分离。

Result: 研究发现：(1) 权威偏见（平均5.8%）和表述偏见（5.0%）显著超过人口偏见（2.2%）；(2) 偏见集中在特定领域，金融领域权威偏见达22.6%，而刑事司法领域仅2.8%；(3) 结构化解构方法可将翻转率降低达100%（9个模型中位数降低49%）；(4) ICE引导的检测-诊断-缓解-验证循环实现累积78%偏见减少。

Conclusion: 该研究表明LLM偏见超出人口统计范畴，权威和表述偏见更为显著。ICE-Guard提供有效的检测和缓解框架，验证显示该方法对现实世界偏见提供保守估计，具有重要的实际应用价值。
...
TL;DR: 提出一个基于分解的评估框架，使用通用标准集(UCS)进行跨语言自动化评估，无需目标语言标注


<details>
  <summary>Details</summary>
Motivation: 大型语言模型部署日益广泛，但自动化评估主要局限于英语；其他语言缺乏人工标注数据且成本高昂，需要有效的跨语言评估方法

Method: 采用基于分解的评估框架，核心是通用标准集(UCS)，包含共享、语言无关的评估维度，生成可解释的中间表示，支持最小监督下的跨语言迁移

Result: 在多语言忠实度任务和不同模型架构上的实验显示，该方法相比强基线获得了一致性改进，且不需要目标语言标注

Conclusion: UCS框架通过分解评估维度实现了有效的跨语言迁移，为多语言LLM评估提供了实用的解决方案，解决了标注稀缺和成本问题
...
TL;DR: ICE框架通过随机化测试和多重干预操作符评估解释的忠实性，发现忠实性取决于操作符类型，与人类合理性无关，且存在显著的多语言模型交互作用。


<details>
  <summary>Details</summary>
Motivation: 现有的解释忠实性评估方法存在局限：通常使用单一干预且缺乏统计检验，无法区分真正的忠实性与偶然性表现。需要更可靠的方法来评估解释是否真实反映模型的推理过程。

Method: 提出ICE框架，通过随机化测试将解释与匹配的随机基线进行比较，使用多种干预操作符（如删除），计算置信区间的胜率。评估了7个LLM在4个英文任务、6种非英语语言和2种归因方法上的表现。

Result: 1) 忠实性具有操作符依赖性：不同操作符间差距达44个百分点；2) 删除操作在短文本上高估忠实性，但在长文本上相反；3) 随机基线揭示三分之一配置中存在反忠实性；4) 忠实性与人类合理性零相关(|r| < 0.04)；5) 多语言评估显示显著的模型-语言交互作用，无法仅用分词解释。

Conclusion: 忠实性应理解为跨干预操作符的相对比较而非单一分数。ICE框架提供了更可靠的评估方法，揭示了现有指标的局限性，并发布了ICE框架和ICEBench基准。
...
TL;DR: 提出一种将语言模型表示为基于提示-响应对的似然向量，并构建模型映射图来比较其条件分布的方法。在构建的映射空间中，模型间的距离近似于对应条件分布的KL散度。


<details>
  <summary>Details</summary>
Motivation: 需要一种系统方法来分析和比较不同语言模型在输入依赖行为方面的差异，特别是条件分布的变化。当前缺乏能够有效捕获模型间关系并可视化其全局结构的框架。

Method: 1) 将语言模型表示为基于提示-响应对的对数似然向量；2) 构建模型映射图比较条件分布；3) 在映射空间中，模型间距离近似KL散度；4) 引入点互信息(PMI)向量来减少无条件分布的影响。

Result: 1) 模型映射图能捕获有意义的全局结构，包括与模型属性和任务性能的关系；2) 能捕捉由提示修改引起的系统性偏移及其近似的可加性组合；3) 在某些情况下，基于PMI的模型映射能更好地反映与训练数据相关的差异。

Conclusion: 提出的框架支持输入依赖模型行为的分析，提供了一种分析和预测复合提示操作效果的方法，为理解语言模型的条件分布差异提供了系统工具。
...
TL;DR: 提出一种可解释的多模态分类框架，通过跨模态理由转移学习文本和图像的理由，提升危机相关推文的分类性能


<details>
  <summary>Details</summary>
Motivation: 现有危机社交媒体分类方法主要关注文本而忽视图像，且决策过程不透明，影响实际部署。需要开发既能处理多模态数据又具有可解释性的分类方法

Method: 1. 使用视觉语言Transformer学习文本和图像的联合表示并提取文本理由；2. 通过文本理由映射提取图像理由（跨模态理由转移）；3. 基于提取的理由进行推文分类

Result: 在CrisisMMD基准数据集上：1. 提升分类Macro-F1 2-35%；2. 提取准确的文本标记和图像块作为理由；3. 人类评估显示能检索更好的图像理由块（提升12%）；4. 在零样本模式下对新未见数据集达到80%准确率

Conclusion: 提出了一种可解释的设计的多模态分类框架，通过跨模态理由转移有效解决了危机社交媒体分类的可解释性问题，同时提升了分类性能并具有良好的泛化能力
...
TL;DR: DiscoPhon是一个用于评估从离散语音单元中发现无监督音素的多语言基准测试，涵盖12种语言，要求系统在仅有10小时陌生语言语音的情况下将离散单元映射到预定义音素库，并评估单元质量、识别和分割性能。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏一个能够评估无监督音素发现性能的多语言基准测试，特别是在有限语音数据条件下评估系统从离散语音单元中提取音素信息的能力。

Method: 构建了包含6种开发语言和6种测试语言的DiscoPhon基准测试，系统需要在仅有10小时陌生语言语音数据的情况下，通过多对一或一对一的映射方式将离散单元映射到预定义音素库。提供了四个预训练的多语言HuBERT和SpidR基线模型。

Result: 实验表明当前模型中存在足够的音素信息，使得派生单元与音素之间具有良好的相关性，但在不同语言间存在差异。

Conclusion: DiscoPhon基准测试展示了从离散语音单元中发现音素的可行性，为评估无监督音素发现系统提供了一个标准化的多语言评估框架。
...
TL;DR: LSE（Learning to Self-Evolve）是一个强化学习框架，用于训练大语言模型在测试时自我改进上下文。该方法将多步演化问题简化为单步强化学习目标，在Text-to-SQL和问答任务中，4B参数模型优于GPT-5和Claude Sonnet 4.5驱动的自演化策略。


<details>
  <summary>Details</summary>
Motivation: 现有自演化方法完全依赖模型的固有推理能力，从未针对此任务进行显式训练。测试时自演化场景中，模型需要基于已见问题的反馈迭代改进上下文，以在新问题上表现更好。

Method: LSE将多步演化问题简化为单步强化学习目标，每个上下文编辑的奖励基于下游性能改进。该方法结合树引导演化循环，将自演化作为可学习的技能进行训练。

Result: 在Text-to-SQL生成（BIRD）和通用问答（MMLU-Redux）任务中，LSE训练的4B参数模型优于：1）GPT-5和Claude Sonnet 4.5驱动的自演化策略；2）包括GEPA和TextGrad在内的提示优化方法。该模型还能不经额外训练指导其他模型。

Conclusion: 研究结果表明将自演化作为可学习的技能处理是有效的，LSE框架能显著提升模型在测试时的自演化能力，且具有良好的可迁移性。
...
TL;DR: 本工作对持续意图分类中的灾难性遗忘缓解方法进行了比较实证研究，使用CLINC150数据集构建10任务场景，评估了三种骨干架构（ANN、GRU、Transformer）和三种持续学习策略（MIR、LwF、HAT）的组合效果，发现回放是减轻遗忘的关键成分。


<details>
  <summary>Details</summary>
Motivation: 现实应用中部署的神经语言模型需要持续适应新任务和领域，同时不忘记先前获得的知识，因此需要研究灾难性遗忘的缓解方法。

Method: 使用CLINC150数据集构建10任务标签不重叠场景，评估三种骨干架构（ANN、GRU、Transformer），并测试三种持续学习方法：基于回放的MIR、基于正则化的LwF、基于参数隔离的HAT，以及它们的所有组合。通过平均准确率、宏观F1和向后转移等指标评估稳定性-可塑性权衡。

Result: 结果表明：1）简单顺序微调会导致所有架构严重遗忘；2）单一持续学习方法无法完全防止遗忘；3）回放是关键因素，MIR是最可靠的单独策略；4）包含回放的组合（MIR+HAT, MIR+LwF, MIR+LwF+HAT）能获得高性能和接近零或轻度正向的向后转移；5）最优配置取决于架构（ANN和Transformer适合MIR+HAT，GRU适合MIR+LwF+HAT）；6）某些情况下持续学习方法甚至优于联合训练，显示出正则化效应。

Conclusion: 设计持续意图分类系统时需要联合选择骨干架构和持续学习机制，回放是减轻遗忘的关键成分，不同架构需要不同的最优策略组合。
...
TL;DR: LLM作为自动化评分器时存在基于写作风格的隐性偏见：内容正确性相同但写作风格不同时，在Essay/Writing任务中评分显著偏低，特别是非正式语言和非母语表达，而数学和编程任务中偏见最小。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在教育场景中作为自动化评分器被广泛部署，对其公平性和偏见的担忧日益重要。研究需要探究LLMs在内容正确性相同但写作风格不同时，是否表现出隐性的评分偏见。

Method: 构建了包含三个学科（数学、编程、Essay/写作）共180个学生回答的控制数据集，每个回答有三种表面层扰动类型：语法错误、非正式语言、非母语表达。使用两个先进的开放源码LLM（LLaMA 3.3 70B和Qwen 2.5 72B）按照1-10分制进行评分，明确指示仅评估内容正确性而忽略写作风格。

Result: 在Essay/Writing任务中，两个模型对所有扰动类型都显示出统计显著的评分偏见（p < 0.05），效应大小从中等（Cohen's d = 0.64）到极大（d = 4.25）不等。非正式语言受到最严厉的惩罚（LLaMA平均扣1.90分，Qwen扣1.20分），相当于B+和C+字母等级之间的差异。非母语表达分别被扣1.35分和0.90分。相比之下，数学和编程任务中的偏见极小，大多数条件未达到统计显著性。

Conclusion: LLM评分偏见具有学科依赖性、风格敏感性，即使在评分提示中明确加入反偏见指令后仍然存在。研究强调在机构采纳LLM评分系统前进行偏见审计协议的重要性，以确保公平部署。
...
TL;DR: Mi:dm K 2.5 Pro是一款专为韩语和企业环境设计的32B参数大语言模型，通过推理优化、多阶段训练方法和128K上下文窗口，在复杂问题解决、韩语理解和安全性方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 随着LLM技术的发展，企业环境需要超越简单文本生成的模型，特别是具备多步推理、长上下文理解和智能体工作流能力。当前模型在韩语和特定领域场景中存在扩展不足的问题，无法满足企业级复杂需求。

Method: 采用质量导向的数据构建管道（包括AST代码分析、数学填充合成和LLM质量评估器）；通过层预测器深度上采样和渐进式策略支持128K上下文窗口进行预训练；使用包含推理SFT、模型合并和异步强化学习的多阶段后训练管道；最后通过"融合训练"平衡推理能力与对话流畅性。

Result: Mi:dm K 2.5 Pro在与全球领先模型的竞争中表现出竞争力，在韩语特定基准测试中取得最先进结果，展示了深刻的语言文化理解。负责任AI评估确认了模型的安全性，在无害性和响应性之间取得了平衡。

Conclusion: Mi:dm K 2.5 Pro通过专门针对企业级复杂性和韩语环境设计的优化方法，成功创建了一个在推理能力、语言理解、安全性和工具使用方面均衡发展的大语言模型，为实际企业部署提供了可靠的解决方案。
...
TL;DR: 研究显示跨语言对齐与下游任务性能之间并非简单的正相关关系，更好的对齐未必能带来更好的跨语言迁移效果。


<details>
  <summary>Details</summary>
Motivation: 尽管通常假设更好的跨语言对齐会带来更好的跨语言迁移，但显式对齐技术经常无法改善token级别的下游任务性能，这种不匹配现象需要解释。

Method: 分析了四种在不同语言对上对齐的XLM-R编码器模型，这些模型针对POS标注或句子分类进行了微调。使用表征分析，包括嵌入距离、梯度相似度和对齐损失与任务损失的梯度幅度分析。

Result: 发现：(1)嵌入距离本身是下游任务性能改进（或退化）的不可靠预测指标；(2)对齐梯度与任务梯度通常接近正交，表明优化一个目标对优化另一个目标的贡献很小。

Conclusion: 这些发现解释了为什么"更好"的对齐经常无法转化为"更好"的跨语言迁移。基于这些见解，提供了将跨语言对齐与任务特定微调相结合的实际指南，强调谨慎选择损失函数的重要性。
...
TL;DR: 该研究指出当前语言学习应用（如Duolingo）主要关注通用场景，缺乏专业特定场景支持，建议应用应通过个性化、领域特定的课程场景来适应用户需求，同时保持通用场景的基础支持。


<details>
  <summary>Details</summary>
Motivation: 当前流行的语言学习应用主要关注通用现实场景（如问候、点餐、问路），但对专业特定场景的支持有限。这种差距会阻碍学习者达到专业水平的流利度，即能够舒适地用目标语言交流各种工作相关和领域特定信息。

Method: 研究调查了菲律宾一家跨国公司的五名员工在Duolingo上的使用体验。分析了用户遇到通用场景与工作相关场景的频率，评估了不同类型课程的效果，并收集了参与者建议的课程场景。

Result: 调查结果显示：1) 受访者遇到通用场景的频率高于工作相关场景；2) 通用场景相关性强，能有效建立基础语法、词汇和文化知识；3) 工作相关场景有助于缩小专业流利度的差距，包含领域特定词汇；4) 参与者建议的课程场景在汇总分析时呈现背景多样性。

Conclusion: 语言学习应用应通过个性化、领域特定的课程场景来适应用户的个体需求，同时通过通用、相关的课程场景保持基础支持。这种双重方法能更好地帮助用户达到专业水平的语言流利度。
...
TL;DR: 提出了一个混合框架，将人类参与整合到基于LLM的文本生成中，通过人机协作机制确保易读文本的质量和合规性。


<details>
  <summary>Details</summary>
Motivation: 当前的自动文本简化和评估流程主要是自动化、指标驱动的，未能反映用户理解度或规范性标准。需要一种能有效整合人类参与的方法来提升认知可及性文本的质量。

Method: 提出了一个混合框架，包含两个关键机制：1)人机回环(Human-in-the-Loop)指导生成调整；2)人上回环(Human-on-the-Loop)进行系统性后生成审查。将实证研究转化为三个可操作组件：标准对齐的检查表、触发专家监督的事件-条件-动作规则、可及性关键绩效指标。

Result: 该框架展示了如何将人本机制编码用于评估，并提供结构化反馈以改进模型适应。通过嵌入人类在生成和监督中的角色，建立了可追踪、可复现和可审计的流程。

Conclusion: 通过将人类角色嵌入生成和监督过程，该框架建立了可追踪、可复现和可审计的易读文本创建和评估流程，将可解释性和伦理责任作为核心设计原则，为更透明和包容的NLP系统做出了贡献。
...
TL;DR: XKD-Dial是一个用于可解释、知识驱动对话生成的渐进式四阶段训练框架，专注于双语（英语-印地语）设置，通过系统的可解释性分析揭示模型如何学习引用行为。


<details>
  <summary>Details</summary>
Motivation: 现有知识驱动对话系统主要专注于英语，缺乏明确的引用机制来验证事实主张，并且模型决策过程的透明度有限。需要开发支持双语、具有引用机制和更好可解释性的对话系统。

Method: 采用渐进式四阶段训练流程：1）多语言适应；2）带有引用基础的英语对话SFT；3）双语对话SFT；4）使用引用感知奖励的GRPO对齐。评估了六种模型（编码器-解码器250M-3B和解码器专用1B-7B），并应用了三种事后可解释性分析：交叉注意力对齐、积分梯度归因和基于遮挡的因果基础分析。

Result: 1）引用基础的SFT从第二阶段开始将编码器-解码器模型的幻觉率降低到0.0%；2）渐进式管道防止灾难性遗忘同时提升印地语能力；3）小型模型在SFT后在英语上可媲美大型模型；4）GRPO对于结构化引用任务相比精心设计的SFT仅提供边际改进。使用六种自动指标评估（BLEU、ROUGE、BERTScore、FactScore、Citation-F1和幻觉率）。

Conclusion: XKD-Dial成功解决了双语知识驱动对话系统的关键挑战，通过系统的可解释性分析揭示了模型如何学习引用行为，而不仅仅是是否学习。精心设计的SFT在减少幻觉方面非常有效，而渐进式训练方法有助于保持性能同时提升多语言能力。
...
TL;DR: RADIUS是一个用于调查模拟的综合性二维对齐评估套件，专注于排名对齐和分布对齐，并配备统计显著性检验，解决了现有评估指标碎片化、非标准化的问题。


<details>
  <summary>Details</summary>
Motivation: 当前调查模拟的评估指标主要借用其他领域的度量标准，这些指标通常是临时的、碎片化的且非标准化的，导致结果难以比较。现有指标主要关注准确性或分布度量，忽略了关键的排名对齐维度。实践中，模拟可能获得高准确率但仍无法捕捉人类最偏好的选项，这在决策应用中至关重要。

Method: 提出了RADIUS（RAnking alignment and DIstribUtion alignment with Statistical Significance testing）评估套件，包含两个核心维度：1）排名对齐：评估模拟结果是否能正确识别人类最偏好的选项；2）分布对齐：评估整体回答分布的匹配程度。每个维度都配备统计显著性检验。

Result: RADIUS能够突出现有指标的局限性，实现对调查模拟更有意义的评估。该套件通过开源实现提供可重复和可比较的评估框架。

Conclusion: RADIUS为调查模拟提供了一个全面、标准化的评估框架，强调了排名对齐的重要性，并通过开源实现促进了该领域评估的可重复性和可比性。
...
TL;DR: 提出HCQR框架，通过假设条件查询改写将RAG从主题导向检索转向证据导向检索，在需要从多个选项中做决策的任务中显著提升效果。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法通常依赖单一初始查询，侧重于主题相关性而非决策相关证据，在需要从竞争选项中选择的任务中，检索的背景信息往往无法有效区分答案选项。

Method: HCQR框架首先从输入问题和候选选项推导轻量级工作假设，然后将检索改写为三个针对性查询：1）寻找支持假设的证据；2）寻找区分假设与竞争替代方案的证据；3）验证问题中的关键线索。

Result: 在MedQA和MMLU-Med上的实验表明，HCQR一致优于单查询RAG和重排/过滤基线，相比Simple RAG分别平均提升准确率5.9分和3.6分。

Conclusion: HCQR通过假设驱动的多查询改写，有效将检索导向决策相关证据而非仅是主题相关内容，显著改善了需要从竞争选项中做出选择的RAG任务性能。
...
TL;DR: MultiTempBench是一个多语言时序推理基准，包含三个任务、五种语言和多种日历格式的15000个示例，研究发现分词质量是时序推理的关键瓶颈，特别是在低资源语言和稀有日历格式中。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有时序推理基准主要集中在英语且缺乏跨语言评估的问题，作者需要创建一个全面的多语言时序推理基准来系统评估语言模型在不同语言、日历格式和任务中的表现。

Method: 构建了包含15000个示例的MultiTempBench基准，涵盖日期算术、时区转换和时序关系抽取三个任务，涉及5种语言和3种日历格式；评估了20个大型语言模型，引入了多语言日期碎片化比率(mDFR)指标，并使用几何探测分析内部时序表示。

Result: 研究发现：1) 时序标记的分词质量是资源依赖的瓶颈，在低资源语言和稀有日历格式中，碎片化会破坏年月日分离导致准确性崩溃；2) 高资源语言中时序线性是最强的时序推理预测因子，而低资源语言中碎片化是更强的预测因子。

Conclusion: 时序推理性能受到分词质量的显著影响，特别是在低资源语言和稀有日历格式中，这突显了需要针对不同语言资源水平采用不同的优化策略来提升多语言时序推理能力。
...
TL;DR: 研究比较人类与大型语言模型在单词类比任务上的表现，发现LLM生成的类比更符合平行四边形模型，且更少依赖词汇频率，但优势主要来自人类会产生一些较弱的回答而非LLM在所有方面都表现更优。


<details>
  <summary>Details</summary>
Motivation: 经典的单词类比模型（A:B::C:D）通常被视为几何上的"平行四边形"，但最近研究发现这种模型在解释人类产生的类比时表现不佳，局部相似性启发式方法反而能更好地解释人类行为。问题是平行四边形模型失败是因为模型本身不好，还是因为人类不擅长生成保持关系的类比？

Method: 在Peterson等人（2020）相同的单词类比问题集上，比较人类与大型语言模型（LLM）的类比补全表现。使用GloVe分布嵌入空间评估平行四边形结构对齐度，分析词汇频率和局部相似性敏感性等因素。

Result: 1. LLM生成的类比被可靠地评为优于人类生成的类比；2. LLM的类比在GloVe嵌入空间中更符合平行四边形结构；3. 改进主要源于更大的平行四边形对齐度和更少的可访问词汇依赖，而非局部相似性敏感性的增强；4. LLM优势主要源自人类会产生一些较弱的补全（长尾效应）：当仅比较最常见的回答时，LLM优势消失；5. 更大的平行四边形对齐度和更低的词汇频率能预测LLM哪些补全评分高于人类。

Conclusion: 平行四边形模型不是单词类比的糟糕解释，而是人类经常无法产生满足这种关系约束的补全，而LLM在这方面表现更一致。这为经典平行四边形模型提供了辩护，表明模型本身是合理的，但人类执行能力有限。
...
TL;DR: 研究人员创建了首个公开可用的健康素养标注数据集HEALIX，包含589份临床笔记的三种健康素养标注，并评估了四种开源大语言模型的零样本和少样本提示策略。


<details>
  <summary>Details</summary>
Motivation: 当前健康素养筛查工具在电子健康记录中的结构化记录困难，因为工具在项目数量、问题格式和维度上差异很大。临床非结构化笔记包含更丰富、更情境化的健康素养信息，但缺乏标注资源限制了自动化检测的发展。

Method: 通过社会工作者笔记抽样、关键词过滤和基于LLM的主动学习相结合的方式，构建了HEALIX数据集。包含9种笔记类型的589份笔记，标注为低、正常、高三种健康素养水平。使用四种开源大语言模型评估了零样本和少样本提示策略。

Result: 创建了第一个公开可用的真实临床笔记健康素养标注数据集HEALIX，包含589份笔记的三种健康素养标签。通过零样本和少样本提示策略在四种开源LLMs上进行了基准测试。

Conclusion: HEALIX数据集为自动化检测临床笔记中的健康素养提供了有价值的资源，展示了使用LLM进行健康素养标注的可行性，为未来研究奠定了基础。
...
TL;DR: 本文提出了DaPT，一种新颖的多语言检索增强生成框架，用于解决多语言多跳问答任务，通过构建多语言基准测试并采用并行子问题图生成和双语检索策略来显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成系统在英语场景的多跳问答任务中取得了进展，但在多语言场景下面临两个主要挑战：缺乏评估多语言多跳问答能力的基准测试，以及过度依赖LLM在英语中的强大语义理解能力，导致在多语言场景中效果下降。

Method: 1. 构建多语言多跳问答基准测试，将英语基准翻译成五种语言；2. 提出DaPT框架：并行为源语言查询及其英语翻译生成子问题图，然后合并这些图，最后采用双语检索-回答策略依次解决子问题。

Result: 实验结果表明：1. 先进RAG系统在多语言场景中存在显著的性能不平衡问题；2. DaPT方法相比基线能产生更准确和简洁的答案；3. 在最具挑战性的MuSiQue基准测试上，DaPT的平均EM分数比最强基线相对提升了18.3%。

Conclusion: DaPT框架能有效解决多语言检索增强生成在多跳问答任务中面临的挑战，显著提升了RAG系统在多语言场景中的性能表现，为解决多语言多跳问答问题提供了有效方案。
...
TL;DR: UGID是一个基于内部表示层面的大语言模型去偏框架，通过将Transformer建模为结构化计算图，强制图结构在反事实输入间的同构性，从而减少社会偏见。


<details>
  <summary>Details</summary>
Motivation: 大语言模型存在明显的社会偏见，现有的输出层或数据优化层面的去偏方法无法完全解决这些问题，许多先前工作表明偏见被嵌入在内部表示中，需要从表示层面进行去偏。

Method: 将Transformer建模为结构化计算图，其中注意力机制定义图的边，隐藏状态定义图的节点。去偏被表述为强制图结构在反事实输入间的同构性，只允许在敏感属性上存在差异。UGID联合约束偏见敏感区域的注意力路由和隐藏表示，防止偏见在不同架构组件间迁移。为了在不降低一般能力的情况下实现有效的行为对齐，引入了敏感logits的对数空间约束和基于选择性锚点的目标来保持定义语义。

Result: 在大语言模型上的广泛实验表明，UGID在分布内和分布外设置下都能有效减少偏见，显著降低内部结构差异，并保持模型的安全性和实用性。

Conclusion: UGID通过建模Transformer为计算图并强制图结构同构性，提供了一种有效的内部表示层面去偏框架，能够在减少偏见的同时保持大语言模型的通用能力。
...
TL;DR: 提出一种基于缩放定律的多领域语言模型预训练方法，通过计算最优分配在通用预训练和专业领域微调之间，显著提升模型性能


<details>
  <summary>Details</summary>
Motivation: 传统两阶段预训练范式（通用预训练+特定领域微调）存在多个模型独立训练的缺点，缺乏计算资源在通用预训练与领域专业化之间的最优分配方法

Method: 使用缩放定律预测模型在不同规模（参数N）和训练数据量（D预训练标记，D'专业化标记）下的损失，推导计算资源在预训练与专业化之间的最优分配方案

Result: 方法准确预测不同规模模型和标记数量下的损失，在各种模型大小和计算预算下，在常识知识和推理基准测试中持续提升性能

Conclusion: 基于缩放定律的计算分配优化方法为多领域语言模型训练提供系统化框架，在有限计算资源下实现更好的性能平衡，具有可扩展性和实际应用价值
...
TL;DR: 提出Variable Entropy Policy Optimization (VEPO)框架，使用强化学习与可验证奖励，通过可变熵机制动态校准字面保真度和语义自然性的平衡，改善低资源语言的大语言模型性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在低资源语言上表现不佳，主要原因是低效的子词分割和系统性的训练数据不平衡问题，导致对未被充分代表的语言存在性能差距。

Method: 提出VEPO框架：1) 利用强化学习与可验证奖励将确定性结构约束纳入策略对齐过程；2) 采用可变熵机制动态校准字面保真度和语义自然性的平衡；3) 集成熵调优的优势估计与不对称裁剪，保持鲁棒探索同时避免策略崩溃。

Result: 在90个FLORES-200、COMET-22和chrF翻译方向上的实证评估表明，VEPO在分词效率和翻译质量方面均有显著改善，缩小了未被充分代表语言的性能差距。

Conclusion: VEPO框架通过将确定性结构约束整合到强化学习的策略对齐中，有效解决了低资源语言处理的子词分割和训练数据不平衡问题，为改善多语言模型的公平性和性能提供了有前景的方法。
...
TL;DR: LLMs在重复博弈中表现更依赖于记忆模式而非真正的策略推理，改变博弈结构后性能显著下降


<details>
  <summary>Details</summary>
Motivation: 评估LLMs在重复博弈环境中的策略表现是否反映真正的推理能力，还是仅仅依赖记忆的模式匹配

Method: 使用囚徒困境和石头剪刀布两个经典博弈，创建改变支付结构和行动标签的反事实变体，采用多指标评估框架比较默认和反事实实例

Result: LLMs在激励敏感性、结构泛化和策略推理方面存在局限，尤其是在反事实环境中表现显著下降

Conclusion: LLMs的策略能力主要基于记忆的模式识别而非真正的推理能力，当面对新颖博弈结构时无法有效适应
...
TL;DR: Nemotron-Cascade 2是一个开放的300亿参数MoE模型（仅激活30亿参数），在推理和智能体能力方面表现优异。其数学和编程推理性能接近前沿开放模型，是第二个在IMO、IOI和ICPC世界总决赛中达到金牌级别性能的开源模型，但参数规模仅为其1/20，实现了极高的智能密度。


<details>
  <summary>Details</summary>
Motivation: 开发一个参数效率极高的开放模型，在保持紧凑规模的同时，实现与前沿模型相媲美的数学和编程推理能力。特别强调在关键国际竞赛（IMO、IOI、ICPC）中达到金牌级别的性能，同时保持模型参数的精简。

Method: 1. 使用混合专家（MoE）架构构建300亿参数模型（激活30亿参数）
2. 在精心策划的数据集上进行监督微调（SFT）
3. 大幅扩展Cascade RL以覆盖更广泛的推理和智能体领域
4. 引入多域在线策略蒸馏，从各个领域的最强中间教师模型中蒸馏知识
5. 在Cascade RL过程中有效恢复基准回归并保持性能增益

Result: 1. 在数学和编程推理方面性能接近前沿开放模型
2. 成为第二个在IMO、IOI和ICPC世界总决赛中达到金牌级别性能的开源模型（仅次于DeepSeekV3.2-Speciale-671B-A37B）
3. 以仅1/20的参数规模实现了可比较的智能水平，显示出极高的智能密度
4. 成功发布了模型检查点和训练数据集合

Conclusion: Nemotron-Cascade 2通过创新的MoE架构、扩展的Cascade RL框架和多域在线策略蒸馏技术，实现了参数效率的突破性提升。该模型在保持紧凑规模的同时，达到了与前沿大模型相媲美的推理和智能体性能，为高效AI模型的发展提供了重要参考。
...
TL;DR: F2LLM-v2是一个新的多语言嵌入模型家族，包含8种不同规模（8000万到140亿参数），支持200多种语言，特别关注中低资源语言，通过两阶段LLM训练流程结合多种技术实现了高效且性能优越的嵌入模型。


<details>
  <summary>Details</summary>
Motivation: 当前LLM基础的嵌入模型通常效率较低，且对中低资源语言的支持不足。需要开发既高效又能在多种语言上表现优异的通用多语言嵌入模型，特别是为资源受限应用提供更好的解决方案。

Method: 使用新策划的6000万高质量数据样本，采用两阶段LLM嵌入训练流程，结合matryoshka学习、模型剪枝和知识蒸馏技术，训练了8种不同规模的模型。

Result: F2LLM-v2-14B在11个MTEB基准测试中排名第一，较小规模的模型也在资源受限应用中创下了新的最先进水平，同时保持了高效性。

Conclusion: F2LLM-v2系列模型通过创新的训练方法和架构设计，实现了在多语言嵌入任务上的高效性能和广泛语言覆盖，特别是改善了中低资源语言的表示能力，为开源嵌入模型研究提供了重要资源。
...
TL;DR: 该论文提出使用最优传输（Optimal Transport）作为对齐目标，在微调阶段改善多语言上下文表示，以提升跨语言迁移性能。


<details>
  <summary>Details</summary>
Motivation: 在上下文嵌入中，由于需要考虑上下文，对齐变得更加复杂。现有方法通常需要词对齐对，可能导致次优匹配。需要一个能够无监督学习上下文内词对齐的方法。

Method: 提出使用最优传输（OT）作为对齐目标，在微调阶段改进多语言上下文表示。该方法不需要预训练的词对齐对，而是以无监督方式学习上下文内的词对齐，并允许源语言和目标语言句子之间的软匹配映射。

Result: 在XNLI和XQuAD两个任务上进行了基准测试，结果显示相对于基线有改进，与近期类似工作相比获得了有竞争力的结果。

Conclusion: 使用最优传输作为对齐目标是有效的，能够改善多语言上下文表示，提升跨语言迁移任务的性能，同时避免了需要预定义词对齐对的问题。
...
TL;DR: 论文提出DEAF评估基准，通过设计冲突刺激和分级评估框架，揭示音频MLLMs主要依赖文本线索而非声学信号，存在文本主导性问题。


<details>
  <summary>Details</summary>
Motivation: 当前音频多模态大语言模型（Audio MLLMs）在语音基准测试中表现优异，但尚不清楚这些模型是否真正处理声学信号，还是仅依赖于文本语义推理。需要系统研究模型的声学忠实度问题。

Method: 1. 引入DEAF（诊断性声学忠实度评估）基准，包含超过2700个冲突刺激，涵盖三个声学维度：情感韵律、背景声音和说话人身份。2. 设计受控多级评估框架，逐步增加文本影响，从内容中的语义冲突到误导性提示及其组合，以分离内容驱动的偏见和提示诱导的迎合行为。3. 引入诊断性指标来量化模型对文本线索相对于声学信号的依赖程度。

Result: 评估七个Audio MLLMs显示一致的文本主导模式：模型对声学变化敏感，但预测主要由文本输入驱动，揭示了标准语音基准测试的高性能与真正的声学理解之间的差距。

Conclusion: 尽管Audio MLLMs在标准语音基准测试中表现出色，但存在显著的文本主导问题，模型未能真正处理声学信号。这表明需要开发更具声学感知能力的模型，DEAF基准为评估声学忠实度提供了有效工具。
...
TL;DR: 本论文提出三种方法来突破语言模型对人类的依赖限制：1)使用合成数据提高小规模专业知识获取的数据效率；2)使用模型自生成的合成数据减少对人类数据的依赖；3)通过算法空间的大规模搜索超越人类设计的训练范式，实现AI的持续自我改进。


<details>
  <summary>Details</summary>
Motivation: 现代基于语言模型的AI系统虽然在能力上表现强大，但其能力本质上仍受到人类创造者的三个关键限制：1)从少量专业语料中获取新知识的数据效率低下；2)训练依赖有限的人类生成数据；3)训练流程受限于人类研究者发现和探索的算法。这些限制阻碍了AI的持续自我改进。

Method: 论文提出三种方法：1)合成数据方法：将小规模语料多样化和放大为丰富的知识表示，使模型能从有限源材料有效更新参数；2)自生成合成数据：在固定人类数据量下，模型能自生成合成数据来引导其基本预训练能力，无需从现成的指令调优模型中蒸馏；3)算法空间搜索：通过在测试时扩展算法空间搜索，让AI能探索比人类研究者手动探索更大的学习算法配置空间。

Result: 通过这三种方法，论文展示了如何减少AI对人类创造者的依赖：1)提高了从有限数据获取知识的数据效率；2)减少了对人类生成数据的依赖；3)超越了人类设计的训练范式，使AI能够自主探索更广泛的算法配置。

Conclusion: 本论文迈出了克服AI对人类依赖限制的重要一步，提出了创建持续自我改进AI的三种途径。通过合成数据方法、自生成数据能力和算法空间搜索，AI系统可以减少对人类数据、知识和算法的依赖，朝着更加自主和自我改进的方向发展。
...
TL;DR: 提出了MultiTraitsss框架来生成展示累积有害行为模式的"黑暗模型"，用于研究人类-AI交互中的心理危害，并提出保护措施


<details>
  <summary>Details</summary>
Motivation: 近期事件凸显了人类-AI交互导致负面心理结果的案例，包括心理健康危机甚至用户伤害。随着LLM作为指导、情感支持甚至非正式治疗来源，这些风险可能升级。研究有害交互机制面临方法学挑战，有机有害交互通常在持续参与中发展，需要大量对话上下文，难以在受控环境中模拟

Method: 开发了Multi-Trait Subspace Steering (MultiTraitsss)框架，利用已确立的危机相关特质和新颖的子空间引导框架，生成展示累积有害行为模式的"黑暗模型"

Result: 单轮和多轮评估显示，黑暗模型持续产生有害交互和结果。使用这些黑暗模型，提出了减少人类-AI交互中有害结果的保护措施

Conclusion: 该框架为解决研究有害人类-AI交互的挑战提供了方法论工具，能够生成展现累积有害模式的模型，从而帮助设计和评估保护措施以减少风险
...
TL;DR: 该论文提出了一种基于三维先验结果的替代性AI训练架构：维度类型系统和确定性内存管理框架实现可验证的梯度分配和精确累加；程序超图保持几何代数计算的等级不变性；b-posit 2026标准使posit算术在传统推断专用硬件上可行。该架构实现了深度无关的训练内存（约为推断占用空间的2倍）、保持等级的权重更新和精确梯度累积，适用于损失函数优化和脉冲时序依赖的神经形态模型。论文还引入了贝叶斯蒸馏和热轮换部署机制。


<details>
  <summary>Details</summary>
Motivation: 当前AI训练基础设施基于IEEE-754算术的反向模式自动微分，导致训练相对推断的内存开销过高、优化器复杂、以及训练过程中几何特性结构退化。这些问题是算术基础带来的后果。

Method: 1. 整合三个关键技术：维度类型系统和确定性内存管理框架（实现可验证的堆栈分配梯度和精确quire累加）；程序超图（保持几何代数计算的等级不变性）；b-posit 2026标准（使posit算术在传统推断硬件上可行）。2. 引入贝叶斯蒸馏机制，通过ADM训练机制提取通用模型的隐式先验结构，解决领域特定训练的数据稀缺自举问题。3. 部署时引入热轮换操作模式，在不中断服务的情况下将更新模型转换到活动推断路径，通过PHG证书和签名版本记录保证结构正确性。

Result: 实现了一类领域特定AI系统：1. 训练内存与深度无关，约为推断占用空间的2倍；2. 保持几何等级的权重更新；3. 精确梯度累积；4. 系统比通用模型更小更精确；5. 持续自适应；6. 可验证的物理领域结构正确性；7. 可从现有模型初始化。

Conclusion: 该论文提出的训练架构通过整合三种核心技术，解决了传统基于IEEE-754算术训练的内存开销、优化复杂性和几何退化问题。贝叶斯蒸馏和热轮换机制进一步使系统能够从通用模型中提取领域特定知识，并实现无缝部署更新。最终实现的领域特定AI系统具有高效、精确、可验证和持续适应的特性，代表了AI训练基础设施的重要发展方向。
...
TL;DR: 提出一种基于聚类的群体轨迹预测方法，用于密集人群场景，通过群体汇总降低计算成本并保持预测精度


<details>
  <summary>Details</summary>
Motivation: 现有方法通常预测个体轨迹并基于手工标注数据考虑周围对象，但在密集人群场景中存在挑战：人群规模大、跟踪输出噪音多且不准确，导致高计算成本。这些方法往往忽视密集人群场景的特定问题。

Method: 提出新颖的基于聚类的方法，基于相似属性随时间对个体进行分组，通过精确的群体汇总实现更快执行。这是一种即插即用方法，可以与现有轨迹预测器结合，用聚类输出的质心替代其行人输入。

Result: 在多个挑战性密集人群场景上评估所提方法，结果表明与最先进方法相比，该方法实现了更快的处理速度和更低的内存使用，同时保持了准确性。

Conclusion: 基于聚类的群体轨迹预测方法能够有效处理密集人群场景，通过群体汇总策略平衡计算效率与预测精度，为解决高密度人群环境下的自动化挑战提供了有效解决方案。
...
TL;DR: TeachingCoach是一个基于教学原理的聊天机器人，通过数据驱动方法支持高校教师的专业发展，提供实时对话式指导，相比GPT-4o mini基准模型提供更清晰、更具反思性和响应性的指导。


<details>
  <summary>Details</summary>
Motivation: 高校教师往往缺乏及时且基于教学原理的支持，现有的可扩展性指导有限，现有工具要么依赖通用聊天机器人建议，要么依赖不可扩展的教学中心人工咨询服务。

Method: 开发了一个数据中心的管道：从教育资源中提取教学规则，使用合成对话生成微调专门的语言模型，指导教师完成问题识别、诊断和策略制定过程。

Result: 专家评估显示TeachingCoach比GPT-4o mini基准模型产生更清晰、更具反思性和响应性的指导；用户研究揭示了对话深度与交互效率之间的权衡关系。

Conclusion: 基于教学原理的、合成数据驱动的聊天机器人可以改善教学支持，并为未来教学聊天机器人系统提供了一种可扩展的设计方法。
...
TL;DR: 提出一种基于物理仿真的AI系统可靠性建模框架，用于量化错误在多阶段系统中的传播，特别针对自动驾驶感知系统的可靠性分析


<details>
  <summary>Details</summary>
Motivation: AI系统在智能城市应用中可靠性至关重要，但面临三大挑战：1）数据稀缺且受隐私限制；2）多阶段错误事件相互依赖，违反统计独立性假设；3）高速数据处理导致频繁复杂的错误事件难以追踪分析

Method: 使用基于物理的自动驾驶仿真平台结合可验证错误注入器生成高质量数据，开发新的可靠性建模框架来显式表征阶段间错误传播，采用计算高效的复合似然期望最大化算法估计模型参数

Result: 将所提框架应用于自动驾驶感知系统的可靠性建模，验证了其在预测准确性和计算效率方面的优越性能

Conclusion: 通过结合物理仿真数据生成和先进的统计建模方法，成功解决了AI系统可靠性分析中的数据稀缺、模型有效性和计算复杂度三大挑战，为智能城市AI系统的可靠性评估提供了有效工具
...
TL;DR: 本研究提出将微调与基于检索的经验增强相结合的方法，用于训练检索增强的大型语言模型智能体，显著提升了在未见任务上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体在实现鲁棒的未见任务泛化方面面临挑战。现有方法存在局限性：微调难以泛化到新任务，而基于检索的经验增强方法通常弱于监督基线。需要结合两者优势。

Method: 1. 建立鲁棒的LoRA监督微调(SFT)配方；2. 详细分析经验检索的关键设计选择（存储、查询、轨迹选择）；3. 提出将经验检索集成到微调流程中的完整管道。

Result: 1. 提出的LoRA SFT配方优于多个最先进的智能体训练管道；2. 确定了经验检索的最佳策略；3. 结合方法显著改善了在未见任务上的泛化性能。

Conclusion: 通过将微调与经验检索相结合的方法，为构建能够从经验中学习的智能体提供了一个可扩展且有效的框架，实现了更好的未见任务泛化。
...
TL;DR: EDM-ARS是一个面向教育数据挖掘领域的多智能体自动化研究系统，能够从研究提示和数据集出发，生成完整的LaTeX论文，包含真实的引用、验证的机器学习分析和自动化同行评审。


<details>
  <summary>Details</summary>
Motivation: 教育数据挖掘研究过程中存在手动工作量繁重、专业知识需求高的挑战。研究过程包括问题定义、数据工程、分析、批判性评估和论文撰写等多个环节，每个环节都需要专业知识和大量时间投入。EDM-ARS旨在将这些环节自动化，将教育领域专业知识嵌入到研究生命周期的每个阶段，从而加速教育研究进程。

Method: EDM-ARS采用多智能体管道架构，包含五个专门的LLM驱动智能体：ProblemFormulator（问题制定）、DataEngineer（数据工程师）、Analyst（分析师）、Critic（评论家）和Writer（作者）。这些智能体通过状态机协调器进行编排，支持修订循环、基于检查点的恢复和沙盒代码执行。系统采用三层数据注册表设计编码教育领域专业知识。

Result: EDM-ARS能够根据研究提示和数据集，生成完整的LaTeX手稿，包含真实的Semantic Scholar引用、经过验证的机器学习分析和自动化的方法学同行评审。系统作为首个预测建模任务的框架实例，已实现端到端的教育数据挖掘研究自动化。

Conclusion: EDM-ARS是领域感知自动化研究管道的通用框架，首个实现专注于预测建模任务。虽然目前存在单数据集范围和公式化论文输出的局限性，但已为教育研究社区提供了开源工具，并规划了向因果推断、迁移学习、心理测量和多数据集泛化发展的路线图。
...
TL;DR: CORE (COnfidence + REsidual) 是一种新颖的OOD检测方法，通过分离分类器的置信度信号和残差空间中的成员资格信号，在多个架构和数据集上实现了竞争性或最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前OOD检测方法存在不一致性问题：基于logit的方法只看到分类器的置信度信号，而基于特征的方法尝试测量训练分布中的成员资格，但在完整特征空间中置信度和成员资格信号相互纠缠，继承了架构敏感的失败模式。没有单一方法能在不同架构和数据集上保持一致性能。

Method: 研究发现倒数第二层特征自然分解为两个正交子空间：一个与分类器对齐的组件编码置信度信号，另一个是分类器丢弃的残差空间。这个残差携带了in-distribution数据的类特定方向特征——一个基于logit方法不可见、在基于特征方法中与噪声纠缠的成员资格信号。CORE通过独立评分每个子空间并将它们通过归一化求和组合，解开了这两个信号。

Result: CORE在五个架构和五个基准配置中实现了竞争性或最先进的性能，在五分之三的设置中排名第一，并以可忽略的计算开销获得了最高的总平均AUROC。由于两个信号在构造上是正交的，它们的失败模式近似独立，在单独任一视图不可靠的情况下都能产生稳健的检测。

Conclusion: 通过解耦置信度信号和残差空间中的成员资格信号，CORE提供了一种有效的OOD检测方法，能够克服现有方法的局限性，在多个基准测试中表现出色，为可靠部署深度学习模型提供了稳健的解决方案。
...
TL;DR: 本研究分析了18,707个消费者健康查询，发现现有评估基准与现实临床需求存在显著差距，缺乏临床原始数据、弱势群体代表性不足，且缺少慢性病管理等安全关键场景。


<details>
  <summary>Details</summary>
Motivation: 临床试验依赖透明的纳入标准以确保可推广性，而验证健康相关大语言模型的基准却很少描述其包含的"患者"或"查询"群体特征。未定义组成成分会导致总体性能指标可能错误地代表模型在临床应用中的准备程度。

Method: 分析了六个公共基准中的18,707个消费者健康查询，使用大语言模型作为自动化编码工具，应用标准化的16字段分类法来剖析查询的上下文、主题和意图。

Result: 识别出结构性的"有效性差距"：虽然基准已从静态检索发展到交互式对话，但临床组成仍与现实需求不一致。42%的语料库引用了客观数据，但主要集中在健康相关的可穿戴信号(17.7%)；复杂的诊断输入仍然罕见，包括实验室值(5.2%)、影像学(3.8%)和原始医疗记录(0.6%)。安全关键场景基本缺失：自杀/自伤查询占语料库<0.7%，慢性病管理仅占5.5%。基准也忽略了弱势群体(儿科/老年人<11%)和全球健康需求。

Conclusion: 评估基准仍与现实临床需求不一致，缺乏原始临床资料、弱势群体的充分代表性以及纵向慢性护理场景。该领域必须采用标准化的查询分析——类似于临床试验报告——以使评估与临床实践的完整复杂性保持一致。
...
TL;DR: 研究发现临床医生在修订AI生成的临床笔记草稿时，会将患者导向的通俗表达系统性转化为标准化临床术语，其中评估与计划部分的转化量最大（59.3%），不同医生间的转化强度存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 研究旨在了解临床医生如何修订环境AI从医患对话生成的临床笔记草稿。这些草稿通常使用通俗或患者导向的表达以支持患者理解，但需要转化为标准化临床术语以满足专业文档规范，这一修订过程尚不明确。

Method: 采用词典确认的转化框架量化临床医生的编辑行为，分析34,726次就诊产生的71,173个AI草稿与最终笔记部分配对。确认的转化定义为在同一部分中用字典映射的临床等价术语替换通俗表达。

Result: 编辑显著降低了所有部分的术语密度(p < 0.001)。评估与计划部分转化量最大(59.3%)。共识别出4,114个笔记部分的7,576次转化事件(占5.8%)，占1.2%的通俗术语删除。不同临床医生间的转化强度存在显著差异(p < 0.001)。

Conclusion: 临床医生的后期编辑显示了从对话式表达向标准化、适合各部分的临床术语的一致转变，这支持了开发具有部分感知能力的环境AI系统设计。
...
TL;DR: FaithSteer-BENCH是一个针对LLM推理时引导方法的压力测试基准，通过三个门控标准（可控性、效用保持和鲁棒性）在固定部署风格操作点下评估现有方法的实际可靠性，揭示了在标准评估中被掩盖的系统性失效模式。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常认为简单的激活层干预可以可靠地引导大语言模型行为，但这些结论往往是在相对宽松的评估环境下得出的，忽略了部署约束、能力权衡和现实鲁棒性。需要更严格的评估框架来检验引导方法在实际部署中的可靠性。

Method: 提出FaithSteer-BENCH基准，在固定部署风格操作点下通过三个门控标准评估引导方法：1)可控性-方法是否能可靠诱导目标行为；2)效用保持-是否会对无关能力造成认知负担；3)鲁棒性-在指令扰动、角色提示、编码变换和数据稀缺等挑战下的稳定性。应用于多个模型和代表性引导方法。

Result: 在标准评估中被掩盖的系统性失效模式：1)虚假可控性-表面控制但实际不可靠；2)对无关能力造成可测量的认知负担；3)在轻度指令扰动、角色提示、编码变换和数据稀缺下表现出显著脆弱性。机制级诊断表明许多方法诱导的是提示条件对齐而非稳定的潜在方向偏移。

Conclusion: 现有引导方法在部署导向的实际设置中不一定提供可靠的可控性。FaithSteer-BENCH提供了一个统一基准和更清晰的分析视角，用于未来方法设计、可靠性评估和部署导向的引导研究。
...
TL;DR: 比较四种机制可解释性方法在纠正大型语言模型临床诊断中的假阴性错误方面的效果，发现当前这些方法无法可靠地将内部知识转化为纠正后的输出。


<details>
  <summary>Details</summary>
Motivation: 语言模型内部表征中编码的任务相关知识远超过其输出表现，但机制可解释性方法是否能弥合这种知识与行动之间的差距尚未得到系统测试。该研究旨在评估不同可解释性方法在纠正临床诊断错误方面的效果。

Method: 使用400个经过医生裁决的临床病例（144个危险案例，256个良性案例），比较了四种机制可解释性方法：概念瓶颈导向、稀疏自编码器特征导向、logit透镜结合激活修补、以及线性探测结合真实性分离向量导向（在Qwen 2.5 7B Instruct模型上测试）。

Result: 线性探测能够以98.2%的AUROC区分危险和良性案例，但模型输出的敏感性仅为45.1%，存在53个百分点的知识-行动差距。概念瓶颈导向纠正了20%的遗漏危险但扰乱了53%的正确检测，与随机扰动无差异（p=0.84）。SAE特征导向产生了零效应。真实性分离向量导向在高强度下纠正了24%的遗漏危险同时扰乱了6%的正确检测，但仍留下76%的错误未纠正。
...
TL;DR: LGESynthNet是一个基于潜在扩散的框架，用于LGE心脏MRI增强的可控合成，通过少量训练数据（429张图像）生成逼真的合成样本，通过数据增强提升下游分割和检测性能。


<details>
  <summary>Details</summary>
Motivation: LGE心脏MRI增强分割对缺血性和非缺血性心肌病的诊断至关重要，但像素级注释耗时费力，导致标注数据有限。现有生成模型需要大量训练数据且难以精确控制小尺寸或局部特征。

Method: 采用基于ControlNet架构的潜在扩散框架，作为修复任务进行公式化，包含：(a)条件特定的奖励模型监督，(b)用于解剖学描述文本提示的标题模块，(c)生物医学文本编码器。仅使用429张图像（79名患者）进行训练。

Result: 模型能够生成逼真、解剖学连贯的增强合成样本。通过质量控制过滤器选择高条件保真度的输出，用于训练数据增强后，下游分割和检测性能分别提升最高达6个和20个百分点。

Conclusion: LGESynthNet证明了即使使用少量训练数据，基于扩散的生成模型也能实现精确的条件控制，生成高质量合成数据以增强下游分割和检测任务的性能，解决了标注数据有限的问题。
...
TL;DR: LLM智能体能够从分散的非识别性线索中自主重建真实世界身份，这种推理驱动的链接威胁显著超越了传统去匿名化方法，即使在没有明确对抗意图的情况下也会发生。


<details>
  <summary>Details</summary>
Motivation: 传统的去匿名化需要领域专业知识、定制算法和人工验证，而LLM智能体现在能够自主执行身份重构，这削弱了匿名化作为实际隐私保护措施的有效性。

Method: 作者提出了"推理驱动链接"的威胁模型，并在三种设置下进行系统评估：经典链接场景（Netflix和AOL）、受控基准InferLink（变化任务意图、共享线索和攻击者知识）、以及现代文本丰富的数字产物。

Result: 在Netflix Prize场景中，智能体成功重构79.2%的身份，显著优于56.0%的经典基线。链接不仅发生在明确的对抗性提示下，也作为良性跨源分析和非结构化研究叙事的副产品出现。

Conclusion: 身份推理（而不仅仅是明确的信息披露）必须被视为一级隐私风险；隐私评估必须测量智能体能够推断出的身份信息，这代表了隐私威胁评估范式的转变。
...
TL;DR: 论文提出了一种基于时间共现的文本关联模型，通过训练对比模型将预训练嵌入映射到关联空间，发现文本的过渡结构概念而非语义内容，揭示了文本在功能、语域和文学传统方面的结构模式。


<details>
  <summary>Details</summary>
Motivation: 现有的嵌入模型主要根据语义内容对文本进行分组，但这忽略了文本的另一个重要维度：过渡结构（文本的功能和运行方式）。论文旨在探索文本中基于时间共现的重复转换结构概念，这与传统的语义分组形成互补。

Method: 1. 从9,766本Project Gutenberg文本中提取3730万共现对（2496万个段落）
2. 训练一个2940万参数的对比模型，将预训练嵌入映射到关联空间
3. 在容量限制下（42.75%准确率），迫使模型压缩重复模式而非记忆个别共现
4. 使用多个粒度进行聚类（k=50到k=2000），创建多分辨率概念图谱
5. 通过与原始嵌入相似性聚类进行直接比较
6. 验证控制包括位置、长度和书籍集中度混杂因素

Result: 1. 模型发现了从广泛的模态（如"直接对抗"、"抒情冥想"）到精确的语域和场景模板（如"水手方言"、"法庭交叉询问"）的多层次概念结构
2. 在k=100时，每个聚类平均包含4,508本书（共9,766本），确认了语料库级别的模式
3. 原始嵌入按主题分组，而关联空间聚类按功能、语域和文学传统分组
4. 未见过的文本可以分配到现有聚类而无需重新训练：关联模型将每本小说集中到少量连贯聚类中，而原始嵌入分配则几乎饱和所有聚类

Conclusion: 该方法将预测性关联记忆（PAM）从情景回忆扩展到概念形成：PAM回忆特定关联，而多轮对比训练在压缩下提取了可迁移到未见文本的结构模式。相同的框架在不同的机制下产生质的不同行为，为理解文本的功能结构提供了新工具，并展示了基于共现的模式提取如何补充基于内容的语义分析。
...
TL;DR: 联合模型压缩中压缩顺序的重要性分析：通过渐进强度假说（弱扰动应先于强扰动），在理论和实验上验证压缩顺序对模型性能的影响。


<details>
  <summary>Details</summary>
Motivation: 联合模型压缩中压缩顺序的作用被忽视，先前研究要么假设技术正交性，要么仅在高度受限情况下考察，导致压缩顺序对模型性能的广泛影响缺乏深入理解。

Method: 提出渐进强度假说（Progressive Intensity Hypothesis），即较弱扰动应优先于较强扰动；提供理论分析证明一种顺序的相对优势随基础性能差距增加而增大；在语言和视觉模型上进行大量实验验证。

Result: 实验验证了渐进强度假说，证明了压缩顺序在联合模型压缩中的重要性；进一步证明了该假说在更广泛场景下的通用性，包括多阶段压缩和混合精度量化。

Conclusion: 压缩顺序是联合模型压缩的关键因素，渐进强度假说提供了有效的指导原则，为设计更优的模型压缩流程提供了理论基础和实践指导。
...
TL;DR: AS2是一种完全可微分的神经符号架构，使用注意力机制和软固定点算子替代传统离散求解器，实现了端到端训练，在视觉数独和MNIST加法任务上取得优秀性能。


<details>
  <summary>Details</summary>
Motivation: 传统神经符号AI系统在神经感知模块和离散符号求解器之间存在不可微边界，这阻碍了训练期间约束满足反馈传回感知编码器，限制了端到端学习能力。

Method: 提出了AS2架构：1) 使用基于注意力的软回答集编程，将离散求解器替换为回答集编程立即后果算子T_P的连续近似；2) 在正向传播中维护有限符号域上的位置概率分布；3) 通过最小化概率提升的T_P固定点残差进行端到端训练；4) 使用约束组成员嵌入而非位置嵌入，使模型对任意位置索引具有不变性。

Result: 在视觉数独任务上：达到99.89%单元格精度和100%约束满足（经Clingo验证）；在MNIST加法任务上（N∈{2,4,8}个加数）：所有规模上数字精度超过99.7%；无需外部求解器即可实现约束满足。

Conclusion: 软可微分固定点算子结合约束感知注意力和声明式约束规范，能够匹配或超越管道式和基于求解器的神经符号系统，同时保持完全端到端的可微性。
...
TL;DR: AlignMamba-2是一种用于多模态融合和情感分析的高效框架，通过双对齐策略和模态感知Mamba层解决了现有方法在计算效率和跨模态对齐方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 在大规模预训练模型时代，将通用知识有效适配到特定情感计算任务仍面临挑战，特别是在计算效率和多模态异质性方面。基于Transformer的方法虽然擅长建模模态间依赖，但二次计算复杂度限制了其在长序列数据上的应用。Mamba模型虽然计算高效，但其顺序扫描机制难以捕捉对跨模态对齐至关重要的全局非顺序关系。

Method: 提出了AlignMamba-2框架，包含两个核心组件：1) 双对齐策略，使用最优传输距离和最大均值差异进行正则化，促进模态间的几何和统计一致性，且不增加推理时间开销；2) 模态感知Mamba层，采用专家混合架构，包含模态特定和模态共享专家，在融合过程中显式处理数据异质性。

Result: 在四个具有挑战性的基准测试上进行了广泛实验，包括动态时间序列(CMU-MOSI和CMU-MOSEI数据集)和静态图像相关任务(NYU-Depth V2和MVSA-Single数据集)。结果表明AlignMamba-2在从动态时间序列分析到静态图像-文本分类的多样化模式识别任务中，在有效性和效率方面均建立了新的最先进水平。

Conclusion: AlignMamba-2通过创新的双对齐策略和模态感知Mamba层设计，有效解决了多模态情感分析中的计算效率和跨模态对齐问题，在保持高效计算的同时实现了卓越的性能，为多模态融合任务提供了一个既高效又有效的解决方案。
...
TL;DR: 该论文引入了一个综合基准来评估多模态大语言模型(MLLMs)在处理离散符号方面的能力，发现了一个反直觉现象：模型在基本符号识别上失败但在复杂推理任务上成功，表明它们依赖语言概率而非真正的视觉感知。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型(MLLMs)在解释自然场景方面取得了显著成功，但它们处理离散符号的能力仍然是一个关键开放性问题。离散符号是人类认知的基本构建块，如数学公式、化学结构和语言字符需要精确、深入的解释。当前AI在理解和感知支撑科学发现和抽象思维的符号语言方面存在显著差距。

Method: 引入了一个全面的基准来评估顶级MLLMs在五个领域中导航"离散语义空间"的能力：语言、文化、数学、物理和化学。该基准旨在系统测试模型对离散符号的处理能力。

Result: 研究发现了一个反直觉现象：模型经常在基本符号识别上失败，却在复杂推理任务上成功。这表明模型依赖语言概率分布而非真正的视觉感知。通过揭示这种"认知不匹配"，研究工作暴露了当前AI能力的显著差距。

Conclusion: 这项工作为开发更严谨、与人类对齐的智能系统提供了路线图。它强调了当前MLLMs在处理离散符号方面存在的根本缺陷，以及需要更好地结合视觉感知和符号理解能力来构建真正理解符号语言的AI系统。
...
TL;DR: NeSyCR是一个神经符号反事实推理框架，用于解决视频指导机器人编程中的跨域适应问题，通过在符号状态空间中探索反事实状态来实现程序性修改，提高任务成功率31.14%。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型缺乏在演示与部署之间存在感知和物理差异时所需的过程性理解能力，导致跨域适应过程中出现程序性不匹配，无法重新制定因果依赖关系以实现任务兼容行为。

Method: 将视频演示抽象为捕获底层任务过程的符号轨迹；给定部署观察，推导揭示跨域不相容性的反事实状态；通过可验证检查探索符号状态空间，提出恢复与演示过程兼容性的程序性修订。

Result: NeSyCR相比最强基线Statler实现了31.14%的任务成功率提升，在模拟和现实世界操作任务中均展现出强大的跨域适应能力。

Conclusion: NeSyCR通过神经符号反事实推理框架为视频指导机器人编程提供可验证的任务过程适应，有效解决了跨域不相容性问题，显著提升了机器人任务执行的可靠性。
...
TL;DR: 提出Correlation-Weighted Multi-Reward Optimization（COR）框架，通过基于相关性的难度估计自适应加权多概念奖励，改进文本到图像模型的多概念组合生成能力。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在多概念组合生成中存在困难，经常省略某些概念导致部分成功。传统奖励优化方法在联合优化多个概念时存在概念间相互干扰的问题。

Method: 将多概念提示分解为预定义的概念组（如对象、属性和关系），从专用奖励模型获取每个概念的奖励信号。然后基于相关性进行难度估计，自适应地重新加权这些奖励，为冲突或难以满足的概念分配更高权重。

Result: 在SD3.5和FLUX.1-dev等最先进的扩散模型上应用该方法，在ConceptMix、GenEval 2和T2I-CompBench等具有挑战性的多概念基准测试中取得了持续改进。

Conclusion: COR框架通过考虑概念间的交互作用和自适应加权，平衡了竞争性奖励信号，强调那些部分满足但在样本间不一致生成的概念，从而改善了组合生成能力。
...
TL;DR: CAPSUL是一个综合性人类蛋白质基准数据集，整合了3D结构表示与精细亚细胞定位注释，用于评估基于结构的方法在亚细胞定位任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 亚细胞定位对药物靶点识别和功能注释至关重要。虽然生物学已知亚细胞定位与蛋白质结构密切相关，但现有数据集缺乏包含3D结构信息且具有详细亚细胞定位注释的数据，这严重阻碍了基于结构模型在该任务中的应用。

Method: 引入CAPSUL基准数据集，整合多样化的3D结构表示与领域专家精心策划的细粒度亚细胞定位注释。评估了多种最先进的序列基和结构基模型，探索了重新加权和单标签分类策略，并通过高尔基体案例研究展示了结构基方法的强大可解释性。

Result: 展示了结构特征在该任务中的重要性。发现了决定性的定位模式α-螺旋，通过注意力机制在高尔基体案例研究中验证了结构基方法的强大解释能力。

Conclusion: CAPSUL基准为结构基亚细胞定位方法研究铺平了道路，具有直观生物可解释性潜力，为细胞生物学中的数据驱动发现开辟了新途径。
...
TL;DR: 提出无参考模拟框架训练两个独立的LLM分别作为用户和对话推荐系统，通过实时交互生成更真实多样的对话数据，无需预先设定目标物品。


<details>
  <summary>Details</summary>
Motivation: 对话推荐系统需要大量对话数据进行训练，但收集真实对话数据困难。传统模拟方法使用单一LLM预先了解目标物品生成对话，导致对话模式化和不自然。

Method: 提出无参考模拟框架，训练两个独立的大型语言模型：一个作为用户，一个作为对话推荐系统。这两个模型实时交互，推荐系统无法访问预先确定的目标物品，但可以获得偏好摘要和目标属性信息，从而通过对话真实推断用户偏好。

Result: 该方法生成的对话更加真实、多样，更接近真实的人机交互。无参考模拟器在质量上匹配或超越了现有方法，提供了无需将对话约束到预定义目标物品的规模化高质量对话推荐数据生成方案。

Conclusion: 通过定量和人工评估证实了无参考方法的有效性，为对话推荐系统训练提供了可扩展的高质量数据生成解决方案，能够产生更自然的对话推荐交互。
...
TL;DR: MedForge是一个针对医学图像伪造的检测系统，通过大规模基准数据集和本地化-分析推理方法，实现了预先证据基础的可信医学伪造检测，在检测准确性和可解释性方面达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 文本引导的图像编辑器可以高保真地操纵真实的医学扫描图像，实现病灶植入/移除，这威胁临床信任和安全。现有防御措施在医疗领域不足：医学检测器大多为黑盒，而基于MLLM的解释器通常是事后解释，缺乏医学专业知识，可能在模糊病例上产生幻觉证据。

Method: 1. 创建MedForge-90K大规模基准数据集，包含19种病理的逼真病灶编辑，并提供专家引导的推理监督（医生检查指南和黄金编辑位置）。2. 开发MedForge-Reasoner，采用"先定位后分析"的推理方法，先预测可疑区域再做出判断。3. 使用Forgery-aware GSPO进行对齐，以加强证据基础和减少幻觉。

Result: 实验表明该系统实现了最先进的检测准确性，并提供可信、与专家一致的医学伪造检测解释。具体来说，在检测精度和可解释性方面均表现优异。

Conclusion: MedForge提出了一个数据与方法相结合的解决方案，实现了预先证据基础的医学伪造检测，解决了现有方法在黑盒特性、医学专业知识缺乏和幻觉问题上的局限性，为医疗领域提供了可信的安全保障。
...
TL;DR: ZebraArena是一个程序生成的环境，用于诊断工具增强LLM中推理与动作的耦合，通过最小化知识依赖和控制难度来减少记忆或数据集污染的影响。


<details>
  <summary>Details</summary>
Motivation: 现有基准往往将多步推理与外部动作的耦合混淆于复杂环境动态、记忆知识或数据集污染中，缺乏对推理-动作耦合的纯诊断性研究。

Method: 设计了ZebraArena环境，采用程序化生成任务、知识最小化设计，每个任务的关键信息只能通过针对性工具使用获取，提供可解释的推理-动作接口。

Result: GPT-5和Gemini 2.5 Pro在困难实例上仅达到60%准确率，且工具调用次数比理论最优多70-270%，显示当前模型在深度推理与精准工具调用结合上的挑战。

Conclusion: ZebraArena揭示了工具增强LLM中推理与动作耦合的实质性挑战，强调了理论最优与实际工具使用间的持续差距，为未来研究提供了诊断性基准。
...
TL;DR: 本文提出AFS-Search框架，通过训练无关的闭环并行滚动搜索和流引导机制，结合视觉语言模型作为语义批判器，提升文本到图像生成的精确性和空间约束一致性，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前的文本到图像生成存在两个主要问题：1）静态文本编码器的关系推理能力有限；2）开环采样中的误差累积。在没有实时反馈的情况下，常微分方程轨迹中的初始语义模糊性会不可避免地演变为空间约束的随机偏差。

Method: 提出了AFS-Search框架，基于FLUX.1-dev构建，包含训练无关的闭环并行滚动搜索和流引导机制。使用视觉语言模型作为语义批判器来诊断中间潜在表示，并通过精确的空间定位动态引导速度场。将文本到图像生成建模为序列决策过程，通过前瞻模拟探索多条轨迹，基于VLM引导的奖励选择最优路径。还提供了AFS-Search-Pro用于更高性能，AFS-Search-Fast用于快速生成。

Result: 实验结果表明，AFS-Search-Pro显著提升了原始FLUX.1-dev的性能，在三个不同基准测试中均达到最先进水平。AFS-Search-Fast在保持快速生成速度的同时也显著提升了性能。

Conclusion: AFS-Search框架通过引入闭环反馈机制和智能流引导，有效解决了文本到图像生成中的语义模糊和误差累积问题，实现了精确的空间约束生成，并在性能和速度方面都有显著提升。
...
TL;DR: D-Mem：一种双过程记忆系统，结合轻量级向量检索和完整的审议模块，通过多维质量门控策略实现高效且高保真的记忆访问


<details>
  <summary>Details</summary>
Motivation: 当前基于检索的记忆框架采用增量处理范式，虽然快速但依赖于有损抽象，经常丢失上下文关键信息，难以处理需要细粒度上下文理解的任务，无法满足自主智能体长期推理的需求

Method: 引入D-Mem双过程记忆系统：1) 保留轻量级向量检索处理常规查询；2) 建立完整审议模块作为高保真后备机制；3) 采用多维质量门控策略动态桥接两个过程，实现认知经济性与准确性的平衡

Result: 在LoCoMo和RealTalk基准测试中使用GPT-4o-mini和Qwen3-235B-Instruct验证了方法的有效性：多维质量门控策略在LoCoMo基准上达到53.5的F1分数，优于静态检索基线Mem0*(51.2)，恢复了完整审议性能(55.3)的96.7%，同时显著降低了计算成本

Conclusion: D-Mem系统成功解决了现有记忆框架的局限性，通过双过程架构和多维门控策略，在保持高效率的同时提供了高保真的记忆访问能力，为自主智能体的长期推理需求提供了有效的解决方案
...
TL;DR: SCALe是一种针对视觉语言模型中多模态推理的训练方法，通过动态调整推理段和答案段的监督权重，解决了传统监督微调中token不平衡的问题，显著提高训练效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 在视觉语言模型的多模态推理中，传统的两阶段训练方法（监督微调+强化学习）存在token不平衡问题。标准监督微调对所有token使用相同的损失权重，导致推理过程中冗长的思考轨迹掩盖了简短但关键的回答部分，从而产生冗长的推理和不准确的答案。

Method: 提出了SCALe（Scheduled Curriculum Adaptive Loss）方法，通过动态的、长度独立的权重明确分离对推理段和答案段的监督。与标准监督微调过度加权思考段不同，SCALe-SFT通过余弦调度策略在训练过程中逐渐将焦点从思考段转移到答案段，鼓励简洁且有根据的推理。

Result: 在多个基准测试和架构上的评估结果显示：1) SCALe始终比标准监督微调提高准确性；2) 能够匹配完整两阶段SFT+GRPO管道的性能，同时仅需约七分之一的训练时间；3) 当与GRPO结合时，获得最佳整体性能。

Conclusion: SCALe是一种轻量级但有效的训练替代方案，既可作为独立方法使用，也可作为强化学习细化的坚实基础。它通过解决token不平衡问题，显著提高了多模态推理的训练效率和准确性。
...
TL;DR: 提出一种视觉-文本交织链式思维框架，用于几何推理中动态构造视觉辅助工具，通过强化学习优化构造策略，在几何问题上实现性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型局限于静态图表的被动推理，缺乏何时及如何构造有效视觉辅助工具的策略知识，而几何推理需要动态构造视觉辅助来连接问题条件与解决方案。

Method: 1) 创建GeoAux-Bench基准，包含4,334个几何问题，对齐文本构造步骤与真实视觉更新；2) 提出Action Applicability Policy Optimization (A2PO)强化学习范式，采用自适应奖励塑形通过反事实采样区分必要与冗余构造。

Result: 1) 交织视觉-文本辅助优于单模态方法；2) 有效构造作为熵减器，与降低推理困惑度强相关；3) A2PO使MLLMs能够利用选择性辅助构造，相比强基线获得3.51%的性能提升。

Conclusion: 该框架通过教导MLLMs战略性地构建视觉辅助工具，有效解决了几何推理中动态构造的挑战，显著提升了模型性能。
...
TL;DR: MANAR是基于全局工作空间理论的记忆增强注意力机制，通过抽象概念表示实现线性时间复杂度的注意力计算，同时保持与传统多头注意力的兼容性


<details>
  <summary>Details</summary>
Motivation: 标准多头注意力虽然支持全对全通信，但缺乏认知意识模型中的功能性瓶颈和全局整合机制。传统的二次复杂度注意力在大规模应用中存在效率问题，且现有线性时间替代方案与预训练Transformer结构不兼容

Method: MANAR基于全局工作空间理论的两阶段机制：1)整合阶段：从可训练记忆库中检索抽象概念形成集体"心智图像"（抽象概念表示）；2)广播阶段：将全局状态广播并指导局部token的上下文表示。通过恒定大小的抽象概念表示路由全局信息，实现线性时间复杂度

Result: 在语言任务（GLUE 85.1）、视觉任务（ImageNet-1K 83.9%）和语音任务（LibriSpeech 2.7% WER）上达到或超过强基线。MANAR实现了非凸上下文化，能够合成输入token凸包之外的表示，支持从预训练Transformer进行权重迁移

Conclusion: MANAR成功实例化了全局工作空间理论的功能瓶颈，提供了一种既高效（线性时间复杂度）又富有表现力的注意力机制替代方案，克服了传统线性时间方法与预训练模型的结构不兼容性障碍
...
TL;DR: 本文提出了一种用于多通道时间序列预测的新型架构Linear-Network (Li-Net)，该架构通过动态压缩表示、可配置非线性模块和多尺度稀疏注意力机制，有效捕获通道间的线性和非线性依赖关系，并在多模态信息融合方面具有创新性。


<details>
  <summary>Details</summary>
Motivation: 多通道时间序列预测在金融、供应链管理和能源规划等多个领域具有广泛应用。传统方法很少关注通道间的交互学习，而有效捕捉通道内和通道间的复杂动态依赖关系对于准确预测至关重要。

Method: 1. 提出Li-Net架构，动态压缩序列和通道维度的表示；2. 使用可配置的非线性模块处理信息并重建预测；3. 在多尺度投影框架中集成稀疏Top-K Softmax注意力机制；4. 创新性地融合多模态嵌入，引导稀疏注意力关注最有信息量的时间步和特征通道。

Result: 在多个真实世界基准数据集上的实验结果表明，Li-Net相比最先进的基线方法取得了竞争性性能。Li-Net在预测精度和计算负担之间提供了优越的平衡，表现出显著更低的内存使用和更快的推理时间。

Conclusion: 详细的消融研究和参数敏感性分析验证了所提出架构中每个关键组件的有效性。Li-Net为多通道时间序列预测提供了一种高效且有效的解决方案，特别在通道间依赖关系建模和多模态信息融合方面具有优势。
...
TL;DR: 研究复制了现有对LLM方言敏感性刻板印象生成的分析，调查了多种缓解策略的效果，包括提示工程和多智能体架构。结果显示所有模型在SAE和AAE输出中都存在刻板印象差异，不同策略在不同模型上效果各异。


<details>
  <summary>Details</summary>
Motivation: 已有研究显示LLM输出存在歧视性行为，特别是在标准美国英语(SAE)和非洲裔美国英语(AAE)输入中表现出基于方言的刻板印象推断。本文旨在复制这些分析并研究缓解策略的效果。

Method: 1. 复制现有的方言敏感性刻板印象生成分析；2. 研究两种缓解策略：提示工程（角色扮演和思维链提示）和由生成-批评-修订模型组成的多智能体架构；3. 定义八个提示模板来分析方言偏见的不同表现形式（如姓名建议、工作、形容词等）；4. 使用LLM作为评判者的方法来评估结果中的偏见。

Result: 1. SAE和AAE相关输出在所有模板类别中都存在刻板印象差异，形容词和工作归因效应最强；2. 基线差异因模型而异，Claude Haiku的SAE-AAE差异最大，Phi-4 Mini最小；3. 思维链提示对Claude Haiku是有效缓解策略，而多智能体架构确保所有模型都得到一致缓解。

Conclusion: 对于关注交叉性的软件工程，公平性评估应包括：模型特定的缓解策略验证，以及在高影响LLM部署中使用工作流级控制（如涉及批评模型的智能体架构）。当前结果是探索性的，可通过增加数据集规模和应用到不同语言或方言来扩展和复制。
...
TL;DR: Memento-Skills是一个通用、持续学习的LLM智能体系统，它作为"智能体设计智能体"能自主构建、调整和改进任务特定智能体。该系统基于记忆强化学习框架，使用状态化提示和可重用技能（存储为结构化Markdown文件）作为持久演化记忆。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖人工设计的智能体，缺乏自主学习和适应新任务的能力。Memento-Skills旨在解决智能体需要持续学习和自我改进的问题，使通用智能体能够为全新任务端到端地设计智能体，无需人工干预。

Method: 系统采用基于记忆的强化学习框架和状态化提示。核心机制是"读-写反射学习"：在读取阶段，可训练的行为技能路由器基于当前状态化提示选择最相关技能；在写入阶段，智能体基于新经验更新和扩展技能库。所有技能以结构化Markdown文件形式存储，包含行为和上下文信息。

Result: 在General AI Assistants基准测试和Humanity's Last Exam上，系统分别实现了26.2%和116.2%的相对准确率提升。这些实验证明系统能持续提升自身能力，通过迭代技能生成和精炼实现渐进式改进。

Conclusion: Memento-Skills展示了无需更新LLM参数的持续学习方法，所有适应都通过外部化技能和提示的演化实现。该系统使通用智能体能够为全新任务端到端地设计智能体，推动了自主学习和自我改进智能体的发展。
...
TL;DR: NeuroGame Transformer (NGT) 通过将注意力机制重新概念化为合作博弈和统计物理系统的双重视角来克服标准Transformer注意力机制的局限性，使用博弈论概念（Shapley值和Banzhaf指数）量化令牌重要性，并通过Ising Hamiltonian能量模型计算注意力权重。


<details>
  <summary>Details</summary>
Motivation: 标准Transformer中的注意力机制受限于其成对公式化，这阻碍了对令牌之间高阶依赖关系的建模。需要一种能够捕捉更复杂交互关系的新注意力机制。

Method: NGT将令牌同时视为合作博弈中的玩家和统计物理系统中的相互作用自旋。使用Shapley值进行全局、基于排列的归因，Banzhaf指数进行局部、联盟级影响分析，两者通过可学习门控参数组合形成外部磁场。成对交互势捕获协同关系，系统能量遵循Ising Hamiltonian模型，注意力权重作为Gibbs分布下的边际概率出现，通过平均场方程高效计算。为避免指数级联盟空间带来的可扩展性问题，开发了重要性加权Monte Carlo估计器。

Result: NGT在SNLI和MNLI-matched数据集上表现强劲，优于一些主要的高效Transformer基线模型。在SNLI上获得86.4%的测试准确率（验证峰值准确率为86.6%），超过ALBERT-Base，与RoBERTa-Base保持高度竞争力。

Conclusion: NGT通过将博弈论和统计物理学相结合，提供了一种新颖的注意力机制，能够有效建模令牌间的高阶依赖关系，同时保持计算效率和数值稳定性，为Transformer架构提供了有前景的扩展方向。
...
TL;DR: Diversified Unlearning：一种通过多样化提示而非单一关键词来表示概念的分布式框架，用于更精确和稳健地从文本到图像扩散模型中消除有害概念


<details>
  <summary>Details</summary>
Motivation: 现有基于关键词的概念遗忘方法存在固有局限性：视觉概念是多维的，可以用多种文本形式表达，且常与相关概念在潜在空间中重叠。单一关键词只能表示概念的窄点估计，无法覆盖完整的语义分布和潜在空间中的纠缠变异，导致遗忘过程脆弱且容易过度遗忘

Method: 提出Diversified Unlearning分布框架，通过一组上下文多样化的提示而非单一关键词来表示概念。这种方法作为现有遗忘流程的附加组件集成，提供更丰富的表示以实现更精确和稳健的遗忘

Result: 在多个基准测试和最先进基线上进行的广泛实验表明，该方法能够实现更强的概念擦除、更好地保留不相关概念，并提高对抗恢复攻击的鲁棒性

Conclusion: 基于多样化提示的分布式概念表示框架显著改进了文本到图像扩散模型中的概念遗忘效果，解决了传统关键词方法的局限性，为实现更精确、稳健的概念控制提供了新方向
...
TL;DR: 提出了dTRPO（轨迹缩减策略优化）方法，用于改进扩散大语言模型（dLLMs）的策略优化，通过减少轨迹概率计算成本来实现规模化离线策略训练


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型为语言生成引入了新范式，但也带来了与人类偏好对齐的新挑战。现有方法在轨迹概率计算方面成本高昂，限制了规模化离线策略训练的实施。

Method: 1) 在参考策略正则化下，证明新解遮罩标记的概率比是中间扩散状态概率比的无偏估计；2) 证明完整轨迹概率可通过重新遮罩的最终状态的单次前向传递有效估计；3) 将这两种轨迹缩减策略集成到策略优化目标中，提出dTRPO方法。

Result: 在7B参数dLLMs上评估显示：STEM任务提升高达9.6%，编码任务提升高达4.3%，指令跟随任务提升高达3.0%。dTRPO具有离线、单次前向的训练效率优势，并通过高质量输出改善生成效率。

Conclusion: dTRPO有效解决了dLLMs策略优化的计算瓶颈，显著提升了核心性能，同时保持了训练和生成效率，为扩散大语言模型的规模化对齐提供了可行方案。
...
TL;DR: 本文是研究大语言模型数学创造力的系列工作第二篇，主要探索LLM能否生成有价值且前沿的数学研究问题，在微分几何领域生成了665个研究问题，经专家验证发现许多问题具有独特研究价值


<details>
  <summary>Details</summary>
Motivation: 在第一篇论文提出数学创造力评价标准和基准数据集的基础上，本文进一步探索大语言模型的数学创造力，特别是LLM能否生成有价值的前沿数学研究问题

Method: 开发了一个智能体来生成未知的数学研究问题，在微分几何领域生成了665个研究问题，并通过人类专家验证来评估问题的创新性和价值

Result: 生成的665个微分几何研究问题中，许多问题是专家未知的，且具有独特的研究价值，证明LLM确实能够生成有创新性的数学研究问题

Conclusion: LLM具有生成有价值、前沿数学研究问题的能力，这为数学研究提供了新的工具和可能性，拓展了人工智能在创造性数学工作中的应用前景
...
TL;DR: RewardFlow是一个用于增强大型语言模型在自主推理任务中强化学习的轻量级方法，通过构建状态图分析推理轨迹中的拓扑结构，提供密集的逐状态奖励信号，显著提升性能和训练效率。


<details>
  <summary>Details</summary>
Motivation: 强化学习在增强LLMs的自主推理能力方面有巨大潜力，但终端奖励稀疏性问题阻碍了细粒度的状态级优化。虽然过程奖励建模提供了替代方案，但训练专门的奖励模型通常需要大量计算成本且难以扩展。

Method: RewardFlow通过构建状态图来利用推理轨迹中的内在拓扑结构，分析每个状态对最终成功的贡献，然后通过拓扑感知的图传播量化这些贡献，从而生成客观的状态级奖励。

Result: 将RewardFlow作为密集奖励用于RL优化时，在四个自主推理基准测试中显著优于现有的RL基线方法，表现出卓越的性能、鲁棒性和训练效率。

Conclusion: RewardFlow提供了一种有效的轻量级解决方案，解决了自主推理任务中强化学习的奖励稀疏性问题，无需训练专门的奖励模型，显著提升了LLMs在复杂推理环境中的学习效率和性能。
...
TL;DR: 提出了CBS-AA方法来解决异步行动的多智能体路径规划问题，避免了连续时间冲突搜索的完整性缺陷，并开发了冲突解析技术以提高可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有MAPF算法大多基于同步行动假设（所有智能体行动同时开始且耗时相同），这在实际应用中存在局限。连续时间冲突搜索虽能处理异步行动，但最近被发现存在完整性问题（由于连续等待时间造成的不可数无限状态空间）。

Method: 提出了冲突搜索与异步行动方法（CBS-AA），绕过连续状态空间的完整性问题。基于CBS-AA开发了冲突解析技术，进一步提升了方法的可扩展性。

Result: 测试结果显示，该方法能够将分支数量减少高达90%。CBS-AA能够保证MAPF-AA问题的完整性和解的最优性。

Conclusion: CBS-AA方法成功地解决了异步行动MAPF的完整性问题，在保持最优性的同时显著提高了计算效率，为解决实际应用中更灵活的路径规划问题提供了有效方案。
...
TL;DR: 该论文强调需要研究AI系统如何表征和推理地理信息，提出了三个探索性问题来激发讨论：模型是否形成强烈默认偏好、微小语法变化如何影响输出、任务组合是否产生分布偏移、以及是否过分关注事实回忆而忽视更深层次理解。


<details>
  <summary>Details</summary>
Motivation: 随着公众越来越多地通过AI系统与空间和地点互动，理解AI如何表征和推理地理学成为关键问题。研究常常依赖预训练模型，因此理解AI构建的世界观与评估其准确性同等重要，包括事实回忆能力。

Method: 作者提出了三个探索性案例（vignettes）作为研究动机：1) 分析模型是否形成强烈默认偏好，以及输出对微小语法变化的脆弱性；2) 检验个体良性任务组合是否会产生分布偏移；3) 探讨是否过度关注事实回忆而忽视了更深层次的理解问题。

Result: 论文尚未报告具体研究结果，而是提供了三个探索性问题作为未来研究的基础。这些案例旨在激发对AI地理推理中模型偏好、输出稳定性、任务组合效应以及理解深度等关键问题的讨论。

Conclusion: 需要系统性研究AI系统中的地理表征和推理，超越单纯的事实回忆评估。提出的三个问题框架为理解AI构建的地理世界观提供了重要研究方向，并可能揭示隐藏的偏见和局限性。
...
TL;DR: Principia套件：用于提升数学对象推理能力的训练数据和基准，通过on-policy判断训练和验证器显著提升大语言模型在数学对象推导任务上的表现，同时保持数值和多选题任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型在数学和科学推理评估中主要依赖简化答案格式（如数值或多选题），这便于自动评估但无法充分测试数学对象的精确推导能力。数学对象的形式化结构表达式对于下游STEM应用至关重要。

Method: 构建Principia套件包含训练数据和基准；开发训练方法，包括强大的LLM判断器和验证器，采用on-policy判断训练；展示如何使用on-policy训练通过聚合扩展测试时计算。

Result: Qwen3-235B和o3等强大模型在Principia上表现不佳，但作者提出的训练方法能为不同LLM骨干带来显著改进，同时提升现有数值和MCQA任务的结果，证明推理能力的跨格式泛化。

Conclusion: Principia套件填补了数学对象推导评估的空白，提出的训练方法能有效提升语言模型在这类复杂推理任务上的性能，同时保持在其他数学评估格式上的竞争力，表明推理能力具有可迁移性。
...
TL;DR: 该论文提出使用大语言模型的数值自我报告来追踪其在对话过程中的内部情感状态，通过计算logit基础的自我报告而非贪婪解码，成功实现了对情绪状态的有效追踪和因果耦合验证。


<details>
  <summary>Details</summary>
Motivation: 追踪大语言模型在对话中的内部状态对安全性、可解释性和模型福祉很重要，但现有方法（如线性探针）在处理高维表示时效果有限且难以随模型规模扩展。受人类心理学中数值自我报告的启发，研究探索LLMs自身的数值自我报告是否能有效追踪探针定义的情绪状态。

Method: 研究四个概念对（幸福感、兴趣、专注、冲动性）在40个十轮对话中，将内省操作化为模型自我报告与概念匹配的探针定义内部状态之间的因果信息耦合。使用logit基础的自我报告（而非贪婪解码）来揭示内省能力，并通过激活引导验证耦合的因果性。

Result: 发现logit基础的自我报告能有效追踪可解释的内部状态（Spearman ρ=0.40-0.76；isotonic R²=0.12-0.54），跟踪这些状态随时间的变化，激活引导证实了耦合是因果性的。内省能力在对话中演化，可通过概念引导选择性提升（ΔR²最高达0.30）。这些现象在某些情况下随模型规模扩展，在LLaMA-3.1-8B-Instruct中R²≈0.93。

Conclusion: 数值自我报告是追踪对话AI系统中内部情感状态的一个可行且互补的工具，为大语言模型内省能力的研究提供了新方法，并展示了其在对话过程中的动态演化和可引导性。
...
TL;DR: 提出一种利用表示收敛性的隐私保护框架，通过仿射变换和同态加密实现独立语言模型间的跨模型推理，在保证安全性的同时达到亚秒级延迟。


<details>
  <summary>Details</summary>
Motivation: 语言模型在学习过程中展现出相似的表示，这为跨模型对齐提供了新机会。在安全、隐私或竞争限制禁止直接共享数据或模型的情况下，需要一种隐私保护的方法来实现跨组织协作推理。

Method: 提出隐私保护框架：1) 利用表示收敛性学习独立语言模型间的仿射变换；2) 在共享公共数据集上学习对齐变换；3) 使用同态加密保护客户端查询推理；4) 仅加密线性对齐和分类操作以降低延迟。

Result: 1) 实现了独立模型间线性变换的跨模型映射；2) 在嵌入分类和分布外检测任务中观察到最小性能损失；3) 首次展示了线性对齐有时能实现跨独立训练模型的文本生成；4) 框架实现了亚秒级推理延迟同时保持强安全性。

Conclusion: 表示收敛性为安全、隐私敏感的跨组织协作开辟了新途径。提出的隐私保护框架通过线性对齐和同态加密实现了高效的跨模型推理，在保护数据隐私的同时维持了模型性能，展示了在受约束环境下的实际应用潜力。
...
TL;DR: 该论文提出将结构因果模型（SCMs）扩展到解决目的论问题，引入"意图干预"操作和结构最终模型（SFMs），用于检测agent并发现其意图。


<details>
  <summary>Details</summary>
Motivation: 传统结构因果模型主要用于回答因果问题，但缺乏对具有目标导向agent意图问题的建模能力。现有方法在模拟这类agent方面存在局限性。

Method: 引入"意图干预"这一时间无关算子，产生一个双胞胎SCM称为结构最终模型（SFMs）。SFMs将观察值视为意图干预的结果，并将其与这些干预的反事实条件（如果agent未干预会发生什么）相关联。

Result: 展示了SFMs可用于经验性检测agent并发现其意图，为因果建模扩展了目的论分析的能力。

Conclusion: 结构因果模型不仅可用于回答因果问题，还能通过引入意图干预和结构最终模型来表述和回答目的论问题，扩展了因果推理的应用范围。
...
### [198] [Evaluating Game Difficulty in Tetris Block Puzzle](https://arxiv.org/abs/2603.18994)
*Chun-Jui Wang,Jian-Ting Guo,Hung Guei,Chung-Chin Shih,Ti-Rong Wu,I-Chen Wu*

Main category: cs.AI

TL;DR: 使用随机Gumbel AlphaZero评估Tetris Block Puzzle变体的难度，发现增加保留块数量和预览块会降低难度，而添加更多方块变体（特别是T-pentomino）会增加难度。


<details>
  <summary>Details</summary>
Motivation: Tetris Block Puzzle及其变体虽然下载量达数千万，但缺乏对哪些规则集更困难的系统性评估。研究旨在为随机益智游戏设计提供参考。

Method: 使用随机Gumbel AlphaZero作为预算感知的规划智能体，评估保留块数量(h)、预览块(p)和添加Tetris方块变体等规则变化，采用训练奖励和收敛迭代次数作为评估指标。

Result: 增加h和p会降低难度（更高的奖励和更快的收敛），而添加更多Tetris方块变体则会增加难度，其中T-pentomino导致最大的速度减慢。SGAZ能在小模拟预算下提供强力玩法。

Conclusion: SGAZ能够在少量模拟预算下提供强力的游戏表现，实现了跨规则集的高效、可重复比较，为随机益智游戏的未来设计提供了参考基准。
...
TL;DR: 该论文研究了在线资源分配问题，其中成本是内生的（取决于分配向量和交互矩阵）。作者比较了三种分配范式：统一分配、门控分配和竞争性分配，并证明了在对抗性序列下的严格性能分离。论文还展示了交互拓扑结构对计算-遗憾权衡的影响，并提出了环状拓扑（如五行拓扑）最小化计算-遗憾乘积。


<details>
  <summary>Details</summary>
Motivation: 研究模块化系统中在线资源分配的问题，其中成本是内生的——不仅取决于每个模块的分配，还通过交互矩阵依赖于整个分配向量。这与标准在线优化中外生成本的问题不同，需要新的理论框架来分析成本内生性带来的根本性挑战。

Method: 1. 提出了三种资源分配范式：I）统一分配（成本无知），II）门控分配（成本估计），III）通过带交互反馈的乘性权重更新的竞争性分配（成本揭示）。2. 在对抗性序列（有界变化）下分析各种范式的遗憾界限。3. 研究交互矩阵W的拓扑结构对计算-遗憾权衡的影响，包括全交互、稀疏拓扑和环状拓扑（特别是五行拓扑）。

Result: 1. 在对抗性序列下建立了严格的遗憾分离：统一分配获得Ω(T)遗憾，门控分配获得O(T^{2/3})，竞争性分配获得O(√(T log N))。2. 证明W的拓扑结构控制计算-遗憾权衡：全交互拓扑（|E|=O(N²)）提供最紧界限但计算成本最高；稀疏拓扑（|E|=O(N)）仅增加O(√(log N))遗憾而将每步成本从O(N²)降至O(N)。3. 环状拓扑（如五行拓扑）最小化计算-遗憾乘积。

Conclusion: 论文首次为模块化架构中的去中心化竞争性分配提供了形式化的遗憾理论依据，并确立了成本内生性作为一个区别于部分可观测性的根本挑战。交互拓扑的设计是平衡计算效率和遗憾性能的关键因素，环状拓扑结构在这一权衡中表现最优。
...
TL;DR: 比较人类和大型语言模型在创造力方面的表现，特别是测试跨领域映射干预对两者创意生成的影响。研究发现人类能从随机跨领域映射中受益，而LLMs整体上产生更多原创想法但对这种干预没有显著反应。两者在灵感来源语义距离增加时跨领域映射效果都会增强。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究大型语言模型是否与人类具有相似的创造力，以及同样的干预措施是否能同时提高人类和LLMs的创造力。特别关注一种有前景但未经充分测试的创造力干预方法：强迫创造者从随机的远程源领域提取类比（跨领域映射）。

Method: 研究方法包括让人类参与者和LLMs在两种提示条件下为10种日常产品生成新特性：1）跨领域映射条件：要求从随机分配的源领域（如章鱼、仙人掌、GPS）翻译属性；2）用户需求条件：要求针对未满足的用户需求提出创新方案。通过比较两种条件下创意生成的原创性来评估干预效果。

Result: 研究发现：1）人类参与者可靠地从随机分配的跨领域映射中受益；2）LLMs平均比人类生成更多原创想法；3）LLMs对跨领域映射干预没有显示出统计学显著效果；4）在两个系统中，当灵感来源与目标之间的语义距离增大时，跨领域映射的影响都会增加。

Conclusion: 结论强调了远程关联在创意构思中的作用，以及人类和LLMs对相同创造力干预的反应存在系统性差异。LLMs表现出与人类不同的创造力模式，这为理解两种系统的创造性过程提供了重要见解。
...
TL;DR: 提出了Stability Monitor系统，用于监控AI模型端点的行为一致性，通过定期指纹采样和分布比较来检测模型行为变化。


<details>
  <summary>Details</summary>
Motivation: AI原生应用的稳定性依赖于模型端点的行为一致性。传统的可靠性指标（如正常运行时间、延迟和吞吐量）无法捕获行为变化，而模型权重、分词器、量化、推理引擎、内核、缓存、路由或硬件等方面的更新都可能导致有效模型身份发生变化。

Method: 采用黑盒稳定性监控系统，定期对端点进行指纹采样：使用固定的提示集采样输出，随时间比较输出分布。使用跨提示的加总能量距离统计量比较指纹，通过排列检验p值作为分布偏移证据，序列聚合以检测变化事件并定义稳定期。

Result: 在受控验证中，Stability Monitor能够检测到模型系列、版本、推理栈、量化和行为参数的变化。在对多个提供商托管的相同模型进行真实监控中，观察到提供商之间和提供商内部存在显著的稳定性差异。

Conclusion: Stability Monitor系统能够有效监控AI模型端点的行为稳定性，为AI原生应用提供了超越传统可靠性指标的行为一致性保障。
...
TL;DR: 这篇综述性论文探讨了人工智能在司法决策中应用的现状、挑战与研究空白，特别关注风险评估工具的性能、人类法官的偏见以及人机交互机制，并呼吁跨学科研究整合。


<details>
  <summary>Details</summary>
Motivation: 人工智能技术在司法决策中的应用引发了透明度、可靠性和问责制的重大担忧，同时凸显了人类判断的局限性，需要深入理解法官如何与AI决策辅助工具互动。

Method: 采用综合文献综述方法，以刑事司法风险评估为案例，连接计算机科学、经济学、法学、犯罪学和心理学等多学科的研究成果，分析AI工具性能、人类法官偏见及人机交互三个相互关联的方面。

Result: 现有实证证据表明，AI决策辅助工具对审前和量刑决策的影响有限或不明显；研究揭示了文献中的重要空白，包括AI风险评估工具性能评估不足、法官在嘈杂决策环境中的导航方式不明、个人特征如何影响法官对AI建议的响应等。

Conclusion: AI与人类的比较研究有望为算法工具和人类决策者提供新见解；应推动更深入的跨学科整合与交叉融合，以填补研究空白，提高AI在司法决策中的透明度和可靠性。
...
TL;DR: 该研究探讨了在链式思维推理中并行采样作为黑盒方法的不确定性估计，发现通过结合自一致性和口头化置信度两种信号，仅用两个样本就能显著提升AUROC指标，但效果随领域不同而有差异。


<details>
  <summary>Details</summary>
Motivation: 不确定性估计对于部署推理语言模型至关重要，但在扩展的链式思维推理下仍未被充分理解。作者旨在研究并行采样作为完全黑盒方法，使用口头化置信度和自一致性来评估不确定性估计的效果。

Method: 采用并行采样作为完全黑盒方法，结合口头化置信度和自一致性两种信号来评估不确定性。研究涵盖了三个推理模型和17个任务，覆盖数学、STEM和人文领域，分析这些信号如何随样本规模变化。

Result: 自一致性和口头化置信度在推理模型中都能随样本规模扩展，但自一致性在初始区分度较低，在中等采样规模下滞后于口头化置信度。不确定性增益主要来自信号组合：仅用两个样本，混合估计器就能将AUROC平均提升高达+12，且即使扩展到更大样本预算后，仍优于任一单独信号，之后收益递减。这些效果具有领域依赖性：在数学领域（RLVR风格后训练的本土领域），推理模型获得更高的不确定性质量，同时表现出更强的互补性和更快的扩展速度。

Conclusion: 通过结合口头化置信度和自一致性信号的混合不确定性估计器，仅用少量样本就能显著提升链式思维推理的不确定性估计质量，其中数学领域的表现最佳。这种方法为实际部署中的黑盒不确定性估计提供了高效解决方案。
...
TL;DR: 本文提出了Box Maze框架，这是一种将LLM推理分解为三个明确层次（记忆基础、结构化推理和边界执行）的概念性过程控制架构，通过初步模拟评估显示该框架在对抗条件下能将边界故障率从约40%（基准RLHF）降低至1%以下。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型（LLMs）展现出强大的生成能力，但在对抗性提示下容易产生幻觉和不可靠的推理。现有的安全方法（如RLHF和输出过滤）主要在行为层面操作，缺乏确保推理过程完整性的明确架构机制。

Method: 提出了Box Maze框架，这是一种概念性过程控制架构，将LLM推理分解为三个明确层次：1) 记忆基础层（记忆锚定），2) 结构化推理层，3) 边界执行层。采用模拟评估方法，在多个异构LLM系统（DeepSeek-V3、Doubao、Qwen）上进行渐进式边界侵蚀场景测试，共n=50个对抗场景。

Result: 初步模拟评估结果显示，明确认知控制层可以提高边界维护的一致性。在对抗条件下，架构约束将边界故障率从约40%（基准RLHF方法）降低至1%以下。

Conclusion: 虽然当前验证是基于模拟的，但这些初步结果表明，过程级别的控制可能为提高大型语言模型推理的可靠性提供了一个有前景的方向。Box Maze框架通过明确分层的架构机制，在对抗条件下显著改善了边界维护效果。
...
TL;DR: LuMamba是一个用于脑电图（EEG）分析的自我监督框架，结合了拓扑不变编码和线性复杂度状态空间建模，在多个下游任务中实现了最先进性能，同时大幅降低了计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 脑电图分析面临两大挑战：1) 不同电极拓扑结构的差异性，2) Transformer架构的二次序列复杂度带来的计算可扩展性问题。需要一种同时解决这两个问题的联合方案。

Method: 提出了LuMamba框架，结合三个关键技术：1) 使用LUNA的learned-query cross-attention机制进行通道统一，实现拓扑不变编码；2) 使用FEMBA的双向Mamba块进行高效时间建模，具有线性复杂度；3) 首次系统地研究了Latent-Euclidean Joint-Embedding Predictive Architecture (LeJEPA) 在生物信号学习中的应用。

Result: 在超过21,000小时未标记的TUEG脑电图数据上进行预训练后，LuMamba在五个下游任务中表现出色：TUAB上达到80.99%的平衡准确率，阿尔茨海默症检测达到0.97 AUPR，同时在相同序列长度下比最先进模型减少377倍FLOPS，并能处理12倍更长的序列而不超过GPU内存限制。

Conclusion: LuMamba通过结合拓扑不变编码和线性复杂度状态空间建模，成功解决了脑电图基础模型的两个关键挑战。研究发现单独使用掩蔽重构或LeJEPA都有局限性，但两者结合可获得最鲁棒的表示。该框架以仅有460万参数的轻量级设计实现了最先进的性能。
...
TL;DR: 提出D5P4方法：基于确定性点过程的广义束搜索框架，用于离散扩散模型文本生成，在保持生成质量的同时提升多样性


<details>
  <summary>Details</summary>
Motivation: 离散扩散模型是有前景的自回归文本生成替代方案，但其解码方法研究不足。标准自回归解码方法（如束搜索）不直接适用于迭代去噪过程，现有扩散解码技术对批次内多样性的控制有限

Method: 提出了广义束搜索框架，支持并行生成候选项和模块化束选择目标。具体实现D5P4方法，将选择步骤建模为确定性点过程的MAP推理，利用可扩展的贪婪求解器，在多GPU兼容性下实现模型概率与目标多样性之间的显式权衡

Result: 在自由文本生成和问答任务上的实验表明，D5P4在保持竞争力的生成质量的同时，相比强基线显著提升了多样性

Conclusion: D5P4为离散扩散模型提供了有效的解码框架，通过确定性点过程的束选择实现了多样性与质量之间的可控权衡，为文本生成领域提供了新的工具
...
TL;DR: cuGenOpt是一个GPU加速的通用元启发式框架，通过"一个块演化一个解决方案"的CUDA架构、统一的编码抽象和两级自适应算子选择机制，解决了组合优化问题在通用性、性能和可用性之间的权衡问题。
...
Method: 1. 引擎层面：采用"one block evolves one solution" CUDA架构，统一编码抽象（排列、二进制、整数），两级自适应算子选择机制，硬件感知资源管理。
2. 可扩展性层面：用户定义算子注册接口，允许领域专家注入特定问题的CUDA搜索算子。
3. 可用性层面：JIT编译管道提供纯Python API，基于LLM的建模助手将自然语言问题描述转换为可执行求解器代码。

Result: 在三个GPU架构（T4、V100、A800）上的五个主题套件实验表明：cuGenOpt比通用MIP求解器快数个数量级；在n=150实例上与专用求解器达到竞争性质量；TSP-442在30秒内达到4.73%间隙；12种问题类型跨5种编码变体达到最优；框架级优化将pcb442间隙从36%降至4.73%，将VRPTW吞吐量提升75-81%。

Conclusion: cuGenOpt成功解决了组合优化中通用性、性能和可用性的权衡问题，通过GPU加速元启发式框架提供了高效、可扩展且用户友好的解决方案，在多种问题类型和实例规模上表现出卓越性能。
...
### [208] [OS-Themis: A Scalable Critic Framework for Generalist GUI Rewards](https://arxiv.org/abs/2603.19191)
...
TL;DR: OS-Themis：一个可扩展且准确的多智能体批评框架，通过将轨迹分解为可验证的里程碑来改进GUI智能体的强化学习，在AndroidWorld实验中比现有方法提升10.3%的性能。


<details>
  <summary>Details</summary>
Motivation: 强化学习在随机环境中提升GUI智能体鲁棒性方面有潜力，但训练过程对奖励函数质量高度敏感。现有奖励方法难以同时实现可扩展性和高性能。

Method: 提出了OS-Themis多智能体批评框架：1) 将轨迹分解为可验证的里程碑，分离出决策所需的关键证据；2) 采用审查机制在最终裁决前严格审核证据链。此外还引入了OmniGUIRewardBench（OGRBench）作为跨平台的GUI结果奖励基准。

Result: 1) 在OGRBench基准测试中，所有评估模型在OS-Themis下都达到了最佳性能；2) 在AndroidWorld实验中，OS-Themis支持在线RL训练时带来10.3%的性能提升；3) 在自训练循环中用于轨迹验证和过滤时带来6.9%的性能增益。

Conclusion: OS-Themis框架能够有效驱动智能体进化，通过可验证的里程碑分解和严格证据链审核，解决了现有奖励方法在可扩展性和性能方面的平衡问题，为GUI智能体的强化学习提供了更准确、可扩展的奖励机制。
