...
TL;DR: DOne是一个端到端框架，通过解耦结构理解和元素渲染来解决VLM在Design-to-Code生成中的"整体瓶颈"问题，在复杂布局数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: Vision Language Models在Design-to-Code生成中存在"整体瓶颈"，难以同时兼顾高层次结构层次和细粒度视觉细节，导致布局失真或使用通用占位符。

Method: DOne框架包含三个核心组件：1) 学习的布局分割模块，分解复杂设计避免启发式裁剪限制；2) 专业化的混合元素检索器，处理UI组件的极端宽高比和密度；3) 模式引导的生成范式，连接布局和代码生成。

Result: 在HiFi2Code基准测试中，DOne在高层视觉相似度（如GPT Score超过10%提升）和细粒度元素对齐方面均优于现有方法。人工评估显示生产力提高3倍且视觉保真度更高。

Conclusion: DOne通过解耦结构理解和元素渲染的方法有效解决了VLM在Design-to-Code生成中的瓶颈问题，显著提升了复杂UI设计的代码生成质量和效率。
...
TL;DR: Look Twice (LoT)是一个无需训练、基于推理时间的框架，通过利用注意力模式来估计相关视觉区域和文本元素，并通过轻量级提示标记突出显示这些证据，从而提升多模态大语言模型在知识密集型视觉问答中的表现。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在处理知识密集型查询时存在困难：难以识别最相关的视觉和文本证据。模型需要整合视觉线索与检索的文本证据（这些证据通常存在噪声或仅部分相关），同时还要定位图像中的细粒度视觉信息。

Method: 1. 利用模型注意力模式估计查询相关的视觉区域和检索的文本元素
2. 通过轻量级提示级标记突出显示选定的线索
3. 在生成答案时鼓励模型重新关注相关证据
4. 无需额外训练或架构修改

Result: 1. 在多个基于知识的VQA基准测试中，相比零样本MLLMs取得了一致性改进
2. 在没有文本上下文的视觉中心和幻觉导向基准测试中，仅视觉证据突出显示就能提升模型性能

Conclusion: LoT框架有效提升了预训练MLLMs利用多模态证据的能力，通过推理时间的注意力重新聚焦机制改善模型性能，为知识密集型视觉问答提供了一个简单有效的解决方案。
...
TL;DR: MedQwen：一种参数高效的医学视觉-语言模型，通过频谱路由混合专家系统和理论支撑的缩放规则，在医学影像任务上实现强大而鲁棒的性能，同时极大减少可训练参数并缓解持续学习中的灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 大型视觉-语言模型在通用基准上表现出色，但在医学影像领域缺乏鲁棒性。医学数据具有异质性监督导致跨数据集干扰，且对数据机制敏感。现实临床工作流程中数据和任务按顺序到达，简单的持续训练会导致灾难性遗忘。需要解决这些挑战来构建有效的医学视觉-语言模型。

Method: 提出MedQwen，一种参数高效的医学VLM：1）采用频谱路由的混合专家系统；2）基于理论支撑的缩放规则，使低秩更新与完全微调的完整MoE对齐；3）从预训练权重的非重叠奇异值分解段初始化每个专家；4）引入残差补偿和缩放方案，实现稳定的专家专业化和分布偏移下的一致路由。

Result: 在23个医学数据集上评估，涵盖视觉问答、报告生成、放射学分类和幻觉缓解任务：1）在零样本分类上接近完全微调性能，仅使用1/339的可训练参数；2）将顺序遗忘减少至约5%，而强基线退化20-50%；3）实现强大可靠的性能表现。

Conclusion: MedQwen通过结合频谱路由MoE和理论支持的缩放规则，成功解决了医学视觉-语言模型中的跨数据集干扰和灾难性遗忘问题。该方法在不改变基础架构的情况下实现了参数高效的学习，在广泛的医学任务上展现出卓越的性能和鲁棒性，为临床工作流程中的实际应用提供了有前景的解决方案。
...
TL;DR: 本研究提出了一个基于Vision Transformer的模型，用于从美式足球训练视频中检测危险擒抱动作，并在显著扩展的数据集上取得了比先前基线更好的性能。


<details>
  <summary>Details</summary>
Motivation: 接触性运动中早期识别危险动作可以及时干预并提高运动员安全性。当前缺乏足够的数据集和有效的自动化方法来检测美式足球训练中的危险擒抱技术。

Method: 使用基于Vision Transformer的视频分析模型，结合针对类别不平衡问题的训练策略。数据集包含733个单运动员-人偶擒抱视频片段，每个片段都围绕首次接触点进行了时间定位，并标注了标准化擒抱技术评估(SATT-3)的击打区域分量。

Result: 在交叉验证下获得了0.67的危险召回率和0.59的危险F1分数。相比先前在小数据集上的基线（危险召回率0.58；危险F1分数0.56），危险召回率提高了超过8个百分点，且是在更大的数据集上取得的。

Conclusion: Vision Transformer的视频分析结合类别不平衡的谨慎处理，能够可靠地检测罕见但安全关键的擒抱模式，为教练中心的伤害预防工具提供了实用途径。
...
TL;DR: 通过使用合成蹦床姿势数据集(STP)微调ViTPose模型，显著提升了极端人体姿态和非常规视角下的姿态估计性能，在2D和3D姿态估计上都取得了state-of-the-art结果。


<details>
  <summary>Details</summary>
Motivation: 蹦床体操涉及极端的人体姿态和非常规的观察视角，现有的state-of-the-art姿态估计模型在这些挑战性场景下表现不佳。需要专门的方法来处理这种特定领域的姿态估计问题。

Method: 1. 构建合成蹦床姿势数据集(STP)：从蹦床动作的动捕记录中生成数据；2. 开发数据处理管道：将有噪声的动捕数据拟合到参数化人体模型；3. 生成多视角逼真图像；4. 使用STP数据集微调ViTPose模型；5. 在真实多视角蹦床图像上进行测试。

Result: 1. 2D姿态估计：在挑战性数据上获得state-of-the-art结果，缩小了常规姿态与极端姿态之间的性能差距；2. 3D姿态估计：MPJPE减少了12.5mm，相比预训练的ViTPose模型提升了19.6%；3. 2D准确性的提升直接转化为3D三角测量的改进。

Conclusion: 使用合成数据微调姿态估计模型是解决特定领域极端姿态估计问题的有效方法。该方法显著提升了蹦床体操等挑战性场景下的姿态估计性能，证明了合成数据生成在专业运动分析中的实用价值。
...
TL;DR: 本文提出了一种基于统计学习技术的注意力正则化方法，通过自举重采样估计注意力分数的重要性，消除ViT中噪声引起的虚假注意力，提高解释性。


<details>
  <summary>Details</summary>
Motivation: 当前视觉变换器(ViT)使用注意力分数作为决策过程的解释，但这些分数几乎总是非零，导致注意力图噪声大、扩散性强，限制了可解释性。需要量化注意力分数的不确定性度量并获得正则化的注意力分数。

Method: 提出注意力正则化方法，将ViT注意力分数置于统计框架中，通过自举重采样输入特征生成注意力分数的基线分布，然后使用该自举分布估计注意力分数的重要性和后验概率。

Result: 在自然图像和医学图像上的实验表明，该方法能够直接移除噪声引起的虚假注意力，显著提高收缩性和稀疏性。使用模拟和真实数据集进行了定量评估。

Conclusion: 自举技术可作为实用的正则化工具，在使用注意力分数作为ViT解释时有效提高可解释性。研究强调了统计学习在注意力机制解释中的重要性。
...
TL;DR: 提出AffordTissue框架，用于预测胆囊切除术中工具动作特定的组织可供性区域作为密集热图，通过多模态方法结合视觉、语言和DiT风格解码器，显著优于现有基础模型。


<details>
  <summary>Details</summary>
Motivation: 目前手术动作自动化方法在临床部署中面临挑战：现有方法对器械在组织表面的交互位置预测能力有限，缺乏明确的约束条件来强制执行工具动作特定的安全交互区域。

Method: 提出了AffordTissue多模态框架，包含：1) 时序视觉编码器捕捉多视角下的工具运动和组织动力学；2) 语言条件化支持不同仪器-动作对的泛化；3) DiT风格解码器进行密集可供性预测。构建了首个组织可供性基准数据集，包含103例胆囊切除术的15,638个视频片段，覆盖4种器械的6个工具-动作对。

Result: 实验显示在密集手术可供性预测方面显著优于视觉语言模型基线（20.6像素ASSD vs. Molmo-VLM的60.2像素），表明任务特定的架构优于大规模基础模型。

Conclusion: 通过预测工具动作特定的组织可供性区域，AffordTissue为安全手术自动化提供了明确的空间推理能力，可能实现向适当组织区域的明确策略指导，并在器械偏离预测安全区域时实现早期安全停止。
...
TL;DR: GRAZE：一种无需训练的视频分析方法，用于在美式足球训练视频中定位第一次接触帧（FPOC），采用候选发现与接触确认分离的流程，在真实训练数据上达到97.4%的有效输出率。


<details>
  <summary>Details</summary>
Motivation: 美式足球训练产生大量长视频，但关键交互只占极短时间。可靠的生物力学分析需要精确的时空定位来识别交互实体和接触开始时间。现有方法面临相机运动、场景杂乱、多个相似装备运动员以及冲击瞬间姿态快速变化等挑战。

Method: 提出GRAZE训练免费流水线：1) 使用Grounding DINO发现候选球员-假人交互；2) 通过运动感知时间推理精炼候选帧；3) 使用SAM2作为像素级接触验证器（而非仅依赖检测置信度）。这种候选发现与接触确认的分离使方法对杂乱场景和冲击附近不稳定的基础检测具有鲁棒性。

Result: 在738个擒抱训练视频上：GRAZE为97.4%的剪辑产生有效输出；77.5%的剪辑中FPOC定位在±10帧内；82.7%的剪辑中FPOC定位在±20帧内。无需任务特定训练即可实现帧级准确度。

Conclusion: 在真实世界训练视频中实现帧级准确的接触开始定位是可行的，无需任务特定训练。GRAZE的方法设计使其对杂乱场景具有鲁棒性，分离候选发现和接触确认的策略是关键创新。
...
TL;DR: 提出基于稀疏体素光栅化(SVRaster)的新框架，解决3D高斯溅射(3DGS)在开放词汇3D场景理解中的空间模糊和多层语义模糊问题，实现状态最优性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于3D高斯溅射的开放词汇3D场景理解方法存在两个关键限制：1）非结构化重叠高斯分布导致的空间模糊，需要概率特征注册；2）基于物体级掩码的特征池化导致的多层语义模糊，稀释了细粒度细节。

Method: 使用稀疏体素光栅化(SVRaster)作为结构化、不相交的几何表示，通过单目深度和法线先验进行正则化。在此基础上实现确定性、置信度感知的特征注册过程，并利用AM-RADIO基础模型的密集对齐特性解决多层模糊问题，避免分层训练的计算开销。

Result: 在开放词汇3D物体检索和点云理解基准测试中取得了最先进的性能，特别是在特征注册方法通常失败的细粒度查询任务上表现出色，有效抑制了3DGS中常见的语义渗漏伪影。

Conclusion: 通过结构化几何表示和确定性特征注册，结合基础模型的密集对齐特性，有效解决了现有3DGS方法中的空间和语义模糊问题，为开放词汇3D场景理解提供了更精确和高效的解决方案。
...
TL;DR: EgoFlow：一个基于流匹配的框架，通过多模态第一人称观察合成真实且物理合理的6DoF物体轨迹，采用混合Mamba-Transformer-Perceiver架构结合梯度引导推理，在物理一致性和泛化能力上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 从第一人称视频理解和预测物体运动对于具身感知和交互至关重要，但现有生成模型存在挑战：1)遮挡、快速运动等问题导致6DoF轨迹生成困难；2)缺乏显式的物理推理机制；3)需要后处理过滤或额外监督。

Method: 提出EgoFlow框架：1)采用流匹配框架合成物理合理的轨迹；2)设计混合Mamba-Transformer-Perceiver架构联合建模时间动态、场景几何和语义意图；3)引入梯度引导推理过程，强制执行可微物理约束（如碰撞避免和运动平滑性）。

Result: 在HD-EPIC、EgoExo4D和HOT3D真实数据集上验证，EgoFlow在准确性、泛化性和物理真实性方面优于基于扩散和Transformer的基线方法：1)碰撞率降低高达79%；2)对未见场景具有强泛化能力；3)无需后处理过滤或额外监督。

Conclusion: EgoFlow证明了基于流的生成建模在可扩展和物理基础的第一人称运动理解方面的潜力，通过结合深度学习和可微物理约束，实现了连贯可控的运动生成，为具身AI系统提供了重要的技术基础。
...
TL;DR: 开发了一个通用的人类姿态数据分析流程，结合预处理、降维和递归分析，用于量化运动动力学的时间结构，并通过三个案例研究展示了其灵活性。


<details>
  <summary>Details</summary>
Motivation: 无标记姿态估计技术使得使用标准视频在自然环境中捕捉详细的人类运动成为可能，但姿态数据的高维性、噪声和时序复杂性给提取有意义的协调模式和行为变化带来了重大挑战。

Method: 开发了一个通用分析流程，结合原则性的预处理、降维和基于递归的时间序列分析，以量化运动动力学的时间结构。该流程支持对运动进行线性和非线性表征。

Result: 通过三个案例研究展示了该流程的灵活性：涵盖面部和全身运动、2D和3D数据、以及个体与多智能体行为。这些例子表明，相同的分析工作流可以适应提取复杂姿态时间序列中的理论意义。

Conclusion: 该研究提出了一种通用的人类姿态数据分析框架，能够有效处理高维复杂运动数据，为不同实验环境下的行为分析提供了一个灵活且可扩展的工具，有助于从复杂运动数据中提取有理论意义的洞察。
...
TL;DR: UniRecGen是一个统一框架，将前馈重建和扩散生成相结合，在共享规范空间中解决稀疏视图3D建模中重建保真度与生成合理性之间的矛盾。


<details>
  <summary>Details</summary>
Motivation: 稀疏视图3D建模存在重建保真度与生成合理性的基本矛盾：前馈重建方法效率高且与输入对齐，但缺乏全局先验导致结构不完整；而基于扩散的生成方法能提供丰富几何细节，但难以保持多视图一致性。

Method: 提出了UniRecGen统一框架，将两种范式整合到单一协同系统中。通过将两种模型对齐到共享规范空间来解决坐标空间、3D表示和训练目标的内在冲突。采用解耦协同学习保持训练稳定性，实现推理时的无缝协作：重建模块提供规范几何锚点，扩散生成器利用潜在增强条件来细化和补全几何结构。

Result: 实验结果表明，UniRecGen在保真度和鲁棒性方面表现优越，在从稀疏观察创建完整且一致的3D模型方面超越了现有方法。

Conclusion: UniRecGen成功整合了前馈重建和扩散生成两种范式，通过共享规范空间和解耦协同学习，有效解决了稀疏视图3D建模中的基本矛盾，实现了高质量、一致的3D模型重建。
...
TL;DR: TAG热反鬼影成像框架解决了热成像中的鬼影效应，通过非参数纹理恢复技术实现了高保真夜视，首次实现了鬼影人脸表情恢复和热3D拓扑对齐。


<details>
  <summary>Details</summary>
Motivation: 热成像对夜视至关重要，但受到鬼影效应的根本性限制——在杂乱光子流中丢失细节纹理。虽然传统方法依赖数据后处理，但最新的热辅助探测和测距(HADAR)为高光谱计算热成像开辟了新前景。然而，通用反鬼影成像仍难以实现，因为现有的HADAR仅适用于材料均匀的有限场景，而材料非均匀性在现实世界中普遍存在。

Method: 提出通用的计算热成像框架TAG（热反鬼影），处理材料非均匀性问题并克服鬼影效应。TAG采用高光谱光子流进行非参数纹理恢复，无需先验知识即可从热成像数据中恢复细节纹理。

Result: 1. 实验展示了前所未有的鬼影人脸表情恢复——这是典型的、长期公认的鬼影现象；2. TAG在各种场景中普遍优于HADAR；3. 揭示了材料非均匀性的影响，阐明了HADAR的有效边界；4. 首次实现了热3D拓扑对齐和情绪检测；5. 在白天和夜晚广泛测试了面部纹理和表情恢复。

Conclusion: TAG建立了一个通用的高保真计算夜视基础，在自主导航、侦察、医疗保健和野生动物监测等领域具有潜在应用价值。该工作解决了材料非均匀性这一根本挑战，克服了传统热成像的鬼影限制。
...
TL;DR: PBSeg是一个针对无人机图像的高效原型分割框架，通过原型交叉注意力和多尺度特征提取来平衡分割质量和计算效率。


<details>
  <summary>Details</summary>
Motivation: 无人机图像语义分割面临尺度变化大、物体边界复杂以及边缘设备计算资源有限等挑战。现有的transformer方法计算开销大，轻量级方法又难以捕捉高分辨率航拍场景的细节特征。

Method: 提出了PBSeg框架，包含原型交叉注意力(PBCA)来利用特征冗余降低计算复杂度，以及结合可变形卷积和上下文感知调制的多尺度特征提取模块来捕获局部细节和全局语义。

Result: 在UAVid数据集上达到71.86% mIoU，在UDD6数据集上达到80.92% mIoU，在保持计算效率的同时取得了竞争性性能。

Conclusion: PBSeg为无人机图像语义分割提供了一种高效的原型分割解决方案，在分割质量和计算效率之间取得了良好平衡，适合边缘设备部署。
...
TL;DR: ReFlow是一个用于单目动态场景重建的统一框架，通过自校正流匹配机制从原始视频中学习3D运动，无需依赖外部密集运动引导，实现了稳健准确的动态场景重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法对动态区域的场景初始化不完整，导致重建和运动估计不稳定，通常需要依赖外部密集运动引导（如预计算的光流）来稳定动态组件的重建。这种方法引入了额外的复杂性和潜在的错误传播。

Method: 1. 完整的规范空间构建模块：增强静态和动态区域的初始化
2. 基于分离的动态场景建模模块：解耦静态和动态组件进行针对性运动监督
3. 自校正流匹配机制：包括全流匹配（对齐3D场景流与时间变化的2D观测）和相机流匹配（强制静态对象的多视图一致性）

Result: 在多种场景下的广泛实验表明，ReFlow实现了优越的重建质量和稳健性，为单目4D重建建立了一个新的自校正范式。

Conclusion: ReFlow通过统一的框架和创新的自校正流匹配机制，解决了动态场景重建中的初始化不完整和依赖外部运动引导的问题，实现了无需外部密集运动指导的稳健准确的单目动态场景重建。
...
TL;DR: VideoZeroBench是一个用于长视频问答的层次化基准测试，通过严格的时空证据验证来评估视频多模态大模型，揭示了当前模型在细粒度视觉理解和证据定位方面的显著不足。


<details>
  <summary>Details</summary>
Motivation: 当前视频多模态大语言模型在各种基准测试中取得了令人印象深刻的结果，但现有评估存在两个关键局限：(1) 夸大的分数掩盖了细粒度视觉理解和推理的缺陷；(2) 答案正确性的测量通常没有验证模型是否识别了支持其预测的精确时空证据。

Method: 提出了VideoZeroBench，这是一个针对挑战性长视频问答设计的层次化基准测试，包含500个人工标注的问题，涵盖13个领域，每个问题都配有作为证据的时间间隔和空间边界框。引入了一个五级评估协议，逐步收紧证据要求，以区分答案生成、时间定位和空间定位能力。

Result: 实验结果显示，在标准端到端QA设置（Level-3）下，即使Gemini-3-Pro模型正确回答的问题也不足17%。当施加定位约束时，性能急剧下降：在同时要求正确回答和准确时空定位的情况下（Level-5），没有模型超过1%的准确率，大多数模型无法实现任何正确的定位预测。

Conclusion: 这些结果揭示了表面答案正确性与真正基于证据的推理之间存在显著差距，表明基于证据的视频理解仍然是长视频问答的瓶颈。该基准测试为未来基于证据的视频推理研究提供了见解，并将公开提供基准测试和代码。
...
TL;DR: 本文提出了一种梯度对齐交替学习（GAAL）范式，用于解决多模态表格-图像融合中的梯度冲突问题。该方法通过交替的单模态学习和共享分类器解耦多模态梯度，并设计基于不确定性的跨模态梯度手术选择性对齐梯度，从而提升整体融合性能。


<details>
  <summary>Details</summary>
Motivation: 多模态表格-图像融合是一个新兴任务，但现有方法存在模态间梯度冲突的问题，这会误导单模态学习器的优化。梯度冲突阻碍了有效的模态融合，限制了整体性能的提升。

Method: 提出梯度对齐交替学习（GAAL）范式：1）采用交替单模态学习和共享分类器架构，解耦多模态梯度并促进交互；2）设计基于不确定性的跨模态梯度手术，选择性对齐跨模态梯度，引导共享参数使所有模态受益。

Result: 在广泛使用的数据集上进行实证实验，结果表明该方法优于各种最先进的表格-图像融合基线和测试时表格缺失基线，展示了其优越性和有效性。

Conclusion: GAAL能够提供有效的单模态辅助，帮助提升整体融合性能。该方法通过解决梯度冲突问题，为多模态表格-图像融合任务提供了一种新的解决方案。
...
TL;DR: 提出了一种卫星免费训练(SFT)框架，用于无人机视角地理定位，通过三维场景重建和伪正射影像生成，在没有卫星图像训练的情况下实现跨视图检索


<details>
  <summary>Details</summary>
Motivation: 现有无人机视角地理定位方法依赖卫星图像进行训练，但在卫星数据不可用或受限时部署困难。针对多视图无人机序列，需要开发不依赖卫星图像的训练方法

Method: 卫星免费训练框架包含三个主要阶段：1) 使用3D高斯溅射从多视图无人机图像重建密集三维场景；2) 通过PCA引导的正投影生成几何归一化的伪正射影像，并使用几何引导的轻量级修复获得完整纹理；3) 从生成的伪正射影像中提取DINOv3补丁特征，仅使用无人机数据学习Fisher向量聚合模型，并在测试时重复用于编码卫星图块进行跨视图检索

Result: 在University-1652和SUES-200数据集上的实验结果表明，SFT框架显著优于卫星免费泛化基线，并缩小了与使用卫星图像训练方法之间的性能差距

Conclusion: 提出的卫星免费训练框架通过三维重建和伪正射影像生成，实现了不依赖卫星图像训练的无人机视角地理定位，为在卫星数据受限环境中的实际部署提供了可行解决方案
...
TL;DR: SHOE是一个新的开放词汇人类-物体交互检测评估框架，使用语义相似性代替精确字符串匹配，通过分解动词和物体组件并利用多个LLM评估语义对齐度。


<details>
  <summary>Details</summary>
Motivation: 传统评估指标（如mAP）将HOI类视为离散分类标签，无法正确评估语义有效但词汇不同的预测（如"靠在沙发上"vs"坐在沙发上"），限制了其对超越预定义HOI标签集的开放词汇预测的适用性。

Method: SHOE将每个HOI预测分解为动词和物体组件，使用多个大型语言模型的平均值估计它们的语义相似性，并将它们组合成相似性分数，以评估超越精确字符串匹配的对齐度。

Result: SHOE评分与人类判断的一致性比现有指标（包括基于LLM和嵌入的基线）更接近，与平均人类评分的协议达到85.73%。

Conclusion: 该研究强调了需要基于语义的HOI评估，以更好地反映人类对交互的理解。作者将公开发布评估指标以促进未来研究。
...
TL;DR: ROSETTA是一种鲁棒的开集测试时适应方法，通过角损失和特征范数损失来解决协变量漂移下ID分类和OOD检测之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 开集测试时适应(OSTTA)面临协变量漂移下ID样本变化与OOD样本检测的双重挑战。现有方法结合熵最小化（保持ID性能）和熵最大化（增强OOD检测）存在内在冲突，导致ID分类和OOD检测之间的权衡。

Method: 1. 分析熵最大化在OSTTA中的局限性；2. 引入角损失来调节特征范数大小；3. 提出特征范数损失来抑制csOOD的logits值；4. 这些目标共同构成ROSETTA方法。

Result: 在CIFAR-10-C、CIFAR-100-C、Tiny-ImageNet-C和ImageNet-C上实现了强大的OOD检测性能，同时保持高ID分类准确率。在Cityscapes上的实验验证了该方法在真实世界语义分割中的有效性，在HAC数据集上的结果证明了其在不同开集TTA设置中的适用性。

Conclusion: ROSETTA通过避免熵最小化与最大化之间的直接冲突，提出角损失和特征范数损失作为替代方案，有效解决了开集测试时适应中ID分类与OOD检测的权衡问题，在多个数据集和实际应用中表现出色。
...
TL;DR: F3DGS是一种联邦3D高斯溅射框架，用于分散式多智能体3D重建，通过固定高斯位置保持几何对齐，使用可见性感知聚合解决部分可观察性问题


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯溅射(3DGS)方法假设集中式访问所有观测数据，这在分布式机器人场景中存在局限性，因为智能体独立操作且集中数据聚合可能受限。直接扩展集中式训练到多智能体系统会带来通信开销和几何不一致问题

Method: 首先通过注册多个客户端局部合并的LiDAR点云构建共享几何支架来初始化全局3DGS模型。联邦优化期间，固定高斯位置以保持几何对齐，每个客户端仅更新外观相关属性（协方差、不透明度、球谐系数）。服务器使用可见性感知聚合更新，根据每个客户端观察每个高斯的频率加权贡献

Result: 收集了一个多序列室内数据集，包含同步LiDAR、RGB和IMU测量。实验表明F3DGS在实现与集中式训练相当的重建质量的同时，支持跨智能体的分布式优化

Conclusion: F3DGS为分散式多智能体3D重建提供了一种有效框架，解决了集中式方法的局限性，并计划公开发布数据集、开发工具包和源代码
...
TL;DR: 该论文提出了一种基于稀疏自编码器和视觉语言提示的自动化形态性状标注方法，用于大规模昆虫图像分析，构建了包含8万个标注的BIOSCAN-Traits数据集


<details>
  <summary>Details</summary>
Motivation: 形态性状是生物与环境相互作用的重要线索，但传统的手工提取方法缓慢且依赖专家，限制了在大规模生态研究中的应用。主要瓶颈是缺乏连接生物图像与性状标注的高质量数据集

Method: 在基础模型特征上训练稀疏自编码器，生成单语义、空间定位的神经元；提出性状标注流程：先定位显著区域，再使用视觉语言提示生成可解释的性状描述；构建BIOSCAN-Traits数据集（19K昆虫图像，80K标注）

Result: 人类评估确认生成的形态描述具有生物学合理性；通过消融研究系统评估了设计选择的敏感性；实现了从生物学图像到可解释性状描述的自动化流程

Conclusion: 该方法通过模块化流水线而非昂贵的人工标注，为大规模形态分析提供了可扩展解决方案，在基础模型中注入生物学意义监督，弥合了生态相关性与机器学习实用性之间的差距
...
TL;DR: LivingWorld是一个从单张图像生成具有环境动态的4D世界的交互式框架，通过渐进构建全局一致的运动场来实现大规模场景的动态效果


<details>
  <summary>Details</summary>
Motivation: 当前3D场景生成方法主要关注静态几何重建，而忽略了云、水、烟雾等场景尺度的环境动态。建模这些动态具有挑战性，因为需要在场景扩展时保持运动的一致性，同时支持低延迟的用户反馈。

Method: 1. 渐进构建全局一致的运动场；2. 几何感知对齐模块解决视图间的方向和尺度歧义；3. 基于哈希的紧凑运动场表示，支持高效查询和稳定传播；4. 渲染时支持双向运动传播，生成长时序的4D序列。

Result: 在单个RTX 5090 GPU上，生成每个新场景扩展步骤需要9秒，运动对齐和运动场更新需要3秒，实现了具有全局一致环境动态的交互式4D世界生成。无需依赖昂贵的基于视频的细化。

Conclusion: LivingWorld能够从单张图像生成具有连贯环境动态的4D世界，解决了场景扩展时的运动一致性问题，实现了交互式生成，为动态场景建模提供了有效解决方案。
...
TL;DR: 提出用于稀疏标注单目3D目标检测的新框架，包含道路感知补丁增强(RAPA)和原型过滤(PBF)两个关键模块，以解决实际场景中只有部分目标被标注的挑战。
...
Method: 1. 道路感知补丁增强(RAPA)：利用稀疏标注，将分割的目标补丁增强到道路区域，同时保持3D几何一致性；2. 原型过滤(PBF)：通过原型相似性和深度不确定性过滤预测，生成高质量伪标签，维护全局2D RoI特征原型并选择与学习原型特征一致且深度估计可靠的伪标签。

Result: 广泛实验证明了所提方法的有效性，在稀疏监督下实现了鲁棒的检测性能。

Conclusion: 提出的结合几何保持增强和原型引导伪标注的训练策略，能够有效解决稀疏标注单目3D目标检测问题，为实际应用中标注成本高昂的场景提供了实用解决方案。
...
TL;DR: 提出一种基于物理学的视频认证签名方法，利用莫尔效应区分真实拍摄视频与AI生成视频


<details>
  <summary>Details</summary>
Motivation: 随着视频生成技术的进步，AI合成内容越来越难以与真实拍摄视频区分，需要开发可靠的认证方法来辨别视频真伪

Method: 利用莫尔效应作为物理认证签名：当相机拍摄双层光栅结构时会产生干涉条纹，推导出莫尔运动不变量，证明条纹相位与光栅图像位移之间存在线性耦合关系，且不受观看距离和光栅结构影响。验证器从视频中提取信号并测试其相关性

Result: 在真实拍摄视频和多个最先进AI生成器的视频上验证了该不变量，发现真实视频和AI生成视频产生显著不同的相关性签名，表明该方法能有效区分二者

Conclusion: 确定性光学现象可以作为物理基础的、可验证的签名来对抗AI生成视频，为视频认证提供了稳健的方法
...
TL;DR: DynaVid：一种利用合成运动数据（表示为光流）训练的视频合成框架，通过两阶段生成方法（先合成运动，再合成视频）解决视频扩散模型在动态运动和细粒度运动可控性方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型在合成高度动态运动或需要细粒度运动可控性的视频时表现不佳，主要原因是常用训练数据集中此类示例稀缺。

Method: 1. 使用计算机图形学管道渲染的合成运动数据（光学流表示）进行训练；2. 采用两阶段生成框架：运动生成器先合成运动，运动引导的视频生成器再基于该运动生成视频帧；3. 运动与外观解耦设计，确保模型从合成数据学习动态运动模式，同时从真实世界视频保持视觉真实感。

Result: 在激烈人体运动生成和极端相机运动控制这两个具有挑战性的场景中进行了验证，这些场景现有数据集特别有限。大量实验证明，DynaVid在动态运动生成和相机运动控制的真实感和可控性方面都有显著提升。

Conclusion: 通过利用合成运动数据并采用解耦的两阶段生成框架，DynaVid能够有效解决视频合成中动态运动和可控性的核心挑战，同时保持视觉真实感，为数据稀缺场景提供了可行的解决方案。
...
TL;DR: 提出了一种无领域标识符和无样本的增量学习框架，用于具身多媒体系统的持续环境适应，通过解耦表征机制和权重融合策略，显著减少了灾难性遗忘并提高了泛化能力。


<details>
  <summary>Details</summary>
Motivation: 具身感知系统在开放物理空间中持续交互时面临动态环境分布漂移的挑战。现有领域增量感知方法通常依赖测试阶段预先获得的领域标识符，限制了在未知交互场景中的实用性。同时，模型容易过拟合特定上下文感知噪声，导致泛化能力不足和灾难性遗忘问题。

Method: 设计了无领域标识符和无样本的增量学习框架：1) 使用解耦表征机制去除非必要环境风格干扰，引导模型专注于提取跨场景共享的语义内在特征；2) 采用权重融合策略在参数空间中动态整合新旧环境知识，确保模型适应新分布而无需存储历史数据，同时最大限度保留旧环境的判别能力。

Result: 在多个标准基准数据集上的广泛实验表明，所提出的方法在完全无样本和无领域标识符的设置下，显著减少了灾难性遗忘，其准确性优于现有的最先进方法。

Conclusion: 提出的框架为具身多媒体系统提供了稳健的持续环境适应能力，通过解耦语义特征和动态参数整合，有效解决了开放物理空间中动态分布漂移带来的挑战，在无领域标识符和无样本的情况下实现了优异的性能表现。
...
TL;DR: 本文提出了一种基于频域适应（FDA）和和声约束最优传输（HOT）的方法，用于提高远程光电容积脉搏波描记法（rPPG）在跨域场景下的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 远程光电容积脉搏波描记法（rPPG）虽然能够从面部视频中进行非接触式生理测量，但在实际部署中常常面临领域漂移导致的性能显著下降问题。现有的深度学习方法虽然在单个数据集上表现良好，但往往过度拟合到外观相关因素（如光照、相机特性、颜色响应等），这些因素在不同域之间差异很大。

Method: 提出了两种关键技术：1) 频域适应（FDA）：通过转移编码域依赖外观特征的低频谱成分，使rPPG模型学习对外观变化的鲁棒性，同时保留心脏诱导信号；2) 和声约束最优传输（HOT）：利用心脏信号的谐波特性来引导原始表示与FDA转移表示之间的对齐，确保在存在外观变化时保持生理一致性。

Result: 广泛的跨数据集实验表明，提出的FDA和HOT框架有效增强了rPPG模型在不同数据集上的鲁棒性和泛化能力。

Conclusion: 该研究为解决rPPG领域漂移问题提供了一种原理性的解决方案，通过频域适应和生理约束的对齐机制，显著提升了模型在实际跨域部署中的性能稳定性。
...
TL;DR: Director：一种统一时空高斯表示，联合建模人体性能、高保真渲染和实例级语义，通过实例一致语义嵌入和语言对齐实现动态场景理解


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯的方法主要关注外观，但对实例级结构不敏感，限制了高度动态场景中的稳定跟踪和语义推理。需要一种能同时处理渲染保真度和实例语义的4D表示方法

Method: 1. 使用时间对齐的实例掩码和多模态大语言模型的句子嵌入，通过两个MLP解码器监督高斯点的可学习语义特征
2. 将2D光流与4D高斯连接并微调其运动，提高时间稳定性
3. 引入几何感知的SDF约束和正则化项，增强动态前景建模的时间连贯性

Result: 实验表明Director实现了时间连贯的4D重建，同时支持实例分割和开放词汇查询

Conclusion: 将实例一致的语义嵌入与4D建模自然互补，能够实现更准确的场景分解并支持鲁棒的动态场景理解，为统一的时空高斯表示提供了有效解决方案
...
TL;DR: 提出一种结合外部指导和内部机制的视频对象移除方法，通过从视觉基础模型蒸馏对象关系知识，并利用帧间上下文交叉注意力，实现更清晰的视频对象移除效果，包括消除对象引起的副作用（阴影、反射等）。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的视频对象移除方法在消除对象引起的副作用（如阴影、反射、光照变化）方面存在局限，难以同时保持时空一致性。这种局限性源于对目标对象及其与场景交互的物理和语义理解不足。

Method: 提出从两个互补角度引入理解：1) 外部：设计蒸馏方案，将视觉基础模型中对象与其引发效果的关系知识迁移到视频扩散模型中；2) 内部：提出帧间上下文交叉注意力机制，使每个去噪块都能基于目标区域周围未遮挡的上下文信息进行预测。两者共同使模型理解目标对象、其引发效果及全局背景。

Result: 大量实验证明了方法的先进性能，建立了首个真实世界视频对象移除基准，促进未来研究和社区发展。代码、数据和模型已开源。

Conclusion: 通过结合外部知识蒸馏和内部上下文引导，提出的方法能够更好地理解目标对象及其与场景的交互，实现更清晰、连贯的视频对象移除，包括消除对象引起的各种副作用，在性能上达到先进水平。
...
TL;DR: 本文提出了一种通过群体检测实现端到端共享注意力估计的方法，同时进行群体检测和共享注意力估计，在两个任务上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在估计共享注意力(SA)时存在两个主要问题：1) 不检测实际关注共享注意力的群体；2) 假设图像中只有一个SA点。这些问题限制了SA检测在实际应用中的适用性并影响性能。

Method: 提出一个两步处理过程：1) 基于群体推理中估计的个体注视注意力热图和群体成员标量生成SA热图；2) 细化初始群体成员关系以考虑初始SA热图，并最终预测SA热图。该方法实现了端到端的群体检测和共享注意力联合估计。

Result: 实验证明该方法在群体检测和共享注意力估计两个任务上均优于其他方法。附加分析验证了所提出组件的有效性。

Conclusion: 通过联合处理群体检测和共享注意力估计，该方法克服了现有方法的局限性，提高了SA检测的实际应用性和性能，代码已开源。
...
TL;DR: SteerFlow是一种模型无关的基于流的生成模型编辑框架，通过隐式轨迹拉直和轨迹插值技术，在保持源图像保真度的同时实现文本引导的图像编辑。


<details>
  <summary>Details</summary>
Motivation: 现有的基于流的生成模型编辑方法存在源图像保真度保持不足的问题：高阶求解器需要额外的模型推理，截断反转限制了可编辑性，特征注入方法缺乏架构可转移性。

Method: 提出了三个核心方法：1) 摊销固定点求解器，通过强制连续时间步的速度一致性隐式拉直前向轨迹；2) 轨迹插值，自适应混合目标编辑和源重建速度；3) 自适应掩码机制，通过概念引导分割和源-目标速度差异空间约束编辑信号。

Result: 在FLUX.1-dev和Stable Diffusion 3.5 Medium上的实验表明，SteerFlow相比现有方法实现了更好的编辑质量，且能自然扩展到复杂的多轮编辑范式而不产生漂移累积。
...
TL;DR: 提出DPMO和RPS两个框架，用于从点标注生成密集人群实例分割，通过整合SAM和NNEC约束提升分割质量，并在多个数据集上取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 人群数据集通常只有点标注而缺乏准确区域标注（如边界框），直接应用SAM等基础模型在密集人群场景效果不佳，需要从点标注生成高质量掩码标注来提升分割和计数精度

Method: 1. DPMO：整合SAM与最近邻排他圆约束，从点标注生成密集实例分割掩码；2. RPS：强化点选择框架，使用组相对策略优化训练，从初始点预测中选取最佳点；3. 设计新的掩码监督损失函数

Result: 在ShanghaiTech、UCF-QNRF、JHU-CROWD++和NWPU-Crowd数据集上达到最先进的人群实例分割性能，设计的掩码监督损失函数在不同模型上显著提升计数精度

Conclusion: 提出的DPMO和RPS框架有效解决了从点标注生成密集人群实例分割的问题，证明了掩码标注在提升人群计数精度中的重要作用，为密集场景分割提供了新方法
...
TL;DR: 本文提出了一种几何感知的无人机跨视图地理定位框架，通过显式建模3D场景几何，将粗粒度地点检索和细粒度姿态估计统一到单一推理流程中，在GNSS拒止环境下实现稳健的米级定位精度。


<details>
  <summary>Details</summary>
Motivation: 无人机在GNSS拒止环境下的跨视图地理定位面临挑战，主要原因是倾斜的无人机图像与正交卫星地图之间存在严重的几何差异。现有方法通常采用解耦的地点检索和姿态估计流程，隐含地将透视畸变视为外观噪声而非显式几何变换，限制了定位精度。

Method: 提出几何感知的无人机地理定位框架，使用视觉几何基础Transformer从多视图无人机图像序列重建局部3D场景，渲染虚拟鸟瞰图表示，通过正交校正使无人机视角与卫星图像对齐。引入卫星级注意力块处理多个位置假设，防止候选位置间的干扰并保持线性计算复杂度。发布了重新校准的University-1652数据集，包含精确坐标标注和空间重叠分析。

Result: 在精细化University-1652基准和SUES-200上的广泛实验表明，该方法显著优于最先进的基线方法，在复杂城市环境中实现稳健的米级定位精度和改善的泛化能力。

Conclusion: 该方法通过显式建模3D场景几何，提供了一种统一的框架来处理跨视图地理定位中的几何差异问题，实现了端到端的精确定位，为无人机在GNSS拒止环境下的自主导航提供了有效解决方案。
...
TL;DR: 提出Ultrasound-CLIP框架，构建包含365K样本的大规模超声图像-文本数据集US-365K，建立超声诊断分类学（UDT），通过语义感知对比学习提升超声图像的表示学习能力。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言预训练模型（如CLIP）主要针对其他模态设计，难以直接应用于超声数据。超声图像具有异构的解剖结构和多样的诊断属性，需要专门的方法来弥合这一鸿沟。

Method: 1) 构建US-365K大规模数据集（365K配对样本，52个解剖类别）
2) 建立超声诊断分类学（UDT），包含两个分层知识框架：超声分层解剖分类学和超声诊断属性框架（九个诊断维度）
3) 提出Ultrasound-CLIP框架，引入语义软标签和语义损失来细化样本区分
4) 构建基于UDAF文本表示的异构图模态，支持病灶-属性关系的结构化推理

Result: 在患者级数据划分的广泛实验中，该方法在分类和检索基准上实现了最先进的性能，同时在零样本学习、线性探测和微调任务上表现出强大的泛化能力。

Conclusion: 通过构建大规模超声数据集、建立结构化的诊断分类学和提出语义感知对比学习框架，成功开发了专门针对超声成像的视觉-语言预训练模型，有效解决了超声数据的异构性和诊断多样性挑战。
...
TL;DR: 本文提出了一种轻量级架构和训练策略，用于视频扩散模型的通用条件控制，通过特征解耦实现视频域转移和3D生成等任务。


<details>
  <summary>Details</summary>
Motivation: 现有的视频模型通常通过感知、几何或简单语义信号进行条件控制，主要用作生成渲染器。同时，大规模自监督学习获得的高维特征正成为视觉模型的通用接口。然而，自监督特征（如DINO）包含了风格、光照和语义等纠缠信息，限制了生成能力。本文旨在利用这些特征进行视频域转移和3D生成任务。

Method: 提出轻量级架构和训练策略，将外观特征与其他需要保留的特征解耦，实现稳健的外观变化控制（如风格化和重光照）。同时展示低空间分辨率可以通过更高特征维度补偿，改善显式空间表示生成渲染的可控性。

Result: 该方法能够有效利用自监督特征作为视频扩散模型的通用条件信号，实现视频域转移和从3D生成视频等任务，同时保持对场景结构和内容的控制。

Conclusion: 通过特征解耦的轻量级架构能够利用自监督特征作为通用条件信号，增强预训练视频扩散模型的控制能力，在保持场景语义和结构的同时实现外观变化，为视频生成和编辑提供更灵活的工具。
...
TL;DR: 研究人员发现大型视觉语言模型(LVLMs)在解决需要多步骤神经符号推理的谜语题(rebus puzzle)方面存在严重缺陷，即使它们具备视觉识别和语言理解能力，但缺乏将两者整合的认知推理"粘合剂"。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型在显式视觉识别方面表现出色，但在需要从视觉线索进行复杂多步骤推理的问题上存在认知差距。谜语题(rebus puzzle)要求模型不仅识别图像内容，还需结合语言先验知识(如成语)进行抽象映射，最终合成出超越像素空间的语义含义。

Method: 引入RebusBench基准测试，包含1,164个谜语题，专门用于评估感知与知识整合的神经符号能力。评估了包括Qwen、InternVL、LLaVA在内的先进模型，分析其精确匹配和语义准确性，并考察模型缩放和上下文学习的影响。

Result: 所有评估模型表现严重不足：精确匹配低于10%，语义准确性低于20%。模型缩放和上下文学习均未带来显著改进。这表明模型虽然具备必要的视觉和语言组件，但缺乏连接这些组件的认知推理能力。

Conclusion: 大型视觉语言模型在需要神经符号推理的认知任务上存在根本性限制。尽管它们拥有强大的感知和语言能力，但缺乏将视觉线索与先验知识整合以产生新语义的认知"粘合剂"。这揭示了当前模型架构在抽象推理方面的局限性。
...
TL;DR: DriveDreamer-Policy是一个统一的驾驶世界-动作模型，通过集成深度生成、未来视频生成和运动规划于单一模块化架构中，实现了几何感知的世界表示和动作规划的统一框架。


<details>
  <summary>Details</summary>
Motivation: 现有的世界-动作模型(WAM)方法通常专注于建模2D外观或潜在表示，缺乏几何基础，而几何基础对于在物理世界中运行的具身系统至关重要。需要将几何感知的世界建模整合到统一的驾驶系统中。

Method: 使用大型语言模型处理语言指令、多视角图像和动作，然后通过三个轻量级生成器分别产生深度、未来视频和动作。学习几何感知的世界表示，并用其在统一框架内指导未来预测和规划，保持模块化和可控延迟。

Result: 在Navsim v1和v2基准测试中，模型在闭环规划和世界生成任务上表现出色：Navsim v1达到89.2 PDMS，Navsim v2达到88.7 EPDMS，优于现有的基于世界模型的方法，同时产生更高质量的深度预测和未来视频。

Conclusion: 显式的深度学习为视频想象提供了互补的好处，提高了规划的鲁棒性。将几何感知的世界表示整合到统一的驾驶世界-动作模型中，可以产生更一致的想象未来和更明智的驾驶动作。
...
TL;DR: FSKD是一个基于知识蒸馏的森林结构监测框架，通过多模态教师模型融合LiDAR和RGBI数据，训练仅使用RGBI图像的SegFormer学生模型来预测冠层高度模型、植物面积指数和叶层高度多样性等森林结构指标。


<details>
  <summary>Details</summary>
Motivation: 尽管机载LiDAR是森林结构监测的金标准，但其成本高昂且采集频率低。需要开发能够从更易获取的RGBI（红绿蓝近红外）图像中准确预测森林结构指标的方法，以实现可扩展的高分辨率森林监测。

Method: 提出FSKD知识蒸馏框架：1）多模态教师模型使用交叉注意力机制融合RGBI图像与LiDAR衍生的平面指标和垂直剖面；2）仅使用RGBI输入的SegFormer学生模型通过学习复制教师的输出；3）在384平方公里的德国萨克森森林数据集（20厘米地面采样距离）上进行训练；4）通过非对称蒸馏和适当的模型容量设计。

Result: 在8个地理分布不同的测试区域上，学生模型实现了最先进的零样本CHM性能：MedAE 4.17米，R²=0.51，IoU 0.87。在MAE指标上比HRCHM/DAC基线模型提升了29-46%（5.81米 vs. 8.14-10.84米），相关性系数更强（0.713 vs. 0.166-0.652）。多模态融合比纯RGBI训练提升10-26%性能。框架还能在时间不匹配条件下工作（冬季LiDAR，夏季RGBI）。

Conclusion: FSKD框架能够从易获取的RGBI图像中准确预测多维度森林结构指标，突破了严格同步采集的限制，为德国数字孪生和国家级数字正射影像项目等应用提供了可扩展的20厘米分辨率操作监测解决方案。
...
TL;DR: 提出GardenDesigner框架，通过智能体链编码江南园林美学原则，实现快速数字园林生成，普通用户可通过文本输入在一分钟内创建江南园林。


<details>
  <summary>Details</summary>
Motivation: 江南园林作为数字资产在影视游戏和数字旅游中潜力巨大，但传统建模依赖专家经验且耗时。需要解决手工建模效率低的问题。

Method: 采用程序化建模的智能体链框架：1)地形分布和道路生成智能体应用水系中心地形和探索路径规则；2)资产选择和布局优化智能体遵循美学文化约束；3)构建GardenVerse知识库增强资产布置；4)在Unity中开发交互界面，支持文本输入生成园林。

Result: 开发了可交互的Unity界面，非专家用户可通过文本输入在一分钟内生成江南园林。实验和人工评估表明，GardenDesigner能生成多样且美观的园林设计。

Conclusion: GardenDesigner框架成功将江南园林美学原则编码到智能体链中，实现了快速、高质量的数字化园林生成，降低了创作门槛，为数字内容创作提供了有效工具。
...
TL;DR: 本文提出了一种一致性感知的单目深度估计框架，利用移动机器人的轮式里程计信息，在连续帧间实现稳定、一致的深度预测。


<details>
  <summary>Details</summary>
Motivation: 现有单目深度估计方法在连续帧之间往往缺乏时间一致性，导致深度估计结果出现抖动，并且在深度范围突变时可能导致估计失败。这些问题在自动驾驶和移动机器人感知系统中尤为突出，需要一种能够保持时间一致性的深度估计方法。

Method: 提出一致性感知的单目深度估计框架，利用移动机器人的轮式里程计信息。具体方法包括：1) 使用相邻帧之间的光流进行三角测量，估计相机姿态和稀疏深度；2) 利用稀疏深度估计更新度量尺度的递归贝叶斯估计；3) 将估计的度量尺度应用于预训练深度估计基础模型预测的相对深度，进行重标定。

Result: 该方法在KITTI、TartanAir、MS2以及自建数据集上进行了评估，实验结果表明该方法能够实现鲁棒且准确的深度估计性能。

Conclusion: 通过结合轮式里程计信息和递归贝叶斯尺度估计，本文提出的方法能够有效解决单目深度估计中的时间不一致性问题，为自动驾驶和移动机器人系统提供了更稳定可靠的深度感知能力。
...
TL;DR: STRIVE是一种用于视频问答的时空强化学习框架，通过构建输入视频的多个时空变体并进行跨文本生成和视觉变体的联合归一化，解决组优化方法中奖励方差低的问题，从而提高视频推理性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于组优化的策略方法在多模态大模型中表现良好，但当回答具有相似正确性时，往往面临奖励方差低的挑战，导致优势估计弱或不稳定。这限制了强化学习在视频问答任务中的效果。

Method: STRIVE框架包括：1) 为每个输入视频构建多个时空变体；2) 在文本生成和视觉变体间进行联合归一化；3) 引入重要性感知采样机制，优先选择与输入问题最相关的帧，同时保持时间覆盖；4) 通过扩展组比较从语言多样性到结构化视觉扰动来丰富奖励信号。

Result: 在六个具有挑战性的视频推理基准测试(VideoMME、TempCompass、VideoMMMU、MMVU、VSI-Bench和PerceptionTest)上，STRIVE在多模态大模型中相比强大的强化学习基线方法取得了持续改进，验证了其有效性。

Conclusion: 结构化时空探索是稳定多模态强化学习和提升视频推理性能的原则性机制。STRIVE通过结合语言和视觉视角的多样性，促进了跨互补视觉视角的鲁棒推理，而非过度拟合单一的时空配置。
...
TL;DR: SafeRoPE：一种用于基于transformer的扩散模型（MMDiT）的轻量级细粒度安全生成框架，通过扰动旋转位置嵌入（RoPE）来抑制不安全语义，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前基于rectified-flow transformers的T2I模型（如SD3、FLUX）虽然生成保真度高，但对多令牌交互触发的不安全语义仍然脆弱。现有缓解方法依赖微调或注意力调制，计算成本高且主要针对U-Net去噪器设计，难以直接适配到transformer-based扩散模型（MMDiT）。

Method: 1) 分析MMDiT注意力机制，发现不安全语义集中在头部级别的可解释低维子空间中；2) 提出SafeRoPE框架，首先在安全关键头部构造不安全子空间，计算潜在风险分数（LRS）；3) 引入头部级RoPE扰动，对查询和键向量嵌入进行风险特定的头部级旋转，精确抑制不安全输出。

Result: 广泛实验表明，SafeRoPE在平衡有效有害内容缓解和效用保护方面达到了SOTA性能，能够在不降低良性内容或图像质量的前提下有效抑制不安全语义。

Conclusion: SafeRoPE为基于transformer的扩散模型提供了一种轻量级、细粒度的安全生成解决方案，通过扰动RoPE实现了对不安全语义的精确抑制，同时在保持生成保真度方面表现优异。
...
TL;DR: 本文提出一种简单但有效的随机标签桥接训练方法，无需人工标注即可使大型语言模型参数适应视觉基础任务，并发现部分桥接训练比完全桥接更有优势。


<details>
  <summary>Details</summary>
Motivation: 现有研究认为语言预训练模型由于参数空间差异而不适合下游视觉任务，因此多集中于跨域迁移而非跨模态（语言与视觉）适应。本文挑战这一假设，探索通过桥接训练使语言模型参数适应视觉任务的可能性。

Method: 提出随机标签桥接训练作为模态适应学习器，该方法无需人工标注，通过添加一个桥接训练阶段来对齐语言模型参数与视觉任务。特别发现部分桥接训练常更有利，因为语言模型中某些层具有强基础特性，即使未经视觉任务微调也保持有益。

Result: 实验表明桥接训练能有效对齐LLM参数与视觉任务，随机标签桥接训练简单而强大。更重要的是，部分桥接训练往往比完全桥接更有优势，这为直接利用语言预训练参数于视觉模型开辟了新途径。

Conclusion: 语言预训练模型参数可通过桥接训练有效适应视觉任务，部分桥接训练为跨模态适应提供了实用路径。这一发现挑战了传统假设，为利用语言预训练参数直接应用于视觉模型开辟了新方向。
...
### [45] [HieraVid: Hierarchical Token Pruning for Fast Video Large Language Models](https://arxiv.org/abs/2604.01881)
*Yansong Guo,Chaoyang Zhu,Jiayi Ji,Jianghang Lin,Liujuan Cao*

Main category: cs.CV

TL;DR: HieraVid是一个分层剪枝框架，通过逐步动态减少视觉冗余，将视频token保留率降低至30%的同时，在视频理解基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型（VideoLLMs）在视频理解方面表现出色，但大量输入视频token带来了显著的计算负担。现有方法主要在输入级别剪枝视频token，忽略了视频和大语言模型内部的信息结构。

Method: 提出HieraVid分层剪枝框架，基于两个观察：1）视频具有分段-帧结构，2）LLMs内部单向传播多模态信息。将剪枝分解为三个层级：1）分段级-视频token先按时间分段和空间合并；2）帧级-同一分段内相似帧联合剪枝以保持多样性；3）层级级-随着LLM层数增加，冗余逐渐减少而不影响性能。

Result: 在四个广泛使用的视频理解基准测试上进行广泛实验。仅保留30%的token时，HieraVid实现了新的SOTA性能，同时保持了LLaVA-Video-7B的98%以上性能和LLaVA-OneVision-7B的99%以上性能。

Conclusion: HieraVid通过层次化的、渐进式的视频token剪枝方法，有效解决了VideoLLMs的计算负担问题，在显著减少token数量的同时保持了模型性能，为视频大语言模型的高效部署提供了有效解决方案。
...
TL;DR: 提出了一种新颖的半监督域自适应方法，通过排序评分和连续分布对齐来解决医学图像严重程度分类中的域偏移问题，特别适用于具有自然排序标签的严重程度分类任务。


<details>
  <summary>Details</summary>
Motivation: 现有的半监督域自适应方法在医学图像的严重程度分类任务中表现不佳，因为严重程度分类涉及自然排序的类别标签（如轻度、中度、重度），类别边界不清晰，导致域自适应变得更加复杂。

Method: 提出了一种基于排序评分的方法：1）跨域排序：通过考虑类别顺序来学习排序评分，对跨域的样本对进行排序；2）连续分布对齐：对齐源域和目标域之间的排序评分分布。该方法专门针对具有有序标签的严重程度分类问题设计。

Result: 在溃疡性结肠炎和糖尿病视网膜病变的分类任务上进行实验，验证了所提方法的有效性。实验结果表明，该方法能够成功地对齐类别特定的排序评分分布，改善了跨域严重程度分类的性能。

Conclusion: 该方法通过引入排序评分机制和连续分布对齐策略，有效地解决了医学图像严重程度分类中的半监督域自适应问题，特别适用于具有自然顺序类别的场景，为解决这类复杂的域偏移问题提供了新的思路。
...
TL;DR: 提出一种纹理感知的Transformer方法，用于带纹理的3D网格语义分割，通过融合纹理和几何信息，在城市场景和文化遗产数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 虽然已有一些方法直接在网格上进行操作而不施加几何约束，但它们通常忽略了网格提供的丰富纹理信息。当前方法在同时利用几何、拓扑和纹理信息进行3D网格语义分割方面存在不足。

Method: 提出纹理感知Transformer：1) 从每个网格面关联的原始像素学习；2) 新的分层学习方案用于多尺度特征聚合；3) 纹理分支将所有面级像素汇总为可学习token；4) 与几何描述符融合后通过两阶段Transformer块(TSTB)处理，实现局部和全局信息流。

Result: 在Semantic Urban Meshes (SUM)基准测试中达到81.9% mF1和94.3% OA；在新策划的文化遗产数据集(带纹理屋顶瓦片，三角形级别损伤类型标注)中达到49.7% mF1和72.8% OA，显著优于现有方法。

Conclusion: 该方法通过同时利用纹理和几何信息，有效解决了纹理3D网格语义分割问题，为不规则结构的数据处理提供了新思路，在城市场景和文化遗产保护领域展示了实用价值。
...
TL;DR: 提出PI-VQ（置换不变向量量化自编码器），通过移除潜在编码中的位置信息来学习全局语义特征，并引入匹配量化算法提高信息容量，实现无需先验模型的图像插值采样。


<details>
  <summary>Details</summary>
Motivation: 传统向量量化方法（如VQ-VAE、VQ-GAN）学习的离散表示具有位置依赖性，编码空间排列且上下文纠缠，需要自回归或扩散先验来建模依赖性。研究旨在探索位置信息是否是空间对齐数据离散表示的必要条件。

Method: 提出PI-VQ方法，约束潜在编码不携带位置信息；引入匹配量化算法（基于最优二分图匹配的向量量化），相比朴素最近邻量化将有效瓶颈容量提高3.5倍；利用编码的组合结构实现插值采样。

Result: 在CelebA、CelebA-HQ和FFHQ数据集上评估，获得具有竞争力的精度、密度和覆盖率指标；无需学习先验即可直接插值图像；编码捕获全局语义特征。

Conclusion: 置换不变表示能够在无需位置信息的情况下学习有效的离散表示，实现单次前向传播的图像合成。讨论了位置无关表示在可分离性和可解释性方面的权衡，指出了未来研究方向。
...
TL;DR: FaCT-GS是一个基于高斯溅射(Gaussian Splatting)的快速灵活CT重建框架，通过优化体素化和光栅化流水线，相比现有GS方法实现4-13倍加速。


<details>
  <summary>Details</summary>
Motivation: 高斯溅射(GS)在图像渲染中表现出色并已应用于CT重建，但相比成熟的重建算法优势不够明显，且存在速度限制。需要解决GS方法在CT重建中的主要局限性，特别是计算效率问题。

Method: 提出FaCT-GS框架，通过深度优化体素化和光栅化流水线来加速计算。改进的体素化技术能够快速将高斯拟合到现有体积数据，可作为先验用于"热启动"重建或作为压缩表示。

Result: 在标准512×512投影上比现有最优GS CT重建方法快4倍以上，在2k投影上快13倍以上。方法具有良好的可扩展性，能适应投影和输出体积尺寸的变化。

Conclusion: FaCT-GS解决了基于GS的CT重建方法的关键限制，通过显著的速度提升和灵活性，使其更适用于实际应用。改进的体素化技术提供了新的应用可能性，如先验引导重建和压缩表示。
...
TL;DR: 现代视觉语言模型在基础几何变换下表现出系统性的脆弱性：虽然擅长语义任务，但缺乏稳健的空间不变性和等变性，导致在简单旋转、缩放等变换下无法可靠识别物体。


<details>
  <summary>Details</summary>
Motivation: 研究旨在揭示当前最先进的视觉语言模型在几何推理能力上的根本局限性。虽然这些模型在语义理解方面表现出色，但研究者发现它们在更基础的空间不变性和等变性方面存在系统性缺陷，这限制了模型在现实世界应用中的鲁棒性。

Method: 通过系统性评估方法，在多视觉领域（包括符号草图、自然照片和抽象艺术）测试模型对基本几何变换（旋转、缩放、恒等变换）的鲁棒性。研究了不同架构、模型容量和提示策略下的性能变化。

Result: 性能随着语义内容稀疏化而急剧下降。在简单几何变换下，模型在物体识别方面表现出系统性失败。这种行为在不同架构、模型容量和提示策略中普遍存在，表明这是当前视觉语言模型的共性弱点。

Conclusion: 当前视觉语言模型在语义理解和空间推理之间存在系统性差距。研究结果强调了未来多模态系统需要更强的几何基础，以实现更鲁棒和可靠的视觉理解能力。
...
TL;DR: 论文提出了一种轻量级双损失训练框架，通过仅添加一个额外输出通道和两个辅助损失项来改进时序动作分割的精细粒度分割质量，而无需大量修改架构。


<details>
  <summary>Details</summary>
Motivation: 近年来时序动作分割领域越来越多地依赖复杂架构，这阻碍了实际部署。现有方法需要大量架构修改或推理时细化，缺乏轻量级、架构无关的改进方案。

Method: 提出了一个轻量级双损失训练框架，包含：1）边界回归损失：通过单通道边界预测促进准确的时间定位；2）CDF基分段级正则化损失：通过匹配预测分段和真实分段的累积分布来鼓励连贯的段内结构。该框架与架构无关，可集成到现有TAS模型中作为训练时损失函数。

Result: 在三个基准数据集上，该方法改善了分段级一致性和边界质量，在三种不同模型（MS-TCN、C2F-TCN、FACT）上都获得了更高的F1和编辑分数。帧级精度基本保持不变。

Conclusion: 精确的分割可以通过简单的损失设计实现，而不需要更重的架构或推理时细化。该方法证明了通过最小化架构修改和训练时损失优化，可以显著改善时序动作分割的质量。
...
TL;DR: MAR-MAER是一个创新的分层自回归框架，通过度量感知嵌入正则化和概率潜在模型，解决文本到图像生成中的质量不一致和模糊提示处理问题，在度量一致性和语义灵活性方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 自回归模型在文本到图像生成中取得了显著成功，但面临两个主要挑战：1) 生成的图像质量可能不符合人类预期标准；2) 难以处理可以有多种有效解释的模糊提示。

Method: 提出MAR-MAER分层自回归框架，包含两个主要组件：1) 度量感知嵌入正则化方法，使用轻量级投影头和自适应核回归损失函数，将模型内部表示与人类偏好度量（如CLIPScore和HPSv2）对齐；2) 条件变分模块，在分层令牌生成过程中引入受控随机性，处理模糊语义。

Result: 在COCO和新开发的模糊提示基准上的实验显示，MAR-MAER在度量一致性和语义灵活性方面表现优异，相比基线Hi-MAR模型在CLIPScore上提升+1.6，在HPSv2上提升+5.3，对于模糊输入能产生更广泛多样的输出，结果通过人工评估和自动指标验证。

Conclusion: MAR-MAER框架有效解决了自回归文本到图像生成中的图像质量不一致和模糊提示处理问题，通过将模型表示与人类偏好度对齐并引入受控随机性，实现了高质量的多样化图像生成。
...
TL;DR: A3R：一个用于3D高斯场景中细致可及性推理的智能体框架，通过跨维度证据采集的序列化决策过程来提升推理准确性


<details>
  <summary>Details</summary>
Motivation: 现有方法将3D高斯场景中的可及性推理视为静态一次性预测问题，假设已有足够证据进行推理。但在复杂3D场景中，许多失败案例并非源于预测能力不足，而是在固定观察下任务相关证据不完整所致。需要解决这一局限性。

Method: 将细致可及性推理重新表述为序列化证据获取过程，通过互补的3D几何和2D语义证据逐步减少模糊性。提出A3R框架：基于MLLM的策略迭代选择证据采集动作，通过跨维度证据采集更新可及性信念。引入基于GRPO的策略学习策略优化序列决策。

Result: 在场景级基准测试上的广泛实验表明，A3R一致超越静态一次性基线方法，证明了智能体跨维度证据采集在复杂3D高斯场景中细致可及性推理方面的优势。

Conclusion: 智能体化的跨维度证据采集框架能有效解决复杂3D场景中由于证据不完整导致的可及性推理问题，序列化证据获取过程相比静态一次性预测具有明显优势。
...
TL;DR: GS²通过基于图的空间分布优化，在保持高质量渲染的同时显著减少了3DGS的高斯点数量（仅需约12.5%的点），解决了3D高斯泼溅技术的内存消耗问题。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅(3DGS)在新视角合成和实时渲染方面表现出突破性性能，但由于大量高斯点导致高内存成本，限制了其实用性。现有的剪枝方法往往损害空间一致性并导致渲染伪影。

Method: 提出基于图的空间分布优化框架GS²：1) 基于证据下界(ELBO)的自适应致密化策略，自动控制致密化过程；2) 透明度感知的渐进式剪枝策略，动态移除低透明度高斯点以减少内存；3) 基于图的特征编码模块，通过特征引导的点位移调整空间分布。

Result: 与原始3DGS相比，GS²仅需约12.5%的高斯点即可达到更高的PSNR。在渲染质量和内存效率方面均优于所有基线方法，实现了紧凑的高斯表示同时提供卓越的渲染质量。

Conclusion: GS²通过优化高斯点的空间分布，在保持3DGS优势的同时显著降低内存需求，为解决3D高斯泼溅技术的内存瓶颈问题提供了一种有效的解决方案，平衡了渲染质量与存储效率。
...
TL;DR: 现代文生图模型存在低成本的提示词越狱攻击漏洞，只需自然语言提示就能绕过安全过滤器生成受限内容，攻击成功率最高达74.47%


<details>
  <summary>Details</summary>
Motivation: 尽管文生图模型部署了安全过滤器和审核机制来防止滥用，但仍存在被低成本越狱攻击的风险。研究人员发现仅通过自然语言提示就能绕过现有安全措施，暴露了当前安全机制的薄弱环节。

Method: 提出了系统性的基于提示词的越狱攻击研究，无需模型访问、优化或对抗训练。建立了视觉越狱技术分类法，包括：艺术重构、材料替换、伪教育框架、生活方式美学伪装和模糊动作替换等策略。这些策略通过在良性语义上下文中掩盖不安全意图来利用提示词审核和视觉安全过滤的弱点。

Result: 在多个最先进的文生图系统上评估这些攻击，证明简单的语言修改就能可靠地规避现有安全措施并生成受限图像。在所有测试模型和攻击类别中，观察到攻击成功率最高达74.47%。

Conclusion: 研究结果凸显了表面级提示词过滤与检测生成媒体系统中对抗意图所需的语义理解之间存在关键差距。当前的安全机制在理解深层语义和意图方面存在严重不足，需要更高级的语义理解和上下文分析来应对此类攻击。
...
TL;DR: ProVG是一个新颖的遥感视觉定位框架，通过将语言表达解耦为全局上下文、空间关系和对象属性，采用渐进式跨模态调制器实现从粗到细的视觉-语言对齐，显著提升了定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有的遥感视觉定位方法通常依赖句子级别的视觉-语言对齐，难以充分利用细粒度语言线索（如空间关系和对象属性），而这些线索对于区分具有相似特征的物体至关重要。这些线索在不同定位阶段扮演不同角色，应该被相应利用以提供更明确的指导。

Method: 1. 将语言表达解耦为全局上下文、空间关系和对象属性三个部分；2. 采用渐进式跨模态调制器，通过"survey-locate-verify"方案动态调制视觉注意力，实现从粗到细的视觉-语言对齐；3. 引入跨尺度融合模块以缓解遥感图像的大尺度变化；4. 使用语言指导的校准解码器在预测时细化跨模态对齐；5. 统一的多任务头支持指代表达理解和分割任务。

Result: 在两个基准数据集RRSIS-D和RISBench上的大量实验表明，ProVG一致性地超越了现有方法，达到了新的最先进性能。

Conclusion: ProVG通过解耦语言表达和渐进式跨模态调制，有效地利用了细粒度语言线索，显著提升了遥感视觉定位的准确性，为处理遥感图像中的大尺度变化和复杂语言表达提供了有效的解决方案。
...
TL;DR: SHARC是一个新颖的形状合成框架，使用球谐函数表示距离场，通过优化放置内部参考点来捕捉表面细节，在重建精度和时间效率上都优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在形状合成中难以有效捕捉表面精细细节，同时保持计算效率。需要一种既能准确重建任意拓扑形状（包括复杂曲面），又能高效计算的方法。

Method: 1. 在曲面内部体积中优化放置参考点，最大化稀疏性、中心性和表面可见性；2. 从每个参考点通过光线投射采样可见距离场；3. 使用快速球谐变换计算球谐系数；4. 应用可配置的低通滤波增强几何保真度；5. 基于邻近度的局部一致性约束细化输出。

Result: SHARC在重建精度和时间效率方面都优于最先进的方法，同时保持了模型的简洁性。该方法能够处理任意拓扑形状，包括复杂曲面。

Conclusion: SHARC框架通过优化的球谐函数距离场表示，实现了高精度、高效率的形状合成，在保持模型简洁性的同时显著提升了重建质量，为任意拓扑形状的表示提供了有效解决方案。
...
TL;DR: FTPFusion是一种基于频率感知的红外与可见光视频融合方法，通过时域扰动和稀疏跨模态交互来平衡时空稳定性与细节保留


<details>
  <summary>Details</summary>
Motivation: 红外与可见光视频融合在智能监控和低光监测中至关重要，但现有方法难以同时维持时序稳定性和保留空间细节：要么只关注帧级增强而时序建模有限，要么依赖重型时空聚合而牺牲高频细节
...
Result: 在多个公开基准测试上的大量实验表明，FTPFusion在空间保真度和时序一致性方面均持续优于最先进方法

Conclusion: FTPFusion通过频率感知分解、稀疏跨模态交互和时域扰动策略，有效解决了红外可见光视频融合中的时空平衡问题，在保持时序稳定性的同时更好地保留了空间细节
...
TL;DR: 提出Light-ResKAN模型，通过将ResNet中的卷积替换为KAN卷积，使用Gram多项式作为激活函数，并采用参数共享策略，在保持SAR图像识别精度的同时显著降低了计算复杂度和参数数量。
...
Method: 1) 将ResNet中的卷积替换为KAN卷积，实现自适应特征提取；2) 使用Gram多项式作为激活函数，更适合SAR数据捕捉复杂非线性关系；3) 采用参数共享策略，每个通道共享参数，在保持独特特征的同时减少参数和FLOPs。

Result: 在MSTAR、FUSAR-Ship和SAR-ACD数据集上分别达到99.09%、93.01%和97.26%的准确率。在1024×1024尺寸的MSTAR数据集上，相比VGG16减少82.90倍的FLOPs和163.78倍的参数。

Conclusion: 提出的Light-ResKAN模型为边缘设备上的SAR图像识别提供了一个高效的解决方案，在保持高识别精度的同时显著降低了计算和存储需求。
...
TL;DR: 提出了一种基于2D几何和星座匹配的多重角膜反光检测与匹配管道，专注于可重复性和清晰评估，通过类似星体识别的约束条件适应方法实现稳定的身份保持对应关系。


<details>
  <summary>Details</summary>
Motivation: 角膜反光检测在瞳孔-角膜反射眼动追踪中很重要，但实践中常作为启发式方法嵌入大型系统，使得不同硬件设置下的可重复性困难。需要一种明确分离检测与对应关系、易于复现的方法。

Method: 引入2D几何驱动的基于星座的多重角膜反光检测与匹配管道。受"迷失太空"星体识别启发，将角膜反光视为结构化星座而非独立斑点。提出相似度-布局对齐(SLA)程序，将星座匹配适配到多LED眼动追踪的特定约束中。框架整合了受控过度检测、自适应候选回退、外观感知评分和可选语义布局先验。

Result: 在公共多LED数据集上评估，系统在噪声条件下提供稳定的身份保持对应关系。框架能够有效处理噪声条件下的角膜反光检测与匹配问题。

Conclusion: 该几何驱动框架为多LED眼动追踪提供了一种可重复、明确评估的角膜反光检测解决方案，通过公开代码、预设和评估脚本促进透明复制、比较和数据集标注。
...
TL;DR: KnowMVG：基于知识先验和全局-局部注意力增强的医学视觉定位框架，通过编码医学知识到紧凑嵌入和联合利用全局-局部注意力来提升医学图像中诊断相关短语的定位精度


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在医学视觉定位中缺乏空间精度，主要因为仅依赖潜在嵌入而缺少显式的定位先验知识，导致定位精度不足，无法满足临床决策的可解释性需求

Method: 提出KnowMVG框架：1) 知识增强提示策略，将短语相关医学知识编码为紧凑嵌入；2) 全局-局部注意力机制，联合利用粗粒度全局信息和细粒度局部线索来指导精确区域定位；设计在不解码过程中引入额外文本推理开销的情况下，桥接高级语义理解和细粒度视觉感知

Result: 在四个MVG基准测试上的广泛实验表明，KnowMVG始终优于现有方法，在AP50上提升3.0%，在mIoU上提升2.6%优于先前最先进方法。定性和消融研究进一步验证了每个组件的有效性

Conclusion: KnowMVG通过显式加强解码过程中的空间意识，成功解决了视觉语言模型在医学视觉定位中的空间精度不足问题，为临床决策提供更准确的可解释视觉证据，实现了高级语义理解和细粒度视觉感知的有效结合
...
TL;DR: 本文研究表明，可以直接从波束形成前的每天线距离-多普勒（RD）张量中学习有意义的空间结构，而无需显式构建角度域表示或手工信号处理阶段。


<details>
  <summary>Details</summary>
Motivation: 汽车雷达感知管道通常先通过波束形成构建角度域表示，然后再应用学习模型。本研究探讨一个表征性问题：是否可以直接从波束形成前的每天线距离-多普勒（RD）测量中学习有意义的空间结构？

Method: 采用6TX×8RX（48个虚拟天线）商用汽车雷达，使用A/B啁啾序列频率调制连续波（CS-FMCW）发射方案（其中有效发射孔径随啁啾变化）。在波束形成前的每天线RD张量上操作，使用双啁啾共享权重编码器进行端到端全数据驱动训练。评估使用鸟瞰图（BEV）占据率作为几何探测指标，而非性能驱动目标。监督是可见性感知和跨模态的，源自激光雷达，通过基于射线的可见性建模雷达视场和遮挡感知。

Result: 通过啁啾消融实验（仅A、仅B、A+B）、距离带分析和物理对齐基线，评估发射配置对几何可恢复性的影响。结果表明，空间结构可以直接从波束形成前的每天线RD张量中学习，无需显式的角度域构建或手工信号处理阶段。

Conclusion: 该工作证明了从波束形成前的原始雷达信号中直接学习空间表征的可行性，为汽车雷达感知提供了一种新的表示学习方法，绕过了传统的角度域构建过程，可能为更高效的雷达感知架构开辟新途径。
...
TL;DR: 论文提出了ECAC大规模基准数据集和RSRS混合训练框架，开发了KinderMM-Cap-3B模型，显著提升早期教育场景的图像描述专业性和准确性。


<details>
  <summary>Details</summary>
Motivation: 当前早期儿童教育(ECE)图像描述面临两大挑战：缺乏大规模特定领域数据集，导致模型无法捕捉ECE场景的细粒度语义概念；传统训练范式在提升专业物体描述能力方面存在局限，监督学习倾向于高频表达，而强化学习在困难样本上优化不稳定。

Method: 1) 构建ECAC大规模基准数据集：包含256,121张真实世界图像，配有专家级描述和细粒度标签，并引入教学玩具识别分数(TTS)作为评估指标；2) 提出RSRS混合训练框架：动态交替使用强化学习和监督微调，将零奖励的困难样本重定向到监督微调，缓解优势崩溃；3) 开发KinderMM-Cap-3B多模态大语言模型。

Result: 开发的KinderMM-Cap-3B模型在TTS指标上达到51.06，显著优于现有基线方法，同时在描述质量方面保持优越性能。

Conclusion: 该研究通过构建专用数据集和创新训练框架，成功解决了ECE图像描述的专业性和准确性问题，为专门化教育应用提供了有效的技术方案，展示了领域自适应多模态模型在教育场景中的潜力。
...
TL;DR: 提出AMIMV方法解决医学影像分类中的标签稀缺和类别不平衡问题，并通过实验验证其在长尾分布下的有效性


<details>
  <summary>Details</summary>
Motivation: 医学影像分析面临两大问题：1) 标记训练数据稀缺；2) 数据不平衡（常见类别数据丰富，罕见类别数据有限）。现有自监督学习方法(SSL)虽能部分解决第一个问题，但在处理不平衡数据方面的鲁棒性在医学图像分类领域鲜有研究。

Method: 提出AMIMV方法，扩展先前MIMV方法，采用新的增强策略构建非对称多图像多视角对，以解决数据稀缺和类别不平衡问题。同时系统评估了8种代表性SSL方法在11个医学影像数据集(MedMNIST)上的表现，分析其在长尾分布和有限监督下的鲁棒性。

Result: 在MedMNIST数据集上的实验结果显示性能显著提升：retinaMNIST提高4.25%，tissueMNIST提高1.88%，DermaMNIST提高3.1%。AMIMV方法在多种医学影像数据集上对长尾分布表现出良好的鲁棒性。

Conclusion: AMIMV方法能有效处理医学影像分类中的标签稀缺和类别不平衡问题，为医学影像分析中的自监督学习在长尾分布场景下的应用提供了有价值的解决方案。
...
TL;DR: 使用nnU-Net v2框架提出了一种基于深度学习的多模态mpMRI前列腺自动分割方法，在交叉验证中达到0.96 Dice分数，在外部验证中达到0.82 Dice分数，显著优于通用分割工具。


<details>
  <summary>Details</summary>
Motivation: 手动分割前列腺费时且存在观察者间差异，通用分割工具在前列腺特定任务中准确性不足，需要专门的多模态自动分割方法来支持临床应用和研究。

Method: 采用nnU-Net v2框架，利用多模态mpMRI数据（T2加权成像、扩散加权成像和表观扩散系数图），在PI-CAI数据集的981个病例上进行训练，使用5折交叉验证和外部验证（Hospital La Fe的54名患者）评估性能。

Result: 交叉验证中平均Dice分数为0.96±0.00，外部测试集上为0.82；相比之下，通用方法（TotalSegmentator）仅获得0.15 Dice分数，主要原因是腺体分割不足。模型已容器化并作为现成的推理工具提供。

Conclusion: 该研究表明任务特定、多模态分割策略的重要性，证明了所提方法可靠集成到临床研究工作流程的潜力。高精度和良好的泛化能力展示了其在临床应用中的价值。
...
TL;DR: 提出首个用于评估多模态大语言模型在个性化问答中自我定位能力的数据集MyEgo，发现现有MLLMs在该任务上表现不佳，准确率远低于人类水平，凸显了自我定位和长期记忆在自我中心视频中的重要性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在需要自我定位的个性化问答任务中能力不足，缺乏专门的评估数据集。自我定位是指理解自我中心视频中摄像机佩戴者的能力，这对于个性化助手应用至关重要。

Method: 1. 创建MyEgo数据集：包含541个长视频和5,000个个性化问题，涵盖"我的物品"、"我的活动"和"我的过去"三个类别。2. 系统评估各种MLLMs变体：开源vs专有模型、思维链vs非思维链模型、不同规模模型。3. 分析模型在提供明确证据时的表现变化。

Result: 1. 顶尖封闭模型（如GPT-5）准确率仅约46%，开源模型（如Qwen3-VL）仅约36%，分别比人类低40%和50%。2. 显式推理和模型规模扩展未能带来一致改进。3. 当提供相关证据时模型表现提升，但随时间推移增益下降，表明模型在跟踪和记忆"我"和"我的过去"方面存在局限。

Conclusion: 研究强调了自我定位和长期记忆在自我中心视频个性化问答中的关键作用。MyEgo数据集和分析有望推动自我中心个性化助手领域的进展。数据集和代码已开源。
...
TL;DR: SDesc3D：一个基于短文本描述的3D室内场景生成框架，通过多视图结构先验和区域功能推理增强3D布局推理能力


<details>
  <summary>Details</summary>
Motivation: 现有基于文本描述的3D场景生成方法在物理合理性和细节丰富度方面存在不足，特别是当文本描述较短时。这些方法过度依赖明确的语义关系和空间关系线索，限制了在稀疏文本指导下的3D推理能力

Method: 提出三个核心技术：1）多视图场景先验增强，通过聚合多视图结构知识丰富欠确定的文本输入；2）功能感知布局接地，利用区域功能接地进行隐式空间锚定和分层布局推理；3）迭代反射-矫正方案，通过自我矫正实现渐进式结构合理性优化

Result: 大量实验表明，该方法在短文本条件下的3D室内场景生成任务上优于现有方法

Conclusion: SDesc3D框架通过整合多视图结构先验和功能推理能力，有效解决了短文本条件下3D场景生成的物理合理性和语义一致性问题，为交互式3D环境构建提供了新的解决方案
...
TL;DR: 该论文提出交互式跟踪新范式，构建了首个大规模交互式跟踪基准InteractTrack，并提出Interactive Memory-Augmented Tracking (IMAT)基线方法，使跟踪器能够通过自然语言指令接收用户指导。


<details>
  <summary>Details</summary>
Motivation: 现有视觉跟踪器主要工作在非交互、发射即弃的模式下，这种模式在需要人在回路适应的现实场景中不实用。现有跟踪器无法接收用户指导和适应变化的环境。

Method: 论文提出交互式跟踪新范式，构建了包含150个视频、密集边界框标注和时间戳语言指令的InteractTrack基准，并提出Interactive Memory-Augmented Tracking (IMAT)方法，该方法采用动态记忆机制学习用户反馈并相应更新跟踪行为。

Result: 评估了25个代表性跟踪器，发现在交互场景中表现不佳，传统基准上的强性能无法迁移。IMAT方法为交互式跟踪提供了有效基线，基准、协议和基线为开发更智能、自适应和协作的跟踪系统奠定了基础。

Conclusion: 交互式跟踪填补了自动感知与人工指导之间的差距，为开发智能、自适应和协作的跟踪系统建立了基础，推动了跟踪技术向更实用和交互方向发展。
...
TL;DR: Curia-2是一个针对CT和MRI分析的改进版医学影像基础模型，通过优化的预训练策略提升表示质量，支持扩展到10亿参数ViT架构，并在CuriaBench评估框架中建立了2D和3D两个评估轨道。


<details>
  <summary>Details</summary>
Motivation: 医学影像的快速增长给放射科医生带来了不可持续的工作负担，需要基础模型来减轻这种压力。现有CT和MRI基础模型在从复杂放射学体积数据中学习方面仍有很大的优化空间。

Method: 在Curia框架基础上提出Curia-2，改进原始预训练策略和表示质量，更好地捕捉放射学数据的特异性。方法支持将架构扩展到十亿参数的视觉Transformer，首次实现多模态CT和MRI基础模型的规模化。

Result: Curia-2在视觉任务上优于所有基础模型，在临床复杂任务（如病灶检测）上与视觉-语言模型竞争性相当。研究还扩展和重构了CuriaBench评估框架，分为2D切片视觉模型轨道和3D体积基准测试轨道。

Conclusion: Curia-2通过改进的预训练策略和表示学习，显著提升了医学影像基础模型的性能，为CT和MRI分析提供了可扩展的解决方案。公开模型权重将促进进一步研究。
...
TL;DR: 提出一种训练免费的惯性感知视觉激发(IVE)方法，通过打破多模态大语言模型中视觉注意力的惯性模式，解决认知幻觉问题，显著提升关系推理能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型中的视觉注意力表现出显著的惯性现象：一旦在早期解码步骤中稳定后，注意力大部分保持静态，无法支持认知推理所需的组合性理解。现有的幻觉缓解方法主要针对对象存在或属性的感知幻觉，但对需要对象间关系推理的认知幻觉效果有限。

Method: 提出训练免费的惯性感知视觉激发(IVE)方法：1) 通过标记级注意力分析识别视觉惯性；2) 将认知推理建模为视觉注意力的动态响应性；3) 选择相对于历史注意力趋势动态出现的视觉标记，同时区分表现出惯性行为的标记；4) 引入惯性感知惩罚，防止注意力过度集中于局部区域并限制注意力的持续性。

Result: 大量实验表明，IVE方法在各种基础MLLM模型和多个幻觉基准测试中都表现有效，特别是在认知幻觉方面。该方法能够打破视觉注意力惯性模式，显著提升关系推理能力。

Conclusion: 视觉注意力惯性是多模态大语言模型中认知幻觉的关键因素。提出的IVE方法通过建模认知推理为视觉注意力的动态响应性，有效解决了认知幻觉问题，无需额外训练即可在各种MLLM模型中应用，为改善组合性理解和关系推理提供了新思路。
...
TL;DR: Resonance4D是一个物理驱动的4D动态模拟框架，通过将3D高斯泼溅与质点法结合，使用轻量级但物理表达性强的监督机制，在单张消费级GPU上实现高保真物理驱动的4D模拟，同时显著降低内存消耗。


<details>
  <summary>Details</summary>
Motivation: 现有物理驱动的4D动态模拟方法存在两个主要问题：1) 运动监督依赖在线视频扩散或光流管道，计算成本高；2) 仅优化部分材料参数，限制了复杂材料和动态场景的真实性。需要一种更高效、更完整的物理建模方法。

Method: 提出了Resonance4D框架，将3D高斯泼溅与质点法耦合，引入双域运动监督：空间结构一致性用于局部变形，频域谱一致性用于振荡和全局动态模式。结合零样本文本提示分割与模拟引导初始化，自动将高斯分解为对象部分级区域，支持完整材料参数的联合优化。

Result: 在合成和真实场景实验中，Resonance4D实现了强大的物理保真度和运动一致性，同时将峰值GPU内存从超过35GB降低到约20GB，使单张消费级GPU上的高保真物理驱动4D模拟成为可能。

Conclusion: Resonance4D通过双域运动监督和完整参数物理恢复，解决了现有方法在计算效率和物理真实性方面的矛盾，为高效、高保真的物理驱动4D动态模拟提供了新的解决方案。
...
TL;DR: DR-Seg是一种用于遥感图像开放词汇语义分割的解耦与校正框架，通过将CLIP特征解耦为语义主导和结构主导的子空间，针对性增强结构信息而不破坏语言对齐语义，实现了更精确的边界划分。


<details>
  <summary>Details</summary>
Motivation: 现有方法将CLIP表征视为单一语义空间，无法定位需要结构增强的区域，导致边界划分困难且可能破坏CLIP的语义完整性。为解决这一限制，作者提出了DR-Seg框架。

Method: 1. 将CLIP特征解耦为语义主导和结构主导子空间；2. 在DINO指导下注入高保真结构先验形成精炼分支；3. 通过不确定性引导的自适应融合模块动态集成精炼分支与原CLIP分支进行最终预测。

Result: 在八个基准测试上的综合实验表明，DR-Seg建立了新的最先进水平。

Conclusion: DR-Seg通过解耦CLIP特征的异质性并针对性增强结构信息，有效解决了遥感图像开放词汇分割中语言对齐识别与精细空间划分的双重需求，显著提升了边界划分质量。
...
TL;DR: LinkS²Bench：首个评估视觉语言模型在无人机-卫星协同空间智能的综合性基准，包含17.9k高质量QA对和12个细粒度任务，揭示现有模型与人类基准间的显著差距，并提出跨视图对齐适配器提升性能。


<details>
  <summary>Details</summary>
Motivation: 目前无人机和卫星的空间智能协同对于应急响应和安全操作至关重要，但现有基准仅限于孤立的无人机视频或静态卫星图像，无法评估动态局部到全局的空间映射能力，Vision-Language Models在这方面的表现仍未被充分探索。

Method: 1）构建LinkS²Bench基准，链接1,022分钟动态无人机视频与覆盖超过200km²的高分辨率卫星图像；2）采用LLM辅助流程和严格人工标注，构建17.9k高质量问答对；3）设计12个细粒度任务跨四个维度：感知、定位、关系和推理；4）评估18个代表性VLM；5）提出跨视图对齐适配器以解决对齐瓶颈。

Result: 1）现有VLM与人类基准存在显著性能差距；2）准确跨视图动态对齐是关键瓶颈；3）提出的跨视图对齐适配器显著提升模型性能；4）微调实验证明LinkS²Bench在推进复杂空间推理方面的潜力。

Conclusion: LinkS²Bench填补了评估VLM在无人机-卫星协同空间智能方面的基准空白，揭示了跨视图动态对齐是关键挑战，提出的对齐方法有效提升性能，为推进VLM在复杂空间推理任务上的适应提供了重要工具。
...
TL;DR: 本文提出针对自编码器中空间非均匀采样问题的解决方案，通过自熵损失和样本传播机制来处理空间数据不平衡，提升对统计罕见位置的重建质量。


<details>
  <summary>Details</summary>
Motivation: 自编码器在处理空间非均匀采样的图像内容时面临挑战，这在医学成像、生物学和物理学中很常见。信息丰富的模式通常只出现在特定的图像坐标上，而大多数样本中背景在这些位置占主导，导致重建偏向多数外观，造成罕见空间输入的细节丢失和模糊重建。

Method: 提出两种互补组件：1）基于自熵的损失函数，为统计上不常见的空间位置赋予更高权重；2）样本传播，一种重放机制，在训练过程中有选择性地重新将难以重建的样本暴露给模型。同时评估了现有数据平衡策略在无监督重建场景中的局限性。

Result: 在模拟数据集（控制空间不平衡条件）和三个真实世界数据集（物理、生物、天文领域）中验证。该方法在各种重建指标上优于基线方法，特别是在空间不平衡分布下表现突出。结果强调批次中数据表示的重要性，以及在无监督图像重建中关注罕见样本的价值。

Conclusion: 针对自编码器中的空间数据不平衡问题，提出的自熵损失和样本传播机制有效提升了模型对统计罕见位置的重建一致性，证明了处理空间不平衡对改善无监督图像重建质量的重要性。所有代码和相关数据将公开。
...
TL;DR: IndoorCrowd是一个多场景室内人群数据集，包含31个视频（9,913帧），用于人体检测、实例分割和多目标跟踪，通过基准测试评估了三种基础模型自动标注器的性能，并在四个校园场景中建立了基准线。


<details>
  <summary>Details</summary>
Motivation: 当前数据集难以捕获真实室内环境的复杂性，限制了在监控、智能建筑和人机交互等应用中理解人群行为的研究。需要大规模、多场景的室内人群数据集来支持计算机视觉任务的开发与评估。

Method: 1) 在四个校园位置（ACS-EC、ACS-EG、IE-Central、R-Central）收集31个视频（5fps，共9,913帧）；2) 创建带有人工验证的每实例分割掩码标注；3) 使用620帧控制子集评估三种基础模型自动标注器（SAM3、GroundingSAM、EfficientGroundingSAM）的性能；4) 提供2,552帧子集用于多目标跟踪（MOTChallenge格式）；5) 使用YOLOv8n、YOLOv26n、RT-DETR-L检测器与ByteTrack、BoT-SORT、OC-SORT跟踪器建立基准线。

Result: 1) 数据集包含9,913帧带有人工验证的实例分割标注；2) 自动标注器评估显示Cohen's κ、AP、精度、召回率和掩码IoU等指标结果；3) 建立了检测、分割和跟踪的基准性能；4) 场景分析显示ACS-EC场景最具有挑战性（79.3%密集帧，平均实例尺度60.8像素），不同场景的难度差异主要由人群密度、尺度变化和遮挡程度决定。

Conclusion: IndoorCrowd数据集填补了室内人群数据集在规模和场景多样性方面的空白，为检测、分割和跟踪任务提供了高质量基准。场景难度差异分析为未来算法开发提供了重要指导，特别是处理高密度、小尺度和严重遮挡的室内环境。
...
TL;DR: WISE通过"思考两次"范式实现高效推理：首先生成简洁理由和答案，再生成详细解释。训练时使用自蒸馏目标强制简洁理由成为详细解释的充分摘要，推理时只输出简洁理由和答案，并通过WISE-S提示技术处理分布偏移，在ReasonSeg基准上实现SOTA性能的同时将推理长度减少5倍。


<details>
  <summary>Details</summary>
Motivation: Chain-of-Thought（CoT）推理显著提升了大型多模态模型在语言引导分割中的性能，但其生成冗长理由的高计算成本限制了实际应用。需要一种既能保持CoT推理优势又能大幅降低计算开销的高效方法。

Method: WISE采用"思考两次"范式：训练模型生成结构化序列（简洁理由→最终答案→详细解释），通过自回归条件强制简洁理由成为详细解释的充分摘要。使用自蒸馏目标联合奖励语义保真度和简洁性，迫使模型将详细推理内化为紧凑形式。推理时省略详细解释，通过WISE-S提示技术注入简洁性指令处理条件分布偏移，激活学习到的简洁策略。

Result: 在ReasonSeg基准上，WISE-S实现了58.3 cIoU的零样本SOTA性能，同时将平均推理长度从112个token减少到23个token，实现了近5倍的压缩。这证明了在保持高性能的同时显著降低计算成本的有效性。

Conclusion: WISE成功实现了高效推理与高性能的统一，通过"思考两次"范式和自蒸馏训练使模型学习生成简洁但信息丰富的推理，为实际应用中的高效CoT推理提供了可行的解决方案。
...
TL;DR: 提出了Jagle数据集——迄今为止最大的日本多模态预训练数据集，包含约920万个实例，通过多种策略生成视觉问答对，显著提升了日本视觉语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 构建高质量多语言和非英语视觉语言模型面临主要障碍：非英语语言（如日语）的视觉问答数据集在规模和领域覆盖上都有限制。现有的英语数据集构建方法（聚合现有VQA资源）无法直接扩展到其他语言。

Method: 1. 收集异质源数据（图像、图像-文本对、PDF文档）
2. 通过多种策略生成VQA对：
   - 基于VLM的问答生成
   - 翻译
   - 文本渲染
...
Result: 1. 使用Jagle训练的2.2B模型在十个日本评估任务上平均得分超过InternVL3.5-2B
2. 接近Qwen3-VL-2B-Instruct（差距在5分以内）
3. Jagle与FineVision结合使用时，不仅不会降低英语性能，反而比仅使用FineVision训练时英语性能更好
4. 发布了数据集、训练模型和代码以确保可复现性

Conclusion: Jagle数据集通过创新的多策略生成方法有效解决了日本多模态数据稀缺问题，为构建高质量日语视觉语言模型提供了关键资源。该方法不仅提升了日语任务性能，还能与英语训练数据兼容并提升整体性能，展示了在多语言VLM开发中的实用价值。
...
TL;DR: 提出了一种全自动、可扩展的方法论，用于系统评估虚拟人类生成流程中的肤色保真度，包括肤色提取、纹理重着色、实时渲染和定量色彩分析


<details>
  <summary>Details</summary>
Motivation: 准确再现面部肤色对于虚拟人类渲染的真实性、身份保持和公平性至关重要。当前大多数可访问的化身创建流程依赖于缺乏色彩校准的摄影输入，可能导致不一致性和偏见

Method: 1) 定义完整工作流程：肤色和光照提取、纹理重着色、实时渲染和定量色彩分析；2) 使用芝加哥面部数据库图像比较两种肤色提取策略：基于文献的颊部区域采样和基于全脸分析的多维掩码；3) 使用预训练的TRUST框架进行光照隔离；4) 将提取的肤色应用于MetaHuman纹理并在多种光照配置下渲染；5) 在CIELAB色彩空间中使用ΔE度量和个体类型角进行客观评估

Result: 生成了约19,848个渲染实例进行分析，结果显示提取策略表现出表型依赖性行为，且深色肤色的色度误差始终较高

Conclusion: 提出了一种无需手动干预的低计算成本、可大规模评估的方法论框架，揭示了当前虚拟人类生成流程中存在的肤色保真度问题，特别是在深色肤色方面存在系统性误差
...
TL;DR: 论文提出一种新的3D affordance grounding设置，解决多物体场景中功能相似的"混淆对"问题，通过构建首个关注隐含意图的基准数据集CompassAD，并提出CompasNet框架，在实例边界内进行语言-几何对齐，并在几何组和点级别进行对比优化。


<details>
  <summary>Details</summary>
Motivation: 现有3D affordance方法主要评估孤立单物体，且通常需要查询中提供明确的类别名称，忽视了真实场景中多个物体可能具有相同功能但只有一个是任务上下文中合适的情况（称为混淆对）。机器人需要在混淆的多物体场景中根据隐含的自然语言意图选择正确的物体。

Method: 1. 提出"Multi-Object Affordance Grounding under Intent-Driven Instructions"新设置；2. 构建CompassAD基准数据集（包含30个混淆物体对、16种affordance类型、6,422个场景、88K+查询-答案对）；3. 提出CompasNet框架：包含实例边界交叉注入(ICI)模块限制语言-几何对齐在物体边界内，防止跨物体语义泄漏；以及双层对比优化(BCR)模块在几何组和点级别强制区分目标和混淆表面。

Result: 在CompassAD基准上，CompasNet在可见和未见查询上都取得了最先进的结果，并在机器人操纵器上的部署实验证实了其在真实世界混淆多物体场景中有效转移到抓取任务。

Conclusion: 该工作正式定义了新的3D affordance grounding设置，构建了首个关注隐含意图的混淆多物体场景基准，并提出专门设计的框架有效解决了语言-几何对齐中的跨物体语义泄漏问题，提升了在混淆对场景中的affordance grounding性能。
...
TL;DR: PLUME：一种基于潜在推理的通用多模态嵌入框架，用连续潜在状态的自回归展开替代显式思维链，在减少推理开销的同时提升检索性能


<details>
  <summary>Details</summary>
Motivation: 现有的通用多模态嵌入方法通过生成显式思维链来提高性能，但这带来显著的推理开销，并且将丰富的多模态证据压缩到狭窄的文本瓶颈中。需要一种既保持中间推理优势又避免显式推理生成开销的解决方案。

Method: 提出PLUME框架：1）用连续潜在状态的自回归展开替代显式思维链；2）引入语义锚点引导的转换适配器，引导不同推理轨迹；3）采用渐进式显式到潜在课程学习，使用显式推理作为训练支架并逐步转移到潜在状态计算，推理时消除显式思维链。

Result: 在78个任务的MMEB-v2基准测试中，PLUME超越了显式思维链的UMC基线方法，同时将推理从数百个生成令牌减少到少于10个潜在步骤，实现超过30倍的推理加速。在视频和视觉文档检索等证据密集、结构复杂的检索场景中表现尤为出色。

Conclusion: 结构化潜在计算可以保持中间推理的优势而无需显式理由生成的开销，为实际检索系统提供了更强大、更高效的范式。潜在推理框架在多模态检索中具有显著优势，特别是在处理复杂、密集的多模态证据时。
...
TL;DR: GroundVTS：一种用于视频时序定位的Vid-LLM架构，通过查询引导的视觉token采样机制，聚焦信息最丰富的时序片段，避免了均匀帧采样导致的关键帧稀疏问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型(Vid-LLMs)依赖均匀帧采样提取视频信息，导致关键帧分布稀疏且丢失重要时序线索。这限制了Vid-LLMs在视频时序定位任务中的应用效果。

Method: 提出Grounded Visual Token Sampling (GroundVTS)架构：1) 采用细粒度查询引导机制过滤视觉token，保留关键时空信息；2) 引入渐进优化策略，使LLM适应视觉特征的非均匀分布，增强时序依赖建模能力。

Result: 在三个标准VTG基准测试中优于现有方法：在时刻检索任务上mIoU提升7.7点，在亮点检测任务上mAP提升12.0点。

Conclusion: GroundVTS通过聚焦信息丰富时序段落的视觉token采样机制，有效解决了Vid-LLMs在视频时序定位中的关键帧稀疏问题，显著提升了定位精度，为视频大语言模型在更广泛应用中的扩展提供了有效解决方案。
...
TL;DR: LatentUM是一种新型统一模型，通过在共享语义潜在空间中表示所有模态，消除了视觉理解和生成之间像素空间调解的需求，实现了灵活的交错跨模态推理和生成。


<details>
  <summary>Details</summary>
Motivation: 现有统一模型由于视觉理解和生成的分离表示需要像素解码作为桥梁，这种方法既低效又无效。需要一种能够直接在语义层面进行跨模态理解和生成的统一模型。

Method: LatentUM在共享语义潜在空间中表示所有模态，消除了像素空间调解的需求。这种方法允许视觉理解和生成在相同的表示空间中进行，支持灵活的交错跨模态推理。

Result: LatentUM在视觉空间规划基准上达到最先进性能，通过自反思机制推动视觉生成极限，并支持通过预测共享语义潜在空间中的未来视觉状态来进行世界建模。

Conclusion: 共享语义潜在空间表示显著减轻了编解码器偏差并加强了跨模态对齐，使LatentUM能够高效地进行交错跨模态推理和生成，为统一模型设计提供了新方向。
...
TL;DR: 提出CoRegOVCD方法，一种无需训练的开放式词汇变化检测框架，通过一致性正则化解决遥感图像中开放词汇查询概念变化检测的挑战。


<details>
  <summary>Details</summary>
Motivation: 传统遥感变化检测方法假设固定标签空间，无法回答用户定义的任意查询。开放式词汇变化检测(OVCD)旨在获取查询概念的变化掩码，但在无需训练的情况下，密集概念响应直接跨时间比较存在困难：外观变化、弱跨概念竞争以及许多土地覆盖类别的空间连续性通常产生噪声、碎片化和语义不可靠的变化证据。

Method: CoRegOVCD框架将概念特定变化重新定义为校准后验差异。主要技术包括：竞争后验校准(CPC)将原始概念响应转换为竞争感知的查询概念后验；语义后验差异(SPD)量化跨时间差异；几何令牌一致性门(GeoGate)通过几何感知结构验证抑制不支持响应；区域共识差异(RCD)通过区域共识提高空间一致性。

Result: 在四个基准测试上（涵盖建筑导向和多类设置），CoRegOVCD相比之前最强的无需训练基线在F1_C指标上持续提升2.24到4.98个百分点，在SECOND数据集上达到六类平均47.50%的F1_C。

Conclusion: CoRegOVCD通过一致性正则化框架有效解决了开放词汇变化检测中的可比性问题，无需额外训练即可显著提升性能，为遥感变化检测提供了更灵活和准确的方法。
...
TL;DR: 标准的面部动作单元检测评估协议存在随机方差问题，交叉验证本身引入可测量的噪声，留出数据集验证能提供更稳定的结果评估。


<details>
  <summary>Details</summary>
Motivation: 面部动作单元检测领域使用主题独占交叉验证作为标准评估协议，但报告的性能提升通常很小。研究者发现交叉验证本身会引入可测量的随机方差，这可能使一些报告的小幅改进实际上属于协议变异范围内，缺乏统计显著性。

Method: 1. 在BP4D+数据集上进行重复3折主题独占分割，评估交叉验证的随机方差；2. 比较不同评估指标（如F1和AUC）的波动性；3. 使用留出数据集协议（Leave-One-Dataset-Out, LODO）在五个AU数据集上进行跨数据集鲁棒性评估。

Result: 1. 重复3折主题独占分割产生的经验噪声底限为平均F1±0.065，低流行度AU的方差更大；2. 工作点度量（如F1）比与阈值无关的度量（如AUC）波动更大，不同折叠分配下模型排名可能变化；3. LODO协议消除了分割随机性，揭示了单数据集交叉验证无法观察到的域级不稳定性。

Conclusion: 交叉验证中经常报告的性能提升可能落在协议方差范围内，缺乏实际意义。留出数据集交叉验证能提供更稳定且可解释的发现，建议在AU检测研究中采用LODO协议以获得更可靠的评估结果。
...
TL;DR: 本文提出了一种用于图像合成的反射生成方法，通过向基础扩散模型中注入反射位置和外观的先验信息，并采用类型感知的模型设计来处理两种不同的反射类型。该方法在构建的大型对象反射数据集DEROBA上进行训练，能够生成物理一致且视觉真实的反射效果。


<details>
  <summary>Details</summary>
Motivation: 图像合成涉及将前景对象插入背景中，同时合成环境一致的效果如阴影和反射。虽然阴影生成已被广泛研究，但反射生成在很大程度上仍未得到充分探索。本文专门聚焦于反射生成这一研究空白。

Method: 1. 向基础扩散模型注入反射位置和反射外观的先验信息
2. 将反射分为两种类型并采用类型感知的模型设计
3. 构建了首个大规模对象反射数据集DEROBA用于训练
4. 设计了专门的模型架构来处理不同类型的反射

Result: 实验证明，该方法能够生成物理一致且视觉真实的反射效果。该方法在反射生成任务上建立了新的基准，展示了优于现有方法的性能。

Conclusion: 本文提出了一种新颖的反射生成方法，解决了图像合成中反射生成这一相对未探索的问题。通过结合先验信息注入和类型感知设计，以及构建专门的数据集，该方法能够生成高质量的反射效果，为反射生成任务设立了新的标准。
...
TL;DR: ViT-Explainer：一个基于Web的交互式系统，用于可视化Vision Transformer从patch tokenization到最终分类的完整推理过程


<details>
  <summary>Details</summary>
Motivation: Transformer架构已成为NLP和CV的共享骨干，但在视觉领域中理解这些模型如何操作仍具挑战性。现有可解释性工具往往专注于孤立组件或面向专家的分析，缺乏对完整推理管道的引导式端到端理解。

Method: 开发了ViT-Explainer系统，结合了动画演练、patch级注意力覆盖和视觉适配的Logit Lens，提供引导式和自由探索两种模式，展示Vision Transformer从patch tokenization到最终分类的全过程。

Result: 6名参与者的用户研究表明，ViT-Explainer易于学习和使用，能帮助用户解释和理解Vision Transformer的行为。

Conclusion: ViT-Explainer填补了Vision Transformer可解释性工具中的空白，提供了集成的可视化方案，有助于增强对视觉Transformer模型工作方式的理解。
...
TL;DR: UniDriveVLA：基于混合变换器的统一驾驶视觉-语言-动作模型，通过专家解耦解决感知-推理冲突，在自动驾驶任务中实现先进性能


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作（VLA）模型在自动驾驶任务中面临空间感知与语义推理之间的关键困境：直接采用2D视觉语言模型空间感知有限，而用3D空间表示增强又会损害VLM的原生推理能力

Method: 提出UniDriveVLA，基于混合变换器架构，包含三个专家模块（驾驶理解、场景感知、动作规划），通过掩码联合注意力协调。结合稀疏感知范式和三阶段渐进训练策略，在提升空间感知的同时保持语义推理能力

Result: 在nuScenes的开环评估和Bench2Drive的闭环评估中达到最先进性能。在3D检测、在线建图、运动预测、驾驶导向VQA等多种感知、预测和理解任务上表现强劲，展示了作为统一模型的广泛适用性

Conclusion: 通过专家解耦解决了VLA模型在自动驾驶中的感知-推理冲突，UniDriveVLA能够同时保持优秀的空间感知和语义推理能力，为自动驾驶提供了一个有效的统一模型解决方案
...
TL;DR: SCALE是一个用于零样本骨架动作识别的轻量确定性框架，通过语义和置信度感知的列表式能量损失来区分语义混淆的动作类别，无需生成样本即可评估未见类别。


<details>
  <summary>Details</summary>
Motivation: 现有零样本骨架动作识别方法依赖于显式的骨架-文本对齐，在动作名称无法详细描述细粒度动态时表现脆弱，且在未见类别语义混淆时效果不佳。需要更鲁棒的方法来处理这些限制。

Method: 提出了SCALE框架，采用条件变分自编码器，冻结的文本表示参数化潜在先验和解码器。引入语义和置信度感知的列表式能量损失，强调语义相似的困难负样本，并利用后验不确定性调整决策边界。同时使用潜在原型对比目标对齐后验均值与文本派生的潜在原型。

Result: 在NTU-60和NTU-120数据集上的实验表明，SCALE始终优于基于VAE和对齐的基线方法，同时与基于扩散的方法保持竞争力。

Conclusion: SCALE通过将零样本动作识别建模为类条件能量排序，提供了更鲁棒和有效的框架，能够处理语义混淆和细粒度动态识别问题，同时保持计算效率。
...
TL;DR: Omni123是一个3D原生基础模型，在单一自回归框架中统一文本到2D和文本到3D生成，利用2D数据作为几何先验改进3D表示


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在文本和图像理解与生成方面表现出色，但扩展到3D领域面临挑战：高质量3D资产稀缺导致3D合成欠约束；现有方法依赖间接流程（2D编辑+3D优化）牺牲了几何一致性

Method: 1. 将文本、图像和3D表示为共享序列空间中的离散标记；2. 引入交错X到X训练范式，在异构配对数据集上协调多样跨模态任务；3. 通过语义-视觉-几何循环（如文本→图像→3D→图像）在自回归序列中联合强制语义对齐、外观保真和多视图几何一致性

Result: 实验表明Omni123显著改进文本引导的3D生成和编辑，展示了向多模态3D世界模型扩展的可扩展路径

Conclusion: Omni123通过统一的自回归框架和利用跨模态一致性作为隐式结构约束，为3D原生基础模型提供了可扩展解决方案，解决了3D数据稀缺和几何一致性问题
...
### [92] [VOID: Video Object and Interaction Deletion](https://arxiv.org/abs/2604.02296)
*Saman Motamed,William Harvey,Benjamin Klein,Luc Van Gool,Zhuoning Yuan,Ta-Ying Cheng*

Main category: cs.CV

TL;DR: VOID是一个视频对象移除框架，能够生成物理上合理的修复结果，特别处理对象间有显著物理交互（如碰撞）的复杂场景


<details>
  <summary>Details</summary>
Motivation: 现有视频对象移除方法擅长处理对象"背后"的内容修复和外观级伪影（如阴影、反射），但当移除对象与其他对象有显著物理交互（如碰撞）时，当前模型无法修正这些交互并产生不合理结果

Method: 1. 使用Kubric和HUMOTO生成新的配对反事实对象移除数据集；2. 推理时，视觉语言模型识别受移除对象影响的场景区域；3. 使用视频扩散模型以这些区域为引导生成物理一致的反事实结果

Result: 在合成和真实数据上的实验表明，相比现有视频对象移除方法，VOID能更好地在对象移除后保持一致的场景动力学

Conclusion: 该框架展示了如何通过高级因果推理使视频编辑模型成为更好的世界模拟器，为实现物理上合理的视频对象移除提供了新途径
...
TL;DR: SPAR (Single-Pass Any-Resolution ViT) 是一种分辨率无关的密集特征提取器，通过特征回归损失将滑动窗口教师模型的空间推理能力蒸馏到单次推理的学生模型中，显著提升了高分辨率推理效率，在开放词汇分割任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 基础视觉变换器(ViT)在需要细粒度空间理解的任务中存在局限性，因为其固定的预训练分辨率和固有的粗粒度补丁级表示。这些挑战在密集预测场景中尤为突出，如基于ViT的视觉语言模型的开放词汇分割任务，其中高分辨率输入对于准确的像素级推理至关重要。现有方法通常使用滑动窗口策略处理大分辨率图像，虽然通过更细的步幅提高了准确性，但计算成本显著增加。

Method: 引入SPAR：单次任意分辨率ViT，这是一种分辨率无关的密集特征提取器。通过特征回归损失将精细步幅滑动窗口教师模型的空间推理能力蒸馏到单次推理的学生模型中，无需架构更改或像素级监督。该方法使模型能够高效处理高分辨率输入。

Result: 在开放词汇分割任务中，SPAR相比单次推理基线提升了高达10.5 mIoU，甚至超越了教师模型的性能。这证明了该方法在高分辨率推理效率和准确性方面的有效性。

Conclusion: SPAR提供了一种高效的高分辨率推理方法，通过知识蒸馏技术成功解决了基础ViT在细粒度空间理解任务中的局限性，为密集预测任务提供了实用的解决方案。
...
TL;DR: 该论文提出了一种基于推理时间引导的文本到图像生成安全控制框架，利用冻结的预训练基础模型提供的梯度反馈来引导生成过程，无需修改底层生成器。该方法将安全引导公式化为基于能量的采样问题，实现了模块化、无需训练的安全控制。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像生成模型安全控制方法通常依赖于模型微调或策划数据集，这会降低生成质量或限制可扩展性。需要一种能够在不修改生成器的情况下实现高效安全控制的解决方案。

Method: 提出了一种推理时间引导框架，利用冻结的视觉语言基础模型作为现成的监督信号。通过在每个采样步骤中通过干净潜在估计注入梯度反馈，将安全引导公式化为基于能量的采样问题。该方法与扩散模型和流匹配模型都兼容，并能在不同视觉概念间泛化。

Result: 实验表明该方法在NSFW红队测试基准上达到了最先进的鲁棒性，并实现了有效的多目标引导。同时，该方法在良性非目标提示上保持了高生成质量，证明了其有效性。

Conclusion: 该框架提供了一种利用基础模型作为语义能量估计器的原则性方法，为文本到图像生成实现了可靠且可扩展的安全控制。该方法具有模块化、无需训练的优势，能够在不牺牲生成质量的前提下确保安全性。
...
TL;DR: 提出可操纵视觉表示方法，通过早期文本融合使视觉特征能够根据自然语言提示关注图像中的任意目标，同时保持基础表示质量


<details>
  <summary>Details</summary>
Motivation: 现有预训练视觉变换器（如DINOv2、MAE）专注于图像中最显著的视觉线索，无法引导至不太突出的感兴趣概念；而多模态大语言模型虽可通过文本提示引导，但其表示偏向语言中心，在通用视觉任务中效果下降

Method: 引入可操纵视觉表示，通过在视觉编码器的各层中注入文本进行早期融合（而非像CLIP等模型的后期融合），使用轻量级交叉注意力机制将文本直接注入视觉编码器

Result: 方法在可操纵性基准测试中表现良好，能够聚焦于图像中任何指定目标同时保持底层表示质量；在异常检测和个性化对象识别任务中匹配或优于专用方法，并在分布外任务中展示零样本泛化能力

Conclusion: 提出了一种新型视觉表示方法，结合了预训练视觉变换器的通用视觉能力和多模态语言模型的引导性，实现了通过自然语言提示可操纵的视觉特征表示
...
TL;DR: ActionParty：首个能够同时控制多达七个玩家的视频扩散世界模型，通过引入主体状态令牌解决了现有模型中的动作绑定问题。


<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型在构建"世界模型"方面取得进展，但主要限于单智能体设置，无法在场景中同时控制多个主体。现有模型存在动作绑定问题，难以将特定动作与其对应的主体关联起来。

Method: 提出ActionParty模型，引入主体状态令牌作为潜在变量，持续捕捉场景中每个主体的状态。通过空间偏置机制联合建模状态令牌和视频潜在表示，实现全局视频帧渲染与个体动作控制的主体更新的解耦。

Result: 在Melting Pot基准测试上进行评估，展示了首个能够在46个多样化环境中同时控制多达七个玩家的视频世界模型。结果显示在动作跟随准确性和身份一致性方面有显著改进，同时能够在复杂交互中实现稳健的自回归主体跟踪。

Conclusion: ActionParty解决了多主体视频生成中的动作绑定问题，为生成式视频游戏开发提供了有效的多主体可控世界模型框架，在动作控制精度和身份保持方面超越了现有方法。
...
TL;DR: 提出了一种快速表面配准方法，通过将表面网格表示为概率度量，使用对数线性计算复杂度的切片Wasserstein距离衡量网格差异，并提出了AdamFlow优化方法在概率空间中最小化该距离


<details>
  <summary>Details</summary>
Motivation: 现有的表面配准方法在效率和鲁棒性之间存在权衡：局部点匹配方法计算高效但对噪声和初始化敏感，全局点集对齐方法计算成本高昂。需要一种既能保持效率又具有鲁棒性的表面配准方法

Method: 1) 将表面网格表示为概率度量，将表面配准转化为分布优化问题；2) 使用具有对数线性计算复杂度的切片Wasserstein距离衡量网格差异；3) 提出AdamFlow优化方法，将Adam优化从欧几里得空间推广到概率空间以最小化切片Wasserstein距离

Result: 理论分析了AdamFlow的渐近收敛性，并通过实验证明其在各种解剖结构的仿射和非刚性表面配准中具有优越性能

Conclusion: 该方法成功解决了表面配准中效率与鲁棒性的权衡问题，提供了一种快速、鲁棒的表面配准解决方案，适用于医学成像中的解剖形状分析
...
TL;DR: LCA提出了一种大规模编解码器化身模型，采用预训练/后训练范式，结合大规模野外视频的高泛化性与高质量数据的保真度，实现了高保真全身3D化身建模并具备世界级泛化能力。


<details>
  <summary>Details</summary>
Motivation: 3D化身建模面临保真度与泛化性之间的权衡：基于多视图工作室数据的方法保真度高但泛化性差；基于大规模野外数据的方法泛化性好但质量低。需要一种能兼顾两者的解决方案。

Method: 提出预训练/后训练范式：1)在100万个野外视频上进行预训练，学习外观和几何的广泛先验；2)在高质量精选数据上进行后训练，增强表达能力和保真度。采用前馈方式实现高效推理。

Result: LCA在发型、服装和人口统计学特征上具有广泛泛化能力，同时提供精确的面部表情和手指级关节控制，保持强身份特征。观察到对重新照明、宽松服装支持的涌现泛化，以及对风格化图像的零样本鲁棒性。

Conclusion: LCA通过大规模预训练/后训练范式成功解决了3D化身建模中保真度与泛化性的权衡问题，为构建高质量、高泛化性的3D化身模型提供了可行方案，展现出在大规模实际应用中的潜力。
...
TL;DR: 论文提出了基于场景的视觉定位新任务，引入了Referring Scenario Comprehension(RSC)基准，包含段落级查询描述目标对象角色、用户意图和上下文线索，并提出了ScenGround课程推理方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位基准主要评估图像区域与字面指称表达的对齐，模型通过匹配显著命名类别就能成功。本文探索更具挑战性的基于场景的视觉定位，目标必须从角色、意图和关系上下文推断而非显式命名。

Method: 引入Referring Scenario Comprehension(RSC)基准，查询为段落长度文本，描述对象角色、用户目标和上下文线索，包含故意引用的干扰对象。提出ScenGround课程推理方法，结合监督热启动和难度感知强化学习。

Result: RSC包含约31k训练样本、4k域内测试样本和3k域外分集。实验显示基于场景的查询暴露了当前模型的系统性失败，标准基准未能揭示这些问题。课程训练提高了在挑战性切片上的性能，并可迁移到标准基准。

Conclusion: 基于场景的视觉定位是一个互补且更具挑战性的设置，RSC基准支持细粒度分析，ScenGround方法为解决这一设置提供了参考点，揭示了当前模型的局限性。
...
TL;DR: ModMap是一个原生的多视角多模态3D异常检测与分割框架，通过跨模态特征映射和视图依赖的特征调制，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法独立处理各个视角，无法有效利用多视角和多模态信息。工业3D异常检测需要处理高分辨率数据，且缺乏专门针对工业数据集的基础深度编码器。

Method: 1) 采用跨模态特征映射范式，学习跨模态和跨视角的特征映射；2) 通过特征级调制显式建模视图依赖关系；3) 引入跨视角训练策略，利用所有可能的视角组合；4) 通过多视角集成和聚合进行异常评分；5) 训练并公开发布针对工业数据集的基础深度编码器。

Result: 在SiM3D基准测试（首个用于3D异常检测与分割的多视角多模态基准）上，ModMap取得了最先进的性能，以较大优势超越了先前的方法。

Conclusion: ModMap框架通过原生多视角多模态处理和跨视角特征映射，显著提升了3D异常检测与分割的性能，为工业应用提供了有效的解决方案。
...
TL;DR: 通过从AAA游戏中提取的大规模动态数据集，结合创新的捕获方法和VLM评估协议，显著提升了生成逆渲染和正向渲染在真实场景中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有合成数据集在真实感和时间一致性方面的局限性，阻碍了生成逆渲染和正向渲染在真实世界场景中的扩展应用。为解决这一领域差距，需要更具真实性和连贯性的数据。

Method: 1. 采用新颖的双屏拼接捕获方法从视觉复杂的AAA游戏中提取数据
2. 创建包含4百万连续帧的大型动态数据集，包含同步的RGB和五个G-buffer通道
3. 涵盖多样场景、视觉效果、恶劣天气和运动模糊变体
4. 提出基于VLM的评估协议来测量语义、空间和时间一致性

Result: 1. 在该数据集上微调的逆渲染器展现出卓越的跨数据集泛化能力
2. 实现了可控生成功能
3. VLM评估与人类判断高度相关
4. 正向渲染器允许用户通过文本提示从G-buffer编辑AAA游戏的风格

Conclusion: 该工作通过高质量的AAA游戏数据集、创新的捕获方法和VLM评估协议，显著推进了双向渲染的发展，解决了真实世界场景中生成逆渲染和正向渲染的关键瓶颈问题。
...
TL;DR: 研究了重复拉长形式（RLF）这种非正式表达方式在情感分析中的重要性，提出了首个多领域RLF数据集Lengthening和可解释指令微调框架ExpInstruct，验证了RLF的表达性和作为文档情感签名的作用。


<details>
  <summary>Details</summary>
Motivation: 虽然语言模型对非正式交流的讨论广泛，但重复拉长形式（RLF）这种独特且富有表现力的风格多年来一直被忽视。需要探究两个研究问题：1）RLF对情感分析是否重要？2）语言模型能否理解RLF？

Method: 1. 构建首个多领域RLF数据集Lengthening（85万个样本）；2. 提出可解释指令微调框架ExpInstruct，采用两阶段指令微调提升LLMs对RLF的性能和可解释性；3. 提出统一方法量化语言模型对非正式表达的理解能力。

Result: 1. RLF句子是表达性表达，可作为文档级情感的签名；2. RLF对在线内容分析具有潜在价值；3. 微调PLMs能超越零样本GPT-4性能，但解释能力不如GPT-4；4. ExpInstruct能提升开源LLMs，使其在有限样本下匹配零样本GPT-4的性能和可解释性。

Conclusion: RLF在情感分析中具有重要价值，ExpInstruct框架有效提升了语言模型对RLF的理解能力和解释能力，为在线非正式语言处理提供了新工具和方法。
...
TL;DR: 论文研究通过训练时强化学习和测试时并行思考两种互补方法来扩展竞争性编程的推理token预算，使用多轮并行思考流水线在有限token预算下实现高效推理。


<details>
  <summary>Details</summary>
Motivation: 需要扩展竞争性编程中的推理token预算以提高问题解决能力，传统单次生成的推理方式在RL训练中成本高昂，需要更高效的推理框架。

Method: 1) 训练时RL：探索验证准确率与生成推理token数之间的对数线性关系，采用验证RL预热和随机截断技术优化训练轨迹；2) 测试时并行思考：设计多轮并行思考流水线，将token预算分配到多个线程和生成-验证-精炼轮次中，进行端到端训练。

Result: 从Seed-OSS-36B模型开始，使用16线程和每线程16轮次的完整系统平均每个问题使用760万个token，在单次尝试(pass@1)下匹配底层RL模型的oracle pass@16性能，并在AetherCode的456个困难竞争性编程问题上超越了GPT-5-high。

Conclusion: 结合训练时RL优化和测试时并行思考流水线的互补方法能有效扩展推理token预算，在竞争性编程任务中实现高效的性能扩展，展示了结构化推理框架的重要价值。
...
TL;DR: M2-Verify：一个用于检查科学主张一致性的新型大规模多模态数据集，包含46.9万个跨16个领域的实例，基准实验显示现有模型在处理视觉复杂性和领域转移时存在困难。


<details>
  <summary>Details</summary>
Motivation: 评估科学论据需要检验主张与其多模态证据之间的严格一致性。现有基准缺乏评估这种对齐所需的规模、领域多样性和视觉复杂性，因此需要一个新的数据集来解决这一差距。

Method: 从PubMed和arXiv获取数据，创建了M2-Verify数据集，包含超过46.9万个实例，涵盖16个领域，并通过专家审核进行严格验证。进行广泛的基准实验，评估最先进模型在处理不同复杂程度挑战时的表现。

Result: 最先进模型在一致性检查上表现不佳：在低复杂性医学扰动上可达85.8% Micro-F1，但在高复杂性挑战（如解剖学转移）上性能下降至61.6%。专家评估还发现模型在生成科学解释时存在幻觉问题。数据集具有实用价值并提供了使用指南。

Conclusion: M2-Verify填补了多模态科学论证评估的数据集空白，揭示了现有模型在处理复杂视觉一致性挑战时的局限性，为未来研究提供了有价值的基准资源和方向指导。
...
TL;DR: 提出一个特征增强的框架来更好地学习语言模型中的人类偏好，通过在文本表示中添加可解释信号（响应长度、拒绝指示器、毒性分数和语义相似度），在多个人类偏好数据集上显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 语言模型中学习人类偏好的根本挑战在于奖励建模依赖于微妙、主观的比较而非清晰的标签，当前方法在多维人类判断捕捉方面存在局限，标准设置的性能较低（ROC AUC < 0.74）。

Method: 使用Anthropic HHRLHF数据集评估十个多样化的大型语言模型，提出特征增强的混合方法：在文本表示中增加可解释信号（响应长度、拒绝指示器、毒性分数、提示-响应语义相似度），以显式捕捉有帮助性、安全性和相关性的关键方面。

Result: 提出的混合方法在所有模型上都获得了持续改进，实现了高达0.84的ROC AUC和显著更高的配对准确性，其中DeBERTa-v3-Large表现最佳。使用SHAP和LIME进行细粒度可解释性分析，发现模型决策依赖于语境化的安全性和支持性框架而非孤立关键词。

Conclusion: 特征增强方法有效地提升了语言模型对人类偏好的学习能力，揭示了多维度信号在偏好建模中的重要性，同时通过可解释性分析加深了对模型决策机制的理解，为进一步改进偏好学习提供了指导。
...
TL;DR: Reasoning Memory：一个检索增强生成（RAG）框架，通过检索和重用程序性知识来提升语言模型在复杂推理任务中的表现，在数学、科学和编程基准测试中显著优于传统RAG方法和计算匹配的测试时扩展基线。


<details>
  <summary>Details</summary>
Motivation: 现有测试时扩展方法通常将每个问题视为独立，未能系统性地重用先前推理轨迹中的知识，特别是对程序性知识（如何重构问题、选择方法、验证或回溯）的利用不足，限制了模型在复杂推理任务中的表现。

Method: 提出Reasoning Memory框架：1）将现有的逐步推理轨迹语料库分解为自包含的子问题-子程序对，构建包含3200万条程序性知识条目的数据存储；2）在推理时，通过轻量级内部提示让模型表达核心子问题，在推理轨迹中检索相关子程序，并在多样化的检索子程序作为隐式程序性先验下进行推理。

Result: 在六个数学、科学和编程基准测试中，Reasoning Memory持续优于基于文档、轨迹和模板知识的RAG方法，以及计算匹配的测试时扩展基线。在较高推理预算下，比无检索方法提升高达19.2%，比最强计算匹配基线提升7.9%。消融研究表明，这些增益来自两个关键因素：源轨迹的广泛程序性覆盖以及分解和检索设计。

Conclusion: Reasoning Memory框架通过系统性地检索和重用程序性知识，有效解决了现有推理方法中知识重用不足的问题，为语言模型在复杂推理任务中的表现提升提供了新的有效途径，展示了程序性知识在推理任务中的重要性。
...
TL;DR: 研究发现语言模型通过稀疏的MLP神经元选择性地表示实体知识，这些神经元集中在早期层，通过因果干预可实现对实体知识的具体调控


<details>
  <summary>Details</summary>
Motivation: 尽管语言模型能够回答实体相关的事实性问题，但其内部机制仍不清楚。研究者希望探究语言模型在实体知识检索和表示方面的内部工作机制。

Method: 使用模板化提示定位实体选择性MLP神经元，在PopQA基准的200个实体上进行实验。通过因果干预（负向消融和可控注入）验证神经元功能，分析其对别名、缩写、拼写错误和多语言形式的鲁棒性。

Result: 1. 实体选择性神经元集中在早期层 2. 负向消融导致实体特异性遗忘 3. 在占位符处注入单个神经元足以恢复实体一致性预测 4. 神经元对实体的多种形式具有鲁棒性 5. 效果强烈但不普遍，更流行的实体覆盖度更高

Conclusion: 语言模型通过稀疏、因果可操作的神经元实现实体知识的紧凑检索而非深度渐进丰富，这为分析和调控实体条件化事实行为提供了具体的切入点。
...
TL;DR: 该论文比较了三种计算翻译过程中打字暂停阈值的方法，并提出并评估了一种新的生产单元断点计算方法。


<details>
  <summary>Details</summary>
Motivation: 在文本生产（包括翻译）过程中，打字活动被按键暂停所打断。通常假设快速打字反映无障碍/自动化的翻译生产，而较长暂停则表明翻译问题或困难。长期以来关于确定区分自动化与反思性翻译过程的暂停阈值存在争议，本文旨在解决这一问题。

Method: 比较了三种近期计算暂停阈值的方法，并提出了一种新的生产单元断点计算方法，并对该方法进行了评估。

Result: 论文通过比较现有方法和提出新方法，为翻译过程研究中暂停阈值的确定提供了新的视角和工具。

Conclusion: 该研究通过系统比较不同的暂停阈值计算方法，并引入新的生产单元断点计算方法，有助于更准确地识别翻译过程中的认知活动，区分自动化与反思性翻译过程，为翻译过程研究提供了方法论上的贡献。
...
TL;DR: MiCP是首个用于多轮推理LLM的Conformal Prediction框架，通过分配不同轮次的误差预算实现提前停止，在保持总体覆盖保证的同时减少轮数、推理成本和预测集大小。


<details>
  <summary>Details</summary>
Motivation: 多轮推理LLM（如自适应RAG和ReAct智能体）在回答困难问题时提高了准确性，但缺乏形式化的停止标准。现有方法依赖启发式停止规则或固定轮数预算，无法保证最终预测包含正确答案，这在金融和医疗等高风险领域尤其成问题，因为不必要的轮次增加成本和延迟，而过早停止又可能导致错误决策。

Method: 提出Multi-Turn Language Models with Conformal Prediction (MiCP)框架，这是首个用于多轮推理的Conformal Prediction框架。MiCP在不同轮次间分配误差预算，使模型能够提前停止同时保持总体覆盖保证。框架在自适应RAG和ReAct上进行演示。

Result: 在单跳和多跳问答基准测试中，MiCP实现了目标覆盖率，同时减少了轮数、推理成本和预测集大小。进一步引入了联合评估覆盖有效性和回答效率的新度量指标。

Conclusion: MiCP成功解决了多轮推理LLM中的自适应停止问题，为高风险应用提供了形式化的覆盖保证和效率提升，是Conformal Prediction在多轮推理场景中的首个应用框架。
...
TL;DR: 通过提出WILD数据集和基于改进的多维项目反应理论与自适应项目选择的预测框架，实现了在大规模语言模型基准测试中高效预测未见任务性能，显著降低了评估成本。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型基准测试虽然数量庞大，但研究表明模型性能往往可以由少量潜在因素解释，这为更高效和原理化的基准测试提供了可能。然而，当前缺乏评估不同基准测试方法质量的统一框架，特别是如何基于已有数据预测模型在未见任务上的性能。

Method: 1. 收集构建WIDE-scale Item Level Dataset (WILD)，包含65个模型在163个任务（来自27个数据集）的109,564个独特项目上的响应对
2. 提出基于预测效度的基准测试质量评估框架，重点关注在预算约束下预测模型在未见任务上性能的效率
3. 结合改进的多维项目反应理论（IRT）模型和基于最优实验设计的自适应项目选择方法
4. 引入成本感知的折扣因子到选择标准中，优化令牌使用效率

Result: 1. 提出的方法能在观察仅16个项目后，以小于7%的平均绝对误差预测112个保留基准任务的性能
2. 通过整合成本感知折扣因子，将达到7% MAE所需的令牌数从141,000减少到22,000，降低了85%的评估成本
3. WILD数据集首次支持了在大规模多样化未见任务集合上分析不同预测技术的能力

Conclusion: 基于预测效度的基准测试框架结合改进的多维IRT和自适应项目选择方法，能够显著提高大语言模型评估的效率，大大降低评估成本，为更高效、原理化的基准测试提供了可行方案。WILD数据集为未来研究不同预测方法在多样化任务上的表现提供了重要资源。
...
TL;DR: 研究通过分布数据（词嵌入）成功区分印地语中梵语和波斯-阿拉伯语来源的同义词对，证明即使语义相同，使用模式仍保留词源信号，支持同义词反映不同视角的观点。


<details>
  <summary>Details</summary>
Motivation: 同义性是广泛但复杂的语言现象。理论上绝对同义词不应存在，因为它们不扩大语言表达能力。然而，有观点认为即使同义词表示相同概念，它们可能反映不同视角或带有不同的文化联想，但这些主张很少得到定量验证。印地语中与波斯语的长期接触产生了许多波斯-阿拉伯语借词，与其梵语对应词共存形成同义词对。

Method: 使用随机森林分类器，基于印地语同义词对的词嵌入数据进行训练，仅使用分布数据（不考虑语义内容）来分类单词的梵语或波斯-阿拉伯语来源。该方法旨在测试仅通过使用模式是否能区分词源。

Result: 随机森林成功将单词按其梵语或波斯-阿拉伯语来源进行分类，即使这些单词在语义上不相关。这表明使用模式保留了词源痕迹，上下文编码了词源信号。

Conclusion: 研究结果为上下文编码词源信号提供了定量证据，表明同义性可能反映与来源相关的微妙但系统性的区别。支持同义词可以提供不同视角的观点，并且词源相关的词可能形成不同的概念子空间，创造一种由历史来源塑造的新型语义框架。结果突出了上下文在捕捉超越传统语义相似性的微妙区别方面的能力。
...
TL;DR: 论文研究发现：在基于归因的生成任务中，强制细粒度（句子级）引用反而会降低归因质量，中等粒度（段落级）是最优选择，这一性能差距随模型规模非单调变化。


<details>
  <summary>Details</summary>
Motivation: 引用粒度（句子级、段落级或文档级）是归因生成中的关键设计选择。虽然细粒度引用更利于人工验证，但其对模型性能的影响尚未得到充分研究。

Method: 分析了四种模型规模（8B-120B），比较了不同引用粒度（句子、段落、多段落）对归因质量的影响，系统评估了粒度选择与模型性能之间的关系。

Result: 1. 强制细粒度引用使归因质量相比最佳粒度下降16-276%；2. 归因质量在中等级别粒度（段落级）达到峰值；3. 细粒度引用的负面影响随模型规模非单调变化，对大模型影响更大；4. 最优引用粒度在提升归因质量的同时保持甚至提高了答案正确性。

Conclusion: 仅为了便于人工验证而强制细粒度引用忽视了模型的内在限制，会同时损害归因忠实性和生成可靠性。有效的归因需要使引用粒度与模型的自然语义范围对齐，段落级是最优选择。
...
TL;DR: 通过对LLM过度自信的电路级机制分析，发现中间到后期层的特定MLP块和注意力头负责生成自信膨胀信号，可通过有针对性的推理时干预显著改善校准效果


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生事实错误但同时又过度自信地表达这些错误，这种口头表达的过度自信会误导用户并削弱置信度分数作为可靠不确定性信号的价值，但其内部机制尚不清楚

Method: 围绕三个轴心进行电路级机制分析：将口头自信捕捉为可微分的内部信号；识别导致自信膨胀的因果电路；利用这些见解进行有针对性的推理时重新校准。在两个指令调优的LLM和三个数据集上进行实验

Result: 发现一组紧凑的MLP块和注意力头（集中在中间到后期层）在最终标记位置一致地写入自信膨胀信号。有针对性的推理时干预对这些电路的干预能显著改善校准效果

Conclusion: LLM中的口头过度自信是由可识别的内部电路驱动的，可以通过有针对性的干预来缓解，为改善LLM校准提供了机制性见解
...
TL;DR: 论文提出了"魔法、疯狂、天堂、罪恶"框架，将语言模型输出变化置于同质-异质轴上进行分析，根据任务规范目标组织为四个规范性语境，并揭示了跨语境优化时的意外影响。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型研究中对"多样性"的术语使用碎片化，主要是因为任务背后的规范性目标很少明确表述。研究者认为需要澄清这些目标，并建立统一的分析框架来理解输出变化如何受不同规范性目标影响。

Method: 引入Magic, Madness, Heaven, Sin框架，将任务组织到四个规范性语境中：认知性（事实性）、交互性（用户效用）、社会性（代表性）、安全性（鲁棒性）。分析每个语境中的失败模式和相关术语，并应用框架分析所有成对的跨语境交互。

Result: 框架揭示了优化一个目标（如提高安全性）可能无意中损害人口统计学代表性或创造性多样性。跨语境分析显示了不同规范性目标之间的权衡和冲突。

Conclusion: 应该基于上下文进行输出变化的评估，将其重新定义为由任务目标塑造的属性，而非模型的内在特性。这种规范性视角有助于更全面地理解语言模型的多样性问题。
...
TL;DR: 扩散模型无法仅通过自然语言指令有效遗忘目标概念，需要在推理时间之外进行干预


<details>
  <summary>Details</summary>
Motivation: 研究基于指令的遗忘范式是否适用于扩散模型这类生成模型，因为这种范式在大型语言模型中已被证明有效，但在扩散模型中的适用性尚不明确

Method: 通过控制实验研究多个概念和提示变体；分析CLIP文本编码器和去噪过程中的交叉注意力动态

Result: 扩散模型在仅使用自然语言遗忘指令引导时，系统性地无法抑制目标概念；遗忘指令不会导致对目标概念token的持续注意力减少，目标概念表示在整个生成过程中持续存在

Conclusion: 扩散模型存在提示级指令的根本局限性，有效的遗忘需要在推理时间语言控制之外进行干预
...
TL;DR: 研究重新审视了网页HTML观察表示对LLM智能体性能的影响，发现最优观察表示取决于模型能力和思考token预算：低能力模型适合紧凑表示（无障碍树），高能力模型从详细HTML中获益，增加思考token会放大HTML的优势。


<details>
  <summary>Details</summary>
Motivation: 先前研究普遍认为HTML的冗长性是性能障碍，将观察缩减作为标准做法。本文重新审视这一趋势，研究不同观察表示对LLM智能体性能的实际影响。

Method: 通过对比分析不同模型能力和思考token预算下各种观察表示（HTML vs 无障碍树）的性能差异，进行错误分析以了解模型如何利用布局信息，并评估观察历史和基于差异表示的效果。

Result: 1) 紧凑观察适合低能力模型，详细HTML对高能力模型更优；增加思考token会增强HTML优势。2) 高能力模型利用HTML布局信息提升动作定位准确性，低能力模型在长输入下产生更多幻觉。3) 加入观察历史能提升大多数模型性能，基于差异的表示是token高效替代方案。

Conclusion: 提出了实用指南：基于模型能力和思考token预算自适应选择观察表示，并使用基于差异的表示结合观察历史，以实现最优性能。
...
TL;DR: 本文提出了一种模型合并框架，通过合并通用指令模型和临床基础模型来缓解LLMs在医疗领域微调时的灾难性遗忘问题，实现了在保留指令跟随能力的同时获得临床专业知识，且具有训练效率优势。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在临床文档应用中存在严重问题：当使用特定医疗数据集微调时，模型会大量"遗忘"指令跟随能力，这是将通用LLMs应用于临床领域的关键挑战。

Method: 提出模型合并框架，通过插值基合并方法将临床基础模型(GatorTronLlama)与通用指令模型(Llama-3.1-8B-Instruct)合并，以获得既具备临床任务性能又保留指令跟随能力的领域适应模型。

Result: 综合评估显示，合并模型能有效缓解灾难性遗忘，保留临床领域专业知识并维持指令跟随能力。模型合并策略在严重受限的监督下（如64-shot vs 256-shot）达到与完全微调基线相当的性能。
...
TL;DR: DeltaMem是一个基于单智能体设置的人格中心记忆管理系统，通过记忆演化启发的方法和强化学习框架，在各种长期记忆基准测试中超越了现有产品级基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有的人格中心记忆管理系统虽然展现了在多智能体对话场景中的强大能力，但这些复杂框架经常遭受信息损失问题，在不同场景下表现脆弱，导致性能不佳。

Method: 1) 将人格中心记忆管理建模为单智能体设置下的端到端任务；2) 从人类记忆演化中获得启发，合成用户-助手对话数据集及操作级记忆更新标签；3) 引入基于记忆的Levenshtein距离来形式化记忆更新奖励；4) 提出定制的强化学习框架来增强DeltaMem的管理能力。

Result: 大量实验表明，无论是免训练的DeltaMem还是经过强化学习训练的DeltaMem，在多个长期记忆基准测试（LoCoMo、HaluMem、PersonaMem）中都优于所有产品级基线模型。

Conclusion: DeltaMem通过创新的单智能体端到端记忆管理方法和基于记忆演化的强化学习框架，有效解决了现有复杂记忆系统中的信息损失和场景脆弱性问题，在长期记忆管理任务中实现了卓越性能。
...
TL;DR: Swift-SVD是一个针对大语言模型权重和KV缓存压缩的激活感知闭式压缩框架，实现了理论最优、实践高效且数值稳定的低秩近似，在压缩准确性和速度上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的部署受到静态权重和动态KV缓存的内存和带宽需求的限制。现有的SVD压缩方法存在两个关键局限性：一些方法在重构误差方面表现次优，而另一些方法理论最优但实际效率低下。

Method: 提出了Swift-SVD框架，通过增量聚合给定输入批次输出激活的协方差，并在聚合后执行单次特征值分解，实现免训练、快速且最优的层间低秩近似。采用有效秩分析局部层间可压缩性，并设计了动态秩分配策略，联合考虑局部重构损失和端到端层重要性。

Result: 在6个LLM和8个数据集上的广泛实验表明，Swift-SVD优于最先进的基线方法，在实现最优压缩精度的同时，端到端压缩时间提升了3-70倍。

Conclusion: Swift-SVD提供了一个硬件友好、同时保证理论最优、实践效率和数值稳定的压缩解决方案，有效解决了大语言模型部署中的内存和带宽限制问题。
...
TL;DR: 对印度尼西亚349名K-12教师的全国性调查显示，AI在教学实践中使用逐渐增加，主要用于减轻教学准备工作量，但采用程度不均且面临多种障碍。


<details>
  <summary>Details</summary>
Motivation: 印度尼西亚课堂中AI使用日益增多，但缺乏大规模、以教师为中心的实证研究，无法了解AI实际使用情况和教师需求，这阻碍了开发适合当地背景的AI系统和政策。

Method: 对全国349名K-12教师（包括小学、初中和高中）进行大规模问卷调查，收集教师使用AI的实践经验和需求数据。

Result: 发现AI在教学法、内容开发和教学媒体方面的使用增加，但采用程度不均：小学教师使用更一致，高中教师参与较少；中年教师更重视AI，东印度尼西亚教师感知价值更高；教师主要使用AI减轻教学准备工作量（如评估、课程规划和材料开发）。

Conclusion: 尽管AI使用在增长，但通用输出质量、基础设施限制和情境适应有限等障碍阻碍了有效的课堂整合；需要开发更具上下文敏感性的AI工具，并提供针对性的教师支持以促进可持续采用。
...
TL;DR: 论文分析了9个声明验证数据集中24K样本的推理模式，发现当前基准主要测试证据提取能力，而多句合成和数值推理严重不足，错误模式因领域而异，建议构建更具挑战性的评估套件来更好地测试验证系统的推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前声明验证领域虽然有快速发展，但缺乏对现有基准测试实际考察何种推理能力的系统性理解。研究人员希望了解这些基准测试实际上测试的是什么推理类型，以及是否存在系统性的偏差。

Method: 使用GPT-4o-mini为9个数据集中的24K声明验证示例生成结构化推理追踪；通过紧凑的1B参数推理验证器分析五种错误类型；进行数据集层面的分析，识别不同数据集的偏差模式。

Result: 发现直接证据提取主导了当前基准测试，而多句合成和数值推理严重不足；不同数据集存在明显偏差：有些数据集几乎只测试词汇匹配，而其他数据集约一半案例需要信息合成；错误模式因领域而异：通用领域验证受词汇重叠偏差主导，科学验证偏向过度谨慎，数学验证主要是算术推理失败。

Conclusion: 当前基准测试的高分主要反映检索加蕴涵能力，而非复杂的推理能力。需要构建更具挑战性的评估套件，以更好地测试验证系统所需的推理能力，特别是多句合成和数值推理等方面。
...
TL;DR: PRCCF框架通过人物角色引导的检索和因果感知的认知过滤，提升情感支持对话中深层次上下文理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有的情感支持对话方法在有效支持深层次上下文理解方面面临挑战，需要更好的机制来处理复杂情感推理。

Method: 提出了PRCCF框架，包含：1）人物角色引导的检索机制，联合建模语义兼容性和人物角色对齐；2）因果感知的认知过滤模块，优先考虑因果相关的外部知识。

Result: 在ESConv数据集上的广泛实验表明，PRCCF在自动评估指标和人工评估上都优于最先进的基线方法。

Conclusion: PRCCF框架通过结合人物角色引导和因果感知机制，有效提升了情感支持对话中的上下文理解和情感推理能力。
...
TL;DR: 研究发现监督微调中使用不同来源的思维链轨迹会产生"训练损失悖论"：DeepSeek-R1-0528数据训练损失更低但泛化性能更差，原因是其轨迹发散且分支多，而gpt-oss-120b轨迹收敛且具演绎性。通过过滤频繁分支轨迹可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 尽管监督微调在长思维链轨迹上是构建大型推理模型的关键阶段，但不同来源的CoT轨迹如何影响模型泛化性能仍然是一个未解问题。本研究旨在探究来自不同模型的验证CoT轨迹对SFT效果的差异。

Method: 使用DeepSeek-R1-0528和gpt-oss-120b两个竞争模型生成的已验证CoT轨迹进行对比研究，控制问题集相同。从token级SFT损失和step级推理行为两个维度进行多层面分析。最后提出过滤频繁分支轨迹的方法来改善泛化性能。

Result: 揭示了"训练损失悖论"：DeepSeek-R1-0528数据SFT训练损失显著更低，但在推理基准上表现出更差的泛化性能。分析发现gpt-oss-120b轨迹高度收敛且具演绎性，而DeepSeek-R1-0528偏好发散且分支多的探索模式。通过过滤频繁分支轨迹，在选定DeepSeek-R1-0528子集上训练可使AIME25性能提升5.1%，BeyondAIME提升5.5%，五个基准平均提升3.6%。

Conclusion: 训练损失不能可靠预测推理模型的泛化性能。CoT轨迹的质量而非数量更为关键，高度收敛、演绎性的推理模式比发散、分支多的探索模式更有利于模型学习。简单过滤频繁分支轨迹是提升SFT泛化能力的有效策略。
...
TL;DR: EndoASR是一个针对胃肠内窥镜领域优化的语音识别系统，通过两阶段适应策略显著提高转录准确性和临床实用性，在真实世界多中心验证中表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 自动语音识别(ASR)是胃肠内窥镜中人机交互的关键接口，但在真实临床环境中的可靠性受到领域特定术语和复杂声学条件的限制。需要开发专门针对内窥镜工作流程优化的ASR系统。

Method: 开发了EndoASR系统，采用两阶段适应策略：1）基于合成内窥镜报告进行领域特定语言建模；2）针对噪声鲁棒性优化。系统采用Paraformer作为基础模型，参数规模为220M，支持边缘部署。

Result: 在回顾性评估中，字符错误率从20.52%降至14.14%，医学术语准确率从54.30%提升至87.59%。在多中心前瞻性研究中，CER从16.20%降至14.97%，Med ACC从61.63%提升至84.16%。实时因子达到0.005，优于Whisper-large-v3(0.055)。与大型语言模型集成显示改进的ASR质量能增强下游信息提取和人机交互。

Conclusion: 领域适应ASR可以作为胃肠内窥镜中人机协同的可靠接口，在多中心真实临床环境中表现出稳定性能，具有高效边缘部署能力，显著提升了临床可用性和下游任务性能。
...
TL;DR: 该论文对基于大语言模型的智能体中的记忆模块进行了系统研究，提出了统一的记忆框架，在标准基准上全面比较了现有记忆方法，并设计了一种新的记忆方法


<details>
  <summary>Details</summary>
Motivation: 记忆是大语言模型智能体处理长时程复杂任务（如多轮对话、游戏、科学发现）的核心模块，但目前文献中提出的各种记忆方法缺乏在相同实验设置下的系统和全面比较

Method: 1. 总结了一个统一框架，从高层视角整合所有现有智能体记忆方法；2. 在两个知名基准上广泛比较代表性记忆方法；3. 通过分析现有方法的模块设计了一种新的记忆方法

Result: 提供了对各种记忆方法的深入分析，新设计的记忆方法在性能上超过了现有最先进的方法

Conclusion: 对现有记忆方法的深入理解可以为未来研究提供有价值的新见解，并提供了有前景的未来研究方向
...
TL;DR: 提出一种人机协作框架，通过LLM推理和置信度路由机制，结合声学特征模型和人类知识，解决越南语语音情感识别中模糊样本分类的问题。


<details>
  <summary>Details</summary>
Motivation: 越南语语音情感识别面临两大挑战：1）声学模式模糊，特别是在真实场景中情感边界不清晰；2）缺乏可靠的标注数据。传统数据驱动方法在处理模糊样本时效果有限。

Method: 提出基于LLM推理的人机协作框架，包括：1）声学特征模型提供置信度和特征级证据；2）置信度路由机制区分简单和模糊样本；3）模糊样本交由LLM进行结构化规则指导的深层推理；4）迭代精化策略通过错误分析和规则更新持续改进系统。

Result: 在越南语语音数据集（2,764个样本，3种情感：平静、愤怒、恐慌）上实验，标注者间一致性高（Fleiss Kappa=0.8574）。该方法达到86.59%准确率和0.85-0.86的Macro F1分数，在处理模糊和难分类样本方面表现优异。

Conclusion: 该研究证明了将数据驱动模型与人类推理相结合的重要性，为低资源环境下的语音情感识别提供了一种鲁棒且模型无关的方法，有效处理了模糊情感边界问题。
...
TL;DR: 本文提出了一个针对保加利亚语的毒性内容检测方法，通过构建本体论和数据集，训练BERT模型实现0.89 F1分数的毒性分类，同时避免误伤医疗术语和少数群体相关文本。


<details>
  <summary>Details</summary>
Motivation: 当前在线毒性内容检测方案经常错误地屏蔽有价值信息，包括医疗术语和少数群体相关文本。需要一种更细致的方法来识别保加利亚语中的毒性内容，同时保留重要信息。

Method: 1. 提出一个建模保加利亚语中潜在毒性词汇的本体论；2. 构建包含4,384个手动标注句子的数据集，涵盖毒性语言、医学术语、非毒性语言和少数群体相关术语四个类别；3. 训练基于BERT的毒性语言分类模型。

Result: 训练的BERT模型在毒性语言分类任务上达到了0.89的F1宏平均分数。该模型可直接应用于真实环境，并可作为毒性内容检测系统的组件集成。

Conclusion: 研究开发了两种不同的毒性内容检测方法，这些方法在各种在线平台和内容审核系统中具有潜在应用价值。模型表现出良好的性能，能够在有效检测毒性内容的同时避免误伤重要信息。
...
TL;DR: 提出了LiveMathematicianBench动态多选基准，基于arXiv最新论文构建研究级数学推理评估，采用证明草图引导的干扰项管道和替换抵抗机制，显著降低模型性能，凸显当前LLMs在真实数学推理上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有数学推理基准存在合成设置和数据污染问题，无法有效评估LLMs的真实数学能力。随着LLMs越来越多地融入科学工作流程，对其数学能力的严格评估成为实际需求。

Method: 1) 基于arXiv在模型训练截止后发表的最新论文构建动态多选基准；2) 引入13类逻辑分类法（蕴含、等价、存在、唯一性等）；3) 采用证明草图引导的干扰项管道，利用高级证明策略构建似是而非的错误选项；4) 引入替换抵抗机制区分答案识别与实质推理；5) 实施双模式协议评估证明草图访问的影响。

Result: 基准远未饱和：最佳模型Gemini-3.1-pro-preview仅达43.5%准确率。在替换抵抗评估下，GPT-5.4最高为30.6%，Gemini-3.1-pro-preview降至17.6%（低于20%随机基线）。证明草图访问带来一致的准确性提升，表明模型可利用高级证明策略进行推理。

Conclusion: LiveMathematicianBench提供了一个可扩展、抗污染的研究级数学推理测试平台，能够更真实地评估LLMs的数学能力，揭示了当前模型在实质性数学推理而非模式记忆方面的局限性。
...
TL;DR: 该论文提出了一种基于指令微调和离散控制标记的领域无关CATS框架，研究发现小模型可以达到竞争力，但可靠可控性依赖于训练数据中目标属性的足够变化，标准评估指标对控制测量不足，需要基于误差的度量。


<details>
  <summary>Details</summary>
Motivation: 当前可控自动文本简化(CATS)中的可控性通常被当作解码问题处理，且使用不能反映控制程度的指标进行评估。作者观察到CATS中的可控性受到数据和评估方法的显著限制，需要更好的框架和评估方法。

Method: 提出基于指令微调和离散控制标记的领域无关CATS框架，使用三个模型家族(Llama, Mistral, Qwen; 1-14B)在四个领域(医学、公共管理、新闻、百科全书文本)进行实验，研究了训练数据中目标属性变化对可控性的影响。

Result: 研究发现：1)小模型(1-3B)可以达到竞争力；2)可靠的可控性强烈依赖于训练数据是否编码足够的目标属性变化；3)可读性控制(FKGL, ARI, Dale-Chall)一致地学习到，而压缩控制由于现有语料库中信号变化有限而表现不佳；4)标准简化和相似性指标不足以衡量控制效果；5)简单分割可能引入影响训练和评估的分布不匹配。

Conclusion: CATS中的可靠可控性需要训练数据具有足够的目标属性变化，需要开发更好的基于误差的评估指标来衡量目标-输出对齐，并需注意数据分割策略以避免分布不匹配问题。
...
TL;DR: 提出DEFT（Distribution-guided Efficient Fine-Tuning）框架，通过数据过滤和分布引导来提升大语言模型与人类偏好的对齐效率和性能，同时缓解泛化能力损失。


<details>
  <summary>Details</summary>
Motivation: 当前基于人类反馈的强化学习（RLHF）方法如PPO存在成本高、不稳定的问题。现有替代方法（结合监督微调和对比学习）虽能直接微调和对齐，但仍需要大量数据学习偏好，且可能削弱大语言模型的泛化能力。需要更高效的对齐方法。

Method: 提出DEFT框架，包含数据过滤和分布引导两个核心组件。通过计算基于语言模型输出分布和偏好数据差异分布的差分分布奖励，从原始数据中过滤出小而高质量的子集。将该子集融入现有对齐方法中，引导模型的输出分布。

Result: 实验结果表明，使用DEFT增强的方法在不对齐能力和泛化能力方面均优于原始方法，且训练时间显著减少。

Conclusion: DEFT框架能够有效提升大语言模型对齐的效率和性能，同时缓解泛化能力的损失，为高效的对齐方法提供了新的解决方案。
...
TL;DR: PLOT：基于最优传输的token级偏好学习方法，显著提升大语言模型对齐性能，同时保持稳定性和鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型偏好学习方法存在以下问题：性能提升有限、计算成本高、超参数敏感、对全局token级关系建模不足。需要一种更有效的偏好学习方法来克服这些限制。

Method: 提出PLOT方法，将偏好学习形式化为最优传输问题，从最优传输推导出token级损失函数。该方法通过token嵌入捕获语义关系，实现全局信息化的优化，同时保持原始LLM分布。

Result: 在两个偏好类别（人类价值观、逻辑与问题解决）共七个子偏好上的实验表明，PLOT持续提升对齐性能，同时保持流畅性和连贯性，验证了最优传输作为偏好学习原则性方法的有效性。

Conclusion: 最优传输为偏好学习提供了理论依据的框架，PLOT方法证实了最优传输作为大语言模型偏好学习的理论基础方法论的有效性，为LLM偏好学习提供了新的见解。
...
TL;DR: 提出自适应占位符完成（APC）框架，通过在高不确定性位置输出显式占位符替代硬完成范式，在保持代码完成质量的同时降低编辑成本


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在代码完成中通常采用硬完成范式，即使在上下文不足时也强制生成具体代码。通过对300万次真实交互的分析发现，61%的生成建议被编辑或拒绝，尽管与用户后续代码相似度超过80%，这表明模型经常在特定令牌位置做出错误预测

Method: 1. 将代码完成形式化为不确定性下的成本最小化问题；2. 基于"填充占位符比纠正错误成本更低"的观察，证明存在临界熵阈值，超过该阈值时APC的期望成本严格低于硬完成；3. 从过滤后的真实世界编辑日志构建训练数据；4. 设计基于成本的奖励函数进行强化学习

Result: 在1.5B到14B参数模型上的广泛评估显示，APC将期望编辑成本从19%降低到50%，同时保持了标准硬完成性能

Conclusion: APC为不确定性感知的代码完成提供了理论基础和实用训练框架，证明自适应弃权可以在不牺牲传统完成质量的情况下进行端到端学习
...
TL;DR: SURE框架通过不确定性感知和迭代推理提升对话中多模态情感识别的鲁棒性和上下文建模能力


<details>
  <summary>Details</summary>
Motivation: 现有的多模态对话情感识别方法往往强调融合但忽视了噪声特征的不确定性以及细粒度推理，需要更好的鲁棒性和上下文建模能力

Method: 提出了SURE框架，包含三个组件：不确定性感知混合专家模块处理模态特定噪声，迭代推理模块进行多轮上下文推理，Transformer门模块捕捉模态内和模态间交互

Result: 在基准MERC数据集上的实验显示，SURE始终优于最先进的方法，证明了其在鲁棒多模态推理方面的有效性

Conclusion: 不确定性建模和迭代推理对于推进对话场景中的情感识别具有重要意义，SURE框架为多模态情感识别提供了新的解决方案
...
TL;DR: 基于Transformer的模型在法国心力衰竭队列上评估了文本、结构化数据、多模态及LLM方法，发现实体感知的多模态Transformer在短期死亡率预测上表现最佳，而LLM提示方法表现不一致。


<details>
  <summary>Details</summary>
Motivation: 心力衰竭（HF）的短期死亡率预测仍然具有挑战性，特别是在仅依赖结构化电子健康记录（EHR）数据时。需要探索如何有效整合临床文本和结构化数据以提高预测准确性。

Method: 在法国HF队列中评估了多种方法：仅文本模型、仅结构化数据模型、多模态融合模型（文本+结构化变量），以及基于大语言模型（LLM）的方法。特别关注了如何丰富临床文本表示（实体级表示vs CLS嵌入）和不同的模态融合策略。

Result: 1. 用实体级表示丰富临床文本比仅使用CLS嵌入能提高预测性能；2. 文本和结构化变量的有监督多模态融合实现了最佳整体性能；3. LLM在不同模态和解码策略下表现不一致，仅文本提示优于结构化或多模态输入。

Conclusion: 实体感知的多模态Transformer为短期HF结局预测提供了最可靠的解决方案，而当前的LLM提示方法在临床决策支持方面仍有局限性，需要进一步改进。
...
TL;DR: 本文提出了一种结合非上下文文本嵌入与符号概念特征的神经符号方法，用于增强宣传新闻检测的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 宣传新闻特别阴险，因为它们倾向于将有导向性的信息与看似可靠的事实报道混合。现有的基于语言模型（如BERT）的方法虽然有前景，但由于数据收集中的偏差，往往在训练数据集上过拟合，影响了模型对新闻源的泛化能力。

Method: 采用神经符号方法，将非上下文文本嵌入（fastText）与符号概念特征相结合，包括体裁、主题和说服技术等特征，以提高分类的鲁棒性。

Result: 该方法相比仅使用文本的等效方法有所改进。消融研究和可解释性分析证实了添加特征的益处，表明该方法能够增强分类的鲁棒性并提高对新新闻源的泛化能力。

Conclusion: 结合非上下文文本嵌入与符号概念特征的神经符号方法是提高宣传新闻检测鲁棒性和泛化性的有效途径，有助于克服现有基于语言模型方法的过拟合问题。
...
TL;DR: 该研究提出了一种测量词序变异相对于交换距离最小化优化程度的新数学框架，证明了跨语言手势至少77%是优化的，并将二次分配问题引入语言研究作为优化问题的统一框架。


<details>
  <summary>Details</summary>
Motivation: 研究动机基于一个假设：语言中的词序会最小化排列超立方体中的交换距离。研究者希望开发一种方法来测量词序变异相对于交换距离最小化的优化程度，并检验这一假设在语言和手势中的有效性。

Method: 该方法引入了一个新颖的数学框架来测量交换距离最小化的优化程度。研究使用排列超立方体表示所有序列排列的结构，其中顶点是排列，如果通过交换相邻元素可以从一个顶点得到另一个顶点，则两顶点相连。研究还将二次分配问题引入语言研究作为统一优化问题的框架。

Result: 研究发现跨语言手势在交换距离最小化方面至少达到77%的优化程度。统计分析表明，跨语言手势多次达到最优化的概率不是偶然的，支持了交换距离最小化假设。

Conclusion: 研究为词序或手势顺序在交换距离最小化方面的优化研究奠定了理论基础，并提出一个统一的优化分配原则，将交换距离最小化与其他语言优化原则联系起来，为语言优化研究提供了新的数学工具和理论框架。
...
TL;DR: SAFE是一个动态基准测试框架，通过用可验证的基于知识图谱的实体序列替换无基础的思维链，解决多跳问答中虚假正确性问题。它包括训练时验证（消除噪声监督）和推理时验证（实时检测无基础步骤），在保证可验证轨迹的同时实现8.4个百分点的准确率提升。


<details>
  <summary>Details</summary>
Motivation: 当前多跳问答基准测试中，大型语言模型经常因虚假正确性而获得奖励，这掩盖了无基础或有缺陷的推理步骤。需要转向更严格的推理评估，确保推理过程的可靠性和可验证性。

Method: 提出SAFE框架，包含两个阶段：1) 训练时验证：建立原子错误分类和基于知识图谱的验证流水线，消除标准基准中的噪声监督，识别不可回答的实例（最多14%）；2) 推理时验证：在已验证数据集上训练反馈模型，实时动态检测推理过程中的无基础步骤。

Result: SAFE不仅暴露了现有基准测试的关键缺陷（在训练阶段识别不可回答实例），还在推理时显著优于标准基线，平均准确率提升8.4个百分点，同时保证推理轨迹的可验证性。

Conclusion: SAFE框架通过引入严格的可验证性要求，能够有效识别多跳问答中的虚假正确性问题，提供更可靠的基准测试和推理评估方法，为改进大型语言模型的推理能力提供了重要工具。
...
TL;DR: 论文研究高斯扩散模型在离散数据生成中的挑战，发现DDPM采样器在处理离散分布时会遇到多模态密度区域的问题，提出了结合自条件和q-采样器的改进方法。


<details>
  <summary>Details</summary>
Motivation: 扩散模型已成为连续域生成建模的标准方法，但在离散数据上的应用仍面临挑战。研究者旨在探究为什么高斯扩散模型配合DDPM求解器难以从离散分布中采样，这些离散分布在连续空间中表示为delta分布的混合。

Method: 使用玩具随机层次模型识别关键采样区间，在该区间内加噪数据的密度变为多模态。分析DDPM采样器在多模态区域中的表现，评估现有启发式方法（包括自条件和q-采样器）的效果。提出在关键区间内从DDPM切换到q-采样器并结合自条件的混合策略。

Result: 发现DDPM采样器在关键区间内偶尔会进入模态间的低密度区域，产生分布外输入，从而降低采样质量。自条件和q-采样器能缓解此问题。结合两种方法的混合策略在真实数据上提高了生成质量。

Conclusion: 在文本、编程代码和蛋白质等多个领域的条件和无条件任务中验证了这些发现。研究揭示了高斯扩散模型在离散数据生成中的固有挑战，并提出有效的解决方案来改进采样质量。
...
TL;DR: 提出BidirLM：一种将因果生成语言模型转化为双向编码器的通用方法，通过权重合并和数据混合策略解决灾难性遗忘问题，在文本、视觉和音频表示任务上优于现有方案。


<details>
  <summary>Details</summary>
Motivation: 当前将因果生成语言模型转化为双向编码器的方法存在三个主要问题：1）缺乏对最优训练目标的共识；2）在大规模应用时出现灾难性遗忘；3）无法灵活集成专门的生成模型生态系统。需要一种通用且高效的转换方法。

Method: 基于Gemma3和Qwen3家族进行系统消融实验，识别成功适应的关键因素，特别强调常被忽略的先验掩码阶段的重要性。提出双策略方法：1）线性权重合并；2）轻量级多领域数据混合，避免使用原始预训练数据即可缓解灾难性遗忘。通过合并专门因果模型来增强编码器能力。

Result: 开发出适用于任何因果解码器LLM的开源配方，产生BidirLM家族的五个编码器，在文本、视觉和音频表示基准测试中均优于现有替代方案。

Conclusion: 提出了一种通用、高效的将因果生成语言模型转化为双向编码器的方法，通过权重合并和数据混合解决了灾难性遗忘问题，成功创建了多功能编码器家族BidirLM，在跨模态表示任务上表现出色。
...
TL;DR: GOOSE框架通过构建自适应脊柱树，利用高质量上下文匹配令牌形成深链，低质量备选令牌形成宽分支，在无训练的情况下显著加速大型语言模型推理。


<details>
  <summary>Details</summary>
Motivation: 现有的无训练投机解码方法在组织候选令牌树时，没有区分来自不同来源的候选令牌质量差异。观察发现，两种常见的无训练令牌源（输入上下文的n-gram匹配和先前前向传播的统计预测）在接受率上存在显著差异（中位数差距约6倍）。这种质量差异使得对称平衡树无法充分利用高质量令牌源。

Method: 提出GOOSE框架，构建自适应脊柱树：高接受率的上下文匹配令牌形成深链（脊柱），低接受率的备选令牌在每个节点处形成宽分支。这种方法实现了各向异性（不对称）树结构，在固定验证预算下突破了平衡树的深度限制。

Result: 在五个LLM（7B-33B）和五个基准测试上，GOOSE实现了1.9-4.3倍的无损加速，在相同预算下优于平衡树基线12-33%。理论证明显示，每个步骤接受的令牌数量至少与单独使用任一令牌源时相同。

Conclusion: 通过区分不同令牌源的质量并构建自适应脊柱树，GOOSE框架有效地利用高质量上下文匹配令牌形成深链，显著提高了投机解码的效率，在无训练情况下实现了显著的推理加速。
...
TL;DR: ReRanking Preference Optimization (RRPO)：一种基于强化学习的重排序偏好优化框架，通过LLM反馈直接对齐重排序与生成质量，无需昂贵的人工标注，显著提升检索增强生成效果。


<details>
  <summary>Details</summary>
Motivation: 当前重排序模型通常基于静态人工标注的相关性标签进行优化，与下游生成过程脱节，导致检索指标认定的相关文档在实际生成答案时缺乏真正效用，存在基础性不对齐问题。

Method: 提出RRPO强化学习框架，将重排序建模为顺序决策过程，使用LLM反馈优化上下文效用。引入参考锚定的确定性基线以确保训练稳定性，无需人工标注。

Result: 在知识密集型基准测试中，RRPO显著优于包括强大的列表式重排序器RankZephyr在内的强基线。框架具有良好的泛化性、模块兼容性和噪声鲁棒性。

Conclusion: RRPO通过强化学习将重排序直接与LLM生成质量对齐，解决了传统重排序模型与下游生成脱节的问题，为检索增强生成提供了一种高效、可扩展且鲁棒的优化方案。
...
TL;DR: 提出Prosodic ABX框架评估自监督语音模型中韵律对比的敏感性，构建英语、日语、普通话的最小对数据集，发现模型和层级排名在不同实验条件下保持一致性


<details>
  <summary>Details</summary>
Motivation: 自监督语音模型(S3Ms)的表征已知对音素对比敏感，但其对韵律对比的敏感性尚未直接测量。ABX辨别任务已用于通过最小对测量S3M表征中的音素对比，但缺乏对韵律对比的系统评估方法。

Method: 提出Prosodic ABX框架，扩展ABX辨别任务以评估韵律对比，仅需少量示例且无需显式标签。构建并发布了英语和日语的最小对数据集，结合已有的普通话数据集，评估英语重音、日语音高重音和普通话声调的对比敏感性。

Result: 发现自监督语音模型表征对韵律对比具有敏感性。更重要的是，模型和层级排名在多种实验条件下通常保持一致性，表明该方法在低资源环境下具有实用性。

Conclusion: Prosodic ABX框架为评估自监督语音模型中的韵律对比敏感性提供了有效且实用的方法。该方法仅需少量示例且无需标注，模型排名的跨条件一致性使其特别适用于低资源场景下的模型评估和选择。
...
TL;DR: 提出稳定性过滤方法构建有效的思维边界引导向量，解决现有基于关键词匹配方法因行为不稳定边界导致的信号稀释问题，在数学推理任务上显著提升性能


<details>
  <summary>Details</summary>
Motivation: 现有构建引导向量的方法依赖于关键词匹配检测思维边界，但假设每个检测到的边界都编码真实行为信号是错误的。研究发现93.3%的边界行为不稳定，无法在相同前缀下重现目标行为，这导致引导信号被稀释

Method: 1) 提出概率模型将内在推理行为形式化为具有上下文相关触发概率的随机事件；2) 开发稳定性过滤方法，仅保留模型能一致重现目标行为的边界；3) 结合内容子空间投影去除残余问题特定噪声

Result: 在MATH-500数据集上达到0.784准确率，比最强基线提升5.0%。生成的引导向量可在同一架构系列模型中跨模型转移，无需重新提取：Nemotron-Research-Reasoning-1.5B提升5.0%，DeepScaleR-1.5B-Preview提升6.0%

Conclusion: 稳定性过滤是构建有效引导向量的关键，通过区分稳定和不稳定边界显著提升引导性能，且该方法具有跨模型可迁移性，为训练自由控制语言模型推理行为提供了更可靠的方法
...
TL;DR: 本文介绍了GaelEval，这是首个针对苏格兰盖尔语的多维度基准测试，包含形态句法MCQA、文化翻译和文化遗产知识问答任务，评估了19个大语言模型在盖尔语上的表现，发现专有模型优于开源模型，盖尔语提示能带来小幅但稳定的性能提升。


<details>
  <summary>Details</summary>
Motivation: 多语言大语言模型在未经官方支持的语言中常表现出"影子"能力，但这些语言（特别是形态句法丰富的少数民族语言如苏格兰盖尔语）的性能仍不均衡且测量不足。现有翻译基准无法充分捕捉语言结构能力，因此需要针对盖尔语开发更全面的评估方法。

Method: 研究团队开发了GaelEval基准，包含三个维度：1)专家编写的形态句法多项选择题任务；2)文化基础的翻译基准；3)大规模文化遗产知识问答任务。评估了19个大语言模型，并与30名流利盖尔语使用者的人类基线进行比较。

Result: Gemini 3 Pro Preview在语言任务上达到83.3%准确率，超过人类基线(78.1%)。专有模型始终优于开源模型，盖尔语提示带来2.4%的稳定性能提升。在文化任务上，领先模型超过90%准确率，但多数模型在盖尔语提示下表现较差。

Conclusion: GaelEval基准揭示了前沿模型在盖尔语语法的多个维度上达到超越人类的性能，证明了盖尔语提示的积极效果，并显示了专有模型相对开源模型的持续性能优势。这为少数民族语言的模型评估提供了新方法。
...
TL;DR: 该研究发现，在结构化工具使用场景中，思维链推理长度与准确率之间存在非单调关系：简短推理（32令牌）可大幅提升准确率45%，而过度推理（256令牌）反而使性能恶化。作者提出了函数路由思维链方法，以结构化方式引导模型在推理开始时选择正确函数。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究语言智能体在采取行动前应该思考多少。尽管思维链推理被广泛认为能提升智能体性能，但在结构化工具使用场景中，推理长度与准确率之间的关系尚未得到充分理解，特别是在函数调用任务中。

Method: 研究方法包括：1）系统研究思维链预算对函数调用智能体的影响，在Berkeley函数调用排行榜v3的200个任务上测试6个令牌预算（0-512）；2）对Qwen2.5-1.5B-Instruct模型进行详细分析；3）进行错误分解分析；4）进行神谕分析确定最优推理长度；5）提出函数路由思维链方法，以结构化模板强制模型在推理开始时选择有效函数名。

Result: 主要发现包括：1）简短推理（32令牌）将准确率从44.0%提升至64.0%（提升45%）；2）过度推理（256令牌）使准确率降至25.0%，低于无思维链基线；3）错误分解显示：无推理时30.5%任务因选错函数而失败，简短推理将此降至1.5%，而长推理导致28.0%选错函数和18.0%产生幻觉函数；4）神谕分析表明88.6%可解任务最多需要32推理令牌，平均27.6令牌；5）提出的函数路由思维链方法实现了与自由形式简短推理相当的准确率，同时将函数幻觉降至0.0%。

Conclusion: 结论是思维链推理在函数调用任务中并非"越长越好"，过度推理会导致性能下降，其主要机制是函数路由错误。函数路由思维链方法通过结构化方式引导模型在推理开始时选择正确函数，既保持了性能优势又提供了结构可靠性保证，无需预算调整。这项研究揭示了结构化工具使用场景中推理预算的关键影响。
...
### [146] [AstroConcepts: A Large-Scale Multi-Label Classification Corpus for Astrophysics](https://arxiv.org/abs/2604.02156)
...
TL;DR: 论文介绍了AstroConcepts语料库，这是一个包含21,702篇天体物理论文摘要的语料，标注了2,367个概念，用于系统研究科学领域中的极端类别不平衡问题，并建立了多种方法的基准。


<details>
  <summary>Details</summary>
Motivation: 科学多标签文本分类面临极端类别不平衡问题，特别是在专业术语方面存在严重的幂律分布。现有的科学语料库缺乏全面的受控词汇表，主要关注宽泛类别，限制了系统研究极端不平衡现象的能力。

Method: 引入AstroConcepts语料库，包含21,702篇天体物理英文论文摘要，使用统一天文学辞典(UAT)的2,367个概念进行标注。语料库呈现严重的标签不平衡(76%的概念少于50个训练样本)。评估了传统方法、神经方法和词汇约束LLM方法，并提出频率分层评估策略。

Result: 研究发现三个关键模式：1)词汇约束LLMs在性能上可与领域自适应模型竞争；2)领域自适应对稀有专业术语提升更大(但绝对性能仍有限)；3)频率分层评估揭示被聚合分数掩盖的性能模式。建立了极端不平衡研究的基准。

Conclusion: AstroConcepts语料库实现了对科学领域中极端类别不平衡的系统研究，为科学NLP提供了可行的见解，并为极端不平衡研究建立了基准。频率分层评估应成为科学多标签评估的核心组成部分。
...
TL;DR: 本文介绍了参与SOMD 2026跨文档软件提及共指消解共享任务的研究，系统在三个子任务中均排名第二。研究比较了两种免微调方法：基于词法字符串相似性的模糊匹配和结合提及级与文档级嵌入的上下文感知表示法。


<details>
  <summary>Details</summary>
Motivation: 软件名称通常具有高度的表面规律性，这减少了对复杂语义推理的需求，但仍需要有效的共指消解方法来处理跨文档的软件提及。研究旨在探索在免微调设置下，不同方法对软件提及共指消解任务的效果和适用性。

Method: 采用了两种免微调方法：1) 模糊匹配(FM)：基于词法字符串相似性的传统方法；2) 上下文感知表示(CAR)：结合了提及级嵌入和文档级嵌入的方法。进行了对比实验，包括噪声注入研究（边界噪声和提及替换）和推理时间分析。

Result: 两种方法在所有子任务中都取得了有竞争力的性能（CoNLL F1得分为0.94-0.96）。CAR始终比FM高出1个百分点的F1得分。噪声注入研究显示：在边界噪声增加时，CAR的F1下降仅0.07，而FM下降0.20；在提及替换时，FM下降0.52，CAR下降0.63。推理时间分析表明FM随语料库规模呈超线性扩展，而CAR近似线性扩展。

Conclusion: 系统选择应综合考虑上游提及检测器的噪声特征和目标语料库的规模。CAR在效率上更适合大规模应用，而FM在某些噪声条件下更具鲁棒性。本研究为这一研究不足的任务提供了实用的方法比较和代码支持。
...
TL;DR: 本文提出基于文本数据频率的LLM性能提升框架，包含文本频率法则、文本频率蒸馏和课程文本频率训练三个组件，在多个推理任务上验证了高频文本表达对LLM提示和微调的有效性。


<details>
  <summary>Details</summary>
Motivation: 文本频率已被验证与人类阅读速度等认知能力相关，但其与大型语言模型（LLMs）的相关性研究较少。目前许多LLM的训练数据是闭源的，难以直接获取频率信息，因此需要探索文本数据频率这一未被充分研究的话题对LLM性能的影响。

Method: 1. 提出文本频率法则（TFL）：主张对LLMs的提示和微调应优先使用高频文本数据，通过在线资源估算句子级频率，并利用输入重述器将输入改写成更频繁的文本表达。
2. 提出文本频率蒸馏（TFD）：通过查询LLMs进行故事补全，扩展数据集中的句子，用生成的语料调整初始频率估计。
3. 提出课程文本频率训练（CTFT）：按句子级频率递增的顺序对LLMs进行微调。
4. 构建文本频率配对数据集（TFPD）并在数学推理、机器翻译、常识推理和智能体工具调用等任务上进行实验。

Result: 实验结果表明，该框架在多个任务上均有效。具体来说，使用高频文本表达进行提示和按频率递增顺序进行微调都能提升LLMs在数学推理、机器翻译、常识推理和工具调用等任务上的性能。

Conclusion: 文本数据频率是影响LLM性能的一个重要但被忽视的因素。提出的TFL、TFD和CTFT框架通过优先使用高频文本数据、改进频率估计以及按频率递增微调，能有效提升LLMs在多种推理任务中的表现。这项工作为LLM优化开辟了新的研究方向。
...
TL;DR: 提出Neuro-RIT框架，通过神经元引导的指令微调增强检索增强语言模型对噪声的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有的检索增强语言模型在处理不相关或噪声检索上下文时性能会下降，现有增强鲁棒性的方法通常在层或模块级别进行粗粒度参数更新，忽视了大型语言模型的神经元级稀疏性

Method: 提出Neuro-RIT框架：1)使用基于归因的神经元挖掘显式分离处理相关和不相关上下文的神经元；2)引入两阶段指令微调策略，实现双重能力：通过功能失活只处理不相关上下文的神经元实现直接噪声抑制，同时优化目标层进行证据蒸馏

Result: 在多种问答基准测试上的广泛实验表明，Neuro-RIT持续优于强基准和鲁棒性增强方法

Conclusion: Neuro-RIT将范式从密集适应转向精准驱动的神经元对齐，有效增强了检索增强语言模型对噪声检索上下文的鲁棒性
...
TL;DR: L3TR是一个基于大型语言模型的新型列表式人才推荐框架，通过块注意力机制和局部位置编码解决传统点式方法的局限性，减少位置偏差和token消耗，提升推荐效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的人才推荐系统多采用点式范式，需要重复处理文本且无法捕捉候选人间关系，导致token消耗高且推荐效果不佳。同时LLM存在位置偏差和"迷失在中间"问题，影响处理长文档和多选任务的表现。

Method: 提出L3TR框架：1）采用块注意力机制增强文档间处理；2）局部位置编码方法减轻位置偏差和并发token偏差；3）ID采样方法解决训练和推理阶段候选集大小不一致问题；4）设计偏差检测方法和训练去偏方法。

Result: 在两个真实世界数据集上的大量实验验证了L3TR的有效性，相比现有基线方法取得了持续改进。

Conclusion: L3TR通过创新的列表式处理范式，有效解决了传统基于LLM的人才推荐系统中存在的位置偏差、token消耗高和候选关系忽略等问题，为人才推荐任务提供了更高效准确的解决方案。
...
TL;DR: 首个用于阿拉伯语语音命名实体识别的公开数据集CV-18 NER，端到端模型在该数据集上显著优于级联管道系统。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语端到端语音命名实体识别研究不足，主要由于阿拉伯语的形态复杂性、短元音缺失以及标注资源有限。现有研究已在英语、法语和中文上证明端到端方法优于级联管道，但阿拉伯语领域仍缺乏公开基准数据集。

Method: 1. 创建首个阿拉伯语语音NER公开数据集CV-18 NER，通过在阿拉伯语Common Voice 18语料库上增加手动NER标注，遵循细粒度Wojood模式（21种实体类型）。
2. 基准测试包括：级联系统（ASR + 文本NER）和基于Whisper与AraBEST-RQ的端到端模型。
3. 分析不同预训练策略对性能的影响。

Result: 端到端系统在测试集上显著优于最佳级联配置：AraBEST-RQ 300M达到37.0% CoER，Whisper-medium达到38.0% CVER。分析发现：阿拉伯语特定自监督预训练在ASR上表现强，而多语言弱监督在联合语音到实体学习中转移更有效；在低资源设置中，较大模型可能更难适应。

Conclusion: 本研究提出了首个阿拉伯语语音NER公开数据集CV-18 NER，并证明端到端方法在该任务上优于传统级联管道。研究还揭示了不同预训练策略对阿拉伯语语音NER的影响，为未来研究提供了首个开放基准。
...
TL;DR: 论文提出了一种评估和优化大语言模型生成全面、多样化回答的方法，通过引入多样性覆盖率指标来评估模型生成唯一答案的能力，并设计了一个路由模型来选择最适合生成多样化回答的模型。


<details>
  <summary>Details</summary>
Motivation: 当面临允许多个有效答案的提示时，全面生成这些答案是满足广泛用户需求的第一步。现有大语言模型在生成多样化响应方面存在局限，需要系统方法来评估和优化模型生成全面答案集的能力。

Method: 1. 引入多样性覆盖率(diversity coverage)指标，测量预测答案集中每个唯一答案的质量得分相对于相同数量下最佳可能答案集的总质量得分比例；2. 评估18个大语言模型在生成多样化响应方面的表现；3. 设计并训练一个路由模型，为每个查询预测最适合生成多样化答案集的模型。

Result: 1. 评估发现没有一个单一模型在生成多样化响应方面全面占优，但每个提示都存在一个显著优于其他模型的特定模型；2. 在NB-Wildchat数据集上，训练的路由模型性能优于单一最佳模型基线(26.3% vs 23.8%)；3. 路由模型在跨域数据集(NB-Curated)和不同答案生成提示策略上都表现出良好的泛化能力。

Conclusion: 研究表明，通过路由模型选择最适合生成多样化答案的模型是有效的策略，为在拥有多个模型的情况下研究生成全面答案奠定了基础。该方法在多个数据集和提示策略上都展现出一致的性能改进。
...
TL;DR: 提出了一种基于语言模型词汇扩展的新方法GTI（Grounded Token Initialization），通过语言基础化初始化新词元来提升生成式推荐任务性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法在向语言模型添加新可学习词元时，通常采用均值初始化策略，但这种方法会导致新词元在嵌入空间中坍缩到退化的子空间，消除了词元间的区分性，使得后续微调难以完全恢复这些区分。

Method: 提出了Grounded Token Initialization (GTI)方法：在微调之前，通过配对的语言监督将新词元映射到预训练嵌入空间中具有区分性和语义意义的位置，从而为新词元提供更好的初始化。

Result: 在多个生成式推荐基准测试（包括工业规模和公开数据集）中，GTI方法在大多数评估设置下优于均值初始化和现有的辅助任务适应方法。分析表明，经过基础化的嵌入在微调过程中保持了更丰富的词元间结构。

Conclusion: 词元初始化是语言模型词汇扩展的关键瓶颈，提出的Grounded Token Initialization假说和GTI方法通过语言基础化有效解决了这一问题，为新词元提供了更好的初始表示，使模型能够更好地利用其通用知识处理新词元领域。
...
TL;DR: 结构化LLM路由不仅是提示工程问题，更是系统级负担分配问题。研究发现没有通用的最佳路由模式，后端特定的交互效应主导性能，需要基于异构后端条件的可部署框架。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型成为智能AI系统的核心控制组件，可靠的结构化路由必须在实际部署约束下平衡正确性、延迟和实现成本。当前研究往往将结构化路由视为提示工程问题，但作者认为这本质上是系统级负担分配问题。

Method: 通过全面的全因子基准测试，评估48种部署配置和15,552个请求，涵盖OpenAI、Gemini和Llama三种后端。研究分析结构工作如何分配到生成堆栈的不同层次：模型直接输出结构、传输过程中压缩结构，或生成后本地重构结构。

Result: 核心发现：不存在通用的最佳路由模式。后端特定的交互效应主导性能表现：在Gemini和OpenAI上保持高可靠性的模式，在Llama上可能出现显著正确性下降；压缩实现的效率收益也强烈依赖后端特性。

Conclusion: 本研究提出了在异构后端条件下进行结构化路由推理的可部署框架，包括跨后端评估方法和实际部署指南，帮助生产级智能专家系统在正确性-成本-延迟边界上进行导航。
...
TL;DR: 提出数字孪生反事实框架(DTCF)，通过模拟而非估计生成反事实，并建立分层验证体系，将不可检验的反事实主张转化为可检验的假设


<details>
  <summary>Details</summary>
Motivation: 传统因果推断方法无法观测个体反事实结果，只能通过假设（如可忽略性、平行趋势、排除限制）替代缺失数据，但这些方法都无法产生真正的反事实本身

Method: DTCF框架：1)使用数字孪生模拟反事实而非统计估计；2)将孪生模拟器形式化为潜在结果框架中的随机映射；3)引入孪生保真度假设层次结构（边际保真度、联合保真度、结构保真度）；4)建立五层验证架构；5)正式分解因果量

Result: 提出了三个核心贡献：1)将不可检验的反事实主张转化为可检验的数据测试；2)区分边际可验证的因果量（ATE、CATE、QTE）和依赖copula的因果量（ITE分布、获益/伤害概率）；3)提供边界、敏感性和不确定性量化工具

Conclusion: DTCF并未解决因果推断的根本问题，但提供了一个框架：边际因果主张变得可检验，联合因果主张变得明确假设索引化，且两者之间的差距被正式表征
...
TL;DR: IDEA2是一个结合大型语言模型(LLM)的半自动化工作流程，用于从需求文档中提取能力问题(CQs)，通过专家参与和反馈驱动的迭代过程，提高CQ提取的效率和质量。


<details>
  <summary>Details</summary>
Motivation: 能力问题(CQ)提取是本体工程中的关键但资源密集型瓶颈，传统方法存在领域专家与本体工程师之间的沟通鸿沟问题。

Method: 提出IDEA2工作流程，包含核心迭代循环：1)基于LLM从需求文档中初始提取CQs；2)领域专家在协作平台上进行共同创审和反馈；3)对拒绝的CQs进行反馈驱动的LLM迭代重新表述直到达成共识。使用溯源模型跟踪每个CQ的完整生命周期。

Result: 在两个真实世界场景（科学数据、文化遗产）中验证，结果表明IDEA2能加速需求工程过程，提高CQs的接受度和相关性，并在领域专家中表现出高可用性和有效性。

Conclusion: IDEA2通过LLM与专家循环结合的方法，有效解决了能力问题提取中的沟通瓶颈问题，提供了一种透明、可重复且高效的工作流程。
...
TL;DR: 提出世界中心多智能体系统作为传统智能体中心架构的替代方案，通过共享显式世界表示确保语义一致性、可解释性和长期稳定性


<details>
  <summary>Details</summary>
Motivation: 传统智能体中心架构在结构化领域（如企业系统、制度系统）中面临语义一致性、可解释性和长期稳定性的挑战，需要一种共享、显式的世界表示来确保系统行为的全局一致性和可验证性

Method: 1) 引入世界中心多智能体系统概念；2) 沿本体显式性、规范性等维度对世界进行分类；3) 提出语义模型作为表示此类世界的数学形式化方法；4) 开发Ontobox平台作为WMAS的具体实现

Result: 提出了一套完整的世界中心多智能体系统理论框架：包括WMAS概念定义、世界分类维度、基于共享世界模型的学习与协调机制，以及语义模型的数学形式化表示方法，并通过Ontobox平台提供了具体实现

Conclusion: 世界中心多智能体系统为结构化领域中的多智能体系统设计提供了更优越的架构方案，通过共享、显式的世界表示确保了语义一致性、可解释性和系统长期稳定性，是传统智能体中心架构的重要替代方案
...
TL;DR: 该研究提出AI自动化存在两种模式："崩溃波"（AI能力在少数任务上突然爆发）和"上升潮"（AI能力在广泛任务上持续普遍提升），通过对3000多个文本任务的评估发现AI自动化主要表现为"上升潮"模式，AI在大多数文本任务上的成功率预计到2029年将达到80%-95%。


<details>
  <summary>Details</summary>
Motivation: 现有研究对AI能力增长的描述不一致，有些研究（如METR）发现AI能力在特定任务上突然爆发（"崩溃波"），而本研究旨在通过系统评估验证AI自动化是连续的普遍提升（"上升潮"）还是集中在少数任务的爆发。

Method: 研究使用美国劳工部O*NET分类中的3000多个文本任务进行大规模评估，这些任务都是基于文本且适合LLM处理的。通过对17000多名相关行业工作者的评估，系统测量AI在不同任务上的表现和能力增长轨迹。

Result: 研究发现极少证据支持"崩溃波"模式，但有充分证据表明"上升潮"是AI自动化的主要形式。AI在2024年第二季度能以约50%的成功率完成需要人类3-4小时的任务，到2025年第三季度将提升至65%。如果当前趋势持续，到2029年LLM在大多数文本任务上的平均成功率将达到80%-95%。

Conclusion: AI自动化主要表现为广泛的、持续的能力提升（"上升潮"），而非集中在少数任务的突然爆发。这种能力提升将对经济和组织采用AI产生重要影响，虽然组织层面的实际应用可能滞后于技术能力的发展。
...
TL;DR: 大语言模型在认知任务中表现出系统性的认知偏差，这些偏差在模型激活空间中表现为线性可分离方向，通过激活导向技术可以在减少偏差的同时保持下游任务能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多地用于高风险决策场景，尽管已有研究表明LLMs在行为上表现出认知偏差，但这些偏差是否对应于可识别的内部表征，以及能否通过针对性干预来缓解，仍然是一个未解决的问题。

Method: 1. 定义LLM认知偏差为任务中系统性、可重复的与可计算基准答案的偏差；2. 引入LLM CogBias基准，围绕四类认知偏差构建：判断、信息处理、社交和反应偏差；3. 使用线性探针在对比设计下分析模型激活空间；4. 应用激活导向技术来调节偏差行为。

Result: 1. 三个LLM在所有四类偏差中均表现出系统性认知偏差，且去偏响应强烈依赖于偏差类型：提示级去偏显著减少反应偏差但加剧判断偏差；2. 认知偏差在模型激活空间中编码为线性可分离方向；3. 激活导向技术将偏差分数降低26-32%，同时在下游25个基准测试中保持能力；4. 尽管跨模型偏差表征接近正交，但导向在类似程度上减少偏差，表明共享功能组织。

Conclusion: LLM认知偏差确实对应于可识别的内部表征，且可以通过激活导向技术有效缓解。尽管不同模型的偏差表征在几何上差异很大，但它们的功能组织方式相似，表明存在跨架构的共享偏差减少机制。
...
TL;DR: 提出RIFT：一种用于系统性诊断评分标准（rubric）质量问题的分类框架，包含八个故障模式，分为三个高层类别，并开发了自动化的评分标准质量指标来支持规模化诊断。


<details>
  <summary>Details</summary>
Motivation: 评分标准在LLM基准测试和训练中被广泛用于开放式、可验证性较差的任务。虽然先前工作通过下游信号（如强化学习结果）证明了评分标准的有效性，但仅从这些聚合或下游信号中无法系统性地诊断评分标准的质量问题。因此需要一种原则性的方法来分析和改进评分标准的设计。

Method: 1. 开发RIFT分类法：使用扎根理论方法，从五个多样化基准测试中迭代标注评分标准，直至没有新的故障模式出现。2. 评估分类法一致性：通过独立人工标注者间的一致性（87%成对同意度和0.64平均Cohen's kappa）来衡量。3. 提出自动化评分标准质量指标：以支持规模化诊断，并验证这些指标与人工故障模式标注的对齐程度。

Result: 1. 开发了RIFT分类法，包含三个高层类别的八个故障模式：可靠性故障、内容效度故障、后果效度故障。2. 人工标注者间表现出良好的一致性（87%成对同意度，0.64平均Cohen's kappa）。3. 自动化评分标准质量指标与人工故障模式标注有良好对齐，达到最高0.86 F1分数。

Conclusion: RIFT为系统性诊断评分标准质量问题提供了原则性框架，填补了现有工作的空白。开发的自动化指标支持规模化应用，有助于提高LLM评估和训练中评分标准的质量和可靠性。
...
TL;DR: 提出基于价值信息（VOI）的POMDP规划方法VOIMCP，通过有条件处理观测信息来减少计算成本，在有限规划时间内获得高性能策略。


<details>
  <summary>Details</summary>
Motivation: 解决POMDP规划中的维数灾难和历史灾难问题。虽然POMDP为状态和转移不确定性下的规划提供了原则性形式，但在有限规划时间内获得高性能策略仍然困难，主要挑战来自维数灾难和历史灾难。许多POMDP问题中，价值信息在信念空间中变化，这提供了优化的机会。

Method: 提出动态规划框架，利用价值信息结构条件性地处理观测。在此基础上提出价值信息蒙特卡洛规划（VOIMCP），这是一种蒙特卡洛树搜索算法，通过有选择性地忽略低价值信息的观测来更有效地分配计算努力，避免不必要的观测分支。

Result: 为VOI推理框架提供了理论上的接近最优性保证，并为VOIMCP推导了非渐近收敛界。仿真评估表明VOIMCP在多个POMDP基准测试中优于基线方法。

Conclusion: 提出的VOIMCP方法通过在信念空间中条件性地处理观测信息，有效缓解了POMDP规划中的计算负担，在有限规划时间内实现了更好的性能表现，为处理大规模POMDP问题提供了新的有效途径。
...
TL;DR: 本文通过模拟场景分析AI辅助用药系统的可靠性，关注系统失败类型及其临床后果，强调在医疗领域需要风险感知的评估方法。


<details>
  <summary>Details</summary>
Motivation: 尽管AI系统在医疗和药学工作流程中应用日益广泛，但在标准评估指标下表现良好的系统，其在实际决策中的可靠性仍未被充分理解。在高风险的用药管理领域，即使是单一的错误推荐也可能导致严重的患者伤害。

Method: 通过一系列涉及药物相互作用和剂量决策的受控模拟场景，分析不同类型的系统失败，包括漏检相互作用、错误风险标记和不适当的剂量推荐。

Result: 研究发现，在用药相关情境中，AI错误可能导致不良反应、无效治疗或延误护理，特别是在缺乏足够人工监督的情况下。论文还讨论了过度依赖AI推荐的风险以及决策过程透明度有限的挑战。

Conclusion: 本文为医疗AI评估提供了一个以可靠性为中心的视角，强调理解失败行为和实际影响的重要性。在安全关键领域如药学实践中，需要用风险感知的评估方法来补充传统性能指标。
...
### [163] [Infeasibility Aware Large Language Models for Combinatorial Optimization](https://arxiv.org/abs/2604.01455)
*Yakun Wang,Min Chen,Zeguan Wu,Junyu Liu,Sitao Zhang,Zhenwen Shao*

Main category: cs.AI

TL;DR: 论文提出一个不可行性感知框架，结合可认证数据集构建、监督微调和LLM辅助下游搜索，用于组合优化问题中的不可行性检测和解决方案生成。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在组合优化中的应用主要关注可行实例的解决方案生成，而忽视了不可行性检测。需要一种能够同时处理可行解生成和不可行性检测的框架。

Method: 1) 针对minor-embedding问题提出新的数学规划公式和可证明的零相位不可行性筛查；2) 构建带有结构化证书的可认证训练数据集；3) 使用80亿参数LLM进行监督微调，联合执行解决方案生成和不可行性检测；4) 利用LLM输出作为下游局部搜索的暖启动。

Result: 微调模型比GPT-5.2提高整体准确率30%；LLM引导的暖启动相比从零开始的下游局部搜索提供2倍加速。

Conclusion: 提出的不可行性感知框架能有效解决组合优化中的不可行性检测问题，并证明LLM可以通过结构化训练数据学习复杂的数学规划概念，为不完美的LLM输出提供了实用的下游优化加速方法。
...
### [164] [Reducing Hallucinations in LLM-based Scientific Literature Analysis Using Peer Context Outlier Detection](https://arxiv.org/abs/2604.01461)
*Daniel Xie,Maxwell J. Jacobson,Adil Wazeer,Haiyan Wang,Xinghang Zhang,Yexiang Xue*

Main category: cs.AI

TL;DR: 提出了一种新颖的Peer Context Outlier Detection (P-COD)方法，通过利用文档间的相关性来减少大语言模型在数据提取中的幻觉，在科学文献总结领域实现高达98%的异常检测精度。


<details>
  <summary>Details</summary>
Motivation: 当前减少大语言模型幻觉的方法（如提示工程、思维链提示）主要关注单个文档，忽略了整个语料库中文档间的关系。在科学文献总结领域，具有相似实验设置的论文应该得出相似的结论，因此需要一种能利用文档间关系来提高提取准确性的方法。

Method: 提出了Peer Context Outlier Detection (P-COD)方法：通过比较从文档中提取的数据与语料库中已验证的同行信息，利用文档间的相关性来调整置信度分数，标记低置信度结果供专家审查，同时将经过同行验证的高置信度结果视为可靠。

Result: 实验在6个科学领域进行，展示了高达98%的异常检测精度。该方法减少了幻觉，增强了对自动化系统的信任，允许研究人员专注于模糊案例，从而简化了数据提取工作流程。

Conclusion: P-COD方法通过利用文档间的相关性，在减少大语言模型幻觉方面是有效的，特别适用于科学文献总结等需要跨文档一致性验证的应用场景。该方法不仅提高了提取准确性，还优化了工作流程，使研究人员能够更有效地处理数据提取任务。
...
### [165] [A Self-Evolving Agentic Framework for Metasurface Inverse Design](https://arxiv.org/abs/2604.01480)
*Yi Huang,Bowen Zheng,Yunxi Dong,Hong Tang,Huan Zhao,S. M. Rakibul Hasan Shawon,Hualiang Zhang*

Main category: cs.AI

TL;DR: 提出了一个基于技能演化的智能体框架，用于超表面逆向设计，通过上下文级别的技能进化积累可重用的求解器特定专业知识，提高设计工作流的自主性和可访问性。


<details>
  <summary>Details</summary>
Motivation: 超表面逆向设计需要计算电磁学和求解器特定软件工程的专业知识，现有语言驱动系统大多是会话受限的，无法跨任务保存可重用的工作流知识。

Method: 开发了一个包含编码智能体、演化技能工件和基于物理仿真的确定性评估器的智能体框架。该框架通过迭代精炼求解器特定策略，在不修改模型权重或底层物理求解器的情况下实现跨任务的技能进化。

Result: 在训练对齐任务中，技能演化将任务成功率从38%提升到74%，标准通过率从0.510提高到0.870，平均尝试次数从4.10减少到2.30。在保持任务中，虽然二元成功率变化不大，但最佳裕度改进、错误构成变化和智能体行为表明工作流知识有部分迁移。

Conclusion: 技能演化的主要价值在于围绕可靠的计算引擎积累可重用的求解器特定专业知识，为实现更自主、更易访问的超表面逆向设计工作流提供了一条实用路径。
...
TL;DR: 本研究提出了一个基于角色的LLM框架，通过分配专门角色（政策分析师、法律策略专家和食品系统专家）来自动化从非结构化健康食品政策文件中提取信息，解决了传统LLM方法在信息提取中的幻觉、误分类和遗漏问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在健康食品政策领域的信息提取面临诸多挑战，包括幻觉、误分类和遗漏，这些问题源于政策文档的结构多样性和不一致性。需要一种可靠的方法来自动化政策信息提取。

Method: 提出了基于角色的LLM框架，包含三个专门角色：LLM政策分析师（负责元数据和机制分类）、LLM法律策略专家（识别复杂法律方法）、LLM食品系统专家（分类食品系统阶段）。框架将结构化领域知识（法律机制定义和分类标准）融入角色特定提示中，模拟专家分析工作流程。

Result: 使用来自健康食品政策项目数据库的608个政策进行评估，与零样本、少样本和思维链基线（使用Llama-3.3-70B）进行比较。提出的框架在复杂推理任务中表现出优越性能。

Conclusion: 该基于角色的LLM框架为自动化健康政策信息提取提供了可靠且透明的方法论，在复杂推理任务中表现优异，能够有效解决传统LLM方法在政策信息提取中的局限性。
...
### [167] [PHMForge: A Scenario-Driven Agentic Benchmark for Industrial Asset Lifecycle Maintenance](https://arxiv.org/abs/2604.01532)
*Ayan Das,Dhaval Patel*

Main category: cs.AI

TL;DR: PHMForge：首个专门评估LLM智能体在预测与健康管理(PHM)任务上的综合基准，通过领域特定的MCP服务器实现现实交互，涵盖75个专家策划场景、7类工业资产和5项核心任务类别。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法捕捉工业领域的严格要求（错误决策可能带来重大安全和财务后果），需要专门针对预测与健康管理(PHM)任务设计新的基准，以评估LLM智能体在复杂工具编排任务中的表现。

Method: 构建PHMForge基准，包含75个专家策划场景，涵盖7类工业资产（涡轮风扇发动机、轴承、电机、变速箱、航空发动机）和5项核心任务类别。开发65个专门工具分布在两个MCP服务器上，实现基于执行的评估器，采用任务对应指标（MAE/RMSE用于回归，F1-score用于分类，分类匹配用于健康评估）。评估主流框架（ReAct、Cursor Agent、Claude Code）结合前沿LLM（Claude Sonnet 4.0、GPT-4o、Granite-3.0-8B）。

Result: 即使最佳配置也只达到68%任务完成率，存在系统性故障：工具编排错误（23%序列不正确）、多资产推理（性能下降14.9个百分点）、跨设备泛化（在未见数据集上为42.7%）。

Conclusion: PHMForge揭示了当前LLM智能体在工业AI任务中的局限性，特别是在工具编排、多资产推理和泛化能力方面存在显著挑战。开源的基准将促进智能体工业AI的研究。
...
TL;DR: RAE-AR：通过分布归一化和高斯噪声注入，将表示自编码器整合到连续自回归模型中，缩小与VAE在生成建模上的性能差距。


<details>
  <summary>Details</summary>
Motivation: 生成建模的潜空间长期以来由VAE编码器主导，预训练表示编码器（如DINO、SigLIP、MAE）的潜在表示被认为不适合生成建模。虽然RAE方法显示了希望，但将表示自编码器整合到连续自回归模型中仍未被充分探索。

Method: 提出RAE-AR框架，针对AR模型特性解决两大挑战：1) 通过分布归一化进行标记简化，降低复杂标记分布建模难度；2) 训练期间加入高斯噪声注入，缓解高维性放大的训练-推理差距（暴露偏差）。

Result: 这些修改显著缩小了性能差距，使表示自编码器在AR模型上能够取得与传统VAE相当的结果。

Conclusion: 这项工作为视觉理解和生成建模之间更统一的架构铺平了道路，展示了表示自编码器在生成任务中的潜力。
...
TL;DR: 研究发现LLM训练中归一化层与优化器之间的交互作用：在Muon优化器下，Dynamic Erf归一化会产生显著的性能下降，而Dynamic Tanh则无此问题，揭示了优化器与归一化层的交互影响不容忽视。


<details>
  <summary>Details</summary>
Motivation: 传统LLM训练中将归一化层和优化器视为独立设计选择，但本文旨在验证这一假设是否成立，特别是在新型归一化方法(Derf)与新型优化器(Muon)组合时是否存在意外交互效应。

Method: 在10亿参数规模、1000训练步数的设定下，进行3x2因子设计实验。比较Dynamic Erf、Dynamic Tanh（作为有界归一化控制组）和RMSNorm在不同优化器（AdamW vs Muon）下的表现。分析失效模式，并提出两种缓解方案：1) EMA混合方法重新引入运行尺度估计；2) 调整Derf的alpha参数从0.5降低到0.3。

Result: Derf与Muon组合存在显著的负向交互作用：相对于RMSNorm，Derf在AdamW下的优势为+0.31 nats，但在Muon下扩大到+0.97 nats（约3倍差距）。Dynamic Tanh无此惩罚。失效机制归因于erf在Muon较快谱范数增长下的两种失效模式：饱和（有损压缩）和尺度盲目性（丢弃激活幅度）。EMA混合方法恢复了约84%的差距，降低alpha至0.3恢复了约80%的差距。

Conclusion: 归一化层与优化器不能视为独立设计选择，特别是在新方法组合时需系统评估交互效应。Derf的发布默认alpha参数(0.5)与Muon组合会产生0.66 nat的交互惩罚且不会引发NaN或发散，在短时试验中容易遗漏此问题。这强调了在LLM架构设计中进行全面组合测试的重要性。
...
TL;DR: NED-Tree是一个通过递归树结构分解非线性元素来解决自然语言到运筹学问题自动转换的框架，在复杂非线性场景中实现了72.51%的平均准确率


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在将自然语言运筹学问题转换为可执行模型时，在线性任务上表现良好，但在现实世界非线性场景中性能严重下降，主要原因是数学公式与求解器代码之间的语义不对齐以及信息提取不稳定

Method: 提出NED-Tree框架，采用逐句提取策略确保稳健的参数映射和可追溯性，并使用递归树结构将复杂非线性项自适应分解为求解器兼容的子元素。同时构建了NEXTOR基准测试用于复杂非线性、多约束运筹学问题评估

Result: 在10个基准测试上的实验表明，NED-Tree取得了72.51%的平均准确率，建立了新的最先进水平，是首个通过元素分解解决非线性建模困难、实现建模语义与代码语义对齐的框架

Conclusion: NED-Tree通过系统化框架有效解决了运筹学问题自然语言建模中的语义对齐挑战，特别是在非线性场景下，通过递归树分解实现了建模语义与求解器代码语义的一致性，显著提升了自动建模的准确性和可靠性
...
<details>
  <summary>Details</summary>
Motivation: 现有的强化学习与人类反馈方法通常需要额外的正确性信号或批判性标注来训练自我精炼能力，这限制了效率和应用范围。需要一种能够同时优化推理和精炼能力，且不依赖额外监督信号的方法。

Method: 提出ThinkTwice两阶段框架：1)第一阶段优化模型解决推理问题；2)第二阶段优化模型精炼自己对同一问题的解决方案。使用相同的二元正确性奖励，无需正确性信号或批判性标注，基于Group Relative Policy Optimization (GRPO)进行优化。

Result: 在五个数学推理基准和两个模型家族（Qwen3-4B和Olmo3-7B）上，ThinkTwice显著优于竞争性在线策略优化基线。具体而言，在Qwen3-4B上，ThinkTwice在AIME上比GRPO精炼前提高5个百分点，一次自我精炼后提高11.5个百分点（按pass@4测量）。训练动态分析揭示了隐式的"修正-巩固"课程：精炼早期主要纠正错误，随着模型改进自然转向保护已正确的解决方案。

Conclusion: ThinkTwice框架确立了联合训练推理和自我精炼作为一种原则性有效的RLVR方法学，通过简单的两阶段优化实现了显著的性能提升，并揭示了训练过程中自动形成的有利课程结构。
...
TL;DR: 大型语言模型(LLMs)在教学决策中主要采用贝叶斯最优教学策略，通过逆规划推断学习者的知识状态，而非依赖简单启发式规则。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究LLMs在教学决策中的认知机制：它们是通过推理学习者的知识状态来做出教学选择，还是依赖更简单的经验法则？该研究旨在将人类教学策略研究范式应用于LLMs，比较其与人类教学策略的相似性。

Method: 研究方法采用先前用于研究人类教学策略的控制任务：教师LLM观察假设学习者在带奖励标注的有向图中的轨迹，需要揭示单个边以帮助学习者在重新规划时选择更好的路径。研究运行多个LLM作为模拟教师，使用与人类研究相同的认知模型（贝叶斯最优教师模型、弱贝叶斯变体、启发式基线、非心智化效用模型）通过贝叶斯信息准则(BIC)拟合其试次选择。实验包括基线实验和支架干预实验。

Result: 在匹配人类刺激的基线实验中，大多数LLM表现良好，策略在试次间变化小，性能与人类相似。模型比较显示贝叶斯最优教学最能解释多数模型的决策。在支架干预下，模型遵循推理或奖励导向的提示，但这些干预在启发式不一致的测试图上并不能可靠提升后续教学表现，有时反而降低性能。

Conclusion: 认知模型拟合揭示了LLMs的教学策略本质，表明提示遵循性并不保证更好的教学决策。LLMs在教学任务中主要采用基于学习者知识推理的贝叶斯最优策略，而非简单启发式，但其对教学支架的响应有限。
...
### [173] [ByteRover: Agent-Native Memory Through LLM-Curated Hierarchical Context](https://arxiv.org/abs/2604.01599)
*Andy Nguyen,Danh Doan,Hoang Pham,Bao Ha,Dat Pham,Linh Nguyen,Hieu Nguyen,Thien Nguyen,Cuong Do,Phat Nguyen,Toan Nguyen*

Main category: cs.AI

TL;DR: ByteRover提出了一种原生于智能体的记忆架构，通过让LLM直接管理记忆（而非外部服务），解决了现有MAG系统中语义漂移、协调上下文丢失和故障恢复脆弱等问题，在零外部基础设施下实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有MAG方法将记忆作为外部服务处理，委托给独立的文本分块、嵌入和图提取流水线。这种架构分离导致存储系统不理解知识，造成：1）智能体意图记忆与实际捕获内容之间的语义漂移；2）跨智能体协调上下文丢失；3）故障后恢复脆弱。

Method: 1）记忆架构反转：让推理任务的同一LLM负责知识策展、结构化和检索；2）分层上下文树表示：基于文件的层次化知识图，组织为域、主题、子主题和条目；3）自适应知识生命周期：包含重要性评分、成熟度层级和近期性衰减；4）5层渐进检索策略：多数查询在亚100ms延迟内解决，无需LLM调用。

Result: 在LoCoMo基准测试中达到最先进的准确率，在LongMemEval上取得竞争性结果。系统无需外部基础设施（无向量数据库、无图数据库、无嵌入服务），所有知识存储为本地文件系统上的人类可读markdown文件。

Conclusion: ByteRover通过将记忆管理内置于智能体推理过程，解决了传统MAG的语义不匹配问题，实现了更高效、更鲁棒的长上下文推理，同时显著降低了系统复杂性和依赖。
...
### [174] [MM-ReCoder: Advancing Chart-to-Code Generation with Reinforcement Learning and Self-Correction](https://arxiv.org/abs/2604.01600)
*Zitian Tang,Xu Zhang,Jianbo Yuan,Yang Zou,Varad Gunjal,Songyao Jiang,Davide Modolo*

Main category: cs.AI

TL;DR: MM-ReCoder：一个基于强化学习训练并具备自我校正能力的图表转代码生成多模态大语言模型，通过两阶段多轮自我校正策略在三个基准测试中取得最先进性能


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在图表转代码生成任务中主要依赖监督微调，但这种方法存在两个主要问题：1) 模型无法通过代码执行环境学习；2) 现有模型在利用执行反馈进行有效自我校正方面表现不佳，即使是最先进的模型也在这方面存在困难

Method: 提出MM-ReCoder模型，采用基于Group Relative Policy Optimization（GRPO）的两阶段多轮自我校正强化学习策略。第一阶段通过滚动共享的第一轮增强模型的自我校正能力；第二阶段通过完整轨迹优化提升编码能力。模型通过与环境的交互和迭代自我校正输出来学习生成更准确和可执行的代码

Result: 在三个图表转代码基准测试中取得了最先进的性能，证明MM-ReCoder能够生成更准确且可执行的代码

Conclusion: 通过强化学习和自我校正策略的结合，MM-ReCoder显著提升了多模态大语言模型在图表转代码生成任务中的能力，克服了传统监督微调方法的局限性，为解决代码生成质量提供了有效的技术路径
...
### [175] [CRaFT: Circuit-Guided Refusal Feature Selection via Cross-Layer Transcoders](https://arxiv.org/abs/2604.01604)
*Su-Hyeon Kim,Hyundong Jin,Yejin Lee,Yo-Sub Han*

Main category: cs.AI

TL;DR: CRaFT是一个基于电路引导的拒绝特征选择框架，通过评估特征对模型拒绝-合规决策的影响来识别因果性拒绝特征，相比传统基于激活强度的方法能更有效提高攻击成功率


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型安全问题的日益凸显，理解拒绝行为的内部机制变得至关重要。现有方法依赖于特征在有害提示上的激活强度，这往往捕捉的是表面信号而非拒绝决策的因果因素，需要更准确的特征选择方法。

Method: 提出了CRaFT框架，通过评估特征对模型拒绝-合规决策的影响来排名特征，特别关注拒绝边界附近的提示。该方法基于电路引导，识别那些在拒绝决策中起因果作用的特征，而不是仅看激活强度。

Result: 在Gemma-3-1B-it模型上，CRaFT将攻击成功率（ASR）从6.7%提升到48.2%，并在多个越狱基准测试中优于基线方法。这表明电路影响力比激活幅度更能可靠地识别因果性拒绝特征。

Conclusion: 电路影响力是基于激活强度的传统方法的更可靠替代，能够更准确地识别在拒绝行为中起因果作用的特征，为提高模型安全分析和越狱攻击的效力提供了新视角。
...
### [176] [From Multi-Agent to Single-Agent: When Is Skill Distillation Beneficial?](https://arxiv.org/abs/2604.01608)
*Binyan Xu,Dong Fang,Haitao Li,Kehuan Zhang*

Main category: cs.AI

TL;DR: 提出Metric Freedom (F)作为预测多智能体系统蒸馏为单智能体技能效用的先验指标，并开发了基于F的两阶段自适应蒸馏框架，能够在降低成本的同时保持或提升性能。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统(MAS)处理复杂任务时面临协调开销大、上下文碎片化和脆弱的阶段排序问题。虽然将MAS蒸馏为单智能体技能可以绕过这些成本，但缺乏关于何时及如何蒸馏的原则性指导，导致实验结果不一致（技能提升从28%改进到2%退化不等）。

Method: 引入Metric Freedom (F)，通过Mantel测试量化输出多样性与得分方差之间的耦合程度，度量评分空间的拓扑刚性。基于F提出两阶段自适应蒸馏框架：阶段1作为选择性提取机制，在"自由"指标上提取工具和知识同时丢弃限制性结构；阶段2专门针对"刚性"指标(F ≤ 0.6)进行计算密集的迭代优化，避免轨迹局部过拟合。

Result: 在4个任务、11个数据集和6个指标上的评估表明，F强烈预测技能效用(ρ = -0.62, p < 0.05)。相同的智能体轨迹在刚性与自由指标下产生截然相反的技能提升。基于该信号的适配智能体匹配或超过原始MAS，同时将成本降低高达8倍、延迟降低高达15倍。

Conclusion: 技能效用本质上是指标层面的属性而非任务属性，Metric Freedom (F)作为第一个先验预测指标能够有效指导自适应蒸馏策略，在显著降低多智能体系统成本的同时保持性能。
...
TL;DR: 系统评估AWS Bedrock平台上的四种指令调优大语言模型在收据项目分类任务中的性能与成本，发现Claude 3.7 Sonnet在分类准确性和成本效率之间取得最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 在面向生产的分类框架中，需要系统评估不同大语言模型在收据项目分类任务中的实际应用价值，特别是在性能（准确性、响应稳定性）与成本（token级成本）之间的权衡，以及探索最适合的提示方法（零样本或少样本）。

Method: 采用系统性、成本感知的评估方法，在AWS Bedrock平台上比较四种指令调优模型：Claude 3.7 Sonnet、Claude 4 Sonnet、Mixtral 8x7B Instruct和Mistral 7B Instruct。研究评估准确性、响应稳定性和token级成本，并比较零样本和少样本提示方法在准确性和成本方面的表现。

Result: 实验结果表明，Claude 3.7 Sonnet在分类准确性和成本效率之间实现了最有利的平衡。研究还确定了在准确性和成本方面最合适的提示方法。

Conclusion: Claude 3.7 Sonnet是收据项目分类任务中最具成本效益的模型选择，其研究结果为生产环境中的大语言模型部署提供了实用的成本-性能权衡指导。
...
TL;DR: OSCAR是一个无需训练、基于推理时的框架，通过并行去噪链的交叉链熵分析来定位并修正扩散语言模型中的事实幻觉问题，显著提升生成质量和事实准确性。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型(DLMs)暴露了其去噪轨迹，为推理时控制提供了自然机制。理想的幻觉缓解框架应该利用这种模型原生信号进行干预，而不是依赖外部训练的幻觉分类器。

Method: 提出承诺不确定性定位：给定去噪轨迹，识别交叉链熵超过无监督阈值的令牌位置。引入OSCAR框架：运行N条并行去噪链(随机揭示顺序)，计算交叉链香农熵检测高不确定性位置，然后基于检索证据进行针对性重掩码。还引入了一套轨迹级评估指标，包括交叉链发散度指标(CDH)。

Result: 在TriviaQA、HotpotQA、RAGTruth和CommonsenseQA数据集上使用LLaDA-8B和Dream-7B模型，OSCAR通过显著减少幻觉内容和改进事实准确性，提升了生成质量。消融实验确认定位和修正带来互补增益，且在N={4,8,16}时鲁棒。基于熵的不确定性信号优于专门的训练检测器。

Conclusion: 扩散语言模型具有识别事实不确定性的内在能力，这种能力在自回归模型的顺序令牌承诺结构中不存在。OSCAR框架展示了利用DLMs原生信号的潜力，为定位和不确定性感知生成提供了新的研究方向。
...
TL;DR: CORAL是一个用于自主多智能体进化的框架，通过持久内存、异步执行和心跳干预取代固定启发式方法，在开放发现问题上实现知识积累和持续搜索，显著提升了进化效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的进化方法主要依赖固定启发式和硬编码探索规则，限制了智能体的自主性，无法有效支持开放发现所需的知识积累和持续搜索。

Method: CORAL框架采用长时间运行的智能体，通过共享持久记忆、异步多智能体执行和基于心跳的干预来实现探索、反思和协作。包含隔离工作空间、评估分离、资源管理以及智能体会话和健康管理等安全保障措施。

Result: 在多样化数学、算法和系统优化任务上，CORAL在10个任务中取得了最先进的结果，相比固定的进化搜索基线，改进率提高了3-10倍且评估次数更少。在Anthropic内核工程任务中，4个协同进化智能体将已知最佳分数从1363周期提升到1103周期。

Conclusion: 机制分析表明这些提升源于知识复用和多智能体探索与通信。结果表明更大的智能体自主性和多智能体进化能显著改善开放发现问题，为自主智能体系统提供了实用框架。
...
TL;DR: 这篇论文提出了七种面向临床AI系统的本体感知设计模式，旨在构建能够抵抗文档扭曲影响的AI流水线。这些模式基于GoF模式语言，为临床AI系统提供了一个设计词汇表。


<details>
  <summary>Details</summary>
Motivation: 临床AI系统通常在结构上受到文档工作流程、计费激励和术语碎片化扭曲的健康数据上训练。现有研究虽然描述了这些扭曲机制，但将这些见解转化为可实施的软件架构仍然是一个开放问题。

Method: 提出了七种本体感知设计模式：Ontological Checkpoint、Dormancy-Aware Pipeline、Drift Sentinel、Dual-Ontology Layer、Reification Circuit Breaker、Terminology Version Gate和Regulatory Compliance Adapter。每种模式都按问题、力量、解决方案、后果、已知用途和相关模式进行规范说明。论文还通过初级保健AI系统的参考架构和糖尿病风险预测场景来说明这些模式的组合应用。

Result: 提供了完整的设计模式集，其中三种模式在现有系统中已有部分先例，其余四种尚未被正式描述。论文还提供了一个参考架构和场景演练来展示这些模式的实际应用。

Conclusion: 这篇论文为构建能够抵抗本体扭曲的临床AI系统提供了一个基于理论分析的设计词汇表，但尚未进行实证验证。该框架需要在生产系统中进行未来评估，并且目前仅限于德国和欧盟监管环境。
...
TL;DR: BACM-RL：一种预算感知的上下文管理框架，通过强化学习学习在有限上下文预算下的最优压缩策略，在长时推理任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在长时程推理中表现出强大潜力，但部署因素（内存、延迟、成本）限制了其上下文大小，形成了有限的上下文预算。随着交互历史增长，需要在保留历史信息和保持在上下文限制内之间做出权衡。

Method: 提出BACM（预算感知上下文管理），将上下文管理建模为具有上下文预算约束的顺序决策问题。开发BACM-RL，一个端到端的基于课程的强化学习方法，学习在不同上下文预算下的压缩策略。

Result: 在组合多目标问答和长时程网页浏览基准测试中，BACM-RL在不同模型规模和任务复杂度下均优于现有方法，在高复杂度设置中超过基线方法1.6倍以上，且在预算缩减时仍保持优势，而大多数方法性能会下降。

Conclusion: BACM-RL通过将上下文管理视为顺序决策问题并采用强化学习方法，能够有效解决LLM智能体在有限上下文预算下的长时程推理挑战，为实际部署提供了可行的解决方案。
...
TL;DR: 提出了分层记忆编排（HMO）框架，通过三层目录组织交互历史来解决长期记忆存储引起的性能瓶颈问题


<details>
  <summary>Details</summary>
Motivation: 虽然长期记忆对智能体保持历史认知至关重要，但大量交互数据的积累会导致性能瓶颈，简单的存储扩展会增加检索噪声和计算延迟，特别是在资源受限的个人设备上会压倒模型的推理能力

Method: 提出分层记忆编排（HMO）框架，将交互历史组织为三层目录结构：紧凑的主缓存（包含近期和关键记忆+演进中的用户画像），高优先级的二级层，以及完整的全局归档。用户画像驱动记忆在层次间的重新分配，将符合长期模式的记录推向更活跃的层级，同时将相关性较低的信息降级

Result: 在多个基准测试中达到最先进性能。在实际部署（如OpenClaw生态系统）中，HMO显著提升了智能体的流畅性和个性化程度

Conclusion: 这种有针对性的编排能够精确地在需要时呈现历史知识，同时保持精简高效的活动搜索空间，解决了长期记忆存储的性能瓶颈问题
...
TL;DR: HeteroFusion是一个用于异构语言模型融合的方法，它解决了不同架构模型（如Llama、Qwen、Mistral）的合并问题，通过拓扑对齐和冲突感知去噪技术实现跨架构的知识转移。


<details>
  <summary>Details</summary>
Motivation: 现有模型合并方法主要针对同构模型（共享同一预训练主干），但在开放的模型生态系统中，有用的专家模型通常基于不同的架构家族（如Llama、Qwen、Mistral），这种异构设置导致直接的权重空间融合变得不适用，存在架构不匹配、潜在基础对齐问题和跨源冲突加剧等问题。

Method: HeteroFusion包含两个关键组件：1）基于拓扑的对齐技术，通过匹配功能模块结构而不是原始张量坐标来在异构主干之间转移知识；2）冲突感知去噪技术，在融合过程中抑制不兼容或嘈杂的转移信号。该方法还通过保持目标适配器基础同时预测结构化更新来确保稳定的转移过程。

Result: 在异构转移、多源融合、噪声源鲁棒性和跨家族泛化等设置下，HeteroFusion一致地优于强大的合并、融合和集成基线方法。

Conclusion: HeteroFusion为异构语言模型融合提供了有效的解决方案，通过结构对齐和冲突抑制技术实现了跨不同架构模型的稳定知识转移，解决了现有合并方法在异构场景下的局限性。
...
TL;DR: 该研究利用中国视频平台数据，分析了AIGC与HGC在创作和消费行为上的差异，发现AIGC创作者通过大规模生产获得与HGC相当的总体参与度，尽管消费者更偏好HGC，并探讨了算法分发机制在调节这些竞争利益中的作用。


<details>
  <summary>Details</summary>
Motivation: 人工智能生成内容(AIGC)的快速增长正在重构在线内容生态，需要对其行为特征和分发影响进行严格研究。随着AIGC在内容平台上的普及，理解其与人类生成内容(HGC)在创作和消费行为上的差异，以及平台算法如何调节这两类内容之间的竞争关系，对于维护内容生态健康具有重要意义。

Method: 利用来自中国领先视频分享平台的全面纵向数据集，包含数千万用户数据。通过分析比较AIGC与HGC的创作和消费行为特征，识别两类内容在生产和消费模式上的差异。深入研究算法内容分发机制在调节AIGC相关竞争利益中的作用。

Result: 研究发现：1) AIGC与HGC在创作和消费行为上存在明显差异；2) 存在普遍的"规模优于偏好"动态，即AIGC创作者通过高产量实现与HGC创作者相当的总体参与度，尽管消费者明显更偏好HGC；3) 算法内容分发机制能够调节围绕AIGC的竞争利益；4) 这些发现支持实施AIGC敏感的分发算法和精确治理框架。

Conclusion: 研究结果表明，为确保在线内容平台的长期健康发展，需要实施AIGC敏感的分发算法和精确的治理框架。算法分发机制在平衡AIGC与HGC之间的竞争关系方面起着关键作用，平台应基于对这两类内容行为特征的深入理解来设计分发策略和治理措施。
...
TL;DR: 论文提出LiteInception框架——一种面向边缘部署的轻量可解释故障诊断方法，通过两阶段级联架构、多方法融合压缩策略、知识蒸馏调节机制和双层可解释性框架，在NGAFID数据集上实现了效率、精度和可解释性的良好平衡。


<details>
  <summary>Details</summary>
Motivation: 通用航空故障诊断对飞行安全至关重要，但在资源受限的边缘设备上部署深度学习模型面临计算能力和可解释性的双重挑战。现有方法难以在计算效率和模型可解释性之间取得平衡，特别是在边缘计算场景下。

Method: 提出LiteInception框架：1）采用两阶段级联架构：阶段1进行高召回故障检测，阶段2对异常样本进行细粒度故障分类；2）基于互信息、梯度分析和SE注意力权重的多方法融合策略压缩输入传感器通道（23→15）；3）引入1+1分支LiteInception架构压缩InceptionTime参数70%；4）利用知识蒸馏作为精确率-召回率调节机制；5）构建集成四种归因方法的双层可解释性框架。

Result: 在NGAFID数据集上的实验结果表明：故障检测准确率81.92%，召回率83.24%；故障识别准确率77.00%；模型参数压缩70%，CPU推理加速超过8倍，F1损失小于3%。框架实现了效率、精度和可解释性的良好平衡。

Conclusion: LiteInception框架成功解决了边缘设备上深度学习模型的计算效率和可解释性挑战，通过创新的架构设计、压缩策略和可解释性方法，为通用航空故障诊断提供了实用解决方案，验证了框架在实际应用中的有效性。
...
TL;DR: 本文提出了AnIML本体论，这是一个基于OWL 2的本体，用于形式化Analytical Information Markup Language (AnIML)的语义，并将其与Allotrope Data Format对齐，以支持跨系统和跨实验室的互操作性。


<details>
  <summary>Details</summary>
Motivation: 异构实验数据系统之间的语义互操作性仍然是数据驱动科学发现的主要障碍。AnIML作为一种灵活的XML标准在工业研发实验室中广泛使用，但其XML模式的表达能力导致了不同利益相关者之间的解释分歧，引入了不一致性，破坏了原本设计的互操作性。

Method: 采用专家参与的方法，结合LLM辅助的需求获取与协作本体工程开发AnIML本体论。通过多层验证方法：将真实AnIML文件转换为知识图谱的数据驱动转换、通过SPARQL验证能力问题，以及基于对抗性负面能力问题的新颖验证协议，这些协议映射到已建立的本体反模式并通过SHACL约束强制执行。

Result: 成功开发了AnIML本体论，能够形式化AnIML的语义并将其与Allotrope Data Format对齐。验证结果表明本体论能够准确捕获AnIML的语义结构，并通过多层验证方法确保了本体的质量和一致性。

Conclusion: AnIML本体论为解决实验数据系统中的语义互操作性问题提供了有效的解决方案，支持跨系统和跨实验室的数据交换与集成。该本体论不仅解决了AnIML使用中的不一致性问题，还为未来更广泛的互操作性标准奠定了基础。
...
TL;DR: 论文提出了一个基于SAT的框架来解决二维单尺寸板材切割库存问题，通过物品需求展开、板材分配变量和不可行方向消除规则等方法，显著优于现有求解器。


<details>
  <summary>Details</summary>
Motivation: 解决二维单尺寸板材切割库存问题中的组合爆炸问题，该问题在需要每种物品类型的多个副本时变得特别复杂，现有方法在处理这类问题时存在局限性。

Method: 1. 将每种物品类型按需求数量展开，为每个副本分配板材分配变量
2. 仅对分配到同一板材的副本激活非重叠约束
3. 引入不可行方向消除规则：当只有一种方向能放入板材时，固定旋转变量
4. 比较三种最小化板材数量的方法：非增量SAT配合二分搜索、增量SAT跨迭代重用子句、加权部分MaxSAT

Result: 在Cui-Zhao基准测试套件上，最优SAT配置比OR-Tools、CPLEX和Gurobi能证明多2-3倍的实例达到最优，并且获得更低的优化差距。增量SAT在无旋转时表现最强，非增量SAT在旋转增加公式大小时更有效。

Conclusion: 提出的SAT框架能有效处理二维切割库存问题的组合复杂性，通过创新的约束建模和求解策略，在实际应用中显著优于商业求解器。不同SAT方法的相对性能取决于旋转约束的影响。
...
TL;DR: 提出了AeroTherm-GPT，一种专门用于高超音速热防护系统设计的LLM智能体，采用约束闭环生成框架解决多约束满足问题


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型在生成可执行仿真工件时存在级联约束违反问题，无法满足安全关键工程工作流中的序列化、多门约束要求

Method: 提出约束闭环生成（CCLG）框架，包含生成、验证、CDG引导修复、执行和审计的迭代工作流；使用约束依赖图（CDG）编码约束类别间的经验共解析结构

Result: 在HyTPS-Bench基准测试上，AeroTherm-GPT达到88.7%的端到端成功率（95%置信区间：87.5-89.9），比无CDG的基线提升12.5个百分点；根因修复效率达到4.16 vs 扁平清单修复的1.76

Conclusion: AeroTherm-GPT通过约束闭环生成框架成功解决了LLM在安全关键工程工作流中的约束满足瓶颈，显著提高了仿真工件的生成质量和效率
...
TL;DR: 本文提出了领域情境化概念图（DCG）框架，将领域信息直接编码到知识表示中，而不是作为外部注释，通过领域索引的必要性运算符在模态世界约束下解释关系。


<details>
  <summary>Details</summary>
Motivation: 知识图谱虽然能高效存储大量关系，但在处理概念含义随领域变化这一难题上表现较弱。当前系统通常将领域信息作为元数据、限定词或图级组织来处理，这些机制仅有助于过滤和溯源，但不会改变断言本身的正式状态。许多知识系统的实际故障源于将领域视为断言的外部因素。

Method: 提出领域情境化概念图（DCG）框架，将领域作为知识表示的一部分而非补充注释。在DCG形式(C, R at D, C')中，标记符at D标识关系成立的世界。形式化上，关系通过领域索引的必要性运算符解释，使真值、推理和冲突检查都在相关世界范围内进行。通过Kripke风格语义学、紧凑谓词系统、Prolog实现以及到RDF、OWL和关系数据库的映射来实现该框架。

Result: DCG框架具有三个重要结果：1) 模糊概念可在表示时进行消歧；2) 无效断言可根据其领域进行挑战；3) 跨领域关系可通过显式谓词连接。为领域本身提供了一种表示性重新解释，赋予领域在表示中的结构和可计算角色。

Conclusion: 领域应被视为知识表示的一部分而非补充注释。DCG通过将领域写入关系并将其解释为模态世界约束，解决了当前知识系统将领域视为外部因素导致的许多实际问题。这是一个关于领域本身在知识表示中角色的根本性重新解释。
...
TL;DR: PGPO（Perception-Grounded Policy Optimization）是一种新颖的细粒度信用分配框架，通过token级别的动态优势重塑，专门解决多模态推理中视觉依赖token的学习信号优化问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于可验证奖励的强化学习框架存在根本性方法缺陷：将所有生成token分配相同优势值，这稀释了多模态推理中关键视觉基础步骤的学习信号。需要区分token对视觉输入的依赖程度，实现更精准的信用分配。

Method: 1. 提出"Token Visual Dependency"概念，通过视觉条件分布与纯文本预测分布的KL散度量token对视觉输入的信息增益。2. 基于此开发PGPO框架，采用阈值门控、质量守恒机制动态重塑token级别的优势值。3. 放大视觉依赖token的学习信号，抑制语言先验带来的梯度噪声。

Result: 基于Qwen2.5-VL系列在七个具有挑战性的多模态推理基准测试上进行广泛实验，PGPO平均提升模型性能18.7%。理论分析和实证验证表明PGPO能有效降低梯度方差、防止训练崩溃，并作为强大的正则化器。

Conclusion: PGPO通过精细的token级别信用分配机制，显著提高了大型视觉语言模型的多模态推理能力，为基于强化学习的多模态模型优化提供了新范式，实现了更鲁棒、感知基础的多模态推理。
...
TL;DR: 提出AWARE任务对齐检索框架，用于提升临床电子健康记录预测中表格上下文学习的性能，解决高维度、异质性、类别不平衡和分布偏移的挑战
...
Method: 1) 建立多队列EHR基准测试，比较经典、深度表格和表格上下文学习模型；2) 提出AWARE框架：使用监督嵌入学习和轻量级适配器实现任务对齐的检索；3) 评估不同数据规模、特征维度、结果稀有性和跨队列泛化能力

Result: AWARE框架在极端不平衡情况下将AUPRC提升高达12.2%，且性能增益随数据复杂性增加而增加；研究发现检索质量和检索-推理对齐是临床预测中部署表格上下文学习的关键瓶颈

Conclusion: AWARE框架通过任务对齐的检索机制显著提升了临床预测中表格上下文学习的性能，特别是在数据异质性高和类别不平衡的情况下，为临床电子健康记录分析提供了有效的解决方案
...
TL;DR: CG-iLAO*算法通过启发式函数避免考虑昂贵动作，使用线性规划和约束生成技术，在随机最短路径问题中平均比iLAO*快2.8倍


<details>
  <summary>Details</summary>
Motivation: 传统SSP算法使用Bellman备份计算每个状态的成本函数，即使使用启发式函数，每个Bellman备份仍需要考虑所有可用动作，即使有些动作明显昂贵，这导致计算时间浪费在无帮助的动作上。

Method: 提出一种新技术，利用启发式函数避免昂贵动作，将启发式搜索重新构建为线性规划问题，并引入针对SSP的高效约束生成实现。基于此开发了CG-iLAO*算法，适配了iLAO*并采用该创新技术。

Result: CG-iLAO*在许多问题上仅考虑iLAO*动作的40%，某些情况下低至1%。平均计算的动作成本函数比iLAO*和LRTDP少3.5倍，分别快2.8倍和3.7倍。

Conclusion: 通过使用启发式函数避免考虑昂贵动作，将SSP的启发式搜索重构为线性规划问题，并采用约束生成技术，可以显著减少不必要的计算，提高算法效率，使CG-iLAO*成为解决SSP问题的更优选择。
...
TL;DR: 该论文提出了一种基于可能性监督的多分类学习方法，通过将可能性分布转化为可允许的概率分布集合，并使用Kullback-Leibler投影来训练模型。


<details>
  <summary>Details</summary>
Motivation: 在多分类问题中，传统的监督学习通常使用硬标签或概率分布作为监督信号。然而，在某些场景下，监督信息可能以可能性分布的形式出现，表达不同类别之间的相对可能性程度。如何有效地利用这种可能性监督信息进行模型训练是一个重要问题。

Method: 1. 将可能性监督转换为非空闭凸的可允许概率分布集合，包含两个约束：与可能性分布导出的概率、必要性测度兼容，以及保持可能性分布定性结构的线性形状约束。
2. 给定模型输出的概率向量，计算其在可允许集合上的Kullback-Leibler投影，得到满足所有约束的最接近概率分布。
3. 使用Dykstra算法和与负熵相关的Bregman投影来计算投影，并提供了每个约束集上的显式投影公式。
4. 通过最小化预测与其投影之间的散度来训练模型，这种散度量化了满足支配和形状约束所需的最小调整。

Result: 在合成数据和基于ChaosNLI数据集的真实世界自然语言推理任务上的实验表明，所提出的投影算法在实践中足够高效，基于投影的学习目标能够提高预测性能。

Conclusion: 该方法成功地将可能性监督信息整合到多分类学习中，通过构建可允许概率分布集合和Kullback-Leibler投影，使得模型能够有效地利用可能性分布中蕴含的定性信息，从而提升模型性能。
...
TL;DR: Qiana是一个用于在特定上下文中进行推理的逻辑框架，支持对公式和上下文进行量化，允许上下文内的次协调逻辑，基于一阶逻辑且可有限公理化，可与现有定理证明器兼容。


<details>
  <summary>Details</summary>
Motivation: 需要开发一个能够处理上下文相关真值的逻辑框架，以表达复杂的认知和语境化陈述（如"每个人都知道Alice说的一切"），同时支持处理上下文内的矛盾，并与现有一阶逻辑工具兼容。

Method: 基于一阶逻辑构建，支持对公式和上下文进行量化，允许在上下文中使用次协调逻辑以容纳矛盾，设计为可有限公理化，确保与现有一阶逻辑定理证明器的兼容性。

Result: 成功开发出Qiana框架，能够表示时间性、事件演算和模态逻辑等复杂概念，展示了如何处理上下文相关的推理和矛盾，并讨论了不同的设计替代方案。

Conclusion: Qiana为上下文相关的逻辑推理提供了一个强大而实用的框架，它结合了一阶逻辑的基础性和灵活性，支持丰富的表达能力，同时保持与现有推理工具的兼容性，为处理复杂语境化知识表示问题提供了有效工具。
...
TL;DR: 研究人员创建了NeuroPose-AHM数据集，这是一个包含2,756条患者组级记录的知识型数据库，涵盖57种神经疾病引发的异常头部运动，并开发了一个四任务分析框架来展示其分析价值。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏一个整合运动学测量、临床严重程度评分和患者人口统计学的多条件资源，这阻碍了基于AI的诊断工具的发展。需要创建一个综合性的知识库来支持异常头部运动的神经学研究。

Method: 使用多LLM提取框架分析1,430篇同行评审文献，构建NeuroPose-AHM数据集。对颈椎性肌张力障碍（CD）应用四任务分析框架：1)多标签AHM类型分类；2)构建头颈严重程度指数（HNSI）统一指标；3)用真实患者数据验证HNSI；4)分析运动类型概率与HNSI的相关性。

Result: 数据集包含2,756条患者组级记录，涵盖57种神经疾病。LLM提取具有强一致性（kappa=0.822）。AHM类型分类F1得分0.856。HNSI与真实CD患者数据验证显示严重带比例匹配（6.7%）。运动类型概率与HNSI显著相关（p<0.001）。

Conclusion: NeuroPose-AHM是一个结构化、基于知识的资源，具有分析实用性，可用于神经性AHM研究，填补了资源空白。数据集已在Zenodo上公开提供。
...
TL;DR: SenseMath基准测试显示，虽然大语言模型在显式提示下能有效应用数值计算捷径策略，但自发使用率低（<40%），且缺乏对捷径适用条件的结构性理解，无法正确判断何时适用或生成有效的捷径问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索大语言模型是否具备类似人类的"数感"能力，即是否能识别数值结构、在适当时应用捷径策略、在不适当时避免使用。当前LLMs往往默认使用逐步计算，即使存在更高效的数值捷径，这引发了对其是否真正具备结构性数值推理能力的基本疑问。

Method: 研究者引入SenseMath基准测试，包含4,800个项目，涵盖8种捷径类别和4种数字规模，每个项目都有强捷径、弱捷径和控制变量三种变体。支持三个逐级递增的认知需求评估设置：1) 捷径使用（模型能否在适合的问题上应用捷径）；2) 适用性判断（能否识别捷径何时适用或会误导）；3) 问题生成（能否生成正确允许特定捷径类型的新问题）。评估了从GPT-4o-mini到Llama-3.1-8B的五个LLMs。

Result: 结果显示一致性模式：显式提示下，模型能采用捷径策略，在适合捷径的项目上准确率提升高达15%；但标准思维链提示下，自发使用率低于40%（即使具备相应能力）。这种能力局限于"使用"层面：模型系统性地将捷径过度推广到不适用的场景，且无法从头生成有效的捷径承载问题。

Conclusion: 当前LLMs表现出程序性捷径流利度，但缺乏支撑人类数感的结构性理解（理解捷径何时为何有效）。它们能执行捷径策略，但缺乏对数值结构的深刻理解，无法可靠判断捷径的适用性或创造性应用。
...
TL;DR: 提出一种基于Transformer掩码自编码器的无标签关节级步态异常检测与运动学矫正框架，仅使用正常步态序列训练，能定位生物力学不一致关节并生成矫正后的运动轨迹。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习步态分析方法依赖于疾病标签的监督学习，难以泛化到异质性病理表现。需要一种无需疾病标签、能定位个体异常并矫正运动学数据的框架。

Method: 1) 使用仅包含150名成人正常步态序列的无标记多摄像头运动捕捉数据进行训练；2) 基于Transformer掩码自编码器学习正常步态先验；3) 推理时采用两阶段处理：首先通过遮蔽单个关节估计关节不一致性评分，然后从剩余时空上下文中重建完整骨架，生成矫正后的运动轨迹。

Result: 验证实验表明：1) 能准确定位生物力学不一致关节；2) 所有分析关节的角度偏差显著减小且效应量大；3) 能保持正常运动学特征；4) 在10名受试者模拟的7种步态异常中均表现良好。

Conclusion: 该方法实现了无需疾病标签、可解释的个体特异性步态障碍定位，为临床步态分析提供了无监督异常检测与运动学矫正的新范式。
...
TL;DR: ProCeedRL 是一种面向多轮智能体任务的强化学习方法，通过过程级批评器和反思性示范引导，解决传统探索策略在处理长时序交互和随机环境反馈时的失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习方法在应用于多轮智能体任务时面临挑战，主要问题在于长时序交互特性和环境反馈的随机性。作者识别出一个结构性失败模式：次优行动引发噪声观察进入误导性上下文，从而进一步削弱后续决策，形成难以恢复的错误累积反馈循环。

Method: 提出了 ProCeedRL（Process Critic with Explorative Demonstration RL）方法：1）使用过程级批评器实时监控交互过程；2）结合基于反思的示范来引导智能体停止错误累积；3）将探索从被动选择转变为主动干预；4）通过从探索性示范和在线策略样本中学习来提升探索效率。

Result: 该方法显著超越了模型饱和的探索性能，展示了实质性的探索优势。在复杂的深度搜索和具身任务上实现了卓越性能，显著提高了探索效率。

Conclusion: ProCeedRL 通过过程级批评和反思示范有效解决了智能体探索中的错误累积问题，为多轮智能体任务中的强化学习应用提供了一种有效解决方案，能够提升探索效率并在复杂任务中取得优越性能。
...
TL;DR: ATBench是一个用于评估基于LLM的智能体安全性的轨迹级基准测试，通过三维风险分类法、异构工具池和长上下文延迟触发协议，实现结构化、多样化和现实化的安全性评估。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹级基准测试存在局限性：交互多样性不足、安全性失效观察粗粒度、长期视野现实性弱。现实部署中风险通常在多步骤交互中涌现，而非孤立提示或最终响应。

Method: 1. 构建三维风险分类法：风险源、失效模式、现实世界危害；2. 使用异构工具池构建轨迹；3. 采用长上下文延迟触发协议捕捉多阶段风险涌现；4. 包含1000条轨迹（503安全，497不安全），平均9.01轮对话和3.95k tokens；5. 基于规则和LLM的过滤加完整人工审核确保数据质量。

Result: 实验表明：1. ATBench对前沿LLM、开源模型和专用防护系统都具有挑战性；2. 支持基于分类法的分层分析；3. 支持跨基准比较；4. 支持诊断长期视野失效模式。

Conclusion: ATBench提供了一个结构化、多样化和现实化的轨迹级基准测试，能够更好地评估LLM智能体在多步骤交互中的安全性风险，支持更全面的安全分析和诊断。
...
TL;DR: 这篇调查性论文系统地综述了基于语言模型中的潜在空间技术，将其作为一个新兴的计算范式，涵盖了从基础概念到最新发展的完整图景，并讨论了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型的发展，显式标记生成的传统理解方式已显局限。越来越多的研究表明，许多关键内部过程更自然地发生在连续潜在空间中，而非人类可读的言语轨迹中。这一转变由显式空间计算的结构性限制驱动，包括语言冗余、离散化瓶颈、序列低效率和语义损失。

Method: 采用调查性研究方法，构建了一个统一的五维框架：基础、演化、机制、能力和展望。从机制视角分析了四大发展主线：架构、表示、计算和优化；从能力视角展示了潜在空间支持的广泛能力谱系。

Result: 提供了基于语言模型中潜在空间的全面、统一且最新的研究景观，系统整理了现有工作，识别了关键发展路径和技术方向，展示了潜在空间如何支持推理、规划、建模、感知、记忆、协作和具身等多种能力。

Conclusion: 潜在空间正在成为基于语言模型的天然计算基底，代表了下一代智能的一般计算和系统范式。这篇综述不仅可作为现有工作的参考，也为理解潜在空间作为新兴智能范式奠定了基础，同时指出了开放挑战和未来研究方向。
...
TL;DR: ARQuest框架使用大型语言模型和替代数据源创建个性化自适应问卷，改进保险申请流程。实验显示，与传统问卷相比，GPT模型驱动的自适应问卷需要更少问题且用户体验更好。


<details>
  <summary>Details</summary>
Motivation: 传统保险申请问卷冗长、标准化，难以捕捉个体差异，且保险公司只能盲目信任用户回答，增加了欺诈风险。

Method: ARQuest框架结合大型语言模型和替代数据源（社交媒体图像分析、地理数据分类、检索增强生成）来提取用户洞察，生成个性化自适应问卷。

Result: 实验测试显示：传统问卷在风险评估准确性上略高，但GPT模型驱动的自适应问卷需要更少问题，用户体验更佳（更流畅、更具吸引力）。

Conclusion: ARQuest在提升用户满意度和简化保险流程方面具有巨大潜力。进一步发展后，该方法可能在风险准确性方面超越传统方法，推动保险行业创新。
...
TL;DR: Diff-KD框架通过将扩散生成式精炼与师生知识蒸馏相结合，解决了多智能体协同感知中传感器和通信损坏的问题，实现了最先进的检测精度和校准鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的传感器和通信损坏严重削弱了多智能体协同感知的优势。现有方法将损坏视为静态扰动或被动适应损坏输入，无法主动恢复潜在的干净语义，这一局限性需要解决。

Method: 提出了Diff-KD框架，包含两个核心组件：1)渐进知识蒸馏(PKD)，将局部特征恢复视为条件扩散过程，从损坏观测中恢复全局语义；2)自适应门控融合(AGF)，在融合过程中基于自我可靠性动态加权邻居信息。

Result: 在OPV2V和DAIR-V2X数据集上对七种损坏类型进行评估，Diff-KD在检测精度和校准鲁棒性方面均实现了最先进的性能。

Conclusion: Diff-KD框架通过主动恢复干净语义来解决多智能体协同感知中的损坏问题，将扩散生成式精炼与知识蒸馏相结合，为鲁棒协同感知提供了有效解决方案。
...
TL;DR: 该论文研究了在没有参考解释的情况下，使用大语言模型作为时间序列解释的生成器和评估器，构建了一个包含350个案例的合成基准测试，发现评估任务比生成任务表现更稳定。


<details>
  <summary>Details</summary>
Motivation: 评估LLM生成的、基于时间序列数据的自然语言解释的事实正确性是一个未解决的挑战。现有评估方法存在局限：基于参考的相似度度量需要真实解释，一致性检查模型也需要基准解释，而传统时间序列方法只处理数值无法评估自由格式文本推理。因此，需要一种无需预定义参考或特定任务规则的通用方法来直接验证解释是否忠实于基础时间序列数据。

Method: 1) 将大语言模型作为时间序列解释的生成器和评估器；2) 在无参考设置下，给定时间序列、问题和候选解释，评估器基于模式识别、数值准确性和答案忠实度分配三元正确性标签；3) 构建包含350个时间序列案例的合成基准，涵盖七种查询类型，每种配对正确、部分正确和错误解释；4) 评估模型在四个任务上的表现：解释生成、相对排名、独立评分和多异常检测。

Result: 结果显示明显的不对称性：生成任务高度依赖模式且在某些查询类型上存在系统性失败，如Seasonal Drop和Volatility Shift的准确率仅为0.00-0.12，而Structural Break为0.94-0.96；评估任务则更稳定，即使模型自身输出错误，也能正确对解释进行排名和评分。

Conclusion: 这些发现证明了基于数据接地的LLM评估时间序列解释的可行性，并突显了它们在时间序列领域中作为数据接地推理可靠评估器的潜力。
...
TL;DR: TRACE-Bot：用于检测LLM驱动社交机器人的统一双通道框架，通过联合建模隐式语义表示和AIGC增强的行为模式，在两个公共数据集上分别达到98.46%和97.50%的准确率。


<details>
  <summary>Details</summary>
Motivation: LLM驱动的社交机器人生成类似人类的内容，能逃避传统检测方法。现有方法存在三个主要问题：过度依赖单模态信号、对AIGC特定生成模式不够敏感、未能充分建模语言模式与行为动态的相互作用。

Method: 提出TRACE-Bot框架，构建来自个人信息数据、交互行为数据和推文数据的细粒度表示。采用双通道架构：一个通道通过预训练语言模型捕捉语言表示，另一个通道通过多维活动特征增强SOTA AIGC检测器信号捕捉行为异常。融合后的表示通过轻量级预测头进行分类。

Result: 在两个公共LLM驱动的社交机器人数据集上实现SOTA性能，准确率分别为98.46%和97.50%。结果表明对先进机器人策略具有强鲁棒性。

Conclusion: 联合利用隐式语义表示和AIGC增强的行为模式对新兴LLM驱动的社交机器人检测非常有效，TRACE-Bot框架为解决现有方法的局限性提供了有效解决方案。
...
TL;DR: 评估LLM生成胸部CT报告日语翻译的教育适用性，发现LLM作为评判者与放射科医师评估存在显著差异，自动化LLM评估不足以替代专家评审


<details>
  <summary>Details</summary>
Motivation: 准确翻译放射学报告对于多语言研究、临床沟通和放射学教育很重要，但LLM评估的有效性尚不明确。需要评估LLM生成翻译的教育适用性，并比较放射科医师评估与LLM作为评判者的差异

Method: 使用CT-RATE-JPN验证集中的150份胸部CT报告。每份英文报告分别有人工编辑的日语翻译和DeepSeek-V3.2生成的LLM翻译。由一名认证放射科医师和一名放射科住院医师独立进行盲法配对评估，评估术语准确性、可读性、整体质量和放射科医师风格真实性。同时，3个LLM评判者（DeepSeek-V3.2、Mistral Large 3和GPT-5）评估相同配对。使用QWK和百分比一致性评估一致性

Result: 放射科医师与LLM评判者之间一致性接近零（QWK=-0.04至0.15）。两名放射科医师之间一致性也较差（QWK=0.01至0.06）。放射科医师1认为59%案例中术语等效，在可读性（51%）和整体质量（51%）上更倾向于LLM翻译。放射科医师2认为75%案例中可读性等效，在整体质量上更倾向于人工编辑翻译（40%对21%）。所有3个LLM评判者在所有标准上都强烈倾向于LLM翻译（70%-99%），并在>93%案例中评为更像放射科医师风格

Conclusion: LLM生成翻译常被判断为自然流畅，但两名放射科医师评估存在显著差异。LLM作为评判者对LLM输出表现出强烈偏好，与放射科医师评估几乎没有一致性。对于放射学报告翻译的教育应用，仅依赖自动化LLM评估是不够的；专家放射科医师评审仍然至关重要
...
TL;DR: 提出一种模型无关的token重要性可视化技术，用于理解生成式AI系统如何处理输入文本信息，避免额外的计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有注意力可视化技术多针对特定Transformer架构，需要反向传播导致GPU内存使用翻倍和计算成本增加。缺乏轻量级、模型无关的注意力可视化方法。

Method: 基于扰动策略结合三矩阵分析框架：1）角度偏差矩阵捕获语义方向变化；2）幅度偏差矩阵测量语义强度变化；3）维度重要性矩阵评估各向量维度贡献。通过系统移除每个token并测量这三方面影响，得出复合重要性评分。

Result: 开发了开源实现，代码和资源已在Infosys Responsible AI Toolkit中公开提供，支持可重复性和广泛应用。

Conclusion: 提出了一种无需额外计算成本的模型无关token重要性可视化方法，通过多维度分析提供了数学基础扎实的token重要性度量，有助于更好理解生成式AI系统的信息处理机制。
...
TL;DR: 本文提出了一种系统化工程方法，用于验证AI/ML系统在安全关键领域（如航空）中的操作设计域（ODD）覆盖完整性，以满足EASA认证标准。


<details>
  <summary>Details</summary>
Motivation: 当前在安全关键领域部署AI系统面临EASA认证标准的挑战，特别是需要证明AI/ML组件的操作设计域（ODD）完全覆盖，避免关键空白。现有的方法在处理高维参数空间时缺乏可扩展性和形式化基础，且没有标准化的工程方法来弥合抽象ODD定义与可验证证据之间的差距。

Method: 提出了一种结构化多步骤ODD覆盖验证方法，整合了参数离散化、基于约束的过滤和基于关键性的维度约简。该方法基于先前AI空中防撞研究的模拟数据，采用系统化工程方法定义和实现覆盖指标。

Result: 该方法能够在更高维度上验证ODD覆盖，展示了一种满足EASA完整性要求的系统化工程途径。它促进了"安全设计"方法，同时符合EASA标准。

Conclusion: 该研究填补了AI系统在安全关键领域认证中的标准化工程方法空白，提供了一种可扩展、形式化基础的ODD覆盖验证方法，有助于满足航空等领域的严格安全认证要求。
...
TL;DR: De Jure是一个全自动、领域无关的管道，用于从原始文档中提取结构化监管规则，无需人工标注或特定领域提示，通过四阶段流程（规范化、语义分解、多维度评估和迭代修复）实现高质量规则提取。


<details>
  <summary>Details</summary>
Motivation: 监管文件包含LLM系统必须遵守的法律义务，但将密集、层次化的法律文本转换为机器可读规则仍然是一个昂贵、专家密集的过程。需要自动化解决方案来降低提取成本并提高可扩展性。

Method: De Jure采用四阶段流水线：1) 将源文档规范化为结构化Markdown；2) LLM驱动的语义分解为结构化规则单元；3) 19个维度的多标准LLM-as-a-judge评估（涵盖元数据、定义和规则语义）；4) 在有限再生预算内对低分提取进行迭代修复，优先修复上游组件。

Result: 在金融、医疗和AI治理三个监管语料库上评估显示：金融领域提取质量持续单调提升，三个评估迭代内达到峰值；在医疗和AI治理领域保持高性能，跨开源和闭源模型有效泛化；在RAG合规问答评估中，基于De Jure提取规则的响应在73.8%（单规则检索）和84.0%（宽检索）情况下优于先前工作。

Conclusion: 明确、可解释的评估标准可以替代复杂监管领域的人工标注，为基于监管的LLM对齐提供可扩展且可审计的路径，证明提取保真度直接转化为下游效用。
...
TL;DR: 提出一种名为Trace Inversion的新型弃答方法，通过比较原始查询与模型推理轨迹重构的查询之间的相似度来判断是否弃答，显著提升了推理模型的弃答性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型可靠部署需要有效的弃答能力，但推理模型在这方面表现较差。研究发现幻觉导致的弃答失败可以重新解释为模型回答了错误的问题，而不是错误地回答问题。

Method: 提出查询失配框架和Trace Inversion方法：1)生成模型的推理轨迹；2)仅基于轨迹重构模型最可能回应的查询；3)比较初始查询与重构查询，低相似度得分表明模型可能回答错误，从而标记弃答。

Result: 在四种前沿大型语言模型和九个弃答问答数据集的广泛实验中，Trace Inversion在36个设置中的33个中击败了竞争基线方法，有效提升了弃答性能。

Conclusion: Trace Inversion方法为推理模型提供了一种有效的弃答策略，通过将幻觉问题重新定义为查询失配，实现了在多种模型和数据集上的最先进弃答性能。
...
TL;DR: 研究情感语气对大型语言模型性能的影响，发现静态情感前缀对准确率影响有限，但自适应情感选择框架EmotionRL能带来更可靠的性能提升


<details>
  <summary>Details</summary>
Motivation: 情感语气在人类交流中普遍存在，但其对大型语言模型行为的影响尚不清楚，需要系统研究情感框架如何影响LLM在不同基准任务中的表现

Method: 1) 在数学推理、医学问答、阅读理解、常识推理和社会推理等六个基准领域，研究第一人称情感框架对LLM性能的影响；2) 引入EmotionRL自适应情感提示框架，根据查询动态选择情感框架；3) 比较静态情感前缀与自适应方法的差异

Result: 1) 静态情感前缀通常只产生小的准确率变化，表明情感措辞是轻度扰动而非可靠干预；2) 效应在社会基础任务中更具变异性；3) 更强的情绪措辞仅引起适度额外变化；4) 自适应情感选择比固定情感提示产生更可靠的性能增益

Conclusion: 情感语气既不是LLM性能的主导驱动力，也不是无关噪声，而是一种弱且输入依赖的信号，可以通过自适应控制来利用
...
TL;DR: 本文提出了一种自适应预算遗忘框架，通过相关性引导评分和有界优化来调节内存，以解决长对话智能体中内存积累导致的性能退化问题。


<details>
  <summary>Details</summary>
Motivation: 长对话智能体需要持久性内存进行连贯推理，但无控制的记忆积累会导致时间衰减和虚假记忆传播。现有基准测试显示性能显著下降（LOCOMO/LOCCO从0.455降至0.05，MultiWOZ在持久保留下准确率为78.2%，虚假记忆率为6.8%）。

Method: 提出自适应预算遗忘框架，通过相关性引导评分和有界优化来调节内存。该方法整合了新近度、频率和语义对齐三个维度，在受限上下文中维持稳定性。

Result: 对比分析显示，长对话F1分数提高至超过基线水平0.583，保持了更高的保留一致性，减少了虚假记忆行为，且未增加上下文使用量。

Conclusion: 结构化遗忘在扩展对话场景中能够保持推理性能，同时防止无限制的内存增长，验证了该框架的有效性。
